#!/usr/bin/env python3
"""Freeze the 5.5e-5 V42I matched-initialization SFT HPO arm."""
...
import run_sft_equal_unit_matched_init_v42i as launcher
...
PREREGISTRATION = RUN_DIR / "preregistration_v42i.json"
...
def arguments():
    ...
    args.stdout_log = str(RUN_DIR / "stdout_v42i.log")
    args.gpu_log = str(RUN_DIR / "gpu_activity_v42i.jsonl")
    args.report = str(RUN_DIR / "runtime_report_v42i.json")
    args.attempt_report = str(RUN_DIR / "attempt_v42i.json")
...
def build() -> dict:
    ...
    result.update({
        ...
        "experiment_name": (
            "sft_matched_init_equal_unit_v42i_fold3_v412_"
            "middle_late_r32_seed17_init20260715041_lr5p5e5"
        ),
        "artifacts": {
            "output_dir": str(OUTPUT_DIR),
            "stdout_log": str(RUN_DIR / "stdout_v42i.log"),
            "gpu_log": str(RUN_DIR / "gpu_activity_v42i.jsonl"),
            "report": str(RUN_DIR / "runtime_report_v42i.json"),
            "attempt_report": str(RUN_DIR / "attempt_v42i.json"),
        },
    })
    ...
    result["implementation"]["hpo_launcher_v42i"] = {
        "path": str((ROOT / "run_sft_equal_unit_matched_init_v42i.py").resolve()),
        "sha256": v42b.engine.file_sha256(
            ROOT / "run_sft_equal_unit_matched_init_v42i.py"
        ),
    }
    result["selection_firewall"]["this_arm_authorizes"] = (
        "train-only 5.5e-5 HPO state and runtime evidence only"
    )
...
def main() -> None:
    ...
    if PREREGISTRATION.exists():
        raise RuntimeError("V42I preregistration already exists")
