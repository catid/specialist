#!/usr/bin/env python3
"""Fail-closed four-GPU train-only runtime for immutable V30A."""
...
import train_eggroll_es_paired_data_compat_v30a as mechanics_v30a


ROOT = Path(__file__).resolve().parent
PREREG_PATH_V30A = prereg_v30a.PREREGISTRATION_PATH
PREREG_FILE_SHA256_V30A = mechanics_v30a.PREREGISTRATION_FILE_SHA256_V30A
PREREG_CONTENT_SHA256_V30A = mechanics_v30a.PREREGISTRATION_CONTENT_SHA256_V30A
EXPERIMENT_NAME_V30A = (
    "s6_v30a_paired_production_v389_train_only_runtime_basis20261003"
)
OUTPUT_DIRECTORY_V30A = (ROOT / "experiments/eggroll_es_hpo/runs").resolve()
ATTEMPT_NAME_V30A = f".{EXPERIMENT_NAME_V30A}.launch_attempt.json"
REPORT_NAME_V30A = "paired_production_v389_compatibility_v30a.json"
WORKER_EXTENSION_V30A = (
    "eggroll_es_worker_v23a_retry_r1."
    "InsertionLocationAuditWorkerExtensionV23ARetryR1"
)
TEST_PATH_V30A = (
    ROOT / "test_run_eggroll_es_paired_data_compat_v30a.py"
).resolve()
MECHANICS_TEST_PATH_V30A = (
    ROOT / "test_train_eggroll_es_paired_data_compat_v30a.py"
).resolve()
BF16_CONFIG_SHA256_V30A = (
...
BOUND_FILES_V30A = {
    "candidate_materializer_v30a": (
        ...
        "8804f4c73eed26d70e149510781c4231c7f7965ec72bd1ddc47a51ee3e3a9dda",
    ),
    "candidate_materializer_tests_v30a": (
        ROOT / "test_build_eggroll_es_v389_train_only_candidate_v30a.py",
        "e29bb0eec1feca891a6b11f03c426a145e98de727a26d6b779ee183d90cdd9e2",
    ),
    "candidate_v389": (
        prereg_v30a.frame_v30a.CANDIDATE_PATH,
        prereg_v30a.frame_v30a.CANDIDATE_SHA256,
    ),
    "candidate_manifest_v389": (
        prereg_v30a.frame_v30a.CANDIDATE_MANIFEST_PATH,
        prereg_v30a.frame_v30a.CANDIDATE_MANIFEST_SHA256,
    ),
    "frame_builder_v30a": (
        ROOT / "build_eggroll_es_joint_panels_v30a.py",
        prereg_v30a.FRAME_BUILDER_SHA256,
    ),
    "frame_tests_v30a": (
        ROOT / "test_build_eggroll_es_joint_panels_v30a.py",
        prereg_v30a.FRAME_TEST_SHA256,
    ),
    "frame_manifest_v30a": (
        prereg_v30a.FRAME_PATH,
        prereg_v30a.FRAME_FILE_SHA256,
    ),
    "prereg_module_v30a": (
        Path(prereg_v30a.__file__).resolve(),
        "7054b538de9c5cec03fc5b5833010aacb71a68f9cf3604c0ef467abd966e16b4",
    ),
    "prereg_tests_v30a": (
        ROOT / "test_eggroll_es_paired_data_compat_preregistration_v30a.py",
        "3061d4c61a2dc81ac947f2280c314dbccd976b6dd54db5445e7e737399105174",
    ),
    "preregistration_v30a": (
        PREREG_PATH_V30A,
        PREREG_FILE_SHA256_V30A,
    ),
}
IMPLEMENTATION_PATHS_V30A = {
    **{key: path for key, (path, _digest) in BOUND_FILES_V30A.items()},
    ...
    "worker_v30a": Path(worker_r1.__file__).resolve(),
    "mechanics_v30a": Path(mechanics_v30a.__file__).resolve(),
    "mechanics_tests_v30a": MECHANICS_TEST_PATH_V30A,
    "runtime_v30a": Path(__file__).resolve(),
    "runtime_tests_v30a": TEST_PATH_V30A,
}
FORBIDDEN_ARGV_TOKENS_V30A = (
    "checkpoint", "update", "heldout", "validation", "ood", "benchmark",
    "eval", "save-model", "train-dataset", "promotion", "union",
)
FORBIDDEN_PERSISTED_KEYS_V30A = {
...
file_sha256 = prereg_v30a.file_sha256
...
def _assert_compact(value):
    overlap = FORBIDDEN_PERSISTED_KEYS_V30A & set(_recursive_keys(value))
    if overlap:
        raise RuntimeError(f"v30a compact output contains forbidden keys: {sorted(overlap)}")


def _assert_train_only_argv(argv):
    for token in argv:
        lowered = str(token).lower()
        if any(value in lowered for value in FORBIDDEN_ARGV_TOKENS_V30A):
            raise ValueError(f"v30a rejects forbidden runtime surface: {token}")


def _verify_bound_files_v30a():
    identities = {}
    for key, (path, expected) in BOUND_FILES_V30A.items():
        path = Path(path).resolve()
        relative = path.relative_to(ROOT).as_posix()
        if file_sha256(path) != expected:
            raise RuntimeError("v30a immutable train-only input changed")
        identities[key] = {
...
def _certify_real_launch_committed_source_v30a():
    relative_paths = sorted({
        ...
        for path in IMPLEMENTATION_PATHS_V30A.values()
...
def load_preregistration_v30a():
    _verify_bound_files_v30a()
    value = json.loads(PREREG_PATH_V30A.read_text(encoding="utf-8"))
    if (
        file_sha256(PREREG_PATH_V30A) != PREREG_FILE_SHA256_V30A
        or value.get("content_sha256_before_self_field")
        != PREREG_CONTENT_SHA256_V30A
        or value.get("content_sha256_before_self_field")
        ...
        raise RuntimeError("v30a immutable preregistration changed")
    prereg_v30a.validate_sha_fields(value)
    return value


def implementation_identity_v30a():
    inherited = runtime_r2.implementation_identity_r2()
    bound_files = _verify_bound_files_v30a()
    files = dict(inherited["files"])
    for key, path in IMPLEMENTATION_PATHS_V30A.items():
        files[key] = {
    ...
    return {
        ...
        "v30a_overlay_bundle_sha256": canonical_sha256({
            key: files[key] for key in IMPLEMENTATION_PATHS_V30A
        }),
        "bundle_sha256": canonical_sha256(files),
    }


def seed_projection_contract_v30a(preregistration):
    ...
    if (
        seeds != prereg_v30a.perturbation_seeds()
        or len(seeds) != 32
        or len(set(seeds)) != 32
        or len({item["numpy_legacy_seed"] for item in projections}) != 32
        or any(item["numpy_legacy_seed"] == 0 for item in projections)
    ):
        raise RuntimeError("v30a perturbation seed projection changed")
    return {
        "schema": "eggroll-es-v30a-seed-projection-contract",
...
def load_layer_bundle_v30a(preregistration):
    ...
    return anchor_v4.load_frozen_layer_plan_v4(
        ...
        expected_model_config_sha256=BF16_CONFIG_SHA256_V30A,
    )


def validate_live_model_v30a(preregistration):
    model = Path(preregistration["frozen_recipe"]["model"]).resolve()
    if (
        file_sha256(model / "config.json") != BF16_CONFIG_SHA256_V30A
        or file_sha256(model / "model.safetensors.index.json")
        != BF16_INDEX_SHA256_V30A
    ):
        raise RuntimeError("v30a live BF16 model identity changed")
    return _seal({
        "schema": "eggroll-es-v30a-live-model-audit",
...
def recipe_v30a(
    ...
    mechanics_v30a.validate_paired_panel_bundle_v30a(panel_bundle)
    schedule = mechanics_v30a.resident_signed_wave_schedule_v30a()
    panels = {}
    for name in mechanics_v30a.PANEL_NAMES_V30A:
    ...
    value = {
        "schema": "eggroll-es-paired-data-compat-runtime-recipe-v30a",
        "experiment_name": EXPERIMENT_NAME_V30A,
        "preregistration": {
            "path": str(PREREG_PATH_V30A),
            "file_sha256": PREREG_FILE_SHA256_V30A,
            "content_sha256": PREREG_CONTENT_SHA256_V30A,
        ...
        "perturbation": {
            ...
            "alpha": 0.0,
            "population_size": 32,
            "basis_seed": prereg_v30a.PERTURBATION_BASIS_SEED,
            "direction_seed_list_sha256": canonical_sha256(
                prereg_v30a.perturbation_seeds()
            ),
        ...
        "seed_projection": seed_projection_contract_v30a(preregistration),
        "scoring": {
        ...
        "matched_full_context_guard": {
            "phase_order": [
                ...
                "all_16_signed_waves", "full_population_boundary_audit",
        ...
        "endpoints": list(prereg_v30a.ENDPOINTS),
        ...
        "fresh_exclusive_paths": {
            "attempt_name": ATTEMPT_NAME_V30A,
            "run_directory_name": EXPERIMENT_NAME_V30A,
            "report_name": REPORT_NAME_V30A,
        },
        "authority": {
            ...
            "pass_authorizes_only_separate_full_v389_train_only_hpo_preregistration": True,
        },
    }
    return _seal(value)


def validate_runtime_v30a(args, preregistration, implementation, recipe):
    load_preregistration_v30a()
    if preregistration.get("content_sha256_before_self_field") != PREREG_CONTENT_SHA256_V30A:
        raise ValueError("v30a preregistration changed")
    if any(
        os.environ.get(key) for key in runtime_v23a.MOE_OVERRIDE_ENVIRONMENT_V23A
    ):
        raise ValueError("v30a rejects external MoE backend overrides")
    if os.environ.get("VLLM_BATCH_INVARIANT") not in (None, "0"):
        raise ValueError("v30a rejects the vLLM batch-invariant backend swap")
    if recipe.get("content_sha256_before_self_field") != canonical_sha256(
        _without_self(recipe)
    ):
        raise ValueError("v30a runtime recipe changed")
    for expected, actual, label in (
        ...
        if not args.v30a_dry_run and expected is None:
            raise ValueError(f"v30a real launch requires expected {label} hash")
...
def _score_panel_outputs(dense_items, outputs):
    ...
    if rewards.shape != (39,) or not np.isfinite(rewards).all():
        raise RuntimeError("v30a per-unit dense score coverage changed")
...
def _phase_identity(phase):
    ...
    for version in mechanics_v30a.VERSIONS_V30A:
        ...
        if values.shape != (4, 5, 39) or not np.isfinite(values).all():
            raise RuntimeError("v30a full-context score geometry changed")
        identities[version] = {
            "shape": [4, 5, 39],
...
def _validate_exact_references_v30a(references, selected):
    """Validate the v4 reference schema; rank mapping is certified separately."""
    if len(references) != 4 or len(selected) != 4:
        raise RuntimeError("v30a exact reference coverage changed")
    if any(
        ...
        raise RuntimeError("v30a exact reference certificate changed")
    if (
        ...
        raise RuntimeError("v30a selected reference identity differs across engines")
    return canonical_sha256(selected[0]["reference"])


class PairedDataCompatRuntimeMixinV30A:
    def configure_paired_data_compat_v30a(
        self, preregistration, panel_bundle, layer_bundle,
    ):
        panel_bundle = mechanics_v30a.validate_paired_panel_bundle_v30a(
        ...
        if (
            ...
            raise RuntimeError("v30a exact four-engine alpha-zero recipe changed")
        projection = seed_projection_contract_v30a(preregistration)
        seeds = preregistration["frozen_recipe"]["perturbation_basis"][
            "direction_seeds"
        ]
        seed_reports = [
        ...
        if (
            ...
            or any(
                ...
                != projection["full_to_numpy_projection_sha256"]
                or report.get("numpy_projection_unique_count") != 32
            ...
            raise RuntimeError("v30a worker seed certificates changed")
        devices = [
        ...
        for rank, report in enumerate(devices):
            if (
                ...
                raise RuntimeError("v30a fixed rank-to-GPU mapping changed")
        installs = [
        ...
        if (
            ...
            raise RuntimeError("v30a layer-plan installation changed")
        references = [
        ...
        selected_reference_identity_sha256 = _validate_exact_references_v30a(
            references, selected,
        )
        self._v30a_preregistration = copy.deepcopy(preregistration)
        self._v30a_panel_bundle = panel_bundle
        self._v30a_references = [item["identity"] for item in references]
        return _seal({
            "schema": "eggroll-es-paired-runtime-configuration-v30a",
    ...
    def _prepared_fixed_batches_v30a(self):
        bundle = mechanics_v30a.validate_paired_panel_bundle_v30a(
            self._v30a_panel_bundle
        )
        prepared = {}
        request_identity = {}
        token_audit = {}
        for version in mechanics_v30a.VERSIONS_V30A:
            ...
            cursor = 0
            token_panels = {}
            for panel_name in mechanics_v30a.PANEL_NAMES_V30A:
            ...
            if cursor != 195 or any(
                ...
                raise RuntimeError("v30a fixed version request/token count changed")
        ...
        self._v30a_fixed_request_identity = request_identity
        self._v30a_token_audit_sha256 = canonical_sha256(token_audit)
        return prepared, request_identity, token_audit

    def _assert_fixed_request_v30a(self, version, prepared):
        if (
            version not in mechanics_v30a.VERSIONS_V30A
            or _request_identity(prepared[version]["prompt_items"])
            != self._v30a_fixed_request_identity[version]
        ):
            raise RuntimeError("v30a fixed version request identity changed")

    def _generate_version_all_engines_v30a(self, version, prepared):
        self._assert_fixed_request_v30a(version, prepared)
        ...
        if len(batches) != 4 or any(len(batch) != 195 for batch in batches):
            raise RuntimeError("v30a all-engine version generation incomplete")
        return batches

    def _score_version_batches_v30a(self, version, prepared, batches):
        scores = np.empty((4, 5, 39), dtype=np.float64)
        commitments = []
        for engine_index, batch in enumerate(batches):
            for panel_index, panel_name in enumerate(mechanics_v30a.PANEL_NAMES_V30A):
    ...
    def _full_context_phase_v30a(self, prepared):
        scores = {}
        commitments = {}
        for version in mechanics_v30a.VERSIONS_V30A:
            batches = self._generate_version_all_engines_v30a(version, prepared)
            scores[version], commitments[version] = self._score_version_batches_v30a(
                version, prepared, batches
            )
        return scores, commitments

    def _perturb_signed_wave_v30a(self, seeds, negate):
        if len(seeds) != 4:
            raise RuntimeError("v30a partial perturbation wave forbidden")
        raw = self._resolve([
            engine.collective_rpc.remote(
    ...
    def _score_resident_version_v30a(
        self, version, prepared, schedule_item, unit_scores, commitments,
    ):
        batches = self._generate_version_all_engines_v30a(version, prepared)
        scores, dense = self._score_version_batches_v30a(
            version, prepared, batches
        )
        version_index = mechanics_v30a.VERSIONS_V30A.index(version)
        sign_index = mechanics_v30a.SIGNS_V30A.index(schedule_item["sign"])
    ...
    def _restore_and_verify_v30a(self):
        restored = [
        ...
        if (
            ...
            or any(
                ...
                != self._v30a_references[rank]["selected"]
                or item.get("current") != item.get("reference")
                for rank, item in enumerate(checks)
            )
        ):
            raise RuntimeError("v30a exact selected restore changed")
        return canonical_sha256(checks)

    def _run_signed_wave_v30a(
        self, schedule_item, prepared, unit_scores, commitments,
    ):
        restore_hashes = []
        captures = mechanics_v30a.execute_paired_resident_signed_wave_v30a(
            schedule_item,
            perturb=lambda seeds, negate: self._perturb_signed_wave_v30a(
                seeds, negate
            ),
            score_version=lambda version: self._score_resident_version_v30a(
                version, prepared, schedule_item, unit_scores, commitments
            ),
            restore=lambda: restore_hashes.append(self._restore_and_verify_v30a()),
        )
        if len(restore_hashes) != 1 or tuple(captures) != tuple(
            schedule_item["resident_version_order"]
        ):
            raise RuntimeError("v30a resident signed-wave transaction changed")
        return restore_hashes[0]

    def _population_boundary_v30a(self):
        reports = [
            ...
            for item in self._resolve([
                self.engines[rank].collective_rpc.remote(
                    ...
                    args=(4, 1, self._v30a_references[rank]["sha256"]),
                )
                for rank in range(4)
            ])
        ]
        if any(
            ...
            != self._v30a_references[index]["sha256"]
            or item.get("current_identity") != self._v30a_references[index]
            for index, item in enumerate(reports)
        ):
            raise RuntimeError("v30a selected/unselected population boundary changed")
        return canonical_sha256(reports)

    def estimate_paired_data_compat_v30a(self):
        schedule = mechanics_v30a.resident_signed_wave_schedule_v30a()
        prepared, request_identity, token_audit = self._prepared_fixed_batches_v30a()
        phase_a = self._full_context_phase_v30a(prepared)
        phase_b = self._full_context_phase_v30a(prepared)
        a_b_equal = _phase_equal(phase_a, phase_b)
        if not all(a_b_equal.values()):
            raise RuntimeError("v30a full-context A/B guard is not exact")
        unit_scores = np.full((2, 5, 2, 32, 39), np.nan, dtype=np.float64)
        dense_commitments = []
        restore_hashes = []
        for schedule_item in schedule:
            restore_hashes.append(self._run_signed_wave_v30a(
                schedule_item, prepared, unit_scores, dense_commitments
            ))
        if (
            not np.isfinite(unit_scores).all()
            or len(dense_commitments) != 640
            or len(restore_hashes) != 16
        ):
            raise RuntimeError("v30a paired population capture incomplete")
        boundary_sha256 = self._population_boundary_v30a()
        phase_c = self._full_context_phase_v30a(prepared)
        a_c_equal = _phase_equal(phase_a, phase_c)
        if not all(a_c_equal.values()):
            raise RuntimeError("v30a full-context A/C guard is not exact")
        compact = mechanics_v30a.build_compact_estimator_summary_v30a(
            unit_scores, self._v30a_panel_bundle
        )
        unit_scores = None
        runtime_integrity = {
        ...
        summary = _seal({
            "schema": "eggroll-es-paired-data-compat-summary-v30a",
            "experiment_name": EXPERIMENT_NAME_V30A,
            "alpha": 0.0,
            "sigma": 0.0003,
            "frame_content_sha256": prereg_v30a.FRAME_CONTENT_SHA256,
            "perturbation_basis_content_sha256": self._v30a_preregistration[
                "frozen_recipe"
        ...
        gate = mechanics_v30a.evaluate_candidate_v30a(summary)
        guard = {
            "schema": "eggroll-es-v30a-full-context-a-b-c-guard",
        ...
        audit = _seal({
            "schema": "eggroll-es-paired-runtime-compact-audit-v30a",
    ...
    @staticmethod
    def _closed_surface_v30a(*_args, **_kwargs):
        raise RuntimeError("v30a closes update checkpoint evaluation and union surfaces")

    configure_anchor = _closed_surface_v30a
    configure_train_panels_v13 = _closed_surface_v30a
    estimate_train_panels_v13 = _closed_surface_v30a
    estimate_step_coefficients = _closed_surface_v30a
    apply_seed_coefficients = _closed_surface_v30a
    train_step = _closed_surface_v30a
    evaluate_handle = _closed_surface_v30a
    evaluate_population_on_batch = _closed_surface_v30a
    eval_step = _closed_surface_v30a
    fit = _closed_surface_v30a


def load_runtime_trainer_v30a(preregistration, layer_bundle):
    model = preregistration["frozen_recipe"]["model"]
    parent = anchor_v4.load_trainer(layer_bundle)

    class PairedDataCompatRuntimeTrainerV30A(
        PairedDataCompatRuntimeMixinV30A, parent,
    ):
        def launch_engines(
            ...
            if num_engines != 4 or n_gpu_per_vllm_engine != 1:
                raise ValueError("v30a requires exactly four TP1 engines")
            import ray
            ...
            for rank in range(4):
                engines.append(ray.remote(
                    ...
                    worker_extension_cls=WORKER_EXTENSION_V30A,
                    dtype=precision,
    ...
    return PairedDataCompatRuntimeTrainerV30A


def _make_trainer_v30a(preregistration, layer_bundle):
    cls = load_runtime_trainer_v30a(preregistration, layer_bundle)
    return cls(
...
def _parser():
    ...
    parser.add_argument("--v30a-dry-run", action="store_true")
...
def run_exact_v30a(
    ...
    run_dir = OUTPUT_DIRECTORY_V30A / EXPERIMENT_NAME_V30A
    report_path = run_dir / REPORT_NAME_V30A
    ...
    try:
        ...
        trainer = _make_trainer_v30a(preregistration, layer_bundle)
        configuration = trainer.configure_paired_data_compat_v30a(
            preregistration, panel_bundle, layer_bundle
        )
        summary, gate, audit = trainer.estimate_paired_data_compat_v30a()
        if gate != mechanics_v30a.evaluate_candidate_v30a(summary):
            raise RuntimeError("v30a compact gate recomputation changed")
    except BaseException as error:
        failure = error
    finally:
...
def main(argv=None):
    ...
    preregistration = load_preregistration_v30a()
    panel_bundle = mechanics_v30a.load_paired_panel_bundle_v30a()
    layer_bundle = load_layer_bundle_v30a(preregistration)
    implementation = implementation_identity_v30a()
    recipe = recipe_v30a(
        preregistration, panel_bundle, layer_bundle, implementation
    )
    ...
    if args.v30a_dry_run:
        payload = _seal({
            ...
            "pass_authority_limited_to_separate_full_v389_train_only_hpo_preregistration": True,
