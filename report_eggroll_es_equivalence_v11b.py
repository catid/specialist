#!/usr/bin/env python3
"""Report V11b dual-manifest exact equivalence to completed V10."""
...
import run_eggroll_es_anchor_equivalence_v11b as retry_v11b
import run_eggroll_es_anchor_line_search as driver_v1


def build_report(journal_path):
    path = Path(journal_path).resolve()
    retry_v11b.driver_v11.driver_v8.offline_audit._assert_no_heldout(
        str(path), "v11b equivalence journal",
    )
    journal = json.loads(path.read_text())
    audit = retry_v11b.validate_completed_journal_v11b(journal)
    report = {
        "schema": "eggroll-es-resident-sign-equivalence-report-v11b",
        "metric": "exact_v10_equivalence_with_dual_raw_templated_manifests",
        "passed": audit["equivalence"]["all_exact"] is True,
        "equivalence": audit["equivalence"],
        "failed_v11_evidence_binding_sha256": audit[
            "failed_v11_evidence_binding_sha256"
        ],
        "dual_manifest_binding_sha256": audit[
            "dual_manifest_binding_sha256"
        ],
        "raw_domain_manifests": retry_v11b.anchor_v11b.RAW_DOMAIN_MANIFESTS_V11B,
        "templated_domain_manifests": (
            retry_v11b.anchor_v11b.TEMPLATED_DOMAIN_MANIFESTS_V11B
        ),
        "coverage": {
...
def main(argv=None):
    ...
    retry_v11b.driver_v11.driver_v8.offline_audit._assert_no_heldout(
        str(output), "v11b report output",
    )
    if output.exists() or output.with_name(output.name + ".tmp").exists():
        raise ValueError("v11b report output already exists")
