...
EXPERIMENT = "v48d_generation_boundary_projection_backtracking"
RUN_DIR = (ROOT / "experiments/eggroll_es_hpo/runs" / EXPERIMENT).resolve()
ATTEMPT = (RUN_DIR.parent / f".{EXPERIMENT}.attempt.json").resolve()
REPORT = (RUN_DIR / "generation_boundary_backtracking_report_v48d.json").resolve()
FAILURE = (RUN_DIR / "failure_v48d.json").resolve()
GPU_LOG = (RUN_DIR / "gpu_activity_v48d.jsonl").resolve()
SNAPSHOT = (RUN_DIR / "adapter_backtracked_v48d").resolve()
PREREGISTRATION = (
    ROOT / "experiments/eggroll_es_hpo/preregistrations/"
    "matched_lora_es_generation_boundary_backtracking_v48d.json"
).resolve()
V48B_RUN = (
...
def _artifact_paths_v48d() -> dict[str, Path]:
    result = {}
    for _ratio, label in SCALE_LABELS_V48D.items():
        result[f"candidate_gate_{label}"] = (
            RUN_DIR / f"candidate_gate_{label}_v48d.json"
        )
        result[f"exact_abort_{label}"] = (
            RUN_DIR / f"exact_abort_{label}_v48d.json"
        )
        result[f"candidate_consensus_{label}"] = (
            RUN_DIR / f"candidate_consensus_{label}_v48d.json"
        )
    return {key: value.resolve() for key, value in result.items()}


SCALE_ARTIFACTS = _artifact_paths_v48d()


def implementation_bindings_v48d() -> dict:
    paths = {
        "runtime": Path(__file__).resolve(),
        "builder": ROOT / "build_lora_es_generation_boundary_backtracking_v48d.py",
        "planning": Path(planning.__file__).resolve(),
        "tests": ROOT / "test_lora_es_generation_boundary_backtracking_v48d.py",
...
def load_evidence_v48d() -> dict:
    return planning.load_v48b_evidence_v48d(
        V48B_EVIDENCE_PATHS, SUBSET, V48B_PREREGISTRATION,
    )


def load_preregistration_v48d(args) -> tuple[dict, dict]:
    path = Path(args.preregistration).resolve()
    if v40a.file_sha256(path) != args.preregistration_sha256:
        raise RuntimeError("v48d preregistration file identity changed")
    value = json.loads(path.read_text(encoding="utf-8"))
    compact = {
        key: item for key, item in value.items()
        if key != "content_sha256_before_self_field"
    }
    evidence = load_evidence_v48d()
    ...
    if (
        ...
        != args.preregistration_content_sha256
        or value.get("schema")
        != "matched-lora-es-generation-boundary-backtracking-preregistration-v48d"
        ...
        or value.get("implementation_bindings") != implementation_bindings_v48d()
...
def _persist_v48d(path: Path, value: dict) -> dict:
    return prior._persist_phase(path, value)


def _candidate_consensus_v48d(
...
def _finish_v48d(trainer, monitor, stop, failures, pid_map):
    stop.set()
    monitor.join(timeout=10)
    if monitor.is_alive() or not failures.empty():
        raise RuntimeError("v48d GPU monitor failed") from (
...
def _validate_abort_v48d(aborted: dict, master: dict, runtime_sha: str) -> None:
    if (
        ...
        raise RuntimeError("v48d rejected candidate exact abort changed")


def main(argv: list[str] | None = None) -> int:
    args = parser().parse_args(argv)
    prereg, evidence = load_preregistration_v48d(args)
    if args.dry_run:
        print(json.dumps({
            ...
            "scale_order": list(planning.TARGET_NORM_RATIOS_V48D),
    ...
    if ATTEMPT.exists() or RUN_DIR.exists():
        raise RuntimeError("v48d requires fresh artifact paths")
    preflight = v40a.gpu_preflight()
    attempt = v40a.self_hashed({
        "schema": "generation-boundary-backtracking-attempt-v48d",
        "status": "launching",
    ...
    try:
        sealed_subset = v48b.load_subset_v48b(
            SUBSET, planning.EXPECTED_SUBSET_FILE_SHA256_V48D,
            planning.EXPECTED_SUBSET_CONTENT_SHA256_V48D,
        )
        v48b._SEALED_SUBSET = sealed_subset
        with v48b.patched_v43i_v48b(), patched_prior_paths_v48d():
            ...
            if bundle["content_sha256_before_self_field"] != (
                ...
                raise RuntimeError("v48d frozen train bundle changed")
            ...
            if (
                ...
                raise RuntimeError("v48d initial state differs from V48B exact abort")
            master = masters[0]
            ...
            if update_sequences != {0}:
                raise RuntimeError("v48d restored update sequence changed")
            ...
            if len(reference_generations) != 1:
                raise RuntimeError("v48d reference generation differs across ranks")
            postinstall = prior._exact_base_score(
                trainer, bundle, dense_items, requests,
            )
            if preinstall["consensus"] != postinstall["consensus"]:
                raise RuntimeError("v48d canonical install changed initial score")
            full_plan = prior.fused.fused_requests_v43i(requests, full_anchors)
            if len(full_plan["requests"]) != 896:
                raise RuntimeError("v48d full candidate request coverage changed")
            ...
            for scale_plan in evidence["scale_plans"]:
                ...
                plan_id = v40a.canonical_sha256({
                    "schema": "generation-boundary-backtracking-plan-v48d",
                ...
                if set(gate.get("checks", {})) != expected_checks:
                    raise RuntimeError("v48d candidate gate inventory changed")
                gate_artifact = _persist_v48d(
                    SCALE_ARTIFACTS[f"candidate_gate_{label}"], {
                        "schema": "generation-boundary-candidate-gate-v48d",
                ...
                if acceptance_passed:
                    ...
                    consensus = _candidate_consensus_v48d(
                ...
                if not acceptance_passed:
                    ...
                    _validate_abort_v48d(aborted, master, master_runtime_sha)
                    abort_artifact = _persist_v48d(
                    ...
                    planning.largest_passing_scale_v48d(scale_results)
                    continue
                ...
                if planning.largest_passing_scale_v48d(scale_results) != ratio:
                    raise RuntimeError("v48d largest passing scale changed")
                break

            if accepted_ratio is None and (
                planning.largest_passing_scale_v48d(scale_results) is not None
            ):
                raise RuntimeError("v48d no-pass result changed")
            gpu, cleanup, idle = _finish_v48d(
                trainer, monitor, stop, failures, pid_map,
            )
            trainer = None

        report = v40a.self_hashed({
            "schema": "generation-boundary-backtracking-report-v48d",
            "status": (
        ...
        v40a.atomic_json(FAILURE, v40a.self_hashed({
            "schema": "generation-boundary-backtracking-failure-v48d",
