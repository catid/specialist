"""Content-free V3 quarantine denial with V73H process-start actor install."""
...
def _deny(label: str) -> None:
    raise PermissionError(f"V73E systems-only protected {label} denied")
...
def install(mechanism: str) -> None:
    ...
    if mechanism not in {
        ...
        "ray_actor_sitecustomize_pre_runtime_imports",
    }:
        raise RuntimeError("V73E path guard installation mechanism is invalid")
    with _LOCK:
        if _INSTALLED:
            if (
                ...
                raise RuntimeError(
                    "V73E path guard cannot change installation mechanism"
                )
            return
        _ORIGINALS.update({
...
def receipt() -> dict[str, Any]:
    ...
    result = {
        "schema": "qwen36-v73e-systems-only-path-guard-process-v1",
        "pid": os.getpid(),
