...
import build_qwen36_v73e_exact_phase_profiler_preregistration as prereg
import qwen36_v73e_exact_phase_profiler_contract as contract
import run_lora_es_v71_v72_profile_calibration_v73e as target
import run_qwen36_v73e_exact_phase_profiler as profiler
...
def test_runtime_roots_do_not_import_preregistration_builder():
    for path in (contract.TARGET, contract.LAUNCHER, Path(contract.__file__)):
        ...
        assert "build_qwen36_v73e_exact_phase_profiler_preregistration" not in imports


def test_preregistration_separates_timeline_and_permission_blocked_hbm():
    value = prereg.build_preregistration_v73e()
    ...
    assert command[:7] == [
        ...
        contract.BASHRC_EXEC_SCRIPT, "v73e-nsys-launch",
    ]
...
def test_content_free_panel_and_plan_are_exact_and_semantic_free():
    panel = contract.content_free_token_panel_v73e()
    rebuilt = target.expected_content_free_payload_and_plan_v73e()
    assert panel["content_sha256_before_self_field"] == (
        contract.CONTENT_FREE_TOKEN_PANEL_CONTENT_SHA256
    )
    assert panel["request_count"] == 64
    assert len(panel["records"]) == 64
    assert sum(row["unscored_prefix_token_count"] for row in panel["records"]) == 3174
    assert sum(row["answer_token_count"] for row in panel["records"]) == 1649
    assert sum(row["total_token_count"] for row in panel["records"]) == 4823
    assert all(
        row["text_question_answer_or_semantic_label_present"] is False
        and all(type(token) is int for token in row["prompt_token_ids"])
        for row in panel["records"]
    )
    assert rebuilt["plan"]["plan_sha256"] == contract.CONTENT_FREE_PLAN_SHA256
    assert rebuilt["payload"]["contract"]["evaluation_contract_sha256"] == (
        contract.CONTENT_FREE_EVALUATION_CONTRACT_SHA256
    )


def test_staged_adapter_exact_inverse_and_numeric_suffix_scorer(monkeypatch):
    staged = target.verify_staged_adapter_contract_v73e()
    assert staged["status"] == "sealed_staged_only_exact_inverse_verified"
    assert staged["canonical_master_identity"]["sha256"] == (
        contract.CANONICAL_MASTER_SHA256
    )
    monkeypatch.setattr(target, "_CONTENT_FREE_INPUT_EVIDENCE", None)
    trainer = SimpleNamespace(
        tokenizer=SimpleNamespace(
            all_special_ids=[0, 1, 2],
            __len__=lambda self: 248_320,
        )
    )
    # Special methods are resolved on the type, not the instance.
    class Tokenizer:
        all_special_ids = [0, 1, 2]
    ...
    prepared = target.prepare_content_free_inputs_v73e(trainer, None, None)
    outputs = []
    for item in prepared["dense_items"]:
        token_ids = item["prompt_token_ids"]
        outputs.append(SimpleNamespace(
            prompt_token_ids=token_ids,
            prompt_logprobs=[
                {token: {"logprob": -0.25}} for token in token_ids
            ],
        ))
    scored = target.score_content_free_suffix_outputs_v73e(
        prepared["dense_items"], outputs
    )
    assert scored["example_count"] == 64
    assert scored["answer_token_count"] == 1649
    assert scored["mean_example_mean_logprob"] == -0.25
    assert scored["question_answer_gold_or_semantic_label_present"] is False


def test_actor_work_consistency_rejects_assignment_not_in_sealed_plan(monkeypatch):
    monkeypatch.setattr(target, "_CONTENT_FREE_INPUT_EVIDENCE", {
        "evaluation_contract_sha256": contract.CONTENT_FREE_EVALUATION_CONTRACT_SHA256
    })
    plan = target.expected_content_free_payload_and_plan_v73e()["plan"]
    rows = []
    fields = (
        "wave_index", "engine_rank", "direction_seed", "sign", "pair_id",
        "evaluation_contract_sha256",
    )
    for assignment in (item for wave in plan["waves"] for item in wave):
        coordinate = {field: assignment[field] for field in fields}
        row = {
            "schema": "eggroll-es-actor-cuda-work-receipt-v66d",
            **coordinate,
            "work_id": contract.canonical_sha256(coordinate),
            "worker_pid": 1000 + assignment["engine_rank"],
            "physical_gpu_id": assignment["engine_rank"],
            "cuda_event": {"elapsed_ms": 1.0},
            "output_cardinality": {
                "request_outputs": 64,
                "samples": 64,
                "generated_tokens": 64,
        ...
        row["receipt_sha256"] = contract.canonical_sha256(row)
        rows.append(row)
    value = target.actor_work_self_consistency_v73e(rows, [])
    assert value["sealed_plan_sha256"] == contract.CONTENT_FREE_PLAN_SHA256
    tampered = copy.deepcopy(rows)
    tampered[0]["sign"] *= -1
    tampered[0]["receipt_sha256"] = contract.canonical_sha256({
        key: value for key, value in tampered[0].items()
        if key != "receipt_sha256"
    })
    with pytest.raises(RuntimeError, match="actor work self-consistency"):
        target.actor_work_self_consistency_v73e(tampered, [])
...
def test_phase_ledger_emits_exact_nonoverlapping_order():
    ...
    domain = Domain()
    Phase = target.phase_class_v73e(Base, domain)
    instance = Phase()
    for phase in contract.PHASES[1:]:
        instance.value = phase
    instance.close_v73e_range()
    receipt = target.phase_receipt_v73e("timeline", complete=True)
...
def test_hbm_execute_rejects_before_paths_or_subprocess(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="HBM metrics arm blocked"):
        profiler.execute_profile_v73e(value, "a" * 64, "hbm_metrics")
...
def test_changed_bashrc_identity_rejects_before_paths_or_subprocess(
    ...
    with pytest.raises(RuntimeError, match="launch bashrc"):
        profiler.execute_profile_v73e(value, "a" * 64, "timeline")
    assert list(tmp_path.iterdir()) == []


def test_prelaunch_toolchain_identity_binds_all_timeline_runtime_inputs():
    value = prereg.build_preregistration_v73e()
    receipt = profiler.validate_prelaunch_toolchain_v73e(value)
...
def test_synthetic_timeline_exact_phase_parser(tmp_path):
    ...
    result = profiler.analyze_sqlite_v73e(
...
def test_nccl_debug_transport_requires_explicit_content_free_lines(tmp_path):
    ...
    result = profiler._analyze_nccl_debug_logs_v73e(
...
def test_nccl_debug_rejects_semantic_markers(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="semantic payload marker"):
        profiler._analyze_nccl_debug_logs_v73e(
            str(tmp_path / "nccl_debug.*.log"), pids
        )
...
def test_prelaunch_four_gpu_gate_records_identity_and_no_path_inference(monkeypatch):
    value = prereg.build_preregistration_v73e()
    topology = value["physical_gpu_topology"]
    monkeypatch.setattr(profiler.builder, "gpu_topology_v73e", lambda: topology)
    gpu_rows = "\n".join(
    ...
    result = profiler.four_gpu_idle_preflight_v73e(value)
...
import os
from pathlib import Path
import v73e_path_open_guard as guard
...
def test_guarded_runtime_import_records_denied_legacy_resolution_not_open():
    value = _guard_subprocess(
        """
import json
import run_lora_es_v71_v72_profile_calibration_v73e
import v73e_path_open_guard as guard
...
spec = importlib.util.spec_from_file_location('v73e_path_open_guard', guard_path)
guard = importlib.util.module_from_spec(spec)
spec.loader.exec_module(guard)
relative_prefix = '.v73e-synthetic-prefix'
...
outside_fd, outside_name = tempfile.mkstemp(prefix='v73e-outside-')
os.write(outside_fd, b'never read')
os.close(outside_fd)
inside_directory = Path(tempfile.mkdtemp(prefix='.v73e-link-', dir=guard.ROOT))
...
import json
import sys
import eggroll_es_worker_lora_v73e as worker
...
message = ''
try:
    import eggroll_es_worker_lora_v73e
...
def test_ray_runtime_env_injection_preserves_existing_job_env_and_fails_collision():
    original = {"env_vars": {"V73E_EXISTING": "yes"}, "pip": ["example"]}
    merged, evidence = target.ray_actor_bootstrap_runtime_env_v73e(original)
    assert original == {
        "env_vars": {"V73E_EXISTING": "yes"},
        "pip": ["example"],
    }
    assert merged["env_vars"]["V73E_EXISTING"] == "yes"
    ...
    with pytest.raises(RuntimeError, match="env collided"):
        target.ray_actor_bootstrap_runtime_env_v73e({
...
import ray
import run_lora_es_v71_v72_profile_calibration_v73e as target
runtime_env, evidence = target.ray_actor_bootstrap_runtime_env_v73e({{
    'env_vars': {{
        'PYTHONPATH': {str(contract.ROOT)!r},
        'SPECIALIST_V73E_JOB_ONLY': 'job',
    }}
}})
result = None
with tempfile.TemporaryDirectory(prefix='v73e-cpu-ray-') as ray_temp:
    ray.init(
        num_cpus=1,
        num_gpus=0,
    ...
    class Probe:
        def inspect(self):
            ...
            import sys
            import eggroll_es_worker_lora_v73e as worker
            receipt = worker.LoRAAdapterStateWorkerExtensionV73E.systems_only_path_guard_receipt_v73e(None)
            return {{
                'receipt': receipt,
                'job_env': os.environ.get('SPECIALIST_V73E_JOB_ONLY'),
                'actor_env': os.environ.get('SPECIALIST_V73E_ACTOR_ONLY'),
    ...
    actor = Probe.options(runtime_env={{
        'env_vars': {{'SPECIALIST_V73E_ACTOR_ONLY': 'actor'}}
    }}).remote()
...
def test_gpu_accounting_keeps_reserved_resident_useful_and_promotion_split():
    ...
    value = target.gpu_time_accounting_v73e(legacy)
...
def test_v73d_attempt_1_predecessor_is_immutable_and_nonpromotable():
    value = prereg._validate_v73d_attempt_1_predecessor()
    assert value["status"] == (
        "immutable_failed_closed_legacy_adapter_quarantine_incompatibility"
    )
    assert value["failure_cause"] == (
        "quarantined_legacy_adapter_contract_opened_during_setup"
    )
    assert value["ray_actor_guard_bootstrap_reached"] is False
    assert value["model_load_reached"] is False
    assert value["gpu_work_reached"] is False
    assert value["partial_actor_cuda_receipt_count"] == 0
    assert value["final_all_four_gpus_idle"] is True
    assert value["promotion_charged_gpu_seconds"] == 0
    assert value["protected_or_semantic_data_opened"] is False
    assert value["quality_or_semantic_authority"] is False
...
def test_target_and_profiler_failure_receipts_preserve_gpu_accounting_split(
    ...
    paths = {
        ...
        "phase_receipt": str(run / "phase_v73e.json"),
        "path_guard_receipt": str(run / "guard_v73e.json"),
    }
    predecessor = {
        "status": (
    ...
    target.finalize_failure_v73e(value, "timeline", "a" * 64)
    ...
    profile_failure = profiler.profile_failure_receipt_v73e(
        value, "timeline", 1, "a" * 64
    )
    assert profile_failure["run_failure_exists"] is True
    assert profile_failure["immutable_v73d_attempt_1_predecessor"] == predecessor
