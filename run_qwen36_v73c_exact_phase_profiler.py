#!/usr/bin/env python3
"""Fail-closed launcher and postrun analyzer for V73C exact-phase profiling."""
...
def _require_columns(
    ...
    _require(not missing, f"V73C SQLite {table} columns absent: {sorted(missing)}")
...
def _nvtx_rows(connection: sqlite3.Connection) -> tuple[list[dict], list[dict]]:
    ...
    _require(required.issubset(_tables(connection)), "V73C NVTX tables absent")
    _require_columns(
        connection,
        "NVTX_EVENTS",
...
def _validate_phase_ranges(
    ...
    _require(
        ...
        "V73C trace phase ownership or overlap changed",
    )
    return selected
...
def _phase_for_event(
    ...
    _require(
        len(matches) == 1,
        f"V73C {label} crossed or fell outside exact phase ranges",
    )
...
def _gpu_events(
    ...
    _require(
        ...
        "V73C CUDA kernel/memcpy tables absent",
    )
    kernel_required = (
    ...
    for row in connection.execute(
        ...
        _require(end > start, "V73C CUDA kernel duration changed")
        pid = _decode_pid(global_pid)
        _require(pid in expected_pids, "V73C CUDA kernel PID is not an actor")
        demangled = strings.get(int(row[4]))
        short = strings.get(int(row[5]))
        _require(
            isinstance(demangled, str) and isinstance(short, str),
            "V73C CUDA kernel symbol identity changed",
        )
        kernels.append({
    ...
    for row in connection.execute(
        ...
        _require(end > start and size >= 0, "V73C CUDA memcpy changed")
        pid = _decode_pid(global_pid)
        _require(pid in expected_pids, "V73C CUDA memcpy PID is not an actor")
        _require(kind in copy_labels, "V73C CUDA memcpy kind is unknown")
        copies.append({
    ...
    _require(bool(kernels), "V73C CUDA kernel trace is empty")
    _require(bool(copies), "V73C CUDA memcpy trace is empty")
    _require(
        {row["physical_gpu_id"] for row in kernels} == set(GPU_IDS)
        and {row["pid"] for row in kernels} == expected_pids,
        "V73C CUDA kernel trace omitted a GPU",
    )
...
def _allocation_events(
    ...
    _require(
        ...
        "V73C CUDA allocation tables absent",
    )
    _require_columns(
        connection,
    ...
    for row in connection.execute(
        ...
        pid = _decode_pid(global_pid)
        _require(pid in expected_pids, "V73C allocation PID is not an actor")
        _require(size >= 0 and operation in labels, "V73C allocation event changed")
        rows.append({
    ...
    _require(bool(rows), "V73C allocation event trace is empty")
    _require(
        {row["physical_gpu_id"] for row in rows} == set(GPU_IDS)
        and {row["pid"] for row in rows} == expected_pids,
        "V73C allocation event trace omitted an actor or physical GPU",
    )
    return rows
...
def _summarize_allocations(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for gpu in GPU_IDS:
        ...
        for row in selected:
            ...
            _require(is_alloc != is_free, f"V73C allocation label unsupported: {label}")
            ...
            _require(running >= 0, "V73C traced allocation balance became negative")
...
def _summarize_nccl(
    ...
    _require(bool(rows), "V73C NCCL trace is empty")
    ...
    for row in rows:
        _require(row["pid"] in expected_pids, "V73C NCCL PID is not an actor")
    ...
    _require(
        ...
        "V73C update execute contains no NCCL trace",
    )
    return {
        "by_phase": dict(sorted(by_phase.items())),
...
def _validate_empty_process_streams(
    ...
    _require(
        ...
        "V73C ProcessStreams table absent",
    )
    _require_columns(
        connection,
        "ProcessStreams",
    ...
    for global_pid, filename_id, content_id in connection.execute(
        ...
        _require(
            ...
            "V73C process stream string identity changed",
        )
        _require(
            not content.strip(),
            "V73C target process stream was nonempty; trace rejected",
        )
        rows.append({
...
def _analyze_nccl_debug_logs_v73c(
    pattern: str,
    expected_pids: set[int],
) -> dict[str, Any]:
    path = Path(pattern)
    _require("*" in path.name, "V73C NCCL debug pattern changed")
    files = sorted(path.parent.glob(path.name))
    _require(files, "V73C NCCL metadata logs are absent")
    observed_pids = set()
    logs = []
    explicit = []
    forbidden_markers = (
    ...
    for file_path in files:
        _require(
            ...
            "V73C NCCL metadata log is not a regular file",
        )
        match = re.search(r"\.([0-9]+)\.log$", file_path.name)
        _require(match is not None, "V73C NCCL metadata PID filename changed")
        pid = int(match.group(1))
        observed_pids.add(pid)
        content = file_path.read_text(encoding="utf-8")
        _require(content, "V73C NCCL metadata log is empty")
        lowered = content.casefold()
        _require(
            not any(marker.casefold() in lowered for marker in forbidden_markers),
            "V73C NCCL metadata log contains semantic payload marker",
        )
    ...
    _require(
        ...
        "V73C NCCL metadata logs omitted an actor PID",
    )
    return {
        "debug_level": "INFO",
...
def analyze_sqlite_v73c(
    ...
    _require(path.is_file() and not path.is_symlink(), "V73C SQLite export absent")
...
def validate_target_artifacts_v73c(
    ...
    _require(len(rows) == 16, "V73C actor receipt count changed")
    ...
    for row in rows:
        ...
        _require(
            ...
            "V73C actor receipt self hash changed",
        )
    ...
    _require(
        ...
        "V73C actor PID/GPU binding changed between waves",
    )
    bindings = {
    ...
    _require(
        ...
        "V73C four-wave actor coverage changed",
    )
    guard_processes = [guard.get("controller"), *guard.get("workers", [])]
    _require(
        len(guard_processes) == 5 and all(
            isinstance(row, dict) for row in guard_processes
        ),
        "V73C path guard process coverage changed",
    )
    for row in guard_processes:
        body = dict(row)
        claimed = body.pop("receipt_sha256", None)
        _require(
            claimed == builder.canonical_sha256(body)
            ...
            "V73C path guard process receipt changed",
        )
    _require(
        {row["pid"] for row in guard["workers"]}
        == {row["pid"] for row in bindings.values()}
        and guard["controller"]["pid"]
        not in {row["pid"] for row in guard["workers"]},
        "V73C path guard worker PID attribution changed",
    )
    ...
    postrun_toolchain_identity = validate_prelaunch_toolchain_v73c(
        preregistration
    )
    _require(
        report.get("schema") == "v73c-exact-phase-qwen36-calibration-report"
        ...
        == "zero_successful_protected_open_or_resolve_systems_only"
        ...
        is True,
        "V73C target authority or final idle changed",
    )
    ...
    _require(
        ...
        "V73C same-live/abort/traffic semantics changed",
    )
...
def analyze_profile_v73c(
    preregistration: Mapping[str, Any], arm: str
) -> dict[str, Any]:
    _require(arm == "timeline", "current V73C analyzer accepts timeline arm only")
    paths = preregistration["arms"][arm]["artifacts"]
    target = validate_target_artifacts_v73c(preregistration, arm)
    report_path = Path(paths["nsys_report"])
    sqlite_path = Path(paths["sqlite_export"])
    _require(
        report_path.is_file() and not report_path.is_symlink(),
        "V73C Nsight report absent",
    )
    sqlite_result = analyze_sqlite_v73c(
        sqlite_path,
    ...
    nccl_debug = _analyze_nccl_debug_logs_v73c(
        paths["nccl_debug_pattern"], set(target["actor_pids"])
    )
    value = {
        "schema": "qwen36-v73c-exact-phase-profile-analysis-v1",
...
def _fresh_launch(paths: Mapping[str, str]) -> None:
    ...
    _require(
        ...
        "V73C requires fresh application/profile paths",
    )


def validate_prelaunch_toolchain_v73c(
    ...
    def exact_source(
        ...
        _require(
            ...
            f"V73C prelaunch tool identity changed: {label}",
        )
        return {
            "label": label,
    ...
    _require(
        ...
        "V73C target Python invocation path changed",
    )
    ...
    return {
        "schema": "qwen36-v73c-prelaunch-toolchain-identity-v1",
        "passed": True,
...
def four_gpu_idle_preflight_v73c(
    preregistration: Mapping[str, Any],
) -> dict[str, Any]:
    topology = builder.gpu_topology_v73c()
    _require(
        topology == preregistration["physical_gpu_topology"],
        "V73C physical GPU topology changed before launch",
    )
    gpu_command = [
    ...
    for line in _run_text(gpu_command).splitlines():
        ...
        _require(len(fields) == 5, "V73C idle GPU query shape changed")
        rows.append({
    ...
    _require(
        ...
        "V73C physical GPU attribution changed before launch",
    )
    ...
    for line in raw_processes.splitlines():
        ...
        _require(len(fields) == 4, "V73C compute process query shape changed")
        process_rows.append({
            "gpu_uuid": fields[0],
            "pid": int(fields[1]),
    ...
    return {
        "schema": "qwen36-v73c-four-gpu-idle-preflight-v1",
...
def execute_profile_v73c(
    ...
    _require(
        ...
        "V73C HBM metrics arm blocked before directory creation or subprocess",
    )
    toolchain_identity = validate_prelaunch_toolchain_v73c(preregistration)
    paths = arm_contract["artifacts"]
    _fresh_launch(paths)
    command = builder.expand_command_v73c(
        arm,
        preregistration_file_sha256,
        preregistration["content_sha256_before_self_field"],
    )
    command_sha = builder.canonical_sha256(command)
    preflight = four_gpu_idle_preflight_v73c(preregistration)
    attempt = {
        "schema": "qwen36-v73c-profiler-launch-attempt-v1",
        "status": (
            "prelaunch_accepted_launching_fresh_no_commit_profile"
            if preflight["passed"]
    ...
    _require(
        ...
        "V73C requires all four GPUs idle and foreign-process-free before launch",
    )
    ...
    if completed.returncode != 0:
        failure = {
            "schema": "qwen36-v73c-profiler-launch-failure-v1",
            "status": "target_or_profiler_failed_closed",
            "arm": arm,
            "returncode": int(completed.returncode),
            "expanded_command_sha256": command_sha,
            "run_failure_exists": Path(paths["failure"]).is_file(),
            "protected_dev_ood_or_holdout_opened": False,
            "checkpoint_snapshot_commit_or_promotion_performed": False,
        }
        _atomic_json(Path(paths["profile_failure"]), failure)
        raise RuntimeError(
            f"V73C profiler target failed with return code {completed.returncode}"
        )
    result = analyze_profile_v73c(preregistration, arm)
...
def main(argv=None) -> int:
    ...
    if args.dry_run == args.execute:
        raise ValueError("V73C profiler requires exactly one of --dry-run or --execute")
    preregistration = load_preregistration_v73c(
