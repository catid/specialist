#!/usr/bin/env python3
"""Fail-closed 5e-5 HPO wrapper around the verified V42B runtime."""
...
def build_train_command(args: argparse.Namespace) -> list[str]:
    if args.learning_rate != LEARNING_RATE:
        raise ValueError("V42G learning-rate arm changed")
...
def validate_preregistration(args, command) -> dict:
    ...
    wrapper = value.get("implementation", {}).get("hpo_launcher_v42g")
    expected = {
        "path": str(Path(__file__).resolve()),
        "sha256": engine.file_sha256(Path(__file__).resolve()),
    }
    if wrapper != expected:
        raise RuntimeError("V42G HPO launcher binding changed")
    if (
        value.get("recipe", {}).get("learning_rate") != LEARNING_RATE
        or value.get("recipe", {}).get("only_change_from_v42b")
        != "peak cosine-schedule learning rate 1e-4 -> 5e-5"
    ):
        raise RuntimeError("V42G HPO preregistration recipe changed")
    audit["hpo_launcher_v42g"] = wrapper
    return audit
...
def rewrite_evidence(args) -> None:
    ...
    report["implementation"]["hpo_launcher_v42g"] = {
        "path": str(Path(__file__).resolve()),
        "sha256": engine.file_sha256(Path(__file__).resolve()),
    }
    ...
    attempt.update({
        "schema": "specialist-sft-matched-init-equal-unit-attempt-v42g",
