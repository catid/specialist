#!/usr/bin/env python3
"""Seal launchable V48D from exact train-only V48B evidence."""
...
def build_v48d() -> dict:
    ...
    if (
        ...
        raise RuntimeError("v48d exact source state or scale plans changed")
    value = {
        "schema": (
            "matched-lora-es-generation-boundary-backtracking-"
            "preregistration-v48d"
        ),
        "status": "preregistered_before_train_only_launch",
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "gpu_launch_authorized": True,
        "purpose": (
            "Backtrack the frozen V48B direct generation-boundary LoRA-ES "
            "direction without any population resampling; commit only the "
            "largest descending ratio that strictly passes all train-only gates."
        ),
        ...
        "recipe": {
            ...
            "subset_file_sha256": planning.EXPECTED_SUBSET_FILE_SHA256_V48D,
            "subset_content_sha256": (
                planning.EXPECTED_SUBSET_CONTENT_SHA256_V48D
            ),
            "request_order_sha256": planning.REQUEST_ORDER_SHA256_V48D,
            "matched_initialization": str(runtime.prior.SOURCE),
            "staged_initialization": str(runtime.prior.STAGED),
            "restored_master_sha256": planning.RESTORED_MASTER_SHA256_V48D,
        ...
        "required_gates": {
            ...
            "frozen_v48b_population_direction_reused": True,
        ...
        "implementation_bindings": runtime.implementation_bindings_v48d(),
        "artifacts": {
