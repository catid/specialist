...
import train_eggroll_es_paired_data_compat_v33a as mechanics


def _bundle():
    return mechanics.load_paired_panel_bundle_v33a()


def _identical_scores():
    values = np.empty((2, 5, 2, 64, 39), dtype=np.float64)
    direction = np.linspace(-1.0, 1.0, 64)
...
def test_v33a_materialized_frame_is_exact_5x39_and_content_sealed():
    bundle = _bundle()
    assert bundle["content_sha256_before_self_field"] == (
        "26546dd89dfe1967f0a5482b234cb3dd39a3e0f2bf23754fca18e6d6512185f1"
    )
    assert tuple(bundle["panels"]) == mechanics.PANEL_NAMES_V33A
...
def test_v33a_schedule_is_fresh_64_direction_32_wave_and_alternating():
    schedule = mechanics.resident_signed_wave_schedule_v33a()
    ...
    assert directions == list(range(64))
    assert all(item["restore_after_both_versions"] is True for item in schedule)


def test_v33a_resident_wave_perturbs_once_scores_both_then_restores():
    events = []
    schedule_item = mechanics.resident_signed_wave_schedule_v33a()[1]
    captures = mechanics.execute_paired_resident_signed_wave_v33a(
        schedule_item,
...
def test_v33a_exact_50k_draw_plan_hash_matches_preregistration():
    draws, digest = mechanics._bootstrap_draw_plan_v33a(
        _bundle(), 50_000, mechanics.EXPECTED_DRAW_PLAN_SHA256_V33A,
    )
    assert digest == "ef44acfe80d9afab5e17621eb62acc09572a6c3486f1f0a61dd6848ab6398b37"
    assert sum(
        item["draw"].nbytes for panel in draws.values() for item in panel.values()
    ) == 78_000_000


def test_v33a_synthetic_paired_bootstrap_is_compact_and_has_12_zero_deltas():
...
def test_v33a_gate_is_conjunctive_and_authority_is_narrow():
    endpoints = {
        name: {
        ...
        for name in mechanics.prereg_v33a.ENDPOINTS
    }
    summary = {
        "runtime_integrity": {"all_integrity_audits_passed": True},
        "paired_bootstrap": {"endpoints": endpoints},
    }
    passed = mechanics.evaluate_candidate_v33a(summary)
    ...
    failed_summary["paired_bootstrap"]["endpoints"][
        mechanics.prereg_v33a.ENDPOINTS[0]
    ]["familywise_lcb"] = -1e-12
    failed = mechanics.evaluate_candidate_v33a(failed_summary)
    assert failed["pass"] is False
    assert failed["decision"] == "retain_production_dataset_and_v13_recipe"
    point_failure = json.loads(json.dumps(summary))
    point_failure["paired_bootstrap"]["endpoints"][
        mechanics.prereg_v33a.ENDPOINTS[1]
    ]["candidate_v364_minus_production"] = -1e-12
    assert mechanics.evaluate_candidate_v33a(point_failure)["pass"] is False


def test_v33a_score_geometry_and_bundle_identity_fail_closed():
    with pytest.raises(RuntimeError, match="score tensor changed"):
        mechanics.observed_panel_scores_v33a(np.zeros((2, 5, 2, 64, 38)), _bundle())
    changed = _bundle()
    changed["content_sha256_before_self_field"] = "0" * 64
    with pytest.raises(RuntimeError, match="bundle changed"):
        mechanics.validate_paired_panel_bundle_v33a(changed)
