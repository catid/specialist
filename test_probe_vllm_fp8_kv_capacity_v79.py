...
import probe_vllm_fp8_kv_capacity_v79 as subject
...
def _performance() -> dict:
    ...
    return subject.summarize_generation_timings_v79(rows)


def test_static_preregistration_is_bound_and_authority_false():
    value = subject.validate_static_v79()
...
def test_argv_requires_exact_fresh_fp8_eager_surface(tmp_path: Path):
    ...
    assert subject.validate_argv_v79(
        [
            "--precision-arm",
            "fp8_serialized",
            "--actor-label",
...
def test_rightsized_kwargs_change_only_v78_utilization_target():
    original = {
        "quantization": "fp8",
    ...
    result = subject.rightsized_kwargs_v79(original)
...
def test_live_certificate_binds_capacity_and_mamba_fp32():
    resolved = _resolved()
    ...
    assert subject.resolved_runtime_v79(engine) == resolved
    cache.kv_cache_size_tokens = 159_744
    with pytest.raises(RuntimeError, match="capacity certificate"):
        subject.resolved_runtime_v79(engine)


def test_upgrade_is_single_variable_self_hashed_and_fail_closed():
    result = subject.upgraded_receipt_v79(
        _parent_receipt(), _resolved(), _audit(), _performance()
    )
    assert result["runtime"]["gpu_memory_utilization"] == 0.485
    assert result["single_variable_change_from_v78"] == {
        "gpu_memory_utilization": [0.5, 0.485]
    }
    assert result["resolved_kv_cache_certificate"][
        "mamba_ssm_cache_dtype"
    ] == "float32"
    assert result["routed_fp8_runtime_attestation"][
        "parameter_residency"
    ] == _residency()
    assert result["generation_performance"][
    ...
    assert subject.canonical_sha256_v79(result) == claimed


def test_upgrade_rejects_parent_and_capacity_drift():
    parent = _parent_receipt()
    parent["source_dataset_rows_opened"] = 1
    with pytest.raises(RuntimeError, match="underlying V73"):
        subject.upgraded_receipt_v79(
...
def test_generation_timing_binds_throughput_and_tail_latency():
    ...
    changed = value["calls"][:-1]
    with pytest.raises(RuntimeError, match="timing surface"):
        subject.summarize_generation_timings_v79(changed)
