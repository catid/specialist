#!/usr/bin/env python3
"""Fail-closed lower-LR HPO wrapper around the verified V42B runtime."""
...
def build_train_command(args: argparse.Namespace) -> list[str]:
    if args.learning_rate != LEARNING_RATE:
        raise ValueError("V42C learning-rate arm changed")
    reference = copy.copy(args)
    reference.learning_rate = 1e-4
    command = v42b.build_train_command(reference)
    index = command.index("--learning-rate") + 1
    command[index] = str(LEARNING_RATE)
    return command
...
def validate_preregistration(args, command) -> dict:
    ...
    wrapper = value.get("implementation", {}).get("hpo_launcher_v42c")
    if wrapper != {
        "path": str(Path(__file__).resolve()),
        "sha256": engine.file_sha256(Path(__file__).resolve()),
    }:
        raise RuntimeError("V42C HPO launcher binding changed")
    if (
        ...
        raise RuntimeError("V42C HPO preregistration recipe changed")
    audit["hpo_launcher_v42c"] = wrapper
    return audit
...
def rewrite_evidence(args) -> None:
    ...
    report["recipe"]["learning_rate"] = LEARNING_RATE
    report["recipe"]["hpo_parent"] = (
        "v42b_matched_init_equal_unit_fold3_v412_retry_direct_load"
    )
    report["recipe"]["only_change_from_v42b"] = (
        "peak cosine-schedule learning rate 1e-4 -> 3e-5"
    )
    report["implementation"]["hpo_launcher_v42c"] = {
        "path": str(Path(__file__).resolve()),
        "sha256": engine.file_sha256(Path(__file__).resolve()),
    }
    ...
    attempt.update({
        "schema": "specialist-sft-matched-init-equal-unit-attempt-v42c",
