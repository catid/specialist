#!/usr/bin/env python3
"""V46F replicated exact base versus accepted V43L LoRA-ES, OOD first."""
...
import stage_v43l_adapter_vllm_v46e as es_stage
...
LOGICAL_REPLICAS_V46F = {
    "lora_es_v43l": ("lora_es_v43l_a", "lora_es_v43l_b"),
}
LOGICAL_CANDIDATES_V46F = tuple(LOGICAL_REPLICAS_V46F)
CANDIDATE_ARMS_V46F = tuple(
    arm for replicas in LOGICAL_REPLICAS_V46F.values() for arm in replicas
)
ARMS_V46F = BASE_ARMS_V46F + CANDIDATE_ARMS_V46F
STAGED_BY_LOGICAL_V46F = {
    "lora_es_v43l": es_stage.OUTPUT,
}
STAGED_BY_ARM_V46F = {
    replica: STAGED_BY_LOGICAL_V46F[logical]
    for logical, replicas in LOGICAL_REPLICAS_V46F.items()
    for replica in replicas
}
ADAPTER_IDS_V46F = {
    arm: index + 1 for index, arm in enumerate(CANDIDATE_ARMS_V46F)
}
EXPERIMENT = "v46f_base_vs_lora_es_v43l_replicated_ood_first_eval"
RUN_DIR = (ROOT / "experiments/eggroll_es_hpo/runs" / EXPERIMENT).resolve()
ATTEMPT = (RUN_DIR.parent / f".{EXPERIMENT}.attempt.json").resolve()
RAW = (RUN_DIR / "raw_items_v46f.json").resolve()
GPU_LOG = (RUN_DIR / "gpu_activity_v46f.jsonl").resolve()
REPORT = (
    ROOT / "experiments/eval_reports/"
    "base_vs_lora_es_v43l_replicated_ood_first_eval_v46f.json"
).resolve()
...
STAGE_EXPECTED_V46F = {
    "lora_es_v43l": {
        "arm": "lora_es_v43l",
        "weights": "fa991ae480e958d6f4ebccce9e11104e6ba60a374077d108ff1821938506f49b",
        "config": "ede582c12e82fb50eb97ac934ff08eb553a79d2c2d999235abcd8b29795b1d52",
        "manifest_file": "db511e38c4fa4b5db590296dc542c2d46713c3a7569d7f7fa1ae70fc85019ce5",
        "manifest_content": "14a4fb86adef5faa80f162dbeab51c1d6b1c501e50ee22d45e9a2f45cf3d459f",
        "transformed_identity": "3fb74b2312be5a4b4e071d1300d9f5dbf8ba537d646a3fd715c3713715c3b398",
    },
}
RANK_FIELDS_V46F = (
...
def parent_preregistration_v46f() -> dict:
    if core.file_sha256(PARENT_PREREGISTRATION) != (
        PARENT_PREREGISTRATION_FILE_SHA256
    ):
        raise RuntimeError("V46F V45E parent preregistration file changed")
    ...
    if (
        ...
        raise RuntimeError("V46F V45E parent preregistration content changed")
    return value


def arm_wave_plan_v46f() -> tuple[tuple[tuple[str, int], ...], ...]:
    return (
        tuple((arm, index) for index, arm in enumerate(BASE_ARMS_V46F[:4])),
        tuple(
            (arm, index) for index, arm in enumerate(
                BASE_ARMS_V46F[4:] + CANDIDATE_ARMS_V46F
            )
        ),
    )
...
@contextmanager
def patched_candidate_globals_v46f():
    saved = {
        "BASE": core.BASE_ARMS,
    ...
    core.BASE_ARMS = BASE_ARMS_V46F
    core.CANDIDATE_ARMS = CANDIDATE_ARMS_V46F
    core.ARMS = ARMS_V46F
    core.STAGED_BY_ARM = STAGED_BY_ARM_V46F
    core.ADAPTER_IDS_V44A = ADAPTER_IDS_V46F
    core.ENGINE_INDEX_BY_ARM_V44A = {
        arm: engine for wave in arm_wave_plan_v46f() for arm, engine in wave
    }
    core.arm_wave_plan_v44a = arm_wave_plan_v46f
    try:
        yield
    finally:
...
def canonical_stage_binding_v46f(logical: str) -> dict:
    directory = STAGED_BY_LOGICAL_V46F[logical]
    expected = STAGE_EXPECTED_V46F[logical]
    ...
    if (
        ...
        raise RuntimeError(f"V46F canonical staged artifact changed: {logical}")
    return observed


def replica_stage_bindings_v46f() -> dict:
    logical = {
        name: canonical_stage_binding_v46f(name)
        for name in LOGICAL_CANDIDATES_V46F
    }
    return {
        replica: {
            **logical[name],
            "replica_arm": replica,
            "adapter_id": ADAPTER_IDS_V46F[replica],
        }
        for name, replicas in LOGICAL_REPLICAS_V46F.items()
        for replica in replicas
    }


def implementation_bindings_v46f() -> dict:
    paths = {
        "runtime": Path(__file__).resolve(),
        "builder": ROOT / "build_base_vs_lora_es_ood_first_preregistration_v46f.py",
        "tests": ROOT / "test_base_vs_lora_es_ood_first_v46f.py",
        ...
        "lora_es_v43l_staging_runtime": Path(es_stage.__file__).resolve(),
        "lora_es_v43l_source_weights": es_stage.SOURCE / "adapter_model.safetensors",
        "lora_es_v43l_source_config": es_stage.SOURCE / "adapter_config.json",
        "lora_es_v43l_source_report": es_stage.REPORT,
        "lora_es_v43l_candidate_gate": es_stage.CANDIDATE_GATE,
        "lora_es_v43l_candidate_consensus": es_stage.CANDIDATE_CONSENSUS,
    ...
    for logical, directory in STAGED_BY_LOGICAL_V46F.items():
    ...
    result["v43l_source_seal"] = es_stage.source_seal_v46e()
    return result


def assert_exact_bases_v46f(metrics: dict, label: str) -> dict:
    baseline = metrics["base_a"]
    if any(metrics[arm] != baseline for arm in BASE_ARMS_V46F[1:]):
        raise RuntimeError(f"V46F six-base exact equivalence failed on {label}")
    return {
        "label": label,
        "base_arms": list(BASE_ARMS_V46F),
        "all_six_base_outputs_exact": True,
    }


def mean_shadow_metrics_v46f(shadow: dict, logical: str) -> dict:
    replicas = LOGICAL_REPLICAS_V46F[logical]
    return {
        field: sum(float(shadow[arm][field]) for arm in replicas) / 2.0
        for field in RANK_FIELDS_V46F
    }


def logical_selection_key_v46f(shadow: dict, logical: str) -> tuple:
    mean = mean_shadow_metrics_v46f(shadow, logical)
    return tuple(mean[field] for field in RANK_FIELDS_V46F) + (
        -LOGICAL_CANDIDATES_V46F.index(logical),
    )


def _replica_gate_v46f(
    shadow: dict,
    ood_qa: dict,
    prose: dict,
    raw_sink: dict,
...
def finalize_selection_v46f(
    ...
    for logical, replicas in LOGICAL_REPLICAS_V46F.items():
        replica_gates = [
            _replica_gate_v46f(shadow, ood_qa, prose, raw_sink, arm)
            for arm in replicas
        ]
        table[logical] = {
            ...
            "mean_replicated_shadow_metrics": mean_shadow_metrics_v46f(
                shadow, logical
            ),
        }
    ...
    eligible = tuple(
        logical for logical in LOGICAL_CANDIDATES_V46F
        if table[logical]["eligible"]
    )
    selected_logical = (
        max(eligible, key=lambda item: logical_selection_key_v46f(shadow, item))
        if eligible else None
    )
    selected_arm = (
        "base_a" if selected_logical is None
        else LOGICAL_REPLICAS_V46F[selected_logical][0]
    )
    baseline_key = tuple(
        shadow["base_a"][field] for field in RANK_FIELDS_V46F
    )
    selected_key = (
        None if selected_logical is None
        else logical_selection_key_v46f(shadow, selected_logical)[:-1]
    )
    ...
    return {
        ...
        "ineligible_logical_candidates": [
            item for item in LOGICAL_CANDIDATES_V46F if item not in eligible
        ],
        ...
        "exact_tie_order": list(LOGICAL_CANDIDATES_V46F),
    }


class EvaluationStateV46F:
    def __init__(self):
    ...
    def provisional(self, shadow: dict) -> dict:
        ...
        self.selection = {
            ...
            "ineligible_logical_candidates": list(LOGICAL_CANDIDATES_V46F),
...
def load_preregistration_v46f(args) -> dict:
    path = Path(args.preregistration).resolve()
    if core.file_sha256(path) != args.preregistration_sha256:
        raise RuntimeError("V46F preregistration file identity changed")
    ...
    if (
        ...
        or value.get("schema")
        != "base-vs-lora-es-v43l-replicated-ood-first-preregistration-v46f"
        or value.get("status")
        != "preregistered_before_fresh_replicated_ood_first_evaluation"
        or value.get("heldout_or_holdout_access_authorized") is not False
        or value.get("protected_semantics_inspected_during_v46f_revision")
        is not False
        or value.get("single_access_inputs") != core.PROTECTED_INPUTS_V44A
        or value.get("arms") != list(ARMS_V46F)
        or value.get("logical_candidates") != list(LOGICAL_CANDIDATES_V46F)
        or value.get("replica_staged_adapters") != replica_stage_bindings_v46f()
        or value.get("staged_adapters") != replica_stage_bindings_v46f()
        or value.get("implementation_bindings") != implementation_bindings_v46f()
        or not isinstance(value.get("cpu_preflight_expected"), dict)
    ):
        raise RuntimeError("V46F preregistration content changed")
...
def main(argv: list[str] | None = None) -> int:
    ...
    def evaluate_prose(trainer, rows, raw_sink):
        ...
        state.base_equivalence["ood_prose"] = assert_exact_bases_v46f(
            aggregate, "ood_prose"
        )
        final = finalize_selection_v46f(
    ...
    core.load_preregistration = load_preregistration_v46f
    ...
    try:
        with patched_candidate_globals_v46f():
