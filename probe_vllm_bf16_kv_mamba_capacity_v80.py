#!/usr/bin/env python3
"""Capacity-match the V78c BF16-attention/BF16-Mamba hybrid cache.

V80 reconstructs the reviewed V78c runtime around the sealed V73 data-free
probe and changes only ``gpu_memory_utilization`` from 0.500 to 0.479.  This
avoids changing or falsifying historical V74/V76/V78c certificates.  The arm
is a data-free capacity diagnostic and cannot authorize scored evaluation,
training, checkpointing, or promotion.
"""
...
REFERENCE_GPU_MEMORY_UTILIZATION_V78C = 0.50
TARGET_GPU_MEMORY_UTILIZATION_V80 = 0.479
MIN_KV_CAPACITY_TOKENS_V80 = 161_792
MIN_KV_MAX_CONCURRENCY_V80 = 79.0
V78C_PROBE_SHA256 = (
...
def canonical_sha256_v80(value: object) -> str:
    payload = json.dumps(
...
def validate_argv_v80(argv: list[str]) -> Path:
    arm_positions = [
    ...
    if (
        ...
        raise RuntimeError("V80 requires exactly the serialized-FP8 arm")
    if len(output_positions) != 1 or output_positions[0] + 1 >= len(argv):
        raise RuntimeError("V80 requires exactly one --output")
    if "--graph" in argv:
        raise RuntimeError("V80 retains the eager V78c execution contract")
    output = Path(argv[output_positions[0] + 1]).resolve()
    if output.exists():
        raise RuntimeError("V80 requires a fresh output path")
    return output


def validate_static_v80() -> dict:
    if not PREREG.is_file() or PREREG.is_symlink():
        raise RuntimeError("V80 preregistration is missing or unsafe")
    if base.file_sha256_v73(PREREG) != PREREG_FILE_SHA256:
        raise RuntimeError("V80 preregistration file identity changed")
    ...
    if (
        ...
        or canonical_sha256_v80(body) != claimed
        or value.get("schema")
        != "v80-qwen36-bf16-kv-mamba-capacity-matched-preregistration"
        or value.get("selected_runtime", {}).get("gpu_memory_utilization")
        != TARGET_GPU_MEMORY_UTILIZATION_V80
        or value.get("selected_runtime", {}).get("kv_cache_dtype") != "auto"
        or value.get("selected_runtime", {}).get("mamba_ssm_cache_dtype")
        != "bfloat16"
        or value.get("live_acceptance", {}).get("capacity", {}).get(
            "minimum_tokens_per_actor"
        )
        != MIN_KV_CAPACITY_TOKENS_V80
        or value.get("authority", {}).get("scored_or_training_authority")
        is not False
    ):
        raise RuntimeError("V80 preregistration content or authority changed")
    return value


def rightsized_kwargs_v80(kwargs: dict) -> dict:
    """Validate the V73 call and apply the exact V78c contract plus util."""
    if (
        ...
        raise RuntimeError("V80 underlying V73 engine contract changed")
    result = dict(kwargs)
    result.update(
        {
            "gpu_memory_utilization": TARGET_GPU_MEMORY_UTILIZATION_V80,
...
def resolved_runtime_v80(engine: object) -> dict:
    ...
    if (
        ...
        raise RuntimeError(
            "V80 live hybrid-cache/capacity certificate failed: "
            + json.dumps(result, sort_keys=True)
        )
    return result


def summarize_generation_timings_v80(rows: list[dict]) -> dict:
    call_plan = [
        "reference",
        "candidate",
        "candidate",
    ...
    if (
        ...
        raise RuntimeError("V80 generation timing surface changed")
    measured = rows[2:]
    ...
    p95_index = max(
        0, min(len(elapsed) - 1, (95 * len(elapsed) + 99) // 100 - 1)
    )
    return {
        "schema": "v80-data-free-generation-performance",
        "warmup_call_count": 2,
...
def upgraded_receipt_v80(
    ...
    if (
        ...
        raise RuntimeError("V80 underlying V73 receipt changed")
    ...
    if base.base.base.canonical_sha256(original) != claimed:
        raise RuntimeError("V80 underlying V73 receipt identity changed")
    resolved = dict(resolved)
    if (
        resolved.get("gpu_memory_utilization")
        != TARGET_GPU_MEMORY_UTILIZATION_V80
        or resolved.get("cache_dtype") != "auto"
        or resolved.get("mamba_ssm_cache_dtype") != "bfloat16"
        or resolved.get("kv_cache_size_tokens", 0)
        < MIN_KV_CAPACITY_TOKENS_V80
        or resolved.get("resolved_from_live_engine") is not True
    ):
        raise RuntimeError("V80 resolved runtime certificate changed")
    routed_audit = audit_base.validate_runtime_audit_v76(routed_audit)
    if routed_audit.get("parameter_residency") != PARAMETER_RESIDENCY_V80:
        raise RuntimeError("V80 live parameter-residency identity changed")
    if generation_performance.get("schema") != (
        "v80-data-free-generation-performance"
    ):
        raise RuntimeError("V80 generation performance certificate changed")

    result = original
    result["schema"] = SCHEMA_V80
    ...
    result["resolved_hybrid_cache_certificate"] = resolved
    ...
    result["single_variable_change_from_v78c"] = {
        "gpu_memory_utilization": [
            REFERENCE_GPU_MEMORY_UTILIZATION_V78C,
            TARGET_GPU_MEMORY_UTILIZATION_V80,
        ]
    }
    result["v78c_reference_identity"] = {
        "probe_sha256": V78C_PROBE_SHA256,
        "run_bundle_sha256": V78C_RUN_BUNDLE_SHA256,
        "unchanged_runtime_fields_are_bound_by_v80_preregistration": True,
    }
    result["preflight_gates"] = dict(result["preflight_gates"])
    result["preflight_gates"].update(
        {
            "bf16_attention_kv_auto_resolved": True,
            "bf16_mamba_ssm_cache_resolved": True,
            "kv_capacity_minimum_tokens_passed": True,
            "kv_capacity_minimum_tokens": MIN_KV_CAPACITY_TOKENS_V80,
            "model_config_fp32_ssm_override_warning_expected": True,
    ...
    result["preregistration_v80"] = {
        "file_sha256": PREREG_FILE_SHA256,
        "content_sha256": PREREG_CONTENT_SHA256,
    }
    result["content_sha256_before_self_field"] = (
        canonical_sha256_v80(result)
    )
    return result


def publish_v80(path: Path, value: dict) -> None:
    payload = json.dumps(value, indent=2, sort_keys=True, ensure_ascii=True) + "\n"
    temporary = path.with_name(f".{path.name}.v80-{os.getpid()}")
...
def main() -> int:
    output = validate_argv_v80(sys.argv[1:])
    validate_static_v80()
    prior_environment = {
    ...
    with tempfile.TemporaryDirectory(prefix="v80-empty-fp8-moe-") as folder:
        ...
        def capacity_matched_llm(*args, **kwargs):
            if (
                ...
                raise RuntimeError("V80 clean kernel environment changed")
            engine = original_llm(*args, **rightsized_kwargs_v80(kwargs))
            observed.append(resolved_runtime_v80(engine))
            values = engine.apply_model(audit_base.audit_fp8_routed_runtime_v76)
            if not isinstance(values, list) or len(values) != 1:
                raise RuntimeError("V80 apply_model worker cardinality changed")
    ...
    if (
        ...
        raise RuntimeError("V80 underlying capacity-matched probe failed")
    performance = summarize_generation_timings_v80(timing_rows)
    upgraded = upgraded_receipt_v80(
        json.loads(output.read_text(encoding="utf-8")),
        observed[0],
        audits[0],
        performance,
    )
    publish_v80(output, upgraded)
    print(
        json.dumps(
            {
                "schema": SCHEMA_V80,
                ...
                "gpu_memory_utilization": TARGET_GPU_MEMORY_UTILIZATION_V80,
                "kv_cache_size_tokens": observed[0]["kv_cache_size_tokens"],
