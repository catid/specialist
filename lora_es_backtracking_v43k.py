#!/usr/bin/env python3
"""CPU-only evidence and deterministic untried-scale planning for V43K."""
...
TARGET_NORM_RATIOS_V43K = (0.125, 0.0625)
V43I_NORM_RATIO = 0.5
EXPECTED_FILES_V43K = {
...
def canonical_sha256_v43k(value: object) -> str:
    return fused.canonical_sha256_v43i(value)


def _sealed(path: Path, expected: tuple[str, str]) -> dict:
    if file_sha256_v43k(path) != expected[0]:
        raise RuntimeError(f"v43k parent file identity changed: {path}")
    ...
    if (
        ...
        or canonical_sha256_v43k(compact) != expected[1]
    ):
        raise RuntimeError(f"v43k parent content identity changed: {path}")
    return value


def load_v43i_evidence_v43k(paths: dict[str, Path]) -> dict:
    if set(paths) != set(EXPECTED_FILES_V43K):
        raise ValueError("v43k V43I evidence inventory changed")
    values = {
        label: _sealed(Path(paths[label]), EXPECTED_FILES_V43K[label])
        for label in EXPECTED_FILES_V43K
    }
    ...
    if (
        ...
        raise RuntimeError("v43k required V43I negative/restoration evidence changed")
    plans = scale_plans_v43k(coefficients, projection["diagnostics"])
    return {
        "schema": "sealed-v43i-negative-evidence-for-backtracking-v43k",
        "file_and_content_sha256": {
            label: {"file_sha256": hashes[0], "content_sha256": hashes[1]}
            for label, hashes in EXPECTED_FILES_V43K.items()
        },
        ...
        "restored_runtime_values_sha256": RESTORED_RUNTIME_SHA256_V43K,
...
def scale_plans_v43k(
    ...
    if (
        ...
        raise ValueError("v43k requires the exact V43I 0.5-ratio projection")
    ...
    if not math.isclose(
        ...
        raise RuntimeError("v43k V43I coefficient norm no longer matches projection")
    plans = []
    for target in TARGET_NORM_RATIOS_V43K:
        ...
        if not math.isclose(ratio, target, rel_tol=0.0, abs_tol=1e-12):
            raise RuntimeError("v43k deterministic scaled norm changed")
        plans.append({
...
def selected_diagnostic_scale_v43k(results: list[dict]) -> float | None:
    """Validate the six-gate continuation rule and return an accepted ratio.

    The 0.0625 diagnostic is authorized only when 0.125 fails the existing
    ...
    """
    if not isinstance(results, list) or len(results) > len(TARGET_NORM_RATIOS_V43K):
        raise ValueError("v43k scale result coverage changed")
    for index, result in enumerate(results):
        if result.get("target_norm_ratio") != TARGET_NORM_RATIOS_V43K[index]:
            raise ValueError("v43k scale evaluation order changed")
        ...
        if accepted:
            if index != len(results) - 1:
                raise ValueError("v43k evaluated a smaller scale after a passing scale")
            return TARGET_NORM_RATIOS_V43K[index]
        if result.get("exact_abort_readback_passed") is not True:
            raise ValueError("v43k failed scale was not exactly aborted")
        if six_gate_passed:
            if index != len(results) - 1:
                raise ValueError(
                    "v43k evaluated a smaller scale after six-gate pass"
                )
            return None
        if index == 1 and results[0].get(
            "six_train_only_gates_passed"
        ) is not False:
            raise ValueError("v43k 0.0625 lacked a 0.125 six-gate failure")
    return None
