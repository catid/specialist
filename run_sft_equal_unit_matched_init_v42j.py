#!/usr/bin/env python3
"""Fail-closed 5.25e-5 HPO wrapper around the verified V42B runtime."""
...
def build_train_command(args: argparse.Namespace) -> list[str]:
    if args.learning_rate != LEARNING_RATE:
        raise ValueError("V42J learning-rate arm changed")
...
def validate_preregistration(args, command) -> dict:
    ...
    wrapper = value.get("implementation", {}).get("hpo_launcher_v42j")
    expected = {
        "path": str(Path(__file__).resolve()),
        "sha256": engine.file_sha256(Path(__file__).resolve()),
    }
    if wrapper != expected:
        raise RuntimeError("V42J HPO launcher binding changed")
    if (
        ...
        raise RuntimeError("V42J HPO preregistration recipe changed")
    audit["hpo_launcher_v42j"] = wrapper
    return audit
...
def rewrite_evidence(args) -> None:
    ...
    report["implementation"]["hpo_launcher_v42j"] = {
        "path": str(Path(__file__).resolve()),
        "sha256": engine.file_sha256(Path(__file__).resolve()),
    }
    ...
    attempt.update({
        "schema": "specialist-sft-matched-init-equal-unit-attempt-v42j",
