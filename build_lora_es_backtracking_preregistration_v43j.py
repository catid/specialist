#!/usr/bin/env python3
"""Seal deterministic V43J backtracking from exact V43I train-only evidence."""
...
def _v43i_preregistration() -> dict:
    if runtime.v40a.file_sha256(V43I_PREREGISTRATION) != (
        ...
        raise RuntimeError("v43j V43I preregistration file changed")
    ...
    if (
        ...
        raise RuntimeError("v43j V43I preregistration content changed")
    return value


def build_v43j() -> dict:
    parent = _v43i_preregistration()
    evidence = backtracking.load_v43i_evidence_v43j(
        runtime.V43I_EVIDENCE_PATHS
    )
    if (
        evidence["restored_master_identity"]["sha256"]
        != backtracking.RESTORED_MASTER_SHA256_V43J
        or evidence["restored_runtime_values_sha256"]
        != backtracking.RESTORED_RUNTIME_SHA256_V43J
        or [item["target_norm_ratio"] for item in evidence["scale_plans"]]
        != list(backtracking.TARGET_NORM_RATIOS_V43J)
    ):
        raise RuntimeError("v43j exact restored state/scale derivation changed")
    value = {
        "schema": "matched-lora-es-backtracking-preregistration-v43j",
        ...
        "purpose": (
            "Recover from V43I's train-domain and greedy-QA rejection by "
            "deterministically backtracking the same projected population vector, "
            "without reopening or resampling the population."
        ),
        ...
        "recipe": {
            ...
            "scale_policy": (
                "descending dyadic target norm ratios; stop and commit at first "
                "passing scale, which is therefore the largest passing scale"
            ),
        ...
        "required_gates": {
            ...
            "largest_passing_scale_only": True,
            "rank_zero_snapshot_exact_readback_if_accepted": True,
            "final_all_gpu_idle_cleanup": True,
        },
        "implementation_bindings": runtime.implementation_bindings_v43j(),
        "artifacts": {
