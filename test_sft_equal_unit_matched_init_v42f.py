...
import build_sft_equal_unit_matched_init_preregistration_v42f as builder
import run_sft_equal_unit_matched_init_v42f as runner


class OneEminusThreeHPOV42FTest(unittest.TestCase):
    def test_preregistration_and_command(self):
        value = builder.build()
        command = value["recipe"]["command"]
        self.assertEqual(value["recipe"]["learning_rate"], 1e-3)
