#!/usr/bin/env python3
"""Aggregate-only tests for the V20A negative nested-tier evidence."""
...
import build_eggroll_es_v20a_compact_evidence as evidence_v20a


def test_v20a_evidence_rebuilds_exactly_from_only_two_compact_inputs():
    value = evidence_v20a.build_evidence_v20a()
    assert json.loads(evidence_v20a.OUTPUT_PATH_V20A.read_text()) == value
    assert value["input_artifacts"]["launch_attempt"]["file_sha256"] == (
        evidence_v20a.ATTEMPT_FILE_SHA256_V20A
    )
    assert value["input_artifacts"]["compact_report"]["file_sha256"] == (
        evidence_v20a.REPORT_FILE_SHA256_V20A
    )


def test_v20a_five_by_twelve_conjunctive_endpoints_recompute_negative_gate():
    value = evidence_v20a.build_evidence_v20a()
    assert {
        name: item["observed_pass_count"]
        for name, item in value["contrasts"].items()
    } == evidence_v20a.EXPECTED_OBSERVED_PASS_COUNTS_V20A
    assert all(
        len(item["endpoints"]) == 12
        and item["bootstrap_pass_count"] == 0
        and item["preregistered_contrast_gate_passed"] is False
        for item in value["contrasts"].values()
    )
    assert value["bootstrap"]["seed"] == 20260801
    assert value["bootstrap"]["repetitions"] == 50_000
    assert value["bootstrap"]["one_sided_quantile"] == 0.05 / 60
    assert value["bootstrap"]["total_endpoint_count"] == 60


def test_v20a_runtime_provenance_raw_backend_and_all_four_gpus_are_bound():
    value = evidence_v20a.build_evidence_v20a()
    assert value["source_provenance"]["file_count"] == 7
    assert value["source_provenance"]["all_files_match_launch_commit"] is True
    checks = value["runtime_integrity"]["checks"]
    assert all(
        item is True
        for key, item in checks.items()
        if key != "union_scoring_called_or_used"
    )
    assert checks["union_scoring_called_or_used"] is False
...
def test_v20a_negative_evidence_never_authorizes_update_or_eval_and_is_compact():
    value = evidence_v20a.build_evidence_v20a()
    ...
    contrast = tampered["contrasts"]["all_tiers_vs_production"]
    first = next(iter(contrast["endpoints"]))
    contrast["endpoints"][first]["bootstrap_noninferiority_pass"] = True
    tampered["content_sha256_before_self_field"] = evidence_v20a.canonical_sha256({
