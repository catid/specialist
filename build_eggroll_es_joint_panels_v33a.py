...
CANDIDATE_PROJECTION_PATH = (
    ROOT / "experiments/eggroll_es_hpo/dataset_candidates/v364/"
    "projection_report_v364.json"
).resolve()
CANDIDATE_TEST_PATH = (
    ROOT / "experiments/eggroll_es_hpo/dataset_candidates/v364/"
    "test_frozen_candidate_v364.py"
).resolve()
PRODUCTION_PATH = (ROOT / "data/train_qa_curated_v1.jsonl").resolve()
OUTPUT_PATH = (
    ROOT / "experiments/eggroll_es_hpo/train_panel_sampling_v33a/"
    "paired_data_version_panels_v33a.json"
).resolve()
...
CANDIDATE_PROJECTION_SHA256 = "4ac00be2c11c6c8fc0aed37a92035ac8e50928a14b2b5ae19c4408fcaed4ee85"
CANDIDATE_TEST_SHA256 = "749a143a80f39d14bec3d6092d9bc46856b1f7f5bac26e614ef720b7d64a7bea"
...
EXPECTED_PAIRED_ASSIGNMENT_ROOT_SHA256 = (
    "1091a1be02633a2e18e5b96904454ef96dd97374142204785b84e20a328b7549"
)
EXPECTED_SELECTED_UNIT_ROOT_SHA256 = (
    "49b0ca045fabd99c8b41d2ca3f972a046808563270170041271c8982c921c406"
)
EXPECTED_RUNTIME_FRAME_CONTENT_SHA256 = (
    "49e64e608d52a125689abe18ae85a3332cd9c908e21123402ab425038a22cd01"
)
...
def load_bound_rows():
    expected = {
        ...
        CANDIDATE_PROJECTION_PATH: CANDIDATE_PROJECTION_SHA256,
    ...
    if any(file_sha256(path) != digest for path, digest in expected.items()):
        raise RuntimeError("v33a frozen data or conflict implementation changed")
    freeze = json.loads(CANDIDATE_MANIFEST_PATH.read_text(encoding="utf-8"))
    if (
        freeze.get("schema")
        != "eggroll-es-immutable-train-candidate-manifest-v364"
        or freeze.get("candidate", {}).get("file_sha256") != CANDIDATE_SHA256
        or freeze.get("candidate", {}).get("rows") != CANDIDATE_ROWS
        or freeze.get("projection_report", {}).get("file_sha256")
        != CANDIDATE_PROJECTION_SHA256
        or freeze.get("firewall", {}).get(
            "heldout_validation_ood_eval_or_benchmark_content_opened"
        ) is not False
        or freeze.get("firewall", {}).get("runtime_launch_authorized") is not False
    ):
        raise RuntimeError("v33a exact v364 freeze manifest changed")
    committed = {
        CANDIDATE_PATH: CANDIDATE_FREEZE_COMMIT,
        CANDIDATE_MANIFEST_PATH: CANDIDATE_FREEZE_COMMIT,
        CANDIDATE_PROJECTION_PATH: CANDIDATE_FREEZE_COMMIT,
        CANDIDATE_TEST_PATH: CANDIDATE_FREEZE_COMMIT,
        PRODUCTION_PATH: PRODUCTION_SOURCE_COMMIT,
    }
    for path, commit in committed.items():
        raw = subprocess.check_output(
            ["git", "show", f"{commit}:{path.relative_to(ROOT).as_posix()}"],
            cwd=ROOT,
        )
        if hashlib.sha256(raw).hexdigest() != expected[path]:
            raise RuntimeError("v33a committed train source identity changed")
    ...
    if len(candidate) != CANDIDATE_ROWS or len(production) != PRODUCTION_ROWS:
        raise RuntimeError("v33a frozen row counts changed")
...
def representative(unit_id, side, members, stratum):
    ...
    selected = min(
        ...
        key=lambda item: canonical_sha256({
            "schema": "eggroll-es-v33a-fixed-side-representative",
...
def build_joint_units(candidate, production):
    ...
    for members in components.values():
        ...
        unit_id = canonical_sha256({
            "schema": "eggroll-es-joint-conflict-unit-v33a",
            "members": member_ids,
        })
        documents = {}
        for side in ("candidate", "production"):
        ...
        if eligible_documents:
            shared_document = min(
                ...
                key=lambda document: canonical_sha256({
                    "schema": "eggroll-es-v33a-shared-document-selection",
                    "seed": REPRESENTATIVE_SEED,
                    "unit_id": unit_id,
        ...
        if side_contract["candidate"]["classified_stratum"] != stratum:
            raise RuntimeError("v33a candidate representative missed its stratum")
        paired.append({
            "unit_id": unit_id,
            "stratum": stratum,
...
def _build_runtime_manifest_v33a():
    ...
    if (
        ...
        raise RuntimeError("v33a joint paired capacity changed or is insufficient")
    by_stratum = {
        name: sorted(
    ...
    for panel_index, panel_name in enumerate(PANEL_NAMES):
        ...
        if len(selected) != PANEL_SIZE:
            raise RuntimeError("v33a paired panel size changed")
    ...
    if len(selected_ids) != len(set(selected_ids)):
        raise RuntimeError("v33a panels are not globally conflict-unit disjoint")
    if len(selected_ids) != len(PANEL_NAMES) * PANEL_SIZE:
        raise RuntimeError("v33a selected paired-unit coverage changed")

    manifest = {
        "schema": "eggroll-es-paired-data-version-panel-manifest-v33a",
        ...
        "inputs": {
            "candidate": {
                ...
                "projection_report_path": str(CANDIDATE_PROJECTION_PATH),
                "projection_report_file_sha256": CANDIDATE_PROJECTION_SHA256,
                "freeze_test_path": str(CANDIDATE_TEST_PATH),
                "freeze_test_file_sha256": CANDIDATE_TEST_SHA256,
            },
            "production": {
        ...
        "panels": panels,
        "separation": {
            "v33a_role": "matched_data_version_compatibility_only",
            "future_role": "separate_full_candidate_hpo_only_if_v33a_passes",
    ...
    _validate_runtime_manifest_v33a(manifest)
    return manifest


def _validate_runtime_manifest_v33a(manifest):
    joint = manifest.get("joint_frame", {})
    if (
        manifest.get("schema")
        != "eggroll-es-paired-data-version-panel-manifest-v33a"
        ...
        or joint.get("paired_assignment_root_sha256")
        != EXPECTED_PAIRED_ASSIGNMENT_ROOT_SHA256
        or joint.get("selected_paired_unit_count") != 195
        or joint.get("reserve_paired_unit_count") != 10
        or joint.get("selected_unit_id_root_sha256")
        != EXPECTED_SELECTED_UNIT_ROOT_SHA256
        or [panel.get("name") for panel in manifest.get("panels", [])]
        != list(PANEL_NAMES)
    ):
        raise RuntimeError("v33a paired manifest contract changed")
    ...
    if (
        ...
        raise RuntimeError("v33a paired item identity or weighting changed")
    return manifest


def _aggregate_from_runtime_v33a(runtime_manifest):
    """Drop every per-unit identity before producing a persistent artifact."""
    _validate_runtime_manifest_v33a(runtime_manifest)
    if (
        runtime_manifest.get("content_sha256_before_self_field")
        != EXPECTED_RUNTIME_FRAME_CONTENT_SHA256
    ):
        raise RuntimeError("v33a transient runtime frame identity changed")
    value = {
        "schema": "eggroll-es-paired-data-version-panel-aggregate-v33a",
        "status": "sealed_train_only_aggregate_no_gpu_or_update_authorization",
        "inputs": runtime_manifest["inputs"],
        "joint_frame": runtime_manifest["joint_frame"],
        "panel_contract": runtime_manifest["panel_contract"],
        "panels": [
            {
                key: panel[key]
                for key in (
                    "name", "role", "rows_per_side_per_direction",
                    "stratum_counts", "ordered_unit_identity_sha256",
                    "ordered_side_row_identity_sha256",
                )
            }
            for panel in runtime_manifest["panels"]
        ],
        "runtime_frame_content_sha256": runtime_manifest[
            "content_sha256_before_self_field"
        ],
        "separation": {
            **runtime_manifest["separation"],
            "contains_per_unit_ids_hashes_indices_documents_strata_weights_or_anchors": False,
            "runtime_reconstructs_unit_details_transiently_from_frozen_train_sources": True,
...
def build_runtime_manifest_v33a():
    return _build_runtime_manifest_v33a()


def build_manifest():
    return _aggregate_from_runtime_v33a(_build_runtime_manifest_v33a())
...
def validate_manifest(manifest):
    if (
        ...
        != "eggroll-es-paired-data-version-panel-aggregate-v33a"
        or manifest.get("content_sha256_before_self_field")
        != canonical_sha256({
            key: value for key, value in manifest.items()
            if key != "content_sha256_before_self_field"
        })
        or manifest.get("separation", {}).get(
            "contains_per_unit_ids_hashes_indices_documents_strata_weights_or_anchors"
        ) is not False
        or manifest.get("separation", {}).get("runtime_unit_details_persisted")
        is not False
    ):
        raise RuntimeError("v33a aggregate paired manifest contract changed")
    expected = build_manifest()
    if manifest != expected:
        raise RuntimeError("v33a aggregate paired manifest identity changed")
    return manifest
...
def exclusive_write(path, value):
    ...
    if path != OUTPUT_PATH:
        raise ValueError("v33a output path changed")
    path.parent.mkdir(parents=True, exist_ok=True)
    try:
        descriptor = os.open(path, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
    except FileExistsError as error:
        raise ValueError("v33a paired manifest already exists") from error
...
def main(argv=None):
    ...
    result = {
        "schema": "eggroll-es-paired-panel-build-v33a",
