...
import build_v59_vs_v434_ood_preregistration_v60 as builder
import run_v59_vs_v434_replicated_ood_only_v60 as subject
...
def test_v56_sources_and_stages_are_exact_and_train_only():
    ...
    v59 = subject._source_seal("v59_candidate")
    assert v434["completed_steps"] == 48
    assert v434["shadow_ood_holdout_or_heldout_opened"] is False
    assert v59["all_nine_train_endpoint_gates_passed"] is True
    assert v59["selected_target_norm_ratio"] == 0.25
    assert v59["shadow_ood_holdout_or_heldout_opened"] is False
...
def test_v56_direct_gate_requires_both_replicas_and_all_paired_lcbs():
    ...
    failing_qa["v59_candidate_b"] = _qa_summary(
        reward=0.0, exact=0, nonzero=0
    )
    failing_raw["ood_qa"]["v59_candidate_b"] = _qa_raw(
...
def test_v56_prereg_documents_exact_document_identity_definitions_and_gates():
    ...
    gates = value["direct_v59_vs_v434_gates"]
    assert gates["both_v59_replicas_must_independently_pass"] is True
