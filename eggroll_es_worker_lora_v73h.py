#!/usr/bin/env python3
"""V73H process-start guard bootstrap for controller and real Ray actors.

Both controller and actor guards are installed by sitecustomize before runtime
imports. The exact actor environment and inherited controller PID select a
distinct actor mechanism before Ray/vLLM can preload application modules.
"""
...
ACTOR_SITE_MECHANISM = "ray_actor_sitecustomize_pre_runtime_imports"
GUARD_PATH = (
    Path(__file__).resolve().parent
    / "v73h_sitecustomize/v73e_path_open_guard.py"
).resolve()
PARENT_MODULES = (
...
_preinstalled_mechanism = _guard.installation_mechanism()

if _guard_was_preinstalled and _preinstalled_mechanism == "controller_sitecustomize":
    _require(
        os.environ.get("SPECIALIST_V73E_CONTROLLER_GUARD_PID")
        == str(os.getpid())
...
elif _guard_was_preinstalled and _preinstalled_mechanism == ACTOR_SITE_MECHANISM:
    _require(
        ...
        not in {None, str(os.getpid())},
        "V73E actor bootstrap environment or import order changed",
    )
    _PROCESS_ROLE = "ray_actor_worker_extension"
    _BOOTSTRAP_MECHANISM = ACTOR_SITE_MECHANISM
else:
    raise RuntimeError("V73H process-start sitecustomize guard is absent or invalid")
...
import qwen36_v73h_exact_phase_profiler_contract as contract_v73e
...
class LoRAAdapterStateWorkerExtensionV73E(LoRAAdapterStateWorkerExtensionV72):
    ...
    def systems_only_path_guard_receipt_v73e(self):
        ...
        result = {
            ...
            "actor_sitecustomize_installed_before_runtime_imports": (
                _PROCESS_ROLE != "ray_actor_worker_extension"
                or _BOOTSTRAP_MECHANISM == ACTOR_SITE_MECHANISM
            ),
            "parent_modules_covered_by_process_start_guard": (
                _PROCESS_ROLE != "ray_actor_worker_extension"
                or _guard_was_preinstalled
            ),
