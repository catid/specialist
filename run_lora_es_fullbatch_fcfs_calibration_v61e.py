#!/usr/bin/env python3
"""Matched four-GPU V61E synchronous full-batch FCFS null calibration."""
...
import run_lora_es_singleton_fcfs_calibration_v61d as runtime_v61d


ROOT = Path(__file__).resolve().parent
EXPERIMENT = "v61e_v434_fullbatch_fcfs_paired_evaluator_calibration"
...
FAILURE = (RUN_DIR / "failure_v61e.json").resolve()
GPU_LOG = (RUN_DIR / "gpu_activity_v61e.jsonl").resolve()
PREREGISTRATION = (
    ROOT / "experiments/eggroll_es_hpo/preregistrations/"
    "v434_fullbatch_fcfs_paired_evaluator_calibration_v61e.json"
).resolve()
AUDIT = support_audit.OUTPUT
AUDIT_FILE_SHA256 = (
    "02155a75a3ad6183665fa64ed3899c2d0aab8d85287c29327efe416ad1d247a9"
)
AUDIT_CONTENT_SHA256 = (
    "8d8837f60032074d4b9955385bfe0fe307806f9b5e4d2171aefd115a8665dc37"
)
WORKER_EXTENSION_V61E = runtime_v52.WORKER_EXTENSION_V52
MATCHED_SOURCE_IDENTITIES_V61E = {
    "v61c_evidence": {
        "file_sha256": (
...
def _read_support_audit_v61e() -> dict:
    if support_audit.file_sha256_v61e(AUDIT) != AUDIT_FILE_SHA256:
        raise RuntimeError("v61e installed-vLLM support audit file changed")
    ...
    if (
        ...
        or support_audit.canonical_sha256_v61e(compact) != AUDIT_CONTENT_SHA256
        or value.get("status") != "supported"
        or value.get("fullbatch_fcfs_controls_supported") is not True
        ...
        or value.get("effective_request_batch_size") != 68
        or value.get("global_batch_invariance_claimed") is not False
    ):
        raise RuntimeError("v61e installed-vLLM full-batch FCFS support changed")
    observed_sources = {
        key: support_audit.file_sha256_v61e(path)
        for key, path in support_audit.SOURCE_PATHS.items()
    }
    if observed_sources != value.get("source_file_sha256"):
        raise RuntimeError("v61e installed-vLLM source identities changed")
    return value


def implementation_bindings_v61e() -> dict:
    paths = {
        "runtime_v61e": Path(__file__).resolve(),
        "preregistration_builder_v61e": (
            ROOT / "build_lora_es_fullbatch_fcfs_preregistration_v61e.py"
        ),
        "analysis_v61e": Path(analysis.__file__).resolve(),
        "tests_v61e": ROOT / "test_lora_es_fullbatch_fcfs_calibration_v61e.py",
        "support_audit_v61e": Path(support_audit.__file__).resolve(),
        "runtime_v61d": Path(runtime_v61d.__file__).resolve(),
        "analysis_v61d": Path(runtime_v61d.analysis.__file__).resolve(),
        "finalizer_v61d": (
            ROOT / "finalize_lora_es_singleton_fcfs_evidence_v61d.py"
        ),
        "runtime_v61c": Path(runtime_v61c.__file__).resolve(),
        "analysis_v61c": Path(analysis.v61c.__file__).resolve(),
        "finalizer_v61c": (
            ROOT / "finalize_lora_es_paired_null_evidence_v61c.py"
        ),
    ...
    return {
        ...
        "matched_numeric_sources_without_reopening": (
            MATCHED_SOURCE_IDENTITIES_V61E
        ),
        ...
        "train_semantics_model_gpu_prior_evidence_or_protected_paths_opened": False,
    }


def _artifacts_v61e() -> dict:
    return {
        "attempt": str(ATTEMPT),
...
def load_preregistration_v61e(args) -> dict:
    path = Path(args.preregistration).resolve()
    if runtime_v61a.file_sha256_v61a(path) != args.preregistration_sha256:
        raise RuntimeError("v61e preregistration file identity changed")
    ...
    if (
        ...
        or analysis.canonical_sha256_v61e(compact)
        != args.preregistration_content_sha256
        or value.get("schema")
        ...
        or value.get("matched_numeric_sources")
        != MATCHED_SOURCE_IDENTITIES_V61E
        ...
        or recipe.get("label_plan") != analysis.LABEL_PLAN_V61E
        or recipe.get("request_type_order") != analysis.REQUEST_TYPE_ORDER_V61E
        or recipe.get("pair_periods")
        != [list(pair) for pair in analysis.PAIR_PERIODS_V61E]
        or recipe.get("generation_params_without_seed")
        != analysis.GENERATION_PARAMS_WITHOUT_SEED_V61E
        or recipe.get("teacher_forced_params_without_seed")
        != analysis.TEACHER_FORCED_PARAMS_WITHOUT_SEED_V61E
        ...
        or recipe.get("worker_extension") != WORKER_EXTENSION_V61E
        or value.get("implementation_bindings") != implementation_bindings_v61e()
        or value.get("artifacts") != _artifacts_v61e()
        or value.get("runtime") != design_v52.RUNTIME_V52
        or value.get("raw_question_answer_or_generation_text_may_be_persisted")
        is not False
    ):
        raise RuntimeError("v61e preregistration contract changed")
    _read_support_audit_v61e()
    return value


def _make_trainer_v61e(prereg: dict, prior):
    v40a = prior.v40a
    parent = v40a.base.load_trainer()
    expected_tuned_content = prereg["runtime"]["tuned_table_content_sha256"]

    class FullbatchFCFSTrainerV61E(parent):
        def launch_engines(
            self,
            num_engines=4,
            ...
            if int(num_engines) != 4 or int(n_gpu_per_vllm_engine) != 1:
                raise RuntimeError("v61e requires four TP1 engines")
            import ray
            ...
            class FullbatchFCFSLLMV61E(ESNcclLLM):
                def runtime_identity_v61e(self):
                    import torch
                    ...
                    raw = ray.get_gpu_ids()
                    if len(raw) != 1:
                        raise RuntimeError("v61e actor does not own exactly one GPU")
                    ...
                    if (
                        ...
                        raise RuntimeError("v61e actor determinism identity changed")
                    return {
            ...
            kwargs = engine_kwargs_v61e(v40a, precision)
            engines = [ray.remote(
                num_cpus=0,
                num_gpus=1,
                scheduling_strategy=strategy,
            )(FullbatchFCFSLLMV61E).options(runtime_env={"env_vars": {
    ...
    v40a.EXPERIMENT, v40a.RUN_DIR, v40a.WORKER_EXTENSION = (
        EXPERIMENT,
        RUN_DIR,
        WORKER_EXTENSION_V61E,
    )
    try:
        trainer = FullbatchFCFSTrainerV61E(
...
def engine_kwargs_v61e(v40a, precision="bfloat16") -> dict:
    return {
        "model": str(v40a.MODEL),
        "tensor_parallel_size": 1,
        "worker_extension_cls": WORKER_EXTENSION_V61E,
        "dtype": precision,
...
def _lora_request_v61e(prior):
    from vllm.lora.request import LoRARequest
    return LoRARequest(
        "v434_fullbatch_fcfs_paired_evaluator_v61e",
        1,
...
def build_evidence_v61e(rows, periods, state_receipts) -> dict:
    ...
    analysis.validate_evidence_v61e(value)
    return value


def main(argv=None) -> int:
    args = parser().parse_args(argv)
    prereg = load_preregistration_v61e(args)
    if args.dry_run:
        if args.execute:
            raise RuntimeError("v61e dry-run and execute are mutually exclusive")
        print(json.dumps({
            "schema": prereg["schema"],
            "content_sha256": prereg["content_sha256_before_self_field"],
            "support_audit_file_sha256": AUDIT_FILE_SHA256,
            "fullbatch_fcfs_controls_supported": True,
        ...
        return 0
    if not args.execute:
        raise RuntimeError("v61e live path requires --execute")
    if os.environ.get("VLLM_BATCH_INVARIANT") not in (None, "0"):
        raise RuntimeError("v61e requires batch-invariant mode absent or false")
    runtime_v52.require_live_interpreter_v52()
    if ATTEMPT.exists() or RUN_DIR.exists():
        raise RuntimeError("v61e requires fresh artifact paths")
    ...
    attempt = runtime_v61a.self_hashed_v61a({
        "schema": "v61e-fullbatch-fcfs-paired-evaluator-attempt",
        ...
        "support_audit_file_sha256": AUDIT_FILE_SHA256,
        "runtime_determinism_controls": dict(analysis.RUNTIME_CONTROLS_V61E),
        "matched_numeric_sources": MATCHED_SOURCE_IDENTITIES_V61E,
        "preflight": preflight,
    ...
    try:
        ...
        trainer, saved = _make_trainer_v61e(prereg, prior)
        actor_ids = trainer._resolve([
            engine.runtime_identity_v61e.remote() for engine in trainer.engines
        ])
        if any({
            key: item.get(key) for key in analysis.RUNTIME_CONTROLS_V61E
        } != analysis.RUNTIME_CONTROLS_V61E for item in actor_ids):
            raise RuntimeError("v61e four-actor runtime controls changed")
        ...
        monitor.start()
        request = _lora_request_v61e(prior)
        phase.value = "activate_v434_lora_slot"
        if v40a._rpc_all(trainer, "add_lora", (request,)) != [True] * 4:
            raise RuntimeError("v61e four-actor LoRA activation failed")
        ...
        for period_index in range(4):
            ...
            for request_type in analysis.REQUEST_TYPE_ORDER_V61E[str(period_index)]:
                ...
                if request_type == "generation":
                    batches = trainer._resolve([
                        engine.generate.remote(
                            prompts,
                            runtime_v61c._generation_params_v61c(),
                            use_tqdm=False,
                            lora_request=request,
                        ) for engine in trainer.engines
                    ])
                    if len(batches) != 4 or any(
                        len(batch) != 68 for batch in batches
                    ):
                        raise RuntimeError("v61e generation coverage changed")
                    period[request_type] = [
                    ...
                    batches = trainer._resolve([
                        engine.generate.remote(
                            teacher_requests,
                            runtime_v61c._teacher_params_v61c(),
                            use_tqdm=False,
                            lora_request=request,
                        ) for engine in trainer.engines
                    ])
                    if len(batches) != 4 or any(
                        ...
                        raise RuntimeError("v61e teacher coverage changed")
                    period[request_type] = [
            ...
            if before != after or before != installed:
                raise RuntimeError("v61e adapter state changed across a period")
        ...
        evidence = build_evidence_v61e(rows, periods, state_receipts)
        null_analysis = analysis.build_analysis_v61e(evidence)
        stop.set()
        monitor.join(timeout=10)
        if monitor.is_alive() or not failures.empty():
            raise RuntimeError("v61e GPU monitor failed") from (
        ...
        if not FAILURE.exists():
            ...
            failure.update({
                "schema": "v61e-fullbatch-fcfs-paired-evaluator-failure",
