#!/usr/bin/env python3
"""Seal the unlaunched V47A SFT run on holdout-blind v430 fold 3."""
...
import run_sft_equal_unit_matched_init_v47a as launcher
...
PREREGISTRATION = RUN_DIR / "preregistration_v47a.json"
...
def argument_vector(
    ...
    return [
        ...
        "--dataset-rows", str(refresh.EXPECTED["train_rows"]),
        ...
        "--stdout-log", str(RUN_DIR / "stdout_v47a.log"),
        "--gpu-log", str(RUN_DIR / "gpu_activity_v47a.jsonl"),
        "--report", str(RUN_DIR / "runtime_report_v47a.json"),
        "--attempt-report", str(RUN_DIR / "attempt_v47a.json"),
...
def build() -> dict:
    ...
    dataset = engine.validate_train_dataset(
        refresh.TRAIN, refresh.EXPECTED["train_sha256"],
        refresh.EXPECTED["train_rows"],
    )
    ...
    if (
        engine.file_sha256(refresh.MANIFEST)
        != "ced1c2f48a758d6ed3318eeae2575f9a3425fc4da669f4e372701ee9d54c4106"
        ...
        raise RuntimeError("V47A refreshed fold manifest changed")
    schedule = json.loads(schedule_builder.OUTPUT.read_text(encoding="utf-8"))
    if (
        engine.file_sha256(schedule_builder.OUTPUT)
        != "0dc7f6d9dc97d5cd2b3c49fc962968a2eaf28804a3e79f6805bb6bee58357b20"
        or schedule.get("content_sha256_before_self_field")
        != schedule_builder.EXPECTED_CONTENT_SHA256
        or schedule["step_schedule"]["expected_optimizer_steps"] != 48
        or schedule["refresh_delta"]["unexplained_unit_identity_changes"] != 0
    ):
        raise RuntimeError("V47A refresh/schedule evidence changed")
    ...
    bindings = {
        "launcher": engine.file_sha256(Path(launcher.__file__).resolve()),
        "engine": engine.file_sha256(Path(engine.__file__).resolve()),
        "sft": engine.file_sha256(launcher.SFT_SCRIPT),
        "objective": engine.file_sha256(launcher.OBJECTIVE_SCRIPT),
        "sft_v42a_source": engine.file_sha256(launcher.V42A_SFT_SCRIPT),
        "sft_v42b_loader": engine.file_sha256(launcher.V42B_LOADER_SCRIPT),
        ...
        "v42i_preregistration": engine.file_sha256(
            launcher.V42I_PREREGISTRATION
        ),
    ...
    result = {
        "schema": "specialist-sft-v430-refresh-preregistration-v47a",
        "status": "sealed_unlaunched_holdout_blind",
        "experiment_name": (
            "sft_v47a_matched_init_equal_unit_fold3_v430_lr5p5e5"
        ),
        ...
        "v42i_recipe_parent": {
            ...
            "inherited_exactly": [
                ...
                "per-device batch 7, world size 4, and 48 optimizer steps",
            ...
            "changed_only": (
                "content-addressed fold-3 train partition v412 -> holdout-blind "
                "v430; explicit max_steps=48 restates V42I's realized optimizer "
                "budget and is terminal if scheduling behavior drifts"
            ),
        },
        "artifacts": {
            "output_dir": str(OUTPUT_DIR),
            "stdout_log": str(RUN_DIR / "stdout_v47a.log"),
            "gpu_log": str(RUN_DIR / "gpu_activity_v47a.jsonl"),
            "report": str(RUN_DIR / "runtime_report_v47a.json"),
            "attempt_report": str(RUN_DIR / "attempt_v47a.json"),
        },
        "fold_binding": {
            ...
            "shadow_rows_aggregate_only": 72,
            "shadow_conflict_units_aggregate_only": 51,
            "all_edge_identity_intersections": 0,
            "refresh_delta_evidence_file_sha256": bindings["schedule_evidence"],
        ...
        "recipe": {
            ...
            "source_rows_not_emitted_per_dataloader_epoch": 11,
            "complete_per_epoch_row_coverage_claimed": False,
            "explicit_max_steps_cap": 48,
            "max_steps_is_terminal_authority": True,
            "expected_completed_dataloader_epochs": 3.0,
            "total_examples_emitted": 1344,
            "row_equivalent_passes": 2.9281045751633987,
        ...
        "access_firewall": {
            "training_input": "refreshed fold-3 train file only",
        ...
        "selection_firewall": {
            "this_run_authorizes": "one train-only V47A state and runtime evidence",
...
def main() -> None:
    ...
    if PREREGISTRATION.exists():
        raise RuntimeError("V47A preregistration already exists")
