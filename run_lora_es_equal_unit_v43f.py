#!/usr/bin/env python3
"""Calibrated, replicated, train-only matched LoRA EGGROLL-ES V43F.

V43F retains the exact FP32 four-rank state protocol and replaces an arbitrary
cross-actor score equality tolerance with a preregistered numerical calibration
and equivalence design.  Only the frozen fold-3 training panel is scored.  No
shadow-dev, external evaluation, OOD, or holdout path is opened.
"""
...
EXPERIMENT = "v43f_matched_lora_es_fold3_pop8_calibrated_retry"
RUN_DIR = (ROOT / "experiments/eggroll_es_hpo/runs" / EXPERIMENT).resolve()
ATTEMPT = (RUN_DIR.parent / f".{EXPERIMENT}.attempt.json").resolve()
REPORT = (RUN_DIR / "matched_lora_es_report_v43f.json").resolve()
GPU_LOG = (RUN_DIR / "gpu_activity_v43f.jsonl").resolve()
SNAPSHOT = (RUN_DIR / "adapter_step1_v43f").resolve()
CALIBRATION_ARTIFACT = (RUN_DIR / "numeric_calibration_v43f.json").resolve()
RELIABILITY_ARTIFACT = (RUN_DIR / "population_reliability_v43f.json").resolve()
POST_UPDATE_ARTIFACT = (RUN_DIR / "post_update_consensus_v43f.json").resolve()
...
CALIBRATION_SIGMA = numeric.calibration_sigma_v43f(ALPHA, POPULATION_SIZE)
...
def _paths() -> dict[str, Path]:
    return {
        ...
        "builder": ROOT / "build_lora_es_equal_unit_preregistration_v43f.py",
...
def load_preregistration(args: argparse.Namespace) -> dict:
    ...
    if v40a.file_sha256(path) != args.preregistration_sha256:
        raise RuntimeError("v43f preregistration file identity changed")
    ...
    if (
        ...
        or value.get("schema") != "matched-lora-es-preregistration-v43f"
        ...
        != numeric.CALIBRATION_NOISE_SEED_V43F
        or calibration.get("synthetic_sigma") != CALIBRATION_SIGMA
        or calibration.get("warmup_repeats") != numeric.CALIBRATION_WARMUPS_V43F
        or calibration.get("retained_repeats_per_actor")
        != numeric.CALIBRATION_REPEATS_V43F
        or calibration.get("bootstrap_resamples")
        != numeric.BOOTSTRAP_RESAMPLES_V43F
        or calibration.get("bootstrap_seed")
        != numeric.CALIBRATION_BOOTSTRAP_SEED_V43F
        or calibration.get("familywise_confidence")
        != numeric.BOOTSTRAP_CONFIDENCE_V43F
        or recipe.get("signed_replicates_per_direction") != 2
        or recipe.get("minimum_response_reliability")
        != numeric.RELIABILITY_MINIMUM_V43F
        or recipe.get("maximum_calibration_bound_fraction_of_response_std")
        != numeric.CALIBRATION_TO_RESPONSE_MAX_FRACTION_V43F
        or consensus.get("retained_repeats_per_actor")
        != numeric.POST_UPDATE_REPEATS_V43F
        or consensus.get("bootstrap_resamples") != numeric.BOOTSTRAP_RESAMPLES_V43F
        or consensus.get("bootstrap_seed")
        != numeric.POST_UPDATE_BOOTSTRAP_SEED_V43F
        or consensus.get("familywise_confidence")
        != numeric.BOOTSTRAP_CONFIDENCE_V43F
    ):
        raise RuntimeError("v43f preregistration contract changed")
    ...
    for label in ("dataset", "matched_initialization", "staged_initialization"):
        if any(token in str(recipe[label]).lower() for token in forbidden):
            raise RuntimeError(f"v43f forbidden selection path in {label}")
    return value
...
def augment_unit_membership_v43f(bundle: dict) -> dict:
    ...
    for unit in manifest["content_free_unit_commitments"]:
        ...
        for row_sha in unit["row_sha256"]:
            if row_sha in row_to_unit:
                raise RuntimeError("v43f train row belongs to multiple conflict units")
            row_to_unit[row_sha] = {
    ...
    if (
        ...
        != numeric.EXPECTED_CONFLICT_UNITS_V43F
        or len(memberships) != 448
    ):
        raise RuntimeError("v43f train conflict-unit membership changed")
    bundle = dict(bundle)
    bundle["unit_membership_v43f"] = memberships
    bundle["unit_membership_sha256_v43f"] = v40a.canonical_sha256([
...
def score_batch_detailed_v43f(bundle: dict, dense_items: list, outputs: list) -> dict:
    ...
    if len(rewards) != 448 or len(bundle["unit_membership_v43f"]) != len(rewards):
        raise RuntimeError("v43f score coverage changed")
    grouped: dict[str, dict] = {}
    for reward, membership in zip(
        rewards, bundle["unit_membership_v43f"], strict=True,
    ):
    ...
    if len(units) != numeric.EXPECTED_CONFLICT_UNITS_V43F:
        raise RuntimeError("v43f scored conflict-unit count changed")
    ...
    if not math.isclose(equal_mean, unit_mean, rel_tol=0.0, abs_tol=1e-12):
        raise RuntimeError("v43f equal-unit aggregation changed")
    aggregate = {
...
def _generate_actor_scores(trainer, bundle, dense_items, requests) -> list[dict]:
    ...
    if len(batches) != 4 or any(len(batch) != 448 for batch in batches):
        raise RuntimeError("v43f replicated train-score coverage changed")
    return [
        {
            "actor_rank": actor_rank,
            **score_batch_detailed_v43f(bundle, dense_items, batch),
        }
...
def _exact_base_score(trainer, bundle, dense_items, requests) -> dict:
    ...
    if len(set(score_hashes)) != 1:
        raise RuntimeError("v43f zero-effect base score differs across actors")
    return {
        "consensus": actors[0],
        "actors": actors,
...
def _persist_phase(path: Path, value: dict) -> dict:
    ...
    if (
        ...
        raise RuntimeError(f"v43f phase artifact changed on readback: {path}")
    return sealed
...
def _calibrate_numeric_path(
    ...
    if len(candidate_hashes) != 1 or len(runtime_hashes) != 1:
        raise RuntimeError("v43f calibration state/materialization differs across ranks")
    records = None
    try:
        records = _score_repeats(
            ...
            dense_items,
            requests,
            warmups=numeric.CALIBRATION_WARMUPS_V43F,
            retained=numeric.CALIBRATION_REPEATS_V43F,
        )
    finally:
    ...
    if any(item["restored_identity"]["sha256"] != master_sha for item in restored):
        raise RuntimeError("v43f calibration exact restore changed master identity")
    bounds = numeric.calibration_bootstrap_bounds_v43f(records)
    artifact = _persist_phase(CALIBRATION_ARTIFACT, {
        "schema": "matched-lora-es-numeric-calibration-v43f",
        "status": "complete_before_population",
        "synthetic_state": {
            "seed": numeric.CALIBRATION_NOISE_SEED_V43F,
        ...
        "warmup_repeats_discarded": numeric.CALIBRATION_WARMUPS_V43F,
...
def _standardize(values: list[float]) -> tuple[list[float], dict]:
    ...
    if len(values) != POPULATION_SIZE or not all(math.isfinite(v) for v in values):
        raise RuntimeError("v43f response vector is incomplete or non-finite")
    ...
    coefficients = (
        ...
        if std == 0.0
            ...
            (value - mean) / (std + STANDARDIZATION_EPSILON)
            for value in values
        ]
    )
...
def _replicated_population(
    ...
    scores: dict[str, list[list[dict | None]]] = {
        "plus": [[None, None] for _ in range(POPULATION_SIZE)],
        "minus": [[None, None] for _ in range(POPULATION_SIZE)],
    }
    ...
    restorations = []
    assignments = []
    for wave_start in range(0, POPULATION_SIZE, 4):
        cyclic = numeric.balanced_cyclic_assignments_v43f(wave_start)
        assignments.extend(cyclic)
        for label, sign in (("plus", 1), ("minus", -1)):
            for replicate, assignment in enumerate(cyclic):
                values = trainer._resolve([
                    trainer.engines[item["actor_rank"]].collective_rpc.remote(
                        "materialize_antithetic_adapter_v41a",
                        args=(
                            SEEDS[item["direction_index"]],
                            SIGMA,
                            sign,
                            master_sha,
                        ),
                ...
                if len(values) != 4 or any(len(value) != 1 for value in values):
                    raise RuntimeError("v43f replicated perturbation RPC coverage changed")
                ...
                try:
                    ...
                    restored = v40a._rpc_all(
                        trainer, "restore_adapter_master_v41a",
                    )
                    restorations.extend(restored)
                if len(batches) != 4 or any(len(batch) != 448 for batch in batches):
                    raise RuntimeError("v43f replicated signed wave is incomplete")
                by_actor = {item["actor_rank"]: item for item in assignment}
                for actor_rank, batch in enumerate(batches):
                    direction = by_actor[actor_rank]["direction_index"]
                    scored = score_batch_detailed_v43f(bundle, dense_items, batch)
                    scores[label][direction][replicate] = {
                        "actor_rank": actor_rank,
                        **_compact_signed_score(scored),
                    }
                    perturbations[label][direction][replicate] = certificates[actor_rank]
    if any(
        item is None
        for label in scores.values() for direction in label for item in direction
    ):
        raise RuntimeError("v43f signed score matrix is incomplete")
    if any(
        item["restored_identity"]["sha256"] != master_sha for item in restorations
    ):
        raise RuntimeError("v43f signed exact restore changed master identity")
    for label in ("plus", "minus"):
        for direction in range(POPULATION_SIZE):
            ...
            if (
                ...
                raise RuntimeError(
                    f"v43f replicated state differs for {label} direction {direction}"
                )
    equal_sign = {
        label: [
            [
    ...
    central_replicates = [
        [
            0.5 * (
                equal_sign["plus"][direction][rep]
                - equal_sign["minus"][direction][rep]
            )
            for rep in range(2)
        ]
        for direction in range(POPULATION_SIZE)
    ]
    central = [math.fsum(item) / 2.0 for item in central_replicates]
    coefficients, standardization = _standardize(central)
    return {
        "schema": "balanced-replicated-lora-es-population-v43f",
        "assignments": assignments,
        "signed_scores": scores,
        "equal_unit_sign_scores": equal_sign,
        "unweighted_row_sign_scores": row_sign,
        "central_replicates": central_replicates,
...
def _materialization_consensus(items: list[dict], phase: str) -> str:
    ...
    if len(hashes) != 1 or len(bases) != 1:
        raise RuntimeError(f"v43f runtime/base materialization differs at {phase}")
...
def _apply_update(
    ...
    if (
        ...
        raise RuntimeError("v43f prepared update consensus changed")
    ...
    if len({v40a.canonical_sha256(item) for item in identities}) != 1:
        raise RuntimeError("v43f exact update candidate differs across ranks")
    ...
    if (
        ...
        raise RuntimeError("v43f exact committed identity differs across ranks")
    ...
    if (
        ...
        raise RuntimeError("v43f exact finalized identity differs across ranks")
    ...
    if len({execute_runtime_sha, commit_runtime_sha, finalize_runtime_sha}) != 1:
        raise RuntimeError("v43f exact runtime materialization changed across update phases")
    ...
    if len({
        ...
        raise RuntimeError("v43f post-update exact references differ across ranks")
    ...
    if (
        ...
        raise RuntimeError("v43f exact snapshot readback coverage changed")
    return {
...
def _post_update_consensus(
    ...
    records = _score_repeats(
        ...
        requests,
        warmups=0,
        retained=numeric.POST_UPDATE_REPEATS_V43F,
    )
    result = numeric.post_update_consensus_v43f(
        records, calibration["bootstrap"]["bounds"],
    )
    return _persist_phase(POST_UPDATE_ARTIFACT, {
        "schema": "matched-lora-es-post-update-consensus-v43f",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if ATTEMPT.exists() or RUN_DIR.exists():
        raise RuntimeError("v43f requires fresh artifact paths")
    preflight = v40a.gpu_preflight()
    attempt = v40a.self_hashed({
        "schema": "matched-lora-es-attempt-v43f",
        "status": "launching",
    ...
    try:
        ...
        if bundle["content_sha256_before_self_field"] != TRAIN_BUNDLE_SHA256:
            raise RuntimeError("v43f frozen train bundle identity changed")
        bundle = augment_unit_membership_v43f(bundle)
        ...
        if len({v40a.canonical_sha256(item) for item in masters}) != 1:
            raise RuntimeError("v43f canonical FP32 master differs across ranks")
        if len({
            item["materialization"]["runtime_values_sha256"]
            for item in certificates
        }) != 1:
            raise RuntimeError("v43f initial runtime materialization differs across ranks")
        master = masters[0]
        references = v40a._rpc_all(trainer, "capture_adapter_reference_v41a")
        reference_generations = {item["reference_generation"] for item in references}
        if len(reference_generations) != 1:
            raise RuntimeError("v43f reference generation differs across ranks")
        postinstall = _exact_base_score(trainer, bundle, dense_items, requests)
        if preinstall["consensus"] != postinstall["consensus"]:
            raise RuntimeError("v43f canonical install changed matched-init train score")
        ...
        if post_calibration["consensus"] != postinstall["consensus"]:
            raise RuntimeError("v43f calibration restore changed matched-init score")

        phase.value = "balanced_replicated_population_pop8"
        ...
        if post_population["consensus"] != postinstall["consensus"]:
            raise RuntimeError("v43f replicated population restore changed base score")
        reliability = numeric.reliability_gate_v43f(
        ...
        reliability_artifact = _persist_phase(RELIABILITY_ARTIFACT, {
            "schema": "matched-lora-es-population-reliability-v43f",
        ...
        if not reliability["passed"]:
            raise RuntimeError(
                "v43f replicated population failed preregistered reliability gate: "
                f"reliability={reliability['reliability']} "
                f"bound_fraction={reliability['calibration_bound_fraction_of_response_std']}"
            )

        plan_id = v40a.canonical_sha256({
            "schema": "matched-lora-es-update-plan-v43f",
        ...
        if not post_update["equivalence"]["passed"]:
            raise RuntimeError(
                "v43f post-update scores failed preregistered calibrated equivalence"
            )

        stop.set()
        monitor.join(timeout=10)
        if monitor.is_alive() or not failures.empty():
            raise RuntimeError("v43f GPU monitor failed") from (
        ...
        report = v40a.self_hashed({
            "schema": "matched-lora-es-train-only-report-v43f",
            "status": "complete_calibrated_one_nonzero_update_state_sealed",
            ...
            "train_bundle": {
                ...
                "unit_membership_sha256": bundle["unit_membership_sha256_v43f"],
            },
            "recipe": {
                ...
                "signed_sequence_presentations": 2 * 2 * POPULATION_SIZE * 448,
                "calibration_sequence_presentations": (
                    numeric.CALIBRATION_WARMUPS_V43F
                    + numeric.CALIBRATION_REPEATS_V43F
                ) * 4 * 448,
                "post_update_sequence_presentations": (
                    numeric.POST_UPDATE_REPEATS_V43F * 4 * 448
                ),
        ...
        if monitor is not None:
            monitor.join(timeout=10)
        v40a.atomic_json(RUN_DIR / "failure_v43f.json", v40a.self_hashed({
            "schema": "matched-lora-es-failure-v43f",
