#!/usr/bin/env python3
"""Replace Rope365 resource-author trivia with stage-tying context."""
...
DATA = ROOT / "data"
V250_DIR = DATA / "manual_reviews/context_merit_audit_v250"
sys.path[:0] = [str(ROOT), str(V250_DIR)]
import build_context_merit_audit_v250 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v251.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v251.jsonl"
REPORT = OUT_DIR / "report_context_merit_v251.json"
REVIEWER = "codex-context-merit-audit-v251"
...
SOURCE = DATA / "raw/rope365_682937f92222bf87.json"

SPECS = (
    {
        "fact_id": "fact-b4be418c2c20c06bece4",
        "active_index": 427,
        "markers": (
            "Tying on the stage is different from tying real prisoners; it becomes acceptable for the tie to be escapable, and it doesn’t need to cause pain. The aesthetic, safety and emotions created by the tie are much more important.",
        ),
        "decision": "edit",
        "question": "How does Rope365 distinguish stage tying from tying actual prisoners?",
        "answer": "A stage tie may be escapable and need not cause pain, so its aesthetics, safety, and emotional effect can matter more than prisoner restraint.",
        "reason_code": "replace_resource_author_trivia_with_stage_context",
        "reason": "The replacement trades a resource-author lookup for the source's reusable explanation of how the goals of stage tying differ from prisoner restraint.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v250",
    "direct_rows_without_prior_curation": 1,
    "resource_author_trivia_rows_selected": 1,
    "rows": 492,
    "sha256": "0b16857bd080af176df7110951599b6227f3857309bb426a100ab6fd17889056",
}
EXPECTED_OUTPUT_SHA256 = "8e89377bced9c32d5422bbc058212edded41b9620e54533236cbdb347a722927"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v251-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v250-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v250.jsonl"
        baseline_report = directory / "v250.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v250 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v251 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "resource_author_trivia_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v251",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v251 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v251 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v250": 454,
        ...
        "schema": "context-merit-audit-report-v251",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a resource-author lookup whose source passage supports a more reusable conceptual contrast",
                "score": "transferable explanation, source support, question naturalness, and reduced bibliographic trivia",
