#!/usr/bin/env python3
"""Replace a terminology lookup with a technique-as-language lesson."""
...
DATA = ROOT / "data"
V273_DIR = DATA / "manual_reviews/context_merit_audit_v273"
sys.path[:0] = [str(ROOT), str(V273_DIR)]
import build_context_merit_audit_v273 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v274.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v274.jsonl"
REPORT = OUT_DIR / "report_context_merit_v274.json"
REVIEWER = "codex-context-merit-audit-v274"
...
SOURCE = DATA / "raw/kinbakutoday_dc711609dfc7a35f.json"

SPECS = (
    {
        "fact_id": "fact-49d9b6e4b315dfa5284a",
        "active_index": 135,
        "markers": (
            "Technique is merely grammar and vocabulary. You need it to communicate, but alone it isn’t going to allow you to say very much.",
            ...
            "That learning is a process which requires mindfulness, reflection, attention, and presence.",
        ),
        "decision": "edit",
        "question": "Why does “Tying the One Rope Gote” compare technique to grammar and vocabulary?",
        "answer": "Technique is necessary for communication but not sufficient; a tie becomes meaningful when it conveys intention, feeling, and attention in a conversation with the partner.",
        "reason_code": "replace_terminology_lookup_with_technique_as_language_lesson",
        "reason": "The replacement trades a bare Japanese-term lookup for the article's transferable distinction between technical fluency and attentive, partner-centered communication.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v273",
    "direct_rows_without_prior_curation": 1,
    "terminology_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "11596f8b1d7f94d9dc1ba3568b9440d3767e02f05da9c86fdccff90b9cb7d763",
}
EXPECTED_OUTPUT_SHA256 = "9511a62ecee21748edf5edfe07fbaff920213db4afb8e113e4fe17fb88770721"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v274-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v273-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v273.jsonl"
        baseline_report = directory / "v273.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v273 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v274 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "technique_as_language_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v274",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v274 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v274 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v273": 454,
        ...
        "schema": "context-merit-audit-report-v274",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a terminology lookup whose source passage supports a technique-as-language lesson",
                "score": "transferable learning and relational value, source support, question naturalness, and reduced terminology trivia",
