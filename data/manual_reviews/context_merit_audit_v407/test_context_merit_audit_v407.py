...
HERE=Path(__file__).resolve().parent;sys.path.insert(0,str(HERE));import build_context_merit_audit_v407 as b
class ContextMeritV407(unittest.TestCase):
 @classmethod
 def setUpClass(cls):
  b.main();cls.t=tempfile.TemporaryDirectory(prefix=".test-v407-",dir=HERE);d=Path(cls.t.name);cls.base=d/"base.jsonl";b.build_baseline(cls.base,d/"base.report.json");cls.out=d/"out.jsonl";cls.rep=d/"out.report.json";cls.ds=[];cls.rs=[]
