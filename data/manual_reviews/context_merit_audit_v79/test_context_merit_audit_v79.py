...
class V79(unittest.TestCase):
 ...
 def test_04_direct(self):self.assertEqual(sum(not r.get("curation") for r in self.baseline),128)
 ...
 def test_10_lineage(self):
  for r in self.audit:self.assertEqual((r["review_pass"],r["projection_lineage"]["baseline_sha256"]),("anatomiestudio_etiquette_and_consent_reaudit",b.PROJECTED_SELECTION_BASELINE["sha256"]))
 ...
 def test_14_pins(self):
  pins=self.report["frozen_prior_decision_artifacts"];self.assertEqual(len(pins),234)
 ...
 def test_19_report(self):self.assertEqual((self.report["schema"],self.report["audit"]["by_decision"],self.report["isolated_build_projection"]["output_sha256"]),("context-merit-audit-report-v79",{"edit":2,"keep":3},b.EXPECTED_OUTPUT_SHA256))
