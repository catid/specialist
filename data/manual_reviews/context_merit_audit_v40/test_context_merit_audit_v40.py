#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v40."""
...
import build_context_merit_audit_v40 as builder
from qa_quality import normalize_text


class ContextMeritAuditV40Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline_temp = tempfile.TemporaryDirectory(
            prefix=".test-baseline-v40-", dir=HERE)
        output = Path(cls.baseline_temp.name)
        cls.baseline_path = output / "projection-v39.jsonl"
        cls.baseline_report_path = output / "projection-v39.report.json"
    ...
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v40-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v40.jsonl"),
            ("CURATION", "pending_curation_context_merit_v40.jsonl"),
            ("REPORT", "report_context_merit_v40.json"),
        ):
    ...
    def test_baseline_is_exact_v39_projection(self) -> None:
        self.assertEqual(len(self.baseline), 569)
    ...
    def test_selection_is_exact_deterministic_top_ten(self) -> None:
        ...
        self.assertEqual(len(ranked), 166)
        self.assertEqual(
    ...
    def test_projected_baseline_counts_are_pinned(self) -> None:
        ...
        reviewed = builder.prior_reviewed_fact_ids()
        self.assertEqual(len(direct), 290)
        self.assertEqual(sum(row["fact_id"] in reviewed for row in direct), 124)
    ...
    def test_all_edits_are_extractive(self) -> None:
        ...
        by_id = {row["fact_id"]: row for row in self.audit}
        slipped = by_id["fact-34c77c437152b75d6d55"]
        self.assertIn("unlocked entry and locked exit",
                      slipped["edited_question"])
        self.assertIn("entry point will not be locked", slipped["edited_answer"])
        self.assertIn("lock the exit into place", slipped["edited_answer"])
    ...
    def test_projection_lineage_is_complete(self) -> None:
        for row in self.audit:
            ...
            self.assertEqual(lineage["baseline_rows"], 569)
            ...
            self.assertEqual(row["review_pass"],
                             "first_context_merit_review_of_v39_projection_row")
    ...
    def test_report_counts_projection_and_sealed_policy(self) -> None:
        report = json.loads(builder.REPORT.read_text())
        self.assertEqual(report["schema"], "context-merit-audit-report-v40")
        self.assertEqual(report["audit"]["rows"], 10)
        self.assertEqual(report["selection"]["rows_selected"], 10)
        self.assertEqual(report["selection"]["eligible_unreviewed_rows"], 166)
        ...
        self.assertEqual(
            ...
            "531847b6438eedf9e2b2b19c3ad01738b2857607403577a4ddb36b8226511ab5")
    ...
    def test_future_versions_cannot_enter_v40_by_directory_discovery(self) -> None:
        self.assertEqual(len(builder.CONTEXT_CURATIONS), 39)
        self.assertEqual(len(builder.CONTEXT_AUDITS), 39)
        self.assertIn("context_merit_audit_v39",
                      str(builder.CONTEXT_CURATIONS[-1]))
        self.assertIn("context_merit_audit_v39",
                      str(builder.CONTEXT_AUDITS[-1]))
        self.assertFalse(any("context_merit_audit_v40" in str(path) or
                             "context_merit_audit_v40" in str(path)
                             for path in (*builder.CONTEXT_CURATIONS,
                                          *builder.CONTEXT_AUDITS)))


class IsolatedProjectionV40Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory(
            prefix=".test-projection-v40-", dir=HERE)
    ...
    def test_projection_applies_exact_v40_decisions(self) -> None:
        ...
        prior_v38 = {
            row["fact_id"]: row
            for row in builder.read_jsonl(builder.CONTEXT_AUDITS[-2])
        }
