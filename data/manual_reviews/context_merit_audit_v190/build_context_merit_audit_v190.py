#!/usr/bin/env python3
"""Connect Subspace Designs' alloy specification to its stated tradeoff."""
...
DATA = ROOT / "data"
V189_DIR = DATA / "manual_reviews/context_merit_audit_v189"
sys.path[:0] = [str(ROOT), str(V189_DIR)]
import build_context_merit_audit_v189 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v190.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v190.jsonl"
REPORT = OUT_DIR / "report_context_merit_v190.json"
REVIEWER = "codex-context-merit-audit-v190"
...
SOURCE = DATA / "raw/subspace_designs_custom_order_deposit_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 23,
        "markers": (
            "certified aerospace-grade 6061-T6 aluminum",
        ),
        "decision": "edit",
        ...
        "reason_code": "connect_custom_ring_alloy_to_stated_tradeoff",
        "reason": "The replacement preserves the alloy fact while adding the vendor's reason it matters for custom suspension hardware: strength with low weight.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v189",
    "direct_rows_without_prior_curation": 54,
    "subspace_custom_ring_material_rows_selected": 1,
    "rows": 505,
    "sha256": "3fc6b2fe2061af2a662e983f98cafd4edb70387b9a23cbf4b6796313607bdaa0",
}
EXPECTED_OUTPUT_SHA256 = "1770a9db60003854fc1c110d84a229775795f20d8d52dd421a91813be064db11"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v190-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v188-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v188.jsonl"
        baseline_report = directory / "v188.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v188 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v190 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Subspace Designs custom-ring source lineage drift")
        audit = {
            ...
            "review_pass": "subspace_custom_ring_material_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v190",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v190 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v190 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v189": 467,
            ...
            "projection_strategy": "delegate to the hermetic v189 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v190",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the alloy lookup whose fully inspected product page also states why that material was selected",
                "score": "manual source support, suspension-equipment usefulness, causal completeness, and training merit",
