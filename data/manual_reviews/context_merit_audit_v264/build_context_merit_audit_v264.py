#!/usr/bin/env python3
"""Replace a decades lookup with a historical-source provenance lesson."""
...
DATA = ROOT / "data"
V263_DIR = DATA / "manual_reviews/context_merit_audit_v263"
sys.path[:0] = [str(ROOT), str(V263_DIR)]
import build_context_merit_audit_v263 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v264.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v264.jsonl"
REPORT = OUT_DIR / "report_context_merit_v264.json"
REVIEWER = "codex-context-merit-audit-v264"
...
SOURCE = DATA / "raw/kinbakutoday_cf9d44df63fd716c.json"

SPECS = (
    {
        "fact_id": "fact-a484e790f0fdb4479cb2",
        "active_index": 398,
        "markers": (
            "because I’ve been studying the art for over 40 years I think my book benefited from research that would be very difficult to do today, such as having access to original historical materials to say nothing of having the privilege of interviewing people like the late Nureki Chimuo sensei, the founder of the famous SM Library in Tokyo, Urado Hiroshi sensei, a close, elderly colleague of the famed martial arts expert Nawa Yumio",
        ),
        "decision": "edit",
        "question": "Why does Master K say the historical research behind “The Beauty of Kinbaku” would be difficult to reproduce today?",
        "answer": "He had access to original historical materials and could interview witnesses to the early modern kinbaku era, including Nureki Chimuo and Urado Hiroshi.",
        "reason_code": "replace_decades_lookup_with_research_provenance",
        "reason": "The replacement trades a bare decades lookup for the interview's substantive account of the book's primary-source and oral-history provenance.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v263",
    "direct_rows_without_prior_curation": 1,
    "decades_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "4d2b9378a53de6217d4f0a9515fbbbde299f3f3a600b8196a3ac8c5fb91d2473",
}
EXPECTED_OUTPUT_SHA256 = "a6b90dc965d061b34481bf48f2914eda68e009783c7ec22302fa0e3fa8cda844"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v264-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v263-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v263.jsonl"
        baseline_report = directory / "v263.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v263 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v264 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "research_provenance_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v264",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v264 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v264 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v263": 454,
        ...
        "schema": "context-merit-audit-report-v264",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a decades lookup whose source passage supports a primary-source provenance lesson",
                "score": "historical and source-literacy value, source support, question naturalness, and reduced date trivia",
