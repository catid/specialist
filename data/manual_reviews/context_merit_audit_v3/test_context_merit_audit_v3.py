#!/usr/bin/env python3
"""Focused regression tests for context-merit audit tranche v3."""
...
import build_context_merit_audit_v3 as builder
from qa_quality import normalize_text


class ContextMeritAuditV3Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v3-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output_dir = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v3.jsonl"),
            ("CURATION", "pending_curation_context_merit_v3.jsonl"),
            ("REPORT", "report_context_merit_v3.json"),
        ):
    ...
    def test_selection_is_next_nonoverlapping_ranked_25(self) -> None:
        ...
        for version in ("v1", "v2"):
            path = (builder.DATA / "manual_reviews" /
                    f"context_merit_audit_{version}" /
                    f"context_merit_audit_{version}.jsonl")
    ...
    def test_sources_and_evidence_are_hash_pinned(self) -> None:
        ...
        for specification in builder.SPECS:
            ...
            if specification.get("support_type", "extractive") == "extractive":
    ...
    def test_pending_curation_support_and_append_safety(self) -> None:
        ...
        for path in (builder.QUALITY_MERIT_CURATION,
                     ...
                     builder.CONTEXT_MERIT_V2_CURATION):
            prior.update(row["fact_id"] for row in builder.read_jsonl(path))
        self.assertFalse(prior & {row["fact_id"] for row in self.curation})
        support = [row["support_type"] for row in self.curation
                   if row["action"] == "edit"]
        self.assertEqual(support.count("extractive"), 3)
        self.assertEqual(support.count("manual_paraphrase"), 1)
        paraphrase = next(row for row in self.curation
                          if row.get("support_type") == "manual_paraphrase")
        self.assertTrue(paraphrase["paraphrase_rationale"])
    ...
    def test_report_is_pending_and_projection_is_identity_checked(self) -> None:
        ...
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 3, "edit": 4, "keep": 18})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 786)
