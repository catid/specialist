#!/usr/bin/env python3
"""Clarify Rope365's warning about where not to extend a box-tie rope."""
...
DATA = ROOT / "data"
V173_DIR = DATA / "manual_reviews/context_merit_audit_v173"
sys.path[:0] = [str(ROOT), str(V173_DIR)]
import build_context_merit_audit_v173 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v174.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v174.jsonl"
REPORT = OUT_DIR / "report_context_merit_v174.json"
REVIEWER = "codex-context-merit-audit-v174"
...
SOURCE = DATA / "raw/rope365_1616ffce57d993f3.json"
...
SPECS = (
    {
        ...
        "decision": "edit",
        "question": "Where should a box-tie rope extension not be placed, and why?",
        "answer": "Do not extend it between the arm and torso, because many nerves are exposed at and below the armpit.",
        "reason_code": "clarify_box_tie_extension_nerve_warning",
        "reason": "The replacement supplies the missing box-tie context and states the placement warning and its nerve-risk rationale directly.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v173",
    "direct_rows_without_prior_curation": 70,
    "rope365_box_tie_extension_rows_selected": 1,
    "rows": 506,
    "sha256": "d6e8b304147913c077dbe6e7db3462b64753fbbb8ce631313960f30fdb64adf6",
}
EXPECTED_OUTPUT_SHA256 = "31fff9daccc2dc7f0c5e22dfc6b3ba3ba1be2bb1ef3e73431b23f47bcaed2c3e"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v174-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v173-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v173.jsonl"
        baseline_report = directory / "v173.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v173 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v174 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Rope365 two-rope box-tie source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_box_tie_extension_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v174",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v174 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v174 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v173": 468,
            ...
            "projection_strategy": "delegate to the hermetic v173 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v174",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the context-poor rope-extension Q&A whose fully read source supports a direct box-tie nerve warning",
                "score": "manual source support, safety usefulness, context completeness, and training merit",
