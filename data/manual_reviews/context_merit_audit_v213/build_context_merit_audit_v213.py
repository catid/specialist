#!/usr/bin/env python3
"""Add Crash Restraint's rationale to its coreless-construction guidance."""
...
DATA = ROOT / "data"
V212_DIR = DATA / "manual_reviews/context_merit_audit_v212"
sys.path[:0] = [str(ROOT), str(V212_DIR)]
import build_context_merit_audit_v212 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v213.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v213.jsonl"
REPORT = OUT_DIR / "report_context_merit_v213.json"
REVIEWER = "codex-context-merit-audit-v213"
...
SOURCE = DATA / "raw/crash_restraint_building_rope_kit_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 405,
        "markers": (
            "The page advises against rope with an outer braided sheath around a straight-fiber core, including kernmantle construction, because cored rope is difficult to handle for bondage and produces bulky knots.",
            "It recommends either twisted rope, most commonly three-strand but sometimes four-strand, or a solid braid with no core.",
        ),
        "decision": "edit",
        ...
        "reason_code": "connect_coreless_constructions_to_handling_rationale",
        "reason": "The original answer named two suitable constructions without explaining the selection criterion. The replacement preserves both and adds the page's handling and knot-bulk rationale for avoiding cored rope.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v212",
    "direct_rows_without_prior_curation": 31,
    "crash_coreless_construction_rows_selected": 1,
    "rows": 505,
    "sha256": "f39187d4426c5f2c41772b59ca713e19001fcdbdcb0100355a065671443225bb",
}
EXPECTED_OUTPUT_SHA256 = "61671aa0967dca26e7008ab72ec92b460a502f5f70e6aa57660ffc43812f66cc"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v213-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v212-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v212.jsonl"
        baseline_report = directory / "v212.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v212 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v213 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Crash Restraint source lineage drift")
        audit = {
            ...
            "review_pass": "crash_coreless_construction_rationale_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v213",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v213 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v213 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v212": 467,
            ...
            "projection_strategy": "delegate to the hermetic v212 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v213",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated coreless-construction row whose fully inspected official page explains why cored rope is discouraged",
                "score": "manual source support, equipment-selection usefulness, explanatory completeness, and training merit",
