#!/usr/bin/env python3
"""Connect the ChromaKnotz link to inspected nylon specs and care."""
...
DATA = ROOT / "data"
V231_DIR = DATA / "manual_reviews/context_merit_audit_v231"
sys.path[:0] = [str(ROOT), str(V231_DIR)]
import build_context_merit_audit_v231 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v232.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v232.jsonl"
REPORT = OUT_DIR / "report_context_merit_v232.json"
REVIEWER = "codex-context-merit-audit-v232"
...
SOURCE = DATA / "raw/owner_chromaknotz_resource_20260714.json"

SPECS = (
    {
        "fact_id": "fact-e6435c62d16bab5ba44f",
        "active_index": 381,
        "markers": (
            "The supplied directory recommends ChromaKnotz as a synthetic bondage-rope shop: https://chromaknotz.square.site/.",
            "The inspected questions page describes its rope as one-quarter-inch double-braid nylon and gives a single-strand static breaking-point figure of 1,280 without stating a unit.",
            "For washing, the vendor says to use a mesh bag and a cold gentle cycle, then line dry out of direct sunlight; it says never to use a clothes dryer and warns that too much fabric softener makes rope more slippery.",
        ),
        "decision": "edit",
        "question": "Where can someone find the owner-recommended ChromaKnotz synthetic rope, and what construction and care details does the vendor give?",
        "answer": "ChromaKnotz is at https://chromaknotz.square.site/. It describes one-quarter-inch double-braid nylon; for care it says cold gentle-cycle washing in a mesh bag and line drying out of direct sun, never a clothes dryer. Its stated breaking-point figure is 1,280, but the page omits the unit.",
        "reason_code": "connect_chromaknotz_link_to_specs_care",
        "reason": "The original answer preserved only the shop link. The replacement keeps the exact owner-recommended URL and adds inspected construction and care details while retaining the important caveat that the breaking-point unit is unstated.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v231",
    "direct_rows_without_prior_curation": 12,
    "chromaknotz_resource_specs_rows_selected": 1,
    "rows": 505,
    "sha256": "4511c94bd810a4b54d5cd2f913134fb954d68c149072b0362269c792f6d830d7",
}
EXPECTED_OUTPUT_SHA256 = "d5148dd7f478d3f2c26ba06373f76f1da0ff3429b4fd3735175766e3d15cf499"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v232-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v231-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v231.jsonl"
        baseline_report = directory / "v231.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v231 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v232 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "chromaknotz_resource_specs_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v232",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v232 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v232 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v231": 467,
            ...
            "projection_strategy": "delegate to the hermetic v231 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v232",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended ChromaKnotz link row whose inspected page gives nylon construction and care details",
                "score": "owner-source support, material-care usefulness, specification caveat completeness, and URL preservation",
