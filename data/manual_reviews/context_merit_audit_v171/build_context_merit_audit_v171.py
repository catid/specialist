#!/usr/bin/env python3
"""Replace a loose-knot instruction with its storage-care rationale."""
...
DATA = ROOT / "data"
V170_DIR = DATA / "manual_reviews/context_merit_audit_v170"
sys.path[:0] = [str(ROOT), str(V170_DIR)]
import build_context_merit_audit_v170 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v171.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v171.jsonl"
REPORT = OUT_DIR / "report_context_merit_v171.json"
REVIEWER = "codex-context-merit-audit-v171"
...
SPECS = (
    {
        ...
        "active_index": 52,
        "marker": "This is a good idea for longer-term storage or more fragile natural rope than kinks easily when kept in a tight knot.",
        "decision": "edit",
        ...
        "reason_code": "replace_loose_coiling_instruction_with_storage_rationale",
        "reason": "The replacement explains why the loose knot protects fragile rope instead of training a context-free imperative.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v170",
    "direct_rows_without_prior_curation": 73,
    "rope365_coiling_storage_rows_selected": 1,
    "rows": 506,
    "sha256": "31d84172c5b032fc7c94b17603eb04208955f53ed4ee9e3e4ae828d30e47024c",
}
EXPECTED_OUTPUT_SHA256 = "e0a43aede1adc93e40340fde7afee800740e1823298c5b85f5531112c27dc18e"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v171-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v170-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v170.jsonl"
        baseline_report = directory / "v170.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v170 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v171 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Rope365 coiling source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_coiling_storage_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v171",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v171 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v171 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v170": 468,
            ...
            "projection_strategy": "delegate to the hermetic v170 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v171",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the loose-knot instruction whose fully read source supports a storage-care rationale",
