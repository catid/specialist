#!/usr/bin/env python3
"""Remove two low-merit two-rope box-tie pattern lookups."""
...
DATA = ROOT / "data"
V119_DIR = DATA / "manual_reviews/context_merit_audit_v119"
sys.path[:0] = [str(ROOT), str(V119_DIR)]
import build_context_merit_audit_v119 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v120.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v120.jsonl"
REPORT = OUT_DIR / "report_context_merit_v120.json"
REVIEWER = "codex-context-merit-audit-v120"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__5bae37a6d84f5c38397e.json"

SPECS = (
    {
        "fact_id": "fact-b7974457ec4d1fb9268e",
        "active_index": 279,
        "marker": "Here we use a friction similar to a square lashing, the same technique to tie two poles at an angle for scaffolding.",
        "decision": "drop",
        "reason_code": "low_merit_square_lashing_comparison",
        "reason": "This asks only for the name of a visual lashing comparison and adds little practical value beside the page's stronger active placement and nerve-safety guidance.",
    },
    {
        "fact_id": "fact-e860029493d2db70a437",
        "active_index": 323,
        "marker": "- Frictions – The cross shape of the lower wrap friction allows for different design of frictions. We’ll look into more friction variations in Day 204",
        "decision": "drop",
        "reason_code": "low_merit_cross_shape_lookup",
        "reason": "This isolated shape lookup is pattern trivia without actionable construction or safety context and is lower merit than the page's retained nerve and extension-placement guidance.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v119",
    "direct_rows_without_prior_curation": 92,
    "rope365_two_rope_box_tie_rows_selected": 2,
    "rows": 527,
    "sha256": "a73b919d553645a937746d7428bd80cac3924036429095a9ac6003dca217caa1",
}
EXPECTED_OUTPUT_SHA256 = "dc67811f8f152615ab9d60cbbdaf58212d66ec74d72cb8f92456750ac4ffd853"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v120-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v119-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v119.jsonl"
        baseline_report = directory / "v119.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 527 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v119 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v120 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: two-rope box-tie source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_two_rope_box_tie_pattern_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v120",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 525 or observed["eval_fact_count"] != 612:
        raise ValueError("v120 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v120 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v119": 489,
            "active_after_this_tranche": 487,
            ...
            "projection_strategy": "delegate to the v119 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v120",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the two not-yet-context-audited pattern-trivia Q&A from the fully read Rope365 two-rope box-tie page",
                "score": "manual source support, isolated-trivia risk, actionable value, and comparison with retained safety guidance",
