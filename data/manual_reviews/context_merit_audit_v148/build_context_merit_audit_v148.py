#!/usr/bin/env python3
"""Replace a knot-name lookup with a cultural-learning principle."""
...
DATA = ROOT / "data"
V147_DIR = DATA / "manual_reviews/context_merit_audit_v147"
sys.path[:0] = [str(ROOT), str(V147_DIR)]
import build_context_merit_audit_v147 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v148.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v148.jsonl"
REPORT = OUT_DIR / "report_context_merit_v148.json"
REVIEWER = "codex-context-merit-audit-v148"
...
SPECS = (
    {
        "fact_id": "fact-0837c3d480d69df19e10",
        "active_index": 8,
        "marker": "Properly learning this cultural background, rather than just having classes where you memorize the shapes, gives you a glimpse into the psychological depth of rope work and definitely increases the enjoyment of rope.",
        "decision": "edit",
        "question": "Why does Kasumi Hourai recommend learning the cultural background behind kinbaku’s traditional tying methods?",
        "answer": "She says it reveals the reasons behind techniques, offers insight into rope work’s psychological depth, and can increase enjoyment beyond merely memorizing forms.",
        "reason_code": "replace_knot_name_with_cultural_learning_principle",
        "reason": "The replacement preserves the source’s broader lesson about understanding cultural purpose instead of training another isolated knot-name lookup.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v147",
    "direct_rows_without_prior_curation": 89,
    "kinbakutoday_cultural_learning_rows_selected": 1,
    "rows": 509,
    "sha256": "2da58d79b221768f15e9ec629f21a7f70c9fae822242682bfe94899a8e6460b2",
}
EXPECTED_OUTPUT_SHA256 = "5b208e1f355d13313f3140469b17033afbe6dae96c83bdece88c4663ee4e0ca4"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v148-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v147-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v147.jsonl"
        baseline_report = directory / "v147.report.json"
        ...
        if len(rows) != 509 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v147 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v148 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Kinbaku Today interview source lineage drift")
        audit = {
            ...
            "review_pass": "kinbakutoday_cultural_learning_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v148",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 509 or observed["eval_fact_count"] != 612:
        raise ValueError("v148 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v148 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v147": 471,
            ...
            "projection_strategy": "delegate to the hermetic v147 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v148",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the knot-name Q&A whose fully read source supports a broader cultural-learning principle",
                "score": "manual source support, conceptual usefulness, faithful synthesis, and training merit",
