#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v24."""
...
import build_context_merit_audit_v24 as builder
from qa_quality import normalize_text


class ContextMeritAuditV24Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v24-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v24.jsonl"),
            ("CURATION", "pending_curation_context_merit_v24.jsonl"),
            ("REPORT", "report_context_merit_v24.json"),
        ):
    ...
    def test_secondary_selection_is_exact_and_ranked(self) -> None:
        ...
        self.assertEqual(len(ranked), 180)
        self.assertEqual(
    ...
    def test_projected_baseline_and_lineage_are_pinned(self) -> None:
        ...
        self.assertEqual(baseline["rows"], 625)
        self.assertEqual(
            baseline["sha256"],
    ...
    def test_sources_and_evidence_are_hash_pinned(self) -> None:
        by_id = {row["fact_id"]: row for row in self.audit}
        paraphrase = "fact-abe07db12e2f94fbbb7e"
    ...
    def test_report_projection_and_sealed_policy(self) -> None:
        report = json.loads(builder.REPORT.read_text())
        self.assertEqual(report["schema"], "context-merit-audit-report-v24")
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 6, "edit": 8, "keep": 11})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 619)
        self.assertEqual(
