#!/usr/bin/env python3
"""Replace a magazine-date lookup with a cross-cultural history lesson."""
...
DATA = ROOT / "data"
V148_DIR = DATA / "manual_reviews/context_merit_audit_v148"
sys.path[:0] = [str(ROOT), str(V148_DIR)]
import build_context_merit_audit_v148 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v149.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v149.jsonl"
REPORT = OUT_DIR / "report_context_merit_v149.json"
REVIEWER = "codex-context-merit-audit-v149"
...
SOURCE = DATA / "raw/kinbakutoday_0fde39bdb08f42b9.json"

SPECS = (
    {
        "fact_id": "fact-69ab9289780d8d940308",
        "active_index": 39,
        "marker": "While it is important to document the impact of Japanese rope on western tying practices, we must also understand that cross cultural communication flows in both directions.",
        "decision": "edit",
        "question": "What broader point about kinbaku history does Uramado’s publication of Western fetish art illustrate?",
        "answer": "Cross-cultural influence moved in both directions: Japanese rope influenced Western artists, while Western images published in Japan likely shaped Japanese approaches to SM and bondage.",
        "reason_code": "replace_magazine_dates_with_bidirectional_influence",
        "reason": "The replacement teaches the source’s central historical argument about two-way cultural influence instead of an isolated publication-date lookup.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v148",
    "direct_rows_without_prior_curation": 89,
    "kinbakutoday_bidirectional_influence_rows_selected": 1,
    "rows": 509,
    "sha256": "5b208e1f355d13313f3140469b17033afbe6dae96c83bdece88c4663ee4e0ca4",
}
EXPECTED_OUTPUT_SHA256 = "bf9d7389f249f4363751d92fae6d32fffc2096febf5afa8a890159f23ffc6942"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v149-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v148-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v148.jsonl"
        baseline_report = directory / "v148.report.json"
        ...
        if len(rows) != 509 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v148 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v149 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Kinbaku Today Uramado source lineage drift")
        audit = {
            ...
            "review_pass": "kinbakutoday_bidirectional_influence_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v149",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 509 or observed["eval_fact_count"] != 612:
        raise ValueError("v149 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v149 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v148": 471,
            ...
            "projection_strategy": "delegate to the hermetic v148 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v149",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the magazine-date Q&A whose fully read source supports a broader cross-cultural history lesson",
                "score": "manual source support, historical usefulness, faithful synthesis, and training merit",
