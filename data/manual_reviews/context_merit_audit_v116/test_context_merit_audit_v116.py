#!/usr/bin/env python3
"""Focused regression tests for Rope365 cross-partner hygiene audit v116."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v116 as b


class V116(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v116-", dir=HERE)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("rope365_cross_partner_hygiene_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v116", {"drop": 0, "edit": 0, "keep": 1}, b.EXPECTED_OUTPUT_SHA256),
        )
