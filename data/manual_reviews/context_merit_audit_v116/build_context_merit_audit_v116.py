#!/usr/bin/env python3
"""Audit Rope365's cross-partner rope-hygiene guidance."""
...
DATA = ROOT / "data"
V115_DIR = DATA / "manual_reviews/context_merit_audit_v115"
sys.path[:0] = [str(ROOT), str(V115_DIR)]
import build_context_merit_audit_v115 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v116.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v116.jsonl"
REPORT = OUT_DIR / "report_context_merit_v116.json"
REVIEWER = "codex-context-merit-audit-v116"
...
SPECS = (
    {
        "fact_id": "fact-97a51e2b3c940de18d8c",
        "active_index": 184,
        "marker": "If you are afraid of pathogens from one person infecting another, the only completely safe way is to use a different kit of rope between partners.",
        "decision": "keep",
        "reason_code": "clear_cross_partner_rope_separation",
        "reason": "The answer accurately preserves the source's concrete cross-contamination precaution to use separate rope kits between partners.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v115",
    "direct_rows_without_prior_curation": 92,
    "rope365_cross_partner_hygiene_rows_selected": 1,
    "rows": 529,
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v116-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v115-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v115.jsonl"
        baseline_report = directory / "v115.report.json"
        ...
        if len(rows) != 529 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v115 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v116 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "rope365_cross_partner_hygiene_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v116",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 529 or observed["eval_fact_count"] != 612:
        raise ValueError("v116 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v116 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v115": 491,
        ...
        "schema": "context-merit-audit-report-v116",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the not-yet-context-audited cross-partner hygiene Q&A from the fully read Rope365 maintenance page",
                "score": "manual source support, infection-risk actionability, answer clarity, and practical merit review",
