#!/usr/bin/env python3
"""Focused regression tests for context-merit audit tranche v5."""
...
import build_context_merit_audit_v5 as builder
from qa_quality import normalize_text


class ContextMeritAuditV5Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v5-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output_dir = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v5.jsonl"),
            ("CURATION", "pending_curation_context_merit_v5.jsonl"),
            ("REPORT", "report_context_merit_v5.json"),
        ):
    ...
    def test_future_context_tranches_are_not_historical_inputs(self) -> None:
        ...
        prior = manual_root / "context_merit_audit_v4" / "synthetic.jsonl"
        ...
        self.assertEqual(
            ...
            frozenset(f"context_merit_audit_v{i}" for i in range(1, 5)),
        )
    ...
    def test_report_is_pending_and_projection_is_identity_checked(self) -> None:
        ...
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 7, "edit": 3, "keep": 15})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 771)
