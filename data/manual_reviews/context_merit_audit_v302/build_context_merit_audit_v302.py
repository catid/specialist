#!/usr/bin/env python3
"""Integrate the thirteenth distinct-document technique tranche."""
...
ADDITION_DIR = DATA / "manual_reviews/technique_additions_v13"
sys.path[:0] = [str(ROOT), str(V301_DIR), str(V290_DIR), str(ADDITION_DIR)]
import build_context_merit_audit_v301 as previous
import build_context_merit_audit_v290 as core
import build_technique_additions_v13 as additions_builder

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v302.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v302.jsonl"
REPORT = OUT_DIR / "report_context_merit_v302.json"
ADDITIONS = additions_builder.OUTPUT
EXPECTED_ADDITIONS_SHA256 = "5ec0b4744704e2f0f409993ac4847c1c3d9c3a63537e93db80535f34d7923e6f"
EXPECTED_OUTPUT_SHA256 = "e71677ffdf831747d3e5f5287fce3812ed0a604b6eba114d28e5c0b5125e6a0e"
BASELINE_ROWS = 528
BASELINE_SHA256 = "cbba5461f1f60bd1770ef6a776be9747d20fd58745ff20263bc4983f31dddbf7"
EXPECTED_CAPACITY = {
    "before": {"conflict_units": 235, "equipment_material": 22, "resources_general": 79, "safety_consent": 79, "technique": 55},
    "after": {"conflict_units": 238, "equipment_material": 22, "resources_general": 79, "safety_consent": 79, "technique": 58},
}
...
PRIOR_ARTIFACTS = (
    V301_DIR / "context_merit_audit_v301.jsonl",
    V301_DIR / "pending_curation_context_merit_v301.jsonl",
    V301_DIR / "report_context_merit_v301.json",
)
...
def build_projection(output: Path, report: Path) -> None:
    directory = output.parent / f".{output.name}.v302-input"
    directory.mkdir(parents=True, exist_ok=True)
    baseline = directory / "v301.jsonl"
    build_baseline(baseline, directory / "v301.report.json")
    core.build_projection_with_inputs(output, report, (), (baseline, ADDITIONS))


def observe() -> dict:
    with tempfile.TemporaryDirectory(prefix=".v302-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main() -> None:
    ...
    for index, row in enumerate(read_jsonl(ADDITIONS), 1):
        ...
        audits.append({
            ...
            "review_pass": "distinct_document_technique_additions",
            "reviewed_at": "2026-07-15",
            "reviewer": "codex-context-merit-audit-v302",
            "schema": "context-merit-audit-v302",
            "source": row["source"],
    ...
    if not observed["dataset_equal"] or not observed["report_equal"]:
        raise ValueError("v302 projection is nondeterministic")
    if (observed["rows"], observed["eval_fact_count"]) != (531, 612):
        raise ValueError("v302 row/eval aggregate drift")
    if observed["baseline_capacity"] != EXPECTED_CAPACITY["before"] or observed["output_capacity"] != EXPECTED_CAPACITY["after"]:
        raise ValueError(f"v302 capacity drift: {observed['baseline_capacity']} -> {observed['output_capacity']}")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v302 output hash drift")
    report = {
        ...
        "schema": "context-merit-audit-report-v302",
