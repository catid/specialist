#!/usr/bin/env python3
"""Connect the Crash Restraint link to its pre-tying learning priorities."""
...
DATA = ROOT / "data"
V222_DIR = DATA / "manual_reviews/context_merit_audit_v222"
sys.path[:0] = [str(ROOT), str(V222_DIR)]
import build_context_merit_audit_v222 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v223.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v223.jsonl"
REPORT = OUT_DIR / "report_context_merit_v223.json"
REVIEWER = "codex-context-merit-audit-v223"
...
SOURCE = DATA / "raw/owner_crash_restraint_resource_20260714.json"

SPECS = (
    {
        "fact_id": "fact-222cfb0d001c49fdaf22",
        "active_index": 374,
        "markers": (
            "The supplied directory recommends Crash Restraint as an introductory suspension-rope learning resource: https://crash-restraint.com/.",
            "Crash Restraint's Getting Started page says that before learning how to tie or be tied, learners should spend time establishing boundaries, identifying safer play partners and environments, and negotiating the play they want.",
            "It advises healthy skepticism because rope education has no universal standards, encourages asking instructors why, and recommends that bottoms learn some tying and review safety information.",
        ),
        "decision": "edit",
        "question": "Where is the owner-recommended introductory suspension resource, and what does its Getting Started page prioritize before tying?",
        "answer": "Crash Restraint is at https://crash-restraint.com/. Before tying, its Getting Started page prioritizes establishing boundaries, finding safer partners and environments, negotiating the intended play, reviewing safety, and asking instructors why rather than accepting claims uncritically.",
        "reason_code": "connect_crash_link_to_pre_tying_priorities",
        "reason": "The original answer preserved only the resource link. The replacement keeps the exact owner-recommended URL and adds the inspected page's safety-first prerequisites and critical-learning guidance.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v222",
    "direct_rows_without_prior_curation": 21,
    "crash_resource_priorities_rows_selected": 1,
    "rows": 505,
    "sha256": "6df3330cc1280ee1960dcda98aac9cac70b92a0f521f05e2736bf6877a3938e7",
}
EXPECTED_OUTPUT_SHA256 = "9640a3b2b2b0ff2b90e7e962bcc66a952ce6bdc6e23558eb4489caeed4b8df2c"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v223-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v222-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v222.jsonl"
        baseline_report = directory / "v222.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v222 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v223 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "crash_resource_priorities_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v223",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v223 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v223 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v222": 467,
            ...
            "projection_strategy": "delegate to the hermetic v222 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v223",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended Crash Restraint link row whose inspected Getting Started page gives pre-tying priorities",
                "score": "owner-source support, learner-safety usefulness, critical-learning context, and URL preservation",
