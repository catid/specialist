...
class V76(unittest.TestCase):
 ...
 def test_02_eval_tooling(self):self.assertEqual(json.loads(self.br.read_text())["eval_fact_count"],612)
 ...
 def test_04_direct(self):self.assertEqual(sum(not r.get("curation") for r in self.baseline),135)
 def test_05_decisions(self):self.assertEqual({x:sum(r["decision"]==x for r in self.audit) for x in ("keep","edit","drop")},{"keep":2,"edit":2,"drop":0})
 def test_06_curation(self):self.assertEqual((len(self.curation),{r["action"] for r in self.curation}),(2,{"edit"}))
 ...
 def test_10_lineage(self):
  for r in self.audit:self.assertEqual((r["review_pass"],r["projection_lineage"]["baseline_sha256"]),("rope365_consent_and_monitoring_reaudit",b.PROJECTED_SELECTION_BASELINE["sha256"]))
 ...
 def test_14_pins(self):
  pins=self.report["frozen_prior_decision_artifacts"];self.assertEqual(len(pins),225)
 ...
 def test_19_report(self):self.assertEqual((self.report["schema"],self.report["audit"]["by_decision"],self.report["isolated_build_projection"]["output_sha256"]),("context-merit-audit-report-v76",{"edit":2,"keep":2},b.EXPECTED_OUTPUT_SHA256))
