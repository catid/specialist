#!/usr/bin/env python3
"""Focused regression tests for Esinem BDSM-consent audit v115."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v115 as b


class V115(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v115-", dir=HERE)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("esinem_bdsm_consent_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v115", {"drop": 0, "edit": 0, "keep": 1}, b.EXPECTED_OUTPUT_SHA256),
        )
