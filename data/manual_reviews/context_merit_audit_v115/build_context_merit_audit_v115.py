#!/usr/bin/env python3
"""Audit Esinem's informed-and-uncoerced consent guidance."""
...
DATA = ROOT / "data"
V114_DIR = DATA / "manual_reviews/context_merit_audit_v114"
sys.path[:0] = [str(ROOT), str(V114_DIR)]
import build_context_merit_audit_v114 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v115.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v115.jsonl"
REPORT = OUT_DIR / "report_context_merit_v115.json"
REVIEWER = "codex-context-merit-audit-v115"
...
SPECS = (
    {
        ...
        "active_index": 359,
        "marker": "Ultimately, the bitter truth is we can only try to ensure our consent is not only fully informed but also willingly given without undue coercion.",
        "decision": "keep",
        "reason_code": "clear_informed_uncoerced_consent_conditions",
        "reason": "The answer accurately preserves both conditions in the source: consent should be fully informed and willingly given without undue coercion.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v114",
    "direct_rows_without_prior_curation": 92,
    "esinem_bdsm_consent_rows_selected": 1,
    "rows": 529,
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v115-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v114-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v114.jsonl"
        baseline_report = directory / "v114.report.json"
        ...
        if len(rows) != 529 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v114 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v115 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: BDSM-consent source lineage drift")
        audit = {
            ...
            "review_pass": "esinem_bdsm_consent_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v115",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 529 or observed["eval_fact_count"] != 612:
        raise ValueError("v115 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v115 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v114": 491,
        ...
        "schema": "context-merit-audit-report-v115",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the not-yet-context-audited consent-conditions Q&A from the fully read Esinem BDSM-consent article",
                "score": "manual source support, consent completeness, answer clarity, and practical merit review",
