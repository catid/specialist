#!/usr/bin/env python3
"""Complete X-POLE's apparatus list with included and separate rigging gear."""
...
DATA = ROOT / "data"
V208_DIR = DATA / "manual_reviews/context_merit_audit_v208"
sys.path[:0] = [str(ROOT), str(V208_DIR)]
import build_context_merit_audit_v208 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v209.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v209.jsonl"
REPORT = OUT_DIR / "report_context_merit_v209.json"
REVIEWER = "codex-context-merit-audit-v209"
...
SOURCE = DATA / "raw/xpole_a_frame_product_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 150,
        "markers": (
            "The standard product includes a 55-inch top bar, four telescopic legs with feet, two leg connectors, pins, stabilization straps, carry bags, instructions, and additional weight bags; sand is not included.",
            "Depending on the apparatus and connection, the page says buyers may also need separately purchased carabiners, swivels, a Terminal 8, span sets, or a top-bar connector.",
        ),
        "decision": "edit",
        "question": "Which apparatus does X-POLE list for the A-FRAME, and what stability or connection gear is included versus sold separately?",
        "answer": "It lists hammocks, silks, hoops, and the X-FLY; stabilization straps and weight bags are included (sand is not), while a setup may also require separately purchased carabiners, swivels, a Terminal 8, span sets, or a top-bar connector.",
        "reason_code": "complete_apparatus_list_with_rigging_gear",
        "reason": "The original answer only listed apparatus. The replacement preserves that list and adds the page's included stability equipment and explicit separately purchased connection options for realistic setup planning.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v208",
    "direct_rows_without_prior_curation": 35,
    "xpole_apparatus_gear_rows_selected": 1,
    "rows": 505,
    "sha256": "f34220f9731d708c23c9ed1a6242d9f010503924f0a31c211162f0adf52f4d36",
}
EXPECTED_OUTPUT_SHA256 = "ea39581fec1343c2a95d74e07419e6e5e7bc9497d3505db25498e9eabf53a782"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v209-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v208-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v208.jsonl"
        baseline_report = directory / "v208.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v208 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v209 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: X-POLE product source lineage drift")
        audit = {
            ...
            "review_pass": "xpole_apparatus_gear_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v209",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v209 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v209 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v208": 467,
            ...
            "projection_strategy": "delegate to the hermetic v208 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v209",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated apparatus-list row whose fully inspected product page also distinguishes included stability gear from separate connection gear",
                "score": "manual source support, setup-planning usefulness, equipment completeness, and training merit",
