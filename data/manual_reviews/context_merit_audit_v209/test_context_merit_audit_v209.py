#!/usr/bin/env python3
"""Focused regression tests for the v209 X-POLE apparatus/gear repair."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v209 as b


class V209(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v209-", dir=HERE)
    ...
    def test_04_direct(self):
        self.assertEqual(sum(not row.get("curation") for row in self.baseline), 35)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("xpole_apparatus_gear_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v209", {"drop": 0, "edit": 1, "keep": 0}, b.EXPECTED_OUTPUT_SHA256),
        )
