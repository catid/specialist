#!/usr/bin/env python3
"""Remove a low-merit rope-history exact-year lookup."""
...
DATA = ROOT / "data"
V130_DIR = DATA / "manual_reviews/context_merit_audit_v130"
sys.path[:0] = [str(ROOT), str(V130_DIR)]
import build_context_merit_audit_v130 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v131.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v131.jsonl"
REPORT = OUT_DIR / "report_context_merit_v131.json"
REVIEWER = "codex-context-merit-audit-v131"
...
SOURCE = DATA / "raw/kinbakutoday_be37dae4ec8e0d88.json"

SPECS = (
    {
        "fact_id": "fact-00fc6c9bde56e7d5cb0d",
        "active_index": 145,
        "marker": "The name Nureki Chimuo as a Kinbakushi emerged in 1973, after he became involved in photo shoots for these magazines.",
        "decision": "drop",
        "reason_code": "low_merit_nureki_name_year_lookup",
        "reason": "This exact-year lookup reduces a broad historical discussion of publishing, commercialization, and rope culture to isolated chronology trivia.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v130",
    "direct_rows_without_prior_curation": 92,
    "kinbakutoday_nureki_name_year_rows_selected": 1,
    "rows": 514,
    "sha256": "2d53ea642a434fc1b55965c9632f9b692380bc2dbbd241b1e6901703041a587c",
}
EXPECTED_OUTPUT_SHA256 = "63a65cfe8a63447648fd91ecee010b975feeacb5b1a30c63d0c734f26a698e77"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v131-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v130-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v130.jsonl"
        baseline_report = directory / "v130.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 514 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v130 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v131 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: evolving-era source lineage drift")
        audit = {
            ...
            "review_pass": "kinbakutoday_nureki_name_year_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v131",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 513 or observed["eval_fact_count"] != 612:
        raise ValueError("v131 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v131 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v130": 476,
            "active_after_this_tranche": 475,
            ...
            "projection_strategy": "delegate to the hermetic v130 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v131",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the not-yet-context-audited exact-year Q&A from the fully read Kinbaku Today evolving-era debate",
