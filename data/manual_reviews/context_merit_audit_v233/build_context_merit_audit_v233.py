#!/usr/bin/env python3
"""Connect the Knot Head Nylon link to handling tradeoffs and care."""
...
DATA = ROOT / "data"
V232_DIR = DATA / "manual_reviews/context_merit_audit_v232"
sys.path[:0] = [str(ROOT), str(V232_DIR)]
import build_context_merit_audit_v232 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v233.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v233.jsonl"
REPORT = OUT_DIR / "report_context_merit_v233.json"
REVIEWER = "codex-context-merit-audit-v233"
...
SOURCE = DATA / "raw/owner_knothead_nylon_resource_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 382,
        "markers": (
            "The supplied directory recommends Knot Head Nylon as a synthetic bondage-rope shop: https://knotheadnylon.net/.",
            "The inspected use guide says nylon is more slippery than jute or hemp, so diagonals and frictions should not be trusted structurally.",
            "It describes nylon as stronger than natural fiber, slower to lose strength through ordinary use, and unaffected by water, making it useful for wet or messy play.",
            "For care, it says to put dirty rope in a mesh bag, machine wash it, and lay it out or hang it to dry; unlike jute, it does not require conditioning.",
        ),
        "decision": "edit",
        "question": "Where can someone find owner-recommended Knot Head Nylon, and what handling and care tradeoffs does its guide describe?",
        "answer": "Knot Head Nylon is at https://knotheadnylon.net/. Its guide says nylon is strong, durable, and water-tolerant but slippery enough that diagonals and frictions should not be trusted structurally; dirty rope can be machine-washed in a mesh bag and laid out or hung to dry without jute-style conditioning.",
        "reason_code": "connect_knothead_link_to_handling_care",
        "reason": "The original answer preserved only the shop link. The replacement keeps the exact owner-recommended URL and adds inspected nylon handling, wet-use, structural-friction, washing, and conditioning tradeoffs.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v232",
    "direct_rows_without_prior_curation": 11,
    "knothead_resource_tradeoff_rows_selected": 1,
    "rows": 505,
    "sha256": "d5148dd7f478d3f2c26ba06373f76f1da0ff3429b4fd3735175766e3d15cf499",
}
EXPECTED_OUTPUT_SHA256 = "625b55d1eb427b163300b2e88d1cbd91a39888947d6b3e3fe5543167712b9a2a"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v233-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v232-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v232.jsonl"
        baseline_report = directory / "v232.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v232 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v233 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "knothead_resource_tradeoff_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v233",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v233 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v233 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v232": 467,
            ...
            "projection_strategy": "delegate to the hermetic v232 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v233",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended Knot Head Nylon link row whose inspected guide gives handling and care tradeoffs",
                "score": "owner-source support, material-selection usefulness, warning completeness, and URL preservation",
