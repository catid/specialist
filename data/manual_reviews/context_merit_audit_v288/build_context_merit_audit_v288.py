#!/usr/bin/env python3
"""Turn a clipped pre-session expectation answer into negotiation guidance."""
...
DATA = ROOT / "data"
V287_DIR = DATA / "manual_reviews/context_merit_audit_v287"
sys.path[:0] = [str(ROOT), str(V287_DIR)]
import build_context_merit_audit_v287 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v288.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v288.jsonl"
REPORT = OUT_DIR / "report_context_merit_v288.json"
REVIEWER = "codex-context-merit-audit-v288"
...
SPECS = (
    {
        ...
        "active_index": 300,
        "markers": (
            "Both parties need to be very clear about what they expect from a session, whether it involves rope or not, before it starts.",
        ),
        "decision": "edit",
        "question": "What should both parties clearly establish before a rope or BDSM session starts?",
        "answer": "They should establish what each person expects, including the kind of scene they want and any hard limits.",
        "reason_code": "add_pre_session_negotiation_context",
        "reason": "The original answer was a bare fragment; the revision adds the source's concrete negotiation examples and expresses the guidance as a complete sentence.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v287",
    "direct_rows_without_prior_curation": 1,
    "clipped_answer_rows_selected": 1,
    "rows": 492,
    "sha256": "527ef93fac6f53b1dc6a433523f62244cad9978bf36103bbb56a2f61d9c0a1b5",
}
EXPECTED_OUTPUT_SHA256 = "cdaf70a32714f3a4e248618688fcfc840d10eade0cafca0e1e9c9b15e65ab2c3"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v288-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v287-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v287.jsonl"
        baseline_report = directory / "v287.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v287 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v288 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "schema": "context-merit-audit-v288",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v288 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v288 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v287": 454,
        ...
        "schema": "context-merit-audit-report-v288",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "an otherwise useful row whose answer is a clipped or ungrammatical fragment",
