#!/usr/bin/env python3
"""Complete Knot Head Nylon's stated nylon handling tradeoffs."""
...
DATA = ROOT / "data"
V181_DIR = DATA / "manual_reviews/context_merit_audit_v181"
sys.path[:0] = [str(ROOT), str(V181_DIR)]
import build_context_merit_audit_v181 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v182.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v182.jsonl"
REPORT = OUT_DIR / "report_context_merit_v182.json"
REVIEWER = "codex-context-merit-audit-v182"
...
SPECS = (
    {
        "fact_id": "fact-96ca7f10d8c5800dee8d",
        "active_index": 471,
        "markers": (
            "Nylon is more slippery than jute or hemp, so diagonals and \"frictions\" don't work well",
        ),
        "decision": "edit",
        "question": "What handling tradeoffs does Knot Head Nylon state for nylon versus natural-fiber rope?",
        "answer": "It says nylon is too slippery for structural diagonals or frictions, but is stronger, loses strength more slowly in normal use, and is unaffected by water.",
        "reason_code": "complete_nylon_handling_tradeoffs",
        "reason": "The replacement preserves the structural-friction warning and adds the vendor-attributed strength, service-life, and water-tolerance tradeoffs from the same source.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v181",
    "direct_rows_without_prior_curation": 62,
    "knothead_nylon_material_tradeoff_rows_selected": 1,
    "rows": 506,
    "sha256": "adc5cd63469b6b89ac6aec240600ef3ee0e7c910d3484c6de764c88d3a9f6068",
}
EXPECTED_OUTPUT_SHA256 = "d922029a4a4c71973841807a292484d190e97239944f2c4911ca2ef928acc0eb"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v182-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v181-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v181.jsonl"
        baseline_report = directory / "v181.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v181 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v182 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != LEGACY_URL:
            raise ValueError(f"{spec['fact_id']}: Knot Head Nylon material source lineage drift")
        audit = {
            ...
            "review_pass": "knothead_nylon_material_tradeoff_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v182",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v182 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v182 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v181": 468,
            ...
            "projection_strategy": "delegate to the hermetic v181 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v182",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the single-property nylon answer whose fully read current successor page states a complete material tradeoff",
                "score": "manual source support, material-choice usefulness, answer completeness, and training merit",
