#!/usr/bin/env python3
"""Replace a Cow-hitch alias lookup with Rope365's actual definition."""
...
DATA = ROOT / "data"
V90_DIR = DATA / "manual_reviews/context_merit_audit_v90"
sys.path[:0] = [str(ROOT), str(V90_DIR)]
import build_context_merit_audit_v90 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v91.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v91.jsonl"
REPORT = OUT_DIR / "report_context_merit_v91.json"
REVIEWER = "codex-context-merit-audit-v91"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__934720bf35bad42ba11f.json"

SPECS = (
    {
        "fact_id": "fact-d70379ce60d247a310e7",
        "active_index": 269,
        "marker": "A hitch created with two consecutive half hitches in opposite directions.",
        "decision": "edit",
        "question": "How does Rope365 define a cow hitch?",
        "answer": "A hitch made from two consecutive half hitches in opposite directions.",
        "reason_code": "replace_cow_hitch_alias_with_definition",
        "reason": "The revision replaces an alternate-name lookup with the source's functional definition of the hitch's structure.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v90",
    "direct_rows_without_prior_curation": 114,
    "rope365_vocabulary_rows_selected": 1,
    "rows": 535,
    "sha256": "c311de5876e7cc672d5275707a71864f2c68ae037ddb52f1761f78385e9b9d53",
}
EXPECTED_OUTPUT_SHA256 = "051c4462cd386195af6f8a8587c8b312b7218a938f7ffb2a22e8ce5e7860fe98"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v91-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v90-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v90.jsonl"
        baseline_report = directory / "v90.report.json"
        ...
        if len(rows) != 535 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v90 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v91 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: vocabulary-source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_vocabulary_merit_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v91",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 535 or observed["eval_fact_count"] != 612:
        raise ValueError("v91 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v91 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v90": 497,
        ...
        "schema": "context-merit-audit-report-v91",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the remaining direct Q&A from the fully read Rope365 vocabulary page",
                "score": "manual source support, functional definition value, answer grammar, and practical merit review",
