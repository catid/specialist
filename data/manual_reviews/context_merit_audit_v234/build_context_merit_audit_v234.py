#!/usr/bin/env python3
"""Connect The Duchy link to free foundations and member progression."""
...
DATA = ROOT / "data"
V233_DIR = DATA / "manual_reviews/context_merit_audit_v233"
sys.path[:0] = [str(ROOT), str(V233_DIR)]
import build_context_merit_audit_v233 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v234.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v234.jsonl"
REPORT = OUT_DIR / "report_context_merit_v234.json"
REVIEWER = "codex-context-merit-audit-v234"
...
SOURCE = DATA / "raw/owner_theduchy_resource_20260714.json"

SPECS = (
    {
        "fact_id": "fact-d45ad2b3ebf26fc99369",
        "active_index": 376,
        "markers": (
            "The supplied directory recommends The Duchy for intermediate floor-based rope-bondage lessons: https://www.theduchy.com/.",
            "The inspected home page describes an education-focused tutorial library for all experience levels, with explicit pages marked.",
            "Its free material includes scene context, consent, risk management, negotiation and planning, foundations, rope choice, basic knots, cuffs, and harnesses.",
            "Membership adds the full main library and sections about combining ties into scenes, reverse-engineering other rope work, and creating rope floggers or whips.",
        ),
        "decision": "edit",
        "question": "Where can someone find owner-recommended floor-based rope lessons at The Duchy, and how do its free and member materials differ?",
        "answer": "The Duchy is at https://www.theduchy.com/. Free material covers consent, risk management, negotiation, foundations, rope choice, and basic knots, cuffs, and harnesses; membership expands into the full library, combining ties into scenes, reverse-engineering rope work, and other advanced sections.",
        "reason_code": "connect_duchy_link_to_learning_progression",
        "reason": "The original answer preserved only the learning link. The replacement keeps the exact owner-recommended URL and adds the inspected site's free-foundation versus member-progression distinction.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v233",
    "direct_rows_without_prior_curation": 10,
    "duchy_resource_progression_rows_selected": 1,
    "rows": 505,
    "sha256": "625b55d1eb427b163300b2e88d1cbd91a39888947d6b3e3fe5543167712b9a2a",
}
EXPECTED_OUTPUT_SHA256 = "ef057d33d9efeedd7a6d9c5c18894d29c1ca4618bc45f8d458dbc1356cdfeca2"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v234-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v233-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v233.jsonl"
        baseline_report = directory / "v233.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v233 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v234 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "duchy_resource_progression_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v234",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v234 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v234 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v233": 467,
            ...
            "projection_strategy": "delegate to the hermetic v233 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v234",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended Duchy link row whose inspected home page distinguishes free foundations from member progression",
                "score": "owner-source support, learner-orientation usefulness, progression completeness, and URL preservation",
