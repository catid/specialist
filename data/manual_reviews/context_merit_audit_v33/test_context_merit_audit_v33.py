#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v33."""
...
import build_context_merit_audit_v33 as builder
from qa_quality import normalize_text


class ContextMeritAuditV33Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline_temp = tempfile.TemporaryDirectory(
            prefix=".test-baseline-v33-", dir=HERE)
        output = Path(cls.baseline_temp.name)
        cls.baseline_path = output / "projection-v32.jsonl"
        cls.baseline_report_path = output / "projection-v32.report.json"
    ...
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v33-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v33.jsonl"),
            ("CURATION", "pending_curation_context_merit_v33.jsonl"),
            ("REPORT", "report_context_merit_v33.json"),
        ):
    ...
    def test_baseline_is_exact_v32_projection(self) -> None:
        self.assertEqual(len(self.baseline), 576)
    ...
    def test_selection_is_exact_deterministic_top_ten(self) -> None:
        ...
        self.assertEqual(len(ranked), 236)
        self.assertEqual(
    ...
    def test_projected_baseline_counts_are_pinned(self) -> None:
        ...
        reviewed = builder.prior_reviewed_fact_ids()
        self.assertEqual(len(direct), 328)
        self.assertEqual(sum(row["fact_id"] in reviewed for row in direct), 92)
    ...
    def test_projection_lineage_is_complete(self) -> None:
        for row in self.audit:
            ...
            self.assertEqual(lineage["baseline_rows"], 576)
            ...
            self.assertEqual(row["review_pass"],
                             "first_context_merit_review_of_v32_projection_row")
    ...
    def test_report_counts_projection_and_sealed_policy(self) -> None:
        report = json.loads(builder.REPORT.read_text())
        self.assertEqual(report["schema"], "context-merit-audit-report-v33")
        self.assertEqual(report["audit"]["rows"], 10)
        self.assertEqual(report["selection"]["rows_selected"], 10)
        self.assertEqual(report["selection"]["eligible_unreviewed_rows"], 236)
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 1, "edit": 5, "keep": 4})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 575)
        self.assertEqual(
            projection["output_sha256"],
            "24245f86d3cceba0e1b6b119bb2cd5e519e8b2379381cae1cc1720d9a120c671")
    ...
    def test_future_versions_cannot_enter_v33_by_directory_discovery(self) -> None:
        self.assertEqual(len(builder.CONTEXT_CURATIONS), 32)
        self.assertEqual(len(builder.CONTEXT_AUDITS), 32)
        self.assertIn("context_merit_audit_v32",
                      str(builder.CONTEXT_CURATIONS[-1]))
        self.assertIn("context_merit_audit_v32",
                      str(builder.CONTEXT_AUDITS[-1]))
        self.assertFalse(any("context_merit_audit_v33" in str(path) or
                             "context_merit_audit_v34" in str(path)
                             for path in (*builder.CONTEXT_CURATIONS,
                                          *builder.CONTEXT_AUDITS)))


class IsolatedProjectionV33Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory(
            prefix=".test-projection-v33-", dir=HERE)
    ...
    def test_projection_applies_exact_v33_decisions(self) -> None:
    ...
    def test_projection_has_unique_ids_and_questions(self) -> None:
        self.assertEqual(len({row["fact_id"] for row in self.rows}), 575)
        self.assertEqual(len({row["question"] for row in self.rows}), 575)
