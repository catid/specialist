#!/usr/bin/env python3
"""Remove two low-merit Rope365 directory-description lookups."""
...
DATA = ROOT / "data"
V120_DIR = DATA / "manual_reviews/context_merit_audit_v120"
sys.path[:0] = [str(ROOT), str(V120_DIR)]
import build_context_merit_audit_v120 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v121.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v121.jsonl"
REPORT = OUT_DIR / "report_context_merit_v121.json"
REVIEWER = "codex-context-merit-audit-v121"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__f9753746348ca9cc2f11.json"

SPECS = (
    {
        "fact_id": "fact-c55a46797e12401b282b",
        "active_index": 228,
        "marker": "| Contributor Writings – Educative writings on rope bondage from other authors hosted on rope365. |",
        "decision": "drop",
        "reason_code": "low_merit_directory_section_description",
        "reason": "This tests a site's internal section description rather than transferable rope knowledge or a stable owner-recommended resource URL.",
    },
    {
        "fact_id": "fact-c65b362536b2bd96851e",
        "active_index": 458,
        "marker": "In this section, you will find a variety of resources where you can learn more about rope bondage, shibari, kinbaku.",
        "decision": "drop",
        "reason_code": "low_merit_directory_topic_terms",
        "reason": "This asks the model to recite three introductory category terms and adds no resource URL, selection guidance, or substantive rope instruction.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v120",
    "direct_rows_without_prior_curation": 92,
    "rope365_resource_directory_rows_selected": 2,
    "rows": 525,
    "sha256": "dc67811f8f152615ab9d60cbbdaf58212d66ec74d72cb8f92456750ac4ffd853",
}
EXPECTED_OUTPUT_SHA256 = "32495f9a6b9bdf71c50e1a7f9a5cc9cf54d26e768ec96b1eff1a224029811615"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v121-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v120-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v120.jsonl"
        baseline_report = directory / "v120.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 525 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v120 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v121 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: resource-directory source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_resource_directory_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v121",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 523 or observed["eval_fact_count"] != 612:
        raise ValueError("v121 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v121 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v120": 487,
            "active_after_this_tranche": 485,
            ...
            "projection_strategy": "delegate to the v120 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v121",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the two not-yet-context-audited site-directory Q&A from the fully read Rope365 resource-directory page",
                "score": "manual source support, site-metadata risk, transferable value, and owner-resource preservation",
