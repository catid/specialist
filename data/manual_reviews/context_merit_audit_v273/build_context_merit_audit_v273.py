#!/usr/bin/env python3
"""Replace a traditional-arts lookup with a balanced learning-model comparison."""
...
DATA = ROOT / "data"
V272_DIR = DATA / "manual_reviews/context_merit_audit_v272"
sys.path[:0] = [str(ROOT), str(V272_DIR)]
import build_context_merit_audit_v272 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v273.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v273.jsonl"
REPORT = OUT_DIR / "report_context_merit_v273.json"
REVIEWER = "codex-context-merit-audit-v273"
...
SOURCE = DATA / "raw/kinbakutoday_82071039cb003b58.json"

SPECS = (
    {
        "fact_id": "fact-66c1cfe4bdf44c7a70bb",
        "active_index": 136,
        "markers": (
            "Japanese culture tends to give importance to mastering repeated simple exercises prior to acting intuitively. Strict rules may be needed to maintain integrity. The Bakuyukai produced many good Kinbakushi, which shows the system working. But I have to remind you this is not the only place where we can learn.",
            "They were attending many different Nawakai, influencing each other and evolving their own styles by incorporating tips from others. These activities quickly raised the bar. Although not explicitly defined, the basic etiquette of a Nawakai is to value diversity and encourage each other by freely imparting knowledge.",
            "Maybe Nawakai is the extreme opposite of the Dojō style, but both have their attributes. Nobody can tell which is better. As Miura did, we may take positives from both styles and, of course, we can stick to one style when it fits the individual.",
        ),
        "decision": "edit",
        "question": "How does Ugo compare dojo-style and Nawakai-style kinbaku learning?",
        "answer": "Dojos use repeated exercises and strict rules that can preserve integrity, while Nawakai emphasize cross-group peer exchange and diversity; he says neither is inherently better and practitioners may draw from both.",
        "reason_code": "replace_traditional_arts_lookup_with_learning_model_comparison",
        "reason": "The replacement trades a bare performance-arts lookup for the source's balanced, transferable comparison of structured dojo learning and peer-network learning.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v272",
    "direct_rows_without_prior_curation": 1,
    "traditional_arts_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "d3d1d4f06eea1e824cb1f05b423599a1bbefe3181da616bce5ae7ca85a1f5b42",
}
EXPECTED_OUTPUT_SHA256 = "11596f8b1d7f94d9dc1ba3568b9440d3767e02f05da9c86fdccff90b9cb7d763"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v273-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v272-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v272.jsonl"
        baseline_report = directory / "v272.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v272 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v273 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "learning_model_balance_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v273",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v273 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v273 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v272": 454,
        ...
        "schema": "context-merit-audit-report-v273",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a traditional-arts lookup whose source passage supports a balanced learning-model comparison",
                "score": "transferable learning value, nuance, source support, question naturalness, and reduced name trivia",
