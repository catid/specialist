...
import build_context_merit_audit_v448 as b


class ContextMeritV448(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        b.main()
        cls.tmp = tempfile.TemporaryDirectory(prefix=".test-v448-", dir=HERE)
    ...
    def test_exact_decisions_and_512_row_projection(self):
        self.assertEqual(
            (len(self.base_rows), b.file_sha256(self.base), len(self.rows), b.file_sha256(self.out)),
            (513, b.BASELINE_SHA256, 512, b.EXPECTED_OUTPUT_SHA256),
        )
        ...
        self.assertEqual(Counter(row["action"] for row in self.curations), Counter(edit=4, drop=1))

    def test_audit_scope_and_review_classes_are_exact(self):
        self.assertEqual(len(self.audits), 10)
        self.assertEqual(Counter(row["decision"] for row in self.audits), Counter(keep=5, edit=4, drop=1))
        self.assertEqual(Counter(row["review_class"] for row in self.audits), Counter({
            "precise_emergency_boundary_keep": 2,
            "medical_uncertainty_edit": 2,
            "scene_management_keep": 2,
            "bottoming_shared_responsibility_edit": 1,
            "nerve_circulation_uncertainty_keep": 1,
            "ambiguous_consent_fragment_drop": 1,
            "injury_prevention_edit": 1,
        }))
        self.assertFalse(any(row["source"] in {"rope365", "crash_restraint"} for row in self.audits))

    def test_repairs_match_exact_qas_and_source_evidence(self):
        ...
        for old_fact_id, spec in b.SPECS.items():
            ...
            b.validate_source(spec, {**output.get(old_fact_id, {}), "fact_id": old_fact_id, "url": audits[old_fact_id]["url"]}, audits[old_fact_id]["support_evidence"])
    ...
    def test_ambiguous_consent_drop_has_operational_survivors(self):
        output = {row["fact_id"]: row for row in self.rows}
        self.assertEqual(self.report["surviving_consent_guidance"], {
            "dropped_fact": "fact-18a6771a5b827a23cbde",
            "surviving_facts": sorted(b.CONSENT_SURVIVORS),
        })
        for fact_id, expected in b.CONSENT_SURVIVORS.items():
