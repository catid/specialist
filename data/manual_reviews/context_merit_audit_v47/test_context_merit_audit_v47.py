#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v47."""
...
import build_context_merit_audit_v47 as builder
from qa_quality import normalize_text


class ContextMeritAuditV47Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v47-", dir=HERE)
    ...
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(prefix=".run-v47-", dir=HERE)
    ...
    def test_baseline_is_exact_v46_projection(self) -> None:
        self.assertEqual(len(self.baseline), 551)
    ...
    def test_selection_is_exact_deterministic_top_ten(self) -> None:
        ...
        self.assertEqual(len(ranked), 96)
    ...
    def test_decisions_and_curation_are_exact(self) -> None:
        ...
        self.assertEqual({d: sum(row["decision"] == d for row in self.audit)
                         ...
                         {"keep": 8, "drop": 2, "edit": 0})
        self.assertEqual(len(self.curation), 2)
        self.assertTrue(all(row["action"] == "drop" for row in self.curation))

    def test_keep_paraphrase_is_documented(self) -> None:
        row = next(row for row in self.audit
                   if row["fact_id"] == "fact-97f65c79582226307d96")
    ...
    def test_report_counts_projection_and_sealed_policy(self) -> None:
        ...
        self.assertEqual(report["schema"], "context-merit-audit-report-v47")
        ...
        self.assertEqual((projection["output_rows"], projection["output_sha256"]),
                         (549, "d2998dcacbe7dbc02ed8ccf6dbbab953fdc431eed3c9845d3051fa9badf4cb23"))
    ...
    def test_future_versions_cannot_enter_v47(self) -> None:
        self.assertEqual((len(builder.CONTEXT_CURATIONS),
                          len(builder.CONTEXT_AUDITS)), (46, 46))
        self.assertIn("context_merit_audit_v46",
                      str(builder.CONTEXT_CURATIONS[-1]))
        self.assertFalse(any("context_merit_audit_v47" in str(path)
                             for path in (*builder.CONTEXT_CURATIONS,
                                          *builder.CONTEXT_AUDITS)))


class IsolatedProjectionV47Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temp = tempfile.TemporaryDirectory(prefix=".projection-v47-", dir=HERE)
        output = Path(cls.temp.name)
    ...
    def test_projection_is_byte_identical_and_hash_pinned(self) -> None:
        self.assertEqual([hashlib.sha256(path.read_bytes()).hexdigest()
                         ...
                         ["d2998dcacbe7dbc02ed8ccf6dbbab953fdc431eed3c9845d3051fa9badf4cb23"] * 2)
    ...
    def test_projection_has_unique_ids_and_questions(self) -> None:
        self.assertEqual(len({row["fact_id"] for row in self.rows}), 549)
        self.assertEqual(len({row["question"] for row in self.rows}), 549)
