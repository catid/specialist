#!/usr/bin/env python3
"""Connect the owner-recommended Shibari Study link to its free primers."""
...
DATA = ROOT / "data"
V220_DIR = DATA / "manual_reviews/context_merit_audit_v220"
sys.path[:0] = [str(ROOT), str(V220_DIR)]
import build_context_merit_audit_v220 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v221.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v221.jsonl"
REPORT = OUT_DIR / "report_context_merit_v221.json"
REVIEWER = "codex-context-merit-audit-v221"
...
SOURCE = DATA / "raw/owner_shibari_study_resource_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 372,
        "markers": (
            "The supplied directory recommends Shibari Study for its membership-based intermediate suspension course catalog: https://shibaristudy.com/.",
            "The inspected public catalog also exposes a free Uplines and Lock-offs 101 course about attaching uplines to limbs or harnesses and using suspension-line lock-offs, plus a free General Rope Safety course covering rope types, anatomy, communication, consent and negotiation, cutting tools, nerve concerns, fainting, pain and risk profiles, and psychological considerations.",
        ),
        "decision": "edit",
        "question": "Where can someone browse the owner-recommended Shibari Study catalog, and which free primers are publicly listed?",
        "answer": "The catalog is at https://shibaristudy.com/. Its public listings include Uplines and Lock-offs 101 and General Rope Safety as free primers alongside the membership-based intermediate suspension catalog.",
        "reason_code": "connect_shibari_study_link_to_free_primers",
        "reason": "The original answer preserved only the catalog link. The replacement keeps the exact owner-recommended URL and identifies two inspected free entry points that help a learner orient before browsing intermediate suspension material.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v220",
    "direct_rows_without_prior_curation": 23,
    "shibari_study_resource_primers_rows_selected": 1,
    "rows": 505,
    "sha256": "b23ae773dcc7b13ae7b8b8478677855bdff6a8e7c467c86ce262d16a8621d579",
}
EXPECTED_OUTPUT_SHA256 = "f63b7079b764a658d5532c264772f745557de867ee74b89cd7fd120a4d1c3b6d"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v221-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v220-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v220.jsonl"
        baseline_report = directory / "v220.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v220 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v221 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "shibari_study_resource_primers_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v221",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v221 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v221 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v220": 467,
            ...
            "projection_strategy": "delegate to the hermetic v220 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v221",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended Shibari Study link row whose inspected catalog exposes two free primers",
                "score": "owner-source support, learner-orientation usefulness, catalog context, and URL preservation",
