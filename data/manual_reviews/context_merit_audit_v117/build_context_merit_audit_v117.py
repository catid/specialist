#!/usr/bin/env python3
"""Audit Rope365's petechiae terminology guidance."""
...
DATA = ROOT / "data"
V116_DIR = DATA / "manual_reviews/context_merit_audit_v116"
sys.path[:0] = [str(ROOT), str(V116_DIR)]
import build_context_merit_audit_v116 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v117.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v117.jsonl"
REPORT = OUT_DIR / "report_context_merit_v117.json"
REVIEWER = "codex-context-merit-audit-v117"
...
SPECS = (
    {
        "fact_id": "fact-1575d99869e41e868091",
        "active_index": 340,
        "marker": "When circulation is restricted, spots of bleeding under the skin called petechiae may appear.",
        "decision": "keep",
        "reason_code": "clear_petechiae_definition",
        "reason": "The answer accurately identifies the source's term for small spots of bleeding under the skin associated with restricted circulation.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v116",
    "direct_rows_without_prior_curation": 92,
    "rope365_petechiae_rows_selected": 1,
    "rows": 529,
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v117-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v116-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v116.jsonl"
        baseline_report = directory / "v116.report.json"
        ...
        if len(rows) != 529 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v116 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v117 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "rope365_petechiae_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v117",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 529 or observed["eval_fact_count"] != 612:
        raise ValueError("v117 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v117 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v116": 491,
        ...
        "schema": "context-merit-audit-report-v117",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the not-yet-context-audited petechiae Q&A from the fully read Rope365 safety page",
                "score": "manual source support, circulation context, terminology clarity, and practical merit review",
