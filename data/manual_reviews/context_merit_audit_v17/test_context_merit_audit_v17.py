#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v17."""
...
import build_context_merit_audit_v17 as builder
from qa_quality import normalize_text


class ContextMeritAuditV17Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v17-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v17.jsonl"),
            ("CURATION", "pending_curation_context_merit_v17.jsonl"),
            ("REPORT", "report_context_merit_v17.json"),
        ):
    ...
    def test_sources_and_evidence_are_hash_pinned(self) -> None:
        ...
        for spec in builder.SPECS:
            ...
            self.assertIn(normalize_text(row["active_answer"]),
    ...
    def test_report_projection_and_sealed_policy(self) -> None:
        ...
        self.assertEqual(report["schema"], "context-merit-audit-report-v17")
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 5, "keep": 20})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 675)
        self.assertEqual(
            projection["output_sha256"],
            "d2ed2de0c864044931042d0a3e3aca12b9516fc67d7ea35f4964169b2eb67e3a")
        self.assertEqual(projection["reviewed_keep_fact_ids_preserved"], 20)
    ...
    def test_generator_is_deterministic_and_prior_is_frozen(self) -> None:
        ...
        prior_files = tuple(
            path for version in range(1, 17)
