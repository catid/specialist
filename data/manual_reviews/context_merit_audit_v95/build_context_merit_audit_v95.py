#!/usr/bin/env python3
"""Audit the remaining direct Anatomie new-partner safety rows."""
...
DATA = ROOT / "data"
V94_DIR = DATA / "manual_reviews/context_merit_audit_v94"
sys.path[:0] = [str(ROOT), str(V94_DIR)]
import build_context_merit_audit_v94 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v95.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v95.jsonl"
REPORT = OUT_DIR / "report_context_merit_v95.json"
REVIEWER = "codex-context-merit-audit-v95"
...
SPECS = (
    {
        ...
        "active_index": 60,
        "marker": "Let them talk first – Give them space to express their needs without being influenced by you.",
        "decision": "edit",
        "question": "How can the person with more power support a new rope partner's agency during planning?",
        "answer": "Let the partner talk first so they can express their needs without being influenced.",
        "reason_code": "complete_power_aware_planning_answer",
        "reason": "The revision turns a short imperative into a complete explanation of how speaking first protects the less-powerful partner's agency.",
    },
    {
        ...
        "decision": "keep",
        "reason_code": "retain_boundary_uncertainty_guidance",
        "reason": "The existing attributed Q&A gives concise, practical guidance for handling uncertain boundaries without pressure to escalate.",
    },
    {
        ...
        "decision": "keep",
        "reason_code": "retain_consent_quality_requirements",
        "reason": "The existing attributed Q&A concisely preserves three important consent qualities from the source.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v94",
    "direct_rows_without_prior_curation": 109,
    "anatomiestudio_new_partner_rows_selected": 3,
    "rows": 533,
    "sha256": "bf2590275d3960cc97aaf4a14fb55f181b6ef0739bbd2915b42601affc64c73d",
}
EXPECTED_OUTPUT_SHA256 = "dd3670c6bffa4a9570f5ab45f159cc0406ec37909cfcdee41d21dbbef5bf7c6d"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v95-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v94-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v94.jsonl"
        baseline_report = directory / "v94.report.json"
        ...
        if len(rows) != 533 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v94 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v95 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: new-partner source lineage drift")
        audit = {
            ...
            "review_pass": "anatomiestudio_new_partner_safety_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v95",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 533 or observed["eval_fact_count"] != 612:
        raise ValueError("v95 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v95 output hash drift")
    report = {
        "active_baseline": {
        ...
        "audit": {
            "by_decision": {"drop": 0, "edit": 1, "keep": 2},
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v94": 495,
        ...
        "schema": "context-merit-audit-report-v95",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the three remaining direct Q&A from the fully read Anatomie new-partner safety article",
                "score": "manual source support, attribution, power awareness, consent quality, and practical merit review",
