#!/usr/bin/env python3
"""Focused regression tests for Anatomie new-partner safety audit v95."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v95 as b


class V95(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v95-", dir=HERE)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("anatomiestudio_new_partner_safety_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v95", {"drop": 0, "edit": 1, "keep": 2}, b.EXPECTED_OUTPUT_SHA256),
        )
