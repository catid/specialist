#!/usr/bin/env python3
"""Replace an ingredients label lookup with a reusable-components lesson."""
...
DATA = ROOT / "data"
V144_DIR = DATA / "manual_reviews/context_merit_audit_v144"
sys.path[:0] = [str(ROOT), str(V144_DIR)]
import build_context_merit_audit_v144 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v145.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v145.jsonl"
REPORT = OUT_DIR / "report_context_merit_v145.json"
REVIEWER = "codex-context-merit-audit-v145"
...
SOURCE = DATA / "raw/esinem_9a5aab43708932b3.json"

SPECS = (
    {
        "fact_id": "fact-b3524c8871da8223d6cc",
        "active_index": 325,
        "marker": "The plan for the next ShibariClasses tutorial is to demonstrate how to use ‘ingredients’ in shibari, i.e. the components that are repeated in ties, to solve a problem.",
        "secondary_marker": "In reality, almost all ties are a combination of simple ingredients used according to a few basic rules.",
        "third_marker": "Not only does the tie need to fit the person, not vice versa, but it also needs to fit the situation and purpose.",
        "decision": "edit",
        "question": "How does Esinem’s “ingredients” approach differ from memorizing fixed rope patterns?",
        "answer": "It treats ties as combinations of reusable components that can be varied or substituted to fit the person, situation, intent, and purpose.",
        "reason_code": "replace_ingredients_label_with_reusable_components",
        "reason": "The replacement teaches the source's modular problem-solving method instead of asking only for its label.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v144",
    "direct_rows_without_prior_curation": 90,
    "esinem_reusable_ingredients_rows_selected": 1,
    "rows": 509,
    "sha256": "6568a6dd175f79bb440e0ef8783e0101f85d94f3312ce7e17addb23ef66505a6",
}
EXPECTED_OUTPUT_SHA256 = "7fe4360ef622022ecd2ab1e2067e5ca415daec22deb62591e06c36e71278ef8c"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v145-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v144-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v144.jsonl"
        baseline_report = directory / "v144.report.json"
        ...
        if len(rows) != 509 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v144 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v145 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Esinem ingredients source lineage drift")
        audit = {
            ...
            "review_pass": "esinem_reusable_ingredients_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v145",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 509 or observed["eval_fact_count"] != 612:
        raise ValueError("v145 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v145 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v144": 471,
            ...
            "projection_strategy": "delegate to the hermetic v144 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v145",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the one-word ingredients Q&A whose fully read source supports a reusable-components lesson",
                "score": "manual source support, modular reasoning, transferable value, and training merit",
