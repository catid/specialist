#!/usr/bin/env python3
"""Complete Rope365's risk-reduction sequence for a new rope technique."""
...
DATA = ROOT / "data"
V174_DIR = DATA / "manual_reviews/context_merit_audit_v174"
sys.path[:0] = [str(ROOT), str(V174_DIR)]
import build_context_merit_audit_v174 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v175.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v175.jsonl"
REPORT = OUT_DIR / "report_context_merit_v175.json"
REVIEWER = "codex-context-merit-audit-v175"
...
SPECS = (
    {
        ...
        "markers": (
            ...
            "Assess if you have the proper skills to mitigate risks",
            "Adapt the type of play when trying new techniques.",
        ),
        "decision": "edit",
        "question": "How should someone reduce risk when trying a new rope technique?",
        "answer": "Research its specific risks, assess whether you have the skills to mitigate them, and first try it in low-intensity lab mode with clear communication.",
        "reason_code": "complete_new_technique_risk_reduction_sequence",
        "reason": "The replacement turns a single research reminder into the source's supported sequence of research, skill assessment, and lower-intensity testing with communication.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v174",
    "direct_rows_without_prior_curation": 69,
    "rope365_new_technique_risk_rows_selected": 1,
    "rows": 506,
    "sha256": "31fff9daccc2dc7f0c5e22dfc6b3ba3ba1be2bb1ef3e73431b23f47bcaed2c3e",
}
EXPECTED_OUTPUT_SHA256 = "577c0dfb8307bc762599d1da1711446e0b4628be2273ac62e2eb2cc7f32acc44"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v175-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v174-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v174.jsonl"
        baseline_report = directory / "v174.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v174 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v175 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Rope365 safety source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_new_technique_risk_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v175",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v175 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v175 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v174": 468,
            ...
            "projection_strategy": "delegate to the hermetic v174 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v175",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the abbreviated risk-research Q&A whose fully read source supports a complete new-technique risk-reduction sequence",
                "score": "manual source support, safety usefulness, procedural completeness, and training merit",
