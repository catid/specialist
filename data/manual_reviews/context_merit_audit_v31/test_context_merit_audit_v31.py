#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v31."""
...
import build_context_merit_audit_v31 as builder
from qa_quality import normalize_text


class ContextMeritAuditV31Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v31-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v31.jsonl"),
            ("CURATION", "pending_curation_context_merit_v31.jsonl"),
            ("REPORT", "report_context_merit_v31.json"),
        ):
    ...
    def test_secondary_selection_is_exact_and_complete(self) -> None:
        active = builder.read_jsonl(builder.ACTIVE_DATASET)
        ranked = builder.secondary_ranked(active)
        self.assertEqual(len(ranked), 5)
        self.assertEqual(tuple(item["row"]["fact_id"] for item in ranked),
                         builder.EXPECTED_SELECTION)
        selected, excluded, provenance = builder.ranked_unreviewed(active)
        self.assertEqual(tuple(item["row"]["fact_id"] for item in selected),
                         builder.EXPECTED_SELECTION)
    ...
    def test_projected_baseline_and_lineage_are_pinned(self) -> None:
        ...
        self.assertEqual(baseline["rows"], 580)
        self.assertEqual(
            baseline["sha256"],
            "3b28bdbdc78503cf1beb497630581d83751982a00d206145003eb92def3099dd")
        ...
        for fact_id in builder.EXPECTED_SELECTION:
            ...
            self.assertEqual(prior["version"], 20)
    ...
    def test_report_counts_projection_and_sealed_policy(self) -> None:
        report = json.loads(builder.REPORT.read_text())
        self.assertEqual(report["schema"], "context-merit-audit-report-v31")
        self.assertEqual(report["audit"]["rows"], 5)
        self.assertEqual(report["selection"]["rows_selected"], 5)
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 1, "edit": 4})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 579)
        self.assertEqual(
            projection["output_sha256"],
            "0707cb2678ff2e7ba6f3eac1355eac2236d4217dcf163a7bf8802be3ff09106d")
        self.assertEqual(projection["reviewed_keep_fact_ids_preserved"], 0)
    ...
    def test_future_versions_cannot_enter_v31_by_directory_discovery(self) -> None:
        self.assertEqual(len(builder.CONTEXT_CURATIONS), 30)
        self.assertEqual(len(builder.CONTEXT_AUDITS), 30)
        self.assertIn("context_merit_audit_v30",
                      str(builder.CONTEXT_CURATIONS[-1]))
        self.assertIn("context_merit_audit_v30",
                      str(builder.CONTEXT_AUDITS[-1]))
        self.assertFalse(any("context_merit_audit_v31" in str(path) or
                             "context_merit_audit_v32" in str(path)
                             for path in (*builder.CONTEXT_CURATIONS,
                                          *builder.CONTEXT_AUDITS)))


class IsolatedProjectionV31Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory(
            prefix=".test-projection-v31-", dir=HERE)
    ...
    def test_projection_applies_exact_v31_decisions(self) -> None:
    ...
    def test_projection_has_unique_ids_and_questions(self) -> None:
        self.assertEqual(len({row["fact_id"] for row in self.rows}), 579)
        self.assertEqual(len({row["question"] for row in self.rows}), 579)
