#!/usr/bin/env python3
"""Replace a direction lookup with an ergonomics-centered answer."""
...
DATA = ROOT / "data"
V256_DIR = DATA / "manual_reviews/context_merit_audit_v256"
sys.path[:0] = [str(ROOT), str(V256_DIR)]
import build_context_merit_audit_v256 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v257.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v257.jsonl"
REPORT = OUT_DIR / "report_context_merit_v257.json"
REVIEWER = "codex-context-merit-audit-v257"
...
SOURCE = DATA / "raw/esinem_4a64b6d0e45a7c34.json"

SPECS = (
    {
        "fact_id": "fact-c0fa47f4453fd6fc739e",
        "active_index": 398,
        "markers": (
            "one determining factor, beyond which feels better to you or whether you prefer to lead with your dominant hand, is ergonomics.",
            "This being that it depends on how you tie the wrist knot. It might also be determined by if you are left or right handed.",
            "Other than the above, I can’t think of any advantage to going one way or another.",
        ),
        "decision": "edit",
        "question": "According to Esinem, what should determine the direction used to tie a gote?",
        "answer": "No direction has a convincing inherent advantage; choose based on ergonomics, handedness, knot method, and smooth efficiency rather than following tradition without question.",
        "reason_code": "replace_direction_lookup_with_ergonomic_decision_rule",
        "reason": "The replacement trades a bare traditional-direction lookup for the article's practical conclusion that ergonomics and efficient handling matter more than an inherited left-or-right rule.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v256",
    "direct_rows_without_prior_curation": 1,
    "direction_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "38519110090b4464abb07f73245825338304cb154d896e10a384ea52732750f2",
}
EXPECTED_OUTPUT_SHA256 = "ff42f8b41d6e452cbfec71f252206cbcb520078970ee244f1f7751f72fd6c328"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v257-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v256-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v256.jsonl"
        baseline_report = directory / "v256.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v256 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v257 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "direction_ergonomics_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v257",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v257 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v257 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v256": 454,
        ...
        "schema": "context-merit-audit-report-v257",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a traditional-direction lookup whose source passage supports a practical ergonomic decision rule",
                "score": "transferable decision value, source support, question naturalness, and reduced rote-tradition trivia",
