#!/usr/bin/env python3
"""Focused regression tests for Rope365 safety audit v74."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v74 as b


class V74(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v74-", dir=HERE)
    ...
    def test_04_direct_count(self):
        self.assertEqual(sum(not row.get("curation") for row in self.baseline), 141)

    def test_05_decisions(self):
        self.assertEqual(
            {
                decision: sum(row["decision"] == decision for row in self.audit)
                for decision in ("keep", "edit", "drop")
            },
    ...
    def test_09_keep_unchanged(self):
        ...
        self.assertEqual(
            (projected["question"], projected["answer"]),
            (kept["active_question"], kept["active_answer"]),
        )
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                (
                    "rope365_safety_supervision_and_preparation_reaudit",
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            (
                "context-merit-audit-report-v74",
                {"edit": 3, "keep": 1},
