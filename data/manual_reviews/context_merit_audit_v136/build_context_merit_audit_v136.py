#!/usr/bin/env python3
"""Replace a vague nerve-count answer with the named nerves."""
...
DATA = ROOT / "data"
V135_DIR = DATA / "manual_reviews/context_merit_audit_v135"
sys.path[:0] = [str(ROOT), str(V135_DIR)]
import build_context_merit_audit_v135 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v136.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v136.jsonl"
REPORT = OUT_DIR / "report_context_merit_v136.json"
REVIEWER = "codex-context-merit-audit-v136"
...
SOURCE = DATA / "raw/esinem_0b1850ee9a40c337.json"

SPECS = (
    {
        "fact_id": "fact-f4afae90cc0caa77ae94",
        "active_index": 197,
        "marker": "The brachial plexus is made from five roots (C5to T1), which branches into three cords (medial, lateral and posterior), cords which branches into 5 nerves, the most important for the upper limb being the radial, median and ulnar nerves.",
        "decision": "edit",
        "question": "Which three major upper-limb nerves does the surgeon quoted by Esinem identify near the upper-cinch region?",
        "answer": "The radial, median, and ulnar nerves.",
        "reason_code": "replace_nerve_count_with_named_nerves",
        "reason": "The replacement names the three nerves and attributes the anatomy claim to the quoted surgeon instead of teaching an unusable count.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v135",
    "direct_rows_without_prior_curation": 92,
    "esinem_named_upper_cinch_nerves_rows_selected": 1,
    "rows": 511,
    "sha256": "c02b9bca5947d89b6c839b8a416439160058f458454c9a30a6294ca15eebe0d7",
}
EXPECTED_OUTPUT_SHA256 = "9a682d5a2a63a7caaa5c194f1d5b0c68271cbd5af1ff35ff932879335f5fc347"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v136-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v135-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v135.jsonl"
        baseline_report = directory / "v135.report.json"
        ...
        if len(rows) != 511 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v135 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v136 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Esinem upper-cinch source lineage drift")
        audit = {
            ...
            "review_pass": "esinem_named_upper_cinch_nerves_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v136",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 511 or observed["eval_fact_count"] != 612:
        raise ValueError("v136 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v136 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v135": 473,
            ...
            "projection_strategy": "delegate to the hermetic v135 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v136",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the nerve-count Q&A whose fully read source explicitly names the nerves",
                "score": "manual source support, anatomical specificity, attribution, and training merit",
