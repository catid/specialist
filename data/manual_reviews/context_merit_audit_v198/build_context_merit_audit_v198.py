#!/usr/bin/env python3
"""Pair X-POLE's stated use limits with the manual's load values."""
...
DATA = ROOT / "data"
V197_DIR = DATA / "manual_reviews/context_merit_audit_v197"
sys.path[:0] = [str(ROOT), str(V197_DIR)]
import build_context_merit_audit_v197 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v198.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v198.jsonl"
REPORT = OUT_DIR / "report_context_merit_v198.json"
REVIEWER = "codex-context-merit-audit-v198"
...
SPECS = (
    {
        ...
        "active_index": 29,
        "markers": (
            "The manual describes the A-FRAME as a lightweight portable frame designed for home and light-studio aerial-fitness use.",
            "X-POLE reports independent testing with a minimum breaking load of 700 kilograms.",
        ),
        "decision": "edit",
        ...
        "reason_code": "pair_frame_use_limits_with_load_values",
        "reason": "The original answer stated the intended and excluded uses but omitted the manual's corresponding breaking-load and working-load values. The replacement keeps the use limits and clearly distinguishes the reported MBL from the 5:1 WLL.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v197",
    "direct_rows_without_prior_curation": 46,
    "xpole_use_load_rows_selected": 1,
    "rows": 505,
    "sha256": "1e2c221d453fb3129be9835f45a21d13f5d2d4e702c335b9a7be2458076b96db",
}
EXPECTED_OUTPUT_SHA256 = "b48cbed164c6fe27e7460133946c25651f45aaa4b2dcdaa1cc8c23bb347d0d3b"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v198-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v197-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v197.jsonl"
        baseline_report = directory / "v197.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v197 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v198 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "xpole_use_load_completeness_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v198",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v198 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v198 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v197": 467,
            ...
            "projection_strategy": "delegate to the hermetic v197 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v198",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated X-POLE use-limit row whose fully inspected 19-page manual also provides load values",
                "score": "manual source support, load-term clarity, safety relevance, and training merit",
