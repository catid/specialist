#!/usr/bin/env python3
"""Complete a Chromaknotz construction fact without inventing a missing unit."""
...
DATA = ROOT / "data"
V192_DIR = DATA / "manual_reviews/context_merit_audit_v192"
sys.path[:0] = [str(ROOT), str(V192_DIR)]
import build_context_merit_audit_v192 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v193.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v193.jsonl"
REPORT = OUT_DIR / "report_context_merit_v193.json"
REVIEWER = "codex-context-merit-audit-v193"
...
SOURCE = DATA / "raw/chromaknotz_questions_answers_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 2,
        "markers": (
            "Chromaknotz says its rope is 1/4-inch double-braid nylon with a single-strand static breaking point of 1,280.",
        ),
        "decision": "edit",
        ...
        "reason_code": "complete_vendor_spec_without_inventing_unit",
        "reason": "The original answer omitted the breaking-point specification on the source page. The replacement preserves its construction details, adds the vendor-stated figure, and explicitly avoids inventing a unit the page does not provide.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v192",
    "chromaknotz_spec_rows_selected": 1,
    "direct_rows_without_prior_curation": 51,
    "rows": 505,
    "sha256": "15847e9f932517cb3c09b62f5b5d3fda7190221d4ccb67ba2cda1785881e3120",
}
EXPECTED_OUTPUT_SHA256 = "ee4d67c367c3ef03421bf0e8c3fcbd31834bdd7a0b631f8c9d3b2cc921ab0257"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v193-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v192-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v192.jsonl"
        baseline_report = directory / "v192.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v192 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v193 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Chromaknotz source lineage drift")
        audit = {
            ...
            "review_pass": "chromaknotz_spec_completeness_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v193",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v193 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v193 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v192": 467,
            ...
            "projection_strategy": "delegate to the hermetic v192 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v193",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated Chromaknotz construction row whose fully inspected page also states a breaking-point figure without a unit",
                "score": "manual source support, specification completeness, unit discipline, and training merit",
