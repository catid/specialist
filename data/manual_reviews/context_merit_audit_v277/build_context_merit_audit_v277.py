#!/usr/bin/env python3
"""Replace a box-tie label lookup with a mechanics-over-patterns lesson."""
...
DATA = ROOT / "data"
V276_DIR = DATA / "manual_reviews/context_merit_audit_v276"
sys.path[:0] = [str(ROOT), str(V276_DIR)]
import build_context_merit_audit_v276 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v277.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v277.jsonl"
REPORT = OUT_DIR / "report_context_merit_v277.json"
REVIEWER = "codex-context-merit-audit-v277"
...
SOURCE = DATA / "raw/esinem_f358e1984a8e04dc.json"

SPECS = (
    {
        "fact_id": "fact-c05390df79a7f67682b1",
        "active_index": 393,
        "markers": (
            "learning Shibari isn’t just about memorizing patterns; it’s about understanding the mechanics that lie beneath the surface.",
            "It’s all about how you do it and meeting your partner’s needs, be they soft and sensual or rough and passionate.",
            "The key to shibari lies in the understanding of each element’s purpose and how they can be interchanged. It’s not about mindlessly replicating ties but rather recognizing the functionality and ensuring the utmost safety and creativity in every moment.",
        ),
        "decision": "edit",
        "question": "Why does ESINEM recommend learning shibari mechanics and components instead of memorizing patterns?",
        "answer": "Understanding each element’s purpose and interchangeability lets someone adapt structures safely and creatively to a partner’s needs instead of blindly reproducing a tie.",
        "reason_code": "replace_box_tie_label_with_mechanics_over_patterns",
        "reason": "The replacement trades a box-tie label lookup for the article's transferable lesson about structural understanding, adaptation, partner needs, safety, and creativity.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v276",
    "direct_rows_without_prior_curation": 1,
    "box_tie_label_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "17d68ea1f7e52511b4830a9bb2765a21fe1db12dd7bfab0369a1ab91071c5776",
}
EXPECTED_OUTPUT_SHA256 = "6e2ec3c74f7c79c80e688294908f16ac8df498e071b9efc882d0f6763d4008ea"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v277-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v276-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v276.jsonl"
        baseline_report = directory / "v276.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v276 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v277 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "mechanics_over_patterns_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v277",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v277 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v277 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v276": 454,
        ...
        "schema": "context-merit-audit-report-v277",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a box-tie label lookup whose source passage supports a mechanics-over-patterns lesson",
                "score": "transferable structural-learning value, partner relevance, safety, source support, question naturalness, and reduced label trivia",
