#!/usr/bin/env python3
"""Replace an overlapping kindness lookup with its non-sacrifice principle."""
...
DATA = ROOT / "data"
V157_DIR = DATA / "manual_reviews/context_merit_audit_v157"
sys.path[:0] = [str(ROOT), str(V157_DIR)]
import build_context_merit_audit_v157 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v158.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v158.jsonl"
REPORT = OUT_DIR / "report_context_merit_v158.json"
REVIEWER = "codex-context-merit-audit-v158"
...
SPECS = (
    {
        ...
        "active_index": 230,
        "marker": "But kindness is not sacrifice. It is not something given at one’s expense",
        "decision": "edit",
        "question": "How does “Rope and Kindness” distinguish kindness from self-sacrifice?",
        "answer": "Kindness should not be given at one’s own expense; it can be fulfilling and enrich both giver and recipient.",
        "reason_code": "replace_overlapping_kindness_lookup_with_non_sacrifice_principle",
        "reason": "The replacement removes overlap with the partner-needs row and preserves the source’s useful boundary between kindness and self-sacrifice.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v157",
    "direct_rows_without_prior_curation": 85,
    "kinbakutoday_non_sacrifice_kindness_rows_selected": 1,
    "rows": 508,
    "sha256": "811f7de96e4c07279764081cfae855a8f23a6a08f98124ed069d33ca9de3699f",
}
EXPECTED_OUTPUT_SHA256 = "9a909a4415c1f77766853f4131d99a05485c722a05bf3eac3ff235d0d671d887"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v158-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v157-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v157.jsonl"
        baseline_report = directory / "v157.report.json"
        ...
        if len(rows) != 508 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v157 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v158 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Kinbaku Today Rope and Kindness source lineage drift")
        audit = {
            ...
            "review_pass": "kinbakutoday_non_sacrifice_kindness_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v158",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 508 or observed["eval_fact_count"] != 612:
        raise ValueError("v158 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v158 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v157": 470,
            ...
            "projection_strategy": "delegate to the hermetic v157 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v158",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the overlapping kindness Q&A whose fully read source supports a distinct non-sacrifice principle",
                "score": "manual source support, conceptual coverage, faithful synthesis, and training merit",
