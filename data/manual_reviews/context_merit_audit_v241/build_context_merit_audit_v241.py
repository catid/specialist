#!/usr/bin/env python3
"""Remove a duplicate Rope365 half-hitch limitation answer."""
...
DATA = ROOT / "data"
V240_DIR = DATA / "manual_reviews/context_merit_audit_v240"
sys.path[:0] = [str(ROOT), str(V240_DIR)]
import build_context_merit_audit_v240 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v241.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v241.jsonl"
REPORT = OUT_DIR / "report_context_merit_v241.json"
REVIEWER = "codex-context-merit-audit-v241"
...
SOURCE = DATA / "raw/rope365_0a23b686a4f493e1.json"

SPECS = (
    {
        "fact_id": "fact-9a15965e45325e947c1c",
        "active_index": 267,
        "markers": (
            "Half hitches are a great way to add structure to a tie but it requires maintained tension in order to keep its grip.",
        ),
        "decision": "drop",
        "reason_code": "drop_duplicate_half_hitch_limitation_answer",
        "reason": "This row duplicates the same answer and source sentence already represented by the adjacent active question about the limitation of a half hitch.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v240",
    "direct_rows_without_prior_curation": 1,
    "duplicate_half_hitch_rows_selected": 1,
    "rows": 503,
    "sha256": "492a06ea3ce8d5ed469dc4d17103420a0981457f71c6d3df9dd8b8517b6db85c",
}
EXPECTED_OUTPUT_SHA256 = "e58917644f91097010ce08ffb4e2fe8ce7531725d6f0eb54c7853abedaed3ab5"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v241-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v240-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v240.jsonl"
        baseline_report = directory / "v240.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 503 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v240 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v241 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "duplicate_half_hitch_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v241",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 502 or observed["eval_fact_count"] != 612:
        raise ValueError("v241 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v241 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v240": 465,
            "active_after_this_tranche": 464,
        ...
        "schema": "context-merit-audit-report-v241",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "two active half-hitch questions with the same answer and the same supporting source sentence",
