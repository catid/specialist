#!/usr/bin/env python3
"""Focused regression tests for Anatomie consent-guidance audit v98."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v98 as b


class V98(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v98-", dir=HERE)
    ...
    def test_05_decisions(self):
        self.assertEqual(
            ...
            {"keep": 4, "edit": 0, "drop": 0},
        )

    def test_06_curation(self):
        self.assertEqual((len(self.curation), {row["action"] for row in self.curation}), (0, set()))

    def test_07_support(self):
        self.assertFalse(self.curation)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("anatomiestudio_consent_guidance_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_14_pins(self):
        ...
        self.assertEqual(len(pins), 291)
        for row in pins:
            self.assertEqual(b.file_sha256(b.ROOT / row["path"]), row["sha256"])

    def test_15_action(self):
        projected = {row["fact_id"]: row for row in self.rows}
        self.assertTrue(set(b.EXPECTED_SELECTION) <= projected.keys())
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v98", {"drop": 0, "edit": 0, "keep": 4}, b.EXPECTED_OUTPUT_SHA256),
        )
