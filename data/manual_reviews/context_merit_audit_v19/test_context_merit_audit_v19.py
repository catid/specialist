#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v19."""
...
import build_context_merit_audit_v19 as builder
from qa_quality import normalize_text


class ContextMeritAuditV19Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v19-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v19.jsonl"),
            ("CURATION", "pending_curation_context_merit_v19.jsonl"),
            ("REPORT", "report_context_merit_v19.json"),
        ):
    ...
    def test_future_context_tranches_are_excluded_through_delegation(self) -> None:
        ...
        prior = manual_root / "context_merit_audit_v18" / "synthetic.jsonl"
        ...
        expected = frozenset(
            f"context_merit_audit_v{i}" for i in range(1, 19))
    ...
    def test_report_projection_and_sealed_policy(self) -> None:
        ...
        self.assertEqual(report["schema"], "context-merit-audit-report-v19")
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 11, "edit": 4, "keep": 10})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 657)
        self.assertEqual(
    ...
    def test_generator_is_deterministic_and_prior_is_frozen(self) -> None:
        ...
        prior_files = tuple(
            path for version in range(1, 19)
