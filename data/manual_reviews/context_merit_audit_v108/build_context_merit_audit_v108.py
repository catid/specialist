#!/usr/bin/env python3
"""Audit the remaining direct Rope365 safety guidance."""
...
DATA = ROOT / "data"
V107_DIR = DATA / "manual_reviews/context_merit_audit_v107"
sys.path[:0] = [str(ROOT), str(V107_DIR)]
import build_context_merit_audit_v107 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v108.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v108.jsonl"
REPORT = OUT_DIR / "report_context_merit_v108.json"
REVIEWER = "codex-context-merit-audit-v108"
...
SPECS = (
    {
        "fact_id": "fact-aa51767b03dd040b7b37",
        "active_index": 273,
        "marker": "- Have someone to check on you/supervise if you want to self-tie (known as spotter),",
        "decision": "edit",
        "question": "What does Rope365 mean by a 'spotter' for self-tying?",
        "answer": "A spotter is someone who checks on or supervises the person who is self-tying.",
        "reason_code": "complete_spotter_definition",
        "reason": "The revision replaces a terminology-only lookup with the source-supported role definition and its self-tying context.",
    },
    {
        ...
        "decision": "edit",
        "question": "What risk information should partners share before rope play, according to Rope365?",
        "answer": "Make sure each partner understands the risks involved in the specific type of play being planned.",
        "reason_code": "complete_partner_risk_disclosure",
        "reason": "The revision turns a fragment answer into a direct mutual planning rule without adding claims beyond the source.",
    },
    {
        "fact_id": "fact-52aaf228bd4fb0739e29",
        "active_index": 85,
        "marker": "- Clean up your play space to prevent tripping hazards (mats, rope bag, other people, clutter, etc.),",
        "decision": "keep",
        "reason_code": "clear_tripping_hazard_mitigation",
        "reason": "The concise answer directly states the practical mitigation and is fully supported by the source.",
    },
    {
        ...
        "decision": "keep",
        "reason_code": "clear_new_technique_risk_research",
        "reason": "The answer accurately preserves the source's actionable preparation guidance without unnecessary elaboration.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v107",
    "direct_rows_without_prior_curation": 94,
    "rope365_safety_rows_selected": 4,
    "rows": 529,
    "sha256": "24e9bc7d53461af1bc244941810a2263e7472981e4417c505b3c5b6435457a0b",
}
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v108-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v107-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v107.jsonl"
        baseline_report = directory / "v107.report.json"
        ...
        if len(rows) != 529 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v107 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v108 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: safety source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_safety_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v108",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 529 or observed["eval_fact_count"] != 612:
        raise ValueError("v108 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v108 output hash drift")
    report = {
        "active_baseline": {
        ...
        "audit": {
            "by_decision": {"drop": 0, "edit": 2, "keep": 2},
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v107": 491,
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "all remaining direct Q&A from the fully read Rope365 safety page",
                "score": "manual source support, operational safety context, answer completeness, and practical merit review",
