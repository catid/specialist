#!/usr/bin/env python3
"""Remove a duplicate Rope365 pressure-point consent answer."""
...
DATA = ROOT / "data"
V244_DIR = DATA / "manual_reviews/context_merit_audit_v244"
sys.path[:0] = [str(ROOT), str(V244_DIR)]
import build_context_merit_audit_v244 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v245.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v245.jsonl"
REPORT = OUT_DIR / "report_context_merit_v245.json"
REVIEWER = "codex-context-merit-audit-v245"
...
SOURCE = DATA / "raw/rope365_02b5a435a4822809.json"

SPECS = (
    {
        "fact_id": "fact-8b108fb6f03443ae221c",
        "active_index": 209,
        "markers": (
            "Communication is key and consent is particularly important with this kind of play.",
        ),
        "decision": "drop",
        "reason_code": "drop_duplicate_pressure_point_consent_answer",
        "reason": "This row repeats the same communication-and-consent sentence as the immediately preceding active pressure-point question, whose wording and answer are more complete.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v244",
    "direct_rows_without_prior_curation": 1,
    "duplicate_pressure_point_consent_rows_selected": 1,
    "rows": 498,
    "sha256": "d002101b402cc3727cbd01c62e6d700917051732c011c53e32d8a22051729994",
}
EXPECTED_OUTPUT_SHA256 = "754a0684676462e7f6546ca948f83472d8153305f6c074e54fcbf68d94a9c733"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v245-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v244-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v244.jsonl"
        baseline_report = directory / "v244.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 498 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v244 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v245 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "duplicate_pressure_point_consent_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v245",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 497 or observed["eval_fact_count"] != 612:
        raise ValueError("v245 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v245 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v244": 460,
            "active_after_this_tranche": 459,
        ...
        "schema": "context-merit-audit-report-v245",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "two active pressure-point communication questions with the same answer and the same supporting source sentence",
