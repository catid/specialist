#!/usr/bin/env python3
"""Focused regression tests for the v245 pressure-point consent duplicate removal."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v245 as b


class V245(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v245-", dir=HERE)
    ...
    def test_01_baseline(self):
        self.assertEqual((len(self.baseline), b.file_sha256(self.base)), (498, b.PROJECTED_SELECTION_BASELINE["sha256"]))
    ...
    def test_04_direct(self):
        self.assertEqual((sum(not row.get("curation") for row in self.baseline), sum(not row.get("curation") for row in self.rows)), (1, 1))

    def test_05_decisions(self):
        self.assertEqual(
            {decision: sum(row["decision"] == decision for row in self.audit) for decision in ("keep", "edit", "drop")},
            {"keep": 0, "edit": 0, "drop": 1},
        )

    def test_06_curation(self):
        self.assertEqual((len(self.curation), {row["action"] for row in self.curation}), (1, {"drop"}))

    def test_07_support(self):
        self.assertEqual([row["support_type"] for row in self.curation if row["action"] == "edit"], [])
    ...
    def test_08_source(self):
        for row in self.audit:
            ...
            self.assertEqual(row["url"], json.loads(b.SOURCE.read_text())["url"])
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("duplicate_pressure_point_consent_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )

    def test_11_projection(self):
        self.assertEqual((len(self.rows), b.file_sha256(self.dataset)), (497, b.EXPECTED_OUTPUT_SHA256))
    ...
    def test_14_pins(self):
        ...
        self.assertEqual(len(pins), 732)
        for row in pins:
            self.assertEqual(b.file_sha256(b.ROOT / row["path"]), row["sha256"])

    def test_15_action(self):
        projected = {row["fact_id"]: row for row in self.rows}
        keep_specs = [spec for spec in b.SPECS if spec["decision"] == "keep"]
        changed_specs = [spec for spec in b.SPECS if spec["decision"] != "keep"]
        self.assertTrue(all(spec["fact_id"] in projected for spec in keep_specs))
        self.assertTrue(all(spec["fact_id"] not in projected for spec in changed_specs))
        edited = {row.get("curation", {}).get("original_fact_id"): row for row in self.rows}
        for spec in b.SPECS:
            if spec["decision"] == "edit":
                self.assertEqual((edited[spec["fact_id"]]["question"], edited[spec["fact_id"]]["answer"]), (spec["question"], spec["answer"]))
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v245", {"drop": 1, "edit": 0, "keep": 0}, b.EXPECTED_OUTPUT_SHA256),
        )
