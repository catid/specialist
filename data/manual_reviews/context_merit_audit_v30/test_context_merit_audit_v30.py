#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v30."""
...
import build_context_merit_audit_v30 as builder
from qa_quality import normalize_text


class ContextMeritAuditV30Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v30-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v30.jsonl"),
            ("CURATION", "pending_curation_context_merit_v30.jsonl"),
            ("REPORT", "report_context_merit_v30.json"),
        ):
    ...
    def test_projected_baseline_and_lineage_are_pinned(self) -> None:
        ...
        self.assertEqual(baseline["rows"], 587)
        self.assertEqual(
            baseline["sha256"],
    ...
    def test_decisions_and_curation(self) -> None:
        ...
        self.assertEqual(
            ...
            {"keep": 1, "drop": 7, "edit": 17})
        self.assertEqual(len(self.curation), 24)
        self.assertEqual(
            {action: sum(row["action"] == action for row in self.curation)
             for action in ("drop", "edit")}, {"drop": 7, "edit": 17})
        edits = [row for row in self.curation if row["action"] == "edit"]
        self.assertEqual(
            sum(row["support_type"] == "extractive" for row in edits), 12)
        self.assertEqual(
    ...
    def test_report_projection_and_sealed_policy(self) -> None:
        report = json.loads(builder.REPORT.read_text())
        self.assertEqual(report["schema"], "context-merit-audit-report-v30")
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 7, "edit": 17, "keep": 1})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 580)
        self.assertEqual(
            projection["output_sha256"],
            "3b28bdbdc78503cf1beb497630581d83751982a00d206145003eb92def3099dd")
        self.assertEqual(projection["reviewed_keep_fact_ids_preserved"], 1)
    ...
    def test_future_versions_cannot_enter_v30_by_directory_discovery(self) -> None:
        self.assertEqual(len(builder.CONTEXT_CURATIONS), 29)
        self.assertEqual(len(builder.CONTEXT_AUDITS), 29)
        self.assertIn("context_merit_audit_v29",
                      str(builder.CONTEXT_CURATIONS[-1]))
        self.assertIn("context_merit_audit_v29",
                      str(builder.CONTEXT_AUDITS[-1]))
        self.assertFalse(any("context_merit_audit_v30" in str(path) or
                             "context_merit_audit_v31" in str(path)
                             for path in (*builder.CONTEXT_CURATIONS,
                                          *builder.CONTEXT_AUDITS)))


class IsolatedProjectionV30Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory(
            prefix=".test-projection-v30-", dir=HERE)
    ...
    def test_projection_applies_exact_v30_decisions(self) -> None:
    ...
    def test_projection_has_unique_ids_and_questions(self) -> None:
        self.assertEqual(len({row["fact_id"] for row in self.rows}), 580)
        self.assertEqual(len({row["question"] for row in self.rows}), 580)
