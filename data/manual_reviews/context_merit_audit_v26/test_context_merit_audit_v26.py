#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v26."""
...
import build_context_merit_audit_v26 as builder
from qa_quality import normalize_text


class ContextMeritAuditV26Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v26-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v26.jsonl"),
            ("CURATION", "pending_curation_context_merit_v26.jsonl"),
            ("REPORT", "report_context_merit_v26.json"),
        ):
    ...
    def test_secondary_selection_is_exact_and_ranked(self) -> None:
        ...
        self.assertEqual(len(ranked), 130)
        self.assertEqual(
    ...
    def test_projected_baseline_and_lineage_are_pinned(self) -> None:
        ...
        self.assertEqual(baseline["rows"], 614)
        self.assertEqual(
            baseline["sha256"],
    ...
    def test_sources_and_evidence_are_hash_pinned(self) -> None:
        by_id = {row["fact_id"]: row for row in self.audit}
        paraphrase = "fact-28d15f24f196d7ea55c6"
        for spec in builder.SPECS:
            ...
            if spec["fact_id"] == paraphrase:
                ...
                self.assertIn("bight accessible", row["support_evidence"])
                self.assertIn("knot can compact", row["support_evidence"])
            else:
    ...
    def test_decisions_and_curation(self) -> None:
        ...
        self.assertEqual(
            ...
            {"keep": 4, "drop": 10, "edit": 11})
        self.assertEqual(len(self.curation), 21)
        self.assertEqual(
            {action: sum(row["action"] == action for row in self.curation)
             for action in ("drop", "edit")}, {"drop": 10, "edit": 11})
        edits = [row for row in self.curation if row["action"] == "edit"]
        self.assertEqual(
            sum(row["support_type"] == "extractive" for row in edits), 10)
        self.assertEqual(
    ...
    def test_report_projection_and_sealed_policy(self) -> None:
        report = json.loads(builder.REPORT.read_text())
        self.assertEqual(report["schema"], "context-merit-audit-report-v26")
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 10, "edit": 11, "keep": 4})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 604)
        self.assertEqual(
