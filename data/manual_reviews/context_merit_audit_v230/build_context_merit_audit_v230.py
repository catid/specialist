#!/usr/bin/env python3
"""Connect the Shibari Supply bamboo link to material variability warnings."""
...
DATA = ROOT / "data"
V229_DIR = DATA / "manual_reviews/context_merit_audit_v229"
sys.path[:0] = [str(ROOT), str(V229_DIR)]
import build_context_merit_audit_v229 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v230.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v230.jsonl"
REPORT = OUT_DIR / "report_context_merit_v230.json"
REVIEWER = "codex-context-merit-audit-v230"
...
SOURCE = DATA / "raw/owner_shibari_supply_bamboo_resource_20260714.json"

SPECS = (
    {
        "fact_id": "fact-68b0c2a8c5e4c12eb2af",
        "active_index": 417,
        "markers": (
            "The supplied directory recommends Shibari Supply for bamboo sold for suspension use: https://shibarisupply.com/.",
            "The inspected product page describes hand-selected, node-to-node, flame-cured bamboo in nominal lengths from four to eight feet.",
            "Because it is natural material, the vendor says diameter is approximate at about 4 to 4.4 inches, length may vary by a few inches, and each pole may taper.",
            "The vendor explicitly makes no guarantee of the bamboo's safety, suitability, or longevity for any purpose.",
        ),
        "decision": "edit",
        "question": "Which owner-recommended supplier sells bamboo intended for suspension use, and what variability and safety caveats does its product page give?",
        "answer": "Shibari Supply is at https://shibarisupply.com/. Its product page describes nominal 4–8-foot, roughly 4–4.4-inch bamboo whose length and diameter can vary and whose poles may taper; the vendor makes no guarantee of safety, suitability, or longevity.",
        "reason_code": "connect_bamboo_link_to_variability_warning",
        "reason": "The original answer preserved only the supplier link. The replacement keeps the exact owner-recommended URL and adds the inspected natural-material variability and explicit vendor safety disclaimer.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v229",
    "direct_rows_without_prior_curation": 14,
    "bamboo_resource_warning_rows_selected": 1,
    "rows": 505,
    "sha256": "20efd0b51cced3a792862f848a8bc27bd0b900e96dd081fe365ffeaecabce8f0",
}
EXPECTED_OUTPUT_SHA256 = "a634baf6f2366c06ee501001bdc4fb186cd52b9effd6471578649bfe149a087e"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v230-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v229-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v229.jsonl"
        baseline_report = directory / "v229.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v229 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v230 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "bamboo_resource_warning_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v230",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v230 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v230 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v229": 467,
            ...
            "projection_strategy": "delegate to the hermetic v229 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v230",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended bamboo link row whose inspected product page gives variability and safety caveats",
                "score": "owner-source support, equipment-selection usefulness, warning completeness, and URL preservation",
