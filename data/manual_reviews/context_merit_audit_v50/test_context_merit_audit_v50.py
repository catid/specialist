#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v50."""
...
import build_context_merit_audit_v50 as builder


class ContextMeritAuditV50Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v50-", dir=HERE)
    ...
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(prefix=".run-v50-", dir=HERE)
    ...
    def test_baseline_is_exact_v49_projection(self) -> None:
        self.assertEqual(len(self.baseline), 547)
    ...
    def test_selection_is_exact_deterministic_top_ten(self) -> None:
        ...
        self.assertEqual(len(ranked), 66)
    ...
    def test_sources_and_evidence_are_hash_pinned(self) -> None:
        ...
        for spec in builder.SPECS:
            ...
            evidence, support = builder.previous.previous.source_evidence(
    ...
    def test_milla_principles_edit_is_exact(self) -> None:
        row = next(row for row in self.audit
                   if row["fact_id"] == "fact-c093a3dd0f041615ac43")
        self.assertEqual(row["edited_question"],
                         "Which four principles did Milla Reika say she formulated for her own kinbaku practice?")
        self.assertEqual(row["edited_answer"],
                         "control, composure, intent, and passion")
    ...
    def test_report_counts_projection_and_sealed_policy(self) -> None:
        ...
        self.assertEqual(report["schema"], "context-merit-audit-report-v50")
        ...
        self.assertEqual((projection["output_rows"], projection["output_sha256"]),
                         (546, "1d8517c1a7681f07e9381cb0ed88f3455324563d94184d5131b9969c68f79cc5"))
    ...
    def test_future_versions_cannot_enter_v50(self) -> None:
        self.assertEqual((len(builder.CONTEXT_CURATIONS),
                          len(builder.CONTEXT_AUDITS)), (49, 49))
        self.assertIn("context_merit_audit_v49",
                      str(builder.CONTEXT_CURATIONS[-1]))
        self.assertFalse(any("context_merit_audit_v50" in str(path)
                             for path in (*builder.CONTEXT_CURATIONS,
                                          *builder.CONTEXT_AUDITS)))


class IsolatedProjectionV50Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temp = tempfile.TemporaryDirectory(prefix=".projection-v50-", dir=HERE)
        output = Path(cls.temp.name)
    ...
    def test_projection_is_byte_identical_and_hash_pinned(self) -> None:
        self.assertEqual([hashlib.sha256(path.read_bytes()).hexdigest()
                         ...
                         ["1d8517c1a7681f07e9381cb0ed88f3455324563d94184d5131b9969c68f79cc5"] * 2)
    ...
    def test_projection_has_unique_ids_and_questions(self) -> None:
        self.assertEqual(len({row["fact_id"] for row in self.rows}), 546)
        self.assertEqual(len({row["question"] for row in self.rows}), 546)
