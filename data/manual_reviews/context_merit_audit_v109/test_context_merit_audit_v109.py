#!/usr/bin/env python3
"""Focused regression tests for Anatomie Studio jute-hygiene audit v109."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v109 as b


class V109(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v109-", dir=HERE)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("anatomiestudio_jute_hygiene_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v109", {"drop": 0, "edit": 0, "keep": 3}, b.EXPECTED_OUTPUT_SHA256),
        )
