#!/usr/bin/env python3
"""Connect the Subspace Designs link to materials, customization, and caveat."""
...
DATA = ROOT / "data"
V234_DIR = DATA / "manual_reviews/context_merit_audit_v234"
sys.path[:0] = [str(ROOT), str(V234_DIR)]
import build_context_merit_audit_v234 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v235.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v235.jsonl"
REPORT = OUT_DIR / "report_context_merit_v235.json"
REVIEWER = "codex-context-merit-audit-v235"
...
SOURCE = DATA / "raw/owner_subspace_designs_resource_20260714.json"

SPECS = (
    {
        "fact_id": "fact-e14d15f8a97e0a51371e",
        "active_index": 385,
        "markers": (
            "The supplied directory recommends Subspace Designs for rigging plates: https://www.subspacedesigns.shop/.",
            "The inspected shop pages describe made-to-order suspension rings and custom builds in certified 6061-T6 aluminum, with anodized finishes in multiple colors and custom concepts accepted.",
            "The home page markets the hardware as FEA and third-party tested, but the public engineering and safety page exposed placeholder content during collection, so those marketing load or testing claims should not be treated as independently verified.",
        ),
        "decision": "edit",
        "question": "Where can someone find owner-recommended Subspace Designs rigging plates, and what material, customization, and verification caveat applies?",
        "answer": "Subspace Designs is at https://www.subspacedesigns.shop/. Its pages describe made-to-order 6061-T6 aluminum suspension rings with anodized colors and custom concepts; although the vendor markets testing, the collected public engineering page contained placeholders, so load and testing claims should be independently verified.",
        "reason_code": "connect_subspace_link_to_material_and_caveat",
        "reason": "The original answer preserved only the equipment link. The replacement keeps the exact owner-recommended URL, adds inspected material and customization details, and retains the important limitation on treating vendor testing claims as verified.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v234",
    "direct_rows_without_prior_curation": 9,
    "subspace_resource_caveat_rows_selected": 1,
    "rows": 505,
    "sha256": "ef057d33d9efeedd7a6d9c5c18894d29c1ca4618bc45f8d458dbc1356cdfeca2",
}
EXPECTED_OUTPUT_SHA256 = "4b939b5dbaf6b6e850dc41fce0c0b6a9795aa8d3eaf672b5e32a46c0234ee722"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v235-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v234-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v234.jsonl"
        baseline_report = directory / "v234.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v234 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v235 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "subspace_resource_caveat_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v235",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v235 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v235 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v234": 467,
            ...
            "projection_strategy": "delegate to the hermetic v234 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v235",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended Subspace Designs link row whose inspected pages provide materials and a verification limitation",
                "score": "owner-source support, equipment-selection usefulness, caveat completeness, and URL preservation",
