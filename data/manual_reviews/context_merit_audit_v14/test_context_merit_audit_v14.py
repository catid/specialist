#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v14."""
...
import build_context_merit_audit_v14 as builder
from qa_quality import normalize_text


class ContextMeritAuditV14Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v14-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v14.jsonl"),
            ("CURATION", "pending_curation_context_merit_v14.jsonl"),
            ("REPORT", "report_context_merit_v14.json"),
        ):
    ...
    def test_future_context_tranches_are_not_historical_inputs(self) -> None:
        ...
        prior = manual_root / "context_merit_audit_v13" / "synthetic.jsonl"
        ...
        self.assertEqual(
            ...
            frozenset(f"context_merit_audit_v{i}" for i in range(1, 14)),
        )
    ...
    def test_report_projection_and_sealed_policy(self) -> None:
        ...
        self.assertEqual(report["schema"], "context-merit-audit-report-v14")
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 7, "edit": 9, "keep": 9})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 700)
        self.assertEqual(
            projection["output_sha256"],
            "9bfd45ef01cbaacddabb84ad92bfa45d854afa031ba93bfee796ff61b9906d97")
    ...
    def test_generator_is_deterministic_and_prior_is_frozen(self) -> None:
        ...
        prior_files = tuple(
            path for version in range(1, 14)
