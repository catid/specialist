#!/usr/bin/env python3
"""Turn a deGiotto material lookup into a complete ready-to-use product fact."""
...
DATA = ROOT / "data"
V194_DIR = DATA / "manual_reviews/context_merit_audit_v194"
sys.path[:0] = [str(ROOT), str(V194_DIR)]
import build_context_merit_audit_v194 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v195.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v195.jsonl"
REPORT = OUT_DIR / "report_context_merit_v195.json"
REVIEWER = "codex-context-merit-audit-v195"
...
SOURCE = DATA / "raw/degiotto_jute_30_product_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 2,
        "markers": (
            "The product page lists 30 feet of premium 6mm jute bondage rope finished with overhand knots.",
        ),
        "decision": "edit",
        ...
        "answer": "It is 30 feet of 6mm jute rope finished with overhand knots, and deGiotto says it arrives conditioned, buttered, and ready to use.",
        "reason_code": "replace_material_lookup_with_complete_product_condition",
        "reason": "The original answer reduced the product to diameter and material. The replacement preserves those facts while adding length, end finish, and the vendor's useful delivered-condition description.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v194",
    "degiotto_product_rows_selected": 1,
    "direct_rows_without_prior_curation": 49,
    "rows": 505,
    "sha256": "40fce6476814c4ac2da4ef455773359bb1a095fb068ce43ae7e34bd5bbc35194",
}
EXPECTED_OUTPUT_SHA256 = "5343494db9e5ae270faa372ec2c5200c17f633cd38353e4ed0ffbb911e06ca41"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v195-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v194-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v194.jsonl"
        baseline_report = directory / "v194.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v194 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v195 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: deGiotto product source lineage drift")
        audit = {
            ...
            "review_pass": "degiotto_product_completeness_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v195",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v195 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v195 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v194": 467,
            ...
            "projection_strategy": "delegate to the hermetic v194 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v195",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated deGiotto material lookup whose fully inspected product page supports a more complete delivered-condition fact",
                "score": "manual source support, product completeness, practical usefulness, and training merit",
