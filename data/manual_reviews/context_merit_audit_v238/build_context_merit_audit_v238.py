#!/usr/bin/env python3
"""Differentiate the two owner-recommended natural-fiber rope suppliers."""
...
DATA = ROOT / "data"
V237_DIR = DATA / "manual_reviews/context_merit_audit_v237"
sys.path[:0] = [str(ROOT), str(V237_DIR)]
import build_context_merit_audit_v237 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v238.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v238.jsonl"
REPORT = OUT_DIR / "report_context_merit_v238.json"
REVIEWER = "codex-context-merit-audit-v238"
...
SOURCE = DATA / "raw/owner_natural_fiber_suppliers_summary_20260714.json"

SPECS = (
    {
        "fact_id": "fact-ca3a4fcac4583ae3a5ca",
        "active_index": 425,
        "markers": (
            "The supplied directory recommends My Nawashi for natural-fiber bondage rope: https://www.etsy.com/shop/MyNawashi.",
            "The Etsy shop page was not reliably inspectable during this review, so the durable supported claim is limited to the owner-supplied resource name, category, and URL.",
            "The supplied directory also recommends De Giotto Rope: https://degiottorope.com/.",
            "Its inspected site lists jute and hemp shibari rope in multiple lengths and colors, spooled options, ready-to-use products, rope-care supplies and guidance, tutorial references, and classes; availability, price, and shipping details are volatile.",
        ),
        "decision": "edit",
        "question": "Which natural-fiber rope suppliers are owner-recommended, and what additional catalog information was verifiable?",
        "answer": "My Nawashi is the owner-listed Etsy resource (https://www.etsy.com/shop/MyNawashi), but its shop page was not reliably inspectable. De Giotto Rope (https://degiottorope.com/) has an inspected catalog showing jute and hemp rope in multiple lengths and colors, spooled and ready-to-use options, care supplies and guidance, tutorials, and classes.",
        "reason_code": "differentiate_natural_fiber_suppliers",
        "reason": "The original answer preserved both supplier links but did not distinguish their verified coverage. The replacement keeps both exact owner-recommended URLs, adds inspected De Giotto catalog context, and explicitly avoids inventing details for the unavailable Etsy page.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v237",
    "direct_rows_without_prior_curation": 6,
    "natural_fiber_supplier_summary_rows_selected": 1,
    "rows": 505,
    "sha256": "bd0a0af45ee16af03071c6da5e43df5fe81b07cc67f0e1ce464db5f34ffcff28",
}
EXPECTED_OUTPUT_SHA256 = "b2e6f11c8900ffa29e76570d362f60bbd5bae9c17d6fcc6ceca00e2d5de1e6ab"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v238-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v237-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v237.jsonl"
        baseline_report = directory / "v237.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v237 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v238 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "natural_fiber_supplier_summary_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v238",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v238 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v238 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v237": 467,
            "active_after_this_tranche": 467,
            ...
            "projection_strategy": "delegate to the hermetic v237 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v238",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the combined natural-fiber supplier row whose inspected coverage supports a transparent comparison",
                "score": "owner-source support, buyer-orientation usefulness, verification transparency, and URL preservation",
