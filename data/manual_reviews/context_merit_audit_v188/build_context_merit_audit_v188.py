#!/usr/bin/env python3
"""Complete Anatomie Studio's advance rope-hygiene consent checklist."""
...
DATA = ROOT / "data"
V187_DIR = DATA / "manual_reviews/context_merit_audit_v187"
sys.path[:0] = [str(ROOT), str(V187_DIR)]
import build_context_merit_audit_v187 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v188.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v188.jsonl"
REPORT = OUT_DIR / "report_context_merit_v188.json"
REVIEWER = "codex-context-merit-audit-v188"
...
SPECS = (
    {
        ...
        "markers": (
            ...
            "If the bottom is happy with ropes that are used on more intimate areas",
            "Whether the boundaries change based on the type of bodily fluid",
        ),
        "decision": "edit",
        "question": "Which rope-hygiene boundaries does Anatomie Studio recommend agreeing on before a scene?",
        "answer": "Agree where rope may touch, whether it is shared between people, whether rope used on one intimate area may touch another, and whether boundaries vary by bodily-fluid type.",
        "reason_code": "complete_rope_hygiene_consent_checklist",
        "reason": "The replacement expands a rope-placement fragment into the source's complete advance-consent checklist for sharing and intimate-area hygiene boundaries.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v187",
    "direct_rows_without_prior_curation": 56,
    "anatomiestudio_hygiene_consent_rows_selected": 1,
    "rows": 506,
    "sha256": "340cf0e2f84245f27f64fbc149bbfc981f095641c12121a88a5dc7cb85191455",
}
EXPECTED_OUTPUT_SHA256 = "bab56e8f4d450e86833931ac1c4dfbfd4cbc036e9cae3ba6aa0dd75b3439feb2"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v188-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v187-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v187.jsonl"
        baseline_report = directory / "v187.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v187 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v188 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "anatomiestudio_hygiene_consent_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v188",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v188 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v188 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v187": 468,
            ...
            "projection_strategy": "delegate to the hermetic v187 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v188",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the rope-placement fragment whose fully read source gives a complete hygiene-consent checklist",
                "score": "manual source support, hygiene usefulness, checklist completeness, and training merit",
