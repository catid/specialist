...
class V78(unittest.TestCase):
 ...
 def test_04_direct(self):self.assertEqual(sum(not r.get("curation") for r in self.baseline),130)
 def test_05_decisions(self):self.assertEqual({x:sum(r["decision"]==x for r in self.audit) for x in ("keep","edit","drop")},{"keep":3,"edit":2,"drop":0})
 ...
 def test_10_lineage(self):
  for r in self.audit:self.assertEqual((r["review_pass"],r["projection_lineage"]["baseline_sha256"]),("anatomiestudio_safeword_and_nonverbal_checkin_reaudit",b.PROJECTED_SELECTION_BASELINE["sha256"]))
 ...
 def test_14_pins(self):
  pins=self.report["frozen_prior_decision_artifacts"];self.assertEqual(len(pins),231)
 ...
 def test_19_report(self):self.assertEqual((self.report["schema"],self.report["audit"]["by_decision"],self.report["isolated_build_projection"]["output_sha256"]),("context-merit-audit-report-v78",{"edit":2,"keep":3},b.EXPECTED_OUTPUT_SHA256))
