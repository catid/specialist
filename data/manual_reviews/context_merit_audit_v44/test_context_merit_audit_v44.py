#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v44."""
...
import build_context_merit_audit_v44 as builder
from qa_quality import normalize_text


class ContextMeritAuditV44Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline_temp = tempfile.TemporaryDirectory(
            prefix=".test-baseline-v44-", dir=HERE)
        output = Path(cls.baseline_temp.name)
        cls.baseline_path = output / "projection-v43.jsonl"
        cls.baseline_report_path = output / "projection-v43.report.json"
    ...
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v44-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v44.jsonl"),
            ("CURATION", "pending_curation_context_merit_v44.jsonl"),
            ("REPORT", "report_context_merit_v44.json"),
        ):
    ...
    def test_baseline_is_exact_v43_projection(self) -> None:
        self.assertEqual(len(self.baseline), 560)
    ...
    def test_selection_is_exact_deterministic_top_ten(self) -> None:
        ...
        self.assertEqual(len(ranked), 126)
    ...
    def test_projected_baseline_counts_are_pinned(self) -> None:
        ...
        reviewed = builder.prior_reviewed_fact_ids()
        self.assertEqual(len(direct), 273)
        self.assertEqual(sum(row["fact_id"] in reviewed for row in direct), 147)
    ...
    def test_decisions_and_curation_are_exact(self) -> None:
        ...
        self.assertEqual(
            ...
            {"keep": 3, "drop": 5, "edit": 2})
        self.assertEqual(len(self.curation), 7)
        self.assertEqual(
            {action: sum(row["action"] == action for row in self.curation)
             for action in ("drop", "edit")}, {"drop": 5, "edit": 2})

    def test_edits_have_exact_support_and_context(self) -> None:
        edits = [row for row in self.curation if row["action"] == "edit"]
        self.assertEqual(len(edits), 2)
        by_id = {row["fact_id"]: row for row in edits}
        slip = by_id["fact-c0369e7b1688f40ad963"]
        self.assertEqual(slip["support_type"], "manual_paraphrase")
        self.assertEqual(slip["answer"], "They tighten and are not stable.")
        self.assertTrue(slip["paraphrase_rationale"])
        consent = by_id["fact-bac953eeaadaa1b7763c"]
        self.assertEqual(consent["support_type"], "extractive")
        self.assertEqual(
            consent["question"],
    ...
    def test_projection_lineage_is_complete(self) -> None:
        for row in self.audit:
            ...
            self.assertEqual(lineage["baseline_rows"], 560)
            ...
            self.assertEqual(row["review_pass"],
                             "first_context_merit_review_of_v43_projection_row")
    ...
    def test_report_counts_projection_and_sealed_policy(self) -> None:
        ...
        self.assertEqual(report["schema"], "context-merit-audit-report-v44")
        self.assertEqual(report["selection"]["eligible_unreviewed_rows"], 126)
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 5, "edit": 2, "keep": 3})
        ...
        self.assertEqual(projection["output_rows"], 555)
        self.assertEqual(
            projection["output_sha256"],
            "489933ac1e1ae563e4894fffbba0627b428d3384843f4d76a3a86fa9285c4d13")
    ...
    def test_future_versions_cannot_enter_v44_by_directory_discovery(self) -> None:
        self.assertEqual(len(builder.CONTEXT_CURATIONS), 43)
        self.assertEqual(len(builder.CONTEXT_AUDITS), 43)
        self.assertIn("context_merit_audit_v43",
                      str(builder.CONTEXT_CURATIONS[-1]))
        self.assertIn("context_merit_audit_v43",
                      str(builder.CONTEXT_AUDITS[-1]))
        self.assertFalse(any("context_merit_audit_v44" in str(path)
                             for path in (*builder.CONTEXT_CURATIONS,
                                          *builder.CONTEXT_AUDITS)))


class IsolatedProjectionV44Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory(
            prefix=".test-projection-v44-", dir=HERE)
    ...
    def test_projection_is_byte_identical_and_hash_pinned(self) -> None:
        ...
        self.assertEqual(len(self.rows), 555)
    ...
    def test_projection_applies_exact_decisions_and_upstream_repairs(self) -> None:
        ...
        injury = by_original["fact-b512f05ececd78e2fd3a"]
        self.assertIn("reduce how much they rig", injury["answer"])
        self.assertNotIn("to go and look", injury["answer"])
    ...
    def test_projection_has_unique_ids_and_questions(self) -> None:
        self.assertEqual(len({row["fact_id"] for row in self.rows}), 555)
        self.assertEqual(len({row["question"] for row in self.rows}), 555)
