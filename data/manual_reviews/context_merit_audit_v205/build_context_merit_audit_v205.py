#!/usr/bin/env python3
"""Complete Austin Rope Slingers' welcome row with entry conditions."""
...
DATA = ROOT / "data"
V204_DIR = DATA / "manual_reviews/context_merit_audit_v204"
sys.path[:0] = [str(ROOT), str(V204_DIR)]
import build_context_merit_audit_v204 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v205.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v205.jsonl"
REPORT = OUT_DIR / "report_context_merit_v205.json"
REVIEWER = "codex-context-merit-audit-v205"
...
SPECS = (
    {
        ...
        "active_index": 439,
        "markers": (
            "Weekly rope labs are held every Monday at varying locations across Austin, currently including community-offered venues in north and south Austin.",
            "The site says participants must be 18 or older.",
        ),
        "decision": "edit",
        "question": "Who can attend Austin Rope Slingers' weekly labs, and what entry conditions does the site state?",
        "answer": "The 18-and-older group welcomes all skill levels, abilities, and identities; attendees need neither a partner nor prerequisite rope knowledge.",
        "reason_code": "complete_inclusive_welcome_with_entry_conditions",
        "reason": "The original answer preserved the inclusive welcome but omitted the page's practical age, partner, and prerequisite conditions. The replacement makes the attendance guidance complete and actionable.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v204",
    "austin_rope_slingers_welcome_rows_selected": 1,
    "direct_rows_without_prior_curation": 39,
    "rows": 505,
    "sha256": "aa72b68b9d460a6de980e744eeef6e64cf45201da2138ea2cc6c556e1bb8e83f",
}
EXPECTED_OUTPUT_SHA256 = "0ba3ce60bc0de8e0b7a3e8df547d360a8445c7ade716e5484ff04e788dd47148"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v205-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v204-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v204.jsonl"
        baseline_report = directory / "v204.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v204 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v205 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "austin_rope_slingers_welcome_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v205",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v205 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v205 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v204": 467,
            ...
            "projection_strategy": "delegate to the hermetic v204 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v205",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated inclusive-welcome row whose fully inspected page also states age, partner, and prerequisite conditions",
                "score": "manual source support, attendance usefulness, entry-condition completeness, and training merit",
