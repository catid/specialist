#!/usr/bin/env python3
"""Audit four direct Anatomie studio etiquette and consent rows."""
...
DATA = ROOT / "data"
V95_DIR = DATA / "manual_reviews/context_merit_audit_v95"
sys.path[:0] = [str(ROOT), str(V95_DIR)]
import build_context_merit_audit_v95 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v96.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v96.jsonl"
REPORT = OUT_DIR / "report_context_merit_v96.json"
REVIEWER = "codex-context-merit-audit-v96"
...
SPECS = (
    {
        ...
        "active_index": 66,
        "marker": "Suspension is edge play. It is risky.",
        "decision": "keep",
        "reason_code": "retain_studio_suspension_risk_classification",
        "reason": "The existing attributed Q&A concisely preserves the studio's explicit risk classification for suspension.",
    },
    {
        ...
        "decision": "keep",
        "reason_code": "retain_negotiation_term_definition",
        "reason": "The existing attributed Q&A teaches the source's term for active consent conversations in a clear standalone form.",
    },
    {
        "fact_id": "fact-97058c8a514e94adf2eb",
        "active_index": 330,
        "marker": "If you would like to watch, keep to a reasonable distance and be unintrusive.",
        "decision": "keep",
        "reason_code": "retain_unintrusive_observer_guidance",
        "reason": "The existing attributed Q&A clearly preserves practical scene-observer etiquette.",
    },
    {
        "fact_id": "fact-a3833a3bcbe7d6dc8400",
        "active_index": 334,
        "marker": "Is your partner enthusiastic verbally and physically? Check in. If you have doubts, don’t proceed.",
        "decision": "edit",
        "question": "What does Anatomie Studio recommend if someone doubts a partner's verbal or physical enthusiasm?",
        "answer": "Check in, and do not proceed if doubt remains.",
        "reason_code": "complete_doubt_and_enthusiasm_response",
        "reason": "The revision attributes the guidance and adds the source's check-in step before its instruction not to proceed when enthusiasm remains doubtful.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v95",
    "direct_rows_without_prior_curation": 108,
    "anatomiestudio_etiquette_rows_selected": 4,
    "rows": 533,
    "sha256": "dd3670c6bffa4a9570f5ab45f159cc0406ec37909cfcdee41d21dbbef5bf7c6d",
}
EXPECTED_OUTPUT_SHA256 = "2253dba56880f83645581fd9b5c5e0fb571542a18ab835ae8f82901819448b42"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v96-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v95-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v95.jsonl"
        baseline_report = directory / "v95.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 533 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v95 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v96 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: etiquette source lineage drift")
        audit = {
            ...
            "review_pass": "anatomiestudio_etiquette_consent_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v96",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 533 or observed["eval_fact_count"] != 612:
        raise ValueError("v96 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v96 output hash drift")
    report = {
        "active_baseline": {
        ...
        "audit": {
            "by_decision": {"drop": 0, "edit": 1, "keep": 3},
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v95": 495,
        ...
        "schema": "context-merit-audit-report-v96",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "four direct Q&A from the fully read Anatomie studio etiquette and consent page",
                "score": "manual source support, attribution, consent actionability, scene etiquette, and practical merit review",
