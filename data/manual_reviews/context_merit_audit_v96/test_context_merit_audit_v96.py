#!/usr/bin/env python3
"""Focused regression tests for Anatomie etiquette and consent audit v96."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v96 as b


class V96(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v96-", dir=HERE)
    ...
    def test_01_baseline(self):
        self.assertEqual((len(self.baseline), b.file_sha256(self.base)), (533, b.PROJECTED_SELECTION_BASELINE["sha256"]))
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("anatomiestudio_etiquette_consent_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )

    def test_11_projection(self):
        self.assertEqual((len(self.rows), b.file_sha256(self.dataset)), (533, b.EXPECTED_OUTPUT_SHA256))
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v96", {"drop": 0, "edit": 1, "keep": 3}, b.EXPECTED_OUTPUT_SHA256),
        )
