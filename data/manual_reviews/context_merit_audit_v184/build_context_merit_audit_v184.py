#!/usr/bin/env python3
"""Explain why rope safewords need precise situational information."""
...
DATA = ROOT / "data"
V183_DIR = DATA / "manual_reviews/context_merit_audit_v183"
sys.path[:0] = [str(ROOT), str(V183_DIR)]
import build_context_merit_audit_v183 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v184.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v184.jsonl"
REPORT = OUT_DIR / "report_context_merit_v184.json"
REVIEWER = "codex-context-merit-audit-v184"
...
SOURCE = DATA / "raw/anatomiestudio_9749de0eb1ff4ef3.json"
...
SPECS = (
    {
        ...
        "active_index": 268,
        "markers": (
            "A single word or colour doesn’t provide enough information to convey the issue",
            "they need to be combined with more precise language relating to body parts, sensations and urgency.",
        ),
        "decision": "edit",
        "question": "Why should a rope safeword include the affected body part, sensation, and urgency?",
        "answer": "A word or colour alone may not identify the problem or safe response; those details help partners decide whether to adjust the tie or end the scene.",
        "reason_code": "add_rationale_to_precise_safeword_communication",
        "reason": "The replacement preserves the body-part, sensation, and urgency details while adding the source's reason they are needed to choose an appropriate response.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v183",
    "direct_rows_without_prior_curation": 60,
    "anatomiestudio_precise_safeword_rows_selected": 1,
    "rows": 506,
    "sha256": "b0cdcc23315b5143a360b902c25d6b39da5544de23806e09dce0038c9143d917",
}
EXPECTED_OUTPUT_SHA256 = "42b2d47a50f37daf9dd3a016d3dbb7f2e76ca02f235f6c09fb31db5186a4b3b1"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v184-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v183-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v183.jsonl"
        baseline_report = directory / "v183.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v183 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v184 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Anatomie Studio safeword source lineage drift")
        audit = {
            ...
            "review_pass": "anatomiestudio_precise_safeword_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v184",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v184 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v184 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v183": 468,
            ...
            "projection_strategy": "delegate to the hermetic v183 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v184",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the safeword-detail list whose fully read source explains how those details guide a safe response",
                "score": "manual source support, communication usefulness, causal completeness, and training merit",
