#!/usr/bin/env python3
"""Turn a Tetruss footprint lookup into a standard-versus-Maxximus comparison."""
...
DATA = ROOT / "data"
V191_DIR = DATA / "manual_reviews/context_merit_audit_v191"
sys.path[:0] = [str(ROOT), str(V191_DIR)]
import build_context_merit_audit_v191 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v192.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v192.jsonl"
REPORT = OUT_DIR / "report_context_merit_v192.json"
REVIEWER = "codex-context-merit-audit-v192"
...
SOURCE = DATA / "raw/tetruss_specs_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 23,
        "markers": (
            "The standard aluminum-alloy rig weighs about 21 pounds, while the Maxximus weighs about 33 pounds.",
            "The standard rig has an eight-foot triangular footprint; the Maxximus has a nine-foot triangular footprint.",
        ),
        "decision": "edit",
        "question": "How does Tetruss compare the standard rig with the Maxximus in weight, assembly time, and footprint?",
        "answer": "It lists the standard rig at about 21 pounds, five-minute assembly, and an eight-foot triangular footprint; the Maxximus at about 33 pounds, 15-minute assembly, and a nine-foot footprint.",
        "reason_code": "replace_footprint_lookup_with_model_comparison",
        "reason": "The replacement preserves the standard footprint while adding the vendor's corresponding weight, assembly-time, and Maxximus specifications for a useful model comparison.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v191",
    "direct_rows_without_prior_curation": 52,
    "tetruss_model_comparison_rows_selected": 1,
    "rows": 505,
    "sha256": "8bcef308db2b40e979c167ef3254b7251c8afc1f5f5aad37c04da6fba82c00da",
}
EXPECTED_OUTPUT_SHA256 = "15847e9f932517cb3c09b62f5b5d3fda7190221d4ccb67ba2cda1785881e3120"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v192-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v191-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v191.jsonl"
        baseline_report = directory / "v191.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v191 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v192 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Tetruss specifications source lineage drift")
        audit = {
            ...
            "review_pass": "tetruss_model_comparison_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v192",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v192 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v192 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v191": 467,
            ...
            "projection_strategy": "delegate to the hermetic v191 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v192",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated footprint lookup whose fully inspected specs page supports a standard-versus-Maxximus comparison",
                "score": "manual source support, equipment-selection usefulness, comparison completeness, and training merit",
