#!/usr/bin/env python3
"""Focused regression tests for standalone-context audit v60."""
import hashlib,json,sys,tempfile,unittest
from pathlib import Path
HERE=Path(__file__).resolve().parent;sys.path.insert(0,str(HERE));import build_context_merit_audit_v60 as b
class V60(unittest.TestCase):
 @classmethod
 def setUpClass(cls):
  cls.temp=tempfile.TemporaryDirectory(prefix=".test-v60-",dir=HERE);o=Path(cls.temp.name);cls.base=o/"base.jsonl";cls.br=o/"base.report.json";b.build_projection(cls.base,cls.br,b.PRIOR_PROJECTION_CURATIONS);cls.baseline=b.read_jsonl(cls.base);cur=(*b.PRIOR_PROJECTION_CURATIONS,b.CURATION);cls.ds=[]
 ...
 def test_06_direct_count(self):self.assertEqual(sum(not r.get("curation") for r in self.baseline),211)
 def test_07_decisions(self):self.assertEqual({x:sum(r["decision"]==x for r in self.audit) for x in ("keep","drop","edit")},{"keep":0,"drop":0,"edit":10})
 def test_08_curation(self):self.assertEqual((len(self.curation),{r["action"] for r in self.curation}),(10,{"edit"}))
 ...
 def test_11_lineage(self):
  for r in self.audit:self.assertEqual((r["review_pass"],r["projection_lineage"]["baseline_rows"],r["projection_lineage"]["baseline_sha256"]),("standalone_lesson_and_article_context_reaudit",537,b.PROJECTED_SELECTION_BASELINE["sha256"]))
 def test_12_report(self):self.assertEqual((self.report["schema"],self.report["audit"]["by_decision"],self.report["new_pending_curation"]["edit_support_types"]),("context-merit-audit-report-v60",{"edit":10},{"extractive":10,"manual_paraphrase":0}))
 def test_13_report_projection(self):self.assertEqual((self.report["isolated_build_projection"]["output_rows"],self.report["isolated_build_projection"]["output_sha256"]),(537,"c1277333dd8889cf0cee0e49be5a00d044ace042fad7bbd6ca7489bee955279c"))
 def test_14_prior_pins(self):
  pins=self.report["frozen_prior_decision_artifacts"];self.assertEqual(len(pins),177)
  for r in pins:self.assertEqual(b.file_sha256(b.ROOT/r["path"]),r["sha256"])
 def test_15_projection_hash(self):self.assertEqual([hashlib.sha256(p.read_bytes()).hexdigest() for p in self.ds],["c1277333dd8889cf0cee0e49be5a00d044ace042fad7bbd6ca7489bee955279c"]*2)
 def test_16_projection_edits(self):
  m={r.get("curation",{}).get("original_fact_id"):r for r in self.rows if r.get("curation")}
  for s in b.SPECS:self.assertEqual((m[s["fact_id"]]["question"],m[s["fact_id"]]["answer"]),(s["question"],s["answer"]))
