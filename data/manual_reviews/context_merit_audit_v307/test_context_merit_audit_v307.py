...
import build_context_merit_audit_v307 as b


class ContextMeritV307(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v307-", dir=HERE)
        d = Path(cls.temp.name)
        cls.base = d / "base.jsonl"
        b.build_baseline(cls.base, d / "base.report.json")
        cls.out, cls.projection_report = d / "out.jsonl", d / "out.report.json"
    ...
    def test_projection(self):
        self.assertEqual(
            ...
            (542, b.BASELINE_SHA256, 545, b.EXPECTED_OUTPUT_SHA256),
        )
        self.assertEqual((self.datasets[0], self.reports[0]), (self.datasets[1], self.reports[1]))

    def test_eval_duplicate_and_protocol_guards(self):
    ...
    def test_urls_capacity_and_policy(self):
        ...
        self.assertEqual(
            ...
            {"conflict_units": 3, "equipment_material": 0, "resources_general": 0, "safety_consent": 2, "technique": 1},
        )
