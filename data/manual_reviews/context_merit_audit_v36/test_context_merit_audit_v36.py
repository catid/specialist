#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v36."""
...
import build_context_merit_audit_v36 as builder
from qa_quality import normalize_text


class ContextMeritAuditV36Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline_temp = tempfile.TemporaryDirectory(
            prefix=".test-baseline-v36-", dir=HERE)
        output = Path(cls.baseline_temp.name)
        cls.baseline_path = output / "projection-v35.jsonl"
        cls.baseline_report_path = output / "projection-v35.report.json"
    ...
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v36-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v36.jsonl"),
            ("CURATION", "pending_curation_context_merit_v36.jsonl"),
            ("REPORT", "report_context_merit_v36.json"),
        ):
    ...
    def test_baseline_is_exact_v35_projection(self) -> None:
        self.assertEqual(len(self.baseline), 574)
    ...
    def test_selection_is_exact_deterministic_top_ten(self) -> None:
        ...
        self.assertEqual(len(ranked), 206)
        self.assertEqual(
    ...
    def test_projected_baseline_counts_are_pinned(self) -> None:
        ...
        self.assertEqual(sum(row["fact_id"] in reviewed for row in direct), 105)
    ...
    def test_projection_lineage_is_complete(self) -> None:
        for row in self.audit:
            ...
            self.assertEqual(lineage["baseline_rows"], 574)
            ...
            self.assertEqual(row["review_pass"],
                             "first_context_merit_review_of_v35_projection_row")
    ...
    def test_report_counts_projection_and_sealed_policy(self) -> None:
        report = json.loads(builder.REPORT.read_text())
        self.assertEqual(report["schema"], "context-merit-audit-report-v36")
        self.assertEqual(report["audit"]["rows"], 10)
        self.assertEqual(report["selection"]["rows_selected"], 10)
        self.assertEqual(report["selection"]["eligible_unreviewed_rows"], 206)
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 1, "edit": 3, "keep": 6})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 573)
        self.assertEqual(
            projection["output_sha256"],
            "b80dd2e09a5e28724a8c4865c188d6a30c00e36f28c3c0d4c694989fb590bdb1")
    ...
    def test_future_versions_cannot_enter_v36_by_directory_discovery(self) -> None:
        self.assertEqual(len(builder.CONTEXT_CURATIONS), 35)
        self.assertEqual(len(builder.CONTEXT_AUDITS), 35)
        self.assertIn("context_merit_audit_v35",
                      str(builder.CONTEXT_CURATIONS[-1]))
        self.assertIn("context_merit_audit_v35",
                      str(builder.CONTEXT_AUDITS[-1]))
        self.assertFalse(any("context_merit_audit_v36" in str(path) or
                             "context_merit_audit_v37" in str(path)
                             for path in (*builder.CONTEXT_CURATIONS,
                                          *builder.CONTEXT_AUDITS)))


class IsolatedProjectionV36Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory(
            prefix=".test-projection-v36-", dir=HERE)
    ...
    def test_projection_is_byte_identical_and_hash_pinned(self) -> None:
        ...
        self.assertEqual(len(self.rows), 573)
        self.assertEqual(self.report["eval_fact_count"], 612)

    def test_projection_applies_exact_v36_decisions(self) -> None:
    ...
    def test_projection_has_unique_ids_and_questions(self) -> None:
        self.assertEqual(len({row["fact_id"] for row in self.rows}), 573)
        self.assertEqual(len({row["question"] for row in self.rows}), 573)
