#!/usr/bin/env python3
"""Focused regression tests for context-merit audit tranche v6."""
...
import build_context_merit_audit_v6 as builder
from qa_quality import normalize_text


class ContextMeritAuditV6Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v6-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output_dir = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v6.jsonl"),
            ("CURATION", "pending_curation_context_merit_v6.jsonl"),
            ("REPORT", "report_context_merit_v6.json"),
        ):
    ...
    def test_future_context_tranches_are_not_historical_inputs(self) -> None:
        ...
        prior = manual_root / "context_merit_audit_v5" / "synthetic.jsonl"
        ...
        self.assertEqual(
            ...
            frozenset(f"context_merit_audit_v{i}" for i in range(1, 6)),
        )
    ...
    def test_selection_is_next_nonoverlapping_ranked_25(self) -> None:
        ...
        for version in (1, 2, 3, 4, 5):
            ...
            prior_ids.update(row["fact_id"] for row in builder.read_jsonl(path))
    ...
    def test_sources_and_evidence_are_hash_pinned(self) -> None:
        ...
        for specification in builder.SPECS:
            ...
            self.assertEqual(builder.file_sha256(specification["source_path"]),
    ...
    def test_report_is_pending_and_projection_is_identity_checked(self) -> None:
        ...
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 9, "edit": 4, "keep": 12})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 762)
    ...
    def test_generator_is_byte_deterministic_and_prior_tranches_are_frozen(self) -> None:
        first = tuple(builder.file_sha256(path) for path in
                      (builder.AUDIT, builder.CURATION, builder.REPORT))
        prior_files = tuple(
            path for version in (1, 2, 3, 4, 5)
            for path in sorted((builder.DATA / "manual_reviews" /
                                f"context_merit_audit_v{version}").glob("*"))
            if path.is_file())
        prior_before = tuple(builder.file_sha256(path) for path in prior_files)
        builder.main()
        second = tuple(builder.file_sha256(path) for path in
                       (builder.AUDIT, builder.CURATION, builder.REPORT))
        prior_after = tuple(builder.file_sha256(path) for path in prior_files)
    ...
    def test_active_s5_is_frozen(self) -> None:
        ...
        self.assertEqual(
            tuple(builder.file_sha256(path) for path in builder.ACTIVE_CURATIONS),
