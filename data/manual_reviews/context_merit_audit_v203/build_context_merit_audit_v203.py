#!/usr/bin/env python3
"""Pair Austin Rope Slingers' volunteer model with its event resources."""
...
DATA = ROOT / "data"
V202_DIR = DATA / "manual_reviews/context_merit_audit_v202"
sys.path[:0] = [str(ROOT), str(V202_DIR)]
import build_context_merit_audit_v202 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v203.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v203.jsonl"
REPORT = OUT_DIR / "report_context_merit_v203.json"
REVIEWER = "codex-context-merit-audit-v203"
...
SOURCE = DATA / "raw/austin_rope_slingers_home_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 47,
        "markers": (
            "Austin Rope Slingers describes itself as a volunteer-run, donation-supported peer rope group focused on in-person peer-to-peer rope education.",
            "At events, community resources include loaner rope for floor work, safety shears and straps, rope books, and negotiation reference guides.",
        ),
        "decision": "edit",
        "question": "How is Austin Rope Slingers organized, and what shared resources does it offer at events?",
        "answer": "It is a volunteer-run, donation-supported peer-education group offering loaner floor-work rope, safety shears and straps, rope books, negotiation guides, and usually community-provided suspension rigs at labs.",
        "reason_code": "pair_volunteer_model_with_event_resources",
        "reason": "The original answer stated only the group's funding and staffing model. The replacement preserves that model while adding concrete shared resources useful to a prospective attendee.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v202",
    "austin_rope_slingers_resource_rows_selected": 1,
    "direct_rows_without_prior_curation": 41,
    "rows": 505,
    "sha256": "9018abeb1a49bdb0d6f37d90a85cfe08d5cc65379c0563e75966d2a3497e29cd",
}
EXPECTED_OUTPUT_SHA256 = "d94ca6595c544dabfb2b17b82496a94e05e54b3c8c26c31744e49e2a0c791463"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v203-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v202-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v202.jsonl"
        baseline_report = directory / "v202.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v202 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v203 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Austin Rope Slingers source lineage drift")
        audit = {
            ...
            "review_pass": "austin_rope_slingers_resources_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v203",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v203 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v203 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v202": 467,
            ...
            "projection_strategy": "delegate to the hermetic v202 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v203",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated organizational-model row whose fully inspected page also lists concrete shared event resources",
                "score": "manual source support, attendee usefulness, organizational completeness, and training merit",
