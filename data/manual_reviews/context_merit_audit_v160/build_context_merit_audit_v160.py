#!/usr/bin/env python3
"""Consolidate Anatomie Studio's rope-scene observer etiquette."""
...
DATA = ROOT / "data"
V159_DIR = DATA / "manual_reviews/context_merit_audit_v159"
sys.path[:0] = [str(ROOT), str(V159_DIR)]
import build_context_merit_audit_v159 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v160.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v160.jsonl"
REPORT = OUT_DIR / "report_context_merit_v160.json"
REVIEWER = "codex-context-merit-audit-v160"
...
SPECS = (
    {
        ...
        "active_index": 313,
        "marker": "If you would like to watch, keep to a reasonable distance and be unintrusive. In particular do not interrupt a scene to ask questions, touch people or their equipment (rope included).",
        "decision": "edit",
        "question": "What etiquette does Anatomie Studio expect from someone watching a rope scene?",
        "answer": "Keep a reasonable distance, remain unintrusive, and do not interrupt or touch participants or their equipment.",
        "reason_code": "consolidate_observer_etiquette",
        "reason": "The replacement combines the source’s related distance, interruption, and no-touch guidance into one complete observer-etiquette answer.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v159",
    "direct_rows_without_prior_curation": 83,
    "anatomiestudio_observer_etiquette_rows_selected": 1,
    "rows": 508,
    "sha256": "36abe94280b60a3de1a5fcd6a958bf8fee0b501ff3f638f0b6c16ff93a82bb86",
}
EXPECTED_OUTPUT_SHA256 = "6d16afc27321c8ac74b369f46043e247c77c72366df51a65ab5903bce1bc046d"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v160-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v159-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v159.jsonl"
        baseline_report = directory / "v159.report.json"
        ...
        if len(rows) != 508 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v159 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v160 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Anatomie Studio etiquette source lineage drift")
        audit = {
            ...
            "review_pass": "anatomiestudio_observer_etiquette_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v160",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 508 or observed["eval_fact_count"] != 612:
        raise ValueError("v160 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v160 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v159": 470,
            ...
            "projection_strategy": "delegate to the hermetic v159 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v160",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the observer-distance Q&A whose fully read source supports a complete etiquette checklist",
                "score": "manual source support, social usefulness, completeness, and training merit",
