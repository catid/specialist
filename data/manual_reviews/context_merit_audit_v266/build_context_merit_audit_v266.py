#!/usr/bin/env python3
"""Replace an aesthetic-term lookup with a model-centered explanation."""
...
DATA = ROOT / "data"
V265_DIR = DATA / "manual_reviews/context_merit_audit_v265"
sys.path[:0] = [str(ROOT), str(V265_DIR)]
import build_context_merit_audit_v265 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v266.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v266.jsonl"
REPORT = OUT_DIR / "report_context_merit_v266.json"
REVIEWER = "codex-context-merit-audit-v266"
...
SOURCE = DATA / "raw/kinbakutoday_f57559bbb4c8b826.json"

SPECS = (
    {
        "fact_id": "fact-f0faf9784dacf1893fe5",
        "active_index": 120,
        "markers": (
            "The focus of Kinbiken was always to bring out the beauty of the model, rather than to focus on the beauty of the rope.",
            "For others it is the expression of emotion, feeling, and communication which makes rope beautiful.",
            "What is beautiful is not the rope itself, but what it reveals in the woman who is tied.",
        ),
        "decision": "edit",
        "question": "How does “Nureki, Kinbiken, and the Aesthetics of Kinbaku” describe Kinbiken’s model-centered aesthetic?",
        "answer": "Kinbiken treated rope as a means: beauty lay in the emotion, communication, and experience it drew from the person tied, rather than primarily in decorative or technical ropework.",
        "reason_code": "replace_aesthetic_term_lookup_with_model_centered_explanation",
        "reason": "The replacement trades a bare aesthetic-term lookup for the article's substantive contrast between rope-centered technique and the emotion and expression revealed in the person tied.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v265",
    "direct_rows_without_prior_curation": 1,
    "aesthetic_term_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "47adeeabf3fe29b30d76e99872739ef1360c3efa01fe5c6c73178257400e468e",
}
EXPECTED_OUTPUT_SHA256 = "8c39a349e5dd7ef5c87da72ce5be3c9b5d795036ef6e7f895a891f1c488412fb"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v266-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v265-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v265.jsonl"
        baseline_report = directory / "v265.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v265 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v266 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "model_centered_aesthetic_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v266",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v266 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v266 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v265": 454,
        ...
        "schema": "context-merit-audit-report-v266",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "an aesthetic-term lookup whose source passage supports a model-centered explanation",
                "score": "relational and interpretive value, source support, question naturalness, and reduced terminology trivia",
