#!/usr/bin/env python3
"""Focused regression tests for the v266 model-centered-aesthetic replacement."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v266 as b


class V266(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v266-", dir=HERE)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("model_centered_aesthetic_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v266", {"drop": 0, "edit": 1, "keep": 0}, b.EXPECTED_OUTPUT_SHA256),
        )
