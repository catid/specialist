#!/usr/bin/env python3
"""Replace the bare partner-vetting answer with Crash Restraint's process."""
...
DATA = ROOT / "data"
V184_DIR = DATA / "manual_reviews/context_merit_audit_v184"
sys.path[:0] = [str(ROOT), str(V184_DIR)]
import build_context_merit_audit_v184 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v185.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v185.jsonl"
REPORT = OUT_DIR / "report_context_merit_v185.json"
REVIEWER = "codex-context-merit-audit-v185"
...
SPECS = (
    {
        ...
        "active_index": 440,
        "markers": (
            "Checking potential partners through the group and by asking other local bottoms is not a silver bullet",
        ),
        "decision": "edit",
        "question": "How does Crash Restraint recommend screening a potential rope partner?",
        "answer": "Check them through the Rope Bottoms' Share Group and ask other local bottoms; neither is foolproof, but both can reduce risk.",
        "reason_code": "replace_bare_partner_vetting_answer_with_process",
        "reason": "The replacement turns the bare 'other local bottoms' answer into the source's two-channel screening process and preserves its no-silver-bullet caveat.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v184",
    "direct_rows_without_prior_curation": 59,
    "crash_restraint_partner_vetting_rows_selected": 1,
    "rows": 506,
    "sha256": "42b2d47a50f37daf9dd3a016d3dbb7f2e76ca02f235f6c09fb31db5186a4b3b1",
}
EXPECTED_OUTPUT_SHA256 = "869d98123a9d608e606ff6f94253d3c4fa3b62cdc1c0aba79766f849fee096d8"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v185-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v184-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v184.jsonl"
        baseline_report = directory / "v184.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v184 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v185 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Crash Restraint getting-started source lineage drift")
        audit = {
            ...
            "review_pass": "crash_restraint_partner_vetting_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v185",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v185 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v185 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v184": 468,
            ...
            "projection_strategy": "delegate to the hermetic v184 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v185",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the bare partner-vetting answer whose fully read source gives a two-channel process and limitation",
                "score": "manual source support, community-safety usefulness, procedural completeness, and training merit",
