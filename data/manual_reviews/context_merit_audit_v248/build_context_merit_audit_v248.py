#!/usr/bin/env python3
"""Remove a duplicate consent-withdrawal answer."""
...
DATA = ROOT / "data"
V247_DIR = DATA / "manual_reviews/context_merit_audit_v247"
sys.path[:0] = [str(ROOT), str(V247_DIR)]
import build_context_merit_audit_v247 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v248.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v248.jsonl"
REPORT = OUT_DIR / "report_context_merit_v248.json"
REVIEWER = "codex-context-merit-audit-v248"
...
SOURCE = DATA / "raw/rope365_c73bc6fb66977a2d.json"

SPECS = (
    {
        "fact_id": "fact-592b71b40df773ed7a43",
        "active_index": 13,
        "markers": (
            "Consent can be withdrawn at any point during play.",
        ),
        "decision": "drop",
        "reason_code": "drop_duplicate_consent_withdrawal_answer",
        "reason": "This source-attributed lookup duplicates a later active question asking whether consent can be withdrawn after tying starts; the later row has the same supported fact in a complete sentence.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v247",
    "direct_rows_without_prior_curation": 1,
    "duplicate_consent_withdrawal_rows_selected": 1,
    "rows": 495,
    "sha256": "80397e8f0396d455d6d83b34bf73e7c125559ebe47466dda2af4941578adb2f9",
}
EXPECTED_OUTPUT_SHA256 = "bf188cc4dbeae5eef05ca7959f0a503bee7161e93cf8285254ff06fbb6428c88"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v248-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v247-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v247.jsonl"
        baseline_report = directory / "v247.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 495 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v247 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v248 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "duplicate_consent_withdrawal_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v248",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 494 or observed["eval_fact_count"] != 612:
        raise ValueError("v248 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v248 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v247": 457,
            "active_after_this_tranche": 456,
        ...
        "schema": "context-merit-audit-report-v248",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "two active consent-withdrawal questions supported by the same source sentence and carrying the same factual answer",
