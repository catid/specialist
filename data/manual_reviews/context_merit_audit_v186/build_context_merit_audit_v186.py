#!/usr/bin/env python3
"""Complete Crash Restraint's instructor-understanding test."""
...
DATA = ROOT / "data"
V185_DIR = DATA / "manual_reviews/context_merit_audit_v185"
sys.path[:0] = [str(ROOT), str(V185_DIR)]
import build_context_merit_audit_v185 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v186.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v186.jsonl"
REPORT = OUT_DIR / "report_context_merit_v186.json"
REVIEWER = "codex-context-merit-audit-v186"
...
SOURCE = DATA / "raw/crash_restraint_getting_started_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 269,
        "markers": (
            "Ask whether what you are told makes sense and whether it agrees with what you have learned from other sources.",
            "ask lots of \"why\" questions; if an instructor cannot answer",
        ),
        "decision": "edit",
        "question": "How does Crash Restraint suggest testing a rope instructor's understanding?",
        "answer": "Ask many \"why\" questions, check whether the answers make sense and agree with other sources, and be wary if the instructor cannot explain them.",
        "reason_code": "complete_instructor_understanding_test",
        "reason": "The replacement expands the bare 'why questions' answer into the source's reasoning, cross-checking, and inability-to-explain signals.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v185",
    "direct_rows_without_prior_curation": 58,
    "crash_restraint_instructor_evaluation_rows_selected": 1,
    "rows": 506,
    "sha256": "869d98123a9d608e606ff6f94253d3c4fa3b62cdc1c0aba79766f849fee096d8",
}
EXPECTED_OUTPUT_SHA256 = "84e841e6c18fdab7c8d69240a7388f199432ce6c59136ca051382b99aaf96d16"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v186-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v185-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v185.jsonl"
        baseline_report = directory / "v185.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v185 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v186 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Crash Restraint instructor-evaluation source lineage drift")
        audit = {
            ...
            "review_pass": "crash_restraint_instructor_evaluation_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v186",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v186 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v186 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v185": 468,
            ...
            "projection_strategy": "delegate to the hermetic v185 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v186",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the bare 'why questions' answer whose fully read source gives a complete instructor-evaluation method",
                "score": "manual source support, education-safety usefulness, procedural completeness, and training merit",
