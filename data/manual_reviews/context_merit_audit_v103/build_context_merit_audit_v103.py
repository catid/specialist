#!/usr/bin/env python3
"""Complete the Rope365 box-tie cinch-pressure safety guidance."""
...
DATA = ROOT / "data"
V102_DIR = DATA / "manual_reviews/context_merit_audit_v102"
sys.path[:0] = [str(ROOT), str(V102_DIR)]
import build_context_merit_audit_v102 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v103.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v103.jsonl"
REPORT = OUT_DIR / "report_context_merit_v103.json"
REVIEWER = "codex-context-merit-audit-v103"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__6fcf974226515064661b.json"

SPECS = (
    {
        "fact_id": "fact-47a176a9c9f1245c596f",
        "active_index": 167,
        "marker": "- Cinches (if any) do not press into the armpit or inside of the arms",
        "decision": "edit",
        "question": "Where should box-tie cinches avoid pressing around the upper arms?",
        "answer": "Keep cinches out of the armpits and away from the insides of the arms.",
        "reason_code": "complete_cinch_pressure_guidance",
        "reason": "The revision turns an awkward singular body-part lookup into clear, self-contained placement guidance while preserving both unsafe locations named by the source.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v102",
    "direct_rows_without_prior_curation": 102,
    "rope365_box_tie_hand_position_rows_selected": 1,
    "rows": 532,
    "sha256": "1e5b60510f8bf8878a0937f69bb20100e2ed6bb29381b9cf6bcf178b843f68d5",
}
EXPECTED_OUTPUT_SHA256 = "2fa1f56060e9a6a987c35590a68afad699cc08d5bdf4475b19f43f72d9d147b4"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v103-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v102-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v102.jsonl"
        baseline_report = directory / "v102.report.json"
        ...
        if len(rows) != 532 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v102 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v103 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: box-tie hand-position source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_box_tie_hand_position_safety_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v103",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 532 or observed["eval_fact_count"] != 612:
        raise ValueError("v103 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v103 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v102": 494,
        ...
        "schema": "context-merit-audit-report-v103",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the direct cinch-pressure Q&A from the fully read Rope365 box-tie hand-position page",
                "score": "manual source support, upper-arm pressure safety completeness, answer context, and practical merit review",
