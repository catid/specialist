#!/usr/bin/env python3
"""Connect The Empty Space calendar link to the space's durable identity."""
...
DATA = ROOT / "data"
V221_DIR = DATA / "manual_reviews/context_merit_audit_v221"
sys.path[:0] = [str(ROOT), str(V221_DIR)]
import build_context_merit_audit_v221 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v222.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v222.jsonl"
REPORT = OUT_DIR / "report_context_merit_v222.json"
REVIEWER = "codex-context-merit-audit-v222"
...
SOURCE = DATA / "raw/owner_empty_space_calendar_resource_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 373,
        "markers": (
            "The supplied directory recommends The Empty Space's public schedule for Austin rope-community events: https://www.atxempty.space/space-schedule.",
            "The live page is titled Schedule and identifies The Empty Space as a Shibari Study Studio Partner.",
            "The official About page describes The Empty Space as a queer, POC-owned and operated community space for gathering, learning, experience, and growth, with a foundation of respect, awareness, agency, and radical self-expression.",
        ),
        "decision": "edit",
        "question": "Where can someone check The Empty Space's public event calendar, and how does the space describe itself?",
        "answer": "The schedule is at https://www.atxempty.space/space-schedule. The Empty Space describes itself as a queer, POC-owned and operated community space for gathering, learning, experience, and growth, and the schedule page identifies it as a Shibari Study Studio Partner.",
        "reason_code": "connect_empty_space_calendar_to_identity",
        "reason": "The original answer preserved only the schedule link. The replacement keeps the exact owner-recommended URL and adds the official site's durable identity and studio-partner context without copying volatile event details.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v221",
    "direct_rows_without_prior_curation": 22,
    "empty_space_calendar_identity_rows_selected": 1,
    "rows": 505,
    "sha256": "f63b7079b764a658d5532c264772f745557de867ee74b89cd7fd120a4d1c3b6d",
}
EXPECTED_OUTPUT_SHA256 = "6df3330cc1280ee1960dcda98aac9cac70b92a0f521f05e2736bf6877a3938e7"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v222-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v221-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v221.jsonl"
        baseline_report = directory / "v221.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v221 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v222 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "empty_space_calendar_identity_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v222",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v222 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v222 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v221": 467,
            ...
            "projection_strategy": "delegate to the hermetic v221 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v222",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended Empty Space schedule link row that lacked durable identity context",
                "score": "owner-source support, Austin-resource usefulness, nonvolatile context, and URL preservation",
