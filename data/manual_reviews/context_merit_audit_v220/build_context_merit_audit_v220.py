#!/usr/bin/env python3
"""Connect the owner-recommended EMT shears link to cutter readiness."""
...
DATA = ROOT / "data"
V219_DIR = DATA / "manual_reviews/context_merit_audit_v219"
sys.path[:0] = [str(ROOT), str(V219_DIR)]
import build_context_merit_audit_v219 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v220.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v220.jsonl"
REPORT = OUT_DIR / "report_context_merit_v220.json"
REVIEWER = "codex-context-merit-audit-v220"
...
SOURCE = DATA / "raw/owner_emt_shears_resource_20260714.json"

SPECS = (
    {
        "fact_id": "fact-6dc2d6b921d3d72b3d33",
        "active_index": 261,
        "markers": (
            "The supplied directory recommends this EMT shears listing for emergency cutting equipment: https://www.amazon.com/dp/B0195QI218.",
            "Crash Restraint identifies safety cutters as the most important non-rope item in a rope bag.",
            "Its rope-kit guidance also recommends testing that the chosen cutters work well on the actual rope being used, for example when cutting rope to create shorter lengths.",
        ),
        "decision": "edit",
        "question": "Where is the owner-recommended EMT shears listing, and what cutter-readiness check should accompany it?",
        "answer": "The listing is https://www.amazon.com/dp/B0195QI218. Safety cutters should be kept in the rope bag and tested beforehand on the actual rope being used to confirm they cut it effectively.",
        "reason_code": "connect_emt_shears_link_to_readiness_check",
        "reason": "The original answer preserved only the product link. The replacement keeps that exact owner-recommended URL and adds the inspected safety guidance that cutters belong in the rope bag and should be tested on the actual rope.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v219",
    "direct_rows_without_prior_curation": 24,
    "emt_shears_readiness_rows_selected": 1,
    "rows": 505,
    "sha256": "ebed69627ae665c0f0044194bf2eab18fbfb3b09d404ee97c55d6a9305864e1d",
}
EXPECTED_OUTPUT_SHA256 = "b23ae773dcc7b13ae7b8b8478677855bdff6a8e7c467c86ce262d16a8621d579"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v220-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v219-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v219.jsonl"
        baseline_report = directory / "v219.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v219 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v220 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "emt_shears_readiness_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v220",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v220 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v220 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v219": 467,
            ...
            "projection_strategy": "delegate to the hermetic v219 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v220",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended EMT shears link row that lacked a cutter-readiness check",
                "score": "owner-source support, emergency-readiness usefulness, safety context, and URL preservation",
