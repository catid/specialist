#!/usr/bin/env python3
"""Focused regression tests for source-attribution audit v62."""
import hashlib,json,sys,tempfile,unittest
from pathlib import Path
HERE=Path(__file__).resolve().parent;sys.path.insert(0,str(HERE));import build_context_merit_audit_v62 as b
class V62(unittest.TestCase):
 @classmethod
 def setUpClass(cls):
  cls.temp=tempfile.TemporaryDirectory(prefix=".test-v62-",dir=HERE);o=Path(cls.temp.name);cls.base=o/"base.jsonl";cls.br=o/"base.report.json";b.build_projection(cls.base,cls.br,b.PRIOR_PROJECTION_CURATIONS);cls.baseline=b.read_jsonl(cls.base);cur=(*b.PRIOR_PROJECTION_CURATIONS,b.CURATION);cls.ds=[]
 ...
 def test_06_direct_count(self):self.assertEqual(sum(not r.get("curation") for r in self.baseline),193)
 ...
 def test_11_lineage(self):
  for r in self.audit:self.assertEqual((r["review_pass"],r["projection_lineage"]["baseline_rows"],r["projection_lineage"]["baseline_sha256"]),("source_attribution_and_historical_subject_reaudit",537,b.PROJECTED_SELECTION_BASELINE["sha256"]))
 def test_12_report(self):self.assertEqual((self.report["schema"],self.report["audit"]["by_decision"],self.report["new_pending_curation"]["edit_support_types"]),("context-merit-audit-report-v62",{"edit":9},{"extractive":9,"manual_paraphrase":0}))
 ...
 def test_14_prior_pins(self):
  pins=self.report["frozen_prior_decision_artifacts"];self.assertEqual(len(pins),183)
