#!/usr/bin/env python3
"""Audit the remaining direct Rope365 consent-and-communication guidance."""
...
DATA = ROOT / "data"
V106_DIR = DATA / "manual_reviews/context_merit_audit_v106"
sys.path[:0] = [str(ROOT), str(V106_DIR)]
import build_context_merit_audit_v106 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v107.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v107.jsonl"
REPORT = OUT_DIR / "report_context_merit_v107.json"
REVIEWER = "codex-context-merit-audit-v107"
...
SPECS = (
    {
        ...
        "active_index": 221,
        "marker": "Choosing a partner with whom you have good communication is essential. The more you tie, the higher the risk that something will go wrong. You need to pick someone you can trust to deal with difficult situations. It is good practice to make the first contact with a new partner in a public space (or with trusted people around), have a safety call (someone who will check back on you after a certain period of time), and check for references.",
        "decision": "edit",
        "question": "What does Rope365 mean by arranging a safety call before meeting a new rope partner?",
        "answer": "Arrange for a trusted person to check back on you after an agreed period of time.",
        "reason_code": "complete_safety_call_definition",
        "reason": "The revision replaces a fragment with a self-contained definition that makes the outside check-in arrangement and timing explicit.",
    },
    {
        "fact_id": "fact-4cc9e4229c016704eeee",
        "active_index": 381,
        "marker": "Once all the ropes are off, the session is not necessarily over. Intense feelings might need some aftercare. Everyone deals with emotions differently. It is best to discuss this beforehand, when emotions aren’t high, but you might also need to adapt depending on the circumstances. Some people may need some display of affection in order to make peace with what happened. Other people are hungry and thirsty. And some just need solo time to process their feelings. Everyone involved may need aftercare. The necessity for the person receiving rope is expected, and it is important to remember that tying can also lead to intense emotions and physical soreness. Don’t forget to assess the state of being of the person who just tied and see what they need.",
        "decision": "keep",
        "reason_code": "clear_aftercare_timing",
        "reason": "The answer directly and accurately preserves the source's practical recommendation to discuss aftercare before emotions are elevated.",
    },
    {
        "fact_id": "fact-63c654d8cdad2602da36",
        "active_index": 445,
        "marker": "An allowlist gives a better result than a blocklist. Say what is good to you, what you expect, and not just what to avoid. “Anything but this and this” is likely to lead to “oops I forgot to say this and that.”. Don’t accept “everything is fine,” “you can do anything you want to me.” It may sound hot, but in reality, it’s rarely the case. Ask some edge cases instead “How would you feel about having a cucumber inserted inside your anus?”, “How do you feel about permanent scars?”",
        "decision": "drop",
        "reason_code": "redundant_allowlist_lookup",
        "reason": "This terminology-only lookup duplicates active fact-bbcafd2058e69d88b452, which preserves the allowlist recommendation in its more useful negotiation context.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v106",
    "direct_rows_without_prior_curation": 96,
    "rope365_communication_rows_selected": 3,
    "rows": 530,
    "sha256": "1bd01b48ca4b191c42f8a7e8f666fc133ade0eef6611527e78202fdadf94f0a9",
}
EXPECTED_OUTPUT_SHA256 = "24e9bc7d53461af1bc244941810a2263e7472981e4417c505b3c5b6435457a0b"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v107-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v106-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v106.jsonl"
        baseline_report = directory / "v106.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 530 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v106 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v107 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: communication source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_communication_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v107",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 529 or observed["eval_fact_count"] != 612:
        raise ValueError("v107 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v107 output hash drift")
    report = {
        "active_baseline": {
        ...
        "audit": {
            "by_decision": {"drop": 1, "edit": 1, "keep": 1},
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v106": 492,
        ...
        "schema": "context-merit-audit-report-v107",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "all remaining direct Q&A from the fully read Rope365 consent-and-communication page",
                "score": "manual source support, semantic redundancy, communication context, answer completeness, and practical merit review",
