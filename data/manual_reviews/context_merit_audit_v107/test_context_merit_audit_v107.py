#!/usr/bin/env python3
"""Focused regression tests for Rope365 consent-and-communication audit v107."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v107 as b


class V107(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v107-", dir=HERE)
    ...
    def test_01_baseline(self):
        self.assertEqual((len(self.baseline), b.file_sha256(self.base)), (530, b.PROJECTED_SELECTION_BASELINE["sha256"]))
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("rope365_communication_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )

    def test_11_projection(self):
        self.assertEqual((len(self.rows), b.file_sha256(self.dataset)), (529, b.EXPECTED_OUTPUT_SHA256))
    ...
    def test_14_pins(self):
        ...
        self.assertEqual(len(pins), 318)
        for row in pins:
            self.assertEqual(b.file_sha256(b.ROOT / row["path"]), row["sha256"])

    def test_15_action(self):
        spec = next(spec for spec in b.SPECS if spec["decision"] == "edit")
        row = next(row for row in self.rows if row.get("curation", {}).get("original_fact_id") == spec["fact_id"])
        self.assertEqual((row["question"], row["answer"]), (spec["question"], spec["answer"]))
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v107", {"drop": 1, "edit": 1, "keep": 1}, b.EXPECTED_OUTPUT_SHA256),
        )
