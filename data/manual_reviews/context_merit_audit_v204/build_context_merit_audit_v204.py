#!/usr/bin/env python3
"""Turn Austin Rope Slingers' education label into concrete learning options."""
...
DATA = ROOT / "data"
V203_DIR = DATA / "manual_reviews/context_merit_audit_v203"
sys.path[:0] = [str(ROOT), str(V203_DIR)]
import build_context_merit_audit_v203 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v204.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v204.jsonl"
REPORT = OUT_DIR / "report_context_merit_v204.json"
REVIEWER = "codex-context-merit-audit-v204"
...
SPECS = (
    {
        ...
        "decision": "edit",
        "question": "What learning options and entry requirements does Austin Rope Slingers describe for its weekly labs?",
        "answer": "It offers in-person peer education with a weekly 101-level class and member-requested or member-offered higher-level sessions and intensives; attendees need neither a partner nor prerequisite knowledge.",
        "reason_code": "replace_education_label_with_learning_options",
        "reason": "The original answer named peer education without showing what that means in practice. The replacement adds the weekly beginner class, advanced member-driven options, and explicit lack of partner or knowledge prerequisites.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v203",
    "austin_rope_slingers_education_rows_selected": 1,
    "direct_rows_without_prior_curation": 40,
    "rows": 505,
    "sha256": "d94ca6595c544dabfb2b17b82496a94e05e54b3c8c26c31744e49e2a0c791463",
}
EXPECTED_OUTPUT_SHA256 = "aa72b68b9d460a6de980e744eeef6e64cf45201da2138ea2cc6c556e1bb8e83f"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v204-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v203-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v203.jsonl"
        baseline_report = directory / "v203.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v203 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v204 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "austin_rope_slingers_education_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v204",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v204 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v204 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v203": 467,
            ...
            "projection_strategy": "delegate to the hermetic v203 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v204",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated education-focus label whose fully inspected page lists concrete beginner, advanced, and no-prerequisite options",
                "score": "manual source support, attendee usefulness, learning-path clarity, and training merit",
