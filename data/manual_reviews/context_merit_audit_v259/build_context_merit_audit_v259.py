#!/usr/bin/env python3
"""Replace an inspiration-name lookup with a direct safety warning."""
...
DATA = ROOT / "data"
V258_DIR = DATA / "manual_reviews/context_merit_audit_v258"
sys.path[:0] = [str(ROOT), str(V258_DIR)]
import build_context_merit_audit_v258 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v259.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v259.jsonl"
REPORT = OUT_DIR / "report_context_merit_v259.json"
REVIEWER = "codex-context-merit-audit-v259"
...
SOURCE = DATA / "raw/kinbakutoday_3fe23acd4ef143eb.json"

SPECS = (
    {
        "fact_id": "fact-a471d9dc641493cc2784",
        "active_index": 426,
        "markers": (
            "Kinbaku is highly dangerous discipline, and is not something done without thinking. We are holding other people’s life in our hands. My advice is: don’t approach this too lightly. People can be very severely damaged because of that.",
        ),
        "decision": "edit",
        "question": "What safety warning did Naka Akira give to new riggers?",
        "answer": "He warned that kinbaku is highly dangerous, must not be approached casually or without thought, and can severely injure people whose lives are in the rigger’s hands.",
        "reason_code": "replace_inspiration_lookup_with_new_rigger_safety_warning",
        "reason": "The replacement trades a bare historical-inspiration lookup for the interview's explicit, directly useful warning to new riggers about severe injury risk and responsibility.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v258",
    "direct_rows_without_prior_curation": 1,
    "inspiration_name_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "2473ee0b334dc034a7b41aa1432bc5f8abd6d671e3b8c1f025ce5b4fc9c3515f",
}
EXPECTED_OUTPUT_SHA256 = "ce44814eccae520e0cfcdd308928499bab48fca7875e90de91e27e37bbaabdc7"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v259-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v258-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v258.jsonl"
        baseline_report = directory / "v258.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v258 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v259 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "new_rigger_safety_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v259",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v259 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v259 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v258": 454,
        ...
        "schema": "context-merit-audit-report-v259",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "an inspiration-name lookup whose source passage supports an explicit safety warning to new riggers",
                "score": "direct safety value, source support, question naturalness, and reduced biographical trivia",
