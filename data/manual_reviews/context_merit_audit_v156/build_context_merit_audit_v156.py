#!/usr/bin/env python3
"""Expand a rope-conditioning dust location into a precaution checklist."""
...
DATA = ROOT / "data"
V155_DIR = DATA / "manual_reviews/context_merit_audit_v155"
sys.path[:0] = [str(ROOT), str(V155_DIR)]
import build_context_merit_audit_v155 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v156.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v156.jsonl"
REPORT = OUT_DIR / "report_context_merit_v156.json"
REVIEWER = "codex-context-merit-audit-v156"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__c1314f53c65df4af2c20.json"

SPECS = (
    {
        "fact_id": "fact-ea3c8d2b8a4c06b768e0",
        "active_index": 402,
        "marker": "Breaking the rope makes a lot of dust, make sure to do this outside or in a well-ventilated area. Masks, glasses and/or gloves are also recommended.",
        "decision": "edit",
        "question": "What dust precautions does Rope365 recommend when breaking or polishing rope?",
        "answer": "Work outside or in a well-ventilated area, and consider a mask, glasses, and gloves.",
        "reason_code": "expand_conditioning_dust_precautions",
        "reason": "The replacement preserves the ventilation guidance while adding the source’s recommended protective equipment for a more complete safety answer.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v155",
    "direct_rows_without_prior_curation": 87,
    "rope365_conditioning_dust_rows_selected": 1,
    "rows": 508,
    "sha256": "b73bc670876ac8e2a326088adeb091215ca898ef810188c27a6d9e300d0ea4a1",
}
EXPECTED_OUTPUT_SHA256 = "ff5ad59f6f0b6d5c7e4ebe01ecdf7d76256b05f22d35ff8529bb83aacd084f95"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v156-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v155-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v155.jsonl"
        baseline_report = directory / "v155.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 508 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v155 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v156 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Rope365 rope-care source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_conditioning_dust_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v156",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 508 or observed["eval_fact_count"] != 612:
        raise ValueError("v156 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v156 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v155": 470,
            "active_after_this_tranche": 470,
            ...
            "projection_strategy": "delegate to the hermetic v155 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v156",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the conditioning-dust location Q&A whose fully read source supports a more complete precaution checklist",
                "score": "manual source support, safety completeness, actionability, and training merit",
