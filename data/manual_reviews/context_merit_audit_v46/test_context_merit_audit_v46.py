#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v46."""
...
import build_context_merit_audit_v46 as builder
from qa_quality import normalize_text


class ContextMeritAuditV46Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v46-", dir=HERE)
    ...
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(prefix=".run-v46-", dir=HERE)
    ...
    def test_baseline_is_exact_v45_projection(self) -> None:
        self.assertEqual(len(self.baseline), 552)
    ...
    def test_selection_is_exact_deterministic_top_ten(self) -> None:
        ...
        self.assertEqual(len(ranked), 106)
    ...
    def test_decisions_and_curation_are_exact(self) -> None:
        ...
        self.assertEqual({d: sum(row["decision"] == d for row in self.audit)
                         ...
                         {"keep": 7, "drop": 1, "edit": 2})
        self.assertEqual(len(self.curation), 3)
        self.assertEqual({a: sum(row["action"] == a for row in self.curation)
                          for a in ("drop", "edit")},
                         {"drop": 1, "edit": 2})

    def test_edits_are_documented_manual_paraphrases(self) -> None:
        edits = {row["fact_id"]: row for row in self.curation
                 if row["action"] == "edit"}
        self.assertEqual(set(edits), {"fact-d63b471f2cb9a7bf95f9",
                                     "fact-05a050c66a8ee25a8fee"})
        self.assertTrue(all(row["support_type"] == "manual_paraphrase"
                            and row["paraphrase_rationale"] for row in edits.values()))
        self.assertIn("Warm up the arms", edits[
            "fact-d63b471f2cb9a7bf95f9"]["answer"])
        self.assertIn("distinct facets", edits[
    ...
    def test_report_counts_projection_and_sealed_policy(self) -> None:
        ...
        self.assertEqual(report["schema"], "context-merit-audit-report-v46")
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 1, "edit": 2, "keep": 7})
        ...
        self.assertEqual((projection["output_rows"], projection["output_sha256"]),
                         (551, "898af65291f91e80f9f2d8ee548eaecb8f065861cf5ba95f43f452891814a3b6"))
    ...
    def test_future_versions_cannot_enter_v46(self) -> None:
        self.assertEqual((len(builder.CONTEXT_CURATIONS),
                          len(builder.CONTEXT_AUDITS)), (45, 45))
        self.assertIn("context_merit_audit_v45",
                      str(builder.CONTEXT_CURATIONS[-1]))
        self.assertFalse(any("context_merit_audit_v46" in str(path)
                             for path in (*builder.CONTEXT_CURATIONS,
                                          *builder.CONTEXT_AUDITS)))


class IsolatedProjectionV46Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temp = tempfile.TemporaryDirectory(prefix=".projection-v46-", dir=HERE)
        output = Path(cls.temp.name)
    ...
    def test_projection_is_byte_identical_and_hash_pinned(self) -> None:
        self.assertEqual([hashlib.sha256(path.read_bytes()).hexdigest()
                         ...
                         ["898af65291f91e80f9f2d8ee548eaecb8f065861cf5ba95f43f452891814a3b6"] * 2)
    ...
    def test_projection_applies_exact_decisions_and_upstream_repairs(self) -> None:
        ...
        for spec in builder.SPECS:
            ...
            if spec["decision"] == "keep":
                ...
                self.assertEqual((by_original[fact_id]["question"],
                                  by_original[fact_id]["answer"]),
                                 (spec["question"], spec["answer"]))
        self.assertNotIn("fact-93c032484cf3a72fcc5c", by_id)
        self.assertIn("rope or kink activity", by_original[
            "fact-bac953eeaadaa1b7763c"]["question"])
    ...
    def test_projection_has_unique_ids_and_questions(self) -> None:
        self.assertEqual(len({row["fact_id"] for row in self.rows}), 551)
        self.assertEqual(len({row["question"] for row in self.rows}), 551)
