#!/usr/bin/env python3
"""Replace a resource-title lookup with in-page box-tie safety guidance."""
...
DATA = ROOT / "data"
V268_DIR = DATA / "manual_reviews/context_merit_audit_v268"
sys.path[:0] = [str(ROOT), str(V268_DIR)]
import build_context_merit_audit_v268 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v269.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v269.jsonl"
REPORT = OUT_DIR / "report_context_merit_v269.json"
REVIEWER = "codex-context-merit-audit-v269"
...
SOURCE = DATA / "raw/rope365_5c6e72d053d47d66.json"

SPECS = (
    {
        "fact_id": "fact-3788d596fe21028f3b5a",
        "active_index": 419,
        "markers": (
            "Hand sensations and movement (closing and opening hands) should be continuously monitored to ensure everything is in working order.",
            "Continuously monitor hand movement and sensation while the tie is on. A common tip is to scratch the palms and the top of the thumb to monitor the first signs of sensation loss. Be ready to untie quickly and cut the ropes with control in case of emergency.",
        ),
        "decision": "edit",
        "question": "What does Rope365 say should be monitored continuously in a one-rope box tie, and how should someone be prepared to respond?",
        "answer": "Monitor hand movement and sensation throughout; be ready to untie quickly or cut the ropes in a controlled way if warning signs appear or an emergency occurs.",
        "reason_code": "replace_resource_title_lookup_with_in_page_box_tie_safety",
        "reason": "The replacement trades an off-page resource-title lookup for the source page's direct, actionable guidance on continuous monitoring and emergency response.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v268",
    "direct_rows_without_prior_curation": 1,
    "resource_title_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "f702b13fb5f3bbe5175bb056d9fd7dd6a08ccd7f78be4dac9d0af949d7588119",
}
EXPECTED_OUTPUT_SHA256 = "54bcd61936a247d9b34a33c0fe09179173a37e2f1a183f8f826138f9e1522ee7"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v269-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v268-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v268.jsonl"
        baseline_report = directory / "v268.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v268 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v269 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "in_page_box_tie_safety_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v269",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v269 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v269 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v268": 454,
        ...
        "schema": "context-merit-audit-report-v269",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a resource-title lookup whose own source page supports direct box-tie safety guidance",
                "score": "direct safety value, page relevance, source support, question naturalness, and reduced title trivia",
