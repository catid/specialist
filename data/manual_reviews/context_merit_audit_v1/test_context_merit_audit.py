...
class ContextMeritAuditTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v1-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output_dir = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v1.jsonl"),
            ("CURATION", "pending_curation_context_merit_v1.jsonl"),
            ("REPORT", "report_context_merit_v1.json"),
        ):
