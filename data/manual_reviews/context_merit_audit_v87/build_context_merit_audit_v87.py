#!/usr/bin/env python3
"""Replace a bare friction definition with Rope365's practical tradeoff."""
...
DATA = ROOT / "data"
V86_DIR = DATA / "manual_reviews/context_merit_audit_v86"
sys.path[:0] = [str(ROOT), str(V86_DIR)]
import build_context_merit_audit_v86 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v87.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v87.jsonl"
REPORT = OUT_DIR / "report_context_merit_v87.json"
REVIEWER = "codex-context-merit-audit-v87"
...
SPECS = (
    {
        ...
        "active_index": 162,
        "marker": "Frictions are great because they are fast to tie and don’t compact into something hard to untie but they require tension to stay in place.",
        "decision": "edit",
        ...
        "reason_code": "replace_bare_friction_definition_with_tradeoff",
        "reason": "The revision replaces a bare mechanism fragment with the source's more useful balance of speed, ease of untying, and tension dependence.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v86",
    "direct_rows_without_prior_curation": 119,
    "rope365_frictions_rows_selected": 1,
    "rows": 536,
    "sha256": "fc969da7d8f648339366d6dacdd70bf86e6b8652b0ffb40dcd66142e32433f05",
}
EXPECTED_OUTPUT_SHA256 = "c968ab3d367010308c530316d7564788edeaecad8a0f108dcc1833d7d64d149f"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v87-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v86-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v86.jsonl"
        baseline_report = directory / "v86.report.json"
        ...
        if len(rows) != 536 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v86 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v87 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: frictions-source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_frictions_merit_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v87",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 536 or observed["eval_fact_count"] != 612:
        raise ValueError("v87 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v87 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v86": 498,
        ...
        "schema": "context-merit-audit-report-v87",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the remaining direct Q&A from the fully read Rope365 frictions page",
                "score": "manual source support, practical tradeoff value, answer grammar, and instructional merit review",
