#!/usr/bin/env python3
"""Replace a censorship-year lookup with a culture/technique comparison."""
...
DATA = ROOT / "data"
V143_DIR = DATA / "manual_reviews/context_merit_audit_v143"
sys.path[:0] = [str(ROOT), str(V143_DIR)]
import build_context_merit_audit_v143 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v144.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v144.jsonl"
REPORT = OUT_DIR / "report_context_merit_v144.json"
REVIEWER = "codex-context-merit-audit-v144"
...
SOURCE = DATA / "raw/kinbakutoday_241155b848764148.json"

SPECS = (
    {
        "fact_id": "fact-0d8a7a5611c26f1a53e7",
        "active_index": 17,
        "marker": "In Japan, technique developed over a long period of time as the result of an emerging culture of kinbaku. Culture gave birth to technique.",
        "secondary_marker": "Instead of the practice of kinbaku emerging from culture (as is the case in Japan) in the west we have a culture emerging from the practice of kinbaku. Technique is giving birth to culture.",
        "third_marker": "Our communities may define what we do, but it is our culture that defines why we do it.",
        "decision": "edit",
        "question": "How does “Rope: Community and Culture” contrast the relationship between rope culture and technique in Japan and the West?",
        "answer": "It argues that Japanese rope culture developed first and shaped technique over decades, while imported technique spread first in the West and a local culture grew around the practice.",
        "reason_code": "replace_censorship_year_with_culture_technique_comparison",
        "reason": "The replacement teaches the article's central comparative argument instead of testing an isolated censorship year.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v143",
    "direct_rows_without_prior_curation": 90,
    "kinbakutoday_culture_technique_comparison_rows_selected": 1,
    "rows": 509,
    "sha256": "fa63bdec1cca97f40610106538c4fe47da6d6a57aad538f56276b436cfe8533e",
}
EXPECTED_OUTPUT_SHA256 = "6568a6dd175f79bb440e0ef8783e0101f85d94f3312ce7e17addb23ef66505a6"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v144-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v143-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v143.jsonl"
        baseline_report = directory / "v143.report.json"
        ...
        if len(rows) != 509 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v143 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v144 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        support = "\n".join((evidence(document, spec["marker"]), evidence(document, spec["secondary_marker"]), evidence(document, spec["third_marker"])))
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Rope Community and Culture source lineage drift")
        audit = {
            ...
            "review_pass": "kinbakutoday_culture_technique_comparison_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v144",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 509 or observed["eval_fact_count"] != 612:
        raise ValueError("v144 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v144 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v143": 471,
            ...
            "projection_strategy": "delegate to the hermetic v143 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v144",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the censorship-year Q&A whose fully read source supports a central culture/technique comparison",
                "score": "manual source support, conceptual coverage, transferable value, and training merit",
