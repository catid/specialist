#!/usr/bin/env python3
"""Focused regression tests for the v144 culture/technique rewrite."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v144 as b


class V144(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v144-", dir=HERE)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("kinbakutoday_culture_technique_comparison_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v144", {"drop": 0, "edit": 1, "keep": 0}, b.EXPECTED_OUTPUT_SHA256),
        )
