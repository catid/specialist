#!/usr/bin/env python3
"""Explain how Tethered Together's participant-driven Ropenspace works."""
...
DATA = ROOT / "data"
V201_DIR = DATA / "manual_reviews/context_merit_audit_v201"
sys.path[:0] = [str(ROOT), str(V201_DIR)]
import build_context_merit_audit_v201 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v202.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v202.jsonl"
REPORT = OUT_DIR / "report_context_merit_v202.json"
REVIEWER = "codex-context-merit-audit-v202"
...
SOURCE = DATA / "raw/tethered_together_ropenspace_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 80,
        "markers": (
            "Tethered Together calls Ropenspace facilitated, self-organizing peer education run using Open Space Technology.",
        ),
        "decision": "edit",
        "question": "How does Tethered Together's Ropenspace organize peer learning?",
        "answer": "It uses facilitated Open Space Technology: participants propose questions or interests, share what they know, and move among evolving classes, discussions, and hands-on skill shares instead of following a staff-selected curriculum.",
        "reason_code": "replace_ropenspace_label_with_participant_process",
        "reason": "The original answer named the organizational method without explaining it. The replacement preserves Open Space Technology and adds the participant-led proposal, sharing, and evolving-session process described on the page.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v201",
    "direct_rows_without_prior_curation": 42,
    "tethered_ropenspace_rows_selected": 1,
    "rows": 505,
    "sha256": "5719a26a0d66c31cdd297bd23cc7fb18f9fe4c8feb531f9ac24987f8b253ce64",
}
EXPECTED_OUTPUT_SHA256 = "9018abeb1a49bdb0d6f37d90a85cfe08d5cc65379c0563e75966d2a3497e29cd"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v202-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v201-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v201.jsonl"
        baseline_report = directory / "v201.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v201 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v202 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Tethered Together Ropenspace source lineage drift")
        audit = {
            ...
            "review_pass": "tethered_ropenspace_process_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v202",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v202 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v202 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v201": 467,
            ...
            "projection_strategy": "delegate to the hermetic v201 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v202",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated Ropenspace label whose fully inspected page explains the participant-driven learning process",
                "score": "manual source support, process clarity, peer-learning usefulness, and training merit",
