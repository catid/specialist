#!/usr/bin/env python3
"""Normalize a clipped UV-and-wipes answer into a complete sentence."""
...
DATA = ROOT / "data"
V279_DIR = DATA / "manual_reviews/context_merit_audit_v279"
sys.path[:0] = [str(ROOT), str(V279_DIR)]
import build_context_merit_audit_v279 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v280.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v280.jsonl"
REPORT = OUT_DIR / "report_context_merit_v280.json"
REVIEWER = "codex-context-merit-audit-v280"
...
SPECS = (
    {
        ...
        "active_index": 484,
        "markers": (
            "UV and antibac wipes almost certainly won’t reach every spot of your ropes, because of the gaps between strands (and even the gaps between yarns within the strands).",
        ),
        "decision": "edit",
        "question": "Why may UV light and antibacterial wipes fail to reach every spot on a jute rope?",
        "answer": "Light and surface wipes cannot reliably penetrate the gaps between strands or the smaller gaps between yarns within each strand.",
        "reason_code": "normalize_clipped_uv_wipes_answer",
        "reason": "The original answer was a bare noun fragment; the revision preserves the same supported explanation in a grammatical, self-contained sentence.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v279",
    "direct_rows_without_prior_curation": 1,
    "clipped_answer_rows_selected": 1,
    "rows": 492,
    "sha256": "4aa52b3d8e1f794bce6fb2b49834133a510dd00466557f81b08b3df130447b28",
}
EXPECTED_OUTPUT_SHA256 = "f1143f1513b61047309c6809d70255948480bc7e55175618a0ff5b0386a5d9b0"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v280-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v279-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v279.jsonl"
        baseline_report = directory / "v279.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v279 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v280 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "schema": "context-merit-audit-v280",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v280 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v280 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v279": 454,
        ...
        "schema": "context-merit-audit-report-v280",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "an otherwise useful row whose answer is a bare noun fragment",
