#!/usr/bin/env python3
"""Replace transition timing recall with the ongoing-consent rationale."""
...
DATA = ROOT / "data"
V168_DIR = DATA / "manual_reviews/context_merit_audit_v168"
sys.path[:0] = [str(ROOT), str(V168_DIR)]
import build_context_merit_audit_v168 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v169.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v169.jsonl"
REPORT = OUT_DIR / "report_context_merit_v169.json"
REVIEWER = "codex-context-merit-audit-v169"
...
SPECS = (
    {
        ...
        "active_index": 360,
        "marker": "Especially when transitioning from one activity to another, consent can be renegotiated or withheld at any time. Check in every step of the way.",
        "decision": "edit",
        "question": "Why does Consent C.A.R.E.S. call for step-by-step check-ins when moving to another activity?",
        "answer": "Because consent can be renegotiated or withheld at any time, including during transitions.",
        "reason_code": "replace_transition_timing_with_ongoing_consent_rationale",
        "reason": "The replacement explains why transitions require check-ins instead of asking only when they occur.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v168",
    "direct_rows_without_prior_curation": 75,
    "anatomiestudio_transition_consent_rows_selected": 1,
    "rows": 507,
    "sha256": "0bd86f0a8e028e6b057a13d19052214bcf00f37c0b60a9f0e164ac3a0ff36f7b",
}
EXPECTED_OUTPUT_SHA256 = "89c14e53167eea88cebcb59203e11056abd476e8c7632cb6c6a056b064f546a1"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v169-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v168-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v168.jsonl"
        baseline_report = directory / "v168.report.json"
        ...
        if len(rows) != 507 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v168 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v169 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Anatomie Studio consent source lineage drift")
        audit = {
            ...
            "review_pass": "anatomiestudio_transition_consent_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v169",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 507 or observed["eval_fact_count"] != 612:
        raise ValueError("v169 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v169 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v168": 469,
            ...
            "projection_strategy": "delegate to the hermetic v168 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v169",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the transition-timing Q&A whose fully read source supports an ongoing-consent rationale",
