#!/usr/bin/env python3
"""Remove two low-merit Rope365 project-update lookups."""
...
DATA = ROOT / "data"
V121_DIR = DATA / "manual_reviews/context_merit_audit_v121"
sys.path[:0] = [str(ROOT), str(V121_DIR)]
import build_context_merit_audit_v121 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v122.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v122.jsonl"
REPORT = OUT_DIR / "report_context_merit_v122.json"
REVIEWER = "codex-context-merit-audit-v122"
...
SOURCE = DATA / "raw/rope365_b328837c9a9996ba.json"

SPECS = (
    {
        "fact_id": "fact-f1c684d897bf23c912fe",
        "active_index": 446,
        "marker": "I didn’t want to get into the whole hardpoint safety so I though using a closed door as an anchor was an easy and accessible way to make an introduction to suspension. That was until I tried the idea with NathanielFlumen and realized how hard it is for the person tying and the person tied as it is some kind of difficult Hashira technique because you can’t get around it.",
        "decision": "drop",
        "reason_code": "low_merit_abandoned_project_idea",
        "reason": "This asks which idea the author removed from a past curriculum plan, not a validated hardpoint-safety rule or transferable instruction.",
    },
    {
        "fact_id": "fact-12c180d70535a5b1bcd8",
        "active_index": 462,
        "marker": "Seeing Shay’s self-suspension book and Gestalta’s “Shibari Suspensions” book has proven me that this can be done in a positive ways.",
        "decision": "drop",
        "reason_code": "low_merit_anecdotal_book_names",
        "reason": "This asks for two books cited in a personal project update without stable links, selection criteria, or substantive suspension guidance.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v121",
    "direct_rows_without_prior_curation": 92,
    "rope365_project_update_rows_selected": 2,
    "rows": 523,
    "sha256": "32495f9a6b9bdf71c50e1a7f9a5cc9cf54d26e768ec96b1eff1a224029811615",
}
EXPECTED_OUTPUT_SHA256 = "b189579860bdf2ec3be95edbd5d819e029d0af5b8dbb4e1f718e6178339684de"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v122-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v121-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v121.jsonl"
        baseline_report = directory / "v121.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 523 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v121 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v122 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: project-update source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_project_update_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v122",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 521 or observed["eval_fact_count"] != 612:
        raise ValueError("v122 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v122 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v121": 485,
            "active_after_this_tranche": 483,
            ...
            "projection_strategy": "delegate to the v121 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v122",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the two not-yet-context-audited project-update Q&A from the fully read Rope365 Winter update",
                "score": "manual source support, anecdotal metadata risk, transferable value, and training merit",
