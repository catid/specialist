#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v45."""
...
import build_context_merit_audit_v45 as builder
from qa_quality import normalize_text


class ContextMeritAuditV45Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline_temp = tempfile.TemporaryDirectory(
            prefix=".test-baseline-v45-", dir=HERE)
        output = Path(cls.baseline_temp.name)
        cls.baseline_path = output / "projection-v44.jsonl"
        cls.baseline_report_path = output / "projection-v44.report.json"
    ...
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v45-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v45.jsonl"),
            ("CURATION", "pending_curation_context_merit_v45.jsonl"),
            ("REPORT", "report_context_merit_v45.json"),
        ):
    ...
    def test_projected_baseline_counts_are_pinned(self) -> None:
        ...
        reviewed = builder.prior_reviewed_fact_ids()
        self.assertEqual(len(direct), 266)
        self.assertEqual(sum(row["fact_id"] in reviewed for row in direct), 150)
        self.assertEqual(sum(row["fact_id"] not in reviewed for row in direct),
                         116)
        self.assertEqual(json.loads(builder.REPORT.read_text())["selection"]
                         ["projected_baseline"],
    ...
    def test_sources_and_evidence_are_hash_pinned(self) -> None:
        ...
        for spec in builder.SPECS:
            ...
            expected = spec.get("support_type", "normalized_extractive")
            if expected == "manual_paraphrase":
    ...
    def test_decisions_and_curation_are_exact(self) -> None:
        ...
        self.assertEqual(
            ...
            {"keep": 4, "drop": 3, "edit": 3})
        self.assertEqual(len(self.curation), 6)
        self.assertEqual(
            {action: sum(row["action"] == action for row in self.curation)
             for action in ("drop", "edit")}, {"drop": 3, "edit": 3})

    def test_all_three_edits_are_documented_manual_paraphrases(self) -> None:
        ...
        self.assertIn("protecting the trust", by_id[
            "fact-450cc6ca8581da4c2681"]["answer"])
        self.assertIn("not proof", by_id[
            "fact-e587f02d5dbf2375dc9e"]["answer"])
        self.assertIn("recognized a market", by_id[
            "fact-28154fce3bea393c93cd"]["answer"])
    ...
    def test_projection_lineage_is_complete(self) -> None:
        for row in self.audit:
            ...
            self.assertEqual(lineage["baseline_rows"], 555)
            ...
            self.assertEqual(row["review_pass"],
                             "first_context_merit_review_of_v44_projection_row")
    ...
    def test_report_counts_projection_and_sealed_policy(self) -> None:
        ...
        self.assertEqual(report["schema"], "context-merit-audit-report-v45")
        self.assertEqual(report["selection"]["eligible_unreviewed_rows"], 116)
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 3, "edit": 3, "keep": 4})
        ...
        self.assertEqual(projection["output_rows"], 552)
        self.assertEqual(
            projection["output_sha256"],
            "4e7b4cd68cefc09eedd78c81f54ae2c5d3c64930ab4adbb6b09dab52525bf2c2")
    ...
    def test_future_versions_cannot_enter_v45_by_directory_discovery(self) -> None:
        self.assertEqual(len(builder.CONTEXT_CURATIONS), 44)
        self.assertEqual(len(builder.CONTEXT_AUDITS), 44)
        self.assertIn("context_merit_audit_v44",
                      str(builder.CONTEXT_CURATIONS[-1]))
        self.assertFalse(any("context_merit_audit_v45" in str(path)
                             for path in (*builder.CONTEXT_CURATIONS,
                                          *builder.CONTEXT_AUDITS)))


class IsolatedProjectionV45Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory(
            prefix=".test-projection-v45-", dir=HERE)
    ...
    def test_projection_is_byte_identical_and_hash_pinned(self) -> None:
        ...
        self.assertEqual(len(self.rows), 552)
    ...
    def test_projection_applies_exact_decisions_and_upstream_repairs(self) -> None:
        ...
        consent = by_original["fact-bac953eeaadaa1b7763c"]
        self.assertIn("rope or kink activity", consent["question"])
        viewpoints = by_original["fact-a90111f186e1b44206d9"]
        self.assertIn("People who view rope play", viewpoints["answer"])
    ...
    def test_projection_has_unique_ids_and_questions(self) -> None:
        self.assertEqual(len({row["fact_id"] for row in self.rows}), 552)
        self.assertEqual(len({row["question"] for row in self.rows}), 552)
