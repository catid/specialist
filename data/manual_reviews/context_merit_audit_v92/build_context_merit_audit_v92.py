#!/usr/bin/env python3
"""Expand a Rope365 washability lookup into a material tradeoff."""
...
DATA = ROOT / "data"
V91_DIR = DATA / "manual_reviews/context_merit_audit_v91"
sys.path[:0] = [str(ROOT), str(V91_DIR)]
import build_context_merit_audit_v91 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v92.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v92.jsonl"
REPORT = OUT_DIR / "report_context_merit_v92.json"
REVIEWER = "codex-context-merit-audit-v92"
...
SPECS = (
    {
        "fact_id": "fact-6ad3ef0f91eb2f4932f0",
        "active_index": 476,
        "marker": "Synthetic and cotton ropes are easy to wash, so they can be washed after each use. Hemp and jute gets weaker when wet, and will take time drying under tension.",
        "decision": "edit",
        ...
        "reason_code": "expand_washability_lookup_into_material_tradeoff",
        "reason": "The revision replaces a bare material list with the source's useful contrast between washable rope and wet-sensitive natural fibers.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v91",
    "direct_rows_without_prior_curation": 113,
    "rope365_maintenance_rows_selected": 1,
    "rows": 535,
    "sha256": "051c4462cd386195af6f8a8587c8b312b7218a938f7ffb2a22e8ce5e7860fe98",
}
EXPECTED_OUTPUT_SHA256 = "a93ceac0d9cc07a092b0a3156101d0cabb13d1ab676bb19d946c8a304646bd6c"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v92-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v91-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v91.jsonl"
        baseline_report = directory / "v91.report.json"
        ...
        if len(rows) != 535 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v91 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v92 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["document_sha256"] != document["document_sha256"]:
            raise ValueError(f"{spec['fact_id']}: maintenance-source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_maintenance_merit_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v92",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 535 or observed["eval_fact_count"] != 612:
        raise ValueError("v92 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v92 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v91": 497,
        ...
        "schema": "context-merit-audit-report-v92",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the remaining direct Q&A from the fully read Rope365 maintenance page",
                "score": "manual source support, material tradeoff value, answer grammar, hygiene, and practical merit review",
