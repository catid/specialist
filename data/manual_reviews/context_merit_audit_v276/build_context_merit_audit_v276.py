#!/usr/bin/env python3
"""Replace a word lookup with a neutral-role-label tradeoff explanation."""
...
DATA = ROOT / "data"
V275_DIR = DATA / "manual_reviews/context_merit_audit_v275"
sys.path[:0] = [str(ROOT), str(V275_DIR)]
import build_context_merit_audit_v275 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v276.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v276.jsonl"
REPORT = OUT_DIR / "report_context_merit_v276.json"
REVIEWER = "codex-context-merit-audit-v276"
...
SOURCE = DATA / "raw/esinem_dce6c59fa90ecae0.json"

SPECS = (
    {
        "fact_id": "fact-e7ba5d5750bf38e692da",
        "active_index": 114,
        "markers": (
            "we have ‘rigger’ as a word for the active party but this can get confusing as it also applies to theatrical riggers who hang up stage equipment and the like.",
            "we could use simple BDSM designations like (rope) top/bottom but this implies, to me, an SM relationship and master slave strict D/s respectively. The trouble is they have preconceived roles attached.",
            "Terms like ‘rope bunny’, and to a greater extent ‘rope slut’, carry very negative connotations for some and are deemed offensive.",
            "This really only leaves ‘partner’ which ticks the PC boxes but doesn’t tell you who is being tied or doing the tying.",
        ),
        "decision": "edit",
        "question": "Why does ESINEM argue that common English labels for the two parties in bondage are not fully neutral?",
        "answer": "“Rigger” is ambiguous, “top” and “bottom” can imply fixed SM or D/s roles, “bunny” and “slut” may be gendered or offensive, and “partner” does not identify who is tying or tied.",
        "reason_code": "replace_word_lookup_with_role_label_tradeoffs",
        "reason": "The replacement trades a single-word lookup for the article's useful comparison of ambiguity, role assumptions, gendered connotations, and informational limits across common labels.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v275",
    "direct_rows_without_prior_curation": 1,
    "single_word_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "18fca73b8ceceac6b426c6eda578a23b8d4dbf85e8f1c52c8d7c66b9634674e4",
}
EXPECTED_OUTPUT_SHA256 = "17d68ea1f7e52511b4830a9bb2765a21fe1db12dd7bfab0369a1ab91071c5776"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v276-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v275-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v275.jsonl"
        baseline_report = directory / "v275.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v275 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v276 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "role_label_tradeoffs_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v276",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v276 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v276 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v275": 454,
        ...
        "schema": "context-merit-audit-report-v276",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a single-word lookup whose source passage supports a neutral-role-label tradeoff explanation",
                "score": "inclusive-language value, nuance, source support, question naturalness, and reduced vocabulary trivia",
