#!/usr/bin/env python3
"""Replace a bare Tetruss material lookup with practical frame specifications."""
...
DATA = ROOT / "data"
V190_DIR = DATA / "manual_reviews/context_merit_audit_v190"
sys.path[:0] = [str(ROOT), str(V190_DIR)]
import build_context_merit_audit_v190 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v191.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v191.jsonl"
REPORT = OUT_DIR / "report_context_merit_v191.json"
REVIEWER = "codex-context-merit-audit-v191"
...
SOURCE = DATA / "raw/tetruss_standard_rig_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 24,
        "markers": (
            "The standard frame is made from lightweight aluminum alloy, supports 300 pounds",
        ),
        "decision": "edit",
        ...
        "reason_code": "replace_bare_rig_material_with_practical_specs",
        "reason": "The replacement preserves the frame material while adding the vendor-stated weight, load, assembly, ceiling, and footprint specifications needed for a practical equipment decision.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v190",
    "direct_rows_without_prior_curation": 53,
    "tetruss_standard_rig_specs_rows_selected": 1,
    "rows": 505,
    "sha256": "1770a9db60003854fc1c110d84a229775795f20d8d52dd421a91813be064db11",
}
EXPECTED_OUTPUT_SHA256 = "8bcef308db2b40e979c167ef3254b7251c8afc1f5f5aad37c04da6fba82c00da"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v191-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v190-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v190.jsonl"
        baseline_report = directory / "v190.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v190 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v191 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Tetruss standard-rig source lineage drift")
        audit = {
            ...
            "review_pass": "tetruss_standard_rig_specs_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v191",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v191 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v191 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v190": 467,
            ...
            "projection_strategy": "delegate to the hermetic v190 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v191",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the bare frame-material lookup whose fully inspected product page gives practical load and space specifications",
                "score": "manual source support, equipment-selection usefulness, specification completeness, and training merit",
