#!/usr/bin/env python3
"""Audit the direct Rope365 single-column-tie guidance."""
...
DATA = ROOT / "data"
V104_DIR = DATA / "manual_reviews/context_merit_audit_v104"
sys.path[:0] = [str(ROOT), str(V104_DIR)]
import build_context_merit_audit_v104 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v105.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v105.jsonl"
REPORT = OUT_DIR / "report_context_merit_v105.json"
REVIEWER = "codex-context-merit-audit-v105"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__4452768569737af53851.json"

SPECS = (
    {
        "fact_id": "fact-9e1d84a06b0b7efe1dc3",
        "active_index": 20,
        "marker": "Most ties start with a single column tie, it is the first anchor to the body. A single column tie is a cuff that will not tighten when we pull on the tail. “Column” refers to anything you can tie around: a leg, a torso, a bedpost etc.",
        "decision": "keep",
        "reason_code": "clear_single_column_definition",
        "reason": "The concise answer correctly preserves the source's defining non-tightening behavior and is useful without unnecessary technique detail.",
    },
    {
        ...
        "marker": "Most ties start with a single column tie, it is the first anchor to the body. A single column tie is a cuff that will not tighten when we pull on the tail. “Column” refers to anything you can tie around: a leg, a torso, a bedpost etc.",
        "decision": "edit",
        "question": "In Rope365's single-column terminology, what does a 'column' mean?",
        "answer": "It means anything the rope encircles, such as a leg, torso, or bedpost.",
        "reason_code": "complete_column_definition",
        "reason": "The revision replaces the vague answer with a self-contained definition and representative body and object examples supplied by the source.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v104",
    "direct_rows_without_prior_curation": 100,
    "rope365_single_column_tie_rows_selected": 2,
    "rows": 532,
    "sha256": "693ee262007fba339cd1145b0796b74a0ef4d88d8bc7a93916a1468afecb8138",
}
EXPECTED_OUTPUT_SHA256 = "eb0fdac4e579ea5095ea94628f7ffb7fa8b56e0af86ad08aed356650165db123"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v105-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v104-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v104.jsonl"
        baseline_report = directory / "v104.report.json"
        ...
        if len(rows) != 532 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v104 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v105 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: single-column-tie source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_single_column_tie_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v105",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 532 or observed["eval_fact_count"] != 612:
        raise ValueError("v105 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v105 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v104": 494,
        ...
        "schema": "context-merit-audit-report-v105",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "all direct Q&A from the fully read Rope365 single-column-tie page",
                "score": "manual source support, definition completeness, answer context, and practical merit review",
