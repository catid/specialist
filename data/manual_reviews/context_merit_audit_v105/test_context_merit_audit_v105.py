#!/usr/bin/env python3
"""Focused regression tests for Rope365 single-column-tie audit v105."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v105 as b


class V105(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v105-", dir=HERE)
    ...
    def test_04_direct(self):
        self.assertEqual(sum(not row.get("curation") for row in self.baseline), 100)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("rope365_single_column_tie_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v105", {"drop": 0, "edit": 1, "keep": 1}, b.EXPECTED_OUTPUT_SHA256),
        )
