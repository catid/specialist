#!/usr/bin/env python3
"""Replace a consent fragment with an explicit clarification check."""
...
DATA = ROOT / "data"
V138_DIR = DATA / "manual_reviews/context_merit_audit_v138"
sys.path[:0] = [str(ROOT), str(V138_DIR)]
import build_context_merit_audit_v138 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v139.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v139.jsonl"
REPORT = OUT_DIR / "report_context_merit_v139.json"
REVIEWER = "codex-context-merit-audit-v139"
...
SOURCE = DATA / "raw/esinem_e154cd51975ed560.json"

SPECS = (
    {
        "fact_id": "fact-010889a66f8f2d6c11a1",
        "active_index": 196,
        "marker": "The key word is clarity. Both parties need to be very clear about what they expect from a session, whether it involves rope or not, before it starts.",
        "secondary_marker": "Riggers should be asking themselves whether they have the slightest doubt about consent and, if they would be uncomfortable about their actions being examined by their peers, simply ask.",
        "decision": "edit",
        "question": "What consent check does Esinem recommend before a scene when expectations, limits, or an action are unclear?",
        "answer": "Ask explicitly, including what kind of scene each person wants and what hard limits they have, rather than relying on uncertain nonverbal cues.",
        "reason_code": "replace_consent_fragment_with_explicit_check",
        "reason": "The replacement turns the fragment 'simply ask' into the source's concrete clarification check while retaining its emphasis on explicit expectations and hard limits.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v138",
    "direct_rows_without_prior_curation": 92,
    "esinem_explicit_consent_check_rows_selected": 1,
    "rows": 511,
    "sha256": "2392100463ed43aa345adb6dab1f3abd43c5611a17ab57929224b52707f619c2",
}
EXPECTED_OUTPUT_SHA256 = "39e4462b57dc497ee8d3b681c7ad615750878fcd57ae4f2d3d9c0afecbd0dce1"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v139-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v138-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v138.jsonl"
        baseline_report = directory / "v138.report.json"
        ...
        if len(rows) != 511 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v138 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v139 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        support = "\n".join((evidence(document, spec["marker"]), evidence(document, spec["secondary_marker"])))
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Esinem consent source lineage drift")
        audit = {
            ...
            "review_pass": "esinem_explicit_consent_check_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v139",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 511 or observed["eval_fact_count"] != 612:
        raise ValueError("v139 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v139 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v138": 473,
            ...
            "projection_strategy": "delegate to the hermetic v138 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v139",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the fragmentary consent Q&A whose fully read source provides an explicit clarification check",
                "score": "manual source support, consent specificity, transferable value, and training merit",
