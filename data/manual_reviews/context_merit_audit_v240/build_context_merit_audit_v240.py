#!/usr/bin/env python3
"""Remove a duplicate answer created by two earlier merit edits."""
...
DATA = ROOT / "data"
V239_DIR = DATA / "manual_reviews/context_merit_audit_v239"
sys.path[:0] = [str(ROOT), str(V239_DIR)]
import build_context_merit_audit_v239 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v240.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v240.jsonl"
REPORT = OUT_DIR / "report_context_merit_v240.json"
REVIEWER = "codex-context-merit-audit-v240"
...
SPECS = (
    {
        "fact_id": "fact-ca5b0dfdcf25c63308a4",
        "active_index": 8,
        "markers": (
            "We had little or no access to moving images hence the focus on techniques and patterns instead of on interaction.",
        ),
        "decision": "drop",
        "reason_code": "drop_duplicate_early_western_learning_answer",
        "reason": "This row duplicates the same answer and source passage already represented by another active question about how limited moving-image access shaped early Western rope learning.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v239",
    "direct_rows_without_prior_curation": 1,
    "duplicate_learning_history_rows_selected": 1,
    "rows": 504,
    "sha256": "089ca9ab8bd90f5660c049dc58fc363710eb5897b15259f33256f8260338a56d",
}
EXPECTED_OUTPUT_SHA256 = "492a06ea3ce8d5ed469dc4d17103420a0981457f71c6d3df9dd8b8517b6db85c"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v240-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v239-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v239.jsonl"
        baseline_report = directory / "v239.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 504 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v239 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v240 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "duplicate_learning_history_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v240",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 503 or observed["eval_fact_count"] != 612:
        raise ValueError("v240 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v240 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v239": 466,
            "active_after_this_tranche": 465,
        ...
        "schema": "context-merit-audit-report-v240",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "an exact answer duplicate created by two earlier merit edits from the same source passage",
                "score": "answer identity, source-passage identity, semantic question overlap, and redundancy reduction",
