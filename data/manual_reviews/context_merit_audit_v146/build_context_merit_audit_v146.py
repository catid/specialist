#!/usr/bin/env python3
"""Replace a holding-friction term lookup with its securing effect."""
...
DATA = ROOT / "data"
V145_DIR = DATA / "manual_reviews/context_merit_audit_v145"
sys.path[:0] = [str(ROOT), str(V145_DIR)]
import build_context_merit_audit_v145 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v146.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v146.jsonl"
REPORT = OUT_DIR / "report_context_merit_v146.json"
REVIEWER = "codex-context-merit-audit-v146"
...
SPECS = (
    {
        ...
        "active_index": 266,
        "marker": "Holding friction – The capacity of rope to hold in place when wrapping on itself. This will influence the types of knots and friction you use. With slippery rope, you might need to add additional knots to hold things in place, or use a full lock instead of just wrapping things around with friction. It is easier and faster to create a stable structure with ropes that have a good grip.",
        "decision": "edit",
        "question": "How does Rope365 say holding friction affects the way rope must be secured?",
        "answer": "Grippy rope can form stable structures more readily with friction, while slippery rope may require additional knots or a full lock.",
        "reason_code": "replace_holding_friction_term_with_securing_effect",
        "reason": "The replacement teaches the practical effect of holding friction instead of asking only for the term.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v145",
    "direct_rows_without_prior_curation": 90,
    "rope365_holding_friction_effect_rows_selected": 1,
    "rows": 509,
    "sha256": "7fe4360ef622022ecd2ab1e2067e5ca415daec22deb62591e06c36e71278ef8c",
}
EXPECTED_OUTPUT_SHA256 = "32af8cc68c071acaefea1137a3f89e68474c6b872b472ec4aa484bf4b7f8e25a"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v146-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v145-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v145.jsonl"
        baseline_report = directory / "v145.report.json"
        ...
        if len(rows) != 509 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v145 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v146 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Rope365 material source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_holding_friction_effect_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v146",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 509 or observed["eval_fact_count"] != 612:
        raise ValueError("v146 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v146 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v145": 471,
            ...
            "projection_strategy": "delegate to the hermetic v145 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v146",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the holding-friction term Q&A whose fully read source supports a practical securing lesson",
                "score": "manual source support, practical specificity, transferable value, and training merit",
