#!/usr/bin/env python3
"""Replace a wet-jute texture lookup with actionable drying guidance."""
...
DATA = ROOT / "data"
V150_DIR = DATA / "manual_reviews/context_merit_audit_v150"
sys.path[:0] = [str(ROOT), str(V150_DIR)]
import build_context_merit_audit_v150 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v151.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v151.jsonl"
REPORT = OUT_DIR / "report_context_merit_v151.json"
REVIEWER = "codex-context-merit-audit-v151"
...
SPECS = (
    {
        ...
        "answer": "Dry it under tension.",
        "reason_code": "replace_wet_jute_texture_with_drying_action",
        "reason": "The replacement turns a passive texture observation into the source’s directly supported, actionable recovery guidance.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v150",
    "direct_rows_without_prior_curation": 89,
    "anatomiestudio_wet_jute_recovery_rows_selected": 1,
    "rows": 509,
    "sha256": "ea1d69329be942bba802b7dda4d795d873387a2f2ea2b6786dfd09d4e9aa52a6",
}
EXPECTED_OUTPUT_SHA256 = "a3fb93eee09e557707c87ef350f1679abb55dcbc8135b82d2db0ca700b98706f"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v151-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v150-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v150.jsonl"
        baseline_report = directory / "v150.report.json"
        ...
        if len(rows) != 509 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v150 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v151 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "anatomiestudio_wet_jute_recovery_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v151",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 509 or observed["eval_fact_count"] != 612:
        raise ValueError("v151 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v151 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v150": 471,
            ...
            "projection_strategy": "delegate to the hermetic v150 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v151",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the wet-jute texture Q&A whose fully read source supports actionable drying guidance",
                "score": "manual source support, care usefulness, actionability, and training merit",
