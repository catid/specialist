#!/usr/bin/env python3
"""Replace a term lookup with a source-grounded comparison of expression."""
...
DATA = ROOT / "data"
V257_DIR = DATA / "manual_reviews/context_merit_audit_v257"
sys.path[:0] = [str(ROOT), str(V257_DIR)]
import build_context_merit_audit_v257 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v258.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v258.jsonl"
REPORT = OUT_DIR / "report_context_merit_v258.json"
REVIEWER = "codex-context-merit-audit-v258"
...
SOURCE = DATA / "raw/kinbakutoday_34dec041941868d9.json"

SPECS = (
    {
        "fact_id": "fact-2bcaf5b3488c71c04ccc",
        "active_index": 110,
        "markers": (
            "in the posturing called “Omoi-ire”, a Kabuki actor gives expression to the Omoi, without resort to language, only with the look on his face and the movement of his body.",
            "This too is the power of rope, the space for the person who is tied to reveal “omoi” to bring the thoughts, feelings, and emotions from the private hidden place, the space of Ura, to the surface and to allow them to be displayed.",
        ),
        "decision": "edit",
        "question": "How does “Itoh Seiu: Urami and the Drama of Rope” compare a rope scene to Kabuki omoi-ire?",
        "answer": "Both use face and body rather than words to bring hidden thoughts and feelings into visible expression; the article presents rope as a space where the tied person can reveal this private omoi.",
        "reason_code": "replace_term_lookup_with_expression_comparison",
        "reason": "The replacement trades a bare vocabulary lookup for the article's substantive comparison between wordless Kabuki expression and the tied person's embodied expression in a rope scene.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v257",
    "direct_rows_without_prior_curation": 1,
    "term_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "ff42f8b41d6e452cbfec71f252206cbcb520078970ee244f1f7751f72fd6c328",
}
EXPECTED_OUTPUT_SHA256 = "2473ee0b334dc034a7b41aa1432bc5f8abd6d671e3b8c1f025ce5b4fc9c3515f"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v258-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v257-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v257.jsonl"
        baseline_report = directory / "v257.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v257 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v258 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "embodied_expression_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v258",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v258 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v258 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v257": 454,
        ...
        "schema": "context-merit-audit-report-v258",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a vocabulary lookup whose source passage supports a substantive comparison of wordless expression",
                "score": "interpretive value, source support, question naturalness, and reduced terminology trivia",
