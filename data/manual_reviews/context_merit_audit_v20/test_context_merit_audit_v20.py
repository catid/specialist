#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v20."""
...
import build_context_merit_audit_v20 as builder
from qa_quality import normalize_text


class ContextMeritAuditV20Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v20-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v20.jsonl"),
            ("CURATION", "pending_curation_context_merit_v20.jsonl"),
            ("REPORT", "report_context_merit_v20.json"),
        ):
    ...
    def test_projected_selection_baseline_is_pinned(self) -> None:
        ...
        self.assertEqual(baseline["rows"], 657)
        self.assertEqual(
            baseline["sha256"],
            "1f7a87c35f0c336c9cfec52388d6e31338cb6074119d7f8af3ce8e9a38287a40")
        self.assertEqual(report["selection"]["eligible_unreviewed_rows"], 30)
        self.assertEqual(
            {row["fact_id"]: row["active_index"] for row in self.audit},
    ...
    def test_sources_and_evidence_are_hash_pinned(self) -> None:
        ...
        paraphrase_id = "fact-d11aba1283f69293bf55"
        for spec in builder.SPECS:
            ...
            if spec["fact_id"] == paraphrase_id:
                self.assertEqual(row["source_support"], "source_composite")
                self.assertNotEqual(normalize_text(answer),
                                    normalize_text(row["active_answer"]))
            else:
    ...
    def test_decisions_and_curation(self) -> None:
        ...
        self.assertEqual(
            ...
            {"keep": 10, "drop": 9, "edit": 6})
        self.assertEqual(len(self.curation), 15)
        self.assertEqual(
            {action: sum(row["action"] == action for row in self.curation)
             for action in ("drop", "edit")}, {"drop": 9, "edit": 6})
        support = {row["fact_id"]: row.get("support_type")
                   for row in self.curation if row["action"] == "edit"}
        self.assertEqual(list(support.values()).count("extractive"), 5)
        self.assertEqual(list(support.values()).count("manual_paraphrase"), 1)
    ...
    def test_report_projection_and_sealed_policy(self) -> None:
        report = json.loads(builder.REPORT.read_text())
        self.assertEqual(report["schema"], "context-merit-audit-report-v20")
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 9, "edit": 6, "keep": 10})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 648)
        self.assertEqual(
    ...
    def test_generator_is_deterministic_and_prior_is_frozen(self) -> None:
        ...
        prior_files = tuple(
            path for version in range(1, 20)
