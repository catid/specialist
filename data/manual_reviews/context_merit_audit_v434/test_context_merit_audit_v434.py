...
import build_context_merit_audit_v434 as b


class ContextMeritV434(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        b.main()
        cls.tmp = tempfile.TemporaryDirectory(prefix=".test-v434-", dir=HERE)
        d = Path(cls.tmp.name)
    ...
    def test_source_evidence_url_and_normalization_gate(self):
        self.assertEqual(
            (len(self.curations), Counter(row["action"] for row in self.curations)),
            (3, Counter({"edit": 3})),
        )
        self.assertEqual(sum(row["evidence_normalized"] for row in self.audits), 1)
        ...
        instructor = by_fact["fact-6d1fd5a6040e7583d9ab"]
        self.assertEqual(instructor["evidence"], b.INSTRUCTOR_EVIDENCE)
        self.assertNotIn("\n", instructor["evidence"])
