#!/usr/bin/env python3
"""Audit the remaining Esinem SHARE, CARE, ACT consent guidance."""
...
DATA = ROOT / "data"
V109_DIR = DATA / "manual_reviews/context_merit_audit_v109"
sys.path[:0] = [str(ROOT), str(V109_DIR)]
import build_context_merit_audit_v109 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v110.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v110.jsonl"
REPORT = OUT_DIR / "report_context_merit_v110.json"
REVIEWER = "codex-context-merit-audit-v110"
...
SPECS = (
    {
        ...
        "active_index": 201,
        "marker": "Riggers should be asking themselves whether they have the slightest doubt about consent and, if they would be uncomfortable about their actions being examined by their peers, simply ask.",
        "decision": "keep",
        "reason_code": "clear_consent_doubt_response",
        "reason": "The answer accurately preserves the source's direct, actionable response to any uncertainty about consent.",
    },
    {
        "fact_id": "fact-e5973f5d8c50295bda92",
        "active_index": 327,
        "marker": "Both parties need to be very clear about what they expect from a session, whether it involves rope or not, before it starts.",
        "decision": "keep",
        "reason_code": "clear_pre_session_expectations",
        "reason": "The answer correctly identifies mutual expectations as the item both parties should establish before the session begins.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v109",
    "direct_rows_without_prior_curation": 92,
    "esinem_share_care_act_rows_selected": 2,
    "rows": 529,
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v110-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v109-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v109.jsonl"
        baseline_report = directory / "v109.report.json"
        ...
        if len(rows) != 529 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v109 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v110 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: SHARE CARE ACT source lineage drift")
        audit = {
            ...
            "review_pass": "esinem_share_care_act_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v110",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 529 or observed["eval_fact_count"] != 612:
        raise ValueError("v110 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v110 output hash drift")
    report = {
        "active_baseline": {
        ...
        "audit": {
            "by_decision": {"drop": 0, "edit": 0, "keep": 2},
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v109": 491,
        ...
        "schema": "context-merit-audit-report-v110",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "all not-yet-context-audited Q&A from the fully read Esinem SHARE, CARE, ACT article",
                "score": "manual source support, consent actionability, answer completeness, and practical merit review",
