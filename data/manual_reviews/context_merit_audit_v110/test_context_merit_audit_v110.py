#!/usr/bin/env python3
"""Focused regression tests for Esinem SHARE, CARE, ACT audit v110."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v110 as b


class V110(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v110-", dir=HERE)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("esinem_share_care_act_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v110", {"drop": 0, "edit": 0, "keep": 2}, b.EXPECTED_OUTPUT_SHA256),
        )
