#!/usr/bin/env python3
"""Replace a strand-count lookup with construction tradeoffs."""
...
DATA = ROOT / "data"
V146_DIR = DATA / "manual_reviews/context_merit_audit_v146"
sys.path[:0] = [str(ROOT), str(V146_DIR)]
import build_context_merit_audit_v146 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v147.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v147.jsonl"
REPORT = OUT_DIR / "report_context_merit_v147.json"
REVIEWER = "codex-context-merit-audit-v147"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__ee5deb3191155e500237.json"

SPECS = (
    {
        "fact_id": "fact-87d1f8a2e85db346f385",
        "active_index": 18,
        "marker": "When shopping for rope, you might see a mention that rope is single-ply (1-ply) or double-ply (2-ply). This refers to the number of ply per yarn. Generally, a rope with more ply will be more robust, but also stiffer. Single-ply rope will generally be more flexible but will also require more maintenance and have a higher tendency to highstrand. But this is not always true. Other aspects of the construction can have a bigger impact on the rope characteristics. A tighter rope twist will make the rope more rigid with a more coarse texture while a lower twist will feel smoother and flexible.",
        "decision": "edit",
        "question": "How does Rope365 say ply count and twist tightness can affect a rope’s handling?",
        "answer": "More plies generally make rope more robust but stiffer, while a tighter twist tends to make it more rigid and coarse and a looser twist smoother and more flexible; other construction factors can alter these tendencies.",
        "reason_code": "replace_strand_count_with_construction_tradeoffs",
        "reason": "The replacement teaches useful construction tradeoffs and their uncertainty instead of an isolated three-strand count.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v146",
    "direct_rows_without_prior_curation": 90,
    "rope365_construction_tradeoff_rows_selected": 1,
    "rows": 509,
    "sha256": "32af8cc68c071acaefea1137a3f89e68474c6b872b472ec4aa484bf4b7f8e25a",
}
EXPECTED_OUTPUT_SHA256 = "2da58d79b221768f15e9ec629f21a7f70c9fae822242682bfe94899a8e6460b2"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v147-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v146-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v146.jsonl"
        baseline_report = directory / "v146.report.json"
        ...
        if len(rows) != 509 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v146 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v147 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Rope365 construction source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_construction_tradeoff_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v147",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 509 or observed["eval_fact_count"] != 612:
        raise ValueError("v147 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v147 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v146": 471,
            ...
            "projection_strategy": "delegate to the hermetic v146 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v147",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the strand-count Q&A whose fully read source supports practical construction tradeoffs",
                "score": "manual source support, material handling, uncertainty, and training merit",
