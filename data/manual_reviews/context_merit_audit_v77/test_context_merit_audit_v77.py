...
class V77(unittest.TestCase):
 ...
 def test_04_direct(self):self.assertEqual(sum(not r.get("curation") for r in self.baseline),133)
 def test_05_decisions(self):self.assertEqual({x:sum(r["decision"]==x for r in self.audit) for x in ("keep","edit","drop")},{"keep":1,"edit":3,"drop":0})
 def test_06_curation(self):self.assertEqual((len(self.curation),{r["action"] for r in self.curation}),(3,{"edit"}))
 def test_07_support(self):self.assertEqual({r["support_type"] for r in self.curation},{"manual_paraphrase"});self.assertTrue(all(r["paraphrase_rationale"] for r in self.curation))
 ...
 def test_09_keep(self):
  r=next(r for r in self.audit if r["decision"]=="keep");p=next(x for x in self.rows if x["fact_id"]==r["fact_id"]);self.assertEqual((p["question"],p["answer"]),(r["active_question"],r["active_answer"]))
 def test_10_lineage(self):
  for r in self.audit:self.assertEqual((r["review_pass"],r["projection_lineage"]["baseline_sha256"]),("anatomiestudio_jute_hygiene_reaudit",b.PROJECTED_SELECTION_BASELINE["sha256"]))
 ...
 def test_14_pins(self):
  pins=self.report["frozen_prior_decision_artifacts"];self.assertEqual(len(pins),228)
  for r in pins:self.assertEqual(b.file_sha256(b.ROOT/r["path"]),r["sha256"])
 def test_15_actions(self):
  represented={r.get("curation",{}).get("original_fact_id"):r for r in self.rows if r.get("curation")}
  for s in b.SPECS:
   if s["decision"]=="edit":self.assertEqual((represented[s["fact_id"]]["question"],represented[s["fact_id"]]["answer"]),(s["question"],s["answer"]))
 ...
 def test_19_report(self):self.assertEqual((self.report["schema"],self.report["audit"]["by_decision"],self.report["isolated_build_projection"]["output_sha256"]),("context-merit-audit-report-v77",{"edit":3,"keep":1},b.EXPECTED_OUTPUT_SHA256))
