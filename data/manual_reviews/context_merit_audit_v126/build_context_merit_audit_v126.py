#!/usr/bin/env python3
"""Remove a low-merit ancient-rope date lookup."""
...
DATA = ROOT / "data"
V125_DIR = DATA / "manual_reviews/context_merit_audit_v125"
sys.path[:0] = [str(ROOT), str(V125_DIR)]
import build_context_merit_audit_v125 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v126.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v126.jsonl"
REPORT = OUT_DIR / "report_context_merit_v126.json"
REVIEWER = "codex-context-merit-audit-v126"
...
SPECS = (
    {
        "fact_id": "fact-a8ab565300535b5f57f3",
        "active_index": 108,
        "marker": "The first evidence of rope is a 3-ply cord from 40, 000 years ago.",
        "decision": "drop",
        "reason_code": "low_merit_ancient_rope_date",
        "reason": "This tests an isolated archaeological date rather than rope construction properties, selection, handling, maintenance, or safety.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v125",
    "direct_rows_without_prior_curation": 92,
    "rope365_ancient_rope_history_rows_selected": 1,
    "rows": 518,
    "sha256": "695d18de0d96c57c3a939893f6864954e8c8e817c30d0e56e5886ea79354e1a4",
}
EXPECTED_OUTPUT_SHA256 = "c154f844b363fa547cf4a68aa97657e1793a1e29f664af1aeac61c883c9436ea"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v126-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v125-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v125.jsonl"
        baseline_report = directory / "v125.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 518 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v125 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v126 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: rope-construction source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_ancient_rope_history_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v126",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 517 or observed["eval_fact_count"] != 612:
        raise ValueError("v126 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v126 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v125": 480,
            "active_after_this_tranche": 479,
            ...
            "projection_strategy": "delegate to the v125 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v126",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the not-yet-context-audited archaeological-date Q&A from the fully read Rope365 rope-construction page",
