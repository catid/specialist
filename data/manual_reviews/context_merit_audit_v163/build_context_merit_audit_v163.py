#!/usr/bin/env python3
"""Replace a safeword agreement list with the reason its meaning must be explicit."""
...
DATA = ROOT / "data"
V162_DIR = DATA / "manual_reviews/context_merit_audit_v162"
sys.path[:0] = [str(ROOT), str(V162_DIR)]
import build_context_merit_audit_v162 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v163.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v163.jsonl"
REPORT = OUT_DIR / "report_context_merit_v163.json"
REVIEWER = "codex-context-merit-audit-v163"
...
SPECS = (
    {
        ...
        "active_index": 316,
        "marker": "If you decide you’d like to use a safe word in rope, be specific about what it means given the nature of shibari.",
        "decision": "edit",
        "question": "Why should rope partners agree in advance what a safeword means in practice?",
        "answer": "Because stopping rope is rarely immediate, so partners need a shared understanding of what response the word calls for.",
        "reason_code": "replace_safeword_agreement_list_with_response_rationale",
        "reason": "The replacement explains why the safeword’s meaning must be explicit in rope instead of repeating a generic agreement checklist.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v162",
    "direct_rows_without_prior_curation": 81,
    "anatomiestudio_safeword_response_rows_selected": 1,
    "rows": 507,
    "sha256": "df6df701bdb547e36296997284734b57d76a07bf70df5bdc789592c5a2670eaf",
}
EXPECTED_OUTPUT_SHA256 = "506015bea46dccbf0557b02dee662c29a20648dcc05dad4f3edc040714917cc6"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v163-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v162-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v162.jsonl"
        baseline_report = directory / "v162.report.json"
        ...
        if len(rows) != 507 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v162 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v163 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "anatomiestudio_safeword_response_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v163",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 507 or observed["eval_fact_count"] != 612:
        raise ValueError("v163 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v163 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v162": 469,
            ...
            "projection_strategy": "delegate to the hermetic v162 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v163",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the safeword agreement list whose fully read source supports a rope-specific response rationale",
                "score": "manual source support, communication usefulness, causal clarity, and training merit",
