#!/usr/bin/env python3
"""Drop the observer no-touch row subsumed by the consolidated etiquette."""
...
DATA = ROOT / "data"
V160_DIR = DATA / "manual_reviews/context_merit_audit_v160"
sys.path[:0] = [str(ROOT), str(V160_DIR)]
import build_context_merit_audit_v160 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v161.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v161.jsonl"
REPORT = OUT_DIR / "report_context_merit_v161.json"
REVIEWER = "codex-context-merit-audit-v161"
...
SPECS = (
    {
        "fact_id": "fact-eaa4efb1ff903455f6a7",
        "active_index": 277,
        "marker": "do not interrupt a scene to ask questions, touch people or their equipment (rope included)",
        "decision": "drop",
        "reason_code": "drop_subsumed_observer_no_touch_row",
        "reason": "The preceding consolidated observer-etiquette row now includes this no-touch rule alongside distance and interruption guidance, so this narrower row is redundant.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v160",
    "direct_rows_without_prior_curation": 82,
    "anatomiestudio_subsumed_observer_rows_selected": 1,
    "rows": 508,
    "sha256": "6d16afc27321c8ac74b369f46043e247c77c72366df51a65ab5903bce1bc046d",
}
EXPECTED_OUTPUT_SHA256 = "5b1a30d3ce924d7db2c9f3400a30f325e882320f6ae1f937573a98181c234a59"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v161-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v160-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v160.jsonl"
        baseline_report = directory / "v160.report.json"
        ...
        if len(rows) != 508 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v160 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v161 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "anatomiestudio_observer_dedup_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v161",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 507 or observed["eval_fact_count"] != 612:
        raise ValueError("v161 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v161 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v160": 470,
            ...
            "projection_strategy": "delegate to the hermetic v160 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v161",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the no-touch Q&A subsumed by the preceding consolidated observer-etiquette answer",
                "score": "semantic duplication, source support, wording coverage, and training balance",
