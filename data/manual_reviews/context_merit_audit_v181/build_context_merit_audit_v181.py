#!/usr/bin/env python3
"""Complete Knot Head Nylon's lark's-head tightening warning."""
...
DATA = ROOT / "data"
V180_DIR = DATA / "manual_reviews/context_merit_audit_v180"
sys.path[:0] = [str(ROOT), str(V180_DIR)]
import build_context_merit_audit_v180 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v181.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v181.jsonl"
REPORT = OUT_DIR / "report_context_merit_v181.json"
REVIEWER = "codex-context-merit-audit-v181"
...
SOURCE = DATA / "raw/knothead_nylon_knots_to_try_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 68,
        "markers": (
            "It WILL tighten up unless you combine it with a half hitch.",
        ),
        "decision": "edit",
        "question": "How does Knot Head Nylon prevent a lark's head from tightening under reverse tension?",
        "answer": "Add a half hitch; without it, the lark's head will tighten under reverse tension.",
        "reason_code": "complete_larks_head_tightening_warning",
        "reason": "The replacement preserves the half-hitch action and adds the source's explicit consequence and reverse-tension context.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v180",
    "direct_rows_without_prior_curation": 63,
    "knothead_nylon_larks_head_rows_selected": 1,
    "rows": 506,
    "sha256": "2d0ef6e55f192dbcd2fddbcd94901173ef286dd05721787fc2f157abf99be746",
}
EXPECTED_OUTPUT_SHA256 = "adc5cd63469b6b89ac6aec240600ef3ee0e7c910d3484c6de764c88d3a9f6068"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v181-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v180-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v180.jsonl"
        baseline_report = directory / "v180.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v180 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v181 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != LEGACY_URL:
            raise ValueError(f"{spec['fact_id']}: Knot Head Nylon lark's-head source lineage drift")
        audit = {
            ...
            "review_pass": "knothead_nylon_larks_head_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v181",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v181 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v181 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v180": 468,
            ...
            "projection_strategy": "delegate to the hermetic v180 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v181",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the abbreviated half-hitch answer whose fully read current successor page states the reverse-tension consequence",
                "score": "manual source support, functional usefulness, answer completeness, and training merit",
