#!/usr/bin/env python3
"""Normalize a clipped waist-and-breathing answer into a complete sentence."""
...
DATA = ROOT / "data"
V280_DIR = DATA / "manual_reviews/context_merit_audit_v280"
sys.path[:0] = [str(ROOT), str(V280_DIR)]
import build_context_merit_audit_v280 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v281.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v281.jsonl"
REPORT = OUT_DIR / "report_context_merit_v281.json"
REVIEWER = "codex-context-merit-audit-v281"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__03448edb627f81c3e962.json"

SPECS = (
    {
        "fact_id": "fact-f4c4b6f00cefdfee6652",
        "active_index": 149,
        "markers": (
            "A rope around the waist can serve as a practical attachment point. It is also an important part of breathing, so adding a waistline can be a part of play and intensity.",
        ),
        "decision": "edit",
        ...
        "reason_code": "normalize_clipped_waist_breathing_answer",
        "reason": "The original answer was a clipped dependent clause; the revision preserves the source's breathing-and-intensity point in a grammatical, self-contained sentence.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v280",
    "direct_rows_without_prior_curation": 1,
    "clipped_answer_rows_selected": 1,
    "rows": 492,
    "sha256": "f1143f1513b61047309c6809d70255948480bc7e55175618a0ff5b0386a5d9b0",
}
EXPECTED_OUTPUT_SHA256 = "01146198314c39b644aecb03aaf256faed72bafb313a815e84ce9c619aa70e58"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v281-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v280-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v280.jsonl"
        baseline_report = directory / "v280.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v280 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v281 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "schema": "context-merit-audit-v281",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v281 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v281 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v280": 454,
        ...
        "schema": "context-merit-audit-report-v281",
