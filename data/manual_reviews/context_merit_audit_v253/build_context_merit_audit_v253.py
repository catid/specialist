#!/usr/bin/env python3
"""Replace a magazine-name lookup with a source-grounded safety lesson."""
...
DATA = ROOT / "data"
V252_DIR = DATA / "manual_reviews/context_merit_audit_v252"
sys.path[:0] = [str(ROOT), str(V252_DIR)]
import build_context_merit_audit_v252 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v253.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v253.jsonl"
REPORT = OUT_DIR / "report_context_merit_v253.json"
REVIEWER = "codex-context-merit-audit-v253"
...
SOURCE = DATA / "raw/kinbakutoday_de20a4adcc8ec0d5.json"

SPECS = (
    {
        "fact_id": "fact-0d5d066eb2920c5b664a",
        "active_index": 19,
        "markers": (
            "“It’s okay, I’ll put it out right away with a snap. This is just an accessory, too.”",
            "In fact, I had never applied moxibustion before either, but relying on hearsay and trial and error, I painstakingly attached a large moxa cone, hardened with glue, to Sugi-san’s butt so it wouldn’t fall off, and lit it with an incense stick.",
            "The acrid smell of burning moxa stung my nose. Even as I set the camera position and directed her poses, the moxa burned rapidly. “Ah, ah, it’s hot, take it off, take it off…”",
            "Before two minutes had even passed, the flames were already closing in on her skin. In a panic, I grabbed the burning moxa and tossed it into the ashtray, but it was no wonder Sugi-san was squirming. That part of her skin had already turned a reddish-pink.",
            "“I’ve learned my lesson. This kind of thing…”",
        ),
        "decision": "edit",
        "question": "What safety failure does the translated 1955 essay reveal after moxibustion was dismissed as a mere photographic “accessory”?",
        "answer": "An inexperienced applicator let the moxa burn close enough to turn Sugi’s skin reddish-pink before removing it, despite her repeated requests that it be taken off.",
        "reason_code": "replace_magazine_lookup_with_historical_safety_lesson",
        "reason": "The replacement trades a bare magazine-name lookup for a concrete, critically framed example of how minimizing a painful effect led to preventable harm.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v252",
    "direct_rows_without_prior_curation": 1,
    "magazine_name_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "9d191a420031460ba1cf3f717c139ce19bb493d7a0cc9a4c27a8c62dd15dbfb6",
}
EXPECTED_OUTPUT_SHA256 = "ace32971e50f4b9874b20b8ef91db0c22c7d57ebadb47d42ef386878572224ee"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v253-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v252-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v252.jsonl"
        baseline_report = directory / "v252.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v252 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v253 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "historical_safety_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v253",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v253 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v253 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v252": 454,
        ...
        "schema": "context-merit-audit-report-v253",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a magazine-name lookup whose source passage supports a concrete historical safety lesson",
                "score": "transferable safety value, critical framing, source support, question naturalness, and reduced publication trivia",
