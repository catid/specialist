#!/usr/bin/env python3
"""Focused regression tests for Rope365 two-rope box-tie audit v101."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v101 as b


class V101(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v101-", dir=HERE)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("rope365_two_rope_box_tie_safety_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )

    def test_11_projection(self):
        self.assertEqual((len(self.rows), b.file_sha256(self.dataset)), (532, b.EXPECTED_OUTPUT_SHA256))
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v101", {"drop": 0, "edit": 1, "keep": 1}, b.EXPECTED_OUTPUT_SHA256),
        )
