#!/usr/bin/env python3
"""Audit the remaining direct Rope365 two-rope box-tie rows."""
...
DATA = ROOT / "data"
V100_DIR = DATA / "manual_reviews/context_merit_audit_v100"
sys.path[:0] = [str(ROOT), str(V100_DIR)]
import build_context_merit_audit_v100 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v101.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v101.jsonl"
REPORT = OUT_DIR / "report_context_merit_v101.json"
REVIEWER = "codex-context-merit-audit-v101"
...
SPECS = (
    {
        ...
        "active_index": 428,
        "marker": "Extend the rope between the arm and the torso: many nerves are exposed at the armpit and below.",
        "decision": "keep",
        "reason_code": "retain_unsafe_extension_location_warning",
        "reason": "The existing Q&A directly preserves the source's warning against placing a bulky rope extension where many nerves are exposed.",
    },
    {
        "fact_id": "fact-3b647333a1ace1f99666",
        "active_index": 450,
        "marker": "The ulnar nerve is often exposed at the elbow",
        "decision": "edit",
        "question": "Why does Rope365 advise keeping a two-rope box tie's lower wrap away from the elbow?",
        "answer": "The ulnar nerve is often exposed at the elbow, so pressure there can create nerve risk.",
        "reason_code": "replace_funny_bone_lookup_with_ulnar_safety_rationale",
        "reason": "The revision replaces a funny-bone anatomy lookup with the source's practical reason for keeping lower-wrap pressure off the elbow.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v100",
    "direct_rows_without_prior_curation": 104,
    "rope365_two_rope_box_tie_rows_selected": 2,
    "rows": 532,
    "sha256": "de879de32679e1036ea8c811ba373340939c07c87f349eb9e9a984381714b860",
}
EXPECTED_OUTPUT_SHA256 = "077a0a7022e390ee6c352a3e7d82e5566dd97b954b49cebdcc6eace02aced7ac"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v101-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v100-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v100.jsonl"
        baseline_report = directory / "v100.report.json"
        ...
        if len(rows) != 532 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v100 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v101 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "rope365_two_rope_box_tie_safety_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v101",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 532 or observed["eval_fact_count"] != 612:
        raise ValueError("v101 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v101 output hash drift")
    report = {
        "active_baseline": {
        ...
        "audit": {
            "by_decision": {"drop": 0, "edit": 1, "keep": 1},
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v100": 494,
        ...
        "schema": "context-merit-audit-report-v101",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the two remaining direct Q&A from the fully read Rope365 two-rope box-tie page",
                "score": "manual source support, nerve-safety actionability, answer context, and practical merit review",
