#!/usr/bin/env python3
"""Focused regression tests for Rope365 rope-shopping audit v90."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v90 as b


class V90(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v90-", dir=HERE)
    ...
    def test_04_direct(self):
        self.assertEqual(sum(not row.get("curation") for row in self.baseline), 115)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("rope365_rope_shopping_merit_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v90", {"edit": 1, "keep": 0}, b.EXPECTED_OUTPUT_SHA256),
        )
