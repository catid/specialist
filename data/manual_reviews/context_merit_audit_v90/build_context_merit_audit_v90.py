#!/usr/bin/env python3
"""Replace a rope-material lookup with practical Rope365 starter advice."""
...
DATA = ROOT / "data"
V89_DIR = DATA / "manual_reviews/context_merit_audit_v89"
sys.path[:0] = [str(ROOT), str(V89_DIR)]
import build_context_merit_audit_v89 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v90.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v90.jsonl"
REPORT = OUT_DIR / "report_context_merit_v90.json"
REVIEWER = "codex-context-merit-audit-v90"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__077aecb334166abba18c.json"

SPECS = (
    {
        "fact_id": "fact-feadf2d476a11a4a1140",
        "active_index": 450,
        "marker": "Go to the hardware store, take cotton or nylon rope 6mm (1/4 inch), then cut into several pieces around 8m (between 7-10m).",
        "decision": "edit",
        "question": "What inexpensive starter rope does Rope365 suggest when a beginner does not yet know their style?",
        "answer": "6 mm (1/4-inch) cotton or nylon rope from a hardware store, cut into several 7–10 m lengths.",
        "reason_code": "replace_slippery_material_lookup_with_starter_guidance",
        "reason": "The revision replaces a bare material-name answer with the source's concrete, inexpensive starter recommendation for beginners who have not developed a style preference.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v89",
    "direct_rows_without_prior_curation": 115,
    "rope365_rope_shopping_rows_selected": 1,
    "rows": 535,
    "sha256": "2eb89198918a2ab0efd04e6a537729ebda8499c709cbc83afdee85cda28dfdcd",
}
EXPECTED_OUTPUT_SHA256 = "c311de5876e7cc672d5275707a71864f2c68ae037ddb52f1761f78385e9b9d53"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v90-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v89-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v89.jsonl"
        baseline_report = directory / "v89.report.json"
        ...
        if len(rows) != 535 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v89 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v90 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["document_sha256"] != document["document_sha256"]:
            raise ValueError(f"{spec['fact_id']}: rope-shopping source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_rope_shopping_merit_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v90",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 535 or observed["eval_fact_count"] != 612:
        raise ValueError("v90 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v90 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v89": 497,
        ...
        "schema": "context-merit-audit-report-v90",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the remaining direct Q&A from the fully read Rope365 rope-shopping page",
                "score": "manual source support, beginner utility, answer grammar, affordability, and practical merit review",
