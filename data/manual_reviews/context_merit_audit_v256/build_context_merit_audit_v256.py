#!/usr/bin/env python3
"""Replace a bar-name lookup with a gender-representation history lesson."""
...
DATA = ROOT / "data"
V255_DIR = DATA / "manual_reviews/context_merit_audit_v255"
sys.path[:0] = [str(ROOT), str(V255_DIR)]
import build_context_merit_audit_v255 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v256.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v256.jsonl"
REPORT = OUT_DIR / "report_context_merit_v256.json"
REVIEWER = "codex-context-merit-audit-v256"
...
SOURCE = DATA / "raw/kinbakutoday_36c008200d681448.json"

SPECS = (
    {
        "fact_id": "fact-83de76ba9143fdb48674",
        "active_index": 128,
        "markers": (
            "Since Akechi Denki introduced female Kinbakushi in video and published media, the image of Kinbakushi being exclusively male has disappeared in the SM scene. The trend of many dominas adopting rope bondage, both in play and in video shoots seems to have taken root over the past 20 years.",
        ),
        "decision": "edit",
        "question": "What change does Ryuzaki Asuka associate with Akechi Denki presenting women kinbakushi in video and print?",
        "answer": "She says the image of kinbakushi as exclusively male disappeared, and many dominatrices came to adopt rope in both play and video.",
        "reason_code": "replace_bar_lookup_with_gender_representation_history",
        "reason": "The replacement trades a bare bar-name lookup for the interview's substantive account of how women's visibility changed gender representation and practice in Japan's SM scene.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v255",
    "direct_rows_without_prior_curation": 1,
    "bar_name_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "f18a40bff9374111f46d55dbea3a4bfdcc628ccb1a42b5fbb440696cb850c848",
}
EXPECTED_OUTPUT_SHA256 = "38519110090b4464abb07f73245825338304cb154d896e10a384ea52732750f2"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v256-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v255-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v255.jsonl"
        baseline_report = directory / "v255.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v255 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v256 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "gender_representation_history_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v256",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v256 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v256 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v255": 454,
        ...
        "schema": "context-merit-audit-report-v256",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a bar-name lookup whose source passage supports a broader gender-representation history lesson",
                "score": "historical and representational value, source support, question naturalness, and reduced venue trivia",
