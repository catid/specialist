#!/usr/bin/env python3
"""Complete the boundary-uncertainty guidance for new rope partners."""
...
DATA = ROOT / "data"
V171_DIR = DATA / "manual_reviews/context_merit_audit_v171"
sys.path[:0] = [str(ROOT), str(V171_DIR)]
import build_context_merit_audit_v171 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v172.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v172.jsonl"
REPORT = OUT_DIR / "report_context_merit_v172.json"
REVIEWER = "codex-context-merit-audit-v172"
...
SPECS = (
    {
        ...
        "active_index": 188,
        "marker": "If you're not sure of your boundaries, be honest and go slow. You can always go further next time.",
        "decision": "edit",
        "question": "What does Anatomie Studio advise if someone is unsure of their own boundaries with a new rope partner?",
        "answer": "Be honest, go slowly, and remember that you can always explore further next time.",
        "reason_code": "complete_boundary_uncertainty_guidance",
        "reason": "The replacement restores the new-partner context and the source’s reason not to rush uncertain boundaries.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v171",
    "direct_rows_without_prior_curation": 72,
    "anatomiestudio_boundary_uncertainty_rows_selected": 1,
    "rows": 506,
    "sha256": "e0a43aede1adc93e40340fde7afee800740e1823298c5b85f5531112c27dc18e",
}
EXPECTED_OUTPUT_SHA256 = "27e9c5b7d4951637c80be7d9f935741951ad167da1754e34b39dc6c8945895b8"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v172-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v171-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v171.jsonl"
        baseline_report = directory / "v171.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v171 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v172 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "anatomiestudio_boundary_uncertainty_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v172",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v172 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v172 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v171": 468,
            ...
            "projection_strategy": "delegate to the hermetic v171 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v172",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the abbreviated boundary-uncertainty Q&A whose fully read source supports more complete guidance",
                "score": "manual source support, consent usefulness, completeness, and training merit",
