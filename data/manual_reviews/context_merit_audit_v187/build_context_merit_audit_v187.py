#!/usr/bin/env python3
"""Connect safeword failure modes to negotiation and active check-ins."""
...
DATA = ROOT / "data"
V186_DIR = DATA / "manual_reviews/context_merit_audit_v186"
sys.path[:0] = [str(ROOT), str(V186_DIR)]
import build_context_merit_audit_v186 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v187.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v187.jsonl"
REPORT = OUT_DIR / "report_context_merit_v187.json"
REVIEWER = "codex-context-merit-audit-v187"
...
SOURCE = DATA / "raw/crash_restraint_negotiation_consent_20260714.json"

SPECS = (
    {
        "fact_id": "fact-4c78d54846efaa4feef0",
        "active_index": 460,
        "markers": (
            "people very commonly find themselves unable to safeword or otherwise communicate.",
            "You cannot rely on a partner being able to speak before they are hurt",
            "discuss how much the top should check in.",
        ),
        "decision": "edit",
        ...
        "reason_code": "connect_safeword_failure_to_negotiation_and_checkins",
        "reason": "The replacement preserves the inability-to-communicate warning and adds its operational consequence: establish communication methods and active check-ins in advance.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v186",
    "direct_rows_without_prior_curation": 57,
    "crash_restraint_safeword_limit_rows_selected": 1,
    "rows": 506,
    "sha256": "84e841e6c18fdab7c8d69240a7388f199432ce6c59136ca051382b99aaf96d16",
}
EXPECTED_OUTPUT_SHA256 = "340cf0e2f84245f27f64fbc149bbfc981f095641c12121a88a5dc7cb85191455"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v187-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v186-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v186.jsonl"
        baseline_report = directory / "v186.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v186 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v187 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Crash Restraint negotiation-and-consent source lineage drift")
        audit = {
            ...
            "review_pass": "crash_restraint_safeword_limit_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v187",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v187 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v187 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v186": 468,
            ...
            "projection_strategy": "delegate to the hermetic v186 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v187",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the correct safeword-limit answer whose fully read source gives the negotiation and check-in consequence",
                "score": "manual source support, consent-safety usefulness, causal completeness, and training merit",
