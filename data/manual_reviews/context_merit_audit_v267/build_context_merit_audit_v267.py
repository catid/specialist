#!/usr/bin/env python3
"""Replace a linked-synonym lookup with the page's intimacy lesson."""
...
DATA = ROOT / "data"
V266_DIR = DATA / "manual_reviews/context_merit_audit_v266"
sys.path[:0] = [str(ROOT), str(V266_DIR)]
import build_context_merit_audit_v266 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v267.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v267.jsonl"
REPORT = OUT_DIR / "report_context_merit_v267.json"
REVIEWER = "codex-context-merit-audit-v267"
...
SOURCE = DATA / "raw/rope365_a143d6923a8771f7.json"

SPECS = (
    {
        "fact_id": "fact-6325121acc87ecda8df5",
        "active_index": 418,
        "markers": (
            "Intimacy is experienced differently from people to people and takes various forms. Rope allows us to explore seduction, arousal, and emotional bounds.",
            "The goal of this week is to try different techniques to tap into our seductive energy and build our own ritual to create the perfect mood.",
        ),
        "decision": "edit",
        "question": "What does Rope365’s Intimacy lesson encourage people to develop beyond rope technique?",
        "answer": "It encourages experimenting with seductive energy and personal rituals that build mood, while recognizing that intimacy takes different forms for different people.",
        "reason_code": "replace_linked_synonym_lookup_with_intimacy_lesson",
        "reason": "The replacement trades an off-page tie-synonym lookup for the source page's actual lesson about individualized intimacy, mood, and ritual.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v266",
    "direct_rows_without_prior_curation": 1,
    "linked_synonym_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "8c39a349e5dd7ef5c87da72ce5be3c9b5d795036ef6e7f895a891f1c488412fb",
}
EXPECTED_OUTPUT_SHA256 = "eba3ec922055421993cf42099df4f994a58cef2795311101ee0d9dbd2f691cbb"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v267-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v266-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v266.jsonl"
        baseline_report = directory / "v266.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v266 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v267 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "page_core_intimacy_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v267",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v267 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v267 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v266": 454,
        ...
        "schema": "context-merit-audit-report-v267",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a linked-resource synonym lookup whose own source page supports an intimacy lesson",
                "score": "page relevance, relational value, source support, question naturalness, and reduced terminology trivia",
