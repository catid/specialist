...
import build_safety_additions_v11 as b


class SafetyAdditionsV11(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        b.main()
    ...
    def test_baseline_and_artifact(self):
        with tempfile.TemporaryDirectory(prefix=".test-safety-v11-", dir=HERE) as temp:
            p = Path(temp)
            rows = b.build_baseline(p / "baseline.jsonl", p / "baseline.report.json")
            self.assertEqual((len(rows), b.file_sha256(p / "baseline.jsonl")), (522, b.BASELINE_SHA256))
    ...
    def test_report(self):
        ...
        self.assertEqual(self.report["new_independent_inputs"]["expected_strata"], {"safety_consent": 3})
