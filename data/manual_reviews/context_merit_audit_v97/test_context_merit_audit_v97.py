#!/usr/bin/env python3
"""Focused regression tests for Kinbaku Today Yukimura Ryū audit v97."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v97 as b


class V97(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v97-", dir=HERE)
    ...
    def test_04_direct(self):
        self.assertEqual(sum(not row.get("curation") for row in self.baseline), 107)
    ...
    def test_07_support(self):
        self.assertTrue(self.curation[0]["reason"])
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("kinbakutoday_yukimura_merit_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v97", {"drop": 1, "edit": 0, "keep": 0}, b.EXPECTED_OUTPUT_SHA256),
        )
