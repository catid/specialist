#!/usr/bin/env python3
"""Drop an anecdotal generalized connection claim from Yukimura Ryū."""
...
DATA = ROOT / "data"
V96_DIR = DATA / "manual_reviews/context_merit_audit_v96"
sys.path[:0] = [str(ROOT), str(V96_DIR)]
import build_context_merit_audit_v96 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v97.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v97.jsonl"
REPORT = OUT_DIR / "report_context_merit_v97.json"
REVIEWER = "codex-context-merit-audit-v97"
...
SPECS = (
    {
        "fact_id": "fact-2f3e2526a82142759634",
        "active_index": 46,
        "marker": "the rope bottom senses it much earlier than the rope top realizes it.",
        "decision": "drop",
        "reason_code": "drop_anecdotal_connection_mind_reading_claim",
        "reason": "This lesson anecdote generalizes which partner will detect lost connection first, without yielding dependable or actionable rope guidance.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v96",
    "direct_rows_without_prior_curation": 107,
    "kinbakutoday_yukimura_rows_selected": 1,
    "rows": 533,
    "sha256": "2253dba56880f83645581fd9b5c5e0fb571542a18ab835ae8f82901819448b42",
}
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v97-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v96-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v96.jsonl"
        baseline_report = directory / "v96.report.json"
        ...
        if len(rows) != 533 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v96 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v97 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Yukimura Ryū source lineage drift")
        audit = {
            ...
            "review_pass": "kinbakutoday_yukimura_merit_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v97",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 532 or observed["eval_fact_count"] != 612:
        raise ValueError("v97 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v97 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v96": 495,
        ...
        "schema": "context-merit-audit-report-v97",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the remaining direct Q&A from the fully read Kinbaku Today Yukimura Ryū article",
                "score": "manual source support, generalization risk, actionability, and practical merit review",
