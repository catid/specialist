#!/usr/bin/env python3
"""Focused regression tests for context audit v55."""
import hashlib,json,sys,tempfile,unittest
from pathlib import Path
HERE=Path(__file__).resolve().parent;sys.path.insert(0,str(HERE))
import build_context_merit_audit_v55 as b
from qa_quality import normalize_text

class V55(unittest.TestCase):
 @classmethod
 def setUpClass(cls):
  cls.temp=tempfile.TemporaryDirectory(prefix=".test-v55-",dir=HERE);o=Path(cls.temp.name)
 ...
 def test_04_selection(self):
  ranked=b.ranked_unreviewed_direct(self.baseline);self.assertEqual(len(ranked),16);self.assertEqual(tuple(x["row"]["fact_id"] for x in ranked[:10]),b.EXPECTED_SELECTION)
 ...
 def test_06_direct_counts(self):
  reviewed=b.prior_reviewed_fact_ids();d=[r for r in self.baseline if not r.get("curation")];self.assertEqual((len(d),sum(r["fact_id"] in reviewed for r in d),sum(r["fact_id"] not in reviewed for r in d)),(241,225,16))
 def test_07_decisions(self): self.assertEqual({x:sum(r["decision"]==x for r in self.audit) for x in ("keep","drop","edit")},{"keep":8,"drop":0,"edit":2})
 def test_08_curation(self): self.assertEqual((len(self.curation),{r["action"] for r in self.curation}),(2,{"edit"}))
 ...
 def test_12_report(self):
  self.assertEqual((self.report["schema"],self.report["audit"]["by_decision"],self.report["new_pending_curation"]["edit_support_types"]),("context-merit-audit-report-v55",{"edit":2,"keep":8},{"extractive":1,"manual_paraphrase":1}))
 def test_13_report_projection(self): self.assertEqual((self.report["isolated_build_projection"]["output_rows"],self.report["isolated_build_projection"]["output_sha256"]),(541,"cbe89653c42b6b0a871f64f7627610ce2edd86bfb30e92f786e4997585ad5cf6"))
 def test_14_prior_pins(self):
  pins=self.report["frozen_prior_decision_artifacts"];self.assertEqual(len(pins),162)
  for r in pins:self.assertEqual(b.file_sha256(b.ROOT/r["path"]),r["sha256"])
 def test_15_projection_hash(self): self.assertEqual([hashlib.sha256(p.read_bytes()).hexdigest() for p in self.ds],["cbe89653c42b6b0a871f64f7627610ce2edd86bfb30e92f786e4997585ad5cf6"]*2)
