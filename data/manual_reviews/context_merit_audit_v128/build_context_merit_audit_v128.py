#!/usr/bin/env python3
"""Remove a low-merit translated-title terminology lookup."""
...
DATA = ROOT / "data"
V127_DIR = DATA / "manual_reviews/context_merit_audit_v127"
sys.path[:0] = [str(ROOT), str(V127_DIR)]
import build_context_merit_audit_v127 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v128.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v128.jsonl"
REPORT = OUT_DIR / "report_context_merit_v128.json"
REVIEWER = "codex-context-merit-audit-v128"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__8927124d2b46721bd9d0.json"

SPECS = (
    {
        "fact_id": "fact-e9260d5c94a4b00ca262",
        "active_index": 35,
        "marker": "アメリカ ではロープアーティストと言ってますね。",
        "decision": "drop",
        "reason_code": "low_merit_auto_transcript_title_term",
        "reason": "This tests one translated title from an explicitly automated 1999 transcript that warns it may contain mistakes, rather than stable technique or safety knowledge.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v127",
    "direct_rows_without_prior_curation": 92,
    "rope365_auto_transcript_term_rows_selected": 1,
    "rows": 516,
    "sha256": "6e60275822cfb27b074ce11103ea64e1d05ee4eb71091cab30a1b4d07468a436",
}
EXPECTED_OUTPUT_SHA256 = "0d1293f93bde58ea0df221217036ab6215198851ed88d05f5879f0b2c46b7f58"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v128-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v127-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v127.jsonl"
        baseline_report = directory / "v127.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 516 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v127 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v128 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: translated-excerpt source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_auto_transcript_term_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v128",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 515 or observed["eval_fact_count"] != 612:
        raise ValueError("v128 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v128 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v127": 478,
            ...
            "projection_strategy": "delegate to the v127 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v128",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the not-yet-context-audited translated-title Q&A from the fully read Rope365 Akechi excerpt page",
                "score": "manual source support, automatic-transcript uncertainty, transferable value, and training merit",
