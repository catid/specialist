#!/usr/bin/env python3
"""Focused regression tests for the v189 non-bondage product-fact drop."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v189 as b


class V189(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v189-", dir=HERE)
    ...
    def test_01_baseline(self):
        self.assertEqual((len(self.baseline), b.file_sha256(self.base)), (506, b.PROJECTED_SELECTION_BASELINE["sha256"]))
    ...
    def test_04_direct(self):
        self.assertEqual(sum(not row.get("curation") for row in self.baseline), 55)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("rw_rope_bondage_merit_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )

    def test_11_projection(self):
        self.assertEqual((len(self.rows), b.file_sha256(self.dataset)), (505, b.EXPECTED_OUTPUT_SHA256))
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v189", {"drop": 1, "edit": 0, "keep": 0}, b.EXPECTED_OUTPUT_SHA256),
        )
