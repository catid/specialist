#!/usr/bin/env python3
"""Drop a generic sailing-rope catalog fact with no bondage-specific merit."""
...
DATA = ROOT / "data"
V188_DIR = DATA / "manual_reviews/context_merit_audit_v188"
sys.path[:0] = [str(ROOT), str(V188_DIR)]
import build_context_merit_audit_v188 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v189.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v189.jsonl"
REPORT = OUT_DIR / "report_context_merit_v189.json"
REVIEWER = "codex-context-merit-audit-v189"
...
SOURCE = DATA / "raw/rw_rope_langman_posh_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 14,
        "markers": (
            "The page does not identify bondage as a use.",
        ),
        "decision": "drop",
        "reason_code": "drop_non_bondage_product_material_trivia",
        "reason": "The fully inspected page describes sailing, decorative, and architectural uses but no bondage use, so memorizing the polyester material does not improve rope-bondage answers; the owner-recommended supplier URL remains separately preserved.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v188",
    "direct_rows_without_prior_curation": 55,
    "rw_rope_non_bondage_product_rows_selected": 1,
    "rows": 506,
    "sha256": "bab56e8f4d450e86833931ac1c4dfbfd4cbc036e9cae3ba6aa0dd75b3439feb2",
}
EXPECTED_OUTPUT_SHA256 = "3fc6b2fe2061af2a662e983f98cafd4edb70387b9a23cbf4b6796313607bdaa0"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v189-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v188-projection-", dir=OUT_DIR) as temp:
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v189 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: R&W Rope POSH source lineage drift")
        audit = {
            ...
            "review_pass": "rw_rope_bondage_merit_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v189",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v189 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v189 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v188": 468,
            ...
            "projection_strategy": "delegate to the hermetic v188 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v189",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the generic product-material lookup whose fully inspected page contains no bondage-specific use",
                "score": "manual source support, bondage-domain relevance, redundancy with preserved supplier URL, and training merit",
