#!/usr/bin/env python3
"""Expand Tethered Together's event-focus row with attendee-facing programming."""
...
DATA = ROOT / "data"
V200_DIR = DATA / "manual_reviews/context_merit_audit_v200"
sys.path[:0] = [str(ROOT), str(V200_DIR)]
import build_context_merit_audit_v200 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v201.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v201.jsonl"
REPORT = OUT_DIR / "report_context_merit_v201.json"
REVIEWER = "codex-context-merit-audit-v201"
...
SOURCE = DATA / "raw/tethered_together_home_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 4,
        "markers": (
            "Tethered Together describes itself as an event focused on rope bondage, with additional programming in circus arts, kink, relationships, and body movement such as dance and yoga.",
        ),
        "decision": "edit",
        "question": "What programming does Tethered Together describe beyond its rope-bondage workshops?",
        "answer": "It lists circus arts, kink, relationships, dance and yoga, plus attendee-led programming, art installations, entertainment, and a sex-positive dungeon on Friday through Sunday nights.",
        "reason_code": "expand_event_focus_with_attendee_programming",
        "reason": "The original answer named broad subject areas but omitted what attendees can expect beyond workshops. The replacement preserves those areas and adds the site's concrete attendee-led, art, entertainment, and dungeon programming.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v200",
    "direct_rows_without_prior_curation": 43,
    "tethered_event_programming_rows_selected": 1,
    "rows": 505,
    "sha256": "025cc7280df584896e42764e397ce8341b961c1997b3205c0dc9af5f3f30f090",
}
EXPECTED_OUTPUT_SHA256 = "5719a26a0d66c31cdd297bd23cc7fb18f9fe4c8feb531f9ac24987f8b253ce64"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v201-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v200-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v200.jsonl"
        baseline_report = directory / "v200.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v200 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v201 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Tethered Together home source lineage drift")
        audit = {
            ...
            "review_pass": "tethered_event_programming_completeness_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v201",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v201 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v201 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v200": 467,
            ...
            "projection_strategy": "delegate to the hermetic v200 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v201",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated event-focus row whose fully inspected home page supplies concrete attendee-facing programming",
                "score": "manual source support, resource-selection usefulness, event completeness, and training merit",
