#!/usr/bin/env python3
"""Replace a one-word negotiation lookup with a concrete definition."""
...
DATA = ROOT / "data"
V141_DIR = DATA / "manual_reviews/context_merit_audit_v141"
sys.path[:0] = [str(ROOT), str(V141_DIR)]
import build_context_merit_audit_v141 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v142.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v142.jsonl"
REPORT = OUT_DIR / "report_context_merit_v142.json"
REVIEWER = "codex-context-merit-audit-v142"
...
SPECS = (
    {
        ...
        "active_index": 191,
        "marker": "Give and receive enthusiastic, active consent for activities and any kind of touching. We call these conversations ‘negotiation’.",
        "secondary_marker": "– Ask what is okay and not okay",
        "third_marker": "– Clearly state what you like and dislike",
        "decision": "edit",
        "question": "What does Anatomie Studio mean by negotiation before a rope activity or touching?",
        "answer": "It is a conversation in which people ask what is and is not okay, state their preferences, and give active, enthusiastic consent.",
        "reason_code": "replace_negotiation_lookup_with_concrete_definition",
        "reason": "The replacement teaches what negotiation contains instead of asking for the label alone.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v141",
    "direct_rows_without_prior_curation": 91,
    "anatomiestudio_negotiation_definition_rows_selected": 1,
    "rows": 510,
    "sha256": "5793de2bac2e79be3645fd909c114539a43a4c33e128e112040720a47550088a",
}
EXPECTED_OUTPUT_SHA256 = "a0674efa550f03cda7cf6ef25f628926e8e1126d40fbc5190e03bea39f6529c6"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v142-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v141-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v141.jsonl"
        baseline_report = directory / "v141.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 510 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v141 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v142 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "anatomiestudio_negotiation_definition_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v142",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 510 or observed["eval_fact_count"] != 612:
        raise ValueError("v142 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v142 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v141": 472,
            "active_after_this_tranche": 472,
            ...
            "projection_strategy": "delegate to the hermetic v141 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v142",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the one-word negotiation Q&A whose fully read source provides a concrete definition",
