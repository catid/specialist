#!/usr/bin/env python3
"""Normalize a clipped chicken-wing nerve-safety answer."""
...
DATA = ROOT / "data"
V281_DIR = DATA / "manual_reviews/context_merit_audit_v281"
sys.path[:0] = [str(ROOT), str(V281_DIR)]
import build_context_merit_audit_v281 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v282.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v282.jsonl"
REPORT = OUT_DIR / "report_context_merit_v282.json"
REVIEWER = "codex-context-merit-audit-v282"
...
SPECS = (
    {
        "fact_id": "fact-d5e7e5434fe9029b156a",
        "active_index": 216,
        "markers": (
            "Some may experience nerve impingement symptoms with the position itself. It is important to constantly check the hand motricity to monitor the nerves.",
        ...
        "reason_code": "normalize_chicken_wing_nerve_safety_answer",
        "reason": "The original answer used the awkward term 'hand motricity' and omitted the source's actionable warning signs; the revision turns the same evidence into clear nerve-safety guidance.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v281",
    "direct_rows_without_prior_curation": 1,
    "clipped_answer_rows_selected": 1,
    "rows": 492,
    "sha256": "01146198314c39b644aecb03aaf256faed72bafb313a815e84ce9c619aa70e58",
}
EXPECTED_OUTPUT_SHA256 = "239518ddbdf35eac7abd4ea94b3948f8805b3867b3378bca2afc300353fa61b5"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v282-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v281-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v281.jsonl"
        baseline_report = directory / "v281.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v281 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v282 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "schema": "context-merit-audit-v282",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v282 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v282 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v281": 454,
        ...
        "schema": "context-merit-audit-report-v282",
