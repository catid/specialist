#!/usr/bin/env python3
"""Remove a duplicate Rope365 friction tradeoff answer."""
...
DATA = ROOT / "data"
V242_DIR = DATA / "manual_reviews/context_merit_audit_v242"
sys.path[:0] = [str(ROOT), str(V242_DIR)]
import build_context_merit_audit_v242 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v243.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v243.jsonl"
REPORT = OUT_DIR / "report_context_merit_v243.json"
REVIEWER = "codex-context-merit-audit-v243"
...
SOURCE = DATA / "raw/rope365_8e6f4abea3bf4f6d.json"

SPECS = (
    {
        "fact_id": "fact-4bf5c654db4518328263",
        "active_index": 322,
        "markers": (
            "Frictions are great because they are fast to tie and don’t compact into something hard to untie but they require tension to stay in place.",
        ),
        "decision": "drop",
        "reason_code": "drop_duplicate_friction_tradeoff_answer",
        "reason": "This row repeats the same friction benefits and limitation from the same source sentence as an earlier active question whose answer is more complete and grammatically polished.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v242",
    "direct_rows_without_prior_curation": 1,
    "duplicate_friction_tradeoff_rows_selected": 1,
    "rows": 501,
    "sha256": "f70fc1d92a488548cd20235e61a4a621a820224cd6f4812f45d57f9fb19e063d",
}
EXPECTED_OUTPUT_SHA256 = "0512c1aef1e40e66d31926a4f5d4639489c41fa3ee93ec31daa65d43572b2681"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v243-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v242-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v242.jsonl"
        baseline_report = directory / "v242.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 501 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v242 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v243 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "duplicate_friction_tradeoff_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v243",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 500 or observed["eval_fact_count"] != 612:
        raise ValueError("v243 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v243 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v242": 463,
            "active_after_this_tranche": 462,
        ...
        "schema": "context-merit-audit-report-v243",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "two active friction-tradeoff questions with the same answer and the same supporting source sentence",
