#!/usr/bin/env python3
"""Focused regression tests for Rope365 rope-properties audit v88."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v88 as b


class V88(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v88-", dir=HERE)
    ...
    def test_04_direct(self):
        self.assertEqual(sum(not row.get("curation") for row in self.baseline), 118)

    def test_05_decisions(self):
        self.assertEqual(
            {decision: sum(row["decision"] == decision for row in self.audit) for decision in ("keep", "edit", "drop")},
            {"keep": 0, "edit": 1, "drop": 1},
        )

    def test_06_curation(self):
        self.assertEqual((len(self.curation), {row["action"] for row in self.curation}), (2, {"drop", "edit"}))
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("rope365_rope_properties_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )

    def test_11_projection(self):
        self.assertEqual((len(self.rows), b.file_sha256(self.dataset)), (535, b.EXPECTED_OUTPUT_SHA256))
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v88", {"drop": 1, "edit": 1, "keep": 0}, b.EXPECTED_OUTPUT_SHA256),
        )
