#!/usr/bin/env python3
"""Turn the Austin resource list into a differentiated directory answer."""
...
DATA = ROOT / "data"
V228_DIR = DATA / "manual_reviews/context_merit_audit_v228"
sys.path[:0] = [str(ROOT), str(V228_DIR)]
import build_context_merit_audit_v228 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v229.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v229.jsonl"
REPORT = OUT_DIR / "report_context_merit_v229.json"
REVIEWER = "codex-context-merit-audit-v229"
...
SOURCE = DATA / "raw/owner_austin_resources_summary_20260714.json"

SPECS = (
    {
        "fact_id": "fact-185791df4abf9480c370",
        "active_index": 416,
        "markers": (
            "The Empty Space public event calendar is https://www.atxempty.space/space-schedule.",
            "Austin Rope Slingers is https://www.austinropeslingers.com/.",
            "BightBound is https://www.bightbound.com/.",
        ),
        "decision": "edit",
        "question": "Which owner-recommended Austin rope resources serve event discovery, beginner-friendly peer learning, and intermediate or advanced education?",
        "answer": "Use The Empty Space calendar for event discovery (https://www.atxempty.space/space-schedule), Austin Rope Slingers for 18+ peer learning with weekly 101 material and no partner or prerequisite (https://www.austinropeslingers.com/), and BightBound for intermediate or advanced fundamentals-and-why instruction (https://www.bightbound.com/).",
        "reason_code": "differentiate_austin_resource_directory",
        "reason": "The original answer named all three resources but gave only broad labels. The replacement preserves every exact owner-recommended URL and differentiates them using inspected identity, entry, and teaching context.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v228",
    "direct_rows_without_prior_curation": 15,
    "austin_resource_directory_rows_selected": 1,
    "rows": 505,
    "sha256": "33c18701e1b3aa346639f7315d9a0fce1988ba0c730a4133b67b2581150858d7",
}
EXPECTED_OUTPUT_SHA256 = "20efd0b51cced3a792862f848a8bc27bd0b900e96dd081fe365ffeaecabce8f0"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v229-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v228-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v228.jsonl"
        baseline_report = directory / "v228.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v228 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v229 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "austin_resource_directory_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v229",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v229 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v229 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v228": 467,
            ...
            "projection_strategy": "delegate to the hermetic v228 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v229",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the combined Austin resource row whose inspected sources support a clearer division of purposes",
                "score": "owner-source support, local-resource usefulness, role differentiation, and URL preservation",
