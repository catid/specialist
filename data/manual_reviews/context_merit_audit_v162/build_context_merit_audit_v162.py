#!/usr/bin/env python3
"""Replace a discussion-topic list with ongoing boundary self-reflection."""
...
DATA = ROOT / "data"
V161_DIR = DATA / "manual_reviews/context_merit_audit_v161"
sys.path[:0] = [str(ROOT), str(V161_DIR)]
import build_context_merit_audit_v161 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v162.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v162.jsonl"
REPORT = OUT_DIR / "report_context_merit_v162.json"
REVIEWER = "codex-context-merit-audit-v162"
...
SPECS = (
    {
        ...
        "active_index": 330,
        "marker": "Your first partner is you. Knowing and exploring our boundaries is a lifelong conversation with ourselves.",
        "decision": "edit",
        "question": "What does Anatomie Studio mean by saying “Your first partner is you” in consensual rope exploration?",
        "answer": "Understanding and exploring your own boundaries is an ongoing, lifelong conversation with yourself.",
        "reason_code": "replace_topic_list_with_boundary_self_reflection",
        "reason": "The replacement preserves a distinctive, actionable principle about ongoing self-knowledge instead of another three-item discussion list.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v161",
    "direct_rows_without_prior_curation": 82,
    "anatomiestudio_boundary_self_reflection_rows_selected": 1,
    "rows": 507,
    "sha256": "5b1a30d3ce924d7db2c9f3400a30f325e882320f6ae1f937573a98181c234a59",
}
EXPECTED_OUTPUT_SHA256 = "df6df701bdb547e36296997284734b57d76a07bf70df5bdc789592c5a2670eaf"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v162-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v161-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v161.jsonl"
        baseline_report = directory / "v161.report.json"
        ...
        if len(rows) != 507 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v161 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v162 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "anatomiestudio_boundary_self_reflection_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v162",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 507 or observed["eval_fact_count"] != 612:
        raise ValueError("v162 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v162 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v161": 469,
            ...
            "projection_strategy": "delegate to the hermetic v161 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v162",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the discussion-topic list whose fully read source supports a more distinctive boundary self-reflection principle",
                "score": "manual source support, self-knowledge usefulness, conceptual clarity, and training merit",
