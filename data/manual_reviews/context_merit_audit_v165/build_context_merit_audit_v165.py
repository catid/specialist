#!/usr/bin/env python3
"""Replace a feedback-timing lookup with the reason bottom input is necessary."""
...
DATA = ROOT / "data"
V164_DIR = DATA / "manual_reviews/context_merit_audit_v164"
sys.path[:0] = [str(ROOT), str(V164_DIR)]
import build_context_merit_audit_v164 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v165.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v165.jsonl"
REPORT = OUT_DIR / "report_context_merit_v165.json"
REVIEWER = "codex-context-merit-audit-v165"
...
SPECS = (
    {
        ...
        "active_index": 364,
        "marker": "every person’s body is different, what is correct for you, might be dangerous for another person",
        "decision": "edit",
        "question": "Why must a rope bottom give feedback rather than rely on a rigger to judge whether a tie is suitable?",
        "answer": "Bodies and vulnerabilities differ, so the rigger needs the bottom’s communication to adapt the tie to that person’s fitness, anatomy, and responses.",
        "reason_code": "replace_feedback_timing_with_body_specific_rationale",
        "reason": "The replacement explains the body-specific reason communication is necessary instead of asking for a three-part timing list.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v164",
    "direct_rows_without_prior_curation": 79,
    "kinbakutoday_bottom_feedback_rationale_rows_selected": 1,
    "rows": 507,
    "sha256": "a8ab111112fd881c35ebc2b0187b6c5c50e332307cda7e6ed6f86301302a55f9",
}
EXPECTED_OUTPUT_SHA256 = "400fcc9edd26257bca5b832dc84ac23cf3b078639badbae45ed9c32ae25850f4"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v165-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v164-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v164.jsonl"
        baseline_report = directory / "v164.report.json"
        ...
        if len(rows) != 507 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v164 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v165 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Kinbaku Today rope-bottom guide source lineage drift")
        audit = {
            ...
            "review_pass": "kinbakutoday_bottom_feedback_rationale_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v165",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 507 or observed["eval_fact_count"] != 612:
        raise ValueError("v165 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v165 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v164": 469,
            ...
            "projection_strategy": "delegate to the hermetic v164 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v165",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the feedback-timing list whose fully read source supports a body-specific communication rationale",
