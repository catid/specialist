#!/usr/bin/env python3
"""Normalize a clipped informed-consent answer into a complete sentence."""
...
DATA = ROOT / "data"
V288_DIR = DATA / "manual_reviews/context_merit_audit_v288"
sys.path[:0] = [str(ROOT), str(V288_DIR)]
import build_context_merit_audit_v288 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v289.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v289.jsonl"
REPORT = OUT_DIR / "report_context_merit_v289.json"
REVIEWER = "codex-context-merit-audit-v289"
...
SOURCE = DATA / "raw/esinem_e5f199b3251aa141.json"

SPECS = (
    {
        "fact_id": "fact-ea696b838c0f3091c67f",
        "active_index": 326,
        "markers": (
            "Ultimately, the bitter truth is we can only try to ensure our consent is not only fully informed but also willingly given without undue coercion.",
        ),
        "decision": "edit",
        "question": "What two conditions does Esinem say consent must meet?",
        "answer": "Consent must be fully informed and freely given, without undue coercion.",
        "reason_code": "normalize_informed_freely_given_consent_answer",
        "reason": "The original answer was a bare predicate fragment; the revision preserves both source-supported conditions in a grammatical, self-contained sentence.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v288",
    "direct_rows_without_prior_curation": 1,
    "clipped_answer_rows_selected": 1,
    "rows": 492,
    "sha256": "cdaf70a32714f3a4e248618688fcfc840d10eade0cafca0e1e9c9b15e65ab2c3",
}
EXPECTED_OUTPUT_SHA256 = "17948a72bd383f9445f269567c1fe4964468cecc5892259cb88cfcaf217a5cdb"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v289-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v288-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v288.jsonl"
        baseline_report = directory / "v288.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v288 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v289 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "schema": "context-merit-audit-v289",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v289 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v289 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v288": 454,
        ...
        "schema": "context-merit-audit-report-v289",
