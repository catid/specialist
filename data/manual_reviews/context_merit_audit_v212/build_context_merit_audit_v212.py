#!/usr/bin/env python3
"""Connect The Empty Space's ownership statement to its community purpose."""
...
DATA = ROOT / "data"
V211_DIR = DATA / "manual_reviews/context_merit_audit_v211"
sys.path[:0] = [str(ROOT), str(V211_DIR)]
import build_context_merit_audit_v211 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v212.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v212.jsonl"
REPORT = OUT_DIR / "report_context_merit_v212.json"
REVIEWER = "codex-context-merit-audit-v212"
...
SOURCE = DATA / "raw/rope_resources_v1/atx_empty_space__a33a4d4c949a724b153d.json"
...
SPECS = (
    {
        ...
        "active_index": 88,
        "markers": (
            "Kim and Scout have had intentions of creating nurturing and educational space for the community to gather, learn, experience and grow for years.",
            "The Space here is held with a foundation of respect, awareness, agency and radical self expression.",
            "The Empty Space is a Queer, POC owned and operated community space.",
        ),
        "decision": "edit",
        "question": "How does The Empty Space describe its ownership, community purpose, and operating values?",
        "answer": "It describes itself as a queer, POC-owned and operated community space created for people to gather, learn, experience, and grow, with a foundation of respect, awareness, agency, and radical self-expression.",
        "reason_code": "connect_space_identity_to_purpose_and_values",
        "reason": "The original answer retained only an identity label. The replacement preserves that durable ownership information and connects it to the official page's stated community purpose and operating values.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v211",
    "direct_rows_without_prior_curation": 32,
    "empty_space_identity_purpose_rows_selected": 1,
    "rows": 505,
    "sha256": "ef40c995322a60dda5260eef02fae0ecf5996206c87c379754815d86a5435824",
}
EXPECTED_OUTPUT_SHA256 = "f39187d4426c5f2c41772b59ca713e19001fcdbdcb0100355a065671443225bb"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v212-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v211-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v211.jsonl"
        baseline_report = directory / "v211.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v211 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v212 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Empty Space source lineage drift")
        audit = {
            ...
            "review_pass": "empty_space_identity_purpose_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v212",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v212 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v212 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v211": 467,
            ...
            "projection_strategy": "delegate to the hermetic v211 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v212",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated ownership-label row whose fully inspected official page also states its purpose and operating values",
                "score": "manual source support, community-resource usefulness, context completeness, and training merit",
