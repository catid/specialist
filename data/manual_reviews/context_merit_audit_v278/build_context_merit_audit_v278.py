#!/usr/bin/env python3
"""Normalize a clipped jute-preparation answer into a complete sentence."""
...
DATA = ROOT / "data"
V277_DIR = DATA / "manual_reviews/context_merit_audit_v277"
sys.path[:0] = [str(ROOT), str(V277_DIR)]
import build_context_merit_audit_v277 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v278.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v278.jsonl"
REPORT = OUT_DIR / "report_context_merit_v278.json"
REVIEWER = "codex-context-merit-audit-v278"
...
SOURCE = DATA / "raw/esinem_457ffe4f18d12793.json"

SPECS = (
    {
        "fact_id": "fact-03bd5f8446d593319a62",
        "active_index": 326,
        "markers": (
            "Two common mistakes are, firstly, over-oiling or waxing, making the rope heavy and greasy. Secondly, poor burning off. Keep the rope moving to avoid scorching and use a clean flame, e.g. not a candle which tends to be sooty.",
        ),
        "decision": "edit",
        "question": "What two common jute-rope preparation mistakes does Esinem identify?",
        "answer": "Over-oiling or waxing can make the rope heavy and greasy, while poor singeing—especially holding it still or using a sooty flame—can scorch or dirty it.",
        "reason_code": "normalize_clipped_jute_preparation_answer",
        "reason": "The original answer was a clipped list fragment; the revision preserves the same supported facts in one grammatical, self-contained sentence.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v277",
    "direct_rows_without_prior_curation": 1,
    "clipped_answer_rows_selected": 1,
    "rows": 492,
    "sha256": "6e2ec3c74f7c79c80e688294908f16ac8df498e071b9efc882d0f6763d4008ea",
}
EXPECTED_OUTPUT_SHA256 = "77d43340191dae23669e6557531f69749a3c1c48e6a34321653affc637a0ec03"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v278-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v277-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v277.jsonl"
        baseline_report = directory / "v277.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v277 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v278 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "fragment_normalization_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v278",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v278 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v278 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v277": 454,
        ...
        "schema": "context-merit-audit-report-v278",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "an otherwise useful row whose answer is a clipped source fragment",
                "score": "grammatical completeness, source fidelity, brevity, and answer naturalness",
