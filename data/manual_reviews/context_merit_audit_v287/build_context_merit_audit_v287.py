#!/usr/bin/env python3
"""Add the missing prerequisites to a suspension-training safety answer."""
...
DATA = ROOT / "data"
V286_DIR = DATA / "manual_reviews/context_merit_audit_v286"
sys.path[:0] = [str(ROOT), str(V286_DIR)]
import build_context_merit_audit_v286 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v287.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v287.jsonl"
REPORT = OUT_DIR / "report_context_merit_v287.json"
REVIEWER = "codex-context-merit-audit-v287"
...
SOURCE = DATA / "raw/esinem_aed5c1bb8a4ad341.json"

SPECS = (
    {
        "fact_id": "fact-b3becf5f1f501f8054c4",
        "active_index": 3,
        "markers": (
            "However, assuming you have some solid basic skills, these classes will help you begin suspension safely and create a solid skill set upon which you can build",
            "You need underlying competence. If you are not already doing fairly complex ties and have some experience, go back to practice and study so more.",
        ),
        "decision": "edit",
        ...
        "reason_code": "add_suspension_training_prerequisites",
        "reason": "The original answer was a two-item fragment and omitted the source's broader prerequisite; the revision combines the supported requirements into a clear safety-oriented answer.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v286",
    "direct_rows_without_prior_curation": 1,
    "clipped_answer_rows_selected": 1,
    "rows": 492,
    "sha256": "338514d23d367ae3a8240ef686b47227e8443e23942120540d9a3085a40c69c2",
}
EXPECTED_OUTPUT_SHA256 = "527ef93fac6f53b1dc6a433523f62244cad9978bf36103bbb56a2f61d9c0a1b5"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v287-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v286-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v286.jsonl"
        baseline_report = directory / "v286.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v286 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v287 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "schema": "context-merit-audit-v287",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v287 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v287 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v286": 454,
        ...
        "schema": "context-merit-audit-report-v287",
