#!/usr/bin/env python3
"""Turn a two-cutter lookup into a practical Raptor Rescue feature fact."""
...
DATA = ROOT / "data"
V196_DIR = DATA / "manual_reviews/context_merit_audit_v196"
sys.path[:0] = [str(ROOT), str(V196_DIR)]
import build_context_merit_audit_v196 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v197.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v197.jsonl"
REPORT = OUT_DIR / "report_context_merit_v197.json"
REVIEWER = "codex-context-merit-audit-v197"
...
SOURCE = DATA / "raw/amazon_leatherman_raptor_rescue_20260714.json"

SPECS = (
    {
        "fact_id": "fact-8f4e200335536704487f",
        "active_index": 19,
        "markers": (
            "It says the rescue features include a carbide glass breaker, a strap cutter, and a ring cutter.",
        ),
        "decision": "edit",
        ...
        "reason_code": "replace_two_cutter_lookup_with_rescue_feature_set",
        "reason": "The original answer named only two cutters. The replacement preserves them while adding the shears, glass breaker, and holstered carry behavior described by the fully inspected listing.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v196",
    "direct_rows_without_prior_curation": 47,
    "raptor_rescue_rows_selected": 1,
    "rows": 505,
    "sha256": "85f73910ed76cd01479a09df4de6dc345af254d41da89179ec5926716e8856fa",
}
EXPECTED_OUTPUT_SHA256 = "1e2c221d453fb3129be9835f45a21d13f5d2d4e702c335b9a7be2458076b96db"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v197-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v196-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v196.jsonl"
        baseline_report = directory / "v196.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v196 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v197 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Amazon Raptor Rescue source lineage drift")
        audit = {
            ...
            "review_pass": "raptor_rescue_feature_completeness_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v197",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v197 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v197 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v196": 467,
            ...
            "projection_strategy": "delegate to the hermetic v196 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v197",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated two-cutter lookup whose fully inspected product listing supports a practical rescue-feature set",
                "score": "manual source support, emergency usefulness, feature completeness, and training merit",
