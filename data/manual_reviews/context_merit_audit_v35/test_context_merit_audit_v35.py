#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v35."""
...
import build_context_merit_audit_v35 as builder
from qa_quality import normalize_text


class ContextMeritAuditV35Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline_temp = tempfile.TemporaryDirectory(
            prefix=".test-baseline-v35-", dir=HERE)
        output = Path(cls.baseline_temp.name)
        cls.baseline_path = output / "projection-v34.jsonl"
        cls.baseline_report_path = output / "projection-v34.report.json"
    ...
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v35-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v35.jsonl"),
            ("CURATION", "pending_curation_context_merit_v35.jsonl"),
            ("REPORT", "report_context_merit_v35.json"),
        ):
    ...
    def test_baseline_is_exact_v34_projection(self) -> None:
    ...
    def test_selection_is_exact_deterministic_top_ten(self) -> None:
        ...
        self.assertEqual(len(ranked), 216)
        self.assertEqual(
    ...
    def test_projected_baseline_counts_are_pinned(self) -> None:
        ...
        reviewed = builder.prior_reviewed_fact_ids()
        self.assertEqual(len(direct), 317)
        self.assertEqual(sum(row["fact_id"] in reviewed for row in direct), 101)
    ...
    def test_projection_lineage_is_complete(self) -> None:
        for row in self.audit:
            ...
            self.assertEqual(row["review_pass"],
                             "first_context_merit_review_of_v34_projection_row")
    ...
    def test_report_counts_projection_and_sealed_policy(self) -> None:
        report = json.loads(builder.REPORT.read_text())
        self.assertEqual(report["schema"], "context-merit-audit-report-v35")
        self.assertEqual(report["audit"]["rows"], 10)
        self.assertEqual(report["selection"]["rows_selected"], 10)
        self.assertEqual(report["selection"]["eligible_unreviewed_rows"], 216)
        ...
        self.assertEqual(
            ...
            "1d7839996e8249d0528ca8b99204f90766388ff9d742333ecfb1a5e670893429")
    ...
    def test_future_versions_cannot_enter_v35_by_directory_discovery(self) -> None:
        self.assertEqual(len(builder.CONTEXT_CURATIONS), 34)
        self.assertEqual(len(builder.CONTEXT_AUDITS), 34)
        self.assertIn("context_merit_audit_v34",
                      str(builder.CONTEXT_CURATIONS[-1]))
        self.assertIn("context_merit_audit_v34",
                      str(builder.CONTEXT_AUDITS[-1]))
        self.assertFalse(any("context_merit_audit_v35" in str(path) or
                             "context_merit_audit_v36" in str(path)
                             for path in (*builder.CONTEXT_CURATIONS,
                                          *builder.CONTEXT_AUDITS)))


class IsolatedProjectionV35Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory(
            prefix=".test-projection-v35-", dir=HERE)
    ...
    def test_projection_applies_exact_v35_decisions(self) -> None:
