#!/usr/bin/env python3
"""Focused regression tests for context-merit audit tranche v10."""
...
import build_context_merit_audit_v10 as builder
from qa_quality import normalize_text


class ContextMeritAuditV10Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v10-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output_dir = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v10.jsonl"),
            ("CURATION", "pending_curation_context_merit_v10.jsonl"),
            ("REPORT", "report_context_merit_v10.json"),
        ):
    ...
    def test_future_context_tranches_are_not_historical_inputs(self) -> None:
        ...
        prior = manual_root / "context_merit_audit_v9" / "synthetic.jsonl"
        ...
        self.assertEqual(
            ...
            frozenset(f"context_merit_audit_v{i}" for i in range(1, 10)),
        )
    ...
    def test_report_projection_and_sealed_policy(self) -> None:
        ...
        self.assertEqual(projection["output_rows"], 728)
        self.assertEqual(
            projection["output_sha256"],
            "2793bed4b995edb05ba315259f820999982d8f59e3dc1b38ee7803733f869acf")
    ...
    def test_generator_is_deterministic_and_prior_is_frozen(self) -> None:
        ...
        prior_files = tuple(
            path for version in range(1, 10)
