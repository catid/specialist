#!/usr/bin/env python3
"""Replace a book-title lookup with a beginner-learning lesson."""
...
DATA = ROOT / "data"
V264_DIR = DATA / "manual_reviews/context_merit_audit_v264"
sys.path[:0] = [str(ROOT), str(V264_DIR)]
import build_context_merit_audit_v264 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v265.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v265.jsonl"
REPORT = OUT_DIR / "report_context_merit_v265.json"
REVIEWER = "codex-context-merit-audit-v265"
...
SOURCE = DATA / "raw/kinbakutoday_5cea0bc70445dc65.json"
...
SPECS = (
    {
        ...
        "active_index": 393,
        "markers": (
            "To learn ropes in the technical way little notion are needed, above all safety, and basic knots, all can be learned in few hours. On the other hand to learn Kinbaku a lifetime is not enough. You never finish learning. So do not rush and practice, learn, practice and learn again and again, and then practice again.",
            "ropes are easy to handle and first knots are enough to enjoy, but make them part of what we are and really build emotions it is never-ending and unique journey.",
        ),
        "decision": "edit",
        "question": "What does MaestroBD say beginners can learn quickly, and what takes a lifetime?",
        "answer": "Basic safety and knots can be learned in hours, but developing kinbaku as an emotional, relational practice requires patient, repeated learning throughout life.",
        "reason_code": "replace_book_lookup_with_beginner_learning_scope",
        "reason": "The replacement trades a bare book-title lookup for the interview's directly useful distinction between quickly learned technical basics and lifelong relational and expressive development.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v264",
    "direct_rows_without_prior_curation": 1,
    "book_title_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "a6b90dc965d061b34481bf48f2914eda68e009783c7ec22302fa0e3fa8cda844",
}
EXPECTED_OUTPUT_SHA256 = "47adeeabf3fe29b30d76e99872739ef1360c3efa01fe5c6c73178257400e468e"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v265-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v264-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v264.jsonl"
        baseline_report = directory / "v264.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v264 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v265 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "beginner_learning_scope_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v265",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v265 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v265 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v264": 454,
        ...
        "schema": "context-merit-audit-report-v265",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a book-title lookup whose source passage supports a beginner-learning scope lesson",
                "score": "direct beginner value, source support, question naturalness, and reduced title trivia",
