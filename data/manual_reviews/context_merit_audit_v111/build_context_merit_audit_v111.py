#!/usr/bin/env python3
"""Audit the Rope365 diamond-harness breathing guidance."""
...
DATA = ROOT / "data"
V110_DIR = DATA / "manual_reviews/context_merit_audit_v110"
sys.path[:0] = [str(ROOT), str(V110_DIR)]
import build_context_merit_audit_v110 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v111.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v111.jsonl"
REPORT = OUT_DIR / "report_context_merit_v111.json"
REVIEWER = "codex-context-merit-audit-v111"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__b190861046dbc85a6875.json"

SPECS = (
    {
        "fact_id": "fact-39971c0aab91fba7a804",
        "active_index": 234,
        "marker": "Opening the diamonds will make the tie progressively tighter, watch out for the ability to breathe of the person being tied. Make sure you discuss ahead placement between the legs. You can split ropes after one of the knots to go around the waist instead.",
        "decision": "keep",
        "reason_code": "clear_diamond_harness_breathing_monitoring",
        "reason": "The answer accurately preserves the source's key monitoring requirement as opening the diamonds progressively tightens the harness.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v110",
    "direct_rows_without_prior_curation": 92,
    "rope365_chest_diamond_rows_selected": 1,
    "rows": 529,
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v111-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v110-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v110.jsonl"
        baseline_report = directory / "v110.report.json"
        ...
        if len(rows) != 529 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v110 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v111 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: chest-diamond source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_chest_diamond_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v111",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 529 or observed["eval_fact_count"] != 612:
        raise ValueError("v111 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v111 output hash drift")
    report = {
        "active_baseline": {
        ...
        "audit": {
            "by_decision": {"drop": 0, "edit": 0, "keep": 1},
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v110": 491,
        ...
        "schema": "context-merit-audit-report-v111",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the not-yet-context-audited breathing Q&A from the fully read Rope365 chest-diamond page",
                "score": "manual source support, breathing-safety actionability, answer completeness, and practical merit review",
