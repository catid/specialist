#!/usr/bin/env python3
"""Focused regression tests for Rope365 chest-diamond audit v111."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v111 as b


class V111(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v111-", dir=HERE)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("rope365_chest_diamond_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v111", {"drop": 0, "edit": 0, "keep": 1}, b.EXPECTED_OUTPUT_SHA256),
        )
