#!/usr/bin/env python3
"""Audit the highest-risk directly curatable v42 projection rows in v43."""

from __future__ import annotations

import contextlib
import json
import subprocess
import sys
import tempfile
from pathlib import Path

ROOT = Path(__file__).resolve().parents[3]
V42_DIR = ROOT / "data" / "manual_reviews" / "context_merit_audit_v42"
sys.path[:0] = [str(ROOT), str(V42_DIR)]
import build_context_merit_audit_v42 as previous
from qa_quality import normalize_text

BASE = previous.previous.BASE
CORE = previous.CORE
EVIDENCE_PATCH_MODULE = BASE.previous.previous
DATA = ROOT / "data"
OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v43.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v43.jsonl"
REPORT = OUT_DIR / "report_context_merit_v43.json"
REVIEWER = "codex-context-merit-audit-v43"
REVIEWED_AT = "2026-07-14"
RESOURCE_MANIFEST = previous.RESOURCE_MANIFEST
ACTIVE_DATASET = previous.ACTIVE_DATASET
ACTIVE_REPORT = previous.ACTIVE_REPORT
ACTIVE_CURATIONS = previous.ACTIVE_CURATIONS
PRIOR_PENDING_ADDITIONS = previous.PRIOR_PENDING_ADDITIONS
QUALITY_MERIT_CURATION = previous.QUALITY_MERIT_CURATION
TASUKI_CURATION = previous.TASUKI_CURATION
file_sha256 = previous.file_sha256
text_sha256 = previous.text_sha256
read_jsonl = previous.read_jsonl
write_jsonl = previous.write_jsonl

CONTEXT_CURATIONS = tuple(
    DATA / "manual_reviews" / f"context_merit_audit_v{version}" /
    f"pending_curation_context_merit_v{version}.jsonl"
    for version in range(1, 43)
)
CONTEXT_AUDITS = tuple(
    DATA / "manual_reviews" / f"context_merit_audit_v{version}" /
    f"context_merit_audit_v{version}.jsonl"
    for version in range(1, 43)
)


def raw(name: str) -> Path:
    return DATA / "raw" / name


SPECS = (
    {
        "fact_id": "fact-41a7ed130ec0582bb894",
        "source_path": raw("rope365_25f1b23eb40be00e.json"),
        "marker": "the thistle knot and the Matthew Walker knots",
        "decision": "drop",
        "reason_code": "incidental_stopper_knot_name_lookup",
        "reason": (
            "The two-name lookup is low-value knot trivia; v39 already "
            "retains the same page's more transferable practice-and-context "
            "learning goal."
        ),
    },
    {
        "fact_id": "fact-c2d40e5f67bd89c7ea19",
        "source_path": DATA / "rope_resource_factual_qa_v1.jsonl",
        "marker": "Add 1/2 teaspoon of free and clear detergent",
        "decision": "keep",
        "reason_code": "manufacturer_specific_jute_wash_dosage",
        "reason": (
            "The question explicitly limits the dosage to deGiotto's own jute "
            "and its recommended cool-water tub wash, matching the vendor's "
            "material-specific directions."
        ),
    },
    {
        "fact_id": "fact-b64acb7323c233474fc0",
        "source_path": raw("kinbakutoday_be37dae4ec8e0d88.json"),
        "marker": "Performing was financially unrewarding",
        "decision": "edit",
        "question": (
            "According to Marc in the debate, why did early Western rope "
            "professionals shift toward teaching and selling rope or materials?"
        ),
        "answer": (
            "Because performing was financially unrewarding for people trying "
            "to make a living."
        ),
        "support_type": "manual_paraphrase",
        "paraphrase_rationale": (
            "The answer turns the source's cause-and-result sentence into a "
            "direct explanation while retaining the financial limitation and "
            "professional livelihood context."
        ),
        "paraphrase_support_fragments": (
            "performing was financially unrewarding",
            "early professionals who aspired to make a living",
        ),
        "reason_code": "clarify_early_professional_business_shift",
        "reason": (
            "The edit replaces an awkward object lookup with the attributed "
            "economic reason for the early professional business-model shift."
        ),
    },
    {
        "fact_id": "fact-4af7fdc6e5bd4c92eb36",
        "source_path": raw("kinbakutoday_c364f23ce34ae761.json"),
        "marker": "I might be (sexually) aroused",
        "decision": "keep",
        "reason_code": "essential_arousal_consent_distinction",
        "reason": (
            "The source explicitly separates involuntary or situational arousal "
            "from a negotiated decision to engage in sexual activity."
        ),
    },
    {
        "fact_id": "fact-b0e335e672410a009450",
        "source_path": raw("kinbakutoday_9af201153c54e4d5.json"),
        "marker": "Ryū, sensei, grandmaster, seme",
        "decision": "edit",
        "question": (
            "What warning does the author give about Japanese-style titles "
            "used in rope communities?"
        ),
        "answer": (
            "Terms such as ryū, sensei, grandmaster, seme, and deshi can have "
            "promotional value but do not guarantee substantive transmitted "
            "lineage or quality."
        ),
        "support_type": "manual_paraphrase",
        "paraphrase_rationale": (
            "The answer combines the source's adjacent title list, its note "
            "that deshi is even more promotional, and its explicit no-guarantee "
            "caveat into one grammatical warning."
        ),
        "paraphrase_support_fragments": (
            "Ryū, sensei, grandmaster, seme",
            "deshi even more so",
            "None guarantee any substantive transmitted lineage, or quality",
        ),
        "reason_code": "replace_title_list_with_lineage_quality_caveat",
        "reason": (
            "The edit makes the transferable warning about promotional titles, "
            "lineage, and quality the answer instead of testing list recall."
        ),
    },
    {
        "fact_id": "fact-a583cbd8c146bb5aca40",
        "source_path": DATA / "rope_resource_factual_qa_v1.jsonl",
        "marker": "not designed for professional performance use or dynamic drops",
        "decision": "keep",
        "reason_code": "manufacturer_a_frame_use_limit",
        "reason": (
            "The manufacturer-attributed home/light-studio scope and explicit "
            "professional-performance/dynamic-drop exclusion are essential "
            "limitations for the owner-requested frame resource."
        ),
    },
    {
        "fact_id": "fact-9242c0f5751c45d1e4a8",
        "source_path": raw("kinbakutoday_1dccbc876bee51b6.json"),
        "marker": "Matsui Kenji san mitsu-hiki shibari",
        "decision": "drop",
        "reason_code": "contextless_full_body_tie_name",
        "reason": (
            "The proper-name lookup identifies a demonstrated full-body tie "
            "without adding safety, consent, construction, or broadly useful "
            "historical context."
        ),
    },
    {
        "fact_id": "fact-93c032484cf3a72fcc5c",
        "source_path": raw("wykd_a74fec63b0114fff.json"),
        "marker": "Ipponnawa if you were counting ropes",
        "decision": "drop",
        "reason_code": "isolated_terminology_trivia",
        "reason": (
            "The narrow answer only resolves one author's preferred Japanese "
            "label and does not provide broadly useful rope practice, safety, "
            "consent, or history guidance."
        ),
    },
    {
        "fact_id": "fact-6110411abb96bc6a08c0",
        "source_path": raw("rope_resources_v1/rope365__c1314f53c65df4af2c20.json"),
        "marker": "Do not singe synthetic rope as it will melt",
        "decision": "keep",
        "reason_code": "protected_synthetic_singeing_warning",
        "reason": (
            "The concise material-specific warning is directly supported and "
            "retains a protected safety-addition fact ID."
        ),
    },
    {
        "fact_id": "fact-42872b18e8b3a087ed3d",
        "source_path": raw("rope_resources_v1/rope365__c1314f53c65df4af2c20.json"),
        "marker": "conditioning rope does weaken it and will shorten its lifespan",
        "decision": "keep",
        "reason_code": "protected_conditioning_strength_tradeoff",
        "reason": (
            "The source states the important strength-and-lifespan compromise "
            "directly, and retaining it preserves its protected safety-addition ID."
        ),
    },
)

EXPECTED_SELECTION = tuple(spec["fact_id"] for spec in SPECS)
PROJECTED_ACTIVE_INDICES = {
    fact_id: index for fact_id, index in zip(
        EXPECTED_SELECTION, (501, 41, 155, 296, 487, 32, 252, 409, 193, 369))
}
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated cumulative training projection through context-merit v42",
    "direct_rows_without_prior_curation": 278,
    "eligible_unreviewed_direct_rows": 136,
    "prior_context_reviewed_direct_rows_excluded": 142,
    "rows": 563,
    "sha256": "2a2997ee684be4df0c16df269f99638dc4c5d43b3dde100529e6bad2d293521e",
}
ISOLATED_PROJECTION = {
    "active_after_context_merit_v42": 525,
    "active_after_this_tranche": 522,
    "build_script": "build_curated_qa.py",
    "new_drops_applied": 3,
    "new_edits_applied": 2,
    "output_rows": 560,
    "output_sha256": "520ee9b6b37f5729ba98c6232680094b9727d471fe71a9d35481ca7b498c18dd",
    "prior_pending_addition_fact_ids_preserved": 37,
    "repeat_dataset_byte_identical": True,
    "reviewed_keep_fact_ids_preserved": 5,
    "sealed_eval_fact_count_reported_by_tooling": 612,
    "unexpected_fact_ids": 0,
    "validated_runs": 2,
}

PRODUCTION_INPUTS = previous.previous.PRODUCTION_INPUTS
SEALED_EVAL_PATHS = previous.previous.SEALED_EVAL_PATHS
PRIOR_PROJECTION_CURATIONS = (
    *ACTIVE_CURATIONS, QUALITY_MERIT_CURATION, TASUKI_CURATION,
    *CONTEXT_CURATIONS,
)


def prior_decision_artifacts() -> tuple[Path, ...]:
    paths = []
    for version in range(1, 43):
        directory = DATA / "manual_reviews" / f"context_merit_audit_v{version}"
        paths.extend((
            directory / f"context_merit_audit_v{version}.jsonl",
            directory / f"pending_curation_context_merit_v{version}.jsonl",
            directory / f"report_context_merit_v{version}.json",
        ))
    return tuple(paths)


def build_projection(output: Path, report: Path,
                     curations: tuple[Path, ...]) -> None:
    command = [
        sys.executable, "build_curated_qa.py",
        "--inputs", *(str(path.relative_to(ROOT)) for path in PRODUCTION_INPUTS),
        "--eval", *(str(path.relative_to(ROOT)) for path in SEALED_EVAL_PATHS),
        "--curation", *(str(path.relative_to(ROOT)) for path in curations),
        "--output", str(output), "--report", str(report),
    ]
    subprocess.run(command, cwd=ROOT, check=True, stdout=subprocess.DEVNULL)


def prior_reviewed_fact_ids() -> set[str]:
    return {
        row["fact_id"] for path in CONTEXT_AUDITS
        for row in read_jsonl(path)
    }


def ranked_unreviewed_direct(rows: list[dict]) -> list[dict]:
    reviewed = prior_reviewed_fact_ids()
    candidates = []
    for index, row in enumerate(rows, 1):
        if row.get("curation") or row["fact_id"] in reviewed:
            continue
        features = CORE.risk_features(row)
        candidates.append((
            -features["risk_score"], features["question_tokens"],
            features["answer_tokens"], row["fact_id"], index, row, features,
        ))
    candidates.sort(key=lambda item: item[:4])
    ranked = [{"active_index": item[4], "row": item[5], "features": item[6]}
              for item in candidates]
    if len(ranked) != 136:
        raise ValueError(f"v43 candidate drift: {len(ranked)}")
    if tuple(item["row"]["fact_id"] for item in ranked[:10]) != EXPECTED_SELECTION:
        raise ValueError("v43 selection drift")
    return ranked


def selected_ranked(rows: list[dict]) -> tuple[list[dict], int, int]:
    return ranked_unreviewed_direct(rows)[:10], 0, 0


def source_evidence(spec: dict, active: dict) -> tuple[str, str]:
    declared_document_hash = ""
    if spec["source_path"].suffix == ".jsonl":
        source_rows = [row for row in read_jsonl(spec["source_path"])
                       if row["fact_id"] == active["fact_id"]]
        if len(source_rows) != 1:
            raise ValueError(f'{active["fact_id"]}: manual source row drift')
        source_row = source_rows[0]
        document = {"url": source_row["url"], "text": source_row["evidence"]}
        declared_document_hash = source_row.get("document_sha256", "")
    else:
        document = json.loads(spec["source_path"].read_text())
    if document["url"] != active["url"]:
        raise ValueError(f'{active["fact_id"]}: source URL drift')
    document_hash = text_sha256(document["text"])
    active_evidence = active.get("evidence", "")
    evidence_hash = text_sha256(active_evidence) if active_evidence else ""
    if active["document_sha256"] not in {
            document_hash, evidence_hash, declared_document_hash}:
        raise ValueError(f'{active["fact_id"]}: source hash drift')
    if active_evidence and normalize_text(active_evidence) not in normalize_text(
            document["text"]):
        raise ValueError(f'{active["fact_id"]}: stored evidence drift')
    matches = [line for line in document["text"].splitlines()
               if spec["marker"] in line]
    if len(matches) != 1:
        raise ValueError(f'{active["fact_id"]}: evidence marker drift')
    evidence = matches[0]
    answer = spec.get("answer", active["answer"])
    support_type = spec.get("support_type", "normalized_extractive")
    if support_type == "manual_paraphrase":
        if not spec.get("paraphrase_rationale"):
            raise ValueError(f'{active["fact_id"]}: missing paraphrase rationale')
        for fragment in spec.get("paraphrase_support_fragments", ()):
            if normalize_text(fragment) not in normalize_text(evidence):
                raise ValueError(f'{active["fact_id"]}: unsupported paraphrase fragment')
    elif normalize_text(answer) not in normalize_text(evidence):
        raise ValueError(f'{active["fact_id"]}: unsupported answer')
    return evidence, support_type


@contextlib.contextmanager
def patched_base(projected_dataset: Path):
    replacements = {
        "OUT_DIR": OUT_DIR, "AUDIT": AUDIT, "CURATION": CURATION,
        "REPORT": REPORT, "REVIEWER": REVIEWER, "REVIEWED_AT": REVIEWED_AT,
        "CONTEXT_CURATIONS": CONTEXT_CURATIONS, "SPECS": SPECS,
        "EXPECTED_SELECTION": EXPECTED_SELECTION,
        "ISOLATED_PROJECTION": ISOLATED_PROJECTION,
    }
    originals = {name: getattr(BASE, name) for name in replacements}
    original_ranking = CORE.ranked_unreviewed
    original_active = CORE.ACTIVE_DATASET
    original_evidence = EVIDENCE_PATCH_MODULE.source_evidence
    try:
        for name, value in replacements.items():
            setattr(BASE, name, value)
        CORE.ranked_unreviewed = selected_ranked
        CORE.ACTIVE_DATASET = projected_dataset
        EVIDENCE_PATCH_MODULE.source_evidence = source_evidence
        yield
    finally:
        EVIDENCE_PATCH_MODULE.source_evidence = original_evidence
        CORE.ACTIVE_DATASET = original_active
        CORE.ranked_unreviewed = original_ranking
        for name, value in originals.items():
            setattr(BASE, name, value)


def main() -> None:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix=".v42-projection-", dir=OUT_DIR) as temp:
        projected_dataset = Path(temp) / "projection-v42.jsonl"
        projected_report = Path(temp) / "projection-v42.report.json"
        build_projection(projected_dataset, projected_report,
                         PRIOR_PROJECTION_CURATIONS)
        if len(read_jsonl(projected_dataset)) != 563:
            raise ValueError("v42 projection row-count drift")
        if file_sha256(projected_dataset) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v42 projection hash drift")
        with patched_base(projected_dataset):
            BASE.main()

    audits = read_jsonl(AUDIT)
    for row in audits:
        row["schema"] = "context-merit-audit-v43"
        row["review_pass"] = "first_context_merit_review_of_v42_projection_row"
        row["projection_lineage"] = {
            "active_index": PROJECTED_ACTIVE_INDICES[row["fact_id"]],
            "baseline_rows": 563,
            "baseline_sha256": PROJECTED_SELECTION_BASELINE["sha256"],
            "prior_context_merit_review": False,
        }
        spec = next(spec for spec in SPECS if spec["fact_id"] == row["fact_id"])
        if spec.get("support_type") == "manual_paraphrase":
            row["paraphrase_rationale"] = spec["paraphrase_rationale"]
    write_jsonl(AUDIT, audits)

    curations = read_jsonl(CURATION)
    for row in curations:
        spec = next(spec for spec in SPECS if spec["fact_id"] == row["fact_id"])
        if spec.get("support_type") == "manual_paraphrase":
            row["support_type"] = "manual_paraphrase"
            row["paraphrase_rationale"] = spec["paraphrase_rationale"]
    write_jsonl(CURATION, curations)

    report = json.loads(REPORT.read_text())
    report["schema"] = "context-merit-audit-report-v43"
    report["active_baseline"] = {
        "dataset": {"path": str(ACTIVE_DATASET.relative_to(ROOT)), "rows": 784,
                    "sha256": file_sha256(ACTIVE_DATASET)},
        "report": {"path": str(ACTIVE_REPORT.relative_to(ROOT)),
                   "sha256": file_sha256(ACTIVE_REPORT)},
        "curation": [{"path": str(path.relative_to(ROOT)),
                      "sha256": file_sha256(path)} for path in ACTIVE_CURATIONS],
    }
    report["selection"].update({
        "active_rows": 563,
        "eligible_unreviewed_rows": 136,
        "excluded_active_review_provenance": 0,
        "excluded_ledger_fact_ids": 0,
        "rows_selected": 10,
        "projected_baseline": PROJECTED_SELECTION_BASELINE,
        "ranking": {
            "candidate_rule": (
                "the row survives the v42 projection, has no prior curation "
                "metadata, and its fact_id has no context-merit decision in "
                "v1 through v42"
            ),
            "score": (
                "short_question_points + 3*pronoun_count + "
                "bare_answer_points + named_person_trivia_points"
            ),
            "tie_break": (
                "risk_score descending, question tokens ascending, answer "
                "tokens ascending, fact_id ascending"
            ),
        },
    })
    report["audit"]["rows"] = len(audits)
    report["audit"]["sha256"] = file_sha256(AUDIT)
    report["new_pending_curation"]["sha256"] = file_sha256(CURATION)
    report["new_pending_curation"]["edit_support_types"] = {
        "extractive": 0, "manual_paraphrase": 2,
    }
    report["frozen_prior_decision_artifacts"] = [
        {"path": str(path.relative_to(ROOT)), "sha256": file_sha256(path)}
        for path in prior_decision_artifacts()
    ]
    REPORT.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")


if __name__ == "__main__":
    main()
