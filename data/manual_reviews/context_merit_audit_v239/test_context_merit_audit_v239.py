#!/usr/bin/env python3
"""Focused regression tests for the v239 remaining owner resources."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v239 as b


class V239(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v239-", dir=HERE)
    ...
    def test_01_baseline(self):
        self.assertEqual((len(self.baseline), b.file_sha256(self.base)), (505, b.PROJECTED_SELECTION_BASELINE["sha256"]))
    ...
    def test_04_direct(self):
        self.assertEqual((sum(not row.get("curation") for row in self.baseline), sum(not row.get("curation") for row in self.rows)), (5, 1))

    def test_05_decisions(self):
        self.assertEqual(
            {decision: sum(row["decision"] == decision for row in self.audit) for decision in ("keep", "edit", "drop")},
            {"keep": 1, "edit": 3, "drop": 1},
        )

    def test_06_curation(self):
        self.assertEqual((len(self.curation), {row["action"] for row in self.curation}), (4, {"edit", "drop"}))

    def test_07_support(self):
        self.assertEqual({row["support_type"] for row in self.curation if row["action"] == "edit"}, {"manual_paraphrase"})
    ...
    def test_08_source(self):
        for row in self.audit:
            ...
            self.assertIn(row["url"], row["support_evidence"])
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("remaining_owner_resources_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )

    def test_11_projection(self):
        self.assertEqual((len(self.rows), b.file_sha256(self.dataset)), (504, b.EXPECTED_OUTPUT_SHA256))
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v239", {"drop": 1, "edit": 3, "keep": 1}, b.EXPECTED_OUTPUT_SHA256),
        )
