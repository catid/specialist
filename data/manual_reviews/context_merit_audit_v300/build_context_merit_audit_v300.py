#!/usr/bin/env python3
"""Integrate the eleventh distinct-document safety tranche."""
...
ADDITION_DIR = DATA / "manual_reviews/safety_additions_v11"
sys.path[:0] = [str(ROOT), str(V299_DIR), str(V290_DIR), str(ADDITION_DIR)]
import build_context_merit_audit_v299 as previous
import build_context_merit_audit_v290 as core
import build_safety_additions_v11 as additions_builder

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v300.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v300.jsonl"
REPORT = OUT_DIR / "report_context_merit_v300.json"
ADDITIONS = additions_builder.OUTPUT
EXPECTED_ADDITIONS_SHA256 = "630cec95eabab4cf266c26e6e2072c72cda2de5c0e0217fcc05c83170d18bedb"
EXPECTED_OUTPUT_SHA256 = "b0315512acb0af95ff5fd0f0af835b21fcda293ff87c3cf9329a0ca1e44493a0"
BASELINE_ROWS = 522
BASELINE_SHA256 = "6f65b7ac17fd30e92cb843d8fcc158e0532b725d61af834501bd592a709918d0"
EXPECTED_CAPACITY = {
    "before": {"conflict_units": 229, "equipment_material": 21, "resources_general": 79, "safety_consent": 74, "technique": 55},
    "after": {"conflict_units": 232, "equipment_material": 21, "resources_general": 79, "safety_consent": 77, "technique": 55},
}
...
PRIOR_ARTIFACTS = (
    V299_DIR / "context_merit_audit_v299.jsonl",
    V299_DIR / "pending_curation_context_merit_v299.jsonl",
    V299_DIR / "report_context_merit_v299.json",
)
...
def build_projection(output: Path, report: Path) -> None:
    directory = output.parent / f".{output.name}.v300-input"
    directory.mkdir(parents=True, exist_ok=True)
    baseline = directory / "v299.jsonl"
    build_baseline(baseline, directory / "v299.report.json")
    core.build_projection_with_inputs(output, report, (), (baseline, ADDITIONS))


def observe() -> dict:
    with tempfile.TemporaryDirectory(prefix=".v300-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main() -> None:
    ...
    for index, row in enumerate(read_jsonl(ADDITIONS), 1):
        ...
        audits.append({
            ...
            "review_pass": "distinct_document_safety_additions",
            "reviewed_at": "2026-07-15",
            "reviewer": "codex-context-merit-audit-v300",
            "schema": "context-merit-audit-v300",
            "source": row["source"],
    ...
    if not observed["dataset_equal"] or not observed["report_equal"]:
        raise ValueError("v300 projection is nondeterministic")
    if (observed["rows"], observed["eval_fact_count"]) != (525, 612):
        raise ValueError("v300 row/eval aggregate drift")
    if observed["baseline_capacity"] != EXPECTED_CAPACITY["before"] or observed["output_capacity"] != EXPECTED_CAPACITY["after"]:
        raise ValueError(f"v300 capacity drift: {observed['baseline_capacity']} -> {observed['output_capacity']}")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v300 output hash drift")
    report = {
        ...
        "schema": "context-merit-audit-report-v300",
