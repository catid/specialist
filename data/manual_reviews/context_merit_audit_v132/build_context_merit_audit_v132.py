#!/usr/bin/env python3
"""Remove a rope-history claim contradicted by another collected source."""
...
DATA = ROOT / "data"
V131_DIR = DATA / "manual_reviews/context_merit_audit_v131"
sys.path[:0] = [str(ROOT), str(V131_DIR)]
import build_context_merit_audit_v131 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v132.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v132.jsonl"
REPORT = OUT_DIR / "report_context_merit_v132.json"
REVIEWER = "codex-context-merit-audit-v132"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__d99c6a7fa089567b35e6.json"
CONFLICTING_SOURCE = DATA / "raw/kinbakutoday_82071039cb003b58.json"

SPECS = (
    {
        "fact_id": "fact-46e5971565f61e1bb9ae",
        "active_index": 18,
        "marker": "He was a co-founder of Bakuyukai, the first Japanese rope dojo.",
        "conflicting_marker": "Initially established in 1996 by Miura Takumi, its rigorous formulations you might see in a western Dojō.",
        "decision": "drop",
        "reason_code": "unresolved_bakuyukai_founder_conflict",
        "reason": "Two collected sources conflict: Rope365 calls Akechi Denki a co-founder and Bakuyukai the first Japanese rope dojo, while Kinbaku Today says it was initially established in 1996 by Miura Takumi; the training row cannot resolve this discrepancy reliably.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v131",
    "direct_rows_without_prior_curation": 92,
    "bakuyukai_conflict_rows_selected": 1,
    "rows": 513,
    "sha256": "63a65cfe8a63447648fd91ecee010b975feeacb5b1a30c63d0c734f26a698e77",
}
EXPECTED_OUTPUT_SHA256 = "1c682560f79de33e6a0e1bc2dc3d348231087595a28dd6d8a0e22fd7d1786107"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v132-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v131-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v131.jsonl"
        baseline_report = directory / "v131.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 513 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v131 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v132 candidate drift")

    document = json.loads(SOURCE.read_text())
    conflicting_document = json.loads(CONFLICTING_SOURCE.read_text())
    audits = []
    curations = []
    for audit_index, spec in enumerate(SPECS, 1):
        active = by_fact[spec["fact_id"]][1]
        support = evidence(document, spec["marker"])
        conflicting_support = evidence(conflicting_document, spec["conflicting_marker"])
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: rope-artist source lineage drift")
        audit = {
            ...
            "review_pass": "bakuyukai_cross_source_conflict_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v132",
            ...
            "url": document["url"],
            "conflicting_source": conflicting_document["source"],
            "conflicting_source_document": str(CONFLICTING_SOURCE.relative_to(ROOT)),
            "conflicting_source_document_file_sha256": file_sha256(CONFLICTING_SOURCE),
            "conflicting_support_evidence": conflicting_support,
            "conflicting_support_evidence_sha256": text_sha256(conflicting_support),
            "conflicting_url": conflicting_document["url"],
        }
            ...
            curations.append(
                {
                    ...
                    "conflicting_evidence": conflicting_support,
                    "conflicting_evidence_url": conflicting_document["url"],
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 512 or observed["eval_fact_count"] != 612:
        raise ValueError("v132 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v132 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v131": 475,
            "active_after_this_tranche": 474,
            ...
            "projection_strategy": "delegate to the hermetic v131 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v132",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the active Bakuyukai claim after manually reading two collected sources that disagree about its founding",
                "score": "cross-source consistency, claim reliability, attribution, and training merit",
