#!/usr/bin/env python3
"""Focused regression tests for the v132 Bakuyukai conflict audit."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v132 as b


class V132(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v132-", dir=HERE)
    ...
    def test_01_baseline(self):
        self.assertEqual((len(self.baseline), b.file_sha256(self.base)), (513, b.PROJECTED_SELECTION_BASELINE["sha256"]))
    ...
    def test_07_support(self):
        self.assertNotIn("support_type", self.curation[0])
    ...
    def test_08_source(self):
        for row in self.audit:
            ...
            self.assertEqual(row["conflicting_source_document_file_sha256"], b.file_sha256(b.CONFLICTING_SOURCE))
            self.assertEqual(b.text_sha256(row["conflicting_support_evidence"]), row["conflicting_support_evidence_sha256"])
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("bakuyukai_cross_source_conflict_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )

    def test_11_projection(self):
        self.assertEqual((len(self.rows), b.file_sha256(self.dataset)), (512, b.EXPECTED_OUTPUT_SHA256))
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v132", {"drop": 1, "edit": 0, "keep": 0}, b.EXPECTED_OUTPUT_SHA256),
        )
