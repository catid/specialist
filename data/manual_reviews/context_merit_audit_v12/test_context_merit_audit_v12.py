#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v12."""
...
import build_context_merit_audit_v12 as builder
from qa_quality import normalize_text


class ContextMeritAuditV12Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v12-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v12.jsonl"),
            ("CURATION", "pending_curation_context_merit_v12.jsonl"),
            ("REPORT", "report_context_merit_v12.json"),
        ):
    ...
    def test_future_context_tranches_are_not_historical_inputs(self) -> None:
        ...
        prior = manual_root / "context_merit_audit_v11" / "synthetic.jsonl"
        ...
        self.assertEqual(
            ...
            frozenset(f"context_merit_audit_v{i}" for i in range(1, 12)),
        )
    ...
    def test_decisions_and_curation(self) -> None:
        ...
        self.assertEqual(
            ...
            {"keep": 8, "drop": 8, "edit": 9})
        self.assertEqual(len(self.curation), 17)
        self.assertEqual(
    ...
    def test_report_projection_and_sealed_policy(self) -> None:
        ...
        self.assertEqual(report["schema"], "context-merit-audit-report-v12")
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 8, "edit": 9, "keep": 8})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 710)
        self.assertEqual(
            projection["output_sha256"],
            "d24e7218cefbbae9d5ed421bc5d38882a4d4ec9707c7d22764417ef5350e6e42")
        self.assertEqual(projection["reviewed_keep_fact_ids_preserved"], 8)
    ...
    def test_generator_is_deterministic_and_prior_is_frozen(self) -> None:
        ...
        prior_files = tuple(
            path for version in range(1, 12)
