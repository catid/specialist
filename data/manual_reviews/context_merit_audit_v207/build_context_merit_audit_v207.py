#!/usr/bin/env python3
"""Clarify BightBound's teaching priorities and intended fit."""
...
DATA = ROOT / "data"
V206_DIR = DATA / "manual_reviews/context_merit_audit_v206"
sys.path[:0] = [str(ROOT), str(V206_DIR)]
import build_context_merit_audit_v206 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v207.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v207.jsonl"
REPORT = OUT_DIR / "report_context_merit_v207.json"
REVIEWER = "codex-context-merit-audit-v207"
...
SPECS = (
    {
        ...
        "active_index": 193,
        "markers": (
            "Its teaching combines fundamental technique, including why a technique works rather than only what to do, with applications for play, interaction, connection, and shared emotional experiences.",
        ),
        "decision": "edit",
        ...
        "reason_code": "complete_teaching_priorities_with_fit_guidance",
        "reason": "The original answer captured technique and play but omitted the educator's emphasis on connection and its explicit fit guidance. The replacement makes both the teaching philosophy and resource-selection boundary clear.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v206",
    "bightbound_teaching_rows_selected": 1,
    "direct_rows_without_prior_curation": 37,
    "rows": 505,
    "sha256": "3a6ea573a0fdeaf17053eecdc356e3f0fd44152d643b882af585e60861fa48d3",
}
EXPECTED_OUTPUT_SHA256 = "cc71d851d838fbb2bf1e20e4c464c7ec576f5adc752ef1ae62e4c69a5c8cad74"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v207-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v206-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v206.jsonl"
        baseline_report = directory / "v206.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v206 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v207 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "bightbound_teaching_fit_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v207",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v207 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v207 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v206": 467,
            ...
            "projection_strategy": "delegate to the hermetic v206 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v207",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated teaching-emphasis row whose fully inspected FAQ also gives connection goals and fit guidance",
                "score": "manual source support, teaching-philosophy clarity, resource-fit usefulness, and training merit",
