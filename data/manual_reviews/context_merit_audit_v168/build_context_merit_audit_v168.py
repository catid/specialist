#!/usr/bin/env python3
"""Replace a Hempex name lookup with its water-bondage tradeoffs."""
...
DATA = ROOT / "data"
V167_DIR = DATA / "manual_reviews/context_merit_audit_v167"
sys.path[:0] = [str(ROOT), str(V167_DIR)]
import build_context_merit_audit_v167 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v168.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v168.jsonl"
REPORT = OUT_DIR / "report_context_merit_v168.json"
REVIEWER = "codex-context-merit-audit-v168"
...
SPECS = (
    {
        ...
        "active_index": 425,
        "marker": "Hempex – Strong like nylon, with the look of hemp and a somewhat abrasive texture. Hempex floats in water and doesn’t shrink when wet which makes it a very interesting rope for water bondage.",
        "decision": "edit",
        "question": "Why does Rope365 describe Hempex as interesting for water bondage?",
        "answer": "It is strong like nylon, floats, and does not shrink when wet.",
        "reason_code": "replace_hempex_name_lookup_with_water_properties",
        "reason": "The replacement teaches the material properties that motivate the use case instead of asking the model to identify a name from clues.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v167",
    "direct_rows_without_prior_curation": 76,
    "rope365_hempex_water_properties_rows_selected": 1,
    "rows": 507,
    "sha256": "dfe5673681717a69910dab7d0e4e90df206f7a457c8e91d622aca2b41afc4328",
}
EXPECTED_OUTPUT_SHA256 = "0bd86f0a8e028e6b057a13d19052214bcf00f37c0b60a9f0e164ac3a0ff36f7b"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v168-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v167-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v167.jsonl"
        baseline_report = directory / "v167.report.json"
        ...
        if len(rows) != 507 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v167 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v168 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "rope365_hempex_water_properties_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v168",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 507 or observed["eval_fact_count"] != 612:
        raise ValueError("v168 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v168 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v167": 469,
            ...
            "projection_strategy": "delegate to the hermetic v167 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v168",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the Hempex name lookup whose fully read source supports a practical material/use-case explanation",
                "score": "manual source support, material usefulness, causal clarity, and training merit",
