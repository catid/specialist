#!/usr/bin/env python3
"""Audit the remaining direct Rope365 quick-release guidance."""
...
DATA = ROOT / "data"
V105_DIR = DATA / "manual_reviews/context_merit_audit_v105"
sys.path[:0] = [str(ROOT), str(V105_DIR)]
import build_context_merit_audit_v105 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v106.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v106.jsonl"
REPORT = OUT_DIR / "report_context_merit_v106.json"
REVIEWER = "codex-context-merit-audit-v106"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__125a6aa4fb7e0a18174d.json"

SPECS = (
    {
        "fact_id": "fact-c6715c55bd2f15780891",
        "active_index": 421,
        "marker": "- All cuffs are located in a safe place on the body, away from joints and nerves.",
        "decision": "edit",
        "question": "What placement rule does Rope365 give for cuffs used with quick releases?",
        "answer": "Place every cuff in a safe area away from joints and nerves.",
        "reason_code": "complete_quick_release_cuff_placement",
        "reason": "The revision turns a vague location question and fragment answer into a direct, self-contained safety rule supported by the checklist.",
    },
    {
        "fact_id": "fact-e4607b09df701ffca88e",
        "active_index": 51,
        "marker": "A quick-release is a way to create a temporary lock that is very fast to tie and untie. They may not be as solid as other locking methods, but they are great in case of emergency, or for ties that may need adjustments along the way. We just need to be extra careful in case it unties accidentally with movement.",
        "decision": "drop",
        "reason_code": "redundant_quick_release_use_subset",
        "reason": "This narrow use-case lookup is a semantic subset of active fact-c6b465dc3f0061f8a43b, which preserves the same benefits together with the important accidental-untying caution.",
    },
    {
        "fact_id": "fact-a3fb0bf625fecebfa5e0",
        "active_index": 287,
        "marker": "A slipped half hitch is made by not pulling the rope completely through and tightening the hitch. You’ll need to start from a change of direction or some friction on the hitch’s origin as the entry point will not be locked. Then we make a half hitch on the loop created by the first half hitch to lock the exit into place.",
        "decision": "drop",
        "reason_code": "redundant_slipped_hitch_exit_subset",
        "reason": "This narrow exit-lock lookup is a semantic subset of active fact-ada06c08d91a1d39a9a3, which explains both the unlocked entry and the half-hitch-locked exit.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v105",
    "direct_rows_without_prior_curation": 99,
    "rope365_quick_release_rows_selected": 3,
    "rows": 532,
    "sha256": "eb0fdac4e579ea5095ea94628f7ffb7fa8b56e0af86ad08aed356650165db123",
}
EXPECTED_OUTPUT_SHA256 = "1bd01b48ca4b191c42f8a7e8f666fc133ade0eef6611527e78202fdadf94f0a9"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v106-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v105-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v105.jsonl"
        baseline_report = directory / "v105.report.json"
        ...
        if len(rows) != 532 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v105 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v106 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: quick-release source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_quick_release_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v106",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 530 or observed["eval_fact_count"] != 612:
        raise ValueError("v106 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v106 output hash drift")
    report = {
        "active_baseline": {
        ...
        "audit": {
            "by_decision": {"drop": 2, "edit": 1, "keep": 0},
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v105": 494,
            "active_after_this_tranche": 492,
        ...
        "schema": "context-merit-audit-report-v106",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "all remaining direct Q&A from the fully read Rope365 quick-release page",
                "score": "manual source support, semantic redundancy, safety completeness, answer context, and practical merit review",
