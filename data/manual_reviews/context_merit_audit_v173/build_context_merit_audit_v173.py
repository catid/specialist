#!/usr/bin/env python3
"""Turn the abbreviated Rope365 aftercare lookup into actionable guidance."""
...
DATA = ROOT / "data"
V172_DIR = DATA / "manual_reviews/context_merit_audit_v172"
sys.path[:0] = [str(ROOT), str(V172_DIR)]
import build_context_merit_audit_v172 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v173.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v173.jsonl"
REPORT = OUT_DIR / "report_context_merit_v173.json"
REVIEWER = "codex-context-merit-audit-v173"
...
SPECS = (
    {
        ...
        "active_index": 357,
        "marker": "Once all the ropes are off, the session is not necessarily over.",
        "decision": "edit",
        "question": "How should rope partners plan aftercare according to Rope365?",
        "answer": "Discuss aftercare beforehand, when emotions are not high, then adapt it to the circumstances and each person’s needs.",
        "reason_code": "replace_aftercare_timing_lookup_with_planning_guidance",
        "reason": "The replacement preserves the timing rationale while adding the source’s important instruction to adapt aftercare to circumstances and individual needs.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v172",
    "direct_rows_without_prior_curation": 71,
    "rope365_aftercare_rows_selected": 1,
    "rows": 506,
    "sha256": "27e9c5b7d4951637c80be7d9f935741951ad167da1754e34b39dc6c8945895b8",
}
EXPECTED_OUTPUT_SHA256 = "d6e8b304147913c077dbe6e7db3462b64753fbbb8ce631313960f30fdb64adf6"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v173-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v172-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v172.jsonl"
        baseline_report = directory / "v172.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v172 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v173 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Rope365 communication source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_aftercare_planning_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v173",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v173 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v173 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v172": 468,
            ...
            "projection_strategy": "delegate to the hermetic v172 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v173",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the abbreviated aftercare-timing Q&A whose fully read source supports a complete planning rule",
                "score": "manual source support, practical usefulness, completeness, and training merit",
