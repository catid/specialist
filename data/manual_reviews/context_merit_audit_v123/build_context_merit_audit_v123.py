#!/usr/bin/env python3
"""Remove a low-merit Rope365 vocabulary-history flourish."""
...
DATA = ROOT / "data"
V122_DIR = DATA / "manual_reviews/context_merit_audit_v122"
sys.path[:0] = [str(ROOT), str(V122_DIR)]
import build_context_merit_audit_v122 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v123.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v123.jsonl"
REPORT = OUT_DIR / "report_context_merit_v123.json"
REVIEWER = "codex-context-merit-audit-v123"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__a104caf6faf57e072d83.json"

SPECS = (
    {
        "fact_id": "fact-1ee0bb12c5495c72bb70",
        "active_index": 320,
        "marker": "Rope has been around for a while, from sailing to bondage with a detour via macrame, this creates a huge legacy on the vocabulary.",
        "decision": "drop",
        "reason_code": "low_merit_vocabulary_history_flourish",
        "reason": "This tests a colorful sailing-to-macrame phrase rather than a rope term, definition, technique, resource, or safety concept.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v122",
    "direct_rows_without_prior_curation": 92,
    "rope365_starter_history_rows_selected": 1,
    "rows": 521,
    "sha256": "b189579860bdf2ec3be95edbd5d819e029d0af5b8dbb4e1f718e6178339684de",
}
EXPECTED_OUTPUT_SHA256 = "dfd5f6ee141ff1ac0daff204693a65bdfd44a656a7f4b2d2c0e19c1206a9e62e"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v123-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v122-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v122.jsonl"
        baseline_report = directory / "v122.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 521 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v122 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v123 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: starter source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_starter_history_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v123",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 520 or observed["eval_fact_count"] != 612:
        raise ValueError("v123 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v123 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v122": 483,
            "active_after_this_tranche": 482,
            ...
            "projection_strategy": "delegate to the v122 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v123",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the not-yet-context-audited historical-flourish Q&A from the fully read Rope365 starter page",
                "score": "manual source support, isolated-trivia risk, transferable value, and training merit",
