#!/usr/bin/env python3
"""Audit the remaining direct Anatomie rope-safeword rows."""
...
DATA = ROOT / "data"
V99_DIR = DATA / "manual_reviews/context_merit_audit_v99"
sys.path[:0] = [str(ROOT), str(V99_DIR)]
import build_context_merit_audit_v99 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v100.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v100.jsonl"
REPORT = OUT_DIR / "report_context_merit_v100.json"
REVIEWER = "codex-context-merit-audit-v100"
...
SPECS = (
    {
        ...
        "active_index": 279,
        "marker": "they need to be combined with more precise language relating to body parts, sensations and urgency.",
        "decision": "keep",
        "reason_code": "retain_precise_safeword_context_guidance",
        "reason": "The existing Q&A clearly preserves the source's recommendation to pair safewords with actionable location, sensation, and urgency information.",
    },
    {
        ...
        "decision": "keep",
        "reason_code": "retain_safeword_meaning_agreement",
        "reason": "The existing Q&A concisely preserves the source's pre-play agreement requirement.",
    },
    {
        "fact_id": "fact-97d52aa2d87dadfd8226",
        "active_index": 378,
        "marker": "In this case the immediate responsibility will fall to the rigger to check in.",
        "decision": "edit",
        ...
        "reason_code": "restore_joint_context_to_nonverbal_check_in",
        "reason": "The revision preserves the rigger's immediate check-in duty while restoring the source's surrounding joint-responsibility context.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v99",
    "direct_rows_without_prior_curation": 105,
    "anatomiestudio_safeword_rows_selected": 3,
    "rows": 532,
    "sha256": "6124678ce2039db8f615d751410c5dcc514e0ce311804705d5cc720e339a639e",
}
EXPECTED_OUTPUT_SHA256 = "de879de32679e1036ea8c811ba373340939c07c87f349eb9e9a984381714b860"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v100-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v99-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v99.jsonl"
        baseline_report = directory / "v99.report.json"
        ...
        if len(rows) != 532 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v99 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v100 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Anatomie safeword source lineage drift")
        audit = {
            ...
            "review_pass": "anatomiestudio_safeword_context_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v100",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 532 or observed["eval_fact_count"] != 612:
        raise ValueError("v100 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v100 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v99": 494,
        ...
        "schema": "context-merit-audit-report-v100",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the three remaining direct Q&A from the fully read Anatomie rope-safeword article",
                "score": "manual source support, communication actionability, responsibility context, and practical merit review",
