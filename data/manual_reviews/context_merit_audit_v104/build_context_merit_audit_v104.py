#!/usr/bin/env python3
"""Complete the Rope365 kimono-tie finger-safety guidance."""
...
DATA = ROOT / "data"
V103_DIR = DATA / "manual_reviews/context_merit_audit_v103"
sys.path[:0] = [str(ROOT), str(V103_DIR)]
import build_context_merit_audit_v103 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v104.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v104.jsonl"
REPORT = OUT_DIR / "report_context_merit_v104.json"
REVIEWER = "codex-context-merit-audit-v104"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__2207da311f41933620c3.json"

SPECS = (
    {
        "fact_id": "fact-38de97660f344b03efb8",
        "active_index": 167,
        "marker": "If we make sure not to trap the fingers against the forearms, the base of the tie is low risks but remains quite escapable without expanding it with wraps on the chest or on the arms.",
        "decision": "edit",
        "question": "How should the fingers be positioned in Rope365's base kimono tie, and why?",
        "answer": "Do not trap the fingers against the forearms; keeping them free helps the low-risk base remain escapable.",
        "reason_code": "complete_kimono_finger_guidance",
        "reason": "The revision replaces a bare body-part lookup with the supported placement instruction and its practical escapability context.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v103",
    "direct_rows_without_prior_curation": 101,
    "rope365_kimono_tie_rows_selected": 1,
    "rows": 532,
    "sha256": "2fa1f56060e9a6a987c35590a68afad699cc08d5bdf4475b19f43f72d9d147b4",
}
EXPECTED_OUTPUT_SHA256 = "693ee262007fba339cd1145b0796b74a0ef4d88d8bc7a93916a1468afecb8138"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v104-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v103-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v103.jsonl"
        baseline_report = directory / "v103.report.json"
        ...
        if len(rows) != 532 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v103 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v104 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: kimono-tie source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_kimono_tie_safety_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v104",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 532 or observed["eval_fact_count"] != 612:
        raise ValueError("v104 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v104 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v103": 494,
        ...
        "schema": "context-merit-audit-report-v104",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the direct finger-safety Q&A from the fully read Rope365 kimono-tie page",
                "score": "manual source support, finger placement completeness, escapability context, and practical merit review",
