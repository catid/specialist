#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v51."""
...
import build_context_merit_audit_v51 as builder
from qa_quality import normalize_text


class ContextMeritAuditV51Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v51-", dir=HERE)
    ...
    def setUp(self) -> None:
        ...
        temp = tempfile.TemporaryDirectory(prefix=".run-v51-", dir=HERE)
    ...
    def test_baseline_is_exact_v50_projection(self) -> None:
    ...
    def test_selection_is_exact_deterministic_top_ten(self) -> None:
        ...
        self.assertEqual(len(ranked), 56)
    ...
    def test_sources_and_evidence_are_hash_pinned(self) -> None:
        ...
        validator = builder.previous.previous.previous.source_evidence
    ...
    def test_decisions_and_curation_are_exact(self) -> None:
        ...
        self.assertEqual({d: sum(r["decision"] == d for r in self.audit)
                         ...
                         {"keep": 8, "drop": 0, "edit": 2})
        self.assertEqual(len(self.curation), 2)
        self.assertTrue(all(r["action"] == "edit" for r in self.curation))

    def test_storage_paraphrase_is_documented(self) -> None:
        audit = next(r for r in self.audit
                     if r["fact_id"] == "fact-94b42b931282e211349e")
        curation = next(r for r in self.curation
                        if r["fact_id"] == audit["fact_id"])
        self.assertEqual(audit["source_support"], "manual_paraphrase")
        self.assertTrue(audit["paraphrase_rationale"])
        self.assertEqual(curation["support_type"], "manual_paraphrase")
        self.assertEqual(curation["paraphrase_rationale"],
                         audit["paraphrase_rationale"])
    ...
    def test_report_counts_projection_and_sealed_policy(self) -> None:
        ...
        self.assertEqual(report["schema"], "context-merit-audit-report-v51")
        self.assertEqual(report["audit"]["by_decision"], {"edit": 2, "keep": 8})
        ...
        self.assertEqual((projection["output_rows"], projection["output_sha256"]),
                         (546, "48fbff4e40596e9ce544196db7dde7301acfb592ab63a1112997f4a5132ff56a"))
    ...
    def test_future_versions_cannot_enter_v51(self) -> None:
        self.assertEqual((len(builder.CONTEXT_CURATIONS),
                          len(builder.CONTEXT_AUDITS)), (50, 50))
        self.assertIn("context_merit_audit_v50", str(builder.CONTEXT_CURATIONS[-1]))
        self.assertFalse(any("context_merit_audit_v51" in str(p)
                             for p in (*builder.CONTEXT_CURATIONS,
                                       *builder.CONTEXT_AUDITS)))


class IsolatedProjectionV51Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temp = tempfile.TemporaryDirectory(prefix=".projection-v51-", dir=HERE)
        out = Path(cls.temp.name)
    ...
    def test_projection_is_byte_identical_and_hash_pinned(self) -> None:
        self.assertEqual([hashlib.sha256(p.read_bytes()).hexdigest()
                         ...
                         ["48fbff4e40596e9ce544196db7dde7301acfb592ab63a1112997f4a5132ff56a"] * 2)
        self.assertEqual((len(self.rows), self.report["eval_fact_count"]), (546, 612))
    ...
    def test_projection_applies_exact_decisions_and_upstream_repairs(self) -> None:
        ...
        for spec in builder.SPECS:
            if spec["decision"] == "keep":
                ...
                self.assertEqual(by_original[spec["fact_id"]]["question"],
                                 spec["question"])
    ...
    def test_projection_has_unique_ids_and_questions(self) -> None:
        self.assertEqual(len({r["fact_id"] for r in self.rows}), 546)
        self.assertEqual(len({r["question"] for r in self.rows}), 546)
