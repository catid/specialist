#!/usr/bin/env python3
"""Replace a kindness slogan with the source's non-transactional principle."""
...
DATA = ROOT / "data"
V156_DIR = DATA / "manual_reviews/context_merit_audit_v156"
sys.path[:0] = [str(ROOT), str(V156_DIR)]
import build_context_merit_audit_v156 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v157.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v157.jsonl"
REPORT = OUT_DIR / "report_context_merit_v157.json"
REVIEWER = "codex-context-merit-audit-v157"
...
SPECS = (
    {
        ...
        "active_index": 30,
        "marker": "It is about what you can give, without expectation or requirement of reciprocity.",
        "decision": "edit",
        "question": "How does “Rope and Kindness” distinguish kindness from a transaction?",
        "answer": "Kindness is giving without expecting or requiring reciprocity, rather than acting for what one can get.",
        "reason_code": "replace_kindness_slogan_with_nontransactional_principle",
        "reason": "The replacement turns a fragmentary slogan into the source’s clearer principle about giving without expected return.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v156",
    "direct_rows_without_prior_curation": 86,
    "kinbakutoday_nontransactional_kindness_rows_selected": 1,
    "rows": 508,
    "sha256": "ff5ad59f6f0b6d5c7e4ebe01ecdf7d76256b05f22d35ff8529bb83aacd084f95",
}
EXPECTED_OUTPUT_SHA256 = "811f7de96e4c07279764081cfae855a8f23a6a08f98124ed069d33ca9de3699f"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v157-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v156-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v156.jsonl"
        baseline_report = directory / "v156.report.json"
        ...
        if len(rows) != 508 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v156 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v157 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "kinbakutoday_nontransactional_kindness_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v157",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 508 or observed["eval_fact_count"] != 612:
        raise ValueError("v157 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v157 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v156": 470,
            ...
            "projection_strategy": "delegate to the hermetic v156 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v157",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the kindness slogan whose fully read source supports a clearer non-transactional principle",
                "score": "manual source support, conceptual clarity, faithful synthesis, and training merit",
