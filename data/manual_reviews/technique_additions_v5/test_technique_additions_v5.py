...
HERE=Path(__file__).resolve().parent;sys.path.insert(0,str(HERE));import build_technique_additions_v5 as b
...
class Test(unittest.TestCase):
 ...
 def test_content(self):
  ...
  self.assertEqual(Counter(classify_stratum(r) for r in self.rows),Counter({"technique":3}))
 def test_report(self):self.assertEqual(self.report["new_independent_inputs"]["expected_strata"],{"technique":3})
