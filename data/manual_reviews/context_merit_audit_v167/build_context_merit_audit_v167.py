#!/usr/bin/env python3
"""Replace a cleanup instruction with the fall-risk mechanism."""
...
DATA = ROOT / "data"
V166_DIR = DATA / "manual_reviews/context_merit_audit_v166"
sys.path[:0] = [str(ROOT), str(V166_DIR)]
import build_context_merit_audit_v166 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v167.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v167.jsonl"
REPORT = OUT_DIR / "report_context_merit_v167.json"
REVIEWER = "codex-context-merit-audit-v167"
...
SPECS = (
    {
        ...
        "active_index": 81,
        "marker": "When the hands are tied, one may no longer be able to protect themselves in case of a fall. The person tying can also fall from tripping on an obstacle.",
        "decision": "edit",
        "question": "Why does Rope365 advise clearing tripping hazards before rope play?",
        "answer": "A bound person may be unable to protect themselves in a fall, and the person tying can also trip while trying to help.",
        "reason_code": "replace_cleanup_instruction_with_fall_risk_mechanism",
        "reason": "The replacement explains why clearing the space matters instead of training the tautological answer “clean up.”",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v166",
    "direct_rows_without_prior_curation": 77,
    "rope365_fall_hazard_rows_selected": 1,
    "rows": 507,
    "sha256": "e378a4846dc2cc4794cc7d43d76a50ef673535804ab749320f0e2b1270b5d2ac",
}
EXPECTED_OUTPUT_SHA256 = "dfe5673681717a69910dab7d0e4e90df206f7a457c8e91d622aca2b41afc4328"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v167-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v166-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v166.jsonl"
        baseline_report = directory / "v166.report.json"
        ...
        if len(rows) != 507 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v166 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v167 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "rope365_fall_hazard_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v167",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 507 or observed["eval_fact_count"] != 612:
        raise ValueError("v167 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v167 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v166": 469,
            ...
            "projection_strategy": "delegate to the hermetic v166 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v167",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the cleanup instruction whose fully read source supports a clearer fall-risk mechanism",
