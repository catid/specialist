...
import build_context_merit_audit_v446 as b


class ContextMeritV446(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        b.main()
        cls.tmp = tempfile.TemporaryDirectory(prefix=".test-v446-", dir=HERE)
    ...
    def test_exact_decisions_and_515_row_projection(self):
        self.assertEqual(
            (len(self.base_rows), b.file_sha256(self.base), len(self.rows), b.file_sha256(self.out)),
            (518, b.BASELINE_SHA256, 515, b.EXPECTED_OUTPUT_SHA256),
        )
    ...
    def test_audit_scope_and_review_classes_are_exact(self):
        ...
        self.assertEqual(Counter(row["review_class"] for row in self.audits), Counter({
            "documented_contribution_edit": 3,
            "terminology_edit": 1,
            "terminology_keep": 1,
            "mere_chronology_drop": 1,
            "historical_context_edit": 1,
            "redundant_role_list_drop": 1,
            "lineage_relationship_edit": 1,
            "archive_identity_trivia_drop": 1,
        }))
    ...
    def test_redundant_drops_have_pinned_substantive_survivors(self):
        output = {row["fact_id"]: row for row in self.rows}
        self.assertEqual(self.report["redundancy_quarantine"]["dropped_to_surviving_fact"], {
            "fact-558694f6a7e4f740243d": "fact-8bc60f112e90fb781203",
            "fact-b34d490cfff5f65c5e81": "fact-d9cd6ee0dfa1a1dd55da",
        })
    ...
    def test_owner_resources_and_all_requested_urls_survive(self):
    ...
    def test_two_layer_contract_and_v52_isolation(self):
        ...
        self.assertEqual(self.report["source_snapshot_inventory"]["total_characters"], 82494)
