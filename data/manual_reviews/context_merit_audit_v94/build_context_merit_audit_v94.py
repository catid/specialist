#!/usr/bin/env python3
"""Audit the remaining direct Kinbaku Today rope-bottom guide rows."""
...
DATA = ROOT / "data"
V93_DIR = DATA / "manual_reviews/context_merit_audit_v93"
sys.path[:0] = [str(ROOT), str(V93_DIR)]
import build_context_merit_audit_v93 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v94.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v94.jsonl"
REPORT = OUT_DIR / "report_context_merit_v94.json"
REVIEWER = "codex-context-merit-audit-v94"
...
SOURCE = DATA / "raw/kinbakutoday_432c8adfc1abe686.json"
...
SPECS = (
    {
        ...
        "decision": "keep",
        "reason_code": "retain_immediate_tingling_communication",
        "reason": "The existing Q&A directly preserves an urgent, actionable communication step for possible circulation or nerve symptoms.",
    },
    {
        "fact_id": "fact-7021b57b2f77bed4d7db",
        "active_index": 338,
        "marker": "For instance EMT Shears or rescue hooks etc.",
        "decision": "edit",
        "question": "What emergency rope-cutting equipment does Clover's guide say a rigger should have?",
        "answer": "EMT shears or rescue hooks.",
        "reason_code": "complete_and_attribute_emergency_cutting_equipment",
        "reason": "The revision attributes the recommendation, asks about the equipment's purpose, and preserves both source examples instead of one bare product name.",
    },
    {
        "fact_id": "fact-597fa7fd78d5fdcb4f10",
        "active_index": 390,
        "marker": "It is your responsibility to communicate and give feedback before, during and after bondage.",
        "decision": "keep",
        "reason_code": "retain_feedback_timing_guidance",
        "reason": "The existing Q&A concisely preserves the guide's clear before, during, and after feedback timeline.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v93",
    "direct_rows_without_prior_curation": 110,
    "kinbakutoday_rope_bottom_guide_rows_selected": 3,
    "rows": 533,
    "sha256": "0f7b44bafa93d37fa69f7bf44a79f68dcf426c05db6db335572e3e5cf080c2da",
}
EXPECTED_OUTPUT_SHA256 = "bf2590275d3960cc97aaf4a14fb55f181b6ef0739bbd2915b42601affc64c73d"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v94-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v93-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v93.jsonl"
        baseline_report = directory / "v93.report.json"
        ...
        if len(rows) != 533 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v93 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v94 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: rope-bottom-guide source lineage drift")
        audit = {
            ...
            "review_pass": "kinbakutoday_rope_bottom_guide_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v94",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 533 or observed["eval_fact_count"] != 612:
        raise ValueError("v94 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v94 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v93": 495,
        ...
        "schema": "context-merit-audit-report-v94",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the three remaining direct Q&A from the fully read Kinbaku Today rope-bottom guide",
                "score": "manual source support, attribution, emergency readiness, communication safety, and practical merit review",
