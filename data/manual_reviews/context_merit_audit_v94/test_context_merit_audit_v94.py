#!/usr/bin/env python3
"""Focused regression tests for Kinbaku Today rope-bottom guide audit v94."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v94 as b


class V94(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v94-", dir=HERE)
    ...
    def test_04_direct(self):
        self.assertEqual(sum(not row.get("curation") for row in self.baseline), 110)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("kinbakutoday_rope_bottom_guide_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v94", {"drop": 0, "edit": 1, "keep": 2}, b.EXPECTED_OUTPUT_SHA256),
        )
