#!/usr/bin/env python3
"""Expand Shibari Study's general-safety row with its practical risk scope."""
...
DATA = ROOT / "data"
V210_DIR = DATA / "manual_reviews/context_merit_audit_v210"
sys.path[:0] = [str(ROOT), str(V210_DIR)]
import build_context_merit_audit_v210 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v211.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v211.jsonl"
REPORT = OUT_DIR / "report_context_merit_v211.json"
REVIEWER = "codex-context-merit-audit-v211"
...
SOURCE = DATA / "raw/shibari_study_general_rope_safety_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 432,
        "markers": (
            "Shibari Study describes this free course as focusing on bondage safety and the risks associated with shibari, including different rope types, anatomy, and communication.",
            "The page catalogs related topics including consent and negotiation, cutting tools, nerves, fainting, pain and personal risk profiles, psychological considerations, and trauma-release exercises.",
        ),
        "decision": "edit",
        ...
        "reason_code": "expand_general_safety_course_risk_scope",
        "reason": "The original answer named only three broad subjects. The replacement preserves them and adds the page's concrete safety and risk topics, producing a more useful description without turning the row into procedural advice.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v210",
    "direct_rows_without_prior_curation": 33,
    "shibari_study_general_safety_rows_selected": 1,
    "rows": 505,
    "sha256": "b6921c222438ac8bee4ac12697b28b6eec55a93158cf89434a803cb29d953a0b",
}
EXPECTED_OUTPUT_SHA256 = "ef40c995322a60dda5260eef02fae0ecf5996206c87c379754815d86a5435824"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v211-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v210-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v210.jsonl"
        baseline_report = directory / "v210.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v210 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v211 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Shibari Study safety source lineage drift")
        audit = {
            ...
            "review_pass": "shibari_study_general_safety_scope_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v211",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v211 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v211 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v210": 467,
            ...
            "projection_strategy": "delegate to the hermetic v210 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v211",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated general-safety row whose inspected page exposes a more concrete risk-topic catalog",
                "score": "manual source support, learner safety usefulness, topic completeness, and training merit",
