#!/usr/bin/env python3
"""Replace a historical-term lookup with a semantic-drift caution."""
...
DATA = ROOT / "data"
V274_DIR = DATA / "manual_reviews/context_merit_audit_v274"
sys.path[:0] = [str(ROOT), str(V274_DIR)]
import build_context_merit_audit_v274 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v275.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v275.jsonl"
REPORT = OUT_DIR / "report_context_merit_v275.json"
REVIEWER = "codex-context-merit-audit-v275"
...
SOURCE = DATA / "raw/kinbakutoday_5a6e15f57e52a345.json"

SPECS = (
    {
        "fact_id": "fact-33c2b72b6ae8e56fb1ad",
        "active_index": 21,
        "markers": (
            "the difference is dependent on the user’s age, on their knowledge of history, local culture, their own personal history, and even the context of conversation. If you ask ten people, you will get ten different answers.",
            "We should note that Ito Seiu used both words. For him, Shibari meant what I think Shibari or Kinbaku means nowadays. Whereas Kinbaku was the word that represented a kind of historical motive of Kabuki play for him.",
            "What I wanted to say was that the meaning of a word is always drifting and changing.",
        ),
        "decision": "edit",
        "question": "Why does Ugo caution against treating the meanings of kinbaku and shibari as fixed?",
        "answer": "He says their meanings vary with age, history, local culture, personal experience, and context, and have drifted over time; even Ito Seiu used the words differently around 1900.",
        "reason_code": "replace_historical_term_lookup_with_semantic_drift_caution",
        "reason": "The replacement trades a decontextualized historical-term lookup for the interview's explicit warning that these meanings vary across speakers, contexts, and eras.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v274",
    "direct_rows_without_prior_curation": 1,
    "historical_term_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "9511a62ecee21748edf5edfe07fbaff920213db4afb8e113e4fe17fb88770721",
}
EXPECTED_OUTPUT_SHA256 = "18fca73b8ceceac6b426c6eda578a23b8d4dbf85e8f1c52c8d7c66b9634674e4"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v275-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v274-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v274.jsonl"
        baseline_report = directory / "v274.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v274 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v275 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "semantic_drift_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v275",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v275 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v275 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v274": 454,
        ...
        "schema": "context-merit-audit-report-v275",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a historical-term lookup whose source passage supports a semantic-drift caution",
                "score": "source-critical and historical value, nuance, source support, question naturalness, and reduced terminology trivia",
