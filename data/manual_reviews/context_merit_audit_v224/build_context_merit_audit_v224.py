#!/usr/bin/env python3
"""Connect the BightBound link to its teaching approach and fit."""
...
DATA = ROOT / "data"
V223_DIR = DATA / "manual_reviews/context_merit_audit_v223"
sys.path[:0] = [str(ROOT), str(V223_DIR)]
import build_context_merit_audit_v223 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v224.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v224.jsonl"
REPORT = OUT_DIR / "report_context_merit_v224.json"
REVIEWER = "codex-context-merit-audit-v224"
...
SOURCE = DATA / "raw/owner_bightbound_resource_20260714.json"

SPECS = (
    {
        "fact_id": "fact-49a87d11b1076c90e07c",
        "active_index": 374,
        "markers": (
            "The supplied directory recommends BightBound as an Austin-area resource for intermediate and advanced rope education: https://www.bightbound.com/.",
            "BightBound's inspected FAQ says its teaching emphasizes fundamentals and the reasons behind them, then uses rope as play involving interaction, connection, and shared emotion.",
            "It describes its tying style as chaotic, fluid, sadistic, and abstract, and says it generally uses Western terminology while still teaching the function and history of techniques.",
        ),
        "decision": "edit",
        "question": "Where can someone find the owner-recommended Austin educators BightBound, and how does their FAQ describe the teaching approach?",
        "answer": "BightBound is at https://www.bightbound.com/. Its FAQ emphasizes fundamentals and why they work, followed by rope as interactive play centered on connection and shared emotion; it describes the style as chaotic, fluid, sadistic, and abstract.",
        "reason_code": "connect_bightbound_link_to_teaching_approach",
        "reason": "The original answer preserved only the educator link. The replacement keeps the exact owner-recommended URL and adds the inspected FAQ's teaching priorities and style so learners can judge fit.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v223",
    "direct_rows_without_prior_curation": 20,
    "bightbound_resource_approach_rows_selected": 1,
    "rows": 505,
    "sha256": "9640a3b2b2b0ff2b90e7e962bcc66a952ce6bdc6e23558eb4489caeed4b8df2c",
}
EXPECTED_OUTPUT_SHA256 = "5628c04732fca5c1decbe411246a2ae56e8115863b87f1281c264dc5fdf77c05"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v224-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v223-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v223.jsonl"
        baseline_report = directory / "v223.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v223 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v224 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "bightbound_resource_approach_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v224",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v224 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v224 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v223": 467,
            ...
            "projection_strategy": "delegate to the hermetic v223 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v224",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended BightBound link row whose inspected FAQ describes teaching priorities and style",
                "score": "owner-source support, learner-fit usefulness, teaching context, and URL preservation",
