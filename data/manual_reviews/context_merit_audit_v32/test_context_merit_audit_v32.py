#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v32."""
...
import build_context_merit_audit_v32 as builder
from qa_quality import normalize_text


class ContextMeritAuditV32Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline_temp = tempfile.TemporaryDirectory(
            prefix=".test-baseline-v32-", dir=HERE)
        output = Path(cls.baseline_temp.name)
        cls.baseline_path = output / "projection-v31.jsonl"
        cls.baseline_report_path = output / "projection-v31.report.json"
    ...
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v32-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v32.jsonl"),
            ("CURATION", "pending_curation_context_merit_v32.jsonl"),
            ("REPORT", "report_context_merit_v32.json"),
        ):
    ...
    def test_baseline_is_exact_v31_projection(self) -> None:
        self.assertEqual(len(self.baseline), 579)
    ...
    def test_selection_is_exact_deterministic_top_ten(self) -> None:
        ...
        self.assertEqual(len(ranked), 246)
        self.assertEqual(
    ...
    def test_projected_baseline_counts_are_pinned(self) -> None:
        ...
        reviewed = builder.prior_reviewed_fact_ids()
        self.assertEqual(len(direct), 336)
        self.assertEqual(sum(row["fact_id"] in reviewed for row in direct), 90)
    ...
    def test_sources_and_evidence_are_hash_pinned(self) -> None:
        ...
        for spec in builder.SPECS:
            ...
            self.assertEqual(support, "normalized_extractive")
    ...
    def test_decisions_and_curation_are_exact(self) -> None:
        ...
        self.assertEqual(
            ...
            {"keep": 2, "drop": 3, "edit": 5})
        self.assertEqual(len(self.curation), 8)
        self.assertEqual(
            {action: sum(row["action"] == action for row in self.curation)
             for action in ("drop", "edit")}, {"drop": 3, "edit": 5})

    def test_all_edits_are_extractive(self) -> None:
    ...
    def test_projection_lineage_is_complete(self) -> None:
        for row in self.audit:
            ...
            self.assertEqual(lineage["baseline_rows"], 579)
            ...
            self.assertEqual(row["review_pass"],
                             "first_context_merit_review_of_v31_projection_row")
    ...
    def test_report_counts_projection_and_sealed_policy(self) -> None:
        report = json.loads(builder.REPORT.read_text())
        self.assertEqual(report["schema"], "context-merit-audit-report-v32")
        self.assertEqual(report["audit"]["rows"], 10)
        self.assertEqual(report["selection"]["rows_selected"], 10)
        self.assertEqual(report["selection"]["eligible_unreviewed_rows"], 246)
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 3, "edit": 5, "keep": 2})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 576)
        self.assertEqual(
            projection["output_sha256"],
            "cb024742297927177f94e801363c78db7cc5bd7f7d2d9a6cdb376d299bf23c72")
        self.assertEqual(projection["reviewed_keep_fact_ids_preserved"], 2)
    ...
    def test_future_versions_cannot_enter_v32_by_directory_discovery(self) -> None:
        self.assertEqual(len(builder.CONTEXT_CURATIONS), 31)
        self.assertEqual(len(builder.CONTEXT_AUDITS), 31)
        self.assertIn("context_merit_audit_v31",
                      str(builder.CONTEXT_CURATIONS[-1]))
        self.assertIn("context_merit_audit_v31",
                      str(builder.CONTEXT_AUDITS[-1]))
        self.assertFalse(any("context_merit_audit_v32" in str(path) or
                             "context_merit_audit_v33" in str(path)
                             for path in (*builder.CONTEXT_CURATIONS,
                                          *builder.CONTEXT_AUDITS)))


class IsolatedProjectionV32Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory(
            prefix=".test-projection-v32-", dir=HERE)
    ...
    def test_projection_is_byte_identical_and_hash_pinned(self) -> None:
        ...
        self.assertEqual(len(self.rows), 576)
        self.assertEqual(self.report["eval_fact_count"], 612)

    def test_projection_applies_exact_v32_decisions(self) -> None:
    ...
    def test_projection_has_unique_ids_and_questions(self) -> None:
        self.assertEqual(len({row["fact_id"] for row in self.rows}), 576)
        self.assertEqual(len({row["question"] for row in self.rows}), 576)
