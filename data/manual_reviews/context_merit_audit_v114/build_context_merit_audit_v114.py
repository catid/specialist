#!/usr/bin/env python3
"""Audit Rope365's tibia-pressure warning for frog ties."""
...
DATA = ROOT / "data"
V113_DIR = DATA / "manual_reviews/context_merit_audit_v113"
sys.path[:0] = [str(ROOT), str(V113_DIR)]
import build_context_merit_audit_v113 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v114.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v114.jsonl"
REPORT = OUT_DIR / "report_context_merit_v114.json"
REVIEWER = "codex-context-merit-audit-v114"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__e1b4d511580682c46d2d.json"

SPECS = (
    {
        "fact_id": "fact-ac99053d3a764cc0c291",
        "active_index": 427,
        "marker": "- Are there any painful sensations? In particular from overextension at the knee or on the front of the tibia bone.",
        "decision": "keep",
        "reason_code": "clear_frog_tie_tibia_warning",
        "reason": "The answer accurately identifies the front of the tibia as the rope-placement pain area named in the source's self-evaluation checklist.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v113",
    "direct_rows_without_prior_curation": 92,
    "rope365_frog_starting_point_rows_selected": 1,
    "rows": 529,
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v114-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v113-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v113.jsonl"
        baseline_report = directory / "v113.report.json"
        ...
        if len(rows) != 529 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v113 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v114 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: frog-starting-point source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_frog_starting_point_safety_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v114",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 529 or observed["eval_fact_count"] != 612:
        raise ValueError("v114 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v114 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v113": 491,
        ...
        "schema": "context-merit-audit-report-v114",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the not-yet-context-audited tibia-pressure Q&A from the fully read Rope365 frog-tie starting-points page",
                "score": "manual source support, pain-location completeness, safety actionability, and practical merit review",
