#!/usr/bin/env python3
"""Focused regression tests for Rope365 frog-tie tibia safety audit v114."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v114 as b


class V114(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v114-", dir=HERE)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("rope365_frog_starting_point_safety_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_14_pins(self):
        ...
        self.assertEqual(len(pins), 339)
        for row in pins:
            self.assertEqual(b.file_sha256(b.ROOT / row["path"]), row["sha256"])

    def test_15_action(self):
        projected = {row["fact_id"]: row for row in self.rows}
        self.assertEqual(set(projected) & set(b.EXPECTED_SELECTION), set(b.EXPECTED_SELECTION))
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v114", {"drop": 0, "edit": 0, "keep": 1}, b.EXPECTED_OUTPUT_SHA256),
        )
