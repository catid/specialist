#!/usr/bin/env python3
"""Audit Rope365's exposed-nerve warning for capture ties."""
...
DATA = ROOT / "data"
V112_DIR = DATA / "manual_reviews/context_merit_audit_v112"
sys.path[:0] = [str(ROOT), str(V112_DIR)]
import build_context_merit_audit_v112 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v113.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v113.jsonl"
REPORT = OUT_DIR / "report_context_merit_v113.json"
REVIEWER = "codex-context-merit-audit-v113"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__11aac16e1df1419f3e2c.json"

SPECS = (
    {
        "fact_id": "fact-36db6917e8a0f04f49e1",
        "active_index": 423,
        "marker": "Be especially mindful of exposed nerves around the wrists and on the upper arms as you explore this activity.",
        "decision": "keep",
        "reason_code": "clear_capture_tie_nerve_locations",
        "reason": "The answer accurately preserves both high-risk areas named in the source's warning: around the wrists and on the upper arms.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v112",
    "direct_rows_without_prior_curation": 92,
    "rope365_hojojutsu_safety_rows_selected": 1,
    "rows": 529,
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v113-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v112-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v112.jsonl"
        baseline_report = directory / "v112.report.json"
        ...
        if len(rows) != 529 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v112 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v113 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: hojōjutsu source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_hojojutsu_safety_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v113",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 529 or observed["eval_fact_count"] != 612:
        raise ValueError("v113 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v113 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v112": 491,
        ...
        "schema": "context-merit-audit-report-v113",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the not-yet-context-audited exposed-nerve Q&A from the fully read Rope365 hojōjutsu page",
                "score": "manual source support, nerve-location completeness, safety actionability, and practical merit review",
