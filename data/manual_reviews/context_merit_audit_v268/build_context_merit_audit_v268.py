#!/usr/bin/env python3
"""Replace a vague term-usage claim with a peer-learning history lesson."""
...
DATA = ROOT / "data"
V267_DIR = DATA / "manual_reviews/context_merit_audit_v267"
sys.path[:0] = [str(ROOT), str(V267_DIR)]
import build_context_merit_audit_v267 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v268.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v268.jsonl"
REPORT = OUT_DIR / "report_context_merit_v268.json"
REVIEWER = "codex-context-merit-audit-v268"
...
SPECS = (
    {
        "fact_id": "fact-6068b116f45c2417555c",
        "active_index": 17,
        "markers": (
            "Once formed, positive feedback instantly evolved the skills of the participants, leading to the explosion in the diversity of Kinbaku in Japan.",
        ...
        "decision": "edit",
        "question": "How does Ugo say Nawakai networks accelerated skill and diversity in Japanese kinbaku?",
        "answer": "Skilled amateurs circulated among groups, exchanged tips freely, influenced one another, and evolved personal styles; the resulting positive feedback raised both skill and diversity.",
        "reason_code": "replace_vague_term_usage_claim_with_peer_network_history",
        "reason": "The replacement trades a vague claim about why a title spread slowly for the source's concrete account of how peer networks and open exchange accelerated learning and stylistic diversity.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v267",
    "direct_rows_without_prior_curation": 1,
    "vague_term_usage_rows_selected": 1,
    "rows": 492,
    "sha256": "eba3ec922055421993cf42099df4f994a58cef2795311101ee0d9dbd2f691cbb",
}
EXPECTED_OUTPUT_SHA256 = "f702b13fb5f3bbe5175bb056d9fd7dd6a08ccd7f78be4dac9d0af949d7588119"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v268-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v267-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v267.jsonl"
        baseline_report = directory / "v267.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v267 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v268 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "peer_network_history_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v268",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v268 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v268 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v267": 454,
        ...
        "schema": "context-merit-audit-report-v268",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a vague term-usage claim whose source passage supports a concrete peer-learning history lesson",
                "score": "historical and learning value, source support, question naturalness, and reduced unsupported generalization",
