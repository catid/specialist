#!/usr/bin/env python3
"""Replace a low-merit Rope365 tie-name lookup with safety guidance."""
...
DATA = ROOT / "data"
V83_DIR = DATA / "manual_reviews/context_merit_audit_v83"
sys.path[:0] = [str(ROOT), str(V83_DIR)]
import build_context_merit_audit_v83 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v84.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v84.jsonl"
REPORT = OUT_DIR / "report_context_merit_v84.json"
REVIEWER = "codex-context-merit-audit-v84"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__edb82b981dca2042e06d.json"

SPECS = (
    {
        "fact_id": "fact-fc75e65fe01974dba7d2",
        "active_index": 269,
        "marker": "Warm-up each joint, especially the shoulder to allow for maximum range of motion and prevent injuries.",
        "decision": "edit",
        "question": "What preparation and adaptation does Rope365 recommend for hands-behind-back positions?",
        "answer": "Warm up each joint, especially the shoulders, and adapt the position to the person's flexibility.",
        "reason_code": "replace_bunny_ears_name_lookup_with_joint_safety",
        "reason": "The revision replaces multilingual tie-name trivia with the source's more useful joint warm-up and individual-flexibility guidance.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v83",
    "direct_rows_without_prior_curation": 122,
    "rope365_hands_behind_back_rows_selected": 1,
    "rows": 536,
    "sha256": "4958870ef39bf469130da258fe052730bf2c29081a53e609612d012b80ebf3b0",
}
EXPECTED_OUTPUT_SHA256 = "be0611d6c351e7d186b964017ef3da71bfacd608b1b877e35777a90e654420f5"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v84-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v83-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v83.jsonl"
        baseline_report = directory / "v83.report.json"
        ...
        if len(rows) != 536 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v83 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v84 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["document_sha256"] != document["document_sha256"]:
            raise ValueError(f"{spec['fact_id']}: hands-behind-back source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_hands_behind_back_safety_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v84",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 536 or observed["eval_fact_count"] != 612:
        raise ValueError("v84 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v84 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v83": 498,
        ...
        "schema": "context-merit-audit-report-v84",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the remaining direct Q&A from the fully read Rope365 hands-behind-back page",
                "score": "manual source support, safety value, answer grammar, and practical merit review",
