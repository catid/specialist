#!/usr/bin/env python3
"""Focused regression tests for Rope365 hands-behind-back safety audit v84."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v84 as b


class V84(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v84-", dir=HERE)
    ...
    def test_04_direct(self):
        self.assertEqual(sum(not row.get("curation") for row in self.baseline), 122)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("rope365_hands_behind_back_safety_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v84", {"edit": 1, "keep": 0}, b.EXPECTED_OUTPUT_SHA256),
        )
