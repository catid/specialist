#!/usr/bin/env python3
"""Complete Rope365's functional definition of a single-column tie."""
...
DATA = ROOT / "data"
V175_DIR = DATA / "manual_reviews/context_merit_audit_v175"
sys.path[:0] = [str(ROOT), str(V175_DIR)]
import build_context_merit_audit_v175 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v176.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v176.jsonl"
REPORT = OUT_DIR / "report_context_merit_v176.json"
REVIEWER = "codex-context-merit-audit-v176"
...
SPECS = (
    {
        ...
        "active_index": 17,
        "markers": (
            "A single column tie is a cuff that will not tighten when we pull on the tail.",
        ),
        "decision": "edit",
        "question": "What is a single-column tie, and what can count as a column?",
        "answer": "It is a cuff that does not tighten when its tail is pulled; a column can be anything it encircles, such as a leg, torso, or bedpost.",
        "reason_code": "complete_single_column_functional_definition",
        "reason": "The replacement keeps the defining non-tightening behavior and restores the source's explanation that 'column' can mean a body part or object.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v175",
    "direct_rows_without_prior_curation": 68,
    "rope365_single_column_definition_rows_selected": 1,
    "rows": 506,
    "sha256": "577c0dfb8307bc762599d1da1711446e0b4628be2273ac62e2eb2cc7f32acc44",
}
EXPECTED_OUTPUT_SHA256 = "fc2676d8d2fbd8a999daf334a6f3c93f5403b9235afb21d390d05c320c0e08fb"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v176-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v175-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v175.jsonl"
        baseline_report = directory / "v175.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v175 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v176 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Rope365 single-column source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_single_column_definition_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v176",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v176 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v176 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v175": 468,
            ...
            "projection_strategy": "delegate to the hermetic v175 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v176",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the abbreviated single-column definition whose fully read source also explains the scope of 'column'",
                "score": "manual source support, foundational usefulness, definition completeness, and training merit",
