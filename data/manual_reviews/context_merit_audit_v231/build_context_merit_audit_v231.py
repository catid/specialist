#!/usr/bin/env python3
"""Connect the Tetruss link to portable-frame planning specifications."""
...
DATA = ROOT / "data"
V230_DIR = DATA / "manual_reviews/context_merit_audit_v230"
sys.path[:0] = [str(ROOT), str(V230_DIR)]
import build_context_merit_audit_v230 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v231.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v231.jsonl"
REPORT = OUT_DIR / "report_context_merit_v231.json"
REVIEWER = "codex-context-merit-audit-v231"
...
SOURCE = DATA / "raw/owner_tetruss_resource_20260714.json"

SPECS = (
    {
        "fact_id": "fact-f039341f5dca43990ad6",
        "active_index": 384,
        "markers": (
            "The supplied directory recommends Tetruss for portable suspension frames: https://tetruss.com/.",
            "The inspected specifications page describes the standard aluminum-alloy rig as weighing about 21 pounds, assembling or breaking down in about five minutes, requiring an eight-foot-or-higher ceiling unless customized lower, and using an eight-foot triangular footprint.",
            "The vendor describes the frame as supporting up to two people or 300 pounds, requiring no wall or ceiling bolts, brackets, hooks, holes, or nails, and carrying a one-year parts warranty.",
        ),
        "decision": "edit",
        "question": "Where can someone find the owner-recommended Tetruss portable frames, and what planning specs does the vendor give for the standard rig?",
        "answer": "Tetruss is at https://tetruss.com/. The vendor lists the standard rig at about 21 pounds, about five minutes to assemble or break down, an eight-foot triangular footprint, and an eight-foot-or-higher ceiling unless customized; it claims support for up to two people or 300 pounds without wall or ceiling hardware.",
        "reason_code": "connect_tetruss_link_to_planning_specs",
        "reason": "The original answer preserved only the frame link. The replacement keeps the exact owner-recommended URL and adds the inspected vendor's weight, setup-time, footprint, ceiling, load, and installation claims for practical comparison.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v230",
    "direct_rows_without_prior_curation": 13,
    "tetruss_resource_specs_rows_selected": 1,
    "rows": 505,
    "sha256": "a634baf6f2366c06ee501001bdc4fb186cd52b9effd6471578649bfe149a087e",
}
EXPECTED_OUTPUT_SHA256 = "4511c94bd810a4b54d5cd2f913134fb954d68c149072b0362269c792f6d830d7"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v231-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v230-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v230.jsonl"
        baseline_report = directory / "v230.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v230 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v231 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "tetruss_resource_specs_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v231",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v231 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v231 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v230": 467,
            ...
            "projection_strategy": "delegate to the hermetic v230 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v231",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended Tetruss link row whose inspected specifications support practical frame comparison",
                "score": "owner-source support, site-planning usefulness, specification completeness, and URL preservation",
