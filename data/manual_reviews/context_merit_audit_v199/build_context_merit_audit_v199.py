#!/usr/bin/env python3
"""Replace a deGiotto detergent lookup with its full jute tub-wash procedure."""
...
DATA = ROOT / "data"
V198_DIR = DATA / "manual_reviews/context_merit_audit_v198"
sys.path[:0] = [str(ROOT), str(V198_DIR)]
import build_context_merit_audit_v198 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v199.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v199.jsonl"
REPORT = OUT_DIR / "report_context_merit_v199.json"
REVIEWER = "codex-context-merit-audit-v199"
...
SOURCE = DATA / "raw/degiotto_care_maintenance_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 37,
        "markers": (
            "For deGiotto jute, fill a tub or bucket with cool water, add one-half teaspoon of free-and-clear detergent, stir, add the unbundled rope, stir again, leave it for 20 minutes, rinse with cool water, then dry and treat it as directed below.",
        ),
        "decision": "edit",
        "question": "What tub-wash procedure does deGiotto specify for its own jute rope?",
        "answer": "Use cool water and one-half teaspoon of free-and-clear detergent, stir in the unbundled rope, soak it for 20 minutes, rinse with cool water, then follow deGiotto's special jute drying and treatment directions.",
        "reason_code": "replace_detergent_lookup_with_full_jute_wash_procedure",
        "reason": "The original answer isolated a detergent amount from a multistep, manufacturer-specific procedure. The replacement retains that amount while adding the water temperature, unbundling, soak, rinse, and required follow-on care.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v198",
    "degiotto_jute_wash_rows_selected": 1,
    "direct_rows_without_prior_curation": 45,
    "rows": 505,
    "sha256": "b48cbed164c6fe27e7460133946c25651f45aaa4b2dcdaa1cc8c23bb347d0d3b",
}
EXPECTED_OUTPUT_SHA256 = "8082160c496b3f922834ed88a70d5df8e5175ee9e55cbc35e0774e6499b01e38"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v199-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v198-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v198.jsonl"
        baseline_report = directory / "v198.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v198 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v199 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: deGiotto care source lineage drift")
        audit = {
            ...
            "review_pass": "degiotto_jute_wash_completeness_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v199",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v199 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v199 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v198": 467,
            ...
            "projection_strategy": "delegate to the hermetic v198 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v199",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated detergent lookup whose fully inspected care page supplies the complete manufacturer-specific jute wash procedure",
                "score": "manual source support, procedural completeness, care usefulness, and training merit",
