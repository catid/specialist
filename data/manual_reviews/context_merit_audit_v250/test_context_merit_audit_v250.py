#!/usr/bin/env python3
"""Focused regression tests for the v250 single-column definition redundancy removal."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v250 as b


class V250(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v250-", dir=HERE)
    ...
    def test_01_baseline(self):
        self.assertEqual((len(self.baseline), b.file_sha256(self.base)), (493, b.PROJECTED_SELECTION_BASELINE["sha256"]))
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("redundant_column_definition_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )

    def test_11_projection(self):
        self.assertEqual((len(self.rows), b.file_sha256(self.dataset)), (492, b.EXPECTED_OUTPUT_SHA256))
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v250", {"drop": 1, "edit": 0, "keep": 0}, b.EXPECTED_OUTPUT_SHA256),
        )
