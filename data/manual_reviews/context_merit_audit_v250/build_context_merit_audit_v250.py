#!/usr/bin/env python3
"""Remove a redundant single-column terminology answer."""
...
DATA = ROOT / "data"
V249_DIR = DATA / "manual_reviews/context_merit_audit_v249"
sys.path[:0] = [str(ROOT), str(V249_DIR)]
import build_context_merit_audit_v249 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v250.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v250.jsonl"
REPORT = OUT_DIR / "report_context_merit_v250.json"
REVIEWER = "codex-context-merit-audit-v250"
...
SPECS = (
    {
        "fact_id": "fact-66b0bd40009a03e7d4c9",
        "active_index": 124,
        "markers": (
            "A single column tie is a cuff that will not tighten when we pull on the tail. “Column” refers to anything you can tie around: a leg, a torso, a bedpost etc.",
        ),
        "decision": "drop",
        "reason_code": "drop_redundant_column_definition_answer",
        "reason": "This column-only definition is subsumed by a stronger active question from the same source sentence that defines both a single-column tie and what can count as a column.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v249",
    "direct_rows_without_prior_curation": 1,
    "redundant_column_definition_rows_selected": 1,
    "rows": 493,
    "sha256": "2562a77dba0c60eade55b0b702881b6acef6b488e5a9876d9648f0d86b014d7d",
}
EXPECTED_OUTPUT_SHA256 = "0b16857bd080af176df7110951599b6227f3857309bb426a100ab6fd17889056"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v250-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v249-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v249.jsonl"
        baseline_report = directory / "v249.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 493 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v249 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v250 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "redundant_column_definition_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v250",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v250 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v250 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v249": 455,
        ...
        "schema": "context-merit-audit-report-v250",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a column-only definition subsumed by a complete single-column-tie-and-column definition from the same source sentence",
