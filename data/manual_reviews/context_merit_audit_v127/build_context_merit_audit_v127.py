#!/usr/bin/env python3
"""Remove a low-merit square-lashing scaffolding lookup."""
...
DATA = ROOT / "data"
V126_DIR = DATA / "manual_reviews/context_merit_audit_v126"
sys.path[:0] = [str(ROOT), str(V126_DIR)]
import build_context_merit_audit_v126 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v127.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v127.jsonl"
REPORT = OUT_DIR / "report_context_merit_v127.json"
REVIEWER = "codex-context-merit-audit-v127"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__8cda4e4fbfe05ce2b00b.json"

SPECS = (
    {
        "fact_id": "fact-b17c2f6043148affbc7a",
        "active_index": 331,
        "marker": "The square lashing is a great technique to bind two columns at a 90-degree angle. It’s so solid people use it to create scaffolding or even build houses with it.",
        "decision": "drop",
        "reason_code": "low_merit_scaffolding_example",
        "reason": "This asks only for a non-bondage example of square lashing and preserves none of the source's angle, adaptation, pinching, or pressure guidance.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v126",
    "direct_rows_without_prior_curation": 92,
    "rope365_scaffolding_example_rows_selected": 1,
    "rows": 517,
    "sha256": "c154f844b363fa547cf4a68aa97657e1793a1e29f664af1aeac61c883c9436ea",
}
EXPECTED_OUTPUT_SHA256 = "6e60275822cfb27b074ce11103ea64e1d05ee4eb71091cab30a1b4d07468a436"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v127-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v126-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v126.jsonl"
        baseline_report = directory / "v126.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 517 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v126 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v127 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: more-columns source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_scaffolding_example_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v127",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 516 or observed["eval_fact_count"] != 612:
        raise ValueError("v127 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v127 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v126": 479,
            "active_after_this_tranche": 478,
            ...
            "projection_strategy": "delegate to the v126 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v127",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the not-yet-context-audited scaffolding-example Q&A from the fully read Rope365 more-columns page",
                "score": "manual source support, isolated-example risk, transferable value, and training merit",
