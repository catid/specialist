#!/usr/bin/env python3
"""Remove a low-merit magazine launch-year lookup."""
...
DATA = ROOT / "data"
V132_DIR = DATA / "manual_reviews/context_merit_audit_v132"
sys.path[:0] = [str(ROOT), str(V132_DIR)]
import build_context_merit_audit_v132 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v133.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v133.jsonl"
REPORT = OUT_DIR / "report_context_merit_v133.json"
REVIEWER = "codex-context-merit-audit-v133"
...
SPECS = (
    {
        "fact_id": "fact-3d5fdc178be9c920ca5b",
        "active_index": 143,
        "marker": "Growth (1945–69): Kitan Club was launched in 1947 and after Minomura became editor in 1952 it started to focus on minor sexual predilections such as Kinbaku, Seppuku, homosexuality, scatology, etc.",
        "decision": "drop",
        "reason_code": "low_merit_kitan_club_launch_year",
        "reason": "This exact-year lookup discards the source's four-phase historical model and its discussion of how media, learning networks, and cultural change shaped rope practice.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v132",
    "direct_rows_without_prior_curation": 92,
    "kinbakutoday_kitan_club_launch_year_rows_selected": 1,
    "rows": 512,
    "sha256": "1c682560f79de33e6a0e1bc2dc3d348231087595a28dd6d8a0e22fd7d1786107",
}
EXPECTED_OUTPUT_SHA256 = "2d7cc4e367fc7cb2b6b39032642a52d6c2858fe1e2fd37967dd6294420d1dda8"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v133-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v132-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v132.jsonl"
        baseline_report = directory / "v132.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 512 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v132 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v133 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: evolving-era-part-two source lineage drift")
        audit = {
            ...
            "review_pass": "kinbakutoday_kitan_club_launch_year_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v133",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 511 or observed["eval_fact_count"] != 612:
        raise ValueError("v133 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v133 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v132": 474,
            ...
            "projection_strategy": "delegate to the hermetic v132 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v133",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the not-yet-context-audited magazine launch-year Q&A from the fully read Kinbaku Today evolving-era Part 2 debate",
                "score": "manual source support, chronology-trivia risk, transferable value, and training merit",
