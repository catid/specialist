#!/usr/bin/env python3
"""Replace a tie-name lookup with a spatial-experience comparison."""
...
DATA = ROOT / "data"
V260_DIR = DATA / "manual_reviews/context_merit_audit_v260"
sys.path[:0] = [str(ROOT), str(V260_DIR)]
import build_context_merit_audit_v260 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v261.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v261.jsonl"
REPORT = OUT_DIR / "report_context_merit_v261.json"
REVIEWER = "codex-context-merit-audit-v261"
...
SOURCE = DATA / "raw/kinbakutoday_7113a15b5e5e3aa3.json"

SPECS = (
    {
        "fact_id": "fact-f04ec1363b33d00764e5",
        "active_index": 420,
        "markers": (
            "Even when you are tied really tight and suspended to a beam, or a bamboo, you feel the space all around you. It gives you anyway the feeling that you could move a little bit, that you are partially free, for example, to sway in the air.",
        ...
        "decision": "edit",
        "question": "How does “The Evocative Power of Hashira Shibari” contrast a vertical-post tie with suspension in open space?",
        "answer": "Open suspension may leave a sense of surrounding space and some ability to sway, while the post’s hard surface prevents turning, hiding, or adjusting position and makes the person feel more confined.",
        "reason_code": "replace_tie_lookup_with_spatial_experience_contrast",
        "reason": "The replacement trades a bare tie-name lookup for the article's substantive comparison of how open space and a hard vertical surface change movement and perceived confinement.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v260",
    "direct_rows_without_prior_curation": 1,
    "tie_name_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "e838a242f86ce45cea24e710ef806670dcdc54e1edab02259785827cae6dd994",
}
EXPECTED_OUTPUT_SHA256 = "7fc2fe471000b877d55f8fc4576bd8201468c51ec0b296d70b8764014aff9b82"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v261-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v260-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v260.jsonl"
        baseline_report = directory / "v260.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v260 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v261 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "spatial_experience_contrast_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v261",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v261 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v261 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v260": 454,
        ...
        "schema": "context-merit-audit-report-v261",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a tie-name lookup whose source passage supports a spatial-experience comparison",
                "score": "experiential and interpretive value, source support, question naturalness, and reduced label trivia",
