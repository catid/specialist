#!/usr/bin/env python3
"""Connect the owner-recommended X-POLE link to its planning dimensions."""
...
DATA = ROOT / "data"
V218_DIR = DATA / "manual_reviews/context_merit_audit_v218"
sys.path[:0] = [str(ROOT), str(V218_DIR)]
import build_context_merit_audit_v218 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v219.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v219.jsonl"
REPORT = OUT_DIR / "report_context_merit_v219.json"
REVIEWER = "codex-context-merit-audit-v219"
...
SOURCE = DATA / "raw/owner_xpole_a_frame_resource_20260714.json"

SPECS = (
    {
        "fact_id": "fact-d9349b944ec02a40316b",
        "active_index": 262,
        "markers": (
            "The supplied directory recommends the X-POLE A-FRAME as a semi-permanent suspension-frame resource and gives this product page: https://xpoleus.com/shop-all/aerial/a-frame/xpole-a-frame/.",
            "The official manual lists a selectable height range from 2,255 to 3,480 millimetres (89 to 137 inches, or about 7.5 to 11.42 feet).",
            "At those endpoints, the manual lists required floor areas of 3.6 and 8.9 square metres (38 and 95 square feet), respectively.",
        ),
        "decision": "edit",
        "question": "Where is the owner-recommended X-POLE A-FRAME page, and what height and floor-area ranges does its manual specify?",
        "answer": "The page is https://xpoleus.com/shop-all/aerial/a-frame/xpole-a-frame/. The manual gives heights of 2,255–3,480 millimetres (89–137 inches, about 7.5–11.42 feet) and corresponding floor areas of 3.6–8.9 square metres (38–95 square feet).",
        "reason_code": "connect_xpole_link_to_planning_dimensions",
        "reason": "The original answer preserved only the product link. The replacement keeps that exact owner-recommended URL and adds the official manual's height and floor-area ranges for practical site planning.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v218",
    "direct_rows_without_prior_curation": 25,
    "xpole_owner_resource_planning_rows_selected": 1,
    "rows": 505,
    "sha256": "f20b21066a0713dd693737613120f0864f831891a709561e4dbe5b73e39fbce2",
}
EXPECTED_OUTPUT_SHA256 = "ebed69627ae665c0f0044194bf2eab18fbfb3b09d404ee97c55d6a9305864e1d"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v219-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v218-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v218.jsonl"
        baseline_report = directory / "v218.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v218 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v219 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "xpole_owner_resource_planning_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v219",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v219 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v219 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v218": 467,
            ...
            "projection_strategy": "delegate to the hermetic v218 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v219",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended X-POLE link row whose official manual supplies practical planning dimensions",
                "score": "owner-source support, site-planning usefulness, dimensional completeness, and URL preservation",
