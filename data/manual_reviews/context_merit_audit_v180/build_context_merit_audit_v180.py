#!/usr/bin/env python3
"""Replace the bare nylon-bowline answer with its stability rationale."""
...
DATA = ROOT / "data"
V179_DIR = DATA / "manual_reviews/context_merit_audit_v179"
sys.path[:0] = [str(ROOT), str(V179_DIR)]
import build_context_merit_audit_v179 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v180.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v180.jsonl"
REPORT = OUT_DIR / "report_context_merit_v180.json"
REVIEWER = "codex-context-merit-audit-v180"
...
SPECS = (
    {
        ...
        "active_index": 292,
        "markers": (
            "Not every bowline is stable with nylon.",
        ),
        "decision": "edit",
        "question": "Why should a bowline be tested before relying on it with nylon rope?",
        "answer": "Not every bowline is stable with nylon, so test the specific bowline before depending on it.",
        "reason_code": "replace_bare_bowline_test_answer_with_rationale",
        "reason": "The replacement turns the bare 'test it first' answer into the source's material-specific stability warning and action.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v179",
    "direct_rows_without_prior_curation": 64,
    "knothead_nylon_bowline_rows_selected": 1,
    "rows": 506,
    "sha256": "8ae75af86557763a5d0679aff947403874b67e01203aeecaa383eb09d26a22e2",
}
EXPECTED_OUTPUT_SHA256 = "2d0ef6e55f192dbcd2fddbcd94901173ef286dd05721787fc2f157abf99be746"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v180-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v179-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v179.jsonl"
        baseline_report = directory / "v179.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v179 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v180 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != LEGACY_URL:
            raise ValueError(f"{spec['fact_id']}: Knot Head Nylon legacy source lineage drift")
        audit = {
            ...
            "review_pass": "knothead_nylon_bowline_stability_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v180",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v180 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v180 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v179": 468,
            ...
            "projection_strategy": "delegate to the hermetic v179 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v180",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the bare nylon-bowline precaution whose fully read current successor page states the stability rationale",
                "score": "manual source support, material-specific safety usefulness, answer completeness, and training merit",
