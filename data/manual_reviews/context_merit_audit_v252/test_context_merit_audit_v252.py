#!/usr/bin/env python3
"""Focused regression tests for the v252 visual-contrast replacement."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v252 as b


class V252(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v252-", dir=HERE)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("pen_name_lookup_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v252", {"drop": 0, "edit": 1, "keep": 0}, b.EXPECTED_OUTPUT_SHA256),
        )
