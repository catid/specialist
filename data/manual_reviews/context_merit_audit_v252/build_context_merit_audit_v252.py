#!/usr/bin/env python3
"""Replace a pen-name lookup with the source's visual contrast."""
...
DATA = ROOT / "data"
V251_DIR = DATA / "manual_reviews/context_merit_audit_v251"
sys.path[:0] = [str(ROOT), str(V251_DIR)]
import build_context_merit_audit_v251 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v252.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v252.jsonl"
REPORT = OUT_DIR / "report_context_merit_v252.json"
REVIEWER = "codex-context-merit-audit-v252"
...
SOURCE = DATA / "raw/kinbakutoday_dacb6d1879f91b8c.json"

SPECS = (
    {
        "fact_id": "fact-218babff4b15b66072ee",
        "active_index": 275,
        "markers": (
        ...
        "decision": "edit",
        "question": "How does Kinbaku Today contrast the earlier hebizeme art with the December 1953 photo layout?",
        "answer": "The earlier art emphasizes peril, while the photographs blend snakes and rope almost seamlessly and convey a stronger sense of sensuality.",
        "reason_code": "replace_pen_name_lookup_with_visual_contrast",
        "reason": "The replacement trades a bare pen-name lookup for the source's substantive comparison of peril-focused art with a more sensual photographic treatment.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v251",
    "direct_rows_without_prior_curation": 1,
    "pen_name_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "8e89377bced9c32d5422bbc058212edded41b9620e54533236cbdb347a722927",
}
EXPECTED_OUTPUT_SHA256 = "9d191a420031460ba1cf3f717c139ce19bb493d7a0cc9a4c27a8c62dd15dbfb6"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v252-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v251-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v251.jsonl"
        baseline_report = directory / "v251.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v251 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v252 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "pen_name_lookup_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v252",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v252 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v252 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v251": 454,
        ...
        "schema": "context-merit-audit-report-v252",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a pen-name lookup whose source passage supports a more substantive visual comparison",
                "score": "transferable interpretation, source support, question naturalness, and reduced biographical trivia",
