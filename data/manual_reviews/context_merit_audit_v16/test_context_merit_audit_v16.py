#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v16."""
...
import build_context_merit_audit_v16 as builder
from qa_quality import normalize_text


class ContextMeritAuditV16Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v16-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v16.jsonl"),
            ("CURATION", "pending_curation_context_merit_v16.jsonl"),
            ("REPORT", "report_context_merit_v16.json"),
        ):
    ...
    def test_future_context_tranches_are_not_historical_inputs(self) -> None:
        ...
        prior = manual_root / "context_merit_audit_v15" / "synthetic.jsonl"
        ...
        self.assertEqual(
            ...
            frozenset(f"context_merit_audit_v{i}" for i in range(1, 16)),
        )
    ...
    def test_decisions_and_curation(self) -> None:
        ...
        self.assertEqual(
            {action: sum(row["action"] == action for row in self.curation)
             for action in ("drop", "edit")}, {"drop": 10, "edit": 9})
    ...
    def test_report_projection_and_sealed_policy(self) -> None:
        ...
        self.assertEqual(report["schema"], "context-merit-audit-report-v16")
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 10, "edit": 9, "keep": 6})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 680)
        self.assertEqual(
            projection["output_sha256"],
            "41310a560facf9a827caa9f1a04e30d5038706d76d5c479b7aff8550123a9603")
    ...
    def test_generator_is_deterministic_and_prior_is_frozen(self) -> None:
        ...
        prior_files = tuple(
            path for version in range(1, 16)
