#!/usr/bin/env python3
"""Connect the De Giotto Rope link to inspected jute-care guidance."""
...
DATA = ROOT / "data"
V226_DIR = DATA / "manual_reviews/context_merit_audit_v226"
sys.path[:0] = [str(ROOT), str(V226_DIR)]
import build_context_merit_audit_v226 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v227.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v227.jsonl"
REPORT = OUT_DIR / "report_context_merit_v227.json"
REVIEWER = "codex-context-merit-audit-v227"
...
SOURCE = DATA / "raw/owner_degiotto_resource_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 379,
        "markers": (
            "The supplied directory recommends De Giotto Rope as a natural-fiber bondage-rope supplier: https://degiottorope.com/.",
            "The inspected care page says a full jute wash uses cool water and about one-half teaspoon of free-and-clear detergent, followed by unbundling, stirring, a roughly 20-minute soak, thorough rinsing, and complete drying under light tension before treatment.",
            "Its drying setup hangs the rope by its ends at least four feet apart, adds about one pound at the center, and lets it dry completely without retightening so wet jute is less likely to remain curled, shortened, thick, or mushy.",
        ),
        "decision": "edit",
        "question": "Where can someone find the owner-recommended De Giotto natural-fiber rope shop, and what does its care page say about fully washing jute?",
        "answer": "The shop is https://degiottorope.com/. Its care page uses cool water and a small amount of free-and-clear detergent, then thorough rinsing and complete drying under light tension before treatment; the page's setup spaces the ends apart and uses a modest center weight without retightening.",
        "reason_code": "connect_degiotto_link_to_jute_wash_care",
        "reason": "The original answer preserved only the supplier link. The replacement keeps the exact owner-recommended URL and adds the inspected vendor's full-wash and drying principles for the natural-fiber rope it sells.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v226",
    "direct_rows_without_prior_curation": 17,
    "degiotto_resource_care_rows_selected": 1,
    "rows": 505,
    "sha256": "a509e8990cb48b9728e9222257fecd3c687bf82ac586c4f2fd7060072ab83ad1",
}
EXPECTED_OUTPUT_SHA256 = "7cebdbdf63823ba89de94be14733450e15b09e9b3338ef25e988ee4790b20019"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v227-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v226-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v226.jsonl"
        baseline_report = directory / "v226.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v226 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v227 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "degiotto_resource_care_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v227",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v227 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v227 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v226": 467,
            ...
            "projection_strategy": "delegate to the hermetic v226 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v227",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended De Giotto link row whose inspected care page gives a complete jute-wash sequence",
                "score": "owner-source support, material-care usefulness, process completeness, and URL preservation",
