#!/usr/bin/env python3
"""Replace a pseudonym lookup with an archival-provenance lesson."""
...
DATA = ROOT / "data"
V254_DIR = DATA / "manual_reviews/context_merit_audit_v254"
sys.path[:0] = [str(ROOT), str(V254_DIR)]
import build_context_merit_audit_v254 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v255.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v255.jsonl"
REPORT = OUT_DIR / "report_context_merit_v255.json"
REVIEWER = "codex-context-merit-audit-v255"
...
SOURCE = DATA / "raw/kinbakutoday_4f9dec06e4af751a.json"

SPECS = (
    {
        "fact_id": "fact-5a0fa8f81150a759586c",
        "active_index": 109,
        "markers": (
            ...
            "Because no single photographer or rope artist could be identified, one had to be invented. The identity that Nureki created was Kanai Yukio (Akita, Nureki, & Fuji, 1996).",
        ),
        "decision": "edit",
        "question": "What archival problem does “The Ghosts of Kinbaku History” illustrate with the credits Fujimi Iku and Kanai Yukio?",
        "answer": "Fujimi Iku was another Nureki Chimuo pseudonym, while Kanai Yukio was an invented photographer credit applied to images whose makers were unknown, so the credits do not establish stable authorship.",
        "reason_code": "replace_pseudonym_lookup_with_archival_provenance",
        "reason": "The replacement trades a bare pseudonym lookup for the source's broader archival lesson: both author and photographer credits can mask unstable or unknown provenance.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v254",
    "direct_rows_without_prior_curation": 1,
    "pseudonym_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "1766201a339230f1461c23919f201cf0eec719fbaadbf2fb98fca1fabad3b107",
}
EXPECTED_OUTPUT_SHA256 = "f18a40bff9374111f46d55dbea3a4bfdcc628ccb1a42b5fbb440696cb850c848"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v255-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v254-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v254.jsonl"
        baseline_report = directory / "v254.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v254 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v255 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "archival_credit_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v255",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v255 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v255 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v254": 454,
        ...
        "schema": "context-merit-audit-report-v255",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a pseudonym lookup whose source passage supports a broader archival-provenance lesson",
                "score": "transferable source-literacy value, source support, question naturalness, and reduced name trivia",
