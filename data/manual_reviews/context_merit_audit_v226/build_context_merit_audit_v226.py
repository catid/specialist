#!/usr/bin/env python3
"""Connect the Tethered Together link to programming and Ropenspace."""
...
DATA = ROOT / "data"
V225_DIR = DATA / "manual_reviews/context_merit_audit_v225"
sys.path[:0] = [str(ROOT), str(V225_DIR)]
import build_context_merit_audit_v225 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v226.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v226.jsonl"
REPORT = OUT_DIR / "report_context_merit_v226.json"
REVIEWER = "codex-context-merit-audit-v226"
...
SOURCE = DATA / "raw/owner_tethered_together_resource_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 376,
        "markers": (
            "The supplied directory recommends Tethered Together as a United States rope convention resource: https://tetheredtogether.net/.",
            "The inspected site describes a rope-bondage-focused event with additional programming in circus arts, kink, relationships, dance, and yoga, plus educational workshops, attendee-led programming, art installations, entertainment, and a sex-positive dungeon on Friday through Sunday nights.",
            "Its Ropenspace uses a facilitated, self-organizing Open Space process in which participants propose questions or interests and move among evolving classes, discussions, and hands-on skill shares.",
        ),
        "decision": "edit",
        "question": "Where can someone learn about Tethered Together, and what programming model distinguishes its Ropenspace?",
        "answer": "The convention site is https://tetheredtogether.net/. Alongside rope-focused workshops and broader arts, movement, relationship, and kink programming, its Ropenspace is participant-driven: attendees propose interests and move among evolving classes, discussions, and hands-on skill shares.",
        "reason_code": "connect_tethered_link_to_ropenspace_programming",
        "reason": "The original answer preserved only the convention link. The replacement keeps the exact owner-recommended URL and adds the inspected event scope and participant-driven Ropenspace model.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v225",
    "direct_rows_without_prior_curation": 18,
    "tethered_resource_programming_rows_selected": 1,
    "rows": 505,
    "sha256": "ff9a37798358417d98c232707e964674df3d675677e1fa54528b83780eec9639",
}
EXPECTED_OUTPUT_SHA256 = "a509e8990cb48b9728e9222257fecd3c687bf82ac586c4f2fd7060072ab83ad1"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v226-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v225-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v225.jsonl"
        baseline_report = directory / "v225.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v225 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v226 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "tethered_resource_programming_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v226",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v226 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v226 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v225": 467,
            ...
            "projection_strategy": "delegate to the hermetic v225 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v226",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended Tethered Together link row whose inspected site explains the Ropenspace model",
                "score": "owner-source support, attendee-orientation usefulness, programming completeness, and URL preservation",
