...
import build_technique_additions_v13 as b


class TechniqueAdditionsV13(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        b.main()
    ...
    def test_baseline_and_artifact(self):
        with tempfile.TemporaryDirectory(prefix=".test-technique-v13-", dir=HERE) as temp:
            p = Path(temp)
            rows = b.build_baseline(p / "baseline.jsonl", p / "baseline.report.json")
            self.assertEqual((len(rows), b.file_sha256(p / "baseline.jsonl")), (528, b.BASELINE_SHA256))
    ...
    def test_report(self):
        ...
        self.assertEqual(self.report["new_independent_inputs"]["expected_strata"], {"technique": 3})
