#!/usr/bin/env python3
"""Remove a low-merit historical publication-name lookup."""
...
DATA = ROOT / "data"
V142_DIR = DATA / "manual_reviews/context_merit_audit_v142"
sys.path[:0] = [str(ROOT), str(V142_DIR)]
import build_context_merit_audit_v142 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v143.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v143.jsonl"
REPORT = OUT_DIR / "report_context_merit_v143.json"
REVIEWER = "codex-context-merit-audit-v143"
...
SOURCE = DATA / "raw/kinbakutoday_2265a4f9ae40d83a.json"

SPECS = (
    {
        "fact_id": "fact-6ad324204e5a84fb5783",
        "active_index": 301,
        "marker": "In 1953 Suma Toshiyuki moved to Tokyo and began working for the short lived Fūzoku Sōshi and would later start Uramado in 1955.",
        "decision": "drop",
        "reason_code": "low_merit_uramado_name_lookup",
        "reason": "This tests one publication name while preserving neither the album's artistic significance nor the subject's broader publishing influence.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v142",
    "direct_rows_without_prior_curation": 90,
    "kinbakutoday_uramado_name_rows_selected": 1,
    "rows": 510,
    "sha256": "a0674efa550f03cda7cf6ef25f628926e8e1126d40fbc5190e03bea39f6529c6",
}
EXPECTED_OUTPUT_SHA256 = "fa63bdec1cca97f40610106538c4fe47da6d6a57aad538f56276b436cfe8533e"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v143-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v142-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v142.jsonl"
        baseline_report = directory / "v142.report.json"
        ...
        if len(rows) != 510 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v142 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v143 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Kita Reiko album source lineage drift")
        audit = {
            ...
            "review_pass": "kinbakutoday_uramado_name_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v143",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 509 or observed["eval_fact_count"] != 612:
        raise ValueError("v143 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v143 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v142": 472,
            ...
            "projection_strategy": "delegate to the hermetic v142 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v143",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the publication-name Q&A from the fully read Kita Reiko album note",
                "score": "source support, isolated-trivia risk, transferable value, and training merit",
