#!/usr/bin/env python3
"""Focused regression tests for context-merit audit tranche v11."""
...
import build_context_merit_audit_v11 as builder
from qa_quality import normalize_text


class ContextMeritAuditV11Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v11-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output_dir = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v11.jsonl"),
            ("CURATION", "pending_curation_context_merit_v11.jsonl"),
            ("REPORT", "report_context_merit_v11.json"),
        ):
            patcher = mock.patch.object(builder, attribute, output_dir / filename)
            patcher.start()
    ...
    def test_future_context_tranches_are_not_historical_inputs(self) -> None:
        ...
        prior = manual_root / "context_merit_audit_v10" / "synthetic.jsonl"
        ...
        self.assertEqual(
            ...
            frozenset(f"context_merit_audit_v{i}" for i in range(1, 11)),
        )
    ...
    def test_selection_is_exact_and_nonoverlapping(self) -> None:
        ...
        self.assertEqual(tuple(x["row"]["fact_id"] for x in ranked[:25]),
    ...
    def test_report_projection_and_sealed_policy(self) -> None:
        ...
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 10, "edit": 9, "keep": 6})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 718)
        self.assertEqual(
            projection["output_sha256"],
            "948b14244c493d360c8090d425cc20d3c584c157cfd8e467338008b9d16c4abf")
    ...
    def test_generator_is_deterministic_and_prior_is_frozen(self) -> None:
        ...
        prior_files = tuple(
            path for version in range(1, 11)
