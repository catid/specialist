#!/usr/bin/env python3
"""Complete a Chromaknotz wash-care fact with drying and softener cautions."""
...
DATA = ROOT / "data"
V193_DIR = DATA / "manual_reviews/context_merit_audit_v193"
sys.path[:0] = [str(ROOT), str(V193_DIR)]
import build_context_merit_audit_v193 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v194.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v194.jsonl"
REPORT = OUT_DIR / "report_context_merit_v194.json"
REVIEWER = "codex-context-merit-audit-v194"
...
SPECS = (
    {
        ...
        "active_index": 1,
        "markers": (
            "Only wash on the gentle cycle in cold water, place rope in a mesh bag, and line dry out of direct sunlight.",
        ),
        "decision": "edit",
        ...
        "reason_code": "complete_wash_care_with_dryer_and_softener_cautions",
        "reason": "The original answer omitted the source's explicit prohibition on machine drying and its warning that excess fabric softener increases slipperiness. The replacement preserves the wash steps and adds both actionable cautions.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v193",
    "chromaknotz_care_rows_selected": 1,
    "direct_rows_without_prior_curation": 50,
    "rows": 505,
    "sha256": "ee4d67c367c3ef03421bf0e8c3fcbd31834bdd7a0b631f8c9d3b2cc921ab0257",
}
EXPECTED_OUTPUT_SHA256 = "40fce6476814c4ac2da4ef455773359bb1a095fb068ce43ae7e34bd5bbc35194"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v194-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v193-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v193.jsonl"
        baseline_report = directory / "v193.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v193 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v194 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "chromaknotz_care_completeness_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v194",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v194 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v194 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v193": 467,
            ...
            "projection_strategy": "delegate to the hermetic v193 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v194",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the remaining Chromaknotz wash-care row whose fully inspected page includes omitted dryer and softener cautions",
                "score": "manual source support, care completeness, risk relevance, and training merit",
