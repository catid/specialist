...
HERE=Path(__file__).resolve().parent;sys.path.insert(0,str(HERE));import build_group_learning_additions_v23 as b
...
class GroupLearningAdditionsV23(unittest.TestCase):
 @classmethod
 def setUpClass(cls):
  b.main();cls.rows=b.read_jsonl(b.OUTPUT)
  with tempfile.TemporaryDirectory(prefix=".test-group-learning-v23-",dir=HERE) as t:d=Path(t);cls.baseline=b.build_baseline(d/"v318.jsonl",d/"v318.report.json")
 def test_artifact(self):self.assertEqual((len(self.rows),b.file_sha256(b.OUTPUT)),(3,b.EXPECTED_OUTPUT_SHA256));self.assertEqual(Counter(classify_stratum(r) for r in self.rows),Counter({"safety_consent":2,"resources_general":1}))
