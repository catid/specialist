#!/usr/bin/env python3
"""Audit Esinem's prerequisite suspension-safety guidance."""
...
DATA = ROOT / "data"
V111_DIR = DATA / "manual_reviews/context_merit_audit_v111"
sys.path[:0] = [str(ROOT), str(V111_DIR)]
import build_context_merit_audit_v111 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v112.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v112.jsonl"
REPORT = OUT_DIR / "report_context_merit_v112.json"
REVIEWER = "codex-context-merit-audit-v112"
...
SPECS = (
    {
        ...
        "active_index": 5,
        "marker": "Make sure you have absorbed the information on nerve injury and common mistakes.",
        "decision": "keep",
        "reason_code": "clear_suspension_safety_prerequisites",
        "reason": "The answer accurately preserves the source's explicit prerequisite to understand nerve injury and common mistakes before advanced suspension training.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v111",
    "direct_rows_without_prior_curation": 92,
    "esinem_suspension_prerequisite_rows_selected": 1,
    "rows": 529,
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v112-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v111-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v111.jsonl"
        baseline_report = directory / "v111.report.json"
        ...
        if len(rows) != 529 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v111 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v112 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: suspension-prerequisite source lineage drift")
        audit = {
            ...
            "review_pass": "esinem_suspension_prerequisite_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v112",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 529 or observed["eval_fact_count"] != 612:
        raise ValueError("v112 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v112 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v111": 491,
        ...
        "schema": "context-merit-audit-report-v112",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the not-yet-context-audited prerequisite Q&A from the fully read Esinem suspension-course article",
                "score": "manual source support, nerve-safety actionability, answer completeness, and practical merit review",
