#!/usr/bin/env python3
"""Turn a synthetic-rope singeing warning into a causal care lesson."""
...
DATA = ROOT / "data"
V154_DIR = DATA / "manual_reviews/context_merit_audit_v154"
sys.path[:0] = [str(ROOT), str(V154_DIR)]
import build_context_merit_audit_v154 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v155.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v155.jsonl"
REPORT = OUT_DIR / "report_context_merit_v155.json"
REVIEWER = "codex-context-merit-audit-v155"
...
SPECS = (
    {
        ...
        "decision": "edit",
        "question": "Why does Rope365 warn against flame-singeing synthetic rope to remove fuzz?",
        "answer": "Because synthetic rope will melt under the flame.",
        "reason_code": "make_synthetic_singeing_warning_causal",
        "reason": "The replacement states the care context and directly connects the warning to the material’s response to flame.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v154",
    "direct_rows_without_prior_curation": 88,
    "rope365_synthetic_singeing_warning_rows_selected": 1,
    "rows": 508,
    "sha256": "845b42420f64012340c357fde192ae0c5b47ac1f6c254f86e1cce5bb10057c03",
}
EXPECTED_OUTPUT_SHA256 = "b73bc670876ac8e2a326088adeb091215ca898ef810188c27a6d9e300d0ea4a1"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v155-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v154-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v154.jsonl"
        baseline_report = directory / "v154.report.json"
        ...
        if len(rows) != 508 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v154 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v155 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "rope365_synthetic_singeing_warning_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v155",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 508 or observed["eval_fact_count"] != 612:
        raise ValueError("v155 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v155 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v154": 470,
            ...
            "projection_strategy": "delegate to the hermetic v154 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v155",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the synthetic-rope singeing warning whose fully read source supports a clearer causal explanation",
                "score": "manual source support, material-care usefulness, causal clarity, and training merit",
