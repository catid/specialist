#!/usr/bin/env python3
"""Normalize a clipped inversion-injury answer into a complete warning."""
...
DATA = ROOT / "data"
V284_DIR = DATA / "manual_reviews/context_merit_audit_v284"
sys.path[:0] = [str(ROOT), str(V284_DIR)]
import build_context_merit_audit_v284 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v285.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v285.jsonl"
REPORT = OUT_DIR / "report_context_merit_v285.json"
REVIEWER = "codex-context-merit-audit-v285"
...
SOURCE = DATA / "raw/esinem_76f7a14d2b86ea11.json"

SPECS = (
    {
        "fact_id": "fact-62de9de49658141c51c6",
        "active_index": 443,
        "markers": (
            "Inversions carry a high risk of serious injury if you lose control or drop your partner since their head is likely to be the first thing to hit the ground.",
        ),
        "decision": "edit",
        "question": "Why does Esinem describe inversions as carrying a high risk of serious injury?",
        "answer": "Losing control or dropping an inverted partner may send their head into the ground first, creating a high risk of serious injury.",
        "reason_code": "normalize_inversion_head_injury_warning",
        "reason": "The original answer was a dependent if-clause; the revision preserves the same source-supported injury mechanism as a grammatical, self-contained warning.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v284",
    "direct_rows_without_prior_curation": 1,
    "clipped_answer_rows_selected": 1,
    "rows": 492,
    "sha256": "107a43403edbc51a6b2b72f5c964648e865d384fe2156ce12c84b7cbcb63c614",
}
EXPECTED_OUTPUT_SHA256 = "172340fec38e97a8dcf2a70b15157af3a9ce627d359dd780d6b7043324e819ae"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v285-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v284-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v284.jsonl"
        baseline_report = directory / "v284.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v284 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v285 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "schema": "context-merit-audit-v285",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v285 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v285 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v284": 454,
        ...
        "schema": "context-merit-audit-report-v285",
