#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v15."""
...
import build_context_merit_audit_v15 as builder
from qa_quality import normalize_text


class ContextMeritAuditV15Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v15-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v15.jsonl"),
            ("CURATION", "pending_curation_context_merit_v15.jsonl"),
            ("REPORT", "report_context_merit_v15.json"),
        ):
    ...
    def test_future_context_tranches_are_not_historical_inputs(self) -> None:
        ...
        prior = manual_root / "context_merit_audit_v14" / "synthetic.jsonl"
        ...
        self.assertEqual(
            ...
            frozenset(f"context_merit_audit_v{i}" for i in range(1, 15)),
        )
    ...
    def test_report_projection_and_sealed_policy(self) -> None:
        ...
        self.assertEqual(report["schema"], "context-merit-audit-report-v15")
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 10, "edit": 5, "keep": 10})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 690)
        self.assertEqual(
            projection["output_sha256"],
            "ff67453be01515d9d3fa8cdb7c7dfb5416c9db032a669e1ce048183a9674bd74")
    ...
    def test_generator_is_deterministic_and_prior_is_frozen(self) -> None:
        ...
        prior_files = tuple(
            path for version in range(1, 15)
