#!/usr/bin/env python3
"""Replace a chest-harness acronym lookup with a structural comparison."""
...
DATA = ROOT / "data"
V270_DIR = DATA / "manual_reviews/context_merit_audit_v270"
sys.path[:0] = [str(ROOT), str(V270_DIR)]
import build_context_merit_audit_v270 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v271.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v271.jsonl"
REPORT = OUT_DIR / "report_context_merit_v271.json"
REVIEWER = "codex-context-merit-audit-v271"
...
SOURCE = DATA / "raw/kinbakutoday_e55b7fa7c543e266.json"

SPECS = (
    {
        "fact_id": "fact-af26e239d6bce2c6ae9b",
        "active_index": 128,
        "markers": (
            "Modern Kinbaku has been distilled to the point that there is a manual for achieving each harness and knit perfectly, making it highly reproducible, and there are specific standards for judging the level of students. The visual characteristics of Modern Kinbaku include symmetry and involve joining ropes, which makes it highly reproducible.",
            "Classical Kinbaku does have prescribed formats for the chest harness (TK), various hip harnesses, and leg ties, but only the bare minimum is standardized. This allows for a high degree of freedom beyond the basics, allowing each individual to express their own unique style. The ‘finished product’ changes slightly each time.",
            "It’s not a question of whether Classical or Modern is better or worse, or whether rope with wabi–sabi or without is better, it’s all a matter of preference.",
        ),
        "decision": "edit",
        "question": "How does Kasumi Hourai contrast the structure of Modern and Classical Kinbaku?",
        "answer": "Modern Kinbaku emphasizes manuals, repeatable symmetrical forms, and explicit standards; Classical Kinbaku standardizes only the basics, leaving more room for asymmetry, improvisation, and a unique result each time.",
        "reason_code": "replace_chest_harness_acronym_with_modern_classical_contrast",
        "reason": "The replacement trades a bare harness acronym for the interview's nuanced structural comparison, while preserving its explicit statement that neither approach is inherently better.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v270",
    "direct_rows_without_prior_curation": 1,
    "chest_harness_acronym_rows_selected": 1,
    "rows": 492,
    "sha256": "61e6461287ece8d16eef291fa1e37552202796b943e4a314ff6df01330d6c952",
}
EXPECTED_OUTPUT_SHA256 = "7f5904bdc8c985f02553a35d237722452d9d61770a358c02894bb623e1c50a15"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v271-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v270-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v270.jsonl"
        baseline_report = directory / "v270.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v270 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v271 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "modern_classical_structure_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v271",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v271 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v271 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v270": 454,
        ...
        "schema": "context-merit-audit-report-v271",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a chest-harness acronym lookup whose source passage supports a nuanced structural comparison",
                "score": "comparative and conceptual value, source support, question naturalness, and reduced acronym trivia",
