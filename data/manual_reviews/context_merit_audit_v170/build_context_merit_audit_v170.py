#!/usr/bin/env python3
"""Drop the generic change-of-mind row subsumed by transition consent."""
...
DATA = ROOT / "data"
V169_DIR = DATA / "manual_reviews/context_merit_audit_v169"
sys.path[:0] = [str(ROOT), str(V169_DIR)]
import build_context_merit_audit_v169 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v170.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v170.jsonl"
REPORT = OUT_DIR / "report_context_merit_v170.json"
REVIEWER = "codex-context-merit-audit-v170"
...
SPECS = (
    {
        ...
        "decision": "drop",
        "reason_code": "drop_subsumed_change_of_mind_row",
        "reason": "The preceding transition-consent row already teaches that consent may be renegotiated or withheld at any time, making this shorter formulation redundant.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v169",
    "direct_rows_without_prior_curation": 74,
    "anatomiestudio_subsumed_change_of_mind_rows_selected": 1,
    "rows": 507,
    "sha256": "89c14e53167eea88cebcb59203e11056abd476e8c7632cb6c6a056b064f546a1",
}
EXPECTED_OUTPUT_SHA256 = "31d84172c5b032fc7c94b17603eb04208955f53ed4ee9e3e4ae828d30e47024c"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v170-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v169-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v169.jsonl"
        baseline_report = directory / "v169.report.json"
        ...
        if len(rows) != 507 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v169 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v170 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "anatomiestudio_change_of_mind_dedup_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v170",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v170 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v170 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v169": 469,
            ...
            "projection_strategy": "delegate to the hermetic v169 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v170",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the generic change-of-mind Q&A subsumed by the preceding transition-consent rationale",
