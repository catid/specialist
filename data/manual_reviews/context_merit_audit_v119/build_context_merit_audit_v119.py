#!/usr/bin/env python3
"""Consolidate Rope365's chicken-wing ulnar-nerve guidance."""
...
DATA = ROOT / "data"
V118_DIR = DATA / "manual_reviews/context_merit_audit_v118"
sys.path[:0] = [str(ROOT), str(V118_DIR)]
import build_context_merit_audit_v118 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v119.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v119.jsonl"
REPORT = OUT_DIR / "report_context_merit_v119.json"
REVIEWER = "codex-context-merit-audit-v119"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__118a41bb0bd96d9bc195.json"

SPECS = (
    {
        "fact_id": "fact-ea5e41e107f6a4f441b8",
        "active_index": 445,
        "marker": "“Chicken wings” can sometimes be strenuous on the ulnar nerve of the person being tied, not because of the rope placement itself, but just because of the physical compression in the elbow area. If you feel tingles in your little fingers and ring fingers, experiment together with finding alternatives that allow you to open “the wings” more and compress your elbows less.",
        "decision": "edit",
        ...
        "reason_code": "consolidate_ulnar_warning_and_response",
        "reason": "The revision replaces a nerve-name lookup with the source-supported warning signs and immediate pressure-reduction response.",
    },
    {
        "fact_id": "fact-d6733de1da318e6daedf",
        "active_index": 235,
        "marker": "“Chicken wings” can sometimes be strenuous on the ulnar nerve of the person being tied, not because of the rope placement itself, but just because of the physical compression in the elbow area. If you feel tingles in your little fingers and ring fingers, experiment together with finding alternatives that allow you to open “the wings” more and compress your elbows less.",
        "decision": "drop",
        "reason_code": "redundant_ulnar_response_subset",
        "reason": "This response-only row becomes a semantic subset of the consolidated Q&A, which preserves the warning signs, named nerve, and mitigation together.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v118",
    "direct_rows_without_prior_curation": 92,
    "rope365_chicken_wing_rows_selected": 2,
    "rows": 528,
    "sha256": "29468ebdf6f982fc565143b617a1865935ad68b1864b3d44c577267d377b318e",
}
EXPECTED_OUTPUT_SHA256 = "a73b919d553645a937746d7428bd80cac3924036429095a9ac6003dca217caa1"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v119-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v118-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v118.jsonl"
        baseline_report = directory / "v118.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 528 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v118 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v119 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: chicken-wing source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_chicken_wing_ulnar_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v119",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 527 or observed["eval_fact_count"] != 612:
        raise ValueError("v119 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v119 output hash drift")
    report = {
        "active_baseline": {
        ...
        "audit": {
            "by_decision": {"drop": 1, "edit": 1, "keep": 0},
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v118": 490,
            "active_after_this_tranche": 489,
            ...
            "projection_strategy": "delegate to the v118 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v119",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the overlapping ulnar-nerve Q&A from the fully read Rope365 chicken-wing page",
                "score": "manual source support, symptom and mitigation completeness, semantic redundancy, and practical merit review",
