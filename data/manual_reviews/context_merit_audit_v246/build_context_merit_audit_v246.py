#!/usr/bin/env python3
"""Remove a redundant Rope365 quick-release failure answer."""
...
DATA = ROOT / "data"
V245_DIR = DATA / "manual_reviews/context_merit_audit_v245"
sys.path[:0] = [str(ROOT), str(V245_DIR)]
import build_context_merit_audit_v245 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v246.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v246.jsonl"
REPORT = OUT_DIR / "report_context_merit_v246.json"
REVIEWER = "codex-context-merit-audit-v246"
...
SOURCE = DATA / "raw/rope365_c89abf7c3a5c30e1.json"

SPECS = (
    {
        "fact_id": "fact-ff0b2d0c5af68c1db5a3",
        "active_index": 147,
        "markers": (
            "They may not be as solid as other locking methods, but they are great in case of emergency, or for ties that may need adjustments along the way. We just need to be extra careful in case it unties accidentally with movement.",
        ),
        "decision": "drop",
        "reason_code": "drop_redundant_quick_release_failure_answer",
        "reason": "This failure-only row repeats the accidental-untying caution already included in a stronger active question that also preserves quick releases' emergency and adjustment benefits.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v245",
    "direct_rows_without_prior_curation": 1,
    "redundant_quick_release_failure_rows_selected": 1,
    "rows": 497,
    "sha256": "754a0684676462e7f6546ca948f83472d8153305f6c074e54fcbf68d94a9c733",
}
EXPECTED_OUTPUT_SHA256 = "c5ed754920b06f8b602427d5b0d340b8d9ff7cce00f873abecf9072051f56de5"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v246-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v245-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v245.jsonl"
        baseline_report = directory / "v245.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 497 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v245 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v246 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "redundant_quick_release_failure_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v246",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 496 or observed["eval_fact_count"] != 612:
        raise ValueError("v246 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v246 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v245": 459,
            "active_after_this_tranche": 458,
        ...
        "schema": "context-merit-audit-report-v246",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a narrow quick-release failure question whose caution is already included in a more complete benefits-and-failure row",
