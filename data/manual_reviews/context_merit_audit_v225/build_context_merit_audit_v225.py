#!/usr/bin/env python3
"""Connect the Austin Rope Slingers link to entry guidance and resources."""
...
DATA = ROOT / "data"
V224_DIR = DATA / "manual_reviews/context_merit_audit_v224"
sys.path[:0] = [str(ROOT), str(V224_DIR)]
import build_context_merit_audit_v224 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v225.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v225.jsonl"
REPORT = OUT_DIR / "report_context_merit_v225.json"
REVIEWER = "codex-context-merit-audit-v225"
...
SOURCE = DATA / "raw/owner_austin_rope_slingers_resource_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 374,
        "markers": (
            "The supplied directory recommends Austin Rope Slingers as an Austin rope group with regular meetings: https://www.austinropeslingers.com/.",
            "The inspected site describes a volunteer-run, donation-supported peer-education group offering a weekly 101-level class plus higher-level sessions or intensives as members request and offer them.",
            "It says adults 18 and older may attend without a partner or prerequisite knowledge and that all skill levels, abilities, and identities are welcome.",
            "Community resources include loaner floor rope, safety shears and straps, rope books, negotiation guides, and usually member-provided suspension rigs at labs.",
        ),
        "decision": "edit",
        "question": "Where can someone find Austin Rope Slingers, and what should a new adult attendee know about classes and shared resources?",
        "answer": "The group is at https://www.austinropeslingers.com/. It offers weekly 101 peer education and member-driven advanced sessions; adults 18+ need no partner or prerequisite, and labs provide resources such as loaner floor rope, safety shears, straps, books, negotiation guides, and usually member-provided rigs.",
        "reason_code": "connect_ars_link_to_entry_guidance_resources",
        "reason": "The original answer preserved only the group link. The replacement keeps the exact owner-recommended URL and adds inspected entry conditions, learning options, and community resources that a new attendee can act on.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v224",
    "direct_rows_without_prior_curation": 19,
    "ars_resource_entry_rows_selected": 1,
    "rows": 505,
    "sha256": "5628c04732fca5c1decbe411246a2ae56e8115863b87f1281c264dc5fdf77c05",
}
EXPECTED_OUTPUT_SHA256 = "ff9a37798358417d98c232707e964674df3d675677e1fa54528b83780eec9639"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v225-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v224-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v224.jsonl"
        baseline_report = directory / "v224.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v224 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v225 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "ars_resource_entry_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v225",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v225 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v225 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v224": 467,
            ...
            "projection_strategy": "delegate to the hermetic v224 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v225",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended Austin Rope Slingers link row whose inspected site gives entry conditions and shared resources",
                "score": "owner-source support, new-attendee usefulness, resource completeness, and URL preservation",
