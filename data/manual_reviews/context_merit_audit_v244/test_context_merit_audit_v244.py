#!/usr/bin/env python3
"""Focused regression tests for the v244 handcuff-knot duplicate removal."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v244 as b


class V244(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v244-", dir=HERE)
    ...
    def test_01_baseline(self):
        self.assertEqual((len(self.baseline), b.file_sha256(self.base)), (500, b.PROJECTED_SELECTION_BASELINE["sha256"]))
    ...
    def test_05_decisions(self):
        self.assertEqual(
            ...
            {"keep": 0, "edit": 0, "drop": 2},
        )

    def test_06_curation(self):
        self.assertEqual((len(self.curation), {row["action"] for row in self.curation}), (2, {"drop"}))
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("duplicate_handcuff_knot_risk_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )

    def test_11_projection(self):
        self.assertEqual((len(self.rows), b.file_sha256(self.dataset)), (498, b.EXPECTED_OUTPUT_SHA256))
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v244", {"drop": 2, "edit": 0, "keep": 0}, b.EXPECTED_OUTPUT_SHA256),
        )
