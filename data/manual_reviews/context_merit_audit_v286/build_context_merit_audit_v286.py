#!/usr/bin/env python3
"""Turn a clipped wrist-slack answer into practical safety guidance."""
...
DATA = ROOT / "data"
V285_DIR = DATA / "manual_reviews/context_merit_audit_v285"
sys.path[:0] = [str(ROOT), str(V285_DIR)]
import build_context_merit_audit_v285 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v286.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v286.jsonl"
REPORT = OUT_DIR / "report_context_merit_v286.json"
REVIEWER = "codex-context-merit-audit-v286"
...
SOURCE = DATA / "raw/esinem_c93603a2c7d48255.json"

SPECS = (
    {
        "fact_id": "fact-54e6cfc9c08e00207600",
        "active_index": 445,
        "markers": (
            "Firstly, it helps to prevent excessive pressure on the binding due to the loading of the harness during suspension.",
        ...
        "reason_code": "add_gote_wrist_slack_safety_context",
        "reason": "The original answer was a lower-case fragment covering only one benefit; the revision combines both source-supported reasons into concise, practical safety guidance.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v285",
    "direct_rows_without_prior_curation": 1,
    "clipped_answer_rows_selected": 1,
    "rows": 492,
    "sha256": "172340fec38e97a8dcf2a70b15157af3a9ce627d359dd780d6b7043324e819ae",
}
EXPECTED_OUTPUT_SHA256 = "338514d23d367ae3a8240ef686b47227e8443e23942120540d9a3085a40c69c2"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v286-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v285-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v285.jsonl"
        baseline_report = directory / "v285.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v285 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v286 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "schema": "context-merit-audit-v286",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v286 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v286 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v285": 454,
        ...
        "schema": "context-merit-audit-report-v286",
