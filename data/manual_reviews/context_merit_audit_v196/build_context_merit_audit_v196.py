#!/usr/bin/env python3
"""Add practical variability to Shibari Supply's bamboo safety warning."""
...
DATA = ROOT / "data"
V195_DIR = DATA / "manual_reviews/context_merit_audit_v195"
sys.path[:0] = [str(ROOT), str(V195_DIR)]
import build_context_merit_audit_v195 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v196.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v196.jsonl"
REPORT = OUT_DIR / "report_context_merit_v196.json"
REVIEWER = "codex-context-merit-audit-v196"
...
SOURCE = DATA / "raw/shibari_supply_bamboo_product_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 17,
        "markers": (
            "Because bamboo is natural, its diameter is approximate at about 4 to 4.4 inches, the selected length may vary by a few inches, and the pole may taper from one end to the other.",
        ),
        "decision": "edit",
        "question": "What dimensional variability and safety limitation does Shibari Supply disclose for its nominal four-inch suspension bamboo?",
        "answer": "It says natural bamboo may measure about 4 to 4.4 inches in diameter, vary by a few inches from the selected length, and taper end-to-end; the vendor makes no guarantee of safety, suitability, or longevity for any purpose.",
        "reason_code": "add_bamboo_variability_to_vendor_warning",
        "reason": "The original answer reproduced only the guarantee disclaimer. The replacement retains that warning and adds the source's concrete diameter, length, and taper variability so the limitation is useful for product assessment.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v195",
    "direct_rows_without_prior_curation": 48,
    "shibari_supply_bamboo_rows_selected": 1,
    "rows": 505,
    "sha256": "5343494db9e5ae270faa372ec2c5200c17f633cd38353e4ed0ffbb911e06ca41",
}
EXPECTED_OUTPUT_SHA256 = "85f73910ed76cd01479a09df4de6dc345af254d41da89179ec5926716e8856fa"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v196-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v195-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v195.jsonl"
        baseline_report = directory / "v195.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v195 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v196 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Shibari Supply bamboo source lineage drift")
        audit = {
            ...
            "review_pass": "shibari_supply_bamboo_variability_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v196",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v196 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v196 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v195": 467,
            ...
            "projection_strategy": "delegate to the hermetic v195 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v196",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated bamboo disclaimer whose fully inspected product page also documents concrete dimensional variability",
                "score": "manual source support, safety relevance, product-assessment usefulness, and training merit",
