#!/usr/bin/env python3
"""Pair X-POLE A-FRAME heights with their required floor areas."""
...
DATA = ROOT / "data"
V207_DIR = DATA / "manual_reviews/context_merit_audit_v207"
sys.path[:0] = [str(ROOT), str(V207_DIR)]
import build_context_merit_audit_v207 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v208.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v208.jsonl"
REPORT = OUT_DIR / "report_context_merit_v208.json"
REVIEWER = "codex-context-merit-audit-v208"
...
SOURCE = DATA / "raw/xpole_a_frame_manual_v2_2024_20260714.json"

SPECS = (
    {
        "fact_id": "fact-5a79fb4622156c02aaaa",
        "active_index": 141,
        "markers": (
            "The selectable height range is 2,255 to 3,480 millimetres (89 to 137 inches).",
        ),
        "decision": "edit",
        "question": "How do the X-POLE A-FRAME's height and required floor area change from its lowest to fully extended setup?",
        "answer": "At minimum it is 2,255 millimetres (89 inches or 7.5 feet) high and uses 3.6 square metres (38 square feet); fully extended it is 3,480 millimetres (137 inches or 11.42 feet) high and uses 8.9 square metres (95 square feet).",
        "reason_code": "pair_frame_heights_with_floor_areas",
        "reason": "The original answer listed heights without the corresponding setup space. The replacement preserves both endpoint heights and adds the manual's floor areas for practical site planning.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v207",
    "direct_rows_without_prior_curation": 36,
    "xpole_height_footprint_rows_selected": 1,
    "rows": 505,
    "sha256": "cc71d851d838fbb2bf1e20e4c464c7ec576f5adc752ef1ae62e4c69a5c8cad74",
}
EXPECTED_OUTPUT_SHA256 = "f34220f9731d708c23c9ed1a6242d9f010503924f0a31c211162f0adf52f4d36"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v208-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v207-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v207.jsonl"
        baseline_report = directory / "v207.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v207 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v208 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: X-POLE manual source lineage drift")
        audit = {
            ...
            "review_pass": "xpole_height_footprint_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v208",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v208 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v208 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v207": 467,
            ...
            "projection_strategy": "delegate to the hermetic v207 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v208",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated height-endpoint row whose fully inspected manual also gives the corresponding floor areas",
                "score": "manual source support, setup-planning usefulness, dimensional completeness, and training merit",
