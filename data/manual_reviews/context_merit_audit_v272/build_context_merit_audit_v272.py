#!/usr/bin/env python3
"""Replace a genre-label lookup with a production-demands history fact."""
...
DATA = ROOT / "data"
V271_DIR = DATA / "manual_reviews/context_merit_audit_v271"
sys.path[:0] = [str(ROOT), str(V271_DIR)]
import build_context_merit_audit_v271 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v272.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v272.jsonl"
REPORT = OUT_DIR / "report_context_merit_v272.json"
REVIEWER = "codex-context-merit-audit-v272"
...
SPECS = (
    {
        ...
        "reason_code": "replace_genre_label_lookup_with_production_demands",
        "reason": "The replacement trades a bare film-genre label for the article's concrete account of the workload and production demands that ended Mabuki's short run.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v271",
    "direct_rows_without_prior_curation": 1,
    "genre_label_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "7f5904bdc8c985f02553a35d237722452d9d61770a358c02894bb623e1c50a15",
}
EXPECTED_OUTPUT_SHA256 = "d3d1d4f06eea1e824cb1f05b423599a1bbefe3181da616bce5ae7ca85a1f5b42"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v272-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v271-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v271.jsonl"
        baseline_report = directory / "v271.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v271 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v272 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "production_demands_history_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v272",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v272 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v272 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v271": 454,
        ...
        "schema": "context-merit-audit-report-v272",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a genre-label lookup whose source passage supports a concrete production-demands history fact",
                "score": "historical and labor-context value, source support, question naturalness, and reduced label trivia",
