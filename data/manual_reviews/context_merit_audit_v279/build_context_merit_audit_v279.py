#!/usr/bin/env python3
"""Normalize a clipped heat-sterilization answer into a complete sentence."""
...
DATA = ROOT / "data"
V278_DIR = DATA / "manual_reviews/context_merit_audit_v278"
sys.path[:0] = [str(ROOT), str(V278_DIR)]
import build_context_merit_audit_v278 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v279.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v279.jsonl"
REPORT = OUT_DIR / "report_context_merit_v279.json"
REVIEWER = "codex-context-merit-audit-v279"
...
SPECS = (
    {
        ...
        "active_index": 329,
        "markers": (
            "Heat is likely to damage your ropes, and will definitely dry them out.",
        ),
        "decision": "edit",
        "question": "What two effects does Anatomie Studio associate with using heat to sterilize jute rope?",
        "answer": "Heat can damage jute rope and will dry its fibers out.",
        "reason_code": "normalize_clipped_heat_sterilization_answer",
        "reason": "The original answer omitted its subject; the revision preserves the same supported effects in a grammatical, self-contained sentence.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v278",
    "direct_rows_without_prior_curation": 1,
    "clipped_answer_rows_selected": 1,
    "rows": 492,
    "sha256": "77d43340191dae23669e6557531f69749a3c1c48e6a34321653affc637a0ec03",
}
EXPECTED_OUTPUT_SHA256 = "4aa52b3d8e1f794bce6fb2b49834133a510dd00466557f81b08b3df130447b28"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v279-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v278-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v278.jsonl"
        baseline_report = directory / "v278.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v278 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v279 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "schema": "context-merit-audit-v279",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v279 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v279 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v278": 454,
        ...
        "schema": "context-merit-audit-report-v279",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "an otherwise useful row whose answer is a clipped source fragment missing its subject",
