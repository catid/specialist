#!/usr/bin/env python3
"""Replace a tie-name lookup with the post's added physical constraint."""
...
DATA = ROOT / "data"
V269_DIR = DATA / "manual_reviews/context_merit_audit_v269"
sys.path[:0] = [str(ROOT), str(V269_DIR)]
import build_context_merit_audit_v269 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v270.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v270.jsonl"
REPORT = OUT_DIR / "report_context_merit_v270.json"
REVIEWER = "codex-context-merit-audit-v270"
...
SPECS = (
    {
        "fact_id": "fact-f06a2bfefa27b95ec21a",
        "active_index": 273,
        "markers": (
            "As soon as your Takate Kote is fixed to the Hashira, you can’t go anywhere. One of the surfaces of your body quickly realizes that not only the ropes, but a hard point limit its possibility to move, and reduces even more the expansion degree of the ribcage.",
        ),
        "decision": "edit",
        "question": "According to “The Evocative Power of Hashira Shibari,” what added physical constraint can the post impose when a takate-kote is fixed to it?",
        "answer": "Beyond the rope’s restriction, the hard post limits body movement and can further reduce rib-cage expansion.",
        "reason_code": "replace_tie_name_lookup_with_added_physical_constraint",
        "reason": "The replacement trades a redundant name lookup for the source's concrete description of the post as an additional hard limit on movement and rib-cage expansion.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v269",
    "direct_rows_without_prior_curation": 1,
    "tie_name_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "54bcd61936a247d9b34a33c0fe09179173a37e2f1a183f8f826138f9e1522ee7",
}
EXPECTED_OUTPUT_SHA256 = "61e6461287ece8d16eef291fa1e37552202796b943e4a314ff6df01330d6c952"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v270-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v269-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v269.jsonl"
        baseline_report = directory / "v269.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v269 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v270 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "added_physical_constraint_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v270",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v270 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v270 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v269": 454,
        ...
        "schema": "context-merit-audit-report-v270",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a redundant tie-name lookup whose source passage supports a concrete physical-constraint explanation",
                "score": "physical-risk relevance, source support, question naturalness, and reduced label trivia",
