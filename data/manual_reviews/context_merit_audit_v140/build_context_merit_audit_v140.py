#!/usr/bin/env python3
"""Replace a one-word freeze response with a clear consent rule."""
...
DATA = ROOT / "data"
V139_DIR = DATA / "manual_reviews/context_merit_audit_v139"
sys.path[:0] = [str(ROOT), str(V139_DIR)]
import build_context_merit_audit_v139 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v140.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v140.jsonl"
REPORT = OUT_DIR / "report_context_merit_v140.json"
REVIEWER = "codex-context-merit-audit-v140"
...
SPECS = (
    {
        ...
        "active_index": 438,
        "marker": "– Respect “no” (“no” is a complete sentence). Silence is not consent, freezing is not consent.",
        "secondary_marker": "Listen and pay attention to words, feelings, and context. Respect indecision (it is not a yes). Mixed messages mean “no.”",
        "decision": "edit",
        ...
        "answer": "Treat none of them as a yes; consent must be clearly and mutually communicated.",
        "reason_code": "replace_freezing_lookup_with_clear_consent_rule",
        "reason": "The replacement consolidates the source's explicit treatment of silence, freezing, indecision, and mixed messages instead of testing one response word in isolation.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v139",
    "direct_rows_without_prior_curation": 92,
    "anatomiestudio_nonconsent_signals_rows_selected": 1,
    "rows": 511,
    "sha256": "39e4462b57dc497ee8d3b681c7ad615750878fcd57ae4f2d3d9c0afecbd0dce1",
}
EXPECTED_OUTPUT_SHA256 = "500ab83c24cf01a5ee2b0e71acf9b2927130682e5251666d16024083d36dbb0d"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v140-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v139-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v139.jsonl"
        baseline_report = directory / "v139.report.json"
        ...
        if len(rows) != 511 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v139 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v140 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "anatomiestudio_nonconsent_signals_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v140",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 511 or observed["eval_fact_count"] != 612:
        raise ValueError("v140 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v140 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v139": 473,
            ...
            "projection_strategy": "delegate to the hermetic v139 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v140",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the one-word freezing Q&A whose fully read source provides a broader nonconsent rule",
                "score": "manual source support, consent coverage, transferable value, and training merit",
