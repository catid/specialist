#!/usr/bin/env python3
"""Remove a redundant Somerville-bowline limitation answer."""
...
DATA = ROOT / "data"
V246_DIR = DATA / "manual_reviews/context_merit_audit_v246"
sys.path[:0] = [str(ROOT), str(V246_DIR)]
import build_context_merit_audit_v246 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v247.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v247.jsonl"
REPORT = OUT_DIR / "report_context_merit_v247.json"
REVIEWER = "codex-context-merit-audit-v247"
...
SOURCE = DATA / "raw/rope365_f250f228cc370052.json"

SPECS = (
    {
        "fact_id": "fact-4123dd1f5c19a478083f",
        "active_index": 236,
        "markers": (
            "Another downside is that with a lot of pressure, the knot can compact and become difficult to untie, especially when the tail is under tension.",
        ),
        "decision": "drop",
        "reason_code": "drop_redundant_somerville_limitation_answer",
        "reason": "This limitation-only row repeats the compaction drawback already included in a stronger active question that also preserves the Somerville bowline's emergency-untying advantage.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v246",
    "direct_rows_without_prior_curation": 1,
    "redundant_somerville_limitation_rows_selected": 1,
    "rows": 496,
    "sha256": "c5ed754920b06f8b602427d5b0d340b8d9ff7cce00f873abecf9072051f56de5",
}
EXPECTED_OUTPUT_SHA256 = "80397e8f0396d455d6d83b34bf73e7c125559ebe47466dda2af4941578adb2f9"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v247-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v246-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v246.jsonl"
        baseline_report = directory / "v246.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 496 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v246 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v247 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "redundant_somerville_limitation_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v247",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 495 or observed["eval_fact_count"] != 612:
        raise ValueError("v247 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v247 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v246": 458,
            "active_after_this_tranche": 457,
        ...
        "schema": "context-merit-audit-report-v247",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a narrow Somerville-bowline limitation question whose drawback is already included in a more complete advantage-and-drawback row",
