#!/usr/bin/env python3
"""Replace a vague accident warning with a holistic box-tie Q&A."""
...
DATA = ROOT / "data"
V134_DIR = DATA / "manual_reviews/context_merit_audit_v134"
sys.path[:0] = [str(ROOT), str(V134_DIR)]
import build_context_merit_audit_v134 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v135.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v135.jsonl"
REPORT = OUT_DIR / "report_context_merit_v135.json"
REVIEWER = "codex-context-merit-audit-v135"
...
SOURCE = DATA / "raw/esinem_3bbb8f77f7d062a7.json"

SPECS = (
    {
        "fact_id": "fact-161395686733e749920d",
        "active_index": 358,
        "marker": "The person tied, their individual differences and the type of bondage planned are all factors to be considered since it is an holistic exercise.",
        "decision": "edit",
        ...
        "answer": "Consider the person being tied and their individual differences, the bondage planned, and whether the tie fulfills its intended function.",
        "reason_code": "replace_vague_accident_warning_with_holistic_factors",
        "reason": "The replacement turns an uninformative 'Accidents happen' answer into the source's bounded, transferable guidance to assess the person, plan, and tie function together.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v134",
    "direct_rows_without_prior_curation": 92,
    "esinem_holistic_box_tie_rows_selected": 1,
    "rows": 511,
    "sha256": "f7e9d820ad605e734c598810896828514363b7b2d2592dcd6ce64dd808c3510b",
}
EXPECTED_OUTPUT_SHA256 = "c02b9bca5947d89b6c839b8a416439160058f458454c9a30a6294ca15eebe0d7"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v135-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v134-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v134.jsonl"
        baseline_report = directory / "v134.report.json"
        ...
        if len(rows) != 511 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v134 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v135 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Esinem box-tie source lineage drift")
        audit = {
            ...
            "review_pass": "esinem_holistic_box_tie_factors_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v135",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 511 or observed["eval_fact_count"] != 612:
        raise ValueError("v135 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v135 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v134": 473,
            ...
            "projection_strategy": "delegate to the hermetic v134 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v135",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the vague accident-warning Q&A whose fully read source supports a bounded holistic-assessment lesson",
                "score": "manual source support, safety context, specificity, transferable value, and training merit",
