#!/usr/bin/env python3
"""Restore the owner's custom-order note to the RW Rope resource row."""
...
DATA = ROOT / "data"
V216_DIR = DATA / "manual_reviews/context_merit_audit_v216"
sys.path[:0] = [str(ROOT), str(V216_DIR)]
import build_context_merit_audit_v216 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v217.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v217.jsonl"
REPORT = OUT_DIR / "report_context_merit_v217.json"
REVIEWER = "codex-context-merit-audit-v217"
...
SPECS = (
    {
        "fact_id": "fact-f37fe09cfbc59840bb8d",
        "active_index": 261,
        "markers": (
        ...
        "decision": "edit",
        "question": "What is the owner-supplied synthetic-upline rope resource, and what should someone confirm about custom orders?",
        "answer": "RW Rope: https://www.rwrope.com/. The supplied directory says custom orders are accepted, but current availability is volatile and should be confirmed with the vendor.",
        "reason_code": "restore_rw_rope_custom_order_note",
        "reason": "The original answer preserved the URL but omitted the owner's useful custom-order note. The replacement restores it while explicitly flagging availability as volatile and retaining the exact resource URL.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v216",
    "direct_rows_without_prior_curation": 27,
    "rw_rope_custom_order_rows_selected": 1,
    "rows": 505,
    "sha256": "9255cba2f805f916000492fb615a5cf10ee1f22a2e9658cc3f9f06179932cc46",
}
EXPECTED_OUTPUT_SHA256 = "533de076356b5e5c432665254679ba372a610f71966c6ffdfd0cd25a74f11c2e"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v217-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v216-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v216.jsonl"
        baseline_report = directory / "v216.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v216 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v217 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "rw_rope_custom_order_note_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v217",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v217 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v217 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v216": 467,
            ...
            "projection_strategy": "delegate to the hermetic v216 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v217",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated RW Rope link row that omitted the owner's supplied custom-order note",
                "score": "owner-source support, resource usefulness, volatility qualification, and URL preservation",
