#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v22."""
...
import build_context_merit_audit_v22 as builder
from qa_quality import normalize_text


class ContextMeritAuditV22Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v22-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v22.jsonl"),
            ("CURATION", "pending_curation_context_merit_v22.jsonl"),
            ("REPORT", "report_context_merit_v22.json"),
        ):
    ...
    def test_secondary_selection_is_exact_and_ranked(self) -> None:
        # The cumulative curation ledger is the durable representation of the
        # v21 projection; do not make this regression test depend on /tmp.
        active = builder.read_jsonl(builder.ACTIVE_DATASET)
        ranked = builder.secondary_ranked(active)
        self.assertEqual(len(ranked), 230)
        self.assertEqual(
    ...
    def test_every_selection_is_one_prior_keep_not_rereviewed_in_v21(self) -> None:
        prior = {}
        for version, path in enumerate(builder.CONTEXT_AUDITS[:-1], 1):
        ...
        self.assertFalse(v21 & set(builder.EXPECTED_SELECTION))
    ...
    def test_projected_baseline_and_lineage_are_pinned(self) -> None:
        ...
        self.assertEqual(baseline["rows"], 640)
        self.assertEqual(
            baseline["sha256"],
    ...
    def test_decisions_and_curation(self) -> None:
        ...
        self.assertEqual(
            ...
            {"keep": 11, "drop": 7, "edit": 7})
        self.assertEqual(len(self.curation), 14)
        self.assertEqual(
            {action: sum(row["action"] == action for row in self.curation)
             for action in ("drop", "edit")}, {"drop": 7, "edit": 7})
    ...
    def test_report_projection_and_sealed_policy(self) -> None:
        report = json.loads(builder.REPORT.read_text())
        self.assertEqual(report["schema"], "context-merit-audit-report-v22")
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 7, "edit": 7, "keep": 11})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 633)
        self.assertEqual(
            projection["output_sha256"],
            "a990d08a9175a8b7976348c09c66e57fc502dd1b3ec59f18115d2db256e2da69")
        self.assertEqual(projection["reviewed_keep_fact_ids_preserved"], 11)
