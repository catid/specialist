#!/usr/bin/env python3
"""Add BightBound's terminology rationale to its style description."""
...
DATA = ROOT / "data"
V205_DIR = DATA / "manual_reviews/context_merit_audit_v205"
sys.path[:0] = [str(ROOT), str(V205_DIR)]
import build_context_merit_audit_v205 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v206.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v206.jsonl"
REPORT = OUT_DIR / "report_context_merit_v206.json"
REVIEWER = "codex-context-merit-audit-v206"
...
SOURCE = DATA / "raw/bightbound_faq_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 47,
        "markers": (
            "It teaches a modern fusion of rope-bondage styles heavily inspired by Japanese Kinbaku, often called Shibari in the West.",
        ),
        "decision": "edit",
        "question": "How does BightBound describe its rope style, and why does it generally avoid Japanese terminology?",
        "answer": "It teaches a modern fusion heavily inspired by Japanese Kinbaku, often called Shibari in the West, but says Japanese terms are frequently misused and can approach cultural appropriation.",
        "reason_code": "pair_style_description_with_terminology_rationale",
        "reason": "The original answer preserved the style lineage but omitted the FAQ's explicit terminology choice. The replacement adds that rationale without changing the attributed description of BightBound's practice.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v205",
    "bightbound_style_rows_selected": 1,
    "direct_rows_without_prior_curation": 38,
    "rows": 505,
    "sha256": "0ba3ce60bc0de8e0b7a3e8df547d360a8445c7ade716e5484ff04e788dd47148",
}
EXPECTED_OUTPUT_SHA256 = "3a6ea573a0fdeaf17053eecdc356e3f0fd44152d643b882af585e60861fa48d3"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v206-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v205-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v205.jsonl"
        baseline_report = directory / "v205.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v205 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v206 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: BightBound FAQ source lineage drift")
        audit = {
            ...
            "review_pass": "bightbound_style_terminology_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v206",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v206 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v206 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v205": 467,
            ...
            "projection_strategy": "delegate to the hermetic v205 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v206",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated style-description row whose fully inspected FAQ also explains the educator's terminology choice",
                "score": "manual source support, cultural-context clarity, style completeness, and training merit",
