#!/usr/bin/env python3
"""Replace a one-word subspace answer with a contextual definition."""
...
DATA = ROOT / "data"
V214_DIR = DATA / "manual_reviews/context_merit_audit_v214"
sys.path[:0] = [str(ROOT), str(V214_DIR)]
import build_context_merit_audit_v214 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v215.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v215.jsonl"
REPORT = OUT_DIR / "report_context_merit_v215.json"
REVIEWER = "codex-context-merit-audit-v215"
...
SPECS = (
    {
        ...
        "active_index": 261,
        "markers": (
            "The article says many BDSM activities involve an exchange of power and control without pain or humiliation.",
            "It describes a corresponding trance-like state as subspace for a submissive and domspace for a dominant, in a passage that also mentions endorphin effects comparable to a runner's high or post-orgasm afterglow.",
        ),
        "decision": "edit",
        "question": "What does the BDSM article mean by subspace, and what term does it pair with the dominant's analogous state?",
        "answer": "It uses subspace for a submissive's trance-like state and domspace for the dominant's analogous state, in a discussion of power exchange and endorphin effects.",
        "reason_code": "replace_bare_subspace_term_with_context",
        "reason": "The original one-word answer tested only term recall. The replacement preserves the term while defining it, pairing it with the analogous dominant-state term, and retaining the article's surrounding context.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v214",
    "direct_rows_without_prior_curation": 29,
    "wikipedia_subspace_context_rows_selected": 1,
    "rows": 505,
    "sha256": "411e56599311a9f857e5ab6be481cee190bf18bfebe2a5feb34f08fde2557076",
}
EXPECTED_OUTPUT_SHA256 = "1f39a56011198abd020f2f698aa9ad7b113bc4aec880c40663ba91395aea5a04"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v215-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v214-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v214.jsonl"
        baseline_report = directory / "v214.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v214 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v215 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "wikipedia_subspace_context_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v215",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v215 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v215 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v214": 467,
            ...
            "projection_strategy": "delegate to the hermetic v214 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v215",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated one-word subspace row whose inspected source also defines the paired dominant-state term and surrounding context",
                "score": "manual source support, definitional usefulness, context completeness, and training merit",
