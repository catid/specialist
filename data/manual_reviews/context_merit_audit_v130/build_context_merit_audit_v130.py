#!/usr/bin/env python3
"""Remove a low-merit decorative-knot strand-count lookup."""
...
DATA = ROOT / "data"
V129_DIR = DATA / "manual_reviews/context_merit_audit_v129"
sys.path[:0] = [str(ROOT), str(V129_DIR)]
import build_context_merit_audit_v129 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v130.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v130.jsonl"
REPORT = OUT_DIR / "report_context_merit_v130.json"
REVIEWER = "codex-context-merit-audit-v130"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__fc4c7dee8f9fdc985bf7.json"

SPECS = (
    {
        "fact_id": "fact-1f256b465516a6a21ba6",
        "active_index": 107,
        "marker": "This knot is an interesting variation of the crown knot, a type of button knot tied with three stands.",
        "decision": "drop",
        "reason_code": "low_merit_triple_crown_strand_count",
        "reason": "This asks only for a strand count and preserves none of the knot's construction, use, security limits, or decorative application.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v129",
    "direct_rows_without_prior_curation": 92,
    "rope365_triple_crown_count_rows_selected": 1,
    "rows": 515,
    "sha256": "0d1293f93bde58ea0df221217036ab6215198851ed88d05f5879f0b2c46b7f58",
}
EXPECTED_OUTPUT_SHA256 = "2d53ea642a434fc1b55965c9632f9b692380bc2dbbd241b1e6901703041a587c"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v130-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v129-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v129.jsonl"
        baseline_report = directory / "v129.report.json"
        ...
        if len(rows) != 515 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v129 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v130 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: chest-knots source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_triple_crown_count_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v130",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 514 or observed["eval_fact_count"] != 612:
        raise ValueError("v130 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v130 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v129": 477,
            "active_after_this_tranche": 476,
            ...
            "projection_strategy": "delegate to the hermetic v129 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v130",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the not-yet-context-audited strand-count Q&A from the fully read Rope365 chest-knots page",
