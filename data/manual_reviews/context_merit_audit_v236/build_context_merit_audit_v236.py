#!/usr/bin/env python3
"""Connect the ROPECRAFT link to its education and support model."""
...
DATA = ROOT / "data"
V235_DIR = DATA / "manual_reviews/context_merit_audit_v235"
sys.path[:0] = [str(ROOT), str(V235_DIR)]
import build_context_merit_audit_v235 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v236.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v236.jsonl"
REPORT = OUT_DIR / "report_context_merit_v236.json"
REVIEWER = "codex-context-merit-audit-v236"
...
SOURCE = DATA / "raw/owner_ropecraft_resource_20260714.json"

SPECS = (
    {
        "fact_id": "fact-fb98f437712003cfcb5e",
        "active_index": 375,
        "markers": (
            "The supplied directory recommends ROPECRAFT as a United States rope convention resource: https://ropecraft.net/.",
            "The inspected site describes education for tops, bottoms, and switches across Western bondage, Kinbaku, Shibari, solo tying, power exchange, and health or wellness, with fundamentals for beginners and advanced material for experienced attendees.",
            "It says in-person events integrate consent culture and diversity and include a participant-driven Ropenspace, peer support, special-interest groups, dedicated practice and self-tying spaces, nightly parties, custom frames and equipment, and a vendor bazaar.",
        ),
        "decision": "edit",
        "question": "Where can someone learn about owner-recommended ROPECRAFT, and what education and community-support features does it describe?",
        "answer": "ROPECRAFT is at https://ropecraft.net/. It describes beginner-through-advanced education for tops, bottoms, and switches across several rope styles, plus a participant-driven Ropenspace, peer support, special-interest groups, practice and self-tying spaces, and an inclusive consent-oriented event model.",
        "reason_code": "connect_ropecraft_link_to_education_support",
        "reason": "The original answer preserved only the convention link. The replacement keeps the exact owner-recommended URL and adds the inspected education range, roles, peer-learning, and support structure.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v235",
    "direct_rows_without_prior_curation": 8,
    "ropecraft_resource_support_rows_selected": 1,
    "rows": 505,
    "sha256": "4b939b5dbaf6b6e850dc41fce0c0b6a9795aa8d3eaf672b5e32a46c0234ee722",
}
EXPECTED_OUTPUT_SHA256 = "a2ea7976a9d76dcad31e6d486b4620c7fd271dcca467afc203d29254eba4f41c"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v236-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v235-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v235.jsonl"
        baseline_report = directory / "v235.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v235 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v236 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "ropecraft_resource_support_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v236",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v236 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v236 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v235": 467,
            ...
            "projection_strategy": "delegate to the hermetic v235 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v236",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended ROPECRAFT link row whose inspected site explains education and community support",
                "score": "owner-source support, attendee-orientation usefulness, support completeness, and URL preservation",
