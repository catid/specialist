#!/usr/bin/env python3
"""Replace a generic stopper-knot fact with a Rope365 tightening warning."""
...
DATA = ROOT / "data"
V85_DIR = DATA / "manual_reviews/context_merit_audit_v85"
sys.path[:0] = [str(ROOT), str(V85_DIR)]
import build_context_merit_audit_v85 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v86.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v86.jsonl"
REPORT = OUT_DIR / "report_context_merit_v86.json"
REVIEWER = "codex-context-merit-audit-v86"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__a53247111d1623ea270f.json"

SPECS = (
    {
        "fact_id": "fact-888004ffb3d5f1de2543",
        "active_index": 510,
        "marker": "Since it’s a type of slip knot, it comes with the risk that it may tighten when put directly on the body.",
        "decision": "edit",
        "question": "What risk does Rope365 identify when a handcuff knot is placed directly on the body?",
        "answer": "Because it is a slip knot, it may tighten on the body.",
        "reason_code": "replace_stopper_fact_with_handcuff_knot_warning",
        "reason": "The revision replaces a generic rope-end fact with the source's more important warning that a handcuff knot can tighten on the body.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v85",
    "direct_rows_without_prior_curation": 120,
    "rope365_more_knots_rows_selected": 1,
    "rows": 536,
    "sha256": "083cb9e99df4be7917351377970ed8d624a53f337f57bd3df3597973118c5af4",
}
EXPECTED_OUTPUT_SHA256 = "fc969da7d8f648339366d6dacdd70bf86e6b8652b0ffb40dcd66142e32433f05"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v86-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v85-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v85.jsonl"
        baseline_report = directory / "v85.report.json"
        ...
        if len(rows) != 536 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v85 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v86 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["document_sha256"] != document["document_sha256"]:
            raise ValueError(f"{spec['fact_id']}: more-knots source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_more_knots_safety_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v86",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 536 or observed["eval_fact_count"] != 612:
        raise ValueError("v86 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v86 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v85": 498,
        ...
        "schema": "context-merit-audit-report-v86",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the remaining direct Q&A from the fully read Rope365 more-knots page",
