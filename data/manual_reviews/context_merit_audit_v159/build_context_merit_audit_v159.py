#!/usr/bin/env python3
"""Replace a suspension label lookup with the source's readiness warning."""
...
DATA = ROOT / "data"
V158_DIR = DATA / "manual_reviews/context_merit_audit_v158"
sys.path[:0] = [str(ROOT), str(V158_DIR)]
import build_context_merit_audit_v158 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v159.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v159.jsonl"
REPORT = OUT_DIR / "report_context_merit_v159.json"
REVIEWER = "codex-context-merit-audit-v159"
...
SPECS = (
    {
        ...
        "active_index": 57,
        "marker": "Suspension is edge play. It is risky. Do not endanger yourself or others by suspending before you are ready.",
        "decision": "edit",
        "question": "Why does Anatomie Studio reserve its suspension points for experienced people?",
        "answer": "Because suspension is risky edge play, and attempting it before one is ready can endanger the person tying and the person suspended.",
        "reason_code": "replace_edge_play_label_with_suspension_readiness_warning",
        "reason": "The replacement explains the practical readiness restriction instead of asking for the isolated label “edge play.”",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v158",
    "direct_rows_without_prior_curation": 84,
    "anatomiestudio_suspension_readiness_rows_selected": 1,
    "rows": 508,
    "sha256": "9a909a4415c1f77766853f4131d99a05485c722a05bf3eac3ff235d0d671d887",
}
EXPECTED_OUTPUT_SHA256 = "36abe94280b60a3de1a5fcd6a958bf8fee0b501ff3f638f0b6c16ff93a82bb86"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v159-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v158-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v158.jsonl"
        baseline_report = directory / "v158.report.json"
        ...
        if len(rows) != 508 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v158 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v159 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "anatomiestudio_suspension_readiness_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v159",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 508 or observed["eval_fact_count"] != 612:
        raise ValueError("v159 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v159 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v158": 470,
            ...
            "projection_strategy": "delegate to the hermetic v158 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v159",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the suspension label Q&A whose fully read source supports a readiness and danger explanation",
                "score": "manual source support, safety usefulness, causal clarity, and training merit",
