#!/usr/bin/env python3
"""Remove a generic Rope365 video-reference disclaimer lookup."""
...
DATA = ROOT / "data"
V124_DIR = DATA / "manual_reviews/context_merit_audit_v124"
sys.path[:0] = [str(ROOT), str(V124_DIR)]
import build_context_merit_audit_v124 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v125.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v125.jsonl"
REPORT = OUT_DIR / "report_context_merit_v125.json"
REVIEWER = "codex-context-merit-audit-v125"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__5dbad9c884b98d608a5d.json"

SPECS = (
    {
        "fact_id": "fact-dbcf0cdaff9905970dc0",
        "active_index": 366,
        "marker": "Note that video references may display risky or unstable techniques, viewer discernment is advised.",
        "decision": "drop",
        "reason_code": "generic_video_disclaimer_lookup",
        "reason": "This tests page boilerplate about external videos rather than the frog tie's definition, construction, risk controls, or a concrete resource URL.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v124",
    "direct_rows_without_prior_curation": 92,
    "rope365_frog_video_disclaimer_rows_selected": 1,
    "rows": 519,
    "sha256": "d3cd46e07eca8e4f6570448645324b02a34b45c03aac896292496ed4210f4bf7",
}
EXPECTED_OUTPUT_SHA256 = "695d18de0d96c57c3a939893f6864954e8c8e817c30d0e56e5886ea79354e1a4"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v125-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v124-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v124.jsonl"
        baseline_report = directory / "v124.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 519 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v124 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v125 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: frog-tie source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_frog_video_disclaimer_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v125",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 518 or observed["eval_fact_count"] != 612:
        raise ValueError("v125 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v125 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v124": 481,
            "active_after_this_tranche": 480,
            ...
            "projection_strategy": "delegate to the v124 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v125",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the not-yet-context-audited boilerplate-warning Q&A from the fully read Rope365 frog-tie page",
                "score": "manual source support, boilerplate risk, transferable value, and training merit",
