#!/usr/bin/env python3
"""Focused regression tests for the Rope365 rope-material audit v83."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v83 as b


class V83(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v83-", dir=HERE)
    ...
    def test_04_direct(self):
        self.assertEqual(sum(not row.get("curation") for row in self.baseline), 123)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("rope365_rope_material_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v83", {"edit": 1, "keep": 2}, b.EXPECTED_OUTPUT_SHA256),
        )
