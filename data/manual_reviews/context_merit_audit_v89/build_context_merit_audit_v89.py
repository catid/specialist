#!/usr/bin/env python3
"""Audit the remaining direct Rope365 coiling rows."""
...
DATA = ROOT / "data"
V88_DIR = DATA / "manual_reviews/context_merit_audit_v88"
sys.path[:0] = [str(ROOT), str(V88_DIR)]
import build_context_merit_audit_v88 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v89.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v89.jsonl"
REPORT = OUT_DIR / "report_context_merit_v89.json"
REVIEWER = "codex-context-merit-audit-v89"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__2db0b0e55d35bf0aa676.json"

SPECS = (
    {
        "fact_id": "fact-3bc8480b6200731486c6",
        "active_index": 61,
        "marker": "If you want to be extra gentle with your ropes, you can keep the knot loose.",
        "decision": "keep",
        "reason_code": "retain_loose_knot_storage_guidance",
        "reason": "The existing attributed Q&A clearly preserves the source's practical advice for fragile natural rope and longer-term storage.",
    },
    {
        "fact_id": "fact-a236300d1e531bd987b5",
        "active_index": 438,
        "marker": "The overhand hank coiling method is easy, fast and doesn’t add unnecessary twists and stress on the rope for storage.",
        "decision": "edit",
        "question": "Why does Rope365 recommend the overhand hank for coiling rope?",
        "answer": "It is easy and fast, and it avoids adding unnecessary twists and stress during storage.",
        "reason_code": "replace_method_name_lookup_with_coiling_benefits",
        "reason": "The revision turns a bare method-name answer into the source's reusable explanation of the overhand hank's storage benefits.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v88",
    "direct_rows_without_prior_curation": 116,
    "rope365_coiling_rows_selected": 2,
    "rows": 535,
    "sha256": "e178b79e37c6f79066677d22933c5f0505379bb149e2342844cf07e4a620b69c",
}
EXPECTED_OUTPUT_SHA256 = "2eb89198918a2ab0efd04e6a537729ebda8499c709cbc83afdee85cda28dfdcd"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v89-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v88-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v88.jsonl"
        baseline_report = directory / "v88.report.json"
        ...
        if len(rows) != 535 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v88 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v89 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["document_sha256"] != document["document_sha256"]:
            raise ValueError(f"{spec['fact_id']}: coiling-source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_coiling_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v89",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 535 or observed["eval_fact_count"] != 612:
        raise ValueError("v89 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v89 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v88": 497,
            "active_after_this_tranche": 497,
        ...
        "schema": "context-merit-audit-report-v89",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the two remaining direct Q&A from the fully read Rope365 coiling page",
                "score": "manual source support, answer grammar, storage safety, and practical merit review",
