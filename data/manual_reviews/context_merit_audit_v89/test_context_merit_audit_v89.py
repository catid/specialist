#!/usr/bin/env python3
"""Focused regression tests for Rope365 coiling audit v89."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v89 as b


class V89(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v89-", dir=HERE)
    ...
    def test_04_direct(self):
        self.assertEqual(sum(not row.get("curation") for row in self.baseline), 116)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("rope365_coiling_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v89", {"edit": 1, "keep": 1}, b.EXPECTED_OUTPUT_SHA256),
        )
