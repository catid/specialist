#!/usr/bin/env python3
"""Replace a style-name lookup with a communication-centered explanation."""
...
DATA = ROOT / "data"
V259_DIR = DATA / "manual_reviews/context_merit_audit_v259"
sys.path[:0] = [str(ROOT), str(V259_DIR)]
import build_context_merit_audit_v259 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v260.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v260.jsonl"
REPORT = OUT_DIR / "report_context_merit_v260.json"
REVIEWER = "codex-context-merit-audit-v260"
...
SOURCE = DATA / "raw/kinbakutoday_5a3f9c2342298380.json"

SPECS = (
    {
        "fact_id": "fact-bcf65af4c8ebdc4ebb82",
        "active_index": 416,
        "markers": (
            "“Communication” is the key word. Yukimura sensei communicates with rope. His Kinbaku transmits messages to his partners. Messages of love, of sensuality, of power, of whatever he wishes. This is why the pictures of his models are always unique. The lovely women he ties and photographs never seem to “model”. Instead, they express their true feelings and so expose their true selves to the camera.",
        ),
        "decision": "edit",
        "question": "Why does the preface to “Affection of Kinbaku” call communication the key to Yukimura Haruki’s caressing style?",
        "answer": "It says his rope conveys emotion and intent to his partners, drawing genuine expression rather than merely posed modeling.",
        "reason_code": "replace_style_lookup_with_communication_explanation",
        "reason": "The replacement trades a bare style-name lookup for the preface's substantive explanation of how Yukimura's rope communicates with partners and elicits genuine expression.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v259",
    "direct_rows_without_prior_curation": 1,
    "style_name_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "ce44814eccae520e0cfcdd308928499bab48fca7875e90de91e27e37bbaabdc7",
}
EXPECTED_OUTPUT_SHA256 = "e838a242f86ce45cea24e710ef806670dcdc54e1edab02259785827cae6dd994"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v260-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v259-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v259.jsonl"
        baseline_report = directory / "v259.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v259 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v260 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "communication_over_style_lookup_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v260",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v260 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v260 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v259": 454,
        ...
        "schema": "context-merit-audit-report-v260",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a style-name lookup whose source passage supports a communication-centered explanation",
                "score": "relational and interpretive value, source support, question naturalness, and reduced label trivia",
