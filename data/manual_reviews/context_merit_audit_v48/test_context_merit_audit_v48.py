#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v48."""
...
import build_context_merit_audit_v48 as builder
from qa_quality import normalize_text


class ContextMeritAuditV48Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v48-", dir=HERE)
    ...
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(prefix=".run-v48-", dir=HERE)
    ...
    def test_baseline_is_exact_v47_projection(self) -> None:
        self.assertEqual(len(self.baseline), 549)
    ...
    def test_selection_is_exact_deterministic_top_ten(self) -> None:
        ...
        self.assertEqual(len(ranked), 86)
    ...
    def test_report_counts_projection_and_sealed_policy(self) -> None:
        ...
        self.assertEqual(report["schema"], "context-merit-audit-report-v48")
        ...
        self.assertEqual((projection["output_rows"], projection["output_sha256"]),
                         (548, "1c585357d18896cdd0264ce0502dfb600a2c2bf76f6c726e3b74b46d4902a8f7"))
    ...
    def test_future_versions_cannot_enter_v48(self) -> None:
        self.assertEqual((len(builder.CONTEXT_CURATIONS),
                          len(builder.CONTEXT_AUDITS)), (47, 47))
        self.assertIn("context_merit_audit_v47",
                      str(builder.CONTEXT_CURATIONS[-1]))
        self.assertFalse(any("context_merit_audit_v48" in str(path)
                             for path in (*builder.CONTEXT_CURATIONS,
                                          *builder.CONTEXT_AUDITS)))


class IsolatedProjectionV48Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temp = tempfile.TemporaryDirectory(prefix=".projection-v48-", dir=HERE)
        output = Path(cls.temp.name)
    ...
    def test_projection_is_byte_identical_and_hash_pinned(self) -> None:
        self.assertEqual([hashlib.sha256(path.read_bytes()).hexdigest()
                         ...
                         ["1c585357d18896cdd0264ce0502dfb600a2c2bf76f6c726e3b74b46d4902a8f7"] * 2)
    ...
    def test_projection_has_unique_ids_and_questions(self) -> None:
        self.assertEqual(len({row["fact_id"] for row in self.rows}), 548)
        self.assertEqual(len({row["question"] for row in self.rows}), 548)
