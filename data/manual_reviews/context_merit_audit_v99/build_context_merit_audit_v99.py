#!/usr/bin/env python3
"""Replace an Esinem term lookup with a high-stranding failure warning."""
...
DATA = ROOT / "data"
V98_DIR = DATA / "manual_reviews/context_merit_audit_v98"
sys.path[:0] = [str(ROOT), str(V98_DIR)]
import build_context_merit_audit_v98 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v99.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v99.jsonl"
REPORT = OUT_DIR / "report_context_merit_v99.json"
REVIEWER = "codex-context-merit-audit-v99"
...
SOURCE = DATA / "raw/esinem_b249eddc1f5e1864.json"

SPECS = (
    {
        "fact_id": "fact-38c6234e2a72dd14287f",
        "active_index": 339,
        "marker": "Catch any imbalance early or you could end up with rope fit for the bin",
        "decision": "edit",
        "question": "Why does Esinem advise catching high-stranding early?",
        "answer": "A severe imbalance can make the rope irrecoverable unless it is dismantled into its three strands and remade.",
        "reason_code": "replace_high_stranding_term_lookup_with_failure_warning",
        "reason": "The revision replaces a bare terminology lookup with the source's practical consequence of allowing strand imbalance to become severe.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v98",
    "direct_rows_without_prior_curation": 106,
    "esinem_high_stranding_rows_selected": 1,
    "rows": 532,
    "sha256": "efe4e6b8987e10ab648423633c42b6650be67ee8a927ced19742ec3c8229d37b",
}
EXPECTED_OUTPUT_SHA256 = "6124678ce2039db8f615d751410c5dcc514e0ce311804705d5cc720e339a639e"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v99-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v98-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v98.jsonl"
        baseline_report = directory / "v98.report.json"
        ...
        if len(rows) != 532 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v98 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v99 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Esinem high-stranding source lineage drift")
        audit = {
            ...
            "review_pass": "esinem_high_stranding_merit_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v99",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 532 or observed["eval_fact_count"] != 612:
        raise ValueError("v99 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v99 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v98": 494,
        ...
        "schema": "context-merit-audit-report-v99",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the remaining direct Q&A from the fully read Esinem rope-problems article",
                "score": "manual source support, maintenance actionability, failure consequence, and practical merit review",
