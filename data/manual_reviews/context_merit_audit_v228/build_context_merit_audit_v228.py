#!/usr/bin/env python3
"""Connect the House of Bound link to its visible catalog and delivery."""
...
DATA = ROOT / "data"
V227_DIR = DATA / "manual_reviews/context_merit_audit_v227"
sys.path[:0] = [str(ROOT), str(V227_DIR)]
import build_context_merit_audit_v227 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v228.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v228.jsonl"
REPORT = OUT_DIR / "report_context_merit_v228.json"
REVIEWER = "codex-context-merit-audit-v228"
...
SOURCE = DATA / "raw/owner_house_of_bound_resource_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 386,
        "markers": (
            "The supplied directory recommends Lief Bound's purchasable rope tutorials at https://www.houseofbound.com/tutorials.",
            "The inspected public page directs buyers to the House of Bound Etsy shop and says purchased videos are shared through Vimeo for continued access.",
            "Visible titles include Lief's Hug Harness, Lief's Booty Basket, Lief's Booty Basket Attachment Methods, Lief's Half Mermaid, Lief's Calf Binder, Lief's Ballerina Boot, Lief's Arms Free Chest Harness, and Lief's Rope Conditioning Tutorial.",
        ),
        "decision": "edit",
        "question": "Where can someone find Lief Bound's purchasable rope tutorials, and how does House of Bound say bought videos are accessed?",
        "answer": "The catalog is at https://www.houseofbound.com/tutorials. The page directs buyers to its Etsy shop and says purchased videos are shared through Vimeo for continued access; visible topics include harnesses, binders, attachment methods, and rope conditioning.",
        "reason_code": "connect_house_of_bound_link_to_catalog_access",
        "reason": "The original answer preserved only the tutorial link. The replacement keeps the exact owner-recommended URL and adds the inspected purchase, delivery, and visible topic context.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v227",
    "direct_rows_without_prior_curation": 16,
    "house_of_bound_resource_access_rows_selected": 1,
    "rows": 505,
    "sha256": "7cebdbdf63823ba89de94be14733450e15b09e9b3338ef25e988ee4790b20019",
}
EXPECTED_OUTPUT_SHA256 = "33c18701e1b3aa346639f7315d9a0fce1988ba0c730a4133b67b2581150858d7"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v228-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v227-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v227.jsonl"
        baseline_report = directory / "v227.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v227 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v228 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "house_of_bound_resource_access_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v228",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v228 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v228 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v227": 467,
            ...
            "projection_strategy": "delegate to the hermetic v227 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v228",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated owner-recommended House of Bound link row whose inspected page exposes purchase, delivery, and topic context",
                "score": "owner-source support, buyer-orientation usefulness, catalog completeness, and URL preservation",
