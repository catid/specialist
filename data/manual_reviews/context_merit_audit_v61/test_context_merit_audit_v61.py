#!/usr/bin/env python3
"""Focused regression tests for name/term naturalness audit v61."""
import hashlib,json,sys,tempfile,unittest
from pathlib import Path
HERE=Path(__file__).resolve().parent;sys.path.insert(0,str(HERE));import build_context_merit_audit_v61 as b
class V61(unittest.TestCase):
 @classmethod
 def setUpClass(cls):
  cls.temp=tempfile.TemporaryDirectory(prefix=".test-v61-",dir=HERE);o=Path(cls.temp.name);cls.base=o/"base.jsonl";cls.br=o/"base.report.json";b.build_projection(cls.base,cls.br,b.PRIOR_PROJECTION_CURATIONS);cls.baseline=b.read_jsonl(cls.base);cur=(*b.PRIOR_PROJECTION_CURATIONS,b.CURATION);cls.ds=[]
 ...
 def test_06_direct_count(self):self.assertEqual(sum(not r.get("curation") for r in self.baseline),201)
 def test_07_decisions(self):self.assertEqual({x:sum(r["decision"]==x for r in self.audit) for x in ("keep","drop","edit")},{"keep":0,"drop":0,"edit":8})
 def test_08_curation(self):self.assertEqual((len(self.curation),{r["action"] for r in self.curation}),(8,{"edit"}))
 ...
 def test_11_lineage(self):
  for r in self.audit:self.assertEqual((r["review_pass"],r["projection_lineage"]["baseline_rows"],r["projection_lineage"]["baseline_sha256"]),("generic_name_and_term_naturalness_reaudit",537,b.PROJECTED_SELECTION_BASELINE["sha256"]))
 def test_12_report(self):self.assertEqual((self.report["schema"],self.report["audit"]["by_decision"],self.report["new_pending_curation"]["edit_support_types"]),("context-merit-audit-report-v61",{"edit":8},{"extractive":8,"manual_paraphrase":0}))
 def test_13_report_projection(self):self.assertEqual((self.report["isolated_build_projection"]["output_rows"],self.report["isolated_build_projection"]["output_sha256"]),(537,"a8603dae26aec6c1882cb3bc26f5f0644390562fae82998bf46d10653c71d4e7"))
 def test_14_prior_pins(self):
  pins=self.report["frozen_prior_decision_artifacts"];self.assertEqual(len(pins),180)
  for r in pins:self.assertEqual(b.file_sha256(b.ROOT/r["path"]),r["sha256"])
 def test_15_projection_hash(self):self.assertEqual([hashlib.sha256(p.read_bytes()).hexdigest() for p in self.ds],["a8603dae26aec6c1882cb3bc26f5f0644390562fae82998bf46d10653c71d4e7"]*2)
