#!/usr/bin/env python3
"""Restore the owner's RW Rope note to the combined synthetic supplier row."""
...
DATA = ROOT / "data"
V217_DIR = DATA / "manual_reviews/context_merit_audit_v217"
sys.path[:0] = [str(ROOT), str(V217_DIR)]
import build_context_merit_audit_v217 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v218.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v218.jsonl"
REPORT = OUT_DIR / "report_context_merit_v218.json"
REVIEWER = "codex-context-merit-audit-v218"
...
SOURCE = DATA / "raw/owner_synthetic_rope_resources_20260714.json"

SPECS = (
    {
        "fact_id": "fact-63283786303c4df91008",
        "active_index": 425,
        "markers": (
            "RW Rope is the supplied synthetic-upline rope resource: https://www.rwrope.com/.",
            "The supplied directory says custom orders are accepted, but treats current custom-order availability as volatile and advises verifying it with the vendor.",
            "ChromaKnotz is a supplied synthetic bondage-rope shop: https://chromaknotz.square.site/.",
            "Knot Head Nylon is a supplied synthetic bondage-rope shop: https://knotheadnylon.net/.",
        ),
        "decision": "edit",
        "question": "Which owner-recommended suppliers cover synthetic upline rope versus synthetic bondage rope, and what custom-order caveat applies?",
        "answer": "RW Rope supplies synthetic upline rope (https://www.rwrope.com/); the directory says it accepts custom orders, but current availability should be verified. ChromaKnotz (https://chromaknotz.square.site/) and Knot Head Nylon (https://knotheadnylon.net/) are the synthetic bondage-rope shops.",
        "reason_code": "restore_rw_note_in_synthetic_supplier_summary",
        "reason": "The original summary correctly separated upline and bondage-rope suppliers but omitted the owner's RW Rope custom-order note. The replacement restores it with a volatility caveat and preserves all three exact URLs.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v217",
    "direct_rows_without_prior_curation": 26,
    "synthetic_supplier_summary_rows_selected": 1,
    "rows": 505,
    "sha256": "533de076356b5e5c432665254679ba372a610f71966c6ffdfd0cd25a74f11c2e",
}
EXPECTED_OUTPUT_SHA256 = "f20b21066a0713dd693737613120f0864f831891a709561e4dbe5b73e39fbce2"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v218-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v217-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v217.jsonl"
        baseline_report = directory / "v217.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v217 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v218 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "synthetic_supplier_summary_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v218",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v218 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v218 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v217": 467,
            ...
            "projection_strategy": "delegate to the hermetic v217 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v218",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the combined synthetic-supplier row that omitted the owner's supplied RW Rope custom-order note",
                "score": "owner-source support, supplier-role clarity, volatility qualification, and URL preservation",
