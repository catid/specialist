#!/usr/bin/env python3
"""Complete the remaining Rope365 one-rope box-tie pressure guidance."""
...
DATA = ROOT / "data"
V101_DIR = DATA / "manual_reviews/context_merit_audit_v101"
sys.path[:0] = [str(ROOT), str(V101_DIR)]
import build_context_merit_audit_v101 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v102.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v102.jsonl"
REPORT = OUT_DIR / "report_context_merit_v102.json"
REVIEWER = "codex-context-merit-audit-v102"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__7c10995ad3190fa770df.json"

SPECS = (
    {
        "fact_id": "fact-531fe868840964d4d4af",
        "active_index": 427,
        "marker": "The rope pressure should be on the forearms. Avoid pressure directly on the wrists or hands.",
        "decision": "edit",
        "question": "Where does Rope365 say rope pressure should and should not sit around the wrists and hands in a box tie?",
        "answer": "Place the pressure on the forearms, not directly on the wrists or hands.",
        "reason_code": "complete_forearm_pressure_guidance",
        "reason": "The revision preserves both halves of the source's safety instruction instead of answering only with the preferred location.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v101",
    "direct_rows_without_prior_curation": 103,
    "rope365_one_rope_box_tie_rows_selected": 1,
    "rows": 532,
    "sha256": "077a0a7022e390ee6c352a3e7d82e5566dd97b954b49cebdcc6eace02aced7ac",
}
EXPECTED_OUTPUT_SHA256 = "1e5b60510f8bf8878a0937f69bb20100e2ed6bb29381b9cf6bcf178b843f68d5"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v102-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v101-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v101.jsonl"
        baseline_report = directory / "v101.report.json"
        ...
        if len(rows) != 532 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v101 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v102 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: one-rope box-tie source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_one_rope_box_tie_safety_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v102",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 532 or observed["eval_fact_count"] != 612:
        raise ValueError("v102 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v102 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v101": 494,
        ...
        "schema": "context-merit-audit-report-v102",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the remaining direct Q&A from the fully read Rope365 one-rope box-tie page",
                "score": "manual source support, wrist and hand safety completeness, answer context, and practical merit review",
