#!/usr/bin/env python3
"""Replace a lesson-label lookup with Rope365's foundational tying sequence."""
...
DATA = ROOT / "data"
V84_DIR = DATA / "manual_reviews/context_merit_audit_v84"
sys.path[:0] = [str(ROOT), str(V84_DIR)]
import build_context_merit_audit_v84 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v85.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v85.jsonl"
REPORT = OUT_DIR / "report_context_merit_v85.json"
REVIEWER = "codex-context-merit-audit-v85"
...
SOURCE = DATA / "raw/rope_resources_v1/rope365__0297c98bd31c6b0d53d1.json"

SPECS = (
    {
        "fact_id": "fact-5b934d2edcb41402cdf8",
        "active_index": 476,
        "marker": "A single column tie to start in a safe way, then some frictions to tie with the rest of the rope, and from there we can add up.",
        "decision": "edit",
        "question": "What foundational sequence does Rope365 describe for building a tie?",
        "answer": "Start with a single-column tie, use frictions with the remaining rope, and then add other elements.",
        "reason_code": "replace_lesson_label_lookup_with_foundational_sequence",
        "reason": "The revision replaces schedule trivia with the source's reusable sequence for safely anchoring and developing a tie.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v84",
    "direct_rows_without_prior_curation": 121,
    "rope365_foundation_rows_selected": 1,
    "rows": 536,
    "sha256": "be0611d6c351e7d186b964017ef3da71bfacd608b1b877e35777a90e654420f5",
}
EXPECTED_OUTPUT_SHA256 = "083cb9e99df4be7917351377970ed8d624a53f337f57bd3df3597973118c5af4"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v85-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v84-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v84.jsonl"
        baseline_report = directory / "v84.report.json"
        ...
        if len(rows) != 536 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v84 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v85 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["document_sha256"] != document["document_sha256"]:
            raise ValueError(f"{spec['fact_id']}: foundation-source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_foundation_merit_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v85",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 536 or observed["eval_fact_count"] != 612:
        raise ValueError("v85 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v85 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v84": 498,
        ...
        "schema": "context-merit-audit-report-v85",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the remaining direct Q&A from the fully read Rope365 foundation page",
                "score": "manual source support, reusable instructional value, answer grammar, and practical merit review",
