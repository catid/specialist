#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v13."""
...
import build_context_merit_audit_v13 as builder
from qa_quality import normalize_text


class ContextMeritAuditV13Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v13-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v13.jsonl"),
            ("CURATION", "pending_curation_context_merit_v13.jsonl"),
            ("REPORT", "report_context_merit_v13.json"),
        ):
    ...
    def test_future_context_tranches_are_not_historical_inputs(self) -> None:
        ...
        prior = manual_root / "context_merit_audit_v12" / "synthetic.jsonl"
        ...
        self.assertEqual(
            ...
            frozenset(f"context_merit_audit_v{i}" for i in range(1, 13)),
        )
    ...
    def test_sources_and_evidence_are_hash_pinned(self) -> None:
        ...
        for spec in builder.SPECS:
            ...
            if row["source_support"] == "source_composite":
                self.assertTrue(all(normalize_text(part) in
    ...
    def test_report_projection_and_sealed_policy(self) -> None:
        ...
        self.assertEqual(report["schema"], "context-merit-audit-report-v13")
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 3, "edit": 8, "keep": 14})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 707)
        self.assertEqual(
            projection["output_sha256"],
            "5d09acd6c5c0b5aa31de6d6ab51e405318ef11fc7db9b76639ab31624c1b5e39")
    ...
    def test_generator_is_deterministic_and_prior_is_frozen(self) -> None:
        ...
        prior_files = tuple(
            path for version in range(1, 13)
