#!/usr/bin/env python3
"""Replace a one-word switch answer with a contextual role definition."""
...
DATA = ROOT / "data"
V215_DIR = DATA / "manual_reviews/context_merit_audit_v215"
sys.path[:0] = [str(ROOT), str(V215_DIR)]
import build_context_merit_audit_v215 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v216.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v216.jsonl"
REPORT = OUT_DIR / "report_context_merit_v216.json"
REVIEWER = "codex-context-merit-audit-v216"
...
SOURCE = DATA / "raw/wikipedia_bdsm_roles_states_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 318,
        "markers": (
            "The article distinguishes a top or dominant as the partner who performs or controls an activity and a bottom or submissive as the partner who receives or is controlled.",
            "It calls people who change between top or dominant and bottom or submissive roles switches, whether the change happens within one relationship or between relationships.",
        ),
        "decision": "edit",
        "question": "What is a switch in BDSM role terminology, and can switching occur within one relationship as well as between relationships?",
        "answer": "A switch is someone who changes between top or dominant and bottom or submissive roles; the term can apply when that change happens within one relationship or across different relationships.",
        "reason_code": "replace_bare_switch_term_with_context",
        "reason": "The original one-word answer tested only term recall. The replacement preserves the term, defines the role change, and clarifies the article's point that switching may occur within or between relationships.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v215",
    "direct_rows_without_prior_curation": 28,
    "wikipedia_switch_context_rows_selected": 1,
    "rows": 505,
    "sha256": "1f39a56011198abd020f2f698aa9ad7b113bc4aec880c40663ba91395aea5a04",
}
EXPECTED_OUTPUT_SHA256 = "9255cba2f805f916000492fb615a5cf10ee1f22a2e9658cc3f9f06179932cc46"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v216-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v215-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v215.jsonl"
        baseline_report = directory / "v215.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v215 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v216 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Wikipedia terminology source lineage drift")
        audit = {
            ...
            "review_pass": "wikipedia_switch_context_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v216",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v216 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v216 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v215": 467,
            ...
            "projection_strategy": "delegate to the hermetic v215 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v216",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated one-word switch row whose inspected source defines the role change and its relationship scope",
                "score": "manual source support, definitional usefulness, relationship-scope completeness, and training merit",
