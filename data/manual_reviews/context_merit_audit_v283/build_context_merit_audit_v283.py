#!/usr/bin/env python3
"""Normalize a clipped diamond-harness breathing-safety answer."""
...
DATA = ROOT / "data"
V282_DIR = DATA / "manual_reviews/context_merit_audit_v282"
sys.path[:0] = [str(ROOT), str(V282_DIR)]
import build_context_merit_audit_v282 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v283.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v283.jsonl"
REPORT = OUT_DIR / "report_context_merit_v283.json"
REVIEWER = "codex-context-merit-audit-v283"
...
SOURCE = DATA / "raw/rope365_6cc5a0fc438e230f.json"

SPECS = (
    {
        "fact_id": "fact-39971c0aab91fba7a804",
        "active_index": 218,
        "markers": (
            "Opening the diamonds will make the tie progressively tighter, watch out for the ability to breathe of the person being tied.",
        ),
        "decision": "edit",
        "question": "What should you monitor while opening the diamonds in a chest harness?",
        "answer": "Monitor the tied person's breathing as you open the diamonds, because the harness becomes progressively tighter.",
        "reason_code": "normalize_diamond_harness_breathing_answer",
        "reason": "The original question was ungrammatical and its answer was a bare noun phrase; the revision preserves the same source-supported safety guidance in a direct, self-contained form.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v282",
    "direct_rows_without_prior_curation": 1,
    "clipped_answer_rows_selected": 1,
    "rows": 492,
    "sha256": "239518ddbdf35eac7abd4ea94b3948f8805b3867b3378bca2afc300353fa61b5",
}
EXPECTED_OUTPUT_SHA256 = "83d14d9d42740c836b49a8ec9e4237766e9d751c827c21d4d2c79500ee4bc3b9"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v283-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v282-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v282.jsonl"
        baseline_report = directory / "v282.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v282 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v283 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "schema": "context-merit-audit-v283",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v283 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v283 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v282": 454,
        ...
        "schema": "context-merit-audit-report-v283",
