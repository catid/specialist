#!/usr/bin/env python3
"""Strengthen a tingling warning into the guide's conservative action."""
...
DATA = ROOT / "data"
V165_DIR = DATA / "manual_reviews/context_merit_audit_v165"
sys.path[:0] = [str(ROOT), str(V165_DIR)]
import build_context_merit_audit_v165 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v166.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v166.jsonl"
REPORT = OUT_DIR / "report_context_merit_v166.json"
REVIEWER = "codex-context-merit-audit-v166"
...
SPECS = (
    {
        ...
        "active_index": 319,
        "marker": "if you have tingles in any body part while in bondage, it is time to be untied. You can always be tied up again later.",
        "decision": "edit",
        ...
        "reason_code": "strengthen_tingling_warning_with_conservative_action",
        "reason": "The replacement preserves immediate communication and adds the source’s conservative untie-and-reassess response.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v165",
    "direct_rows_without_prior_curation": 78,
    "kinbakutoday_tingling_response_rows_selected": 1,
    "rows": 507,
    "sha256": "400fcc9edd26257bca5b832dc84ac23cf3b078639badbae45ed9c32ae25850f4",
}
EXPECTED_OUTPUT_SHA256 = "e378a4846dc2cc4794cc7d43d76a50ef673535804ab749320f0e2b1270b5d2ac"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v166-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v165-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v165.jsonl"
        baseline_report = directory / "v165.report.json"
        ...
        if len(rows) != 507 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v165 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v166 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "kinbakutoday_tingling_response_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v166",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 507 or observed["eval_fact_count"] != 612:
        raise ValueError("v166 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v166 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v165": 469,
            ...
            "projection_strategy": "delegate to the hermetic v165 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v166",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the tingling notification Q&A whose fully read source supports a more complete conservative response",
                "score": "manual source support, safety usefulness, actionability, and training merit",
