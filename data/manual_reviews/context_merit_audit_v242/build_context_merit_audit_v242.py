#!/usr/bin/env python3
"""Remove a duplicate kokoro-and-katachi answer."""
...
DATA = ROOT / "data"
V241_DIR = DATA / "manual_reviews/context_merit_audit_v241"
sys.path[:0] = [str(ROOT), str(V241_DIR)]
import build_context_merit_audit_v241 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v242.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v242.jsonl"
REPORT = OUT_DIR / "report_context_merit_v242.json"
REVIEWER = "codex-context-merit-audit-v242"
...
SOURCE = DATA / "raw/kinbakutoday_c799ecf7b51f866b.json"

SPECS = (
    {
        "fact_id": "fact-9ea4407b42aea1d2eaed",
        "active_index": 334,
        "markers": (
            "When we talk about a Ryuu, I think we are talking about both. The kokoro is the “why” and the katachi is the “how” of tying.",
        ),
        "decision": "drop",
        "reason_code": "drop_duplicate_kokoro_katachi_answer",
        "reason": "This row repeats the same answer and source sentence already represented by the more informative active question that asks for the meanings of both kokoro and katachi.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v241",
    "direct_rows_without_prior_curation": 1,
    "duplicate_kokoro_katachi_rows_selected": 1,
    "rows": 502,
    "sha256": "e58917644f91097010ce08ffb4e2fe8ce7531725d6f0eb54c7853abedaed3ab5",
}
EXPECTED_OUTPUT_SHA256 = "f70fc1d92a488548cd20235e61a4a621a820224cd6f4812f45d57f9fb19e063d"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v242-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v241-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v241.jsonl"
        baseline_report = directory / "v241.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 502 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v241 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v242 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "duplicate_kokoro_katachi_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v242",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 501 or observed["eval_fact_count"] != 612:
        raise ValueError("v242 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v242 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v241": 464,
            "active_after_this_tranche": 463,
        ...
        "schema": "context-merit-audit-report-v242",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "two active kokoro-and-katachi questions with the same answer and the same supporting source sentence",
