...
HERE=Path(__file__).resolve().parent;sys.path.insert(0,str(HERE));import build_context_merit_audit_v297 as b
class Test(unittest.TestCase):
 @classmethod
 def setUpClass(cls):
  cls.temp=tempfile.TemporaryDirectory(prefix=".test-v297-",dir=HERE);d=Path(cls.temp.name);cls.base=d/"base.jsonl";b.build_baseline(cls.base,d/"base.report.json");cls.out=d/"out.jsonl";cls.rep=d/"out.report.json";cls.datasets=[];cls.reports=[]
 ...
 def test_inputs(self):self.assertEqual((len(b.read_jsonl(self.base)),b.file_sha256(self.base)),(513,b.BASELINE_SHA256));self.assertEqual((len(self.add),b.file_sha256(b.ADDITIONS)),(3,b.EXPECTED_ADDITIONS_SHA256));self.assertEqual((len({r["url"]for r in self.add}),len({r["document_sha256"]for r in self.add})),(3,3))
 def test_audit(self):self.assertEqual((len(self.audit),{r["decision"]for r in self.audit},b.read_jsonl(b.CURATION)),(3,{"add"},[]))
 def test_projection(self):self.assertEqual((len(self.rows),b.file_sha256(self.out)),(516,b.EXPECTED_OUTPUT_SHA256));self.assertTrue({r["fact_id"]for r in self.add}.issubset({r["fact_id"]for r in self.rows}))
 ...
 def test_checkpoint(self):self.assertEqual(self.report["prior_checkpoint"]["candidate"],{"rows":513,"sha256":b.BASELINE_SHA256});self.assertTrue(all(b.file_sha256(b.ROOT/r["path"])==r["sha256"]for r in self.report["prior_checkpoint"]["artifacts"]))
 ...
 def test_report(self):self.assertEqual((self.report["schema"],self.report["isolated_build_projection"]["output_sha256"]),("context-merit-audit-report-v297",b.EXPECTED_OUTPUT_SHA256))
