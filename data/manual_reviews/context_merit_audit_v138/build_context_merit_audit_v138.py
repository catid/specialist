#!/usr/bin/env python3
"""Replace a one-word empathy lookup with its learning context."""
...
DATA = ROOT / "data"
V137_DIR = DATA / "manual_reviews/context_merit_audit_v137"
sys.path[:0] = [str(ROOT), str(V137_DIR)]
import build_context_merit_audit_v137 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v138.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v138.jsonl"
REPORT = OUT_DIR / "report_context_merit_v138.json"
REVIEWER = "codex-context-merit-audit-v138"
...
SOURCE = DATA / "raw/kinbakutoday_eb5b3fd68f12e8e9.json"

SPECS = (
    {
        "fact_id": "fact-8710838903494b21fe97",
        "active_index": 27,
        "marker": "Empathy is key to really understand kinbaku.",
        "secondary_marker": "We need maturity and life experience to see beyond the rope. When you can competently rig, there’s so much more to explore. I’m on that path and I’ve only been tying forty-one years. Still a beginner, still learning.",
        "decision": "edit",
        "question": "According to Sin, why is technical ability alone insufficient for understanding kinbaku?",
        "answer": "He says empathy, maturity, and life experience are needed to see beyond competent rigging, and that learning continues without end.",
        "reason_code": "replace_empathy_lookup_with_learning_context",
        "reason": "The replacement preserves the source's fuller point about empathy, maturity, experience, and continual learning instead of testing one abstract word.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v137",
    "direct_rows_without_prior_curation": 92,
    "kinbakutoday_empathy_learning_context_rows_selected": 1,
    "rows": 511,
    "sha256": "9ff0d5d80553afb56783dbc4c26bd79311f09fc1f2d00e32ddd5dbfd5b0d3edf",
}
EXPECTED_OUTPUT_SHA256 = "2392100463ed43aa345adb6dab1f3abd43c5611a17ab57929224b52707f619c2"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v138-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v137-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v137.jsonl"
        baseline_report = directory / "v137.report.json"
        ...
        if len(rows) != 511 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v137 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v138 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Rope and Sin interview source lineage drift")
        audit = {
            ...
            "review_pass": "kinbakutoday_empathy_learning_context_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v138",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 511 or observed["eval_fact_count"] != 612:
        raise ValueError("v138 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v138 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v137": 473,
            ...
            "projection_strategy": "delegate to the hermetic v137 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v138",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the one-word empathy Q&A whose fully read source provides a broader learning principle",
                "score": "manual source support, practitioner mindset, transferable value, and training merit",
