#!/usr/bin/env python3
"""Make Rope365's Angel Tie arm-tension warning self-contained."""
...
DATA = ROOT / "data"
V177_DIR = DATA / "manual_reviews/context_merit_audit_v177"
sys.path[:0] = [str(ROOT), str(V177_DIR)]
import build_context_merit_audit_v177 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v178.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v178.jsonl"
REPORT = OUT_DIR / "report_context_merit_v178.json"
REVIEWER = "codex-context-merit-audit-v178"
...
SOURCE = DATA / "raw/rope365_15518f0912cce205.json"
...
SPECS = (
    {
        ...
        "active_index": 155,
        "markers": (
            "the tension on the arm should be very loose in order to make this tie safe",
        ),
        "decision": "edit",
        "question": "How should arm tension be set in Rope365's Angel Tie, and why?",
        "answer": "Keep the tension on the arms very loose; Rope365 says this is necessary to make the tie safer.",
        "reason_code": "complete_angel_tie_arm_tension_warning",
        "reason": "The replacement makes the answer a complete instruction and restores the source's explicit safety rationale for very loose arm tension.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v177",
    "direct_rows_without_prior_curation": 66,
    "rope365_angel_tie_tension_rows_selected": 1,
    "rows": 506,
    "sha256": "b06297f966237cd92b8c5a5192b0ff08f1ffe5c00846bf472a18ca416f3b61c4",
}
EXPECTED_OUTPUT_SHA256 = "d36159240b7cb4755e4a7d7400e84943aef5f50998a1839a4b7024389fefbddf"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v178-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v177-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v177.jsonl"
        baseline_report = directory / "v177.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v177 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v178 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Rope365 elbow-ties source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_angel_tie_tension_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v178",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v178 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v178 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v177": 468,
            ...
            "projection_strategy": "delegate to the hermetic v177 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v178",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the correct but fragmentary Angel Tie precaution whose fully read source states its safety purpose",
                "score": "manual source support, safety usefulness, answer completeness, and training merit",
