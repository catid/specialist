#!/usr/bin/env python3
"""Clarify WykD's distinction between Ichinawa and one-rope practice."""
...
DATA = ROOT / "data"
V176_DIR = DATA / "manual_reviews/context_merit_audit_v176"
sys.path[:0] = [str(ROOT), str(V176_DIR)]
import build_context_merit_audit_v176 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v177.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v177.jsonl"
REPORT = OUT_DIR / "report_context_merit_v177.json"
REVIEWER = "codex-context-merit-audit-v177"
...
SOURCE = DATA / "raw/wykd_a74fec63b0114fff.json"
...
SPECS = (
    {
        ...
        "active_index": 102,
        "markers": (
            "as opposed to the Ichinawa technique which is a technique that does not exist just for practice",
        ),
        "decision": "edit",
        ...
        "reason_code": "clarify_ichinawa_versus_one_rope_practice",
        "reason": "The replacement preserves the source's distinction while making the contrast between a named technique and a practice constraint explicit.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v176",
    "direct_rows_without_prior_curation": 67,
    "wykd_ichinawa_distinction_rows_selected": 1,
    "rows": 506,
    "sha256": "fc2676d8d2fbd8a999daf334a6f3c93f5403b9235afb21d390d05c320c0e08fb",
}
EXPECTED_OUTPUT_SHA256 = "b06297f966237cd92b8c5a5192b0ff08f1ffe5c00846bf472a18ca416f3b61c4"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v177-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v176-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v176.jsonl"
        baseline_report = directory / "v176.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v176 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v177 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: WykD Ichinawa source lineage drift")
        audit = {
            ...
            "review_pass": "wykd_ichinawa_distinction_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v177",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v177 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v177 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v176": 468,
            ...
            "projection_strategy": "delegate to the hermetic v176 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v177",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the awkward Ichinawa fragment whose fully read source supports a clear contrast with one-rope practice",
                "score": "manual source support, conceptual usefulness, contrast clarity, and training merit",
