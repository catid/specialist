#!/usr/bin/env python3
"""Focused regression tests for the v141 consent-duplicate audit."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v141 as b


class V141(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v141-", dir=HERE)
    ...
    def test_01_baseline(self):
        self.assertEqual((len(self.baseline), b.file_sha256(self.base)), (511, b.PROJECTED_SELECTION_BASELINE["sha256"]))
    ...
    def test_04_direct(self):
        self.assertEqual(sum(not row.get("curation") for row in self.baseline), 91)
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("anatomiestudio_mixed_message_duplicate_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )

    def test_11_projection(self):
        self.assertEqual((len(self.rows), b.file_sha256(self.dataset)), (510, b.EXPECTED_OUTPUT_SHA256))
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v141", {"drop": 1, "edit": 0, "keep": 0}, b.EXPECTED_OUTPUT_SHA256),
        )
