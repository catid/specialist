#!/usr/bin/env python3
"""Remove a consent fragment made redundant by the v140 rewrite."""
...
DATA = ROOT / "data"
V140_DIR = DATA / "manual_reviews/context_merit_audit_v140"
sys.path[:0] = [str(ROOT), str(V140_DIR)]
import build_context_merit_audit_v140 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v141.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v141.jsonl"
REPORT = OUT_DIR / "report_context_merit_v141.json"
REVIEWER = "codex-context-merit-audit-v141"
...
SPECS = (
    {
        "fact_id": "fact-a06ff9e08fb64f7e987f",
        "active_index": 66,
        "marker": "– Mixed messages mean “no.”",
        "decision": "drop",
        "reason_code": "semantic_duplicate_of_v140_nonconsent_rule",
        "reason": "The v140 rewrite now teaches mixed messages together with silence, freezing, and indecision, so this one-word fragment is redundant.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v140",
    "direct_rows_without_prior_curation": 91,
    "anatomiestudio_mixed_message_duplicate_rows_selected": 1,
    "rows": 511,
    "sha256": "500ab83c24cf01a5ee2b0e71acf9b2927130682e5251666d16024083d36dbb0d",
}
EXPECTED_OUTPUT_SHA256 = "5793de2bac2e79be3645fd909c114539a43a4c33e128e112040720a47550088a"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v141-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v140-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v140.jsonl"
        baseline_report = directory / "v140.report.json"
        ...
        if len(rows) != 511 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v140 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v141 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "anatomiestudio_mixed_message_duplicate_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v141",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 510 or observed["eval_fact_count"] != 612:
        raise ValueError("v141 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v141 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v140": 473,
            ...
            "projection_strategy": "delegate to the hermetic v140 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v141",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the one-word mixed-message row now subsumed by the v140 consent rewrite",
                "score": "semantic coverage, redundancy, source support, and training merit",
