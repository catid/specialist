#!/usr/bin/env python3
"""Drop a redundant wet-jute recovery row after the stronger rewrite."""
...
DATA = ROOT / "data"
V152_DIR = DATA / "manual_reviews/context_merit_audit_v152"
sys.path[:0] = [str(ROOT), str(V152_DIR)]
import build_context_merit_audit_v152 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v153.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v153.jsonl"
REPORT = OUT_DIR / "report_context_merit_v153.json"
REVIEWER = "codex-context-merit-audit-v153"
...
SPECS = (
    {
        "fact_id": "fact-81f75dbff6633b0f72f8",
        "active_index": 192,
        "marker": "make sure you dry the ropes under tension to help them return to their original form",
        "decision": "drop",
        "reason_code": "drop_redundant_wet_jute_recovery_row",
        "reason": "The immediately preceding source-backed rewrite already preserves the same drying instruction in cleaner wording, so retaining both would overweight one fact.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v152",
    "direct_rows_without_prior_curation": 89,
    "anatomiestudio_redundant_wet_jute_rows_selected": 1,
    "rows": 509,
    "sha256": "ec94278098f07050739d36ad9cd118e2e0074835567959bf8de8987e559345ae",
}
EXPECTED_OUTPUT_SHA256 = "c62d124e96e04811ec015096fb4cc5cd51d4b5fea54f1919aadcee3b025a6ca9"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v153-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v152-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v152.jsonl"
        baseline_report = directory / "v152.report.json"
        ...
        if len(rows) != 509 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v152 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v153 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "anatomiestudio_wet_jute_dedup_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v153",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 508 or observed["eval_fact_count"] != 612:
        raise ValueError("v153 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v153 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v152": 471,
            ...
            "projection_strategy": "delegate to the hermetic v152 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v153",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the wet-jute recovery Q&A made redundant by the preceding stronger rewrite",
                "score": "semantic duplication, source support, wording quality, and training balance",
