#!/usr/bin/env python3
"""Replace a teacher-name lookup with a relationship-centered lesson."""
...
DATA = ROOT / "data"
V253_DIR = DATA / "manual_reviews/context_merit_audit_v253"
sys.path[:0] = [str(ROOT), str(V253_DIR)]
import build_context_merit_audit_v253 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v254.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v254.jsonl"
REPORT = OUT_DIR / "report_context_merit_v254.json"
REVIEWER = "codex-context-merit-audit-v254"
...
SOURCE = DATA / "raw/kinbakutoday_381214111022a952.json"

SPECS = (
    {
        "fact_id": "fact-4d570d3865e3fd877364",
        "active_index": 426,
        "markers": (
            "Since Kinbaku involves risk, we need basic knowledge and technique in order to protect the rope bottoms’ safety. With that in mind, Yukimura san told us that rather than showing rope technique, showing the beauty of a rope bottom is more important.",
        ),
        "decision": "edit",
        ...
        "reason_code": "replace_teacher_lookup_with_relationship_centered_lesson",
        "reason": "The replacement trades a bare teacher-name lookup for the source's substantive lesson that partner connection and the bottom's expression matter more than technique performed for its own sake, without minimizing safety skills.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v253",
    "direct_rows_without_prior_curation": 1,
    "teacher_name_lookup_rows_selected": 1,
    "rows": 492,
    "sha256": "ace32971e50f4b9874b20b8ef91db0c22c7d57ebadb47d42ef386878572224ee",
}
EXPECTED_OUTPUT_SHA256 = "1766201a339230f1461c23919f201cf0eec719fbaadbf2fb98fca1fabad3b107"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v254-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v253-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v253.jsonl"
        baseline_report = directory / "v253.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v253 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v254 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "relationship_over_technique_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v254",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v254 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v254 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v253": 454,
        ...
        "schema": "context-merit-audit-report-v254",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a teacher-name lookup whose source passage supports a relationship-centered lesson",
                "score": "transferable partner-connection value, explicit safety qualification, source support, question naturalness, and reduced biographical trivia",
