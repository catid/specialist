#!/usr/bin/env python3
"""Replace a one-pound lookup with deGiotto's complete jute drying setup."""
...
DATA = ROOT / "data"
V199_DIR = DATA / "manual_reviews/context_merit_audit_v199"
sys.path[:0] = [str(ROOT), str(V199_DIR)]
import build_context_merit_audit_v199 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v200.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v200.jsonl"
REPORT = OUT_DIR / "report_context_merit_v200.json"
REVIEWER = "codex-context-merit-audit-v200"
...
SPECS = (
    {
        ...
        "active_index": 358,
        "markers": (
            "Wet jute fibers curl, making rope up to 15 percent shorter and fatter.",
        ),
        "decision": "edit",
        ...
        "reason_code": "replace_weight_lookup_with_complete_jute_drying_setup",
        "reason": "The original answer isolated one weight from a safety-relevant drying procedure. The replacement preserves it while adding the height, end attachment, no-retightening instruction, and the fiber behavior the setup addresses.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v199",
    "degiotto_jute_drying_rows_selected": 1,
    "direct_rows_without_prior_curation": 44,
    "rows": 505,
    "sha256": "8082160c496b3f922834ed88a70d5df8e5175ee9e55cbc35e0774e6499b01e38",
}
EXPECTED_OUTPUT_SHA256 = "025cc7280df584896e42764e397ce8341b961c1997b3205c0dc9af5f3f30f090"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v200-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v199-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v199.jsonl"
        baseline_report = directory / "v199.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v199 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v200 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "degiotto_jute_drying_completeness_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v200",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v200 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v200 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v199": 467,
            ...
            "projection_strategy": "delegate to the hermetic v199 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v200",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated one-pound lookup whose fully inspected care page supplies the complete manufacturer-specific jute drying setup",
                "score": "manual source support, procedural completeness, failure-mode explanation, and training merit",
