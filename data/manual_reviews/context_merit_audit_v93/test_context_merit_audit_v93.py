#!/usr/bin/env python3
"""Focused regression tests for Kinbaku Today Rope and Kindness audit v93."""
import json
import sys
import tempfile
import unittest
...
import build_context_merit_audit_v93 as b


class V93(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix=".test-v93-", dir=HERE)
    ...
    def test_01_baseline(self):
        self.assertEqual((len(self.baseline), b.file_sha256(self.base)), (535, b.PROJECTED_SELECTION_BASELINE["sha256"]))
    ...
    def test_07_support(self):
        self.assertTrue(all(row["reason"] for row in self.curation))
    ...
    def test_10_lineage(self):
        for row in self.audit:
            self.assertEqual(
                ...
                ("kinbakutoday_rope_and_kindness_merit_reaudit", b.PROJECTED_SELECTION_BASELINE["sha256"]),
            )
    ...
    def test_19_report(self):
        self.assertEqual(
            ...
            ("context-merit-audit-report-v93", {"drop": 2, "edit": 0, "keep": 2}, b.EXPECTED_OUTPUT_SHA256),
        )
