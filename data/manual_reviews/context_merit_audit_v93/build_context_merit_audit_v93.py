#!/usr/bin/env python3
"""Audit the direct Kinbaku Today Rope and Kindness rows for merit."""
...
DATA = ROOT / "data"
V92_DIR = DATA / "manual_reviews/context_merit_audit_v92"
sys.path[:0] = [str(ROOT), str(V92_DIR)]
import build_context_merit_audit_v92 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v93.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v93.jsonl"
REPORT = OUT_DIR / "report_context_merit_v93.json"
REVIEWER = "codex-context-merit-audit-v93"
...
SOURCE = DATA / "raw/kinbakutoday_73b16e835ab63cc2.json"
...
SPECS = (
    {
        ...
        "decision": "keep",
        "reason_code": "retain_article_giving_thesis",
        "reason": "The concise attributed Q&A captures the article's central partner-oriented thesis without presenting it as universal fact.",
    },
    {
        "fact_id": "fact-38ec1728983f826643a2",
        "active_index": 100,
        "marker": "As I see it, kindness is nothing more than the quality of seeing another in a profound and real way",
        "decision": "drop",
        "reason_code": "drop_abstract_kindness_definition",
        "reason": "This long philosophical definition is peripheral to rope instruction and overlaps the retained partner-oriented thesis and needs-focused scene guidance.",
    },
    {
        "fact_id": "fact-7c33f617d6092f800235",
        "active_index": 101,
        "marker": "But kindness is not sacrifice.",
        "decision": "drop",
        "reason_code": "drop_peripheral_sacrifice_distinction",
        "reason": "The abstract distinction between kindness and sacrifice adds little actionable rope knowledge and is not needed alongside the article's central thesis.",
    },
    {
        "fact_id": "fact-51dce9783812bf1a03c2",
        "active_index": 244,
        "marker": "So what does it mean to be kind with our rope?",
        "decision": "keep",
        "reason_code": "retain_partner_connection_self_reflection",
        "reason": "The attributed Q&A concisely preserves the article's directly rope-relevant recommendation to understand partner, connection, and self.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v92",
    "direct_rows_without_prior_curation": 112,
    "kinbakutoday_rope_and_kindness_rows_selected": 4,
    "rows": 535,
    "sha256": "a93ceac0d9cc07a092b0a3156101d0cabb13d1ab676bb19d946c8a304646bd6c",
}
EXPECTED_OUTPUT_SHA256 = "0f7b44bafa93d37fa69f7bf44a79f68dcf426c05db6db335572e3e5cf080c2da"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v93-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v92-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v92.jsonl"
        baseline_report = directory / "v92.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 535 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v92 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v93 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Rope and Kindness source lineage drift")
        audit = {
            ...
            "review_pass": "kinbakutoday_rope_and_kindness_merit_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v93",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 533 or observed["eval_fact_count"] != 612:
        raise ValueError("v93 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v93 output hash drift")
    report = {
        "active_baseline": {
        ...
        "audit": {
            "by_decision": {"drop": 2, "edit": 0, "keep": 2},
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v92": 497,
            "active_after_this_tranche": 495,
        ...
        "schema": "context-merit-audit-report-v93",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the four remaining direct Q&A from the fully read Kinbaku Today Rope and Kindness article",
                "score": "manual source support, attribution, redundancy, rope relevance, and practical merit review",
