...
HERE=Path(__file__).resolve().parent;sys.path.insert(0,str(HERE));import build_context_merit_audit_v326 as b
class ContextMeritV326(unittest.TestCase):
 @classmethod
 def setUpClass(cls):
  b.main();cls.t=tempfile.TemporaryDirectory(prefix=".test-v326-",dir=HERE);d=Path(cls.t.name);cls.base=d/"base.jsonl";b.build_baseline(cls.base,d/"base.report.json");cls.out=d/"out.jsonl";cls.rep=d/"out.report.json";cls.ds=[];cls.rs=[]
 ...
 def test_projection_actions(self):
  self.assertEqual((len(b.read_jsonl(self.base)),b.file_sha256(self.base),len(self.rows),b.file_sha256(self.out)),(537,b.BASELINE_SHA256,537,b.EXPECTED_OUTPUT_SHA256));self.assertEqual((self.ds[0],self.rs[0]),(self.ds[1],self.rs[1]));facts={r["fact_id"] for r in self.rows};self.assertTrue(all(s["fact_id"] not in facts for s in b.SPECS));edited={r.get("curation",{}).get("original_fact_id"):r for r in self.rows};self.assertEqual({k:(v["question"],v["answer"]) for k,v in edited.items() if k in {s["fact_id"] for s in b.SPECS}},{s["fact_id"]:(s["question"],s["answer"]) for s in b.SPECS})
 def test_curation_support(self):self.assertEqual((len(self.curations),Counter(r["action"] for r in self.curations)),(3,Counter({"edit":3})));self.assertTrue(all(r["evidence"] for r in self.curations));self.assertIn(b.SCOTT_EVIDENCE,json.loads(b.SCOTT_RAW.read_text())["text"])
