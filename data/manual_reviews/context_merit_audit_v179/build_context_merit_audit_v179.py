#!/usr/bin/env python3
"""Complete the bight-and-tail vocabulary in Rope365's legacy starter."""
...
DATA = ROOT / "data"
V178_DIR = DATA / "manual_reviews/context_merit_audit_v178"
sys.path[:0] = [str(ROOT), str(V178_DIR)]
import build_context_merit_audit_v178 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v179.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v179.jsonl"
REPORT = OUT_DIR / "report_context_merit_v179.json"
REVIEWER = "codex-context-merit-audit-v179"
...
SOURCE = DATA / "raw/rope365_603ebb6ae5af91f0.json"
...
SPECS = (
    {
        ...
        "decision": "edit",
        "question": "In Rope365's legacy starter vocabulary, what are the bight and tail of a rope?",
        "answer": "The bight is a U-shaped arc, usually at the rope's middle; the tail is the unused part extending away from the bight.",
        "reason_code": "complete_bight_and_tail_vocabulary",
        "reason": "The replacement preserves the bight definition, pairs it with the closely related tail definition, and identifies the source as a legacy starter page.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v178",
    "direct_rows_without_prior_curation": 65,
    "rope365_bight_tail_vocabulary_rows_selected": 1,
    "rows": 506,
    "sha256": "d36159240b7cb4755e4a7d7400e84943aef5f50998a1839a4b7024389fefbddf",
}
EXPECTED_OUTPUT_SHA256 = "8ae75af86557763a5d0679aff947403874b67e01203aeecaa383eb09d26a22e2"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v179-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v178-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v178.jsonl"
        baseline_report = directory / "v178.report.json"
        ...
        if len(rows) != 506 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v178 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v179 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Rope365 legacy starter source lineage drift")
        audit = {
            ...
            "review_pass": "rope365_bight_tail_vocabulary_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v179",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 506 or observed["eval_fact_count"] != 612:
        raise ValueError("v179 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v179 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v178": 468,
            ...
            "projection_strategy": "delegate to the hermetic v178 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v179",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated bight definition whose fully read legacy source also defines the closely related tail",
                "score": "manual source support, foundational usefulness, vocabulary completeness, and training merit",
