#!/usr/bin/env python3
"""Expand Shibari Study's uplines course row with its practical topic scope."""
...
DATA = ROOT / "data"
V209_DIR = DATA / "manual_reviews/context_merit_audit_v209"
sys.path[:0] = [str(ROOT), str(V209_DIR)]
import build_context_merit_audit_v209 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v210.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v210.jsonl"
REPORT = OUT_DIR / "report_context_merit_v210.json"
REVIEWER = "codex-context-merit-audit-v210"
...
SOURCE = DATA / "raw/shibari_study_uplines_lockoffs_20260714.json"
...
SPECS = (
    {
        ...
        "active_index": 224,
        "markers": (
            "Shibari Study describes this free course by Gorgone as teaching how to attach uplines to limbs or harnesses and multiple lock-off methods for suspension lines.",
            "The page catalogs related topics including safety, weight handling, basic anchoring, upper and lower lock-offs, bamboo and carabiner lock-offs, and Y-hanger or epsilon configurations.",
        ),
        "decision": "edit",
        ...
        "reason_code": "expand_uplines_course_practical_scope",
        "reason": "The original answer repeated only the course description's broad promise. The replacement preserves that promise and adds the page's concrete topic catalog so the row better answers what a learner can expect.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v209",
    "direct_rows_without_prior_curation": 34,
    "shibari_study_uplines_rows_selected": 1,
    "rows": 505,
    "sha256": "ea39581fec1343c2a95d74e07419e6e5e7bc9497d3505db25498e9eabf53a782",
}
EXPECTED_OUTPUT_SHA256 = "b6921c222438ac8bee4ac12697b28b6eec55a93158cf89434a803cb29d953a0b"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v210-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v209-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v209.jsonl"
        baseline_report = directory / "v209.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v209 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v210 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        if active["source"] != document["source"] or active["url"] != document["url"]:
            raise ValueError(f"{spec['fact_id']}: Shibari Study source lineage drift")
        audit = {
            ...
            "review_pass": "shibari_study_uplines_scope_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v210",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v210 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v210 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v209": 467,
            ...
            "projection_strategy": "delegate to the hermetic v209 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v210",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the isolated uplines-course row whose inspected page exposes a more concrete practical topic catalog",
                "score": "manual source support, learner usefulness, topic completeness, and training merit",
