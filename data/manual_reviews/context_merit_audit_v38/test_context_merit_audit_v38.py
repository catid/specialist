#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v38."""
...
import build_context_merit_audit_v38 as builder
from qa_quality import normalize_text


class ContextMeritAuditV38Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline_temp = tempfile.TemporaryDirectory(
            prefix=".test-baseline-v38-", dir=HERE)
        output = Path(cls.baseline_temp.name)
        cls.baseline_path = output / "projection-v37.jsonl"
        cls.baseline_report_path = output / "projection-v37.report.json"
    ...
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v38-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v38.jsonl"),
            ("CURATION", "pending_curation_context_merit_v38.jsonl"),
            ("REPORT", "report_context_merit_v38.json"),
        ):
    ...
    def test_baseline_is_exact_v37_projection(self) -> None:
        self.assertEqual(len(self.baseline), 571)
    ...
    def test_selection_is_exact_deterministic_top_ten(self) -> None:
        ...
        self.assertEqual(len(ranked), 186)
        self.assertEqual(
    ...
    def test_projected_baseline_counts_are_pinned(self) -> None:
        ...
        reviewed = builder.prior_reviewed_fact_ids()
        self.assertEqual(len(direct), 301)
        self.assertEqual(sum(row["fact_id"] in reviewed for row in direct), 115)
    ...
    def test_decisions_and_curation_are_exact(self) -> None:
        ...
        self.assertEqual(
            ...
            {"keep": 6, "drop": 1, "edit": 3})
        self.assertEqual(len(self.curation), 4)
        self.assertEqual(
            {action: sum(row["action"] == action for row in self.curation)
            for action in ("drop", "edit")}, {"drop": 1, "edit": 3})

    def test_repairs_are_grammatical_and_full_premise_is_pinned(self) -> None:
        ...
        by_id = {row["fact_id"]: row for row in self.audit}
        cultural = by_id["fact-b1976a3e15c9d8fae3fc"]["edited_answer"]
        self.assertEqual(
            cultural,
            "Because shibari reflects the people practicing it—their local "
            "BDSM customs, beliefs, and views about what a rope interaction "
            "should be and what its goals are.")
        self.assertNotIn("shibari, at the end of the day, reflecting", cultural)
        preference = by_id["fact-c1697f4f94aa32b0b9f6"]["edited_answer"]
        self.assertEqual(
            preference,
            "Try different ropes and compare their properties to discover "
            "your own preferences.")
        self.assertNotIn("experiment with it", preference)
        reproducibility = by_id["fact-ac7fbf74e38372bfbbbd"]["edited_answer"]
        self.assertNotIn("knit", reproducibility.casefold())
        venue = by_id["fact-55da0d1915d37b1d077f"]
        ...
        self.assertEqual(venue["source_support"], "source_composite")
    ...
    def test_projection_lineage_is_complete(self) -> None:
        for row in self.audit:
            ...
            self.assertEqual(lineage["baseline_rows"], 571)
            ...
            self.assertEqual(row["review_pass"],
                             "first_context_merit_review_of_v37_projection_row")
    ...
    def test_report_counts_projection_and_sealed_policy(self) -> None:
        report = json.loads(builder.REPORT.read_text())
        self.assertEqual(report["schema"], "context-merit-audit-report-v38")
        self.assertEqual(report["audit"]["rows"], 10)
        self.assertEqual(report["selection"]["rows_selected"], 10)
        self.assertEqual(report["selection"]["eligible_unreviewed_rows"], 186)
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 1, "edit": 3, "keep": 6})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 570)
        self.assertEqual(
            projection["output_sha256"],
            "2cbc3790de21a4f38a8e8ad578485cbbb99c141706f33d6dab8f55e786bbd915")
    ...
    def test_future_versions_cannot_enter_v38_by_directory_discovery(self) -> None:
        self.assertEqual(len(builder.CONTEXT_CURATIONS), 37)
        self.assertEqual(len(builder.CONTEXT_AUDITS), 37)
        self.assertIn("context_merit_audit_v37",
                      str(builder.CONTEXT_CURATIONS[-1]))
        self.assertIn("context_merit_audit_v37",
                      str(builder.CONTEXT_AUDITS[-1]))
        self.assertFalse(any("context_merit_audit_v38" in str(path) or
                             "context_merit_audit_v38" in str(path)
                             for path in (*builder.CONTEXT_CURATIONS,
                                          *builder.CONTEXT_AUDITS)))


class IsolatedProjectionV38Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory(
            prefix=".test-projection-v38-", dir=HERE)
    ...
    def test_projection_is_byte_identical_and_hash_pinned(self) -> None:
        ...
        self.assertEqual(len(self.rows), 570)
        self.assertEqual(self.report["eval_fact_count"], 612)

    def test_projection_applies_exact_v38_decisions(self) -> None:
    ...
    def test_projection_has_unique_ids_and_questions(self) -> None:
        self.assertEqual(len({row["fact_id"] for row in self.rows}), 570)
        self.assertEqual(len({row["question"] for row in self.rows}), 570)
