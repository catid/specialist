#!/usr/bin/env python3
"""Focused, non-mutating regression tests for context audit v34."""
...
import build_context_merit_audit_v34 as builder
from qa_quality import normalize_text


class ContextMeritAuditV34Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.baseline_temp = tempfile.TemporaryDirectory(
            prefix=".test-baseline-v34-", dir=HERE)
        output = Path(cls.baseline_temp.name)
        cls.baseline_path = output / "projection-v33.jsonl"
        cls.baseline_report_path = output / "projection-v33.report.json"
    ...
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix=".test-context-merit-v34-", dir=HERE)
        self.addCleanup(temporary.cleanup)
        output = Path(temporary.name)
        for attribute, filename in (
            ("AUDIT", "context_merit_audit_v34.jsonl"),
            ("CURATION", "pending_curation_context_merit_v34.jsonl"),
            ("REPORT", "report_context_merit_v34.json"),
        ):
    ...
    def test_baseline_is_exact_v33_projection(self) -> None:
        self.assertEqual(len(self.baseline), 575)
    ...
    def test_selection_is_exact_deterministic_top_ten(self) -> None:
        ...
        self.assertEqual(len(ranked), 226)
        self.assertEqual(
    ...
    def test_projected_baseline_counts_are_pinned(self) -> None:
        ...
        reviewed = builder.prior_reviewed_fact_ids()
        self.assertEqual(len(direct), 322)
        self.assertEqual(sum(row["fact_id"] in reviewed for row in direct), 96)
    ...
    def test_projection_lineage_is_complete(self) -> None:
        for row in self.audit:
            ...
            self.assertEqual(lineage["baseline_rows"], 575)
            ...
            self.assertEqual(row["review_pass"],
                             "first_context_merit_review_of_v33_projection_row")
    ...
    def test_report_counts_projection_and_sealed_policy(self) -> None:
        report = json.loads(builder.REPORT.read_text())
        self.assertEqual(report["schema"], "context-merit-audit-report-v34")
        self.assertEqual(report["audit"]["rows"], 10)
        self.assertEqual(report["selection"]["rows_selected"], 10)
        self.assertEqual(report["selection"]["eligible_unreviewed_rows"], 226)
        self.assertEqual(report["audit"]["by_decision"],
                         {"drop": 1, "edit": 4, "keep": 5})
        projection = report["isolated_build_projection"]
        self.assertEqual(projection["output_rows"], 574)
        self.assertEqual(
            projection["output_sha256"],
            "dbda721170f9005357bacade032dcc80c27841469b6495a1dc68a0c850c70d2d")
    ...
    def test_future_versions_cannot_enter_v34_by_directory_discovery(self) -> None:
        self.assertEqual(len(builder.CONTEXT_CURATIONS), 33)
        self.assertEqual(len(builder.CONTEXT_AUDITS), 33)
        self.assertIn("context_merit_audit_v33",
                      str(builder.CONTEXT_CURATIONS[-1]))
        self.assertIn("context_merit_audit_v33",
                      str(builder.CONTEXT_AUDITS[-1]))
        self.assertFalse(any("context_merit_audit_v34" in str(path) or
                             "context_merit_audit_v35" in str(path)
                             for path in (*builder.CONTEXT_CURATIONS,
                                          *builder.CONTEXT_AUDITS)))


class IsolatedProjectionV34Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory(
            prefix=".test-projection-v34-", dir=HERE)
    ...
    def test_projection_applies_exact_v34_decisions(self) -> None:
    ...
    def test_projection_has_unique_ids_and_questions(self) -> None:
        self.assertEqual(len({row["fact_id"] for row in self.rows}), 574)
        self.assertEqual(len({row["question"] for row in self.rows}), 574)
