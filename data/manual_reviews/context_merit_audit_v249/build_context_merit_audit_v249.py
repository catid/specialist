#!/usr/bin/env python3
"""Remove a redundant frog-tie pain-check answer."""
...
DATA = ROOT / "data"
V248_DIR = DATA / "manual_reviews/context_merit_audit_v248"
sys.path[:0] = [str(ROOT), str(V248_DIR)]
import build_context_merit_audit_v248 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v249.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v249.jsonl"
REPORT = OUT_DIR / "report_context_merit_v249.json"
REVIEWER = "codex-context-merit-audit-v249"
...
SOURCE = DATA / "raw/rope365_e5eb58544cbf8458.json"

SPECS = (
    {
        "fact_id": "fact-ac99053d3a764cc0c291",
        "active_index": 390,
        "markers": (
            "Are there any painful sensations? In particular from overextension at the knee or on the front of the tibia bone.",
        ),
        "decision": "drop",
        "reason_code": "drop_redundant_frog_tie_tibia_answer",
        "reason": "This tibia-only row is subsumed by a stronger active question from the same checklist that preserves both stated pain concerns: knee overextension and pressure on the front of the tibia.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v248",
    "direct_rows_without_prior_curation": 1,
    "redundant_frog_tie_tibia_rows_selected": 1,
    "rows": 494,
    "sha256": "bf188cc4dbeae5eef05ca7959f0a503bee7161e93cf8285254ff06fbb6428c88",
}
EXPECTED_OUTPUT_SHA256 = "2562a77dba0c60eade55b0b702881b6acef6b488e5a9876d9648f0d86b014d7d"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v249-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v248-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v248.jsonl"
        baseline_report = directory / "v248.report.json"
        build_projection(baseline, baseline_report, PRIOR_PROJECTION_CURATIONS)
        rows = read_jsonl(baseline)
        if len(rows) != 494 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v248 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v249 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "redundant_frog_tie_tibia_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v249",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 493 or observed["eval_fact_count"] != 612:
        raise ValueError("v249 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v249 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v248": 456,
            "active_after_this_tranche": 455,
        ...
        "schema": "context-merit-audit-report-v249",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "a tibia-only pain-check row subsumed by a complete knee-and-tibia checklist row from the same source sentence",
