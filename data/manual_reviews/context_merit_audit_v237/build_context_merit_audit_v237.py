#!/usr/bin/env python3
"""Differentiate the two owner-recommended United States rope conventions."""
...
DATA = ROOT / "data"
V236_DIR = DATA / "manual_reviews/context_merit_audit_v236"
sys.path[:0] = [str(ROOT), str(V236_DIR)]
import build_context_merit_audit_v236 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v237.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v237.jsonl"
REPORT = OUT_DIR / "report_context_merit_v237.json"
REVIEWER = "codex-context-merit-audit-v237"
...
SOURCE = DATA / "raw/owner_us_rope_conventions_summary_20260714.json"

SPECS = (
    {
        "fact_id": "fact-d8460d7680f1fee4ee02",
        "active_index": 434,
        "markers": (
            "Tethered Together is https://tetheredtogether.net/.",
            "Its inspected site describes rope-focused workshops, broader circus, kink, relationship, dance, and yoga programming, attendee-led content, art, entertainment, and a participant-driven Ropenspace.",
            "ROPECRAFT is https://ropecraft.net/.",
            "Its inspected site describes beginner-through-advanced education for tops, bottoms, and switches across several rope styles, plus Ropenspace, peer support, special-interest groups, practice and self-tying spaces, and an inclusive consent-oriented event model.",
        ),
        "decision": "edit",
        "question": "Which United States rope conventions are owner-recommended, and how do their inspected programming emphases differ?",
        "answer": "Tethered Together (https://tetheredtogether.net/) combines rope workshops with circus, kink, relationship, movement, attendee-led, art, and entertainment programming. ROPECRAFT (https://ropecraft.net/) emphasizes multi-style, beginner-through-advanced role education plus peer support, special-interest, practice, and self-tying spaces; both describe participant-driven Ropenspace learning.",
        "reason_code": "differentiate_us_rope_conventions",
        "reason": "The original answer preserved both convention links but did not distinguish them. The replacement keeps both exact owner-recommended URLs and summarizes inspected programming and support differences.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v236",
    "direct_rows_without_prior_curation": 7,
    "us_convention_summary_rows_selected": 1,
    "rows": 505,
    "sha256": "a2ea7976a9d76dcad31e6d486b4620c7fd271dcca467afc203d29254eba4f41c",
}
EXPECTED_OUTPUT_SHA256 = "bd0a0af45ee16af03071c6da5e43df5fe81b07cc67f0e1ce464db5f34ffcff28"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v237-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v236-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v236.jsonl"
        baseline_report = directory / "v236.report.json"
        ...
        if len(rows) != 505 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v236 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v237 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "review_pass": "us_convention_summary_reaudit",
            "reviewed_at": REVIEWED_AT,
            "reviewer": REVIEWER,
            "risk_features": CORE.risk_features(active),
            "schema": "context-merit-audit-v237",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 505 or observed["eval_fact_count"] != 612:
        raise ValueError("v237 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v237 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v236": 467,
            ...
            "projection_strategy": "delegate to the hermetic v236 second-stage projection and append this tranche's decisions",
        ...
        "schema": "context-merit-audit-report-v237",
        ...
        "selection": {
            ...
            "projected_baseline": PROJECTED_SELECTION_BASELINE,
            "ranking": {
                "candidate_rule": "the combined United States convention row whose inspected sites support a clearer comparison",
                "score": "owner-source support, attendee-orientation usefulness, programming differentiation, and URL preservation",
