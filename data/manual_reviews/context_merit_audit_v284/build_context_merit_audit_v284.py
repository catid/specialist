#!/usr/bin/env python3
"""Replace a clipped upper-cinch answer with actionable nerve context."""
...
DATA = ROOT / "data"
V283_DIR = DATA / "manual_reviews/context_merit_audit_v283"
sys.path[:0] = [str(ROOT), str(V283_DIR)]
import build_context_merit_audit_v283 as previous

OUT_DIR = Path(__file__).resolve().parent
AUDIT = OUT_DIR / "context_merit_audit_v284.jsonl"
CURATION = OUT_DIR / "pending_curation_context_merit_v284.jsonl"
REPORT = OUT_DIR / "report_context_merit_v284.json"
REVIEWER = "codex-context-merit-audit-v284"
...
SPECS = (
    {
        "fact_id": "fact-c7bf7db4694d5144906a",
        "active_index": 160,
        "markers": (
            "At the level of the upper cinches, the three nerves are still medial to the humeral bone, and then may be crushed between something and the bone.",
            "I believe it’s a danger zone, and it’s best to avoid upper cinches.",
        ),
        "decision": "edit",
        ...
        "reason_code": "add_upper_cinch_nerve_context",
        "reason": "The original answer merely called the area dangerous; the revision preserves the surgeon's recommendation while adding the source-supported anatomical reason that makes it useful safety guidance.",
    },
)
...
PROJECTED_SELECTION_BASELINE = {
    "description": "isolated corrected training projection through context-merit v283",
    "direct_rows_without_prior_curation": 1,
    "clipped_answer_rows_selected": 1,
    "rows": 492,
    "sha256": "83d14d9d42740c836b49a8ec9e4237766e9d751c827c21d4d2c79500ee4bc3b9",
}
EXPECTED_OUTPUT_SHA256 = "107a43403edbc51a6b2b72f5c964648e865d384fe2156ce12c84b7cbcb63c614"
...
def observation():
    with tempfile.TemporaryDirectory(prefix=".v284-observation-", dir=OUT_DIR) as temp:
        directory = Path(temp)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=".v283-projection-", dir=OUT_DIR) as temp:
        directory = Path(temp)
        baseline = directory / "v283.jsonl"
        baseline_report = directory / "v283.report.json"
        ...
        if len(rows) != 492 or file_sha256(baseline) != PROJECTED_SELECTION_BASELINE["sha256"]:
            raise ValueError("v283 projection drift")
        ...
        if {fact_id: by_fact[fact_id][0] for fact_id in EXPECTED_SELECTION} != PROJECTED_ACTIVE_INDICES:
            raise ValueError("v284 candidate drift")
    ...
    for audit_index, spec in enumerate(SPECS, 1):
        ...
        audit = {
            ...
            "schema": "context-merit-audit-v284",
    ...
    if not observed["dataset_equal"] or not observed["report_equal"] or observed["rows"] != 492 or observed["eval_fact_count"] != 612:
        raise ValueError("v284 deterministic projection drift")
    if EXPECTED_OUTPUT_SHA256 != "PENDING" and observed["dataset_sha256"] != EXPECTED_OUTPUT_SHA256:
        raise ValueError("v284 output hash drift")
    report = {
        "active_baseline": {
        ...
        "isolated_build_projection": {
            "active_after_context_merit_v283": 454,
        ...
        "schema": "context-merit-audit-report-v284",
