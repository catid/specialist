...
ROOT = HERE.parents[2]
URL = "https://www.ropeconnections.com/suspension-vs-partial-suspension/"
...
class PublicTrainingShardTests(unittest.TestCase):
    ...
    def test_decisions_and_output_counts_are_pinned(self) -> None:
        ...
        self.assertEqual({"drop": 7, "edit": 3, "add": 3}, action_counts)
        self.assertEqual(6, len(self.qa))
        self.assertEqual(23, sum(len(row["candidate_fact_ids"]) for row in self.review if row["action"] == "drop"))
        self.assertEqual(10, sum(len(row["candidate_fact_ids"]) for row in self.review if row["action"] == "edit"))
    ...
    def test_questions_are_clean_unique_and_not_url_trivia(self) -> None:
    ...
    def test_source_is_separate_dense_markdown(self) -> None:
        self.assertTrue(self.source.startswith("# Full Suspension Vs Partial Suspension\n"))
        self.assertIn(f"Source: {URL}", self.source)
        self.assertGreaterEqual(len(re.findall(r"\b[\w’'-]+\b", self.source)), 650)
    ...
    def test_report_counts_hashes_and_promotion_boundary(self) -> None:
        ...
        self.assertEqual(6, self.report["review"]["output_qa_rows"])
