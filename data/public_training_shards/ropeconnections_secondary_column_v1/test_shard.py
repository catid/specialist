...
ROOT = HERE.parents[2]
URL = "https://www.ropeconnections.com/the-secondary-column-tie/"
...
class PublicTrainingShardTests(unittest.TestCase):
    ...
    def test_exact_candidate_shard_is_fully_consumed_once(self) -> None:
        self.assertEqual(37, len(self.candidates))
        ...
        self.assertEqual(37, len(consumed))
        self.assertEqual(37, len(set(consumed)))
    ...
    def test_decisions_and_output_counts_are_pinned(self) -> None:
        ...
        self.assertEqual({"drop": 1, "edit": 5, "add": 8}, action_counts)
        self.assertEqual(13, len(self.qa))
        self.assertEqual(12, sum(len(row["candidate_fact_ids"]) for row in self.review if row["action"] == "drop"))
        self.assertEqual(25, sum(len(row["candidate_fact_ids"]) for row in self.review if row["action"] == "edit"))
    ...
    def test_questions_are_clean_unique_and_not_trivia(self) -> None:
        ...
        for row in self.qa:
            ...
            self.assertNotIn("nerve damage", rendered.casefold())
            self.assertNotIn("circulation issues", rendered.casefold())

    def test_source_is_separate_dense_markdown(self) -> None:
        self.assertTrue(self.source.startswith("# The Secondary Column Tie\n"))
    ...
    def test_source_copy_preserves_both_original_training_chunks(self) -> None:
        ...
        original = original.replace("How To Do The Tie", "## How To Do The Tie")
    ...
    def test_report_counts_hashes_and_promotion_boundary(self) -> None:
        ...
        self.assertEqual(37, self.report["candidate_input"]["matching_rows"])
        ...
        self.assertEqual(13, self.report["review"]["output_qa_rows"])
        self.assertEqual(0, self.report["quality"]["unqualified_medical_claims_in_qa"])
        self.assertTrue(self.report["quality"]["raw_source_medical_warning_preserved"])
        self.assertTrue(self.report["quality"]["raw_source_requires_downstream_safety_review"])
