...
ROOT = HERE.parents[2]
URL = "https://www.ropeconnections.com/knotted-vs-whipped-ends-the-update/"
...
class PublicTrainingShardTests(unittest.TestCase):
    ...
    def test_exact_candidate_shard_is_fully_consumed_once(self) -> None:
        ...
        self.assertEqual(39, len(consumed))
        self.assertEqual(39, len(set(consumed)))
    ...
    def test_decisions_and_output_counts_are_pinned(self) -> None:
        ...
        self.assertEqual({"drop": 1, "edit": 6, "add": 5}, action_counts)
        self.assertEqual(11, len(self.qa))
        self.assertEqual(3, sum(len(row["candidate_fact_ids"]) for row in self.review if row["action"] == "drop"))
        self.assertEqual(36, sum(len(row["candidate_fact_ids"]) for row in self.review if row["action"] == "edit"))
    ...
    def test_questions_are_clean_unique_and_not_trivia(self) -> None:
        ...
        for row in self.qa:
            ...
            self.assertNotIn("youtube", rendered.casefold())
            self.assertNotIn("bad nun", rendered.casefold())

    def test_source_is_separate_dense_markdown(self) -> None:
        self.assertTrue(self.source.startswith("# Knotted Vs Whipped Ends: The Update\n"))
        self.assertIn(f"Source: {URL}", self.source)
        self.assertGreaterEqual(len(re.findall(r"\b[\w’'-]+\b", self.source)), 700)
    ...
    def test_source_copy_preserves_both_original_training_chunks(self) -> None:
        ...
        original = original.replace("Pros of the Matthew Walker:", "## Pros of the Matthew Walker")
        original = original.replace("Cons of the Matthew Walker:", "## Cons of the Matthew Walker")
    ...
    def test_report_counts_hashes_and_promotion_boundary(self) -> None:
        ...
        self.assertEqual(39, self.report["candidate_input"]["matching_rows"])
        ...
        self.assertEqual(11, self.report["review"]["output_qa_rows"])
        quality = self.report["quality"]
        self.assertEqual(0, quality["platform_learning_trivia"])
        self.assertTrue(quality["raw_source_explicit_scene_narrative_preserved"])
        self.assertTrue(quality["raw_source_anecdotal_durability_claims_preserved"])
        self.assertTrue(quality["raw_source_requires_downstream_content_and_safety_review"])
