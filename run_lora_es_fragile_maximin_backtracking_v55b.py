#!/usr/bin/env python3
"""Execute V55B strict-interior maximin train-only backtracking."""

from __future__ import annotations

import argparse
import json
import os
import sys
import time
from contextlib import contextmanager
from datetime import datetime, timezone
from pathlib import Path

import build_lora_es_fragile_maximin_backtracking_preregistration_v55b as builder
import lora_es_fragile_maximin_projection_v55b as maximin
import lora_es_nested_population_v52 as design
import run_lora_es_nested_population_v52 as v52


ROOT = Path(__file__).resolve().parent
RUN_DIR = builder.RUN_DIR.resolve()
ATTEMPT = (RUN_DIR.parent / ".v55b_lora_es_fragile_maximin_backtracking.attempt.json").resolve()
GPU_LOG = (RUN_DIR / "gpu_activity_v55b.jsonl").resolve()
PHASE = "v55b_strict_interior_maximin_projection_no_population_rerun"
PHASE_BARRIER_SECONDS = 1.25
NUMERIC = (RUN_DIR / "numeric_calibration_v55b.json").resolve()
ANCHOR = (RUN_DIR / "anchor_calibration_v55b.json").resolve()
PREINSTALL = (RUN_DIR / "preinstall_actor_baseline_v55b.json").resolve()
MASTER_AUDIT = (RUN_DIR / "master_identity_audit_v55b.json").resolve()
SNAPSHOT = (RUN_DIR / "selected_candidate_v55b").resolve()
INTERNAL_REPORT = (RUN_DIR / "nested_population_report_v52.json").resolve()
P8_RECEIPT = (RUN_DIR / "p8_train_gate_v52.json").resolve()
P16_GATE = (RUN_DIR / "p16_train_gate_v52.json").resolve()
REPORT = (RUN_DIR / "fragile_maximin_backtracking_report_v55b.json").resolve()
ORIGINAL_EVALUATE = v52.evaluate_train_arm_v52


def load_preregistration_v55b(args) -> dict:
    path = Path(args.preregistration).resolve()
    if builder.file_sha256(path) != args.preregistration_sha256:
        raise RuntimeError("v55b preregistration file changed")
    value = json.loads(path.read_text(encoding="utf-8"))
    compact = {
        key: item for key, item in value.items()
        if key != "content_sha256_before_self_field"
    }
    projection = maximin.maximin_projection_v55b()
    plans = maximin.scale_plans_v55b(projection)
    authorization = value.get("authorization", {})
    if (
        value.get("schema")
        != "lora-es-fragile-maximin-backtracking-preregistration-v55b"
        or value.get("status")
        != "sealed_before_v55b_model_ray_gpu_or_train_gate_access"
        or value.get("content_sha256_before_self_field")
        != args.preregistration_content_sha256
        or design.canonical_sha256_v52(compact)
        != args.preregistration_content_sha256
        or value.get("maximin_projection") != projection
        or value.get("scale_plans") != plans
        or value.get("artifacts") != builder.artifacts_v55b()
        or value.get("telemetry") != {
            "gpu_log": str(GPU_LOG), "projection_phase": PHASE,
            "minimum_projection_phase_barrier_seconds": 1.25,
            "inherited_v52_filename_or_population_phase_allowed": False,
        }
        or value.get("selected_v53_input", {}).get("p16_seeds")
        != list(design.P16_SEEDS_V52)
        or value.get("selected_v53_input", {}).get(
            "optimizer_update_alpha"
        ) != design.ALPHA_V52
        or value.get("single_scientific_change", {}).get("variable")
        != "projection_direction_only"
        or value.get("single_scientific_change", {}).get(
            "every_scale_coefficient_l2_norm_exactly_matches_v54"
        ) is not True
        or value.get("train_only_gate", {}).get("required_checks")
        != list(design.TRAIN_GATE_NAMES_V52)
        or value.get("train_only_gate", {}).get(
            "all_nine_checks_required_without_weakening"
        ) is not True
        or authorization.get("gpu_launch") is not True
        or authorization.get("reuse_persisted_signed_scores") is not True
        or authorization.get("population_generation_or_scoring") is not False
        or authorization.get("optimizer_master_commit") is not False
        or authorization.get("p8_evaluation") is not False
        or any(authorization.get(key) is not False for key in (
            "ood_shadow_benchmark_or_holdout", "protected_semantics",
            "sealed_holdout",
        ))
        or value.get("fixed_recipe", {}).get("master_sha256")
        != design.MASTER_SHA256_V52
        or value.get("fixed_recipe", {}).get("dataset_sha256")
        != design.DATASET_SHA256_V52
        or value.get("runtime") != design.RUNTIME_V52
        or value.get("protected_semantics_opened") is not False
        or value.get("sealed_holdout_opened") is not False
    ):
        raise RuntimeError("v55b preregistration contract changed")
    for binding in value["implementation_bindings"].values():
        if builder.file_sha256(Path(binding["path"])) != binding["file_sha256"]:
            raise RuntimeError("v55b implementation binding changed")
    return value


def selected_maximin_projection_v55b(*_args, **_kwargs) -> dict:
    projection = maximin.maximin_projection_v55b()
    plans = maximin.scale_plans_v55b(projection)
    time.sleep(PHASE_BARRIER_SECONDS)
    arm = json.loads(maximin.V53_SELECTED_ARM.read_text(encoding="utf-8"))
    return {
        "schema": "selected-v53-p16-fragile-maximin-projection-v55b",
        "status": "complete_without_population_rerun",
        "source_signed_scores_file_sha256": (
            maximin.V53_SELECTED_ARM_FILE_SHA256
        ),
        "source_signed_scores_sha256": arm["signed_scores_sha256"],
        "arms": {
            "p8": {"schema": "no-p8-arm-v55b", "evaluation_authorized": False},
            "p16": {
                "schema": "selected-v53-p16-fragile-maximin-arm-v55b",
                "population_size": 16, "sigma": 0.0048,
                "reliability": arm["reliability"],
                "projection": projection, "scale_plans": plans,
            },
        },
        "population_generation_or_scoring_performed": False,
        "projection_performed_from_persisted_scores": True,
        "optimizer_update_or_train_gate_opened": False,
        "protected_semantics_opened": False,
        "sealed_holdout_opened": False,
        "timing": {"coverage": {
            "schema": "no-population-rerun-v55b", "states": 0,
            "total_actor_phase_receipts": 0,
            "projection_phase_barrier_seconds": PHASE_BARRIER_SECONDS,
        }},
    }


def require_selected_projection_v55b(population: dict) -> None:
    if (
        population.get("schema")
        != "selected-v53-p16-fragile-maximin-projection-v55b"
        or population.get("population_generation_or_scoring_performed")
        is not False
        or population.get("projection_performed_from_persisted_scores")
        is not True
        or population.get("arms", {}).get("p8", {}).get(
            "evaluation_authorized"
        ) is not False
    ):
        raise RuntimeError("v55b selected projection contract changed")


def evaluate_only_p16_v55b(name, *args, **kwargs):
    if name == "p8":
        return {
            "schema": "no-p8-evaluation-receipt-v55b", "arm": "p8",
            "evaluation_performed": False, "candidate_transactions_opened": 0,
            "selected_target_norm_ratio": None, "selected_snapshot": None,
            "master_committed": False, "protected_semantics_opened": False,
            "sealed_holdout_opened": False,
        }
    if name != "p16":
        raise RuntimeError("v55b unexpected train arm")
    result = dict(ORIGINAL_EVALUATE(name, *args, **kwargs))
    result["schema"] = "fragile-maximin-p16-backtracking-gate-v55b"
    result["population_rerun_performed"] = False
    result["p8_evaluation_performed"] = False
    return result


@contextmanager
def patched_executor_v55b():
    replacements = {
        "RUN_DIR": RUN_DIR, "ATTEMPT": ATTEMPT,
        "PREINSTALL_BASELINE": PREINSTALL,
        "MASTER_IDENTITY_AUDIT": MASTER_AUDIT,
        "NUMERIC_CALIBRATION": NUMERIC, "ANCHOR_CALIBRATION": ANCHOR,
        "P8_SNAPSHOT": RUN_DIR / "forbidden_p8_snapshot",
        "P16_SNAPSHOT": SNAPSHOT,
        "GPU_LOG_OVERRIDE_V52": GPU_LOG,
        "POPULATION_PHASE_OVERRIDE_V52": PHASE,
        "replicated_population_v52": selected_maximin_projection_v55b,
        "require_all_arms_reliable_v52": require_selected_projection_v55b,
        "evaluate_train_arm_v52": evaluate_only_p16_v55b,
    }
    saved = {key: getattr(v52, key) for key in replacements}
    for key, value in replacements.items():
        setattr(v52, key, value)
    try:
        if v52.runtime_telemetry_contract_v52() != {
            "gpu_log": GPU_LOG, "population_phase": PHASE,
        }:
            raise RuntimeError("v55b telemetry override failed")
        yield
    finally:
        for key, value in saved.items():
            setattr(v52, key, value)


def _write_report(value: dict) -> dict:
    result = dict(value)
    result["content_sha256_before_self_field"] = (
        design.canonical_sha256_v52(result)
    )
    payload = (json.dumps(
        result, indent=2, sort_keys=True, ensure_ascii=True, allow_nan=False,
    ) + "\n").encode("ascii")
    temporary = REPORT.with_name(f".{REPORT.name}.tmp-{os.getpid()}")
    with temporary.open("xb") as handle:
        handle.write(payload); handle.flush(); os.fsync(handle.fileno())
    os.replace(temporary, REPORT)
    return result


def execute_v55b(preregistration: dict) -> int:
    with patched_executor_v55b():
        code = v52._execute_v52(preregistration)
    if (
        code != 0 or not INTERNAL_REPORT.exists()
        or not P8_RECEIPT.exists() or not P16_GATE.exists()
    ):
        raise RuntimeError("v55b internal executor did not complete")
    internal = json.loads(INTERNAL_REPORT.read_text(encoding="utf-8"))
    p8 = json.loads(P8_RECEIPT.read_text(encoding="utf-8"))
    p16 = json.loads(P16_GATE.read_text(encoding="utf-8"))
    phases, foreign = set(), 0
    with GPU_LOG.open("r", encoding="utf-8") as handle:
        for line in handle:
            row = json.loads(line)
            phases.add(row["phase"])
            foreign += len(row["foreign_compute_pids"])
    if (
        p8.get("schema") != "no-p8-evaluation-receipt-v55b"
        or p8.get("evaluation_performed") is not False
        or p8.get("candidate_transactions_opened") != 0
        or p16.get("schema")
        != "fragile-maximin-p16-backtracking-gate-v55b"
        or p16.get("population_rerun_performed") is not False
        or internal.get("optimizer_master_committed") is not False
        or internal.get("protected_semantics_opened") is not False
        or PHASE not in phases or "nested_p16_population_v52" in phases
        or foreign != 0
    ):
        raise RuntimeError("v55b dedicated completion contract changed")
    report = _write_report({
        "schema": "lora-es-fragile-maximin-backtracking-report-v55b",
        "status": "complete_train_only_no_master_commit",
        "completed_at_utc": datetime.now(timezone.utc).isoformat(),
        "preregistration_content_sha256": preregistration[
            "content_sha256_before_self_field"
        ],
        "selected_v53_sigma": 0.0048,
        "projection_direction_sha256": preregistration[
            "maximin_projection"
        ]["direction_sha256"],
        "maximin_margin": preregistration[
            "maximin_projection"
        ]["maximin_margin"],
        "population_rerun_performed": False,
        "p8_evaluation_performed": False,
        "selected_target_norm_ratio": p16["selected_target_norm_ratio"],
        "passing_candidate_saved": (
            p16["selected_target_norm_ratio"] is not None
        ),
        "p16_train_gate": {
            "path": str(P16_GATE),
            "file_sha256": builder.file_sha256(P16_GATE),
            "content_sha256": p16["content_sha256_before_self_field"],
        },
        "internal_exact_state_report": {
            "path": str(INTERNAL_REPORT),
            "file_sha256": builder.file_sha256(INTERNAL_REPORT),
            "content_sha256": internal["content_sha256_before_self_field"],
        },
        "compatibility_shell_metadata": {
            "internal_report_schema": internal.get("schema"),
            "internal_shared_by_p8_and_p16_field": internal.get(
                "shared_fresh_calibration", {}
            ).get("shared_by_p8_and_p16"),
            "shared_by_p8_and_p16_is_factual_v55b_science": False,
            "actual_v55b_science": (
                "one selected persisted V53 P16 arm, strict-interior "
                "six-objective direction, no P8 evaluation or population rerun"
            ),
        },
        "telemetry": {
            "path": str(GPU_LOG),
            "file_sha256": builder.file_sha256(GPU_LOG),
            "required_phase_present": True,
            "inherited_v52_phase_absent": True,
            "foreign_compute_pid_rows": 0,
        },
        "optimizer_master_committed": False,
        "all_candidates_exactly_aborted_to_master": True,
        "ood_shadow_benchmark_holdout_or_protected_semantics_opened": False,
        "sealed_holdout_opened": False,
    })
    print(json.dumps({
        "report": str(REPORT),
        "file_sha256": builder.file_sha256(REPORT),
        "content_sha256": report["content_sha256_before_self_field"],
        "selected_target_norm_ratio": report["selected_target_norm_ratio"],
    }, sort_keys=True))
    return 0


def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser()
    value.add_argument("--preregistration", required=True)
    value.add_argument("--preregistration-sha256", required=True)
    value.add_argument("--preregistration-content-sha256", required=True)
    value.add_argument("--dry-run", action="store_true")
    value.add_argument("--execute", action="store_true")
    return value


def main(argv=None) -> int:
    args = parser().parse_args(argv)
    if args.dry_run == args.execute:
        raise ValueError("v55b requires exactly one of --dry-run or --execute")
    prereg = load_preregistration_v55b(args)
    if args.dry_run:
        print(json.dumps({
            "schema": prereg["schema"], "selected_sigma": 0.0048,
            "population_rerun": False, "p8_evaluation": False,
            "projection_direction_sha256": prereg[
                "maximin_projection"
            ]["direction_sha256"],
            "maximin_margin": prereg[
                "maximin_projection"
            ]["maximin_margin"],
            "gpu_log": str(GPU_LOG), "phase": PHASE,
            "filesystem_writes": False,
            "model_ray_gpu_or_train_semantics_loaded": False,
            "protected_semantics_loaded": False,
            "sealed_holdout_opened": False,
        }, sort_keys=True))
        return 0
    if Path(sys.executable).absolute() != design.REQUIRED_PYTHON_V52:
        raise RuntimeError("v55b requires the sealed es-at-scale interpreter")
    return execute_v55b(prereg)


if __name__ == "__main__":
    raise SystemExit(main())
