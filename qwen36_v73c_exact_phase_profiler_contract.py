#!/usr/bin/env python3
"""Immutable runtime-only contract surface for the V73C systems trace.
...
GUARD_DIRECTORY = (ROOT / "v73c_sitecustomize").resolve()
GUARD = (GUARD_DIRECTORY / "v73c_path_open_guard.py").resolve()
SITECUSTOMIZE = (GUARD_DIRECTORY / "sitecustomize.py").resolve()
WORKER = (ROOT / "eggroll_es_worker_lora_v73c.py").resolve()
WORKER_EXTENSION = (
    "eggroll_es_worker_lora_v73c.LoRAAdapterStateWorkerExtensionV73C"
)
...
def arm_artifacts_v73c(arm: str) -> dict[str, str]:
    ...
    return {
        ...
        "actor_cuda_work_log": str(run / "actor_cuda_work_receipts_v73c.jsonl"),
        ...
        "path_guard_receipt": str(run / "systems_only_path_guard_v73c.json"),
...
def validate_generated_preregistration_v73c(
    ...
    _require(
        ...
        "V73C preregistration self hash changed",
    )
    _require(
        value.get("schema")
        ...
        == arm_artifacts_v73c("timeline")
        and value.get("arms", {}).get("hbm_metrics", {}).get("artifacts")
        == arm_artifacts_v73c("hbm_metrics")
        ...
        is False,
        "V73C generated runtime contract changed",
    )
    ...
    _require(
        ...
        == 0,
        "V73C runtime closure or systems-only quarantine boundary changed",
    )
