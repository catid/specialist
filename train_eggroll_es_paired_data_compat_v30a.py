#!/usr/bin/env python3
"""Pure train-only mechanics for the immutable V30A paired data scan."""
...
PREREGISTRATION_FILE_SHA256_V30A = (
    "543c90672961ba08e30a0bf87f0278a517257372f7564c84a848350e8951afcb"
)
PREREGISTRATION_CONTENT_SHA256_V30A = (
    "dbb0f0bd704cc7c598856f94045550c30b11c2ab3ca5881abcf3b44fab5b1423"
)
PANEL_BUNDLE_CONTENT_SHA256_V30A = (
    "c2c18d07399b04d48c1e96b299fde88b73f83bf549a272f0d087d3bfc1502e80"
)
CANDIDATE_PATH_V30A = (
    CANONICAL_ROOT / "experiments/eggroll_es_hpo/dataset_candidates/v389/"
    "train_qa_context_merit_v389.jsonl"
)
PRODUCTION_PATH_V30A = CANONICAL_ROOT / "data/train_qa_curated_v1.jsonl"
VERSIONS_V30A = ("production", "candidate_v389")
VERSION_TO_FRAME_SIDE_V30A = {
    "production": "production",
    "candidate_v389": "candidate",
}
PANEL_NAMES_V30A = frame_v30a.PANEL_NAMES
OPTIMIZATION_PANELS_V30A = frame_v30a.OPTIMIZATION_PANELS
TRAIN_SCREENS_V30A = frame_v30a.TRAIN_SCREENS
STRATA_V30A = tuple(sampler_v13.STRATA)
SIGNS_V30A = ("plus", "minus")
PAIRED_FRAME_POPULATION_V30A = frame_v30a.EXPECTED_PAIRED_UNITS
BOOTSTRAP_CHUNK_SIZE_V30A = 128
BOOTSTRAP_QUANTILE_METHOD_V30A = "linear"
EXPECTED_DRAW_PLAN_SHA256_V30A = (
...
def _read_bound_rows(path, expected_sha256, expected_rows):
    ...
    if file_sha256(path) != expected_sha256:
        raise RuntimeError("v30a paired source file identity changed")
    rows = [
        json.loads(line)
    ...
    if len(rows) != expected_rows or any(not isinstance(row, dict) for row in rows):
        raise RuntimeError("v30a paired source row count or schema changed")
    return rows


def load_hardened_preregistration_v30a():
    if (
        file_sha256(prereg_v30a.PREREGISTRATION_PATH)
        != PREREGISTRATION_FILE_SHA256_V30A
    ):
        raise RuntimeError("v30a preregistration file changed")
    persisted = json.loads(
        prereg_v30a.PREREGISTRATION_PATH.read_text(encoding="utf-8")
    )
    if (
        ...
        raise RuntimeError("v30a preregistration content changed")
    prereg_v30a.validate_sha_fields(persisted)
    return persisted


def _materialize_paired_panel_bundle_v30a():
    preregistration = load_hardened_preregistration_v30a()
    frame, _negative, _layer_plan, _baseline = (
        prereg_v30a.load_bound_evidence()
    )
    sources = {
        "production": _read_bound_rows(
            PRODUCTION_PATH_V30A,
            frame_v30a.PRODUCTION_SHA256,
            frame_v30a.PRODUCTION_ROWS,
        ),
        "candidate_v389": _read_bound_rows(
            CANDIDATE_PATH_V30A,
            frame_v30a.CANDIDATE_SHA256,
            frame_v30a.CANDIDATE_ROWS,
        ),
    }
    panels = {}
    ...
    for manifest_panel in frame["panels"]:
        ...
        version_batches = {}
        for version in VERSIONS_V30A:
            side = VERSION_TO_FRAME_SIDE_V30A[version]
            ...
            for item in items:
                ...
                if (
                    ...
                    raise RuntimeError("v30a fixed representative index changed")
                ...
                if (
                    ...
                    raise RuntimeError("v30a fixed representative content changed")
                rows.append(row)
    ...
    if anchor_counts != {
        ...
        raise RuntimeError("v30a selected pairing-anchor coverage changed")
    result = {
        "schema": "eggroll-es-materialized-paired-data-batches-v30a",
        "preregistration": {
            "file_sha256": PREREGISTRATION_FILE_SHA256_V30A,
            "content_sha256": preregistration["content_sha256_before_self_field"],
        },
        "frame": {
            "file_sha256": prereg_v30a.FRAME_FILE_SHA256,
            "content_sha256": prereg_v30a.FRAME_CONTENT_SHA256,
        ...
        "sources": {
            "production": {
                "rows": frame_v30a.PRODUCTION_ROWS,
                "file_sha256": frame_v30a.PRODUCTION_SHA256,
            },
            "candidate_v389": {
                "rows": frame_v30a.CANDIDATE_ROWS,
                "file_sha256": frame_v30a.CANDIDATE_SHA256,
            },
        },
        "panels": panels,
...
def validate_paired_panel_bundle_v30a(bundle):
    if (
        ...
        != "eggroll-es-materialized-paired-data-batches-v30a"
        ...
        or bundle.get("preregistration") != {
            "file_sha256": PREREGISTRATION_FILE_SHA256_V30A,
            "content_sha256": PREREGISTRATION_CONTENT_SHA256_V30A,
        }
        or bundle.get("frame") != {
            "file_sha256": prereg_v30a.FRAME_FILE_SHA256,
            "content_sha256": prereg_v30a.FRAME_CONTENT_SHA256,
        ...
        or tuple(bundle.get("panels", {})) != PANEL_NAMES_V30A
    ):
        raise RuntimeError("v30a materialized paired panel bundle changed")
    frame = json.loads(frame_v30a.OUTPUT_PATH.read_text(encoding="utf-8"))
    frame_v30a.validate_manifest(frame)
    manifest_panels = {item["name"]: item for item in frame["panels"]}
    anchor_counts = {"shared_document": 0, "joint_component_cross_side_link": 0}
    selected_ids = []
    for panel_name in PANEL_NAMES_V30A:
        ...
        if (
            ...
            or set(panel.get("versions", {})) != set(VERSIONS_V30A)
            or not math.isclose(
                math.fsum(panel["weights"]),
                float(PAIRED_FRAME_POPULATION_V30A),
                rel_tol=0.0,
                abs_tol=1e-12,
            )
        ):
            raise RuntimeError(f"v30a materialized {panel_name} contract changed")
        ...
        for version in VERSIONS_V30A:
            ...
            if (
                ...
                raise RuntimeError(
                    f"v30a materialized {panel_name} {version} batch changed"
                )
    if (
        ...
        raise RuntimeError("v30a selected frame identity changed")
    return bundle


def load_paired_panel_bundle_v30a():
    return validate_paired_panel_bundle_v30a(
        _materialize_paired_panel_bundle_v30a()
    )


def resident_signed_wave_schedule_v30a():
    frozen = load_hardened_preregistration_v30a()
    base_schedule = frozen["frozen_recipe"]["perturbation_basis"][
        "signed_population_schedule"
    ]
    if base_schedule != prereg_v30a.signed_population_schedule():
        raise RuntimeError("v30a signed population schedule changed")
    result = []
    for item in base_schedule:
        ...
        order = (
            list(VERSIONS_V30A)
            if signed_index % 2 == 0
            else list(reversed(VERSIONS_V30A))
        )
        result.append({
            **copy.deepcopy(item),
    ...
    if (
        ...
        or any(set(item["resident_version_order"]) != set(VERSIONS_V30A) for item in result)
        or sum(item["resident_version_order"][0] == "production" for item in result)
        != 8
    ):
        raise RuntimeError("v30a resident version-order balance changed")
    return result


def execute_paired_resident_signed_wave_v30a(
    schedule_item, *, perturb, score_version, restore,
):
    schedule = resident_signed_wave_schedule_v30a()
    ...
    if (
        ...
        raise RuntimeError("v30a resident signed-wave schedule item changed")
    if not all(callable(value) for value in (perturb, score_version, restore)):
        raise TypeError("v30a resident signed-wave callbacks must be callable")
    captures = {}
    try:
        perturb(
    ...
    if tuple(captures) != tuple(schedule_item["resident_version_order"]):
        raise RuntimeError("v30a resident paired-version capture is incomplete")
    return captures
...
def _unit_score_array(unit_scores):
    ...
    if values.shape != expected or not np.isfinite(values).all():
        raise RuntimeError("v30a paired per-unit score tensor changed")
    return values


def _panel_design(panel_bundle):
    panel_bundle = validate_paired_panel_bundle_v30a(panel_bundle)
    weights = np.asarray([
        panel_bundle["panels"][name]["weights"] for name in PANEL_NAMES_V30A
    ], dtype=np.float64)
    strata = [
        list(panel_bundle["panels"][name]["strata"])
        for name in PANEL_NAMES_V30A
    ]
    if weights.shape != (5, 39):
        raise RuntimeError("v30a paired panel design changed")
    return weights, strata


def observed_panel_scores_v30a(unit_scores, panel_bundle):
    ...
    panel_scores /= float(PAIRED_FRAME_POPULATION_V30A)
    if panel_scores.shape != (2, 5, 2, 32) or not np.isfinite(panel_scores).all():
        raise RuntimeError("v30a observed panel-score recomputation changed")
    return panel_scores
...
def _cosine_last_axis(left, right):
    ...
    if np.any(denominator == 0.0):
        raise RuntimeError("v30a endpoint cosine received a zero direction")
...
def _endpoint_arrays(panel_scores):
    ...
    if (
        ...
        raise RuntimeError("v30a panel-score endpoint tensor changed")
    ...
    if np.any(spreads == 0.0):
        raise RuntimeError("v30a panel response spread is zero")
    ...
    endpoints = {}
    for family in prereg_v30a.METRIC_FAMILIES:
    ...
    if set(endpoints) != set(prereg_v30a.ENDPOINTS) or any(
        ...
        raise RuntimeError("v30a endpoint family recomputation changed")
    return {
        "endpoints": endpoints,
...
def recompute_observed_endpoints_v30a(unit_scores, panel_bundle):
    panel_scores = observed_panel_scores_v30a(unit_scores, panel_bundle)
    analyzed = _endpoint_arrays(panel_scores[:, np.newaxis, ...])
    versions = {}
    for version_index, version in enumerate(VERSIONS_V30A):
        endpoint_values = {
        ...
        versions[version] = {
            ...
            "compact_estimator_sha256": canonical_sha256({
                "schema": "eggroll-es-compact-estimator-commitment-v30a",
                "version": version,
    ...
    return {
        ...
        "cross_version_direction_similarity_diagnostic": {
            ...
            "content_sha256": canonical_sha256({
                "schema": "eggroll-es-cross-version-direction-diagnostic-v30a",
                "cosine": cross_cosine,
            }),
        },
    }
...
def _bootstrap_draw_plan_v30a(panel_bundle, repetitions, expected_sha256):
    _weights, strata_by_panel = _panel_design(panel_bundle)
    generator = np.random.default_rng(prereg_v30a.BOOTSTRAP_SEED)
    digest = hashlib.sha256()
    draws = {}
    for panel_index, panel in enumerate(PANEL_NAMES_V30A):
        draws[panel] = {}
        strata = strata_by_panel[panel_index]
        for stratum in STRATA_V30A:
            positions = np.asarray([
                index for index, value in enumerate(strata) if value == stratum
            ], dtype=np.int64)
            quota = frame_v30a.STRATUM_QUOTAS[stratum]
            if len(positions) != quota:
                raise RuntimeError("v30a bootstrap stratum quota changed")
    ...
    if expected_sha256 is not None and actual != expected_sha256:
        raise RuntimeError("v30a bootstrap draw-plan identity changed")
    return draws, actual
...
def _paired_stratified_bootstrap_impl(
    ...
    values = _unit_score_array(unit_scores)
    observed = recompute_observed_endpoints_v30a(values, panel_bundle)["versions"]
    draw_plan, draw_plan_sha256 = _bootstrap_draw_plan_v30a(
        panel_bundle, repetitions, expected_draw_plan_sha256,
    )
    deltas = {name: np.empty(repetitions, dtype=np.float64) for name in prereg_v30a.ENDPOINTS}
    for start in range(0, repetitions, BOOTSTRAP_CHUNK_SIZE_V30A):
        stop = min(start + BOOTSTRAP_CHUNK_SIZE_V30A, repetitions)
        ...
        for panel_index, panel in enumerate(PANEL_NAMES_V30A):
            for stratum in STRATA_V30A:
                ...
                population = frame_v30a.EXPECTED_PAIRED_STRATA[stratum]
                panel_scores[:, :, panel_index] += (
                    float(population) / float(PAIRED_FRAME_POPULATION_V30A)
                ) * stratum_means
    ...
    quantile = prereg_v30a.FAMILYWISE_ALPHA / len(prereg_v30a.ENDPOINTS)
    endpoints = {}
    for name, replicate_values in deltas.items():
        if replicate_values.shape != (repetitions,) or not np.isfinite(replicate_values).all():
            raise RuntimeError("v30a bootstrap replicate coverage changed")
        observed_delta = (
            observed["candidate_v389"]["endpoint_values"][name]
            - observed["production"]["endpoint_values"][name]
        )
        endpoints[name] = {
            "candidate_v389_minus_production": float(observed_delta),
            "familywise_lcb": float(np.quantile(
                replicate_values,
                quantile,
                method=BOOTSTRAP_QUANTILE_METHOD_V30A,
            )),
            "noninferiority_margin": 0.0,
        }
    return {
        "seed": prereg_v30a.BOOTSTRAP_SEED,
...
def paired_stratified_bootstrap_v30a(unit_scores, panel_bundle):
    result = _paired_stratified_bootstrap_impl(
        unit_scores,
        panel_bundle,
        prereg_v30a.BOOTSTRAP_REPETITIONS,
        EXPECTED_DRAW_PLAN_SHA256_V30A,
    )
    if (
        result["repetitions"] != 50_000
        or result["draw_plan_sha256"] != EXPECTED_DRAW_PLAN_SHA256_V30A
        or set(result["endpoints"]) != set(prereg_v30a.ENDPOINTS)
    ):
        raise RuntimeError("v30a compact bootstrap output changed")
    return result


def build_compact_estimator_summary_v30a(unit_scores, panel_bundle):
    observed = recompute_observed_endpoints_v30a(unit_scores, panel_bundle)
    return {
        "versions": copy.deepcopy(observed["versions"]),
        "paired_bootstrap": paired_stratified_bootstrap_v30a(
...
def evaluate_candidate_v30a(summary):
    ...
    result = {
        "schema": "eggroll-es-paired-data-compat-gate-v30a",
