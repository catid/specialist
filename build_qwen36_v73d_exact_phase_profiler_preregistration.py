#!/usr/bin/env python3
"""Seal the prospective, data-free V73D exact-phase profiler contract.
...
import qwen36_v73d_exact_phase_profiler_contract as runtime_contract
...
EVIDENCE = (
    ...
    "qwen36_v73d_exact_phase_profiler_cpu_evidence_20260717.md"
).resolve()
...
SYSTEMS_ONLY_CLOSURE_AUDITOR = (
    ROOT / "audit_v73d_systems_only_import_graph_v2.py"
).resolve()
...
V73C_PREREGISTRATION = (
    ROOT
    / "experiments/eggroll_es_hpo/preregistrations/"
    "qwen36_v73c_exact_phase_profiler.json"
).resolve()
V73C_PROFILE_ATTEMPT = (
    ROOT
...
V73C_ATTEMPT_1_HASHES = {
    ...
    V73C_PROFILE_DIR / "profile_failure_v73c.json": (
    ...
    V73C_RUN_DIR / "failure_v73c.json": (
        "5708f456e7736944b7304c5a17486b82c15c5a451a51679734ff00e86cdefef7"
    ),
    V73C_RUN_DIR / "exact_phase_ranges_v73c.json": (
...
def _load_systems_only_closure_v73d() -> dict[str, Any]:
    _require(
        SYSTEMS_ONLY_CLOSURE.is_file()
        and not SYSTEMS_ONLY_CLOSURE.is_symlink(),
        "V73D external systems-only closure receipt absent",
    )
    value = json.loads(SYSTEMS_ONLY_CLOSURE.read_text(encoding="ascii"))
    _require(isinstance(value, dict), "V73D closure receipt must be an object")
    body = dict(value)
    ...
    _require(
        ...
        and value.get("schema") == "qwen36-v73d-systems-only-closure-v1"
        and value.get("status")
        == "v73d_runtime_closure_systems_only_no_quality_authority"
        ...
        == file_sha256(ROOT / "test_qwen36_v73d_exact_phase_profiler.py")
        ...
        == auditor["file_sha256"],
        "V73D external systems-only closure receipt changed",
    )
    return {
        "receipt": {
...
def _self_hashed_reference(
    ...
    _require(isinstance(value, dict), f"V73C JSON object required: {path}")
    compact = dict(value)
    ...
    _require(
        ...
        f"V73C attempt-1 self hash changed: {path}",
    )
    return {
        **_source(path),
        "content_sha256": claimed,
...
def _validate_v73c_attempt_1_predecessor() -> dict[str, Any]:
    for path, expected in V73C_ATTEMPT_1_HASHES.items():
        _require(path.is_file(), f"V73C attempt-1 artifact absent: {path}")
        _require(
            file_sha256(path) == expected,
            f"V73C attempt-1 artifact changed: {path}",
        )
    ...
    _require(
        ...
        and "V73C worker imported without the pre-import path guard"
        in run_failure.get("message", "")
        ...
        "V73C attempt-1 failure classification changed",
    )
    return {
        "status": "immutable_failed_closed_actor_sitecustomize_assumption",
        "v73c_sealed_source_commit": (
            "35d2837d1632a2b642ad9df15a7eb610d47cdc34"
        ),
        ...
        "failure_actor_pid": 1_416_807,
        "failure_cause": "actor_guard_not_installed_before_v72_import",
        "controller_sitecustomize_succeeded": True,
        "ray_actor_sitecustomize_assumption_invalid": True,
        "partial_actor_cuda_receipt_count": 0,
        "final_all_four_gpus_idle": True,
        "legacy_reserved_wall_gpu_seconds": run_failure[
            "compute_ledger"
        ]["charged_gpu_seconds"],
        "legacy_reserved_wall_not_accepted_as_model_resident_or_useful": True,
...
def _validate_quarantine_boundary_registry_v3() -> dict[str, Any]:
    ...
    _require(
        ...
        "V73D quarantine boundary registry file changed",
    )
    value = json.loads(path.read_text(encoding="ascii"))
    compact = dict(value)
    claimed = compact.pop("content_sha256_before_self_field", None)
    ...
    _require(
        ...
        "V73D quarantine boundary registry content changed",
    )
    return {
        **_source(path),
        "content_sha256": claimed,
...
def arm_artifacts_v73d(arm: str) -> dict[str, str]:
    ...
    return {
        ...
        "actor_cuda_work_log": str(
            run / "actor_cuda_work_receipts_v73d.jsonl"
        ),
...
arm_artifacts_v73d = runtime_contract.arm_artifacts_v73d
command_template_v73d = runtime_contract.command_template_v73d
expand_command_v73d = runtime_contract.expand_command_v73d
gpu_identity_v73d = runtime_contract.gpu_identity_v73d
gpu_topology_v73d = runtime_contract.gpu_topology_v73d
...
def toolchain_receipt_v73d() -> dict[str, Any]:
    required = (
        NSYS,
        NCU,
        BASH,
    ...
    for path in required:
        _require(path.is_file(), f"V73D profiler tool absent: {path}")
...
def build_preregistration_v73d() -> dict[str, Any]:
    permission = _rm_profiling_admin_only()
    _require(
        permission == 1,
        "V73D sealed blocked HBM arm requires RmProfilingAdminOnly=1",
    )
    control = _validate_v73b_control()
    predecessor = _validate_v73c_attempt_1_predecessor()
    boundary_registry = _validate_quarantine_boundary_registry_v3()
    closure = _load_systems_only_closure_v73d()
    tools = toolchain_receipt_v73d()
    gpu_identity = gpu_identity_v73d()
    gpu_topology = gpu_topology_v73d()
    timeline_artifacts = arm_artifacts_v73d("timeline")
    hbm_artifacts = arm_artifacts_v73d("hbm_metrics")
    implementation = {
    ...
    result = {
        "schema": "qwen36-v73d-exact-phase-profiler-preregistration-v1",
        "status": (
            "sealed_cpu_only_before_v73d_model_ray_gpu_or_protected_access"
        ),
        "purpose": (
            "Supersede the immutable V73C actor-bootstrap failure at fresh paths, "
            "install the V3 quarantine guard inside every prestarted Ray actor "
            "before V72, replicate the V73B no-commit workload, and measure "
            "post-V71/V72 "
        ...
        "arms": {
            "timeline": {
                ...
                "command_template": command_template_v73d("timeline"),
            ...
            "hbm_metrics": {
                ...
                "command_template_after_future_reseal": command_template_v73d(
                    "hbm_metrics"
                ),
                "command_template_sha256": canonical_sha256(
...
def render_evidence_v73d(
    ...
    expanded_nsys = runtime_contract.expand_command_v73d(
        "timeline",
        preregistration_file_sha256,
        preregistration_content_sha256,
    )
    ...
    return "\n".join([
        ...
        "",
        "- V73C attempt 1 remains immutable and failed closed because the "
        "controller sitecustomize guard did not run in the prestarted Ray "
        "actor before its V72 import.",
        "- Immutable V73C run/profile failure file SHA-256: "
        f"`{predecessor['run_failure']['file_sha256']}` / "
        f"`{predecessor['profile_failure']['file_sha256']}`.",
        "- V73D controller mechanism: "
...
def main(argv=None) -> int:
    ...
    _require(output == OUTPUT, "V73D preregistration path must remain canonical")
    _require(evidence == EVIDENCE, "V73D evidence path must remain canonical")
    value = build_preregistration_v73d()
    payload = render_json(value)
    evidence_payload = render_evidence_v73d(value, output, payload)
    if args.check:
        _require(output.is_file(), f"V73D preregistration absent: {output}")
        _require(
            output.read_text(encoding="ascii") == payload,
            f"V73D preregistration stale: {output}",
        )
        _require(evidence.is_file(), f"V73D evidence absent: {evidence}")
        _require(
            evidence.read_text(encoding="ascii") == evidence_payload,
            f"V73D evidence stale: {evidence}",
        )
    else:
