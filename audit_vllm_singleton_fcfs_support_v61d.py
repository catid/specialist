#!/usr/bin/env python3
"""CPU-only installed-vLLM audit for V61D singleton FCFS controls."""
...
OUTPUT = (
    ...
    "vllm_singleton_fcfs_support_audit_v61d.json"
).resolve()
SOURCE_PATHS = {
...
def canonical_sha256_v61d(value: object) -> str:
...
def build_audit_v61d() -> dict:
    ...
    controls_supported = (
        ...
        and all(key in fields for key in (
            "max_num_seqs", "scheduling_policy", "async_scheduling",
            "enforce_eager",
        ))
    )
    value = {
        "schema": "v61d-installed-vllm-singleton-fcfs-support-audit",
        ...
        "source_file_sha256": {
            key: file_sha256_v61d(path) for key, path in SOURCE_PATHS.items()
        },
        ...
        "singleton_fcfs_controls_supported": controls_supported,
        ...
        "qwen_gdn_batch_invariance_support_required": False,
        "gpu_model_or_train_semantics_accessed": False,
        "gpu_launch_authorized_by_audit": controls_supported,
        "protected_semantics_opened": False,
    }
    value["content_sha256_before_self_field"] = canonical_sha256_v61d(value)
    return value
