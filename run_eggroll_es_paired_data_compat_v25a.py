#!/usr/bin/env python3
"""Fail-closed four-GPU train-only runtime for immutable V25A."""
...
import train_eggroll_es_paired_data_compat_v25a as mechanics_v25a


ROOT = Path(__file__).resolve().parent
PREREG_PATH_V25A = prereg_v25a.PREREGISTRATION_PATH
PREREG_FILE_SHA256_V25A = mechanics_v25a.PREREGISTRATION_FILE_SHA256_V25A
PREREG_CONTENT_SHA256_V25A = mechanics_v25a.PREREGISTRATION_CONTENT_SHA256_V25A
...
TEST_PATH_V25A = (
    ROOT / "test_run_eggroll_es_paired_data_compat_v25a.py"
).resolve()
MECHANICS_TEST_PATH_V25A = (
    ROOT / "test_train_eggroll_es_paired_data_compat_v25a.py"
).resolve()
BF16_CONFIG_SHA256_V25A = (
    "93a4693fa9d8392fbfccd4b3c9873f4bfdcb14fdede978b123d07d19675efe99"
)
BF16_INDEX_SHA256_V25A = (
    "41b9356101ebf8e7519e150dc811f80c4226e727301fbb032b890f006ed0be83"
)
BOUND_COMMIT_PATHS_V25A = {
    FRAME_COMMIT_V25A: {
        "frame_builder_v25a": (
            ROOT / "build_eggroll_es_joint_panels_v25a.py",
            prereg_v25a.FRAME_BUILDER_SHA256,
        ),
        "frame_tests_v25a": (
            ROOT / "test_build_eggroll_es_joint_panels_v25a.py",
            prereg_v25a.FRAME_TEST_SHA256,
        ),
        "frame_manifest_v25a": (
            prereg_v25a.FRAME_PATH,
            prereg_v25a.FRAME_FILE_SHA256,
        ),
    },
    PREREG_COMMIT_V25A: {
        "prereg_module_v25a": (
            Path(prereg_v25a.__file__).resolve(),
            "3b923cd507ecfece3a4656255106c566f61ee78eb805f25c261a02c7cb02b25c",
        ),
        "prereg_tests_v25a": (
            ROOT / "test_eggroll_es_paired_data_compat_preregistration_v25a.py",
            "ba2041127db4e3635073ca29764235db101f9cadfa78866c1b6d28c0990a8c58",
        ),
        "preregistration_v25a": (
            PREREG_PATH_V25A,
            PREREG_FILE_SHA256_V25A,
        ),
    },
}
IMPLEMENTATION_PATHS_V25A = {
    **{
        key: path
        for files in BOUND_COMMIT_PATHS_V25A.values()
        for key, (path, _digest) in files.items()
    },
    ...
    "worker_v25a": Path(worker_r1.__file__).resolve(),
    "mechanics_v25a": Path(mechanics_v25a.__file__).resolve(),
    "mechanics_tests_v25a": MECHANICS_TEST_PATH_V25A,
    "runtime_v25a": Path(__file__).resolve(),
    "runtime_tests_v25a": TEST_PATH_V25A,
}
FORBIDDEN_ARGV_TOKENS_V25A = (
    "checkpoint", "update", "heldout", "validation", "ood", "benchmark",
    "eval", "save-model", "train-dataset", "promotion", "union",
)
FORBIDDEN_PERSISTED_KEYS_V25A = {
...
def _assert_compact(value):
    overlap = FORBIDDEN_PERSISTED_KEYS_V25A & set(_recursive_keys(value))
    if overlap:
        raise RuntimeError(f"v25a compact output contains forbidden keys: {sorted(overlap)}")


def _assert_train_only_argv(argv):
    for token in argv:
        lowered = str(token).lower()
        if any(value in lowered for value in FORBIDDEN_ARGV_TOKENS_V25A):
            raise ValueError(f"v25a rejects forbidden runtime surface: {token}")


def _verify_bound_commit_files_v25a():
    identities = {}
    for commit, files in BOUND_COMMIT_PATHS_V25A.items():
        for key, (path, expected) in files.items():
            ...
            if commit_sha != expected or file_sha256(path) != expected:
                raise RuntimeError("v25a immutable frame or preregistration changed")
            identities[key] = {
                "commit": commit,
...
def load_preregistration_v25a():
    _verify_bound_commit_files_v25a()
    value = json.loads(PREREG_PATH_V25A.read_text(encoding="utf-8"))
    if (
        file_sha256(PREREG_PATH_V25A) != PREREG_FILE_SHA256_V25A
        or value.get("content_sha256_before_self_field")
        != PREREG_CONTENT_SHA256_V25A
        or value.get("content_sha256_before_self_field")
        ...
        raise RuntimeError("v25a immutable preregistration changed")
    prereg_v25a.validate_sha_fields(value)
    return value


def implementation_identity_v25a():
    inherited = runtime_r2.implementation_identity_r2()
    bound_commits = _verify_bound_commit_files_v25a()
    files = dict(inherited["files"])
    for key, path in IMPLEMENTATION_PATHS_V25A.items():
        files[key] = {
    ...
    return {
        ...
        "immutable_bound_commits": bound_commits,
        "v25a_overlay_bundle_sha256": canonical_sha256({
            key: files[key] for key in IMPLEMENTATION_PATHS_V25A
        }),
        "bundle_sha256": canonical_sha256(files),
    }


def seed_projection_contract_v25a(preregistration):
    ...
    if (
        seeds != prereg_v25a.perturbation_seeds()
        or len(seeds) != 32
        or len(set(seeds)) != 32
        or len({item["numpy_legacy_seed"] for item in projections}) != 32
        or any(item["numpy_legacy_seed"] == 0 for item in projections)
    ):
        raise RuntimeError("v25a perturbation seed projection changed")
    return {
        "schema": "eggroll-es-v25a-seed-projection-contract",
        ...
        "worker_extension": WORKER_EXTENSION_V25A,
    }


def load_layer_bundle_v25a(preregistration):
    ...
    return anchor_v4.load_frozen_layer_plan_v4(
        ...
        expected_model_config_sha256=BF16_CONFIG_SHA256_V25A,
    )


def validate_live_model_v25a(preregistration):
    model = Path(preregistration["frozen_recipe"]["model"]).resolve()
    if (
        file_sha256(model / "config.json") != BF16_CONFIG_SHA256_V25A
        or file_sha256(model / "model.safetensors.index.json")
        != BF16_INDEX_SHA256_V25A
    ):
        raise RuntimeError("v25a live BF16 model identity changed")
    return _seal({
        "schema": "eggroll-es-v25a-live-model-audit",
        "model_path": str(model),
        "config_sha256": BF16_CONFIG_SHA256_V25A,
        "index_sha256": BF16_INDEX_SHA256_V25A,
    })
...
def recipe_v25a(
    ...
    mechanics_v25a.validate_paired_panel_bundle_v25a(panel_bundle)
    schedule = mechanics_v25a.resident_signed_wave_schedule_v25a()
    panels = {}
    for name in mechanics_v25a.PANEL_NAMES_V25A:
    ...
    value = {
        "schema": "eggroll-es-paired-data-compat-runtime-recipe-v25a",
        "experiment_name": EXPERIMENT_NAME_V25A,
        "preregistration": {
            "path": str(PREREG_PATH_V25A),
            "file_sha256": PREREG_FILE_SHA256_V25A,
            "content_sha256": PREREG_CONTENT_SHA256_V25A,
            "commit": PREREG_COMMIT_V25A,
        },
        "frame": {
            **copy.deepcopy(panel_bundle["frame"]),
            "commit": FRAME_COMMIT_V25A,
        ...
        "perturbation": {
            ...
            "alpha": 0.0,
            "population_size": 32,
            "basis_seed": prereg_v25a.PERTURBATION_BASIS_SEED,
            "direction_seed_list_sha256": canonical_sha256(
                prereg_v25a.perturbation_seeds()
            ),
        ...
        "seed_projection": seed_projection_contract_v25a(preregistration),
        "scoring": {
        ...
        "endpoints": list(prereg_v25a.ENDPOINTS),
        ...
        "output_directory": str(OUTPUT_DIRECTORY_V25A),
        "fresh_exclusive_paths": {
            "attempt_name": ATTEMPT_NAME_V25A,
            "run_directory_name": EXPERIMENT_NAME_V25A,
            "report_name": REPORT_NAME_V25A,
        },
        "authority": {
            ...
            "pass_authorizes_only_separate_full_v364_train_only_hpo_preregistration": True,
        },
    }
    return _seal(value)


def validate_runtime_v25a(args, preregistration, implementation, recipe):
    load_preregistration_v25a()
    if preregistration.get("content_sha256_before_self_field") != PREREG_CONTENT_SHA256_V25A:
        raise ValueError("v25a preregistration changed")
    if any(
        os.environ.get(key) for key in runtime_v23a.MOE_OVERRIDE_ENVIRONMENT_V23A
    ):
        raise ValueError("v25a rejects external MoE backend overrides")
    if os.environ.get("VLLM_BATCH_INVARIANT") not in (None, "0"):
        raise ValueError("v25a rejects the vLLM batch-invariant backend swap")
    if recipe.get("content_sha256_before_self_field") != canonical_sha256(
        _without_self(recipe)
    ):
        raise ValueError("v25a runtime recipe changed")
    for expected, actual, label in (
        ...
        if not args.v25a_dry_run and expected is None:
            raise ValueError(f"v25a real launch requires expected {label} hash")
        if expected is not None and expected != actual:
            raise ValueError(f"v25a {label} hash changed")
...
def _score_panel_outputs(dense_items, outputs):
    ...
    if rewards.shape != (39,) or not np.isfinite(rewards).all():
        raise RuntimeError("v25a per-unit dense score coverage changed")
...
def _phase_identity(phase):
    ...
    for version in mechanics_v25a.VERSIONS_V25A:
        ...
        if values.shape != (4, 5, 39) or not np.isfinite(values).all():
            raise RuntimeError("v25a full-context score geometry changed")
        identities[version] = {
            "shape": [4, 5, 39],
...
class PairedDataCompatRuntimeMixinV25A:
    def configure_paired_data_compat_v25a(
        ...
        panel_bundle = mechanics_v25a.validate_paired_panel_bundle_v25a(
        ...
        if (
            ...
            raise RuntimeError("v25a exact four-engine alpha-zero recipe changed")
        projection = seed_projection_contract_v25a(preregistration)
        seeds = preregistration["frozen_recipe"]["perturbation_basis"][
            "direction_seeds"
        ]
        seed_reports = [
        ...
        if (
            ...
            raise RuntimeError("v25a worker seed certificates changed")
        devices = [
        ...
        for rank, report in enumerate(devices):
            if (
                ...
                raise RuntimeError("v25a fixed rank-to-GPU mapping changed")
        installs = [
        ...
        if (
            ...
            raise RuntimeError("v25a layer-plan installation changed")
        references = [
        ...
        return _seal({
            "schema": "eggroll-es-paired-runtime-configuration-v25a",
    ...
    def _prepared_fixed_batches_v25a(self):
        bundle = mechanics_v25a.validate_paired_panel_bundle_v25a(
            self._v25a_panel_bundle
        )
        prepared = {}
        request_identity = {}
        token_audit = {}
        for version in mechanics_v25a.VERSIONS_V25A:
            ...
            cursor = 0
            token_panels = {}
            for panel_name in mechanics_v25a.PANEL_NAMES_V25A:
            ...
            if cursor != 195 or any(
                ...
                raise RuntimeError("v25a fixed version request/token count changed")
        ...
        self._v25a_fixed_request_identity = request_identity
        self._v25a_token_audit_sha256 = canonical_sha256(token_audit)
        return prepared, request_identity, token_audit

    def _assert_fixed_request_v25a(self, version, prepared):
        if (
            version not in mechanics_v25a.VERSIONS_V25A
            or _request_identity(prepared[version]["prompt_items"])
            != self._v25a_fixed_request_identity[version]
        ):
            raise RuntimeError("v25a fixed version request identity changed")

    def _generate_version_all_engines_v25a(self, version, prepared):
        self._assert_fixed_request_v25a(version, prepared)
        ...
        if len(batches) != 4 or any(len(batch) != 195 for batch in batches):
            raise RuntimeError("v25a all-engine version generation incomplete")
        return batches

    def _score_version_batches_v25a(self, version, prepared, batches):
        scores = np.empty((4, 5, 39), dtype=np.float64)
        commitments = []
        for engine_index, batch in enumerate(batches):
            for panel_index, panel_name in enumerate(mechanics_v25a.PANEL_NAMES_V25A):
    ...
    def _full_context_phase_v25a(self, prepared):
        scores = {}
        commitments = {}
        for version in mechanics_v25a.VERSIONS_V25A:
            batches = self._generate_version_all_engines_v25a(version, prepared)
            scores[version], commitments[version] = self._score_version_batches_v25a(
                version, prepared, batches
            )
        return scores, commitments

    def _perturb_signed_wave_v25a(self, seeds, negate):
        if len(seeds) != 4:
            raise RuntimeError("v25a partial perturbation wave forbidden")
        raw = self._resolve([
            engine.collective_rpc.remote(
    ...
    def _score_resident_version_v25a(
        self, version, prepared, schedule_item, unit_scores, commitments,
    ):
        batches = self._generate_version_all_engines_v25a(version, prepared)
        scores, dense = self._score_version_batches_v25a(
            version, prepared, batches
        )
        version_index = mechanics_v25a.VERSIONS_V25A.index(version)
        sign_index = mechanics_v25a.SIGNS_V25A.index(schedule_item["sign"])
    ...
    def _restore_and_verify_v25a(self):
        restored = [
        ...
        if (
            ...
            raise RuntimeError("v25a exact selected restore changed")
        return canonical_sha256(checks)

    def _run_signed_wave_v25a(
        self, schedule_item, prepared, unit_scores, commitments,
    ):
        restore_hashes = []
        captures = mechanics_v25a.execute_paired_resident_signed_wave_v25a(
            schedule_item,
            perturb=lambda seeds, negate: self._perturb_signed_wave_v25a(
                seeds, negate
            ),
            score_version=lambda version: self._score_resident_version_v25a(
                version, prepared, schedule_item, unit_scores, commitments
            ),
            restore=lambda: restore_hashes.append(self._restore_and_verify_v25a()),
        )
        if len(restore_hashes) != 1 or tuple(captures) != tuple(
            schedule_item["resident_version_order"]
        ):
            raise RuntimeError("v25a resident signed-wave transaction changed")
        return restore_hashes[0]

    def _population_boundary_v25a(self):
        reports = [
            ...
            for item in self._resolve([
                self.engines[rank].collective_rpc.remote(
                    ...
                    args=(4, 1, self._v25a_references[rank]["sha256"]),
                )
                for rank in range(4)
            ])
        ]
        if any(
            ...
            != self._v25a_references[index]["sha256"]
            or item.get("current_identity") != self._v25a_references[index]
            for index, item in enumerate(reports)
        ):
            raise RuntimeError("v25a selected/unselected population boundary changed")
        return canonical_sha256(reports)

    def estimate_paired_data_compat_v25a(self):
        schedule = mechanics_v25a.resident_signed_wave_schedule_v25a()
        prepared, request_identity, token_audit = self._prepared_fixed_batches_v25a()
        phase_a = self._full_context_phase_v25a(prepared)
        phase_b = self._full_context_phase_v25a(prepared)
        a_b_equal = _phase_equal(phase_a, phase_b)
        if not all(a_b_equal.values()):
            raise RuntimeError("v25a full-context A/B guard is not exact")
        unit_scores = np.full((2, 5, 2, 32, 39), np.nan, dtype=np.float64)
        dense_commitments = []
        restore_hashes = []
        for schedule_item in schedule:
            restore_hashes.append(self._run_signed_wave_v25a(
        ...
        if (
            ...
            raise RuntimeError("v25a paired population capture incomplete")
        boundary_sha256 = self._population_boundary_v25a()
        phase_c = self._full_context_phase_v25a(prepared)
        a_c_equal = _phase_equal(phase_a, phase_c)
        if not all(a_c_equal.values()):
            raise RuntimeError("v25a full-context A/C guard is not exact")
        compact = mechanics_v25a.build_compact_estimator_summary_v25a(
            unit_scores, self._v25a_panel_bundle
        )
        unit_scores = None
        runtime_integrity = {
        ...
        summary = _seal({
            "schema": "eggroll-es-paired-data-compat-summary-v25a",
            "experiment_name": EXPERIMENT_NAME_V25A,
            "alpha": 0.0,
            "sigma": 0.0003,
            "frame_content_sha256": prereg_v25a.FRAME_CONTENT_SHA256,
            "perturbation_basis_content_sha256": self._v25a_preregistration[
                "frozen_recipe"
        ...
        gate = mechanics_v25a.evaluate_candidate_v25a(summary)
        guard = {
            "schema": "eggroll-es-v25a-full-context-a-b-c-guard",
        ...
        audit = _seal({
            "schema": "eggroll-es-paired-runtime-compact-audit-v25a",
    ...
    @staticmethod
    def _closed_surface_v25a(*_args, **_kwargs):
        raise RuntimeError("v25a closes update checkpoint evaluation and union surfaces")

    configure_anchor = _closed_surface_v25a
    configure_train_panels_v13 = _closed_surface_v25a
    estimate_train_panels_v13 = _closed_surface_v25a
    estimate_step_coefficients = _closed_surface_v25a
    apply_seed_coefficients = _closed_surface_v25a
    train_step = _closed_surface_v25a
    evaluate_handle = _closed_surface_v25a
    evaluate_population_on_batch = _closed_surface_v25a
    eval_step = _closed_surface_v25a
    fit = _closed_surface_v25a


def load_runtime_trainer_v25a(preregistration, layer_bundle):
    model = preregistration["frozen_recipe"]["model"]
    parent = anchor_v4.load_trainer(layer_bundle)

    class PairedDataCompatRuntimeTrainerV25A(
        PairedDataCompatRuntimeMixinV25A, parent,
    ):
        def launch_engines(
            ...
            if num_engines != 4 or n_gpu_per_vllm_engine != 1:
                raise ValueError("v25a requires exactly four TP1 engines")
            import ray
            ...
            for rank in range(4):
                engines.append(ray.remote(
                    ...
                    worker_extension_cls=WORKER_EXTENSION_V25A,
                    dtype=precision,
    ...
    return PairedDataCompatRuntimeTrainerV25A


def _make_trainer_v25a(preregistration, layer_bundle):
    cls = load_runtime_trainer_v25a(preregistration, layer_bundle)
    return cls(
...
def _parser():
    ...
    parser.add_argument("--v25a-dry-run", action="store_true")
...
def run_exact_v25a(
    ...
    attempt_path = OUTPUT_DIRECTORY_V25A / ATTEMPT_NAME_V25A
    run_dir = OUTPUT_DIRECTORY_V25A / EXPERIMENT_NAME_V25A
    report_path = run_dir / REPORT_NAME_V25A
    if attempt_path.exists() or run_dir.exists():
        raise RuntimeError("v25a requires fresh exclusive attempt and run paths")
    provenance = runtime_v23a._source_provenance_v23a(implementation)
    live_model = validate_live_model_v25a(preregistration)
    attempt = {
        "schema": "eggroll-es-durable-launch-attempt-v25a",
        "status": "launching",
    ...
    if run_dir.exists():
        attempt.update({
            ...
            "failure_sha256": canonical_sha256(
                "v25a run directory appeared after exclusive attempt claim"
            ),
        })
        runtime_v23a._rewrite_json_v23a(attempt_path, attempt)
        raise RuntimeError("v25a run directory appeared after exclusive attempt claim")
    trainer = None
    failure = None
    configuration = summary = gate = audit = None
    try:
        runtime_v23a.base.set_seed(43)
        trainer = _make_trainer_v25a(preregistration, layer_bundle)
        configuration = trainer.configure_paired_data_compat_v25a(
            preregistration, panel_bundle, layer_bundle
        )
        summary, gate, audit = trainer.estimate_paired_data_compat_v25a()
        if gate != mechanics_v25a.evaluate_candidate_v25a(summary):
            raise RuntimeError("v25a compact gate recomputation changed")
    ...
    if failure is not None:
        attempt.update({
            "status": "failed",
            "phase": "inside_v25a_train_only_runtime",
        ...
        raise failure
    report = _seal({
        "schema": "eggroll-es-paired-data-compat-report-v25a",
        "recipe": recipe,
...
def main(argv=None):
    ...
    preregistration = load_preregistration_v25a()
    panel_bundle = mechanics_v25a.load_paired_panel_bundle_v25a()
    layer_bundle = load_layer_bundle_v25a(preregistration)
    implementation = implementation_identity_v25a()
    recipe = recipe_v25a(
        preregistration, panel_bundle, layer_bundle, implementation
    )
    validate_runtime_v25a(args, preregistration, implementation, recipe)
    if args.v25a_dry_run:
        payload = _seal({
            "schema": "eggroll-es-paired-data-compat-dry-run-v25a",
            "implementation": implementation,
            "recipe": recipe,
            "fresh_exclusive_attempt_required": True,
            "real_launch_requires_exact_post_cherry_pick_implementation_and_recipe_hashes": True,
            "pass_authority_limited_to_separate_full_v364_train_only_hpo_preregistration": True,
