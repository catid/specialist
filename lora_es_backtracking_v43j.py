#!/usr/bin/env python3
"""CPU-only evidence and deterministic scale planning for V43J."""
...
TARGET_NORM_RATIOS_V43J = (0.25, 0.125, 0.0625)
V43I_NORM_RATIO = 0.5
EXPECTED_FILES_V43J = {
...
def canonical_sha256_v43j(value: object) -> str:
    return fused.canonical_sha256_v43i(value)


def _sealed(path: Path, expected: tuple[str, str]) -> dict:
    if file_sha256_v43j(path) != expected[0]:
        raise RuntimeError(f"v43j parent file identity changed: {path}")
    ...
    if (
        ...
        or canonical_sha256_v43j(compact) != expected[1]
    ):
        raise RuntimeError(f"v43j parent content identity changed: {path}")
    return value


def load_v43i_evidence_v43j(paths: dict[str, Path]) -> dict:
    if set(paths) != set(EXPECTED_FILES_V43J):
        raise ValueError("v43j V43I evidence inventory changed")
    values = {
        label: _sealed(Path(paths[label]), EXPECTED_FILES_V43J[label])
        for label in EXPECTED_FILES_V43J
    }
    ...
    if (
        ...
        raise RuntimeError("v43j required V43I negative/restoration evidence changed")
    plans = scale_plans_v43j(coefficients, projection["diagnostics"])
    return {
        "schema": "sealed-v43i-negative-evidence-for-backtracking-v43j",
        "file_and_content_sha256": {
            label: {"file_sha256": hashes[0], "content_sha256": hashes[1]}
            for label, hashes in EXPECTED_FILES_V43J.items()
        },
        ...
        "restored_runtime_values_sha256": RESTORED_RUNTIME_SHA256_V43J,
...
def scale_plans_v43j(
    ...
    if (
        ...
        raise ValueError("v43j requires the exact V43I 0.5-ratio projection")
    ...
    if not math.isclose(
        ...
        raise RuntimeError("v43j V43I coefficient norm no longer matches projection")
    plans = []
    for target in TARGET_NORM_RATIOS_V43J:
        ...
        if not math.isclose(ratio, target, rel_tol=0.0, abs_tol=1e-12):
            raise RuntimeError("v43j deterministic scaled norm changed")
        plans.append({
...
def largest_passing_scale_v43j(results: list[dict]) -> float | None:
    """Validate sequential backtracking and return the first/largest pass."""
    if not isinstance(results, list) or len(results) > len(TARGET_NORM_RATIOS_V43J):
        raise ValueError("v43j scale result coverage changed")
    for index, result in enumerate(results):
        if result.get("target_norm_ratio") != TARGET_NORM_RATIOS_V43J[index]:
            raise ValueError("v43j scale evaluation order changed")
        passed = result.get("gate_passed") is True
        if passed:
            if index != len(results) - 1:
                raise ValueError("v43j evaluated a smaller scale after a passing scale")
            return TARGET_NORM_RATIOS_V43J[index]
        if result.get("exact_abort_readback_passed") is not True:
            raise ValueError("v43j failed scale was not exactly aborted")
    return None
