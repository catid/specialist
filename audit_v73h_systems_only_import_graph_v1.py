#!/usr/bin/env python3
"""Seal the V73H runtime closure and process-start actor-guard repair."""
...
ROOT_MODULES = (
    "run_qwen36_v73h_exact_phase_profiler.py",
    "run_lora_es_v71_v72_profile_calibration_v73h.py",
)
...
MUTABLE_PREREG_BUILDER = (
    "build_qwen36_v73h_exact_phase_profiler_preregistration.py"
)
TEST_MODULE = "test_qwen36_v73e_exact_phase_profiler.py"
SUCCESSOR_TEST_MODULE = "test_qwen36_v73h_process_start_actor_guard.py"
...
SYNTHETIC_GUARD_TEST_NODES = (
    ...
    f"{SUCCESSOR_TEST_MODULE}::test_controller_sitecustomize_covers_preloaded_parent",
    f"{SUCCESSOR_TEST_MODULE}::test_actor_sitecustomize_covers_realistic_preloaded_parent",
    f"{SUCCESSOR_TEST_MODULE}::test_actor_sitecustomize_wrong_hash_fails_before_worker_parent_import",
    f"{SUCCESSOR_TEST_MODULE}::test_actor_flag_without_inherited_controller_pid_fails_closed",
)
...
def run_synthetic_guard_tests() -> dict[str, Any]:
    # Importing the V73H contract deliberately rebinds inherited V73E globals
    # to fresh guard paths. Keep legacy and successor collection in distinct
    # interpreters so pytest's eager module collection cannot cross-contaminate
    # either sealed contract.
    legacy_nodes = tuple(
        node for node in SYNTHETIC_GUARD_TEST_NODES
        if not node.startswith(f"{SUCCESSOR_TEST_MODULE}::")
    )
    successor_nodes = tuple(
        node for node in SYNTHETIC_GUARD_TEST_NODES
        if node.startswith(f"{SUCCESSOR_TEST_MODULE}::")
    )
    commands = [
        [str(REQUIRED_PYTHON), "-m", "pytest", "-q", *nodes]
        for nodes in (legacy_nodes, successor_nodes)
    ]
    for command in commands:
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                "V73H selected synthetic guard tests failed: "
    ...
    return {
        ...
        "command_sha256": canonical_sha256(commands),
        "isolated_subprocess_count": len(commands),
