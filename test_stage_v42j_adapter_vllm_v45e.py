...
import stage_v42j_adapter_vllm_v45e as subject


def test_v42j_source_is_sealed_matched_train_only_v45e():
    result = subject.audit_source_v45e()
    assert len(result["records"]) == 70
    assert sum(row["elements"] for row in result["records"]) == 4_528_128


def test_v42j_stage_is_byte_exact_v45e(tmp_path):
    output = tmp_path / "v42j"
