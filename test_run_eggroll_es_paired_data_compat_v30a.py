...
def test_v30a_all_engine_generation_submits_four_before_resolve():
    events = []
    ...
    class Harness(runtime.PairedDataCompatRuntimeMixinV30A):
        def __init__(self):
            ...
            self._v30a_fixed_request_identity = {
    ...
    batches = Harness()._generate_version_all_engines_v30a(
        "production", prepared
    )
...
def test_v30a_fresh_schedule_seeds_reach_each_worker_without_bypass():
    events = []
    ...
    schedule_item = runtime.mechanics_v30a.resident_signed_wave_schedule_v30a()[7]
    Harness()._perturb_signed_wave_v30a(
...
def test_v30a_commit_certificate_is_real_launch_only(monkeypatch):
    ...
    dry_actual = runtime.validate_runtime_v30a(
        SimpleNamespace(
            v30a_dry_run=True,
            expected_implementation_bundle_sha256=None,
            expected_recipe_sha256=None,
    ...
    actual = runtime.validate_runtime_v30a(
        SimpleNamespace(
            v30a_dry_run=False,
...
def test_v30a_worktree_allowlist_is_explicit_and_fail_closed():
    allowed = "\n".join((
    ...
    certificate = runtime._validate_worktree_status_v30a(allowed)
    ...
    for rejected in (
        "?? data/manual_reviews/context_merit_audit_v389/late.jsonl",
        "?? unexpected-user-file.txt",
        " M run_eggroll_es_paired_data_compat_v30a.py",
        "M  test_run_eggroll_es_paired_data_compat_v30a.py",
    ):
        with pytest.raises(RuntimeError, match="explicit untracked allowlist"):
            runtime._validate_worktree_status_v30a(rejected)
...
def test_v30a_attempt_claim_provenance_environment_and_model_precede_trainer():
    ...
    assert source.index("assert_all_four_gpus_idle_v30a()") > source.index(
        "validate_live_model_v30a"
    )
    assert source.index("assert_all_four_gpus_idle_v30a()") < source.index(
        "_exclusive_write_json_v23a(attempt_path, attempt)"
    )
    ...
    assert source.index("base.close_trainer(trainer)") < source.rindex(
        "wait_for_final_gpu_idle_v30a(prelaunch_idle)"
    )
    assert source.rindex("wait_for_final_gpu_idle_v30a(prelaunch_idle)") < source.index(
        ...
        "gpus": [
            {
                ...
                "name": runtime.EXPECTED_GPU_NAME_V30A,
                "driver_version": runtime.EXPECTED_DRIVER_VERSION_V30A,
                "nvml_uuid": runtime.EXPECTED_GPU_IDENTITIES_V30A[index][
                    "nvml_uuid"
                ] + identity_suffix,
                "pci_bus_id": runtime.EXPECTED_GPU_IDENTITIES_V30A[index][
                    "pci_bus_id"
                ],
                "total_bytes": runtime.EXPECTED_GPU_IDENTITIES_V30A[index][
                    "total_bytes"
                ],
...
def test_v30a_prelaunch_and_bounded_final_idle_are_fail_closed(monkeypatch):
    monkeypatch.setattr(
        runtime, "_observe_all_four_gpus_v30a", lambda: _gpu_observation()
    )
    prelaunch = runtime.assert_all_four_gpus_idle_v30a()
    ...
    monkeypatch.setattr(
        runtime, "_observe_all_four_gpus_v30a", lambda: next(observations)
    )
    monkeypatch.setattr(runtime.time, "sleep", lambda _seconds: None)
    final = runtime.wait_for_final_gpu_idle_v30a(prelaunch)
    ...
    with pytest.raises(RuntimeError, match="physical GPU identity changed"):
        runtime.wait_for_final_gpu_idle_v30a(prelaunch)


def test_v30a_durable_success_and_failure_both_bind_final_idle(
    ...
    committed_clean = runtime._seal({
        ...
        "allowlist_contract_sha256": runtime.canonical_sha256(
            runtime.WORKTREE_ALLOWLIST_CONTRACT_V30A
        ),
    })
    ...
    monkeypatch.setattr(runtime, "OUTPUT_DIRECTORY_V30A", tmp_path)
    monkeypatch.setattr(
        runtime.runtime_r2,
    ...
    monkeypatch.setattr(
        runtime, "validate_live_model_v30a", lambda _prereg: {"schema": "model"}
    )
    monkeypatch.setattr(
        runtime, "assert_all_four_gpus_idle_v30a", lambda: prelaunch
    )
    monkeypatch.setattr(
        runtime,
        "wait_for_final_gpu_idle_v30a",
    ...
    monkeypatch.setattr(
        runtime.mechanics_v30a, "evaluate_candidate_v30a", lambda _summary: gate
    )
    ...
    class Trainer:
        ...
        @staticmethod
        def configure_paired_data_compat_v30a(*_args):
    ...
    def install_trainer(failure=None):
        run_dir = tmp_path / runtime.EXPERIMENT_NAME_V30A
        ...
        monkeypatch.setattr(
            runtime, "_make_trainer_v30a", make_trainer
        )
        return trainer

    successful_trainer = install_trainer()
    report = runtime.run_exact_v30a(
        {}, {}, {}, implementation, recipe, committed_clean
    )
    attempt_path = tmp_path / runtime.ATTEMPT_NAME_V30A
    ...
    def fail_report_write(path, value):
        if path.name == runtime.REPORT_NAME_V30A:
