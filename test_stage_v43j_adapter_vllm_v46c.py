...
import stage_v43j_adapter_vllm_v46c as subject


def test_v43j_result_gate_consensus_snapshot_and_gpu_seals_are_bound_v46c():
    seal = subject.source_seal_v46c()
    source = subject.audit_source_v46c()
    assert seal["state_complete"] is True
    assert seal["accepted_target_norm_ratio"] == 0.25
...
def test_v43j_stage_is_byte_exact_language_model_namespace_v46c(tmp_path):
    output = tmp_path / "v43j"
    manifest = subject.stage_v46c(output)
    source = subject.audit_source_v46c()
    with safe_open(
