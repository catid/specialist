#!/usr/bin/env python3
"""No-CUDA-compute contract, numeric, finalizer, and tamper tests for V64."""
...
import build_lora_es_v59_vs_v434_robust_confirmation_preregistration_v64 as builder
import finalize_lora_es_v59_vs_v434_robust_confirmation_v64 as finalizer
...
def _file_row(path: Path) -> dict:
    return {
        ...
        "file_sha256": runtime.file_sha256_v64(path),
    }
...
def _toy_base_model(tmp_path: Path) -> tuple[Path, dict]:
    ...
    expectation = {
        ...
        "weight_shard_manifest_sha256": analysis.canonical_sha256_v64(
            weight_shards
        ),
        "non_weight_file_count": 2,
        "non_weight_files": non_weight_files,
        "non_weight_manifest_sha256": analysis.canonical_sha256_v64(
            non_weight_files
        ),
...
def _receipt(period_kind: str, period_index: int) -> dict:
    ...
    assignments = runtime._assignments_v64(period_kind, period_index)
    return {
        ...
        "active_adapter_receipts": [{
            ...
            "active_manager_cache_lora_ids": [assignment["lora_int_id"]],
            ...
            "staged_weights_file_sha256": assignment["weights_file_sha256"],
            "canonical_fp32_state_sha256": assignment[
            ...
            "source_linked_runtime_values_sha256": assignment[
                "runtime_bf16_values_sha256"
            ],
            "registered_slot_view_count": 82,
            "registered_slot_records_sha256": _sha(
                f"registered-slot-{period_kind}-{period_index}-{actor_rank}"
            ),
...
def make_evidence(
    ...
    *,
    ...
    rows = []
    for index in range(analysis.RANKING_UNITS_V64):
        rows.append({
            "row_sha256": _sha(f"row-{index}"),
            "unit_identity_sha256": _sha(f"unit-{index}"),
            "role": "ranking",
        })
    for sentinel_index, unit_sha in enumerate(analysis.SENTINEL_ORDER_V64):
        rows.append({
    ...
    label_seen = {
        ...
        for sentinel_index in range(analysis.EXACT_SENTINEL_UNITS_V64)
    ...
    for period_index in range(analysis.SCORED_PERIODS_V64):
        ...
        for actor_rank in range(analysis.ACTORS_V64):
            label = analysis.LABEL_PLAN_V64[str(actor_rank)][period_index]
            batch = []
            for row_index in range(analysis.RANKING_UNITS_V64):
                base = 0.4
            ...
            for sentinel_index in range(analysis.EXACT_SENTINEL_UNITS_V64):
                ...
                sentinel_sha = analysis.SENTINEL_ORDER_V64[sentinel_index]
                if sentinel_sha in analysis.STABLE_EXACT_UNIT_SHA256_V64:
                    stable_index = analysis.STABLE_EXACT_UNIT_SHA256_V64.index(
                        sentinel_sha
                    )
                    count = (
    ...
    return runtime.build_evidence_v64(
...
def passing_analysis(passing_evidence) -> dict:
    return analysis.build_analysis_v64(passing_evidence)


def _write_self_hashed(path: Path, value: dict) -> finalizer.SelfHashedSourceV64:
    value = copy.deepcopy(value)
    value.pop("content_sha256_before_self_field", None)
    content = analysis.canonical_sha256_v64(value)
    value["content_sha256_before_self_field"] = content
    path.write_text(json.dumps(value, indent=2, sort_keys=True) + "\n")
    return finalizer.SelfHashedSourceV64(
        path, runtime.file_sha256_v64(path), content
    )
...
def _write_gpu_log(path: Path, actors: list[dict]) -> tuple[str, dict]:
    ...
    return runtime.file_sha256_v64(path), summary


def make_finalizer_sources(
    tmp_path: Path,
    evidence: dict,
) -> finalizer.FinalizerSourcesV64:
    prereg = builder.build_v64()
    panel_source = finalizer.SelfHashedSourceV64(
    ...
    evidence["numeric_actor_period_manifest_sha256"] = (
        analysis.canonical_sha256_v64(evidence["rows"])
    )
    evidence.pop("content_sha256_before_self_field", None)
    evidence["content_sha256_before_self_field"] = (
        analysis.canonical_sha256_v64(evidence)
    )
    analysis.validate_evidence_v64(evidence)
    stored_analysis = analysis.build_analysis_v64(evidence)
    prereg_source = _write_self_hashed(tmp_path / "prereg.json", prereg)
    base_model_receipt = runtime.expected_base_model_artifact_receipt_v64(
    ...
    attempt = {
        ...
        "v62b_finalized": runtime.verify_v62b_eligibility_v64(),
        "adapter_artifacts": analysis.expected_adapter_identities_v64(),
        "base_model_artifact_receipt": base_model_receipt,
        "runtime_determinism_controls": analysis.RUNTIME_CONTROLS_V64,
        "fixed_unscored_warmup_periods": analysis.WARMUP_PERIODS_V64,
        "fixed_scored_periods": analysis.SCORED_PERIODS_V64,
        "fixed_scored_replicas_per_conflict_unit": analysis.REPLICAS_PER_UNIT_V64,
        "preflight": {
    ...
    report = {
        ...
        "v62b_finalized": runtime.verify_v62b_eligibility_v64(),
        "adapter_artifact_identities": analysis.expected_adapter_identities_v64(),
    ...
    return finalizer.FinalizerSourcesV64(
...
def _with_rehashed_report(
    sources: finalizer.FinalizerSourcesV64,
    report: dict,
) -> finalizer.FinalizerSourcesV64:
    return replace(
        sources,
...
def test_schedule_is_exact_v62b_eager_design():
    assert analysis.WARMUP_PERIODS_V64 == 4
    assert analysis.SCORED_PERIODS_V64 == 24
    assert analysis.PAIRS_PER_ACTOR_V64 == 12
    assert analysis.REPLICAS_PER_UNIT_V64 == 48
    assert analysis.RUNTIME_CONTROLS_V64 == {
        "VLLM_BATCH_INVARIANT": False,
        "async_scheduling": False,
        "max_num_seqs": 68,
        "scheduling_policy": "fcfs",
...
def test_exact_six_six_order_balance_per_actor():
    ...
    _, receipts = analysis.paired_deltas_v64(generation)
...
def test_point_threshold_is_exactly_twice_frozen_null_width():
    assert analysis.MINIMUM_POINT_IMPROVEMENT_V64 == (
        2 * analysis.FROZEN_NULL_WIDTH_V64
    )


def test_point_threshold_is_inclusive():
    primary = {
        "point": analysis.MINIMUM_POINT_IMPROVEMENT_V64,
        "lcb": 0.01,
    }
    actor = {"maximum_absolute_leave_one_actor_out_shift": 0.0}
    exact = analysis.exact_diagnostics_v64(
        np.ones((4, 4, 24, 3), dtype=np.float64),
        [{"unit_identity_sha256": value} for value in analysis.SENTINEL_ORDER_V64],
    )
    assert analysis.robust_gate_v64(primary, actor, exact)["checks"][
...
def test_stable_per_unit_noninferiority_is_required():
    ...
    gate = analysis.build_analysis_v64(evidence)["required_confirmation_gate"]
...
def test_stress_sentinel_is_diagnostic_only():
    ...
    built = analysis.build_analysis_v64(evidence)
...
def test_evidence_label_tamper_fails(passing_evidence):
    ...
    changed["numeric_actor_period_manifest_sha256"] = analysis.canonical_sha256_v64(
        changed["rows"]
    )
    changed.pop("content_sha256_before_self_field")
    changed["content_sha256_before_self_field"] = analysis.canonical_sha256_v64(changed)
    with pytest.raises(ValueError, match="counterbalance"):
        analysis.validate_evidence_v64(changed)
...
def test_evidence_adapter_receipt_tamper_fails(passing_evidence):
    ...
    changed["numeric_scored_state_receipts_sha256"] = analysis.canonical_sha256_v64(
...
def test_v62b_eligibility_hashes_and_non_authority_are_exact():
    value = runtime.verify_v62b_eligibility_v64()
...
def test_v59_and_v434_adapter_identities_are_exact():
    assert runtime.verify_adapter_artifacts_v64() == (
...
def test_v64_worker_receipt_attests_capacity_and_rejects_raw_namespace():
    import eggroll_es_worker_lora_v64 as worker_v64

    worker = object.__new__(worker_v64.LoRAAdapterStateWorkerExtensionV64)
    worker.lora_config = SimpleNamespace(
        max_loras=1, max_cpu_loras=2, max_lora_rank=32
    )
    assert worker.runtime_lora_capacity_v64() == {
        "schema": "v64-effective-worker-lora-capacity",
        "lora_enabled": True,
        "max_loras": 1,
        "max_cpu_loras": 2,
    ...
    Manager = type("LRUCacheLoRAModelManager", (), {})
    Facade = type("LRUCacheWorkerLoRAManager", (), {})
    adapter_manager = Manager()
    adapter_manager.lora_index_to_id = [2]
    adapter_manager._active_adapters = SimpleNamespace(cache={2: None})
    adapter_manager.list_adapters = lambda: {2: SimpleNamespace(loras={})}
    lora_manager = Facade()
    lora_manager._adapter_manager = adapter_manager
    lora_manager.list_adapters = lambda: {2}
    worker.model_runner = SimpleNamespace(lora_manager=lora_manager)
    raw_weights = (
        runtime.CANDIDATE_SOURCE_ADAPTER / "adapter_model.safetensors"
    )
    with pytest.raises(RuntimeError, match="not in staged vLLM namespace"):
        worker.runtime_applied_lora_v64(
            2,
            raw_weights,
            runtime.file_sha256_v64(raw_weights),
    ...
    with pytest.raises(RuntimeError, match="effective worker LoRA capacity"):
        worker.runtime_lora_capacity_v64()


def test_v64_namespace_binding_catches_v63_noop_and_accepts_stage():
    value = runtime.adapter_namespace_binding_v64()
    assert value["raw_candidate_reference_module_intersection_count"] == 0
    assert value["staged_candidate_reference_module_intersection_count"] == 35
    assert value["expected_postpack_runtime_module_count"] == 23
    assert value["expected_applied_gpu_slot_view_count"] == 82
    assert value["staged_candidate"]["mapped_module_manifest_sha256"] == (
        value["reference"]["mapped_module_manifest_sha256"]
    )
    assert value["all_tensor_bytes_preserved_by_stage"] is True


def test_v64_registered_slot_audit_compares_full_zero_padded_buffers():
    import eggroll_es_worker_lora_v64 as worker_v64

    name = "language_model.model.layers.20.mlp.gate"
    a = torch.arange(6, dtype=torch.bfloat16).reshape(2, 3)
    b = torch.arange(8, dtype=torch.bfloat16).reshape(4, 2)
    a_parent = torch.zeros((1, 1, 3, 4), dtype=torch.bfloat16)
    b_parent = torch.zeros((1, 1, 5, 3), dtype=torch.bfloat16)
    a_parent[0, 0, :2, :3] = a
    b_parent[0, 0, :4, :2] = b
    module = SimpleNamespace(
        tp_size=1,
        n_slices=1,
        lora_a_stacked=(a_parent,),
        lora_b_stacked=(b_parent,),
    )
    weights = SimpleNamespace(lora_a=a, lora_b=b)
    model = SimpleNamespace(loras={name: weights})
    manager = SimpleNamespace(
        modules={name: module},
        _get_lora_layer_weights=lambda _model, module_name: (
            weights if module_name == name else None
        ),
    )
    matched, records = worker_v64._registered_slot_records_v64(
        manager, model, 0
    )
    assert matched == [name]
    assert len(records) == 2
    assert {record["side"] for record in records} == {"A", "B"}
    a_parent[0, 0, 2, 3] = 1
    with pytest.raises(RuntimeError, match="differs from registered LoRA"):
        worker_v64._registered_slot_records_v64(manager, model, 0)


def test_full_worker_execution_and_lora_manager_chain_is_hash_bound():
    import eggroll_es_worker_lora_v64 as worker_v64

    bindings = runtime.implementation_bindings_v64()["code_file_sha256"]
    assert set(runtime.WORKER_EXECUTION_PATHS_V64).issubset(bindings)
    for name, path in runtime.WORKER_EXECUTION_PATHS_V64.items():
        assert path.is_file(), name
        assert bindings[name] == runtime.file_sha256_v64(path)
    assert runtime.fixed_recipe_v64()["worker_execution_binding_keys"] == sorted(
        runtime.WORKER_EXECUTION_PATHS_V64
    )
    assert [item.__module__ for item in worker_v64.LoRAAdapterStateWorkerExtensionV64.__mro__] == [
        "eggroll_es_worker_lora_v64",
    ...
    closure = runtime.live_local_import_closure_v64()
    assert {
...
def test_base_model_expectation_binds_all_shards_and_all_metadata():
    expectation = runtime.base_model_artifact_expectation_v64()
    ...
    assert expectation["weight_shard_manifest_sha256"] == (
        runtime.BASE_MODEL_SHARDS_CONTENT_SHA256_V64
    )
    assert expectation["non_weight_manifest_sha256"] == (
        runtime.BASE_MODEL_NON_WEIGHT_MANIFEST_SHA256_V64
    )
    receipt = runtime.expected_base_model_artifact_receipt_v64(expectation)
    assert runtime.validate_base_model_artifact_receipt_v64(
        receipt, expectation
    ) == receipt
...
def test_base_model_receipt_rejects_rehashed_shard_or_metadata_drift():
    ...
    for section in ("weight_shards", "non_weight_files"):
        ...
        with pytest.raises(RuntimeError, match="base-model artifact receipt"):
            runtime.validate_base_model_artifact_receipt_v64(
                receipt, expectation
            )
...
def test_base_model_live_verifier_rejects_file_byte_drift(
    ...
    monkeypatch.setattr(
        runtime, "base_model_artifact_expectation_v64", lambda: expectation
    )
    path = (
    ...
    with pytest.raises(RuntimeError, match="base-model artifact receipt"):
        runtime.verify_base_model_artifacts_v64(expectation)
...
def test_builder_and_dry_run_never_open_or_enumerate_model_directory(
    ...
    assert runtime.BASE_MODEL_SEAL_V64 in opened
...
def test_live_path_requires_explicit_execute(tmp_path):
    prereg_source = _write_self_hashed(
        tmp_path / "prereg.json", builder.build_v64()
    )
...
def test_finalizer_production_path_requires_all_postrun_hash_arguments():
    ...
    with pytest.raises(RuntimeError, match="production outcome hash is not sealed"):
        finalizer.production_sources_v64(*hashes)
...
def test_finalizer_remains_byte_identical_to_preregistered_implementation():
    ...
    assert prereg["implementation_bindings"]["code_file_sha256"][
        "finalizer_v64"
    ] == runtime.file_sha256_v64(Path(finalizer.__file__).resolve())
...
def test_finalizer_accepts_failed_outcome_without_relaxation(tmp_path):
    ...
    result = finalizer.finalize_v64(make_finalizer_sources(tmp_path, evidence))
...
def test_finalizer_rejects_file_tamper(tmp_path, passing_evidence):
    ...
    with pytest.raises(RuntimeError, match="input file changed"):
        finalizer.finalize_v64(sources)
...
def test_finalizer_rejects_rehashed_analysis_drift(tmp_path, passing_evidence):
    ...
    changed_sources = finalizer.FinalizerSourcesV64(
    ...
    with pytest.raises(RuntimeError, match="differs from exact numeric rebuild"):
        finalizer.finalize_v64(changed_sources)
...
def test_finalizer_rejects_rehashed_nonexclusive_attempt_preflight(
    ...
    with pytest.raises(RuntimeError, match="exclusive-idle launch attempt"):
        finalizer.finalize_v64(changed)
...
def test_finalizer_rejects_rehashed_postrun_base_model_drift(
    ...
    with pytest.raises(RuntimeError, match="base-model artifact receipt"):
        finalizer.finalize_v64(_with_rehashed_report(sources, report))
...
def test_evidence_rejects_requested_vs_effective_active_lora_mismatch(
    ...
    changed["numeric_scored_state_receipts_sha256"] = (
        analysis.canonical_sha256_v64(changed["scored_state_receipts"])
    )
    changed.pop("content_sha256_before_self_field", None)
    changed["content_sha256_before_self_field"] = (
        analysis.canonical_sha256_v64(changed)
    )
...
def test_finalizer_rejects_rehashed_foreign_gpu_process_log(
    ...
    gpu_sha = runtime.file_sha256_v64(sources.gpu_log_path)
    report = json.loads(sources.report.path.read_text())
    ...
    with pytest.raises(RuntimeError, match="foreign, inactive, or mismatched"):
        finalizer.finalize_v64(_with_rehashed_report(changed, report))
...
def test_finalizer_rejects_panel_document_audit_or_evidence_projection_drift(
    ...
    with pytest.raises(RuntimeError, match="numeric panel projection"):
        finalizer._verify_panel_projection_v64(panel, evidence, report)
