#!/usr/bin/env python3
"""Build a content-free paired production/v389 train-only panel frame."""
...
CANDIDATE_PATH = (
    ROOT / "experiments/eggroll_es_hpo/dataset_candidates/v389/"
    "train_qa_context_merit_v389.jsonl"
).resolve()
CANDIDATE_MANIFEST_PATH = (
    ROOT / "experiments/eggroll_es_hpo/dataset_candidates/v389/manifest.json"
).resolve()
PRODUCTION_PATH = (ROOT / "data/train_qa_curated_v1.jsonl").resolve()
OUTPUT_PATH = (
    ROOT / "experiments/eggroll_es_hpo/train_panel_sampling_v30a/"
    "paired_data_version_panels_v30a.json"
).resolve()

CANDIDATE_SHA256 = "4b6da77e7e1ae3d1145b3f2d29c7774b6aad2b4cb520fcea9a48af93d4322388"
CANDIDATE_MANIFEST_SHA256 = "a7d096e7345b54c986ca210a6153cb22bde0e6ef05b94b530ffb9cf260139065"
...
EXPECTED_SELECTED_UNIT_ROOT_SHA256 = (
    "7451a8db02611af79d48aeb42802ac9d35c9cc33996db165a0fdca63dcbd0442"
)
...
def load_bound_rows():
    ...
    if (
        ...
        raise RuntimeError("v30a v389 train-only freeze manifest changed")
    ...
    if len(candidate) != CANDIDATE_ROWS or len(production) != PRODUCTION_ROWS:
        raise RuntimeError("v30a frozen row counts changed")
...
def representative(unit_id, side, members, stratum):
    ...
    selected = min(
        ...
        key=lambda item: canonical_sha256({
            "schema": "eggroll-es-v30a-fixed-side-representative",
...
def build_joint_units(candidate, production):
    ...
    for members in components.values():
        ...
        unit_id = canonical_sha256({
            "schema": "eggroll-es-joint-conflict-unit-v30a",
            "members": member_ids,
        })
        documents = {}
        for side in ("candidate", "production"):
        ...
        if eligible_documents:
            shared_document = min(
                ...
                key=lambda document: canonical_sha256({
                    "schema": "eggroll-es-v30a-shared-document-selection",
                    "seed": REPRESENTATIVE_SEED,
                    "unit_id": unit_id,
        ...
        if side_contract["candidate"]["classified_stratum"] != stratum:
            raise RuntimeError("v30a candidate representative missed its stratum")
        paired.append({
            "unit_id": unit_id,
            "stratum": stratum,
...
def panel_order(name, items):
    return sorted(
        items,
        key=lambda item: canonical_sha256({
            "schema": "eggroll-es-v30a-panel-order",
            "seed": PANEL_SEED,
            "panel": name,
...
def build_manifest():
    ...
    if (
        ...
        raise RuntimeError("v30a joint paired capacity changed or is insufficient")
    by_stratum = {
        name: sorted(
            (unit for unit in paired if unit["stratum"] == name),
            key=lambda unit: canonical_sha256({
                "schema": "eggroll-es-v30a-stratum-permutation",
                "seed": PANEL_SEED,
                "stratum": name,
    ...
    for panel_index, panel_name in enumerate(PANEL_NAMES):
        ...
        if len(selected) != PANEL_SIZE:
            raise RuntimeError("v30a paired panel size changed")
    ...
    if len(selected_ids) != len(set(selected_ids)):
        raise RuntimeError("v30a panels are not globally conflict-unit disjoint")
    if len(selected_ids) != len(PANEL_NAMES) * PANEL_SIZE:
        raise RuntimeError("v30a selected paired-unit coverage changed")

    manifest = {
        "schema": "eggroll-es-paired-data-version-panel-manifest-v30a",
        ...
        "panels": panels,
        "separation": {
            "v30a_role": "matched_data_version_compatibility_only",
            "future_role": "separate_full_candidate_hpo_only_if_v30a_passes",
...
def validate_manifest(manifest):
    ...
    if (
        manifest.get("schema")
        != "eggroll-es-paired-data-version-panel-manifest-v30a"
        ...
        raise RuntimeError("v30a paired manifest contract changed")
    ...
    if (
        ...
        raise RuntimeError("v30a paired item identity or weighting changed")
    return manifest
...
def exclusive_write(path, value):
    ...
    if path != OUTPUT_PATH:
        raise ValueError("v30a output path changed")
    path.parent.mkdir(parents=True, exist_ok=True)
    try:
        descriptor = os.open(path, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
    except FileExistsError as error:
        raise ValueError("v30a paired manifest already exists") from error
...
def main(argv=None):
    ...
    result = {
        "schema": "eggroll-es-paired-panel-build-v30a",
