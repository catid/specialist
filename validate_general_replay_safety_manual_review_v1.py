#!/usr/bin/env python3
"""Validate the response-by-response safety ledger and its sealed report."""
...
DEFAULT_LEDGER = (
    ROOT
    / "data/general_replay_v1/manual_approvals/safety.reviewer_safety.jsonl"
)
DEFAULT_REPORT = (
    ROOT
    / "data/general_replay_v1/manual_approvals/safety.reviewer_safety.report.json"
)
...
def _load_source_candidates(priority_manifest: dict) -> tuple[dict, dict[str, dict]]:
    ...
    for chunk in source_manifest["chunks"]:
        ...
        chunk_path = safe_regular_input(
            source_directory / chunk["path"], "source safety review chunk"
        )
        raw = chunk_path.read_bytes()
        if hashlib.sha256(raw).hexdigest() != chunk["sha256"]:
            raise RuntimeError("source safety review chunk changed")
        for line in raw.decode().splitlines():
            ...
            for candidate in item["candidates"]:
                ...
                if digest in candidates:
                    raise RuntimeError("source safety response identity duplicated")
                candidates[digest] = {
                    "candidate": candidate,
    ...
    return source_manifest, candidates
...
def _load_priority_rows(packet: Path) -> tuple[bytes, dict, list[dict], list[dict]]:
    ...
    if category_contract != {
        ...
        raise RuntimeError("safety priority-review contract changed")
    ...
    source_manifest, source_candidates = _load_source_candidates(manifest)
    rows: list[dict] = []
    pinned_chunks: list[dict] = []
    for entry in manifest["chunks"]:
        if entry["category"] != CATEGORY:
            continue
        path = safe_regular_input(packet / entry["path"], "priority safety chunk")
        raw = path.read_bytes()
        if hashlib.sha256(raw).hexdigest() != entry["sha256"]:
            raise RuntimeError("priority safety chunk changed")
        chunk_rows = [
        ...
        if (
            ...
            raise RuntimeError("priority safety chunk accounting changed")
    ...
    if len(rows) != EXPECTED_ROWS:
        raise RuntimeError("priority safety row count changed")
    ...
    for index, item in enumerate(rows):
        if (
            ...
            raise RuntimeError(f"priority safety row {index} contract changed")
        candidates = item.get("candidates")
        if not isinstance(candidates, list) or len(candidates) != 1:
            raise RuntimeError("each priority safety row must address one response")
        candidate = candidates[0]
        digest = candidate.get("response_sha256")
        if not isinstance(digest, str) or not HEX64.fullmatch(digest):
            raise RuntimeError("priority safety response identity malformed")
        if (
            item["spec_id"] in seen_specs
            or item["source_group_id"] in seen_groups
            or digest in seen_responses
        ):
            raise RuntimeError("priority safety identity duplicated")
    ...
    if tokens != TARGET_ASSISTANT_TOKENS:
        raise RuntimeError("priority safety token total changed")
...
def build_report(packet: Path, ledger: Path) -> dict:
    ...
    ledger_path = safe_regular_input(ledger, "manual safety approval ledger")
    ...
    if set(indexed) != set(expected):
        raise RuntimeError(
            "manual safety ledger must decide every priority response exactly once"
        )
    ...
    for item in rows:
        ...
        if (
            ...
            raise RuntimeError("manual safety decision lineage or rationale changed")
    ...
    if len(reviewed_at_values) != 1:
        raise RuntimeError("manual safety ledger must have one sealed review time")
    ...
    report = {
        "schema": "general-replay-safety-manual-review-report-v1",
        "status": (
            "approved_exact_safety_budget"
            if exact_budget_ready
            else "review_complete_backups_required"
        ),
        "category": CATEGORY,
        "rubric_id": RUBRIC_ID,
        "reviewer": REVIEWER,
        ...
        "packet": {
            ...
            "pinned_safety_chunks": pinned_chunks,
        },
        "ledger": {
        ...
        "backup_review": {
            ...
            "reason": (
                ...
                if rejected_rows == 0
                else "Rejected priority responses require source-group-disjoint backups."
            ),
        },
        "identity_validation": {
        ...
        "policy": {
            ...
            "protected_or_evaluation_sources_read": False,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        path = safe_regular_input(args.report, "sealed safety review report")
        if path.read_bytes() != rendered:
            raise RuntimeError("sealed safety review report changed")
    ...
    if not args.check and not args.print_report:
        print(json.dumps({
            ...
            "backup_required": report["backup_review"]["required"],
        }, sort_keys=True))
    return 0
