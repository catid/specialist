...
import build_base_vs_lora_es_ood_first_preregistration_v46f as builder
import run_base_vs_lora_es_ood_first_v46f as subject
...
def test_two_full_waves_use_every_gpu_v46f():
    waves = subject.arm_wave_plan_v46f()
    assert len(waves) == 2
    assert all(
        tuple(engine for _, engine in wave) == (0, 1, 2, 3)
        for wave in waves
    )
    assert tuple(arm for wave in waves for arm, _ in wave) == subject.ARMS_V46F


def test_stage_replicas_share_bytes_but_outputs_need_not_be_exact_v46f():
    stages = subject.replica_stage_bindings_v46f()
    assert tuple(stages) == subject.CANDIDATE_ARMS_V46F
    for logical, (left, right) in subject.LOGICAL_REPLICAS_V46F.items():
...
def test_mean_replica_ranking_accepts_nonidentical_candidate_outputs_v46f(
    monkeypatch,
):
    shadow = {
        "base_a": _metric(0.10),
        "lora_es_v43l_a": _metric(0.30),
        "lora_es_v43l_b": _metric(0.34),
    }
    ...
    result = subject.finalize_selection_v46f(
        shadow, ood, prose, raw,
        {"shadow": {"all_six_base_outputs_exact": True}},
    )
    assert result["selected_logical_candidate"] == "lora_es_v43l"
    assert result["candidate_replicas_required_bit_exact"] is False
    assert result["mean_replicated_shadow_ranking"] is True
    assert result["per_logical_candidate_gate_table"]["lora_es_v43l"][
        "mean_replicated_shadow_metrics"
    ]["generated_equal_unit_mean_reward"] == pytest.approx(0.32)


def test_either_replica_ood_failure_excludes_whole_logical_candidate_v46f(
    monkeypatch,
):
    shadow = {
        "base_a": _metric(0.10),
        "lora_es_v43l_a": _metric(0.50),
        "lora_es_v43l_b": _metric(0.51),
    }
    ood = {arm: {"passed": True} for arm in shadow}
    ood["lora_es_v43l_b"] = {"passed": False}
    ...
    assert result["selected_logical_candidate"] is None
    assert result["selected_arm"] == "base_a"
    assert "lora_es_v43l" in result["ineligible_logical_candidates"]


def test_builder_loader_and_dry_run_are_zero_protected_access_v46f(
    ...
    subject.load_preregistration_v46f(args)
