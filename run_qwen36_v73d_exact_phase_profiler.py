#!/usr/bin/env python3
"""Fail-closed launcher and postrun analyzer for V73D exact-phase profiling."""
...
def _require_columns(
    ...
    _require(not missing, f"V73D SQLite {table} columns absent: {sorted(missing)}")
...
def _nvtx_rows(connection: sqlite3.Connection) -> tuple[list[dict], list[dict]]:
    ...
    _require(required.issubset(_tables(connection)), "V73D NVTX tables absent")
    _require_columns(
        connection,
        "NVTX_EVENTS",
...
def _validate_phase_ranges(
    ...
    _require(
        ...
        "V73D trace phase ownership or overlap changed",
    )
    return selected
...
def _phase_for_event(
    ...
    _require(
        len(matches) == 1,
        f"V73D {label} crossed or fell outside exact phase ranges",
    )
...
def _gpu_events(
    ...
    _require(
        ...
        "V73D CUDA kernel/memcpy tables absent",
    )
    kernel_required = (
    ...
    for row in connection.execute(
        ...
        _require(end > start, "V73D CUDA kernel duration changed")
        pid = _decode_pid(global_pid)
        _require(pid in expected_pids, "V73D CUDA kernel PID is not an actor")
        demangled = strings.get(int(row[4]))
        short = strings.get(int(row[5]))
        _require(
            isinstance(demangled, str) and isinstance(short, str),
            "V73D CUDA kernel symbol identity changed",
        )
        kernels.append({
    ...
    for row in connection.execute(
        ...
        _require(end > start and size >= 0, "V73D CUDA memcpy changed")
        pid = _decode_pid(global_pid)
        _require(pid in expected_pids, "V73D CUDA memcpy PID is not an actor")
        _require(kind in copy_labels, "V73D CUDA memcpy kind is unknown")
        copies.append({
    ...
    _require(bool(kernels), "V73D CUDA kernel trace is empty")
    _require(bool(copies), "V73D CUDA memcpy trace is empty")
    _require(
        {row["physical_gpu_id"] for row in kernels} == set(GPU_IDS)
        and {row["pid"] for row in kernels} == expected_pids,
        "V73D CUDA kernel trace omitted a GPU",
    )
...
def _allocation_events(
    ...
    _require(
        ...
        "V73D CUDA allocation tables absent",
    )
    _require_columns(
        connection,
    ...
    for row in connection.execute(
        ...
        pid = _decode_pid(global_pid)
        _require(pid in expected_pids, "V73D allocation PID is not an actor")
        _require(size >= 0 and operation in labels, "V73D allocation event changed")
        rows.append({
    ...
    _require(bool(rows), "V73D allocation event trace is empty")
    _require(
        {row["physical_gpu_id"] for row in rows} == set(GPU_IDS)
        and {row["pid"] for row in rows} == expected_pids,
        "V73D allocation event trace omitted an actor or physical GPU",
    )
    return rows
...
def _summarize_allocations(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for gpu in GPU_IDS:
        ...
        for row in selected:
            ...
            _require(is_alloc != is_free, f"V73D allocation label unsupported: {label}")
            ...
            _require(running >= 0, "V73D traced allocation balance became negative")
...
def _summarize_nccl(
    ...
    _require(bool(rows), "V73D NCCL trace is empty")
    ...
    for row in rows:
        _require(row["pid"] in expected_pids, "V73D NCCL PID is not an actor")
    ...
    _require(
        ...
        "V73D update execute contains no NCCL trace",
    )
    return {
        "by_phase": dict(sorted(by_phase.items())),
...
def _validate_empty_process_streams(
    ...
    _require(
        ...
        "V73D ProcessStreams table absent",
    )
    _require_columns(
        connection,
        "ProcessStreams",
    ...
    for global_pid, filename_id, content_id in connection.execute(
        ...
        _require(
            ...
            "V73D process stream string identity changed",
        )
        _require(
            not content.strip(),
            "V73D target process stream was nonempty; trace rejected",
        )
        rows.append({
...
def _analyze_nccl_debug_logs_v73d(
    pattern: str,
    expected_pids: set[int],
) -> dict[str, Any]:
    path = Path(pattern)
    _require("*" in path.name, "V73D NCCL debug pattern changed")
    files = sorted(path.parent.glob(path.name))
    _require(files, "V73D NCCL metadata logs are absent")
    observed_pids = set()
    logs = []
    explicit = []
    forbidden_markers = (
    ...
    for file_path in files:
        _require(
            ...
            "V73D NCCL metadata log is not a regular file",
        )
        match = re.search(r"\.([0-9]+)\.log$", file_path.name)
        _require(match is not None, "V73D NCCL metadata PID filename changed")
        pid = int(match.group(1))
        observed_pids.add(pid)
        content = file_path.read_text(encoding="utf-8")
        _require(content, "V73D NCCL metadata log is empty")
        lowered = content.casefold()
        _require(
            not any(marker.casefold() in lowered for marker in forbidden_markers),
            "V73D NCCL metadata log contains semantic payload marker",
        )
    ...
    _require(
        ...
        "V73D NCCL metadata logs omitted an actor PID",
    )
    return {
        "debug_level": "INFO",
...
def analyze_sqlite_v73d(
    ...
    _require(path.is_file() and not path.is_symlink(), "V73D SQLite export absent")
...
def _validate_guard_process_receipt_v73d(
    ...
    _require(
        ...
        "V73D path guard process receipt changed",
    )
    return dict(value)


def _validate_actor_bootstrap_receipt_v73d(
    value: Mapping[str, Any], guard_source_sha256: str
) -> dict[str, Any]:
    row = dict(value)
    claimed = row.pop("receipt_sha256", None)
    pre_parent = _validate_guard_process_receipt_v73d(
        row.get("pre_parent_guard_receipt", {}),
        "ray_actor_worker_extension_pre_parent_import",
    )
    current = _validate_guard_process_receipt_v73d(
    ...
    _require(
        ...
        and value.get("quality_hpo_or_promotion_authorized") is False,
        "V73D Ray actor bootstrap receipt changed",
    )
    return dict(value)


def validate_target_artifacts_v73d(
    ...
    _require(len(rows) == 16, "V73D actor receipt count changed")
    ...
    for row in rows:
        ...
        _require(
            ...
            "V73D actor receipt self hash changed",
        )
    ...
    _require(
        ...
        "V73D actor PID/GPU binding changed between waves",
    )
    bindings = {
    ...
    _require(
        ...
        "V73D four-wave actor coverage changed",
    )
    guard_source_sha256 = builder.file_sha256(builder.GUARD)
    controller_guard = _validate_guard_process_receipt_v73d(
        guard.get("controller", {}), "controller_sitecustomize"
    )
    worker_bootstraps = [
        _validate_actor_bootstrap_receipt_v73d(row, guard_source_sha256)
    ...
    _require(
        ...
        "V73D path guard process coverage changed",
    )
    _require(
        ...
        == "qwen36-v73d-ray-job-runtime-env-bootstrap-v1"
        ...
        == guard_source_sha256,
        "V73D path guard worker PID attribution changed",
    )
    ...
    postrun_toolchain_identity = validate_prelaunch_toolchain_v73d(
        preregistration
    )
    accounting = report.get("gpu_time_accounting", {})
    legacy_compute = report.get("compute_ledger", {})
    _require(
        report.get("schema") == "v73d-exact-phase-qwen36-calibration-report"
        and report.get("status")
        == "complete_same_live_no_commit_awaiting_parent_trace_analysis"
        ...
        and report.get("immutable_v73c_attempt_1_predecessor")
        == preregistration.get("immutable_v73c_attempt_1_predecessor")
        and report.get("ray_actor_guard_bootstrap")
        == guard.get("ray_job_runtime_env_bootstrap")
        and accounting.get("schema")
        == "qwen36-v73d-gpu-time-accounting-split-v1"
        ...
        is True,
        "V73D target authority or final idle changed",
    )
    compiler = update.get("same_live_compiler_equivalence", {})
    execution = update.get("same_live_update_equivalence", {}).get(
        "live_update_execution", {}
    )
    _require(
        population.get("v71_live_rewards_accepted_before_update_math") is True
        ...
        "V73D same-live/abort/traffic semantics changed",
    )
    ...
    return {
        ...
        "phase_receipt": phase,
        "same_live_compilers_exact": True,
...
def analyze_profile_v73d(
    preregistration: Mapping[str, Any], arm: str
) -> dict[str, Any]:
    _require(arm == "timeline", "current V73D analyzer accepts timeline arm only")
    paths = preregistration["arms"][arm]["artifacts"]
    target = validate_target_artifacts_v73d(preregistration, arm)
    report_path = Path(paths["nsys_report"])
    sqlite_path = Path(paths["sqlite_export"])
    _require(
        report_path.is_file() and not report_path.is_symlink(),
        "V73D Nsight report absent",
    )
    sqlite_result = analyze_sqlite_v73d(
        sqlite_path,
    ...
    nccl_debug = _analyze_nccl_debug_logs_v73d(
        paths["nccl_debug_pattern"], set(target["actor_pids"])
    )
    value = {
        "schema": "qwen36-v73d-exact-phase-profile-analysis-v1",
...
def _fresh_launch(paths: Mapping[str, str]) -> None:
    ...
    _require(
        ...
        "V73D requires fresh application/profile paths",
    )


def validate_prelaunch_toolchain_v73d(
    ...
    def exact_source(
        ...
        _require(
            ...
            f"V73D prelaunch tool identity changed: {label}",
        )
        return {
            "label": label,
    ...
    _require(
        ...
        "V73D target Python invocation path changed",
    )
    ...
    return {
        "schema": "qwen36-v73d-prelaunch-toolchain-identity-v1",
        "passed": True,
...
def four_gpu_idle_preflight_v73d(
    preregistration: Mapping[str, Any],
) -> dict[str, Any]:
    topology = builder.gpu_topology_v73d()
    _require(
        topology == preregistration["physical_gpu_topology"],
        "V73D physical GPU topology changed before launch",
    )
    gpu_command = [
    ...
    for line in _run_text(gpu_command).splitlines():
        ...
        _require(len(fields) == 5, "V73D idle GPU query shape changed")
        rows.append({
    ...
    _require(
        ...
        "V73D physical GPU attribution changed before launch",
    )
    ...
    for line in raw_processes.splitlines():
        ...
        _require(len(fields) == 4, "V73D compute process query shape changed")
        process_rows.append({
            "gpu_uuid": fields[0],
            "pid": int(fields[1]),
    ...
    return {
        "schema": "qwen36-v73d-four-gpu-idle-preflight-v1",
...
def profile_failure_receipt_v73d(
    preregistration: Mapping[str, Any],
    arm: str,
    returncode: int,
    ...
    if run_failure_path.is_file():
        ...
        _require(
            ...
            is True,
            "V73D run failure GPU accounting split changed",
        )
        accounting = copy.deepcopy(observed_accounting)
    return {
        "schema": "qwen36-v73d-profiler-launch-failure-v1",
...
def execute_profile_v73d(
    ...
    _require(
        ...
        "V73D HBM metrics arm blocked before directory creation or subprocess",
    )
    toolchain_identity = validate_prelaunch_toolchain_v73d(preregistration)
    paths = arm_contract["artifacts"]
    _fresh_launch(paths)
    command = builder.expand_command_v73d(
        arm,
        preregistration_file_sha256,
        preregistration["content_sha256_before_self_field"],
    )
    command_sha = builder.canonical_sha256(command)
    preflight = four_gpu_idle_preflight_v73d(preregistration)
    attempt = {
        "schema": "qwen36-v73d-profiler-launch-attempt-v1",
        "status": (
            "prelaunch_accepted_launching_fresh_no_commit_profile"
            if preflight["passed"]
    ...
    _require(
        ...
        "V73D requires all four GPUs idle and foreign-process-free before launch",
    )
    ...
    if completed.returncode != 0:
        failure = profile_failure_receipt_v73d(
            preregistration, arm, completed.returncode, command_sha
        )
        _atomic_json(Path(paths["profile_failure"]), failure)
        raise RuntimeError(
            f"V73D profiler target failed with return code {completed.returncode}"
        )
    result = analyze_profile_v73d(preregistration, arm)
...
def main(argv=None) -> int:
    ...
    if args.dry_run == args.execute:
        raise ValueError("V73D profiler requires exactly one of --dry-run or --execute")
    preregistration = load_preregistration_v73d(
