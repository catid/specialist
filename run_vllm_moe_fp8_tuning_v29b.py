#!/usr/bin/env python3
"""Fail-closed V29B infrastructure retry of the four-GPU FP8 MoE tuner."""
...
PREREG_PATH_V29B = prereg.OUTPUT_PATH_V29B
PREREG_FILE_SHA256_V29B = (
    "1a9316f1d2a1a47e583b2027670626d3434ab0d0a235b26ae003058ae20b5369"
)
PREREG_CONTENT_SHA256_V29B = (
    "0dff2a5c6ac6e8312f72f0221140305aa06cf52a453d855188a329e2952ef7bf"
)
TEST_PATH_V29B = (ROOT / "test_run_vllm_moe_fp8_tuning_v29b.py").resolve()
PREREG_TEST_PATH_V29B = (
    ROOT / "test_build_vllm_moe_fp8_tuning_preregistration_v29b.py"
).resolve()
MOE_ENVIRONMENT_V29B = (
...
ALLOWED_CONFIG_KEYS_V29B = {
    "BLOCK_SIZE_M", "BLOCK_SIZE_N", "BLOCK_SIZE_K", "GROUP_SIZE_M",
    "num_warps", "num_stages",
}
IMPLEMENTATION_PATHS_V29B = {
    "v29b_preregistration_builder": Path(prereg.__file__).resolve(),
    "v29b_preregistration_tests": PREREG_TEST_PATH_V29B,
    "v29b_preregistration": PREREG_PATH_V29B,
    "v29b_runtime": Path(__file__).resolve(),
    "v29b_runtime_tests": TEST_PATH_V29B,
    "official_vllm_025_tuner": prereg.OFFICIAL_TUNER_PATH_V29B,
    "installed_vllm_fused_moe": prereg.FUSED_MOE_PATH_V29B,
    "installed_vllm_fused_moe_config": prereg.FUSED_MOE_CONFIG_PATH_V29B,
    "installed_vllm_envs": prereg.VLLM_ENVS_PATH_V29B,
    ...
    "v28b_positive_evidence": prereg.V28B_EVIDENCE_PATH_V29B,
    "v29a_infrastructure_failure_evidence": (
        prereg.V29A_FAILURE_EVIDENCE_PATH_V29B
    ),
    "pinned_ray_actor_builtin": prereg.RAY_ACTOR_PATH_V29B,
}
FORBIDDEN_COMPACT_KEYS_V29B = {
...
def _assert_compact_v29b(value):
    overlap = FORBIDDEN_COMPACT_KEYS_V29B & set(_recursive_keys(value))
    if overlap:
        raise RuntimeError(f"V29B compact output contains forbidden keys: {sorted(overlap)}")


def _exclusive_write_json_v29b(path, value):
    path = Path(path)
...
def _rewrite_json_v29b(path, value):
    path = Path(path)
...
def load_preregistration_v29b():
    value = json.loads(PREREG_PATH_V29B.read_text(encoding="utf-8"))
    if (
        file_sha256(PREREG_PATH_V29B) != PREREG_FILE_SHA256_V29B
        or value.get("content_sha256_before_self_field")
        != PREREG_CONTENT_SHA256_V29B
        or canonical_sha256(_without_self(value)) != PREREG_CONTENT_SHA256_V29B
    ):
        raise RuntimeError("V29B preregistration identity changed")
    return prereg.validate_preregistration_v29b(value)


def implementation_identity_v29b():
    files = {
        key: {
            "path": str(Path(path).resolve()),
            "file_sha256": file_sha256(path),
        }
        for key, path in IMPLEMENTATION_PATHS_V29B.items()
    }
    return {
        "files": files,
...
def recipe_v29b(preregistration, implementation):
    return _seal({
        "schema": "vllm-moe-fp8-tuning-recipe-v29b",
        "experiment_name": prereg.EXPERIMENT_NAME_V29B,
        "preregistration": {
            "path": str(PREREG_PATH_V29B),
            "file_sha256": PREREG_FILE_SHA256_V29B,
            "content_sha256": PREREG_CONTENT_SHA256_V29B,
        },
        ...
        "fresh_exclusive_paths": {
            "attempt": str(prereg.ATTEMPT_PATH_V29B),
            "report": str(prereg.REPORT_PATH_V29B),
            "selected_table_directory": str(prereg.OUTPUT_DIRECTORY_V29B),
        },
...
def normalize_ray_gpu_id_v29b(value):
    if isinstance(value, bool):
        raise RuntimeError("V29B Ray GPU ID representation changed")
    if isinstance(value, int):
        ...
        raise RuntimeError("V29B Ray GPU ID representation changed")
    if result not in prereg.PHYSICAL_GPU_IDS_V29B:
        raise RuntimeError("V29B Ray GPU ID physical range changed")
    return result


def live_cpu_disk_audit_v29b():
    prereg.validate_static_inputs_v29b()
    ray_actor_source = prereg.RAY_ACTOR_PATH_V29B.read_text(encoding="utf-8")
    if (
        file_sha256(prereg.RAY_ACTOR_PATH_V29B) != prereg.RAY_ACTOR_SHA256_V29B
        or ray_actor_source.count(
            "def __ray_call__(self, fn, *args, **kwargs):"
        ) != 1
        or ray_actor_source.count("return fn(self, *args, **kwargs)") != 1
    ):
        raise RuntimeError("V29B pinned Ray actor built-in semantics changed")
    all_files = model_seal_v23a._file_fingerprints(prereg.MODEL_PATH_V29B)
    if (
        len(all_files) != prereg.MODEL_ALL_FILES_SIZE_MANIFEST_V29B["file_count"]
        or sum(item["bytes"] for item in all_files.values())
        != prereg.MODEL_ALL_FILES_SIZE_MANIFEST_V29B["total_bytes"]
        or canonical_sha256(all_files)
        != prereg.MODEL_ALL_FILES_FINGERPRINT_SHA256_V29B
    ):
        raise RuntimeError("V29B exact FP8 model all-files identity changed")
    index = json.loads(
        (prereg.MODEL_PATH_V29B / "model.safetensors.index.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    if shard_manifest != prereg.MODEL_WEIGHT_SHARDS_V29B:
        raise RuntimeError("V29B exact FP8 model all-shard identity changed")
    ...
    if (
        ...
        raise RuntimeError("V29B official compiler policy or tuner source changed")
    return _seal({
        "schema": "vllm-moe-fp8-tuning-live-cpu-disk-audit-v29b",
        ...
        "model_config_sha256": file_sha256(prereg.MODEL_PATH_V29B / "config.json"),
        "model_index_sha256": file_sha256(
            prereg.MODEL_PATH_V29B / "model.safetensors.index.json"
        ),
        "official_tuner_sha256": file_sha256(prereg.OFFICIAL_TUNER_PATH_V29B),
        "installed_fused_moe_sha256": file_sha256(prereg.FUSED_MOE_PATH_V29B),
        "installed_fused_moe_config_sha256": file_sha256(
            prereg.FUSED_MOE_CONFIG_PATH_V29B
        ),
        "installed_vllm_envs_sha256": file_sha256(prereg.VLLM_ENVS_PATH_V29B),
        "pinned_ray_actor_sha256": file_sha256(prereg.RAY_ACTOR_PATH_V29B),
        "builtin_actor_ray_call_pid_semantics_verified": True,
        "dashboard_or_ray_state_api_required_for_actor_pid": False,
...
def _observe_all_four_gpus_v29b():
    import pynvml
    ...
    try:
        ...
        pynvml.nvmlShutdown()
    for row in rows:
        expected = prereg.GPU_IDENTITIES_V29B[row["physical_gpu_id"]]
        if (
            row["name"]
            ...
            raise RuntimeError("V29B exact physical-GPU identity changed")
    return {
        "gpus": rows,
...
def assert_all_four_gpus_idle_v29b():
    observation = _observe_all_four_gpus_v29b()
    if observation.get("all_four_idle") is not True:
        raise RuntimeError("V29B requires all four GPUs idle before claim")
    return _seal({
        "schema": "vllm-moe-fp8-tuning-prelaunch-idle-certificate-v29b",
        **observation,
    })


def wait_for_final_gpu_idle_v29b(*, timeout_seconds=30.0, interval_seconds=0.5):
    if timeout_seconds != 30.0 or interval_seconds != 0.5:
        raise RuntimeError("V29B final GPU cleanup polling contract changed")
    ...
    while True:
        ...
        if observation.get("all_four_idle") is True:
            return _seal({
                "schema": "vllm-moe-fp8-tuning-final-idle-certificate-v29b",
                **observation,
                "poll_count": polls,
        ...
        if remaining <= 0:
            raise RuntimeError("V29B final GPU cleanup exceeded 30 seconds")
        time.sleep(min(interval_seconds, remaining))


def _load_official_tuner_v29b():
    if file_sha256(prereg.OFFICIAL_TUNER_PATH_V29B) != prereg.OFFICIAL_TUNER_SHA256_V29B:
        raise RuntimeError("V29B official vLLM tuner identity changed")
    spec = importlib.util.spec_from_file_location(
        "vllm_benchmark_moe_fp8_v29b", prereg.OFFICIAL_TUNER_PATH_V29B
    )
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    if module.triton.__version__ != "3.6.0":
        raise RuntimeError("V29B Triton runtime version changed")
    return module


def _validate_config_v29b(config):
    if (
        not isinstance(config, dict)
        or set(config) != ALLOWED_CONFIG_KEYS_V29B
        or any(
            isinstance(item, bool) or not isinstance(item, int)
            for item in config.values()
        )
    ):
        raise RuntimeError("V29B selected official config geometry changed")
    return config


def validate_selected_table_v29b():
    directory = prereg.OUTPUT_DIRECTORY_V29B
    expected = directory / prereg.EXPECTED_OUTPUT_FILENAME_V29B
    ...
    if (
        ...
        raise RuntimeError("V29B official FP8 output filename or file count changed")
    ...
    if (
        ...
        != {str(batch) for batch in prereg.BATCH_SIZES_V29B}
    ):
        raise RuntimeError("V29B selected table batch coverage changed")
    configs = {
        str(batch): _validate_config_v29b(value[str(batch)])
        for batch in prereg.BATCH_SIZES_V29B
    }
    return {
        "path": str(expected),
...
def _probe_identity_matches_v29b(identity, gpu_id):
    expected = prereg.GPU_IDENTITIES_V29B[gpu_id]
    return (
...
def _official_actor_pids_v29b(ray, workers):
    ray.get([worker.__ray_ready__.remote() for worker in workers.values()])
    pid_refs = {
        gpu_id: worker.__ray_call__.remote(lambda self: os.getpid())
        for gpu_id, worker in workers.items()
    }
    resolved = ray.get([
        pid_refs[gpu_id] for gpu_id in prereg.PHYSICAL_GPU_IDS_V29B
    ])
    result = dict(zip(prereg.PHYSICAL_GPU_IDS_V29B, resolved))
    for pid in result.values():
        if isinstance(pid, bool) or not isinstance(pid, int) or pid <= 0:
            raise RuntimeError("V29B official actor PID identity changed")
    if len(set(result.values())) != 4:
        raise RuntimeError("V29B official workers do not have four distinct processes")
    return result


def _monitor_four_tuning_futures_v29b(ray, futures, actor_pids_by_gpu):
    ...
    while pending:
        ...
        if set(rows_by_gpu) != set(prereg.PHYSICAL_GPU_IDS_V29B):
            raise RuntimeError("V29B in-flight physical-GPU surface changed")
        if all(
    ...
    if simultaneous_observation_count < 1 or any(
        ...
        raise RuntimeError(
            "V29B did not observe an active process and positive utilization "
...
def run_official_tuner_v29b(output_directory):
    import ray
    import torch
    from ray.util.placement_group import placement_group, remove_placement_group
    from ray.util.scheduling_strategies import PlacementGroupSchedulingStrategy

    module = _load_official_tuner_v29b()
    if ray.is_initialized():
        raise RuntimeError("V29B requires a fresh Ray runtime")
    ray.init(num_gpus=4, include_dashboard=False)

    @ray.remote(num_gpus=1)
    class PlacementProbeV29B:
        def identity(self):
            import pynvml

            ids = ray.get_gpu_ids()
            if not isinstance(ids, list) or len(ids) != 1:
                raise RuntimeError("V29B probe Ray GPU allocation changed")
            physical = normalize_ray_gpu_id_v29b(ids[0])
            pynvml.nvmlInit()
            try:
    ...
    after_identities = {}
    try:
        for _gpu_id in prereg.PHYSICAL_GPU_IDS_V29B:
        ...
        probes = [
            PlacementProbeV29B.options(scheduling_strategy=strategy).remote()
        ...
        for probe, identity, strategy in zip(probes, discovered, strategies):
            ...
            if gpu_id in strategies_by_gpu or not _probe_identity_matches_v29b(
                identity, gpu_id
            ):
                raise RuntimeError("V29B placement probe identity changed")
            before_identities[gpu_id] = identity
            strategies_by_gpu[gpu_id] = strategy
            ray.kill(probe)
        probes = []
        if set(strategies_by_gpu) != set(prereg.PHYSICAL_GPU_IDS_V29B):
            raise RuntimeError("V29B exact four-GPU placement coverage changed")

        search_space = module.get_configs_compute_bound(False, [128, 128])
        if (
            len(search_space) != 1920
            or canonical_sha256(search_space) != prereg.SEARCH_SPACE_SHA256_V29B
        ):
            raise RuntimeError("V29B official FP8 search space changed")
        workers = {
            gpu_id: module.BenchmarkWorker.options(
                scheduling_strategy=strategies_by_gpu[gpu_id]
            ).remote(prereg.SELECTION_SEED_V29B)
            for gpu_id in prereg.PHYSICAL_GPU_IDS_V29B
        }
        actor_pids_by_gpu = _official_actor_pids_v29b(ray, workers)
        futures = [
            workers[gpu_id].tune.remote(
                prereg.BATCH_BY_GPU_V29B[gpu_id],
                256,
                1024,
                2048,
                8,
        ...
        utilization = _monitor_four_tuning_futures_v29b(
            ray, futures, actor_pids_by_gpu
        )
        selected = ray.get(futures)
        best_configs = {
            prereg.BATCH_BY_GPU_V29B[gpu_id]: module.sort_config(
                _validate_config_v29b(selected[gpu_id])
            )
            for gpu_id in prereg.PHYSICAL_GPU_IDS_V29B
        }
        module.save_configs(
        ...
        for gpu_id, probe, identity in zip(
            prereg.PHYSICAL_GPU_IDS_V29B, probes, rediscovered
        ):
            if not _probe_identity_matches_v29b(identity, gpu_id):
                raise RuntimeError("V29B post-tuning placement identity changed")
...
def validate_launch_arguments_v29b(args, implementation, recipe):
    if any(os.environ.get(key) for key in MOE_ENVIRONMENT_V29B):
        raise ValueError("V29B rejects external MoE tuning and config overrides")
    if args.v29b_dry_run and args.launch_v29b:
        raise ValueError("V29B dry-run and launch flags are mutually exclusive")
    if not args.v29b_dry_run and not args.launch_v29b:
        raise ValueError("V29B requires --v29b-dry-run or explicit --launch-v29b")
    if args.launch_v29b and os.environ.get("CUDA_VISIBLE_DEVICES") != "0,1,2,3":
        raise ValueError("V29B real launch requires CUDA_VISIBLE_DEVICES=0,1,2,3")
    expected = (
    ...
    for supplied, actual, label in expected:
        if args.launch_v29b and supplied is None:
            raise ValueError(f"V29B real launch requires expected {label} hash")
        if supplied is not None and supplied != actual:
            raise ValueError(f"V29B expected {label} hash changed")


def run_exact_v29b(preregistration, implementation, recipe):
    if (
        prereg.ATTEMPT_PATH_V29B.exists()
        or prereg.REPORT_PATH_V29B.parent.exists()
        or prereg.OUTPUT_DIRECTORY_V29B.exists()
    ):
        raise RuntimeError("V29B requires fresh exclusive output paths")
    environment = runtime_r2.certify_runtime_environment_r2()
    live_audit = live_cpu_disk_audit_v29b()
    prelaunch_idle = assert_all_four_gpus_idle_v29b()
    attempt = _seal({
        "schema": "vllm-moe-fp8-tuning-attempt-v29b",
        "status": "running",
    ...
    _assert_compact_v29b(attempt)
    _exclusive_write_json_v29b(prereg.ATTEMPT_PATH_V29B, attempt)
    final_idle = None
    try:
        prereg.OUTPUT_DIRECTORY_V29B.mkdir(parents=True, exist_ok=False)
        run_result = run_official_tuner_v29b(prereg.OUTPUT_DIRECTORY_V29B)
        final_idle = wait_for_final_gpu_idle_v29b()

        selected_table = validate_selected_table_v29b()
        if run_result["selected_configs"] != {
            int(key): value for key, value in selected_table["configs"].items()
        }:
            raise RuntimeError("V29B in-memory and persisted selected configs disagree")
        report = _seal({
            "schema": "vllm-moe-fp8-tuning-selection-report-v29b",
        ...
        _assert_compact_v29b(report)
        _exclusive_write_json_v29b(prereg.REPORT_PATH_V29B, report)
        completed_attempt = _seal({
            **_without_self(attempt),
            ...
            "report_binding": {
                "path": str(prereg.REPORT_PATH_V29B),
                "file_sha256": file_sha256(prereg.REPORT_PATH_V29B),
        ...
        _assert_compact_v29b(completed_attempt)
        _rewrite_json_v29b(prereg.ATTEMPT_PATH_V29B, completed_attempt)
        return report
        ...
        _assert_compact_v29b(failed_attempt)
        _rewrite_json_v29b(prereg.ATTEMPT_PATH_V29B, failed_attempt)
        raise failure


def _parser_v29b():
    parser = argparse.ArgumentParser()
    parser.add_argument("--v29b-dry-run", action="store_true")
    parser.add_argument("--launch-v29b", action="store_true")
...
def main(argv=None):
    args = _parser_v29b().parse_args(argv)
    preregistration = load_preregistration_v29b()
    implementation = implementation_identity_v29b()
    recipe = recipe_v29b(preregistration, implementation)
    validate_launch_arguments_v29b(args, implementation, recipe)
    if args.v29b_dry_run:
        value = _seal({
            "schema": "vllm-moe-fp8-tuning-dry-run-v29b",
            "preregistration_file_sha256": PREREG_FILE_SHA256_V29B,
            "preregistration_content_sha256": PREREG_CONTENT_SHA256_V29B,
            "implementation_bundle_sha256": implementation["bundle_sha256"],
            "recipe_content_sha256": recipe["content_sha256_before_self_field"],
            "expected_output_filename": prereg.EXPECTED_OUTPUT_FILENAME_V29B,
            "gpu_launched": False,
        ...
        return value
    return run_exact_v29b(preregistration, implementation, recipe)
